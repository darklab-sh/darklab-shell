...
- Entity tokens in saved and live transcripts open a compact menu with **Open in Atlas**, **Copy to Clipboard**, and **Insert into command**. Insert replaces the current selection or uses the caret in the visible desktop or mobile command box, then returns focus without running the command. Normal text selection and the browser's right-click menu remain available, and the entity menu can also be opened and navigated from the keyboard.
