...
- Nuclei recommendations use reviewed safe, standard, or intrusive template profiles selected by the saved check policy. Their confirmation lists the included exposure, misconfiguration, CVE, technology, network-service, TLS, API, headless, or low-aggression DAST families, shows the exploit, callback, authentication, brute-force, denial-of-service, code, and local exclusions that apply, and identifies the installed template manifest by its recorded release, SHA-256 revision, and entry count. Safe, standard, and intrusive runs satisfy only their matching saved check. A missing or changed manifest stops the launch until the operator explicitly updates templates and reviews a fresh plan. Simply opening the Assessment never updates templates. Intrusive headless and DAST checks stay behind the deployment gate, one-target bounds, launch-time revalidation, and a fresh warning confirmation.
