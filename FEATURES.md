...
- The Overview tab gives each project a target-first attack-surface summary. It rolls up target count, open ports, services, high-signal findings, certificate status, provider highlights, and recent-change state, then lets you jump into the existing Entities or Findings tabs with the selected target filter already applied.
