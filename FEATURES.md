...
**Related files:** `app/services/runs/comparison.py`, `app/services/runs/comparison_findings.py`, and `app/services/runs/comparison_derived.py` (shared comparison, persisted-finding, and detected-change helpers), `app/services/history/comparison_queries.py` and `app/blueprints/history_compare_requests.py` (owner-scoped loading and request resolution), `app/services/projects/comparisons.py` (project pair resolution), `app/services/history/comparison_logging.py` (bounded comparison lifecycle summaries), `app/blueprints/history.py` (history compare routes), `app/static/js/features/run-comparison/` (compare launcher, controls, navigation, and renderer), and `app/static/css/features/run-comparison.css` (desktop/mobile compare layout).
...
- `darklab-deploy` checks release-owned file drift, creates and verifies SQLite or Postgres backups through one-off release-image containers, restores managed backups, upgrades only to a verified newer exact release, explains clone-to-managed migration, and removes managed files without deleting operator state.

**Limits:** Docker Compose 2.20.0 or newer on Linux is the supported runtime. Production reads a private snapshot of `conf/` at container start, so host-side overlay edits need `docker compose restart shell`. Tour chapters and the curated wordlist map are image-owned rather than operator overlays. Database migrations can be forward-only, so the lifecycle command refuses downgrades and takes a verified backup before upgrades and restores.
...
- Repository-free installs expose the same engine through `./darklab-deploy backup`; the release image supplies Python and Postgres client tools, while the installed command supplies exact mounts, operator files, and managed release metadata. `./darklab-deploy restore` verifies checksums, takes a safety backup, restores SQLite or Postgres state plus local config and durable workspaces, preserves the installed image selection, and restarts the app.
...
- **Opt-in raw-packet scans.** Linux Docker operators can enable capability-backed Nmap, Naabu, and Masscan modes without Docker privileged mode or root scanner processes. Runtime checks keep the feature inactive when `CAP_NET_RAW`, effective/permitted scanner file capabilities, or the executable capability policy is unavailable. Nmap and Naabu retain connect-mode fallbacks. With restricted CIDRs, raw Nmap also requires a matching root-owned firewall marker, while packet-socket Naabu and Masscan modes stay unavailable even when separate host or bridge firewall rules exist. Nmap's raw-only autocomplete choices, including SYN and UDP scans, OS detection, traceroute, and raw host discovery, appear only when Nmap readiness is active; spoofed-source and link-layer bypass options remain blocked.
