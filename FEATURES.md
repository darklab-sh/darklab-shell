...
- Nuclei recommendations use reviewed safe or standard template profiles selected by the saved check policy. Their confirmation lists the included exposure, misconfiguration, CVE, technology, network-service, TLS, or API families, shows the intrusive and code/local exclusions, and makes clear that template updates are a separate action. Intrusive headless and DAST checks stay behind the deployment gate and a fresh confirmation.
