...
- Project assessment routes let API clients discover available cycle profiles, create and review cycles, page and filter target-specific checks, record reasoned manual exclusions, attach compatible saved evidence, compile and page a short-lived bounded batch preview, preview or start a check's current recommended action, preview or start a finding's saved verification action, and complete, archive, preview, or delete a cycle through the same Project and team-permission boundaries as the browser. Project probe and batch-preview schemas describe their complete selection, target, plan, bounds, availability, paging, and launch payloads for generated clients.
