...
- The terminal's `probe list`, `probe plan`, and `probe run` commands expose reviewed, bounded actions without requiring an Assessment cycle. The catalog gives unavailable-profile reasons, recommendation rationales, per-action exclusions, and the cycle-only actions excluded from probes. A plan is anchored to one confirmed Project target and shows the exact command, policy, limits, credential use, expected evidence, availability, missing feature gates, approval digest, and whether the current Team role may launch it. After an action is selected, autocomplete limits target suggestions to confirmed active-Project targets with compatible types. Web commands use bracketed URL hosts for saved IPv6 targets. Team viewers can inspect anonymous plans; a role without run permission sees the plan and a clear permission message but isn't asked to confirm an action it can't start. Launching stays in the originating terminal tab, keeps the `probe run` invocation in Recent commands, links the resulting ordinary run to the requested Project even when automatic capture is off, and records normal History and structured evidence. Protected Curl, HTTPx, Katana, Dalfox, and detection-only SQLmap probes may select an enabled Project HTTP profile by its saved name or stable id. Their plans identify whether the current role also has Secret-management permission; autocomplete labels profiles by name and inserts the stable id. The preview shows the canonical id, name, role, and saved allowed-host, root, include-path, and exclude-path boundary, while private values are added only at launch and never appear in the plan or saved command. Nuclei profiles remain anonymous because generated template requests can't safely inherit that exact saved path boundary. The browser, API v1, and `darklab probe` CLI share the same preview, digest, scope, and launch checks. A probe doesn't allocate ZAP or OAST work and doesn't directly change a cycle.
...
- Curl, HTTPx, Katana, Nuclei, Dalfox, and detection-only SQLmap actions ask which available role to use before the final preview. Disabled or incomplete profiles remain visible for repair but can't be selected.
