...
- On an active **Assessment** cycle, an assessor can mark a check blocked, intentionally skipped, or not applicable with a required reason. The same desktop and mobile action can restore the state calculated from saved evidence later. Completed and archived cycles stay read-only, and view-only team members can review the decision without changing it.
