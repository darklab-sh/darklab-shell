...
- Scripts can start non-interactive runs, wait for final run status, stream broker events as SSE or NDJSON, cancel active personal/team-scope runs, read history/ranged output/artifacts, search saved output with line context, inspect Atlas entities and findings, resolve an exact saved hostname, IP address, or URL without paging through Atlas, download run artifacts by stable artifact id, inspect project data, manage scheduled commands and outbound notification channels, read notification delivery audits, and link or unlink completed external runs from active projects.
...
- `intel cve <CVE-ID>` shows stored FIRST EPSS probability and percentile plus CISA KEV status before querying NVD and Vulners for severity, score, dates, summary, references, exploit counts, and exploitability context. The stored public signals include their source version, publication/fetch time, and freshness; a CISA BOD 22-01 date is labeled as federal directive context rather than the Project's remediation deadline. EPSS is presented as an exploitation estimate, not as a complete risk score.
...
- Evidence packages are draft project manifests built from selected project material.
  ...
  - Packages containing CVE findings snapshot the exact EPSS/KEV rows, source versions, dates, checksums, attribution, and non-endorsement notice used at build time, so a later feed refresh doesn't rewrite what the package reported.
- The Report tab turns selected project material into a narrative engagement report.
  ...
  - CVE findings show the KEV/EPSS reasons behind their placement. Reports and report manifests pin the public source versions, dates, checksums, attribution, and non-endorsement notice used for that export.
...
- The Monitoring tab gives project users a project-scoped view of scheduled watchers that belong to the project, with a **New monitor** action that opens the watcher form already linked to the current project.
  - Public feed changes appear in a separate **CVE Risk Changes** section when an open Project finding becomes KEV-listed or crosses the configured EPSS activation threshold. One acknowledgement applies to the owner-scoped risk event everywhere that event is projected, while each linked Project can choose its own digest delivery setting.
