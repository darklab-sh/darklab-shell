...
- **Run assessment plan** builds a read-only preview for an active cycle before anything starts. Choose targets and categories, keep safe commands selected by default, optionally include standard checks through a separate confirmation, and review the bounds, potential coverage, and estimated completion window. Advanced command and concurrency choices stop at the deployment's configured ceilings, so the planner doesn't offer a value the server will reject. Once the preview is ready, one sticky decision bar names its current state and keeps the valid next action—**Run assessment plan**, **Update templates**, or **Refresh preview**—within reach. **Run assessment plan** remains visible but explains why it is disabled when the preview needs attention. Exact commands and exclusions start as count-labeled disclosures, keeping the main decision clear while preserving all review detail. Building or refreshing the preview, changing the standard-check choice or a batch limit, and updating templates keep the planner at its current scroll position, so you can see the loading message or control where you started the action. Plans that contain Nuclei work add one preflight summary with the installed template release, digest, refresh age, validation state, and command count. A stale but valid snapshot can be updated and rebuilt or kept deliberately; a missing, invalid, unreadable, or incompatible snapshot blocks launch and explains the operator action it needs. A successful update immediately shows the rebuilt, launchable preview; if the updated cache still fails validation, the plan shows that failure instead of enabling a misleading success state. An unstarted plan stays available when the Project workspace is closed and reopened in the same browser page. View-only members can inspect the same plan but can't update templates, start work, or cancel it. After approval, the durable **Assessment batch** monitor survives reloads, follows each command, opens child runs in History, and keeps completed evidence when a command fails or the batch is canceled. Its progress polling updates the monitor in place, so reading elsewhere in the Assessment tab doesn't lose its scroll position or focused control. Status Monitor shows the same active plan progress and its currently launching or running commands, with **View batch** opening the exact Project and cycle. Each child identifies its batch and item in History, Run Details, Recent evidence, Project comparisons, reports, and evidence packages, and **View batch** returns to the exact monitor even when it isn't on the first batch page. Channels subscribed to **Run complete** receive one final batch summary with its outcome counts and link; child commands don't each send a notification. A terminal batch with failed, unavailable, interrupted, or unlaunched work can build a current retry preview. Starting it creates a new linked batch and leaves every successful source command, run, and evidence record unchanged. Repeated Nuclei template-loading failures appear as one diagnosis with an update-and-retry path that rebuilds the retry preview for review before another batch starts.
