...
- Restricted browser sessions expire after both an idle limit and an absolute limit. **Remove from browser** signs out that browser; the session-wide route and operator command can close every browser session for the principal. Revoking or rotating a portable credential also closes sessions that came from it. Open streams disconnect within 15 seconds of losing access, and a controlling interactive PTY stops. Ordinary commands already running can still finish and be viewed through another valid credential.
