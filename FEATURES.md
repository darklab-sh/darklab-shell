...
- The maintained blind-XSS check uses one saved query parameter and a private OAST connector. API clients can review its redacted command without contacting the service, confirm that exact plan to reserve a short-lived callback, recover recent reservation state, and poll the exact reservation until the private worker has made it ready. A separate ready-only confirmation starts the Project-linked run and binds that exact callback without putting it in the public command, response, logs, or audit details. The ordinary recommended-action route can't launch that plan.
