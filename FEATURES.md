...
- The active token is stored in `localStorage` under `session_token`; the original UUID is always preserved under `session_id` so the legacy Options **Clear** action has a stable fallback.
...
- The terminal exposes the replacement `credential` command rather than accepting session-token values. Safe status and list operations render metadata; create, use, expiry, rotation, revocation, and recovery actions open Options so reusable values don't enter `/runs`, prompt history, recents, or saved transcripts.
...
- `credential` or `credential status` shows the current principal, personal workspace, and authentication method without showing a reusable value.
- `credential list` shows safe credential ids, types, labels, expiry, and revocation state.
- `credential create`, `credential use`, `credential expiry`, `credential rotate`, `credential revoke`, and `credential recover` open Options for the matching access task. Supplying an extra value is rejected and the submitted value is removed from browser command history before anything is persisted.
...
If a legacy browser session has run history, workspace files, Project records or assessments, user workflows, completed personal workflow executions, or recent targets, its Options migration controls still use the shared modal confirmation primitive. Credential commands never accept a reusable value in terminal text.
...
- Principals with durable access can create teams, join with invite or recovery codes, manage members, and switch between personal and team scope from Options → Teams, the desktop HUD scope menu, the mobile scope selector, `/api/v1`, the terminal `team` built-in, or the bundled `darklab team ...` CLI commands. The browser still reaches this principal boundary through its temporary session-token adapter until the Access UI replaces those controls.
