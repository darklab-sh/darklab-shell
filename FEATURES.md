...
- Open **Quick Lookup** beside Atlas in the desktop rail or mobile menu, or press `Alt+Q` / `Option+Q`, to find the exact saved profile for one hostname, IP address, or HTTP(S) URL. It searches the current personal, team, or project scope without paging through Atlas, including suppressed or source-less records that belong to that scope. Looking up a value reads only evidence and cached Intel already saved by the app; it doesn't run a command, create an entity, or contact an Intel provider, and **Refresh intel** remains a separate, explicit action.
