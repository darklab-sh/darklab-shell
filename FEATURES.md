...
- The same server pass also attaches structured entity metadata to external command output lines when it sees public IPs, hostnames, hashes, or CVE IDs. Generic hostname extraction uses an offline Public Suffix List check so dotted code snippets like `classlist.add` or `document.queryselector` do not become domains. Full URLs, scanner-specific rows, imports, and explicit project targets use stronger parser paths and keep their current behavior. `dig` and `nslookup` use their DNS records instead: Atlas keeps positive addresses, aliases, reverse-lookup names, and same-domain mail, nameserver, or service targets. Wildcard answers attach to the parent domain, and CAA answers keep the queried owner without adding certificate-authority references. Atlas doesn't add the resolver, rejected provider targets or their addresses, TXT references, DNSSEC data, trace machinery, malformed owners, or a name that returned no data. WHOIS keeps the queried public IP address or domain as an Atlas entity only when the response includes its registration record. Failed lookups, rate limits, and no-match replies add no target; registration ranges, registry and registrar infrastructure, nameservers, contact handles, referral links, and policy URLs remain part of the saved response instead of becoming investigation targets. The complete DNS and WHOIS output still stays in History and remains available to findings, search, comparison, and export. That metadata is kept with live streams, restored history, saved full-output artifacts, and the Session Entity Atlas without re-parsing transcripts.
...
- **Options → Access** shows whether this workspace is linked to the provider. A recent credential sign-in and a fresh provider sign-in are both required to link it. If your credential sign-in is too old, **Sign in again with a credential** takes you through sign-in and returns you to Access. Unlinking requires a usable portable credential and signs out every browser session for the workspace. Provider-only users can ask an operator for recovery access if their provider identity changes.
