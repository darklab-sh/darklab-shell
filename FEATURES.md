...
- `/api/v1` accepts scoped personal access tokens through `Authorization: Bearer`. It rejects browser portable credentials, anonymous IDs, legacy `tok_` values, and `X-Session-ID` rather than silently changing identity.
