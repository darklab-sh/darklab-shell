...
- Nuclei recommendations use reviewed safe or standard template profiles selected by the saved check policy. Their confirmation lists the included exposure, misconfiguration, CVE, technology, network-service, TLS, or API families, shows the intrusive and code/local exclusions, and identifies the installed template manifest by its recorded release, SHA-256 revision, and entry count. A missing or changed manifest stops the launch until the operator explicitly updates templates and reviews a fresh plan. Simply opening the Assessment never updates templates. Intrusive headless and DAST checks stay behind the deployment gate and a fresh confirmation.
