...
- Value slots marked as domains, hosts, targets, IPs, URLs, or port sets capture up to 10 recent targets per kind for the active personal workspace. They show back up only in compatible autocomplete slots, and URLs are saved without query strings or fragments. Recents persist across browser restarts and devices after the workspace is kept.
...
**Configuration:** timestamp and line-number preferences are off by default and follow the active personal workspace. They are mirrored locally for reload continuity, and a kept workspace restores the same option set across browsers and devices. Command outcome summaries are on by default and can be disabled from Options or with `config set command-outcome-summaries off`.
...
**Purpose:** a persistent desktop status row that brings together run state, connection health, workspace access, and environment telemetry without displacing the terminal.
...
- Atlas recognizes public IPs, hostnames, hashes, and CVE IDs in external command output:
  - **Generic:** Bare hostnames use an offline Public Suffix List check, so dotted code snippets like `classlist.add` or `document.queryselector` don't become domains. Full URLs, scanner-specific rows, imports, and explicit Project targets are recognized separately and keep their current behavior.
  - **DNS:** For `dig` and `nslookup`, Atlas keeps positive addresses, aliases, reverse-lookup names, and same-domain mail, nameserver, or service targets. Wildcard answers attach to the parent domain, and CAA answers keep the queried owner without adding certificate-authority references. Atlas doesn't add the resolver, rejected provider targets or their addresses, TXT references, DNSSEC data, trace machinery, malformed owners, or a name that returned no data.
  - **WHOIS:** Atlas keeps the queried public IP address or domain only when the response includes its registration record. Failed lookups, rate limits, and no-match replies add no target. Registration ranges, registry and registrar infrastructure, nameservers, contact handles, referral links, and policy URLs stay in the saved response instead of becoming investigation targets.

  Complete DNS and WHOIS output stays in History and remains available to findings, search, comparison, and export. Recognized entities stay with live streams, restored History, saved full-output artifacts, and the Session Entity Atlas without needing to analyze the transcript again.
...
- Successful reviewed Nmap runs also show their saved informational service facts in Run Details. The same target, service, script, structured values, versions, observation time, and safety-limit notice appear with matching Assessment checks on desktop and mobile; free-form NSE output stays out, and viewing the facts doesn't change a finding or check.
...
- Open **Atlas** from the desktop rail, mobile menu, `Alt+A`, History row actions, Run Details, Project Overview, or a project filtered view. Choosing **Open in Atlas** from a transcript entity menu, or following a direct entity link from Run Details, Projects, or Project Overview, opens the full profile immediately.
...
- The Atlas toolbar can import external triage files from Nuclei JSONL, Nessus XML, Greenbone XML, OWASP ZAP JSON/XML, Burp Suite XML, SARIF 2.1 JSON, CycloneDX JSON, Generic CSV, and Generic JSONL. Imports show a preview with counts, samples, and row warnings before anything is written. Greenbone imports map hosts, CVEs, and NVT findings into the normal review flow without connecting to or managing a scanner. CycloneDX previews separate components, dependency edges, vulnerability assertions, and VEX dispositions. Nessus previews can separately retain exact versioned service CPEs with their host, port, service, scan time, and parser details. You can keep those evidence records with the import batch without turning inventory into a finding. Applying an import writes a high-level audit row with the source tool, format, selected options, project id, row/count summary, and created/updated counts without storing imported row bodies in the audit trail. From a project-filtered Atlas view, you can also link imported entities to that project or create project targets from imported domains, IPs, and URLs.
...
- Saved views remember useful Atlas filter sets for the active personal workspace, including search text, source run, orphan filter, suppression filter, Findings review state, and project scope. Applying a saved view keeps you on the current Atlas tab, and **Clear filters** returns the visible filter controls, project scope, source-run scope, and saved-view picker to their defaults.
...
- Entity tokens in saved and live transcripts open a compact menu with **Open in Atlas**, **Copy to Clipboard**, and **Insert into command**. Insert replaces the current selection or uses the caret in the visible desktop or mobile command box, then returns focus without running the command. The menu stays open as live output scrolls, so you can finish choosing an action. Normal text selection and the browser's right-click menu remain available, and the entity menu can also be opened and navigated from the keyboard.
...
- **Clean Atlas** removes a source run's Atlas links while keeping its History transcript. Use the per-run **Clean from Atlas** action to remove older noisy DNS links after parser behavior changes; it doesn't rewrite or delete the saved output. The shared [Cleanup Confirmations](#cleanup-confirmations) rules explain what else can be removed and what stays protected.
...
- Supported formats are Nuclei JSONL, Nessus XML, Greenbone XML, OWASP ZAP JSON/XML, Burp Suite XML, SARIF 2.1 JSON, CycloneDX JSON, Generic CSV, and Generic JSONL.
- Every supported format can be uploaded directly, as gzip, or in a ZIP containing exactly one report. Compressed uploads have separate upload and expanded-size limits; encrypted, nested, multi-report, and unsafe-path archives are rejected before preview.
...
- Nessus imports preserve exact versioned service CPEs as separate evidence with the affected host, port, protocol, service, plugin id, scanner or report-format version, parser version, and available source scan time. Timezone-qualified values are normalized, while older timezone-free values stay visible exactly as the report supplied them. Wildcard or malformed product versions don't become correlation evidence, and a service-version row isn't treated as proof of a vulnerability. Local NVD correlation re-reads the exact owner-scoped applied evidence row before it can save an inferred finding, so edited candidate data or an unrelated batch fails closed.
- Greenbone imports read native GMP XML result reports and preserve each NVT's OID, family, affected host and port, CVE references, severity, quality of detection, scanner version, timestamps, safe web references, remediation, and result UUID. The NVT OID remains the stable scanner-rule identity, so another export of the same severity for the same host and NVT reuses the finding while retaining each result UUID as occurrence provenance. Greenbone stays operator-managed; preview and apply don't submit a scan, read a feed, or contact the service.
- SARIF 2.1 imports preserve bounded tool and automation identity, rule details, stable fingerprints, source regions, and safe web or repository-relative artifact locations. Local file URIs, traversal paths, credential-bearing URLs, backslashes, and invalid artifact references stay out of evidence and appear as preview warnings; the app never fetches referenced source files.
- CycloneDX imports preserve bounded SBOM provenance, nested components, dependency edges, exact PURL/CPE identifiers, vulnerability ratings, safe references, affected components, and VEX analysis. Inventory rows aren't findings. Affected or exploitable assertions can become imported findings, while not-affected, resolved, and under-investigation assertions remain evidence and never change existing triage or verification state.
- Preview always runs before apply. It shows parsed row counts, new/duplicate/update counts, sample entities, findings, and evidence, row warnings, and which apply options are available for the current personal or team scope.
...
- Dedicated report adapters preserve clear remediation fields as finding triage guidance. Nessus and Greenbone `solution`, ZAP `solution`, and Burp Suite `remediationDetail` text become finding remediation when the imported finding is applied, while existing operator-edited remediation is kept if a later import disagrees.
...
- Team imports use the active team scope. Importing findings requires triage permission, while importing typed evidence or creating entities, project links, or project targets requires project-mutation permission. If a finding references a new normalizable entity, applying that finding needs both permissions.
...
**Purpose:** multi-tab workspace with personal or team run history, full-text search over commands and output, starring, and reload-safe reconnection to in-flight runs.
...
- If the page reloads mid-run, the shell restores a running placeholder tab with the kill action available and subscribes back to the brokered `/runs/<run_id>/stream` for replay plus live output when events are still retained. Active-run recovery is client-aware: another browser with access to the same workspace can see the live run in Status Monitor without automatically creating a terminal tab or taking over the stream. Scheduled and watcher-fired runs also stay in Status Monitor until you explicitly attach to them. Non-running tabs restore separately from `sessionStorage` with labels, transcript previews, statuses, and draft input preserved; restored completed tabs remount a live prompt immediately.
...
- Clicking a step pre-fills the prompt with its `cmd`, and each step can also be run directly. `Run all` keeps legacy workflows in the active tab and starts a durable server execution for explicit v2 and v3 playbooks.
- The **Executions** tab shows recent durable playbooks in the active personal or team scope, including their current step, elapsed time, branch outcome, capture names, and linked runs. Collection steps also show finished, pending, active, succeeded, failed, and skipped child counts plus a short bounded failure-code sample without revealing captured values. Active runs can be attached to a terminal tab, completed runs open in Run Details, and cancellation asks for confirmation before stopping the current step.
...
- The **New Workflow** editor saves personal workflows by default, or shared team workflows when a team scope is active. Saved workflows can be edited or deleted from their detail view; deleting one keeps the workspace open, selects the next definition, and removes the deleted item from the desktop rail immediately. The Parameters section supports text, target, domain, host, URL, port, port-set, Files-path, and wordlist values with labels, defaults, placeholders, help, and required state. Steps have stable IDs, **After success** and **After failure** routes, and repeatable exact-exit-code routes that take priority over those defaults. Steps can be reordered or renamed without losing their routes; a deleted destination stays visibly marked until the route is changed or removed. Each step can capture a single value or a collection from a first line, matching line, structured entity, or JSON Pointer. Collection captures have a visible item limit, and a later step can select that named collection, use it in the command template, and set its failure, retry, parallel-run, and failure-limit policy. Invalid parameters, steps, transitions, exit codes, captures, and collection limits are called out beside the matching field.
- Version 3 playbooks can capture a bounded list from one step and run the next step once per captured item. The saved definition chooses how many children can run together, whether failures stop the batch or allow the remaining items to continue, and whether an ordinary child failure may be retried. Each child is still a normal scoped History run. Execution responses show counts and outcomes without returning the captured list.
- The terminal-native `workflow` command supports `list`, `show`, and `run`; missing required variables are prompted transcript-style. Sensitive values use masked fields and non-echoing prompts, and must be omitted from inline flags so they don't appear in the command transcript. Starting an execution still stores all supplied values, including sensitive inputs, in the owner-scoped execution record and database backups, so credentials that shouldn't become workflow history belong in app-managed Secrets. Sensitive inputs appear as `[redacted]` in active-run summaries, History command text, Run Details, logs, metrics, and notifications, while values captured from an earlier step appear as named placeholders. Execution status responses contain progress and linked-run state, not stored inputs, captured values, definition snapshots, access credentials, or browser ownership hints. This boundary covers app-managed metadata; a command that prints a value can still put it in its own saved output. A terminal-started playbook returns its durable execution id and points to `workflow status` instead of leaving an initial-step snapshot that can go stale. Playbooks started with **Run all** keep progress in the Workflows panel without adding partial status to the active terminal. `workflow runs`, `workflow status <execution-id>`, and `workflow cancel <execution-id>` expose durable execution state.
- Each step can show a short `note` explaining what the command checks.
- Personal workflows belong to the active personal workspace and remain available through any of its valid credentials. Shared team workflows stay in that team scope; owners and admins can create, edit, and delete them, while other team members can use them when running commands.
- Built-in workflows cover DNS troubleshooting, TLS/HTTPS checks, HTTP triage, quick reachability, email server checks, passive domain recon, subdomain enumeration and validation, bounded subdomain assessment, live web review, port service review, web directory discovery, SSL/TLS deep dives, CDN/edge behavior checks, API recon, network path analysis, fast port/service triage, and Files-backed chained recon such as subdomain HTTP triage, Historical Web Surface Triage, and crawl-and-scan.
- **Bounded Subdomain Assessment** collects up to 16 unique subdomains directly from structured output, then resolves, probes, shallow-crawls, and checks each candidate with the maintained safe Nuclei exposure profile. It doesn't need Files for orchestration. Controlled parallelism, per-command time and rate limits, and continue-and-summarize behavior keep partial failures visible without exposing the captured host list in workflow status.
- **Live Web Review** captures one approved live URL as verified screenshot evidence, then saves a bounded structured parameter inventory without sending XSS payloads. It uses the Chromium included with darklab_shell instead of downloading a browser at run time. Files holds only the screenshot image; the parameter results stay with the normal run output and Project evidence flow. Fixed workers, rates, targets, retries, and timeouts keep both steps focused on the URL you chose.
- **Port Service Review** fingerprints one selected TCP port, then asks Nmap to run only its reviewed default scripts that match the detected service. It uses connect mode and fixed retry, script, and host time limits. The resulting Project service evidence can offer deeper reviewed Assessment actions for supported protocols; the playbook itself doesn't guess credentials or choose intrusive scripts.
...
- Custom workflows can be added to `conf/workflows.yaml`; the file is re-read on every request so edits take effect without a restart. Invalid durable entries, unsupported explicit versions, and malformed YAML are rejected instead of exposing a partial playbook.
...
**Limits:** every step still runs through the normal command policy and runtime readiness checks. The server allows three active executions per personal or team owner by default and stops an execution after four hours. Principal state, Team membership, and the required run permission are checked again before each step. Workflow, step, parameter, and capture ids start with a lowercase letter and then use lowercase letters, numbers, and underscores. Saved personal/team definitions allow up to 24 parameters and 40 steps; titles are capped at 120 characters, descriptions and step notes at 1,000, commands at 1,200, and supplied values at 4,096. A step can define up to eight bounded scalar or collection captures. One collection keeps at most 32 unique items, one fan-out runs at most eight children together, and retries stop after three attempts beyond the first run.

**Configuration:** `conf/workflows.yaml` accepts legacy linear entries, scalar `version: 2` playbooks, and collection-enabled `version: 3` playbooks. User-created workflows store the same shapes in the active personal or team scope. `workflow_active_execution_limit` and `workflow_execution_max_runtime_seconds` control the server-side execution bounds. See [Workflow Playbooks](docs/workflows.md) for the complete YAML, parameter, transition, capture, fan-out, terminal, and recovery reference.
...
- Revoking a credential does not move or delete principal-owned schedules. The Access warning identifies future work attributed to that credential and can pause eligible definitions. Disabling the principal remains the complete stop. When a team is archived, team-owned schedules pause in place instead of moving into a member's personal scope. Reactivating the team keeps those schedules paused until someone resumes them.

**Limits:** schedules require a kept workspace. Anonymous workspaces cannot create schedules because there is no durable principal for the worker to check after the browser closes. Cron support is strict five-field POSIX cron, and custom cron expressions cannot run more often than every five minutes. Workflow scheduling, blackout calendars, and per-target schedules are out of scope.
...
**Limits:** watchers require a kept workspace. Anonymous workspaces cannot create watchers because the scheduler needs a durable principal. First-run watchers require a command because there is no completed run to inherit from yet. Watchers monitor one baseline command at a time, use the same five-minute minimum custom cron interval as schedules, and keep bounded diff summaries rather than unlimited raw diff payloads.
...
- **Options with the keyboard open** — credential and other text fields stay readable without focus zoom. The sheet fits above the keyboard, scrolls the focused field into view, and still closes when you tap its backdrop.
...
- `/api/v1` accepts scoped personal access tokens through `Authorization: Bearer`. Browser credentials and anonymous browser identities aren't API credentials, so the API rejects them rather than silently changing identity.
...
- Scripts can start non-interactive runs, wait for final run status, stream broker events as SSE or NDJSON, cancel active personal/team-scope runs, read history/ranged output/artifacts, search saved output with line context, inspect Atlas entities and findings, resolve an exact saved hostname, IP address, or URL without paging through Atlas, read stored EPSS/KEV feed health without starting a refresh, explicitly look up one exact package version in OSV, download run artifacts by stable artifact id, inspect project data and assessment cycles, create and edit assessor findings, manage their typed supporting evidence, manage Project HTTP profiles, manage scheduled commands and outbound notification channels, read notification delivery audits, and link or unlink completed external runs from active projects.
...
- Saved schedules fire through the same brokered run path as manual runs and keep the personal or team scope they were created in. History rows and Run Details mark scheduled runs with their schedule id, and disabled principals, archived teams, lost permissions, or overlapping prior runs leave clear fire-audit rows instead of failing silently.
...
- Project assessment routes let API clients discover available cycle profiles, create and review cycles, page and filter target-specific checks, record reasoned manual exclusions, attach compatible saved evidence, compile and page a short-lived bounded batch preview, list and inspect durable batches, follow their item attempts and sanitized events, preview or start a check's current recommended action, preview or start a finding's saved verification action, and complete, archive, preview, or delete a cycle through the same Project and team-permission boundaries as the browser. Project probe and assessment-batch schemas describe their complete selection, target, plan, bounds, availability, progress, paging, and launch payloads for generated clients.
- The bundled `darklab` CLI wraps the API with `whoami`, `credential`, `run`, `active`, `tail`, `cancel`, `history`, `grep`, `show`, `output`, `artifacts`, `evidence`, `finding`, `http-profile`, `risk`, `advisory`, `atlas`, `projects`, `project`, `project-findings`, `project-runs`, `project-entities`, `project-packages`, `assessment`, `team`, `schedule`, `notify`, `completion`, and `download` commands. `darklab credential status`, `list`, and `revoke` work only with the current PAT and show no reusable value. `darklab http-profile list`, `create`, `show`, `update`, and `delete` accept an active Project slug or stable id. Create and update read a bounded JSON object from a file or stdin, accept Secret names and workspace Files paths instead of credential values, and keep the update revision in an explicit flag. List, show, and the read-only deletion preview show safe profile summaries and reference counts. Protected reference names and Files paths appear in JSON only when the caller already has permission to manage them; Secret values are never returned, and deletion requires `--confirm`. `darklab finding create` and `edit` read a bounded JSON object from a file or stdin, resolve a Project slug or id, and keep duplicate overrides and edit revisions explicit. `darklab evidence list`, `link`, and `unlink` manage typed supporting references without copying or deleting the source; duplicate links are harmless no-ops, and exact run-line evidence uses zero-based line numbers. `darklab evidence services ...` pages through the same bounded typed Nmap service facts shown in Run Details, with readable tables and stable JSON or NDJSON for scripts. `darklab risk status` shows whether the stored EPSS and KEV sources are current, stale, failed, or unavailable, including their origin and live-refresh setting, without contacting a provider. `darklab advisory osv ...` explicitly sends one exact PURL and version to OSV; it's the only CLI command that can start that provider request, and it explains permission, disabled-mode, and provider failures separately. `darklab assessment ...` accepts an active Project slug or stable id, creates cycles from maintained profiles, lists and inspects them, filters their checks, records or clears reasoned manual decisions, and completes or archives them through the same forward-only lifecycle as the browser. Deletion always shows the assessment-owned rows that will be removed, confirms that source records stay in place, and remains read-only until `--confirm`; a running batch produces an actionable wait-and-retry message instead of a generic conflict. Assessment actions also preview a check's current recommendation and start it only after explicit confirmation. Its `batch` commands can preview or start a safe-by-default assessment plan, list and inspect durable batches, follow sanitized progress from a resumable cursor, preview or confirm cancellation, and build a fresh linked batch for failed or unfinished work. Retry stays read-only until `--confirm`, never selects successful source commands, and separately acknowledges standard work. The CLI leaves planning, scope, policy, deduplication, and recovery with the server. Individual actions can select the same protected HTTP profile, reviewed Dalfox observation, or saved Schemathesis schema used by the browser. `darklab run --wait` waits for final status for shell scripts, `darklab active` lists current jobs for the principal, `darklab run --link-project NAME` resolves friendly project names before linking completed external runs, `darklab history --type` filters run history by external commands or built-ins, `darklab grep <pattern>` searches saved output across runs, `darklab atlas ...` reads Atlas summary, source runs, entities, and findings, `darklab team ...` creates teams, manages members and invites, joins scopes, and saves the active CLI team, `darklab schedule ...` manages saved recurring commands, `darklab notify ...` manages notification channels without accepting plaintext secrets on the command line, `darklab completion bash|zsh|fish` prints static shell completion, and `darklab completion install --shell auto` installs it into the current user's shell-completion directory. Live tailing fails clearly if a stream closes before the run reaches an exit, killed, or error event, and Ctrl+C while following output detaches without cancelling the server-side run.
...
- CLI configuration uses flags first, then `DARKLAB_API_URL`, `DARKLAB_PAT`, `DARKLAB_TEAM`, and `DARKLAB_TIMEOUT`, then the TOML file at `~/.config/darklab/config.toml`. CLI writes keep that file at `0600` because it can store a PAT.

**Limits:** API v1 is intentionally non-interactive. Its Project writes are limited to the documented Assessment, manual-finding, finding-evidence, HTTP-profile, and completed-run link contracts rather than exposing general Project mutation routes. It also does not expose Interactive PTY start/input/resize routes, workflow execution, browser credential issuance, or workspace ZIP downloads.

**Configuration:** no server-side API-specific settings. CLI users can set `DARKLAB_API_URL`, `DARKLAB_PAT`, `DARKLAB_TEAM`, `DARKLAB_TIMEOUT`, or `~/.config/darklab/config.toml`; see [docs/api.md](docs/api.md).
...
- External non-PTY run finalization queues a `run_complete` notification with the configured `app_name`, run id, command root, exit code, safe access hint, and summary counts. Built-in commands and PTY sessions do not send `run_complete` by default.
...
- Personal variables belong to the active workspace and remain available when its credential changes; team variables remain in the selected team scope.
...
- **Provider Status** in the Secrets section shows which intel providers are usable in the active scope, which ones still need an API key, the app-facing secret names, supported lookup types or CLI uses, and broad account/free-tier notes. It also identifies the shared FIRST EPSS and CISA KEV data as bundled, live, or local; shows its freshness, publication date, snapshot age, version, and model; and explains exactly how to enable live refresh when it's off. Clicking a secret name opens the add-secret prompt with that key selected.
...
- Each personal workspace has one immutable, validated storage directory under the configured workspace root. Keeping a workspace preserves its existing anonymous directory, and adding, rotating, or revoking credentials never moves it.
...
- Workspace access updates the personal storage directory's activity timestamp. Periodic cleanup removes inactive workspace directories after `workspace_inactivity_ttl_hours`; it does not delete individual files solely because their file timestamps are old.
...
### Assessment state and evidence

- On an active **Assessment** cycle, an assessor can mark a check blocked, intentionally skipped, or not applicable with a required reason. Each check shows the saved reason, when it was recorded, and whether it came from a team member or browser session. The same desktop and mobile action can restore the state calculated from saved evidence later. Completed and archived cycles stay read-only, and view-only team members can review the decision without changing it.
- **Manage evidence** on an active Assessment check links a compatible saved Project run, workflow execution, finding, Atlas entity, artifact, or screenshot by its ID. Desktop and mobile show the newest manual links and can remove one without deleting its saved source. Both actions recalculate the check against the cycle's frozen evidence rules; completed and archived cycles stay read-only.
- The Assessment **Retest queue** collects findings marked ready to verify or needing another retest. Findings share a group only when their Project target, frozen check, action, and HTTP role/profile are identical. A group of two to ten findings can start one shared run only when every plan is safe, credential-free, and has the same exact command; otherwise the queue explains the mismatch and keeps the actions individual. Completion links the run to each finding independently, so one unavailable finding doesn't discard the other evidence. The result can suggest a disposition, but a person still has to review and save it.

### ZAP and OAST connectors

- External ZAP plans show the scanner-side enforcing proxy and check target DNS from both the app and scanner networks. Preview, confirmation, and submission stop if the policy service isn't available, its identity changed, or any scanner-side address leaves the approved networks.
- A URL check can select one to eight visible Project URLs, a supported anonymous HTTP profile, safe or separately enabled intrusive policy, and extra path exclusions. Desktop and mobile show the exact non-secret Automation Framework plan before queueing it. The check recovers the newest saved job after reload, polls bounded status and progress while it's active, allows an authorized user to cancel it, and shows the finished Files path.
- **Review Atlas import** opens a prepared ZAP report in the existing Atlas review window with bounded counts, samples, warnings, and current apply choices. The job remains ready until the assessor presses **Apply**, so finishing a scan never saves remote findings on its own.
- The maintained blind-XSS check uses one saved query parameter and a private OAST connector. Desktop, mobile, and API clients can review its redacted command without contacting the service, confirm that exact plan to reserve a short-lived callback, recover recent reservation state, and poll the exact reservation until the private worker has made it ready.
- The Assessment check shows accepted, duplicate, and rejected OAST interaction counts plus callback and cleanup deadlines, and opens its bound source in Run Details. A separate fresh ready-only confirmation starts the Project-linked run and binds that exact callback without putting it in browser storage, the public command, response, logs, or audit details. The ordinary recommended-action route can't launch that plan.

### Project records

- Projects group top-level source records by link rather than copy. Current project links support completed runs and Atlas entities; Targets are the curated `domain`, `url`, and `ip` set used for scope tracking, while the Entities tab shows every linked Atlas entity type. Domain targets accept DNS names, URL targets accept full HTTP(S) URLs, and IP targets accept single IPv4 or IPv6 addresses. URL paths keep a trailing slash when one is entered because routes such as `/app` and `/app/` can behave differently; a bare host ending in `/` still uses the same root target. Targets discovered from command input enter the pending review queue. Once confirmed, later observations keep that decision instead of returning the target to pending. Legacy `host` target inputs from older API/import callers are still accepted as compatibility aliases and saved as `domain` or `ip`; the target editor and new validation messages only present `domain`, `url`, and `ip`. Run-owned artifacts surface through linked runs, and findings surface through linked runs or linked Atlas entities. Project list rows show run, finding, artifact, and package counts before you open a project, and the Findings tab label can show prefetched new/high counts when that project has triage work waiting. When team scope is active, team-owned Projects and team-owned run links are visible to other members of that team. Snapshots and manually selected workspace files stay in their history/files surfaces and are intentionally not project-linked.
...
- Project cards keep wrapped names and count badges readable as the list grows. The desktop sidebar scrolls within the modal, with the create controls always visible.
...
- Project details expose labels, project notes, editable Atlas-backed targets, linked Atlas entities, linked runs, findings, artifacts, and packages. Project notes use the shared entity-notes model, and suppressed Atlas entities and findings stay hidden from default Project views until they're restored in Atlas.
- Team-owned Projects share linked-run artifacts and evidence packages with team members, while package and artifact rows keep creator context when it's available. Previews, downloads, and package archives read from the source run's personal or team Files workspace, so a teammate doesn't have to switch into the run creator's personal workspace.

### Findings and verification

- The Findings tab pages through large result sets, shows each finding's source command, and keeps its tab count tied to the full server-side total, including findings attached through linked runs or linked Atlas entities.
- **Create finding** records an issue against a confirmed Project target when no parser or scanner produced it. The editor can start from an Assessment check, a Project-scoped Atlas domain/IP/URL profile, or selected Run Details lines with that context already attached. Assessor-authored rows keep **Edit finding**, labels/notes, and triage as separate actions across Projects, Atlas, and Run Details.
- The editor checks required fields, identifiers, CVSS, and safe HTTP(S) references before saving. Likely duplicates require a second confirmation, stale edits are rejected, and title, severity, or detail changes don't replace the stable observation or remediation identity. Browser and API clients use the same saved-finding contract, including explicit empty values for older records.
- Saved findings retain summary, impact, reproduction steps, confidence, CVE/CWE ids, CVSS details, safe reference links, and stable observation/remediation references in Projects, Run Details, Atlas, focused entity profiles, and API clients.
- A finding can keep typed references to runs, exact transcript lines, full output, workspace artifacts, screenshots, Atlas entities, Project targets, assessment checks, and retest runs. Each reference must belong to the same owner and Project. Removed sources stay labeled unavailable, and evidence packages preserve the references selected with the finding.
- The triage view shows the originating assessment check and frozen profile version, warns when the live profile changed or original evidence disappeared, and classifies up to 25 completed Project runs against the saved tool, target, completion, and output rules.
- **Run verification** recomputes the originating check's bounded command from its frozen action and current confirmed target. It shows the exact policy, scope, limits, and credential use for confirmation, then links the run to the Project and opens the terminal. Archived or changed context, stale confirmation, workflows, unsupported commands, and intrusive or destructive policy fail closed. A completed launched run is retained as retest evidence automatically.
- The newest compatible retest can suggest **Needs retest** when the same exact issue appears again, or **Verified** after a successful check that explicitly supports a clean result. The assessor still reviews and saves the final decision. A non-comparable run can be retained after a warning, compared with the original, and removed later without changing status. Final `verified`, `needs_retest`, and `not_applicable` decisions record when and by whom they were saved without exposing an access credential.
...
`probe list` shows the installed app-owned Nmap profiles and managed Nuclei profiles. Nmap choices cover general discovery and fixed service families such as TLS, SSH, mail, SMB, SNMP, LDAP, NFS, RPC, FTP, DNS, MySQL, and Redis; they never accept caller-supplied NSE scripts or argument files. Nuclei offers safe and standard local template sets, plus an intrusive set only when the operator has enabled intrusive Assessment and probe actions. Terminal autocomplete follows that setting: it hides the intrusive value while the feature is off, though an explicitly typed intrusive request still returns an unavailable plan instead of silently changing profiles. Compose keeps the managed templates in a named volume and fills that cache when it is empty. Upgrades from the older volume layout keep that snapshot only after its manifest has been rewritten for the new location; if that migration can't finish safely, startup stops with the legacy cache restored. A failed download doesn't block app startup; Nuclei plans stay unavailable until the cache is ready. Existing templates are never updated in the background. Direct and Assessment-plan Nuclei runs hold a shared lock on their approved snapshot, while an explicit template refresh waits for exclusive access. When an Assessment plan includes Nuclei work, its preflight is the app's explicit update path: an operator can refresh the managed cache and rebuild the preview before approving it. Headless profiles use the Chromium browser bundled with the image, so they don't download and execute a separate browser from temporary storage.

Protected Curl, HTTPx, Katana, Dalfox, and detection-only SQLmap probes can add `--http-profile <profile-name-or-id>`. The browser terminal suggests enabled profiles from the active Project by name and inserts their stable `htp_...` ids; a typed saved name is also accepted case-insensitively within the selected Project and needs quotes when it contains spaces. The preview identifies the canonical profile id, saved name, role, and allowed hosts and paths, while Secret values and Files paths stay out of the command, response, transcript, logs, and History. Nuclei stays anonymous because its templates can generate requests outside an HTTP profile's exact scheme, port, root, and path boundary; selecting a profile returns an unavailable plan before confirmation or launch. A personal owner can launch their own probes. In team scope, viewers can list and plan; launching requires run permission, and a protected launch also requires Secret-management permission.
...
- The **Assessment** tab turns saved Project evidence into a practical coverage worklist. Start a maintained Network, Web, API, TLS, or Combined cycle, switch between current and earlier cycles, review covered, awaiting-review, untested, excluded, and unavailable-evidence totals, and narrow checks by category, state, policy level, or evidence availability. The worklist shows how many filters are active, and **Clear filters** returns all four controls to their defaults.
- **Run assessment plan** turns an active cycle into one reviewable preview and a durable batch.
  - **Preview and approval:** Choose targets and categories, keep safe commands selected by default, and optionally include standard checks through a separate confirmation. The preview shows bounds, potential coverage, and estimated completion time. Advanced command and concurrency choices stop at the deployment's configured ceilings. A sticky decision bar keeps the valid next action—**Run assessment plan**, **Update templates**, or **Refresh preview**—within reach, and explains why launch is disabled when the preview needs attention. Exact commands and exclusions start as count-labeled disclosures. Building or refreshing the preview, changing the standard-check choice or a batch limit, and updating templates keep the current scroll position so the loading state remains visible. View-only members can inspect the plan but can't update templates, start work, or cancel it.
  - **Template preflight:** Plans with Nuclei work show the installed template release, digest, refresh age, validation state, and affected command count. A stale but valid snapshot can be updated and rebuilt or deliberately retained. A missing, invalid, unreadable, or incompatible snapshot blocks launch and explains the operator action it needs. A successful update immediately adopts the rebuilt preview; if validation still fails, the plan shows that result instead of enabling launch.
  - **Progress and navigation:** An unstarted plan remains available when the Project workspace closes and reopens in the same browser page. After approval, the **Assessment batch** monitor survives reloads, follows each command, opens child runs in History, and keeps completed evidence when another command fails or the batch is canceled. Polling updates only the monitor, preserving the surrounding scroll position and focused control. Status Monitor shows the same progress and currently launching or running commands, while **View batch** opens the exact Project and cycle. History, Run Details, Recent evidence, comparisons, reports, and evidence packages identify each child's batch and item and can return to that monitor.
  - **Notifications:** Channels subscribed to **Run complete** receive one final batch summary with its outcome counts and link. Child commands don't each send a notification.
  - **Retry behavior:** A terminal batch with failed, unavailable, interrupted, or unlaunched work can build a current retry preview. Starting it creates a new linked batch without changing successful source commands, runs, or evidence. Repeated Nuclei template-loading failures appear as one diagnosis with an update-and-retry path that rebuilds the retry preview before another batch starts.
- Target rows keep their full-cycle check, covered, and review totals while the bounded worklist pages through individual checks. Expanding a row shows policy level, evidence count, newest matched or manual evidence, manual-decision state, and missing-source warnings. **Recent evidence** identifies the newest sources across the cycle with their check, target, availability, match origin, and observation time.
- Each check can open **Create finding** with its confirmed target and Assessment context already selected, or preview and start its saved recommended action. The preview shows the exact command, target, policy, scope, limits, and credential use. Supported safe and standard commands, plus explicitly enabled maintained intrusive actions, link to the Project and open in the terminal. Stale, changed, unsupported, disabled intrusive, and destructive plans stop safely.
- Use [Project probes](#project-probes) to run one reviewed command outside a cycle. A successful Project-linked probe can provide compatible evidence for normal reconciliation, but launching it doesn't directly change Assessment state.
- API checks use one Project-linked OpenAPI JSON artifact, recheck that unchanged contract, show the GET/HEAD operation count and fixed request limits, and run Schemathesis with private schema and report files. Reviewed operation results stay attached to the cycle. Safe failure summaries become active-confirmation findings linked to the run and check; a clean result counts as coverage only when every expected operation completed, and missing operations remain untested.
- Large check sets page instead of loading at once. Desktop and mobile use the same saved cycle and permissions, and the tab remembers its selected cycle, filters, expanded targets, page, reading position, and return context for each Project. Mobile check actions use the shared action sheet. View-only team members can review the work but can't change a cycle, create a finding, or start a run.
- Completing a cycle adds a **Finding changes** summary against the newest compatible earlier cycle. New, persistent, not-observed, regressed, and incomparable remediation groups stay separate, related observations count once, and each row can open current or earlier evidence. **Not observed** requires the same frozen check, available compatible evidence, and explicit clean-result support; missing or changed evidence is incomparable. A regression requires a previously human-verified issue to appear again.
- Completing or archiving a cycle requires warning confirmation. Deleting an archived cycle first previews its saved check, evidence-link, and comparison counts; source runs, findings, entities, and files remain. Overview and Findings show the same compact remediation-group totals and open the exact cycle behind them.
- Nuclei recommendations use reviewed safe, standard, or intrusive template profiles selected by the saved check policy. The Assessment worklist also checks a bounded set of saved, target-compatible HTTPx technologies, inferred CVE findings, app-captured services, and correlated dangling-DNS signals. When those records support the standard vulnerability or safe takeover check, the row says **Recommended from saved evidence**, explains the signal types, and warns if the read reached its limit. This hint doesn't rewrite the frozen profile, contact a provider, save a finding, or start a run. Intrusive checks are never recommended automatically. The launch confirmation lists the included exposure, misconfiguration, CVE, technology, network-service, TLS, API, headless, or low-aggression DAST families, shows the exploit, callback, authentication, brute-force, denial-of-service, code, and local exclusions that apply, and identifies the installed template manifest by its recorded release, SHA-256 revision, and entry count. Safe, standard, and intrusive runs satisfy only their matching saved check. Simply opening the Assessment never updates templates. An Assessment-plan refresh stages and validates a replacement, keeps the last good cache if anything fails, and rebuilds the complete preview with a new digest before approval. Every Nuclei child in a started batch stays on that one reviewed snapshot. Intrusive headless and DAST checks stay behind the deployment gate, one-target bounds, launch-time revalidation, and a fresh warning confirmation.
...
- Expanded Assessment targets show conservative next steps for explicit Project service fingerprints. Each hint names the saved port and service plus the reviewed action's policy and rationale. Nmap hints also name the fixed app-owned profile, so an assessor can see whether the suggested review covers TLS, SSH, SMB, mail, DNS, database, or another supported service family. Those profiles don't accept custom NSE scripts, arguments, or argument files. The FTP profile makes one standard anonymous-login check with remote listing disabled; it doesn't guess credentials, run the broad `auth` category, or download remote content. Only the exact success result becomes typed evidence. Uncertain, unsupported, or conflicting scanner labels stay in review, and the hint itself never starts a run.
...
- The **Web Surface** tab gives desktop and mobile users a paged gallery of verified HTTPx screenshots linked to the Project. Each card keeps the saved URL, page title, response status, technologies, HTTP role, capture time, and source run together.
- Visual comparison uses the nearest earlier capture from a different run only when the exact URL and HTTP role match. Results are labeled changed, unchanged, no baseline, unavailable, or outside the bounded comparison window. They remain evidence for review, not vulnerability findings.
- Filters narrow captures by target, exact HTTP status, technology, role, visual hash, or comparison result, and grouping organizes the visible page by the same details. When a collection reaches the bounded search or comparison window, the gallery states how many recent captures were checked rather than implying that older captures matched nothing.
- Images expand in place or open in a focused view. Buttons, arrow keys, and horizontal swipes move through the current page with clear position and boundary states, and closing returns focus to the originating card.
- A card can send its screenshot to the package wizard or report draft. Package handoff starts in Raw mode and explains that Redacted mode omits image bytes; reports keep the image behind authenticated storage and include metadata rather than embedding the binary. URL actions open Atlas, and source actions open Run Details.
- Changed, missing, unavailable, or conflicting files remain visible with a clear status. Images load through the authenticated artifact route, captured HTML is never rendered, and new screenshots must fit the owner's Files count, single-file, and total-byte limits. Extra output is removed after the run without evicting earlier evidence.
...
- Desktop and mobile show the role, scope, request bounds, enabled state, and whether referenced Secrets are available. Members with Secret-management permission can create, edit, disable, or delete a profile and use **Manage Secrets** to add or replace values in Options. Team viewers see redacted counts instead of reference names.
- Protected-reference fields list the stored Secret names available in the Project's current personal or team scope, and a missing reference names the field that needs attention. Basic authentication uses separate username and password Secrets. API clients may also supply a unique consumer env binding; the profile saves the canonical Secret name.
- Curl, HTTPx, Katana, Dalfox, and detection-only SQLmap actions ask which available role to use before the final preview. Disabled or incomplete profiles remain visible for repair but can't be selected. Nuclei stays anonymous because its template-generated requests can't enforce the saved profile's exact request boundary.
- Dalfox discovery disables active payloads, remote wordlists, redirects, and callback modes. It saves a bounded, deduplicated list of parameter names and request locations for the exact approved URL; discovery rows don't create findings.
- When maintained intrusive Assessment actions are enabled, desktop, mobile, and API clients can choose one saved query parameter and review a fixed active-validation plan. The plan uses one worker with fixed payload, rate, request, and time limits; it disables discovery, mining, remote, blind, redirect, WAF-bypass, and full-request capture modes.
- The app rechecks the saved run, observation, scope, profile, and command immediately before launch, keeps protected values out of the visible command, and removes short-lived credential material afterward. Saving or viewing a profile never starts a scan.
- A successful reviewed run creates Project findings without copying payloads or response evidence into the summary. Confirmed browser execution is high-confidence, AST-only evidence is medium-confidence pending runtime confirmation, and reflection-only evidence is low-confidence. Each finding links to the exact result line and the discovery run that supplied the parameter.

### Overview and linked evidence
...
- Evidence packages are draft Project manifests built from selected Project material.
  ...
  - Packages containing CVE findings snapshot the exact EPSS/KEV/NVD rows, source origins, source and model versions, dates, checksums, attribution, and non-endorsement notice used at build time, so a later refresh doesn't rewrite what the package reported.
  ...
  - The Metadata step can select any saved Assessment cycle, including archived history, or use the active/newest default. The archive keeps the frozen profile, target and check snapshots, coverage denominator, exclusions, evidence and source-run details, stored tool versions, fix-first work, comparison basis, and warnings for unavailable sources or screenshot files that weren't selected.
...
- The Report tab turns selected Project material into a narrative engagement report.
  ...
  - CVE findings show the KEV, EPSS probability and percentile, CVSS, and positive public-exploit-reference reasons behind their placement. Missing stored KEV, EPSS, or NVD data stays unknown instead of being presented as proof that no exploit exists. Reports and report manifests pin the public source origins, source and model versions, dates, checksums, attribution, and non-endorsement notice used for that export.
  ...
  - The Export section can pin any saved Assessment cycle or use the active/newest default. The Methodology and Findings sections turn that saved context into readable coverage, exclusions, warnings, and fix-first priorities, while the archive manifest keeps the complete machine-readable snapshot and redaction boundaries.
...
- Project rule quotas, preview caps, manual apply caps, run-finalization caps, and per-workspace preview rate limits keep broad rules bounded. Capped previews and applies tell the UI when more matches may exist beyond the returned window.
...
- Command-specific runtime adaptations are also declared in the registry. `inject_flags` handles safe default flags such as `nmap -sT`, `nuclei -ud /tmp/nuclei-templates/current`, `naabu -scan-type c`, and `mtr --report-wide`; managed workspace directories and environment wrappers handle Amass' active personal/team database path.
...
**Limits:** disabled by default, desktop-only, and restricted to commands that explicitly declare PTY behavior in the command registry. PTY runs have a configured max runtime and per-workspace concurrency cap. Multi-worker deployments require Redis unless `run_broker_require_redis` is intentionally relaxed for local development.
...
**Purpose:** live theme picker backed by the named variants under `app/conf/themes/`, with the choice persisted as part of the active workspace preference snapshot and cached locally for reload continuity.
...
- Picking a variant applies it immediately and saves the choice into the current workspace's preference snapshot, while also caching it locally so reloads stay fast.
...
**Limits:** anonymous workspaces keep their own browser-local theme choice, while kept workspaces restore the saved theme across browsers and devices. Clearing browser storage removes the local cache but does not erase a kept workspace's saved theme on the server.
...
**Purpose:** workspace-owned controls for presentation, run behavior, access credentials, teams, and provider secrets, with a local cache for fast reloads.
...
- `credential` or `credential status` shows the current workspace access state without printing a reusable value.
...
- Principals with durable access can create teams, join with invite or recovery codes, manage members, and switch between personal and team scope from Options → Teams, the desktop HUD scope menu, the mobile scope selector, `/api/v1`, the terminal `team` built-in, or the bundled `darklab team ...` CLI commands.
...
**Configuration:** no global config switch. Team-Mode requires a kept workspace because anonymous browsers can't identify the same principal across devices.
...
- Audit rows are intentionally smaller than raw app data. File rows do not store file contents, notification rows do not store webhook URLs or secrets, credential rows store only safe ids and metadata, and import rows store source/options/counts rather than imported bodies.
...
**Limits:** `/audit` is an operator-wide view of personal and team activity, actor labels, target ids, request metadata, and reviewed audit details. Grant operator access only to people trusted to inspect the instance. Exports recheck access while streaming and stop if access is lost. An access-revoked CSV includes an `__access_lost__` marker before the download aborts; discard that incomplete file and export again after verification. Interrupted JSON remains incomplete and cannot be parsed. The audit log is a product-action trail, not a complete replacement for infrastructure logs.
...
`/metrics` is meant for Prometheus, Grafana, and similar monitoring stacks. It exposes `darklab_` metrics for HTTP volume and latency, active and completed runs, PTY activity, durable workflow execution and step outcomes/durations, workflow capture failures, cancellations and recovery, Assessment batch activity, queue depth, retained outcomes, launch latency, rejected work, concurrency deferrals and recovery, rate-limit pressure, broker mode and subscribers, database and Redis health, selected database hot-path latency, Postgres pool health, AI queue health, AI Redis coordination key pressure, workspace usage, Atlas/finding counts, intel provider results and cache size, evidence package builds, snapshots, client errors, and unhandled server exceptions. Labels are bounded to safe values such as command roots, provider IDs, endpoint names, status classes, and coarse outcomes. Workflow and Assessment batch metric labels don't include names, ids, input or capture values, targets, profiles, commands, paths, or output text.
