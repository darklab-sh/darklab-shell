...
- Expanded Assessment targets show conservative next steps for explicit Project service fingerprints. Each hint names the saved port and service plus the reviewed action's policy and rationale. Uncertain, unsupported, or conflicting scanner labels stay in review, and the hint itself never starts a run.
