...
- `project` manages case folders from the terminal: list/create/use/current/clear/archive/unarchive/delete, link or unlink runs and workspace files, link the last eligible run, and list/add/quick-add/remove project targets.
...
- Projects group top-level source records by link rather than copy. Current project links support completed runs and manually selected workspace files; run-owned artifacts and findings surface through linked runs. Snapshots stay in history/share surfaces and are intentionally not project-linked.
...
- Active projects can automatically link completed external runs and supported browser-owned built-in runs. Browser-admin built-ins are excluded, and **Link last run** backfills the most recent eligible run when needed.
- Project details expose project labels, project notes stored through `entity_notes` with `entity_type='project'`, editable targets, linked runs, workspace files, findings, artifacts, and packages. Labels and notes are also editable for linked runs, findings, run file artifacts, workspace files, and packages through the shared entity metadata editor.
