...
- **Live Web Review** captures one approved live URL as verified screenshot evidence, then saves a bounded structured parameter inventory without sending XSS payloads. It uses Files only for the screenshot image; the parameter results stay with the normal run output and Project evidence flow. Fixed workers, rates, targets, retries, and timeouts keep both steps focused on the URL you chose.
