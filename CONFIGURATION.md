...
In `operator-status`, `eligible` describes only the principal and its grant (`eligibility_scope: principal_grant`). `local_access_policy` reports the CLI process's evaluated profile, available sign-in methods, and required recent browser verification. `serving_application.observed` and `browser_verification.observed` are false: this command cannot inspect an existing web worker or browser session. Verify actual access by signing in to `/admin/`.
...
- `open` is the default. New browsers can start anonymously and may keep a workspace later with a portable credential. Signed-in browsers use the same protected cookie session as the other profiles. Configured provider sign-in is optional.
...
Existing open-profile browsers exchange an older saved credential once, then reload with the new browser session before loading workspace data. The secret is removed from browser storage only after the protected session succeeds. Failed exchanges stay blocked; save the credential externally, correct HTTPS or cookie settings, and retry, or use Access to explicitly remove invalid saved access. If a profile change sends the browser straight to sign-in, **Use saved credential** performs the same exchange where credential sign-in is allowed. Link a provider before moving that workspace to `oidc_required`; a portable credential cannot override the profile's policy.

Sign-out returns to the shared sign-in page. In open mode, **Continue anonymously** leaves the saved principal workspace intact and returns to an unrelated anonymous workspace, or a new one after attachment. If an older reusable credential is still saved, confirm that you saved it elsewhere before removing it from this browser. Anonymous access never grants operator access. Changing a profile doesn't transfer ownership or grant authority.
...
**Sign out everywhere** in Access revokes every browser session. In every access profile, it requires a recent sign-in and offers a **Sign in again** link when the current session is too old.
...
Operator grants are absent by default. To enable access:

1. Use HTTPS for browser sign-in. You can retain `open` for anonymous workspace access or choose `token_required`, `oidc_required`, or `mixed` to require sign-in. Follow [restricted browser access](#restricted-browser-access) for bootstrap/provider setup and precautions before changing a live deployment. For provider-based operator access, confirm that fresh authentication returns the signed integer `auth_time` required by [provider compatibility](#provider-compatibility).
...
Project workspace settings cap personal- or team-scoped case folders, links, targets, labels, notes, and package exports. Interactive PTY settings tune the separate guarded terminal path after `INTERACTIVE_PTY_ENABLED=true` enables it.
...
| `team_read_rate_limit_per_minute` | `180` | Max team-management read requests per minute. The Options Teams tab, desktop HUD scope selector, mobile scope selector, and `/api/v1/teams` list/detail routes use this principal-aware limit |
...
`osv_advisory_mode: external` permits bounded lookups against `https://api.osv.dev/v1/query` only when an authenticated client calls `POST /api/v1/advisories/osv/lookup`. Each request sends the exact PURL and version supplied in that action; it never uploads an SBOM, discovered inventory, Project target list, or finding. Team calls require finding-triage permission. The request uses the configured timeout, attempt, and response-size limits, rejects redirects, and keeps positive and negative results under SHA-256 lookup keys until their configured expiry. The normal Team write limit applies, and the app also allows at most ten fresh OSV requests per minute, two at once, and one active request for the same package and version. A busy lookup asks the client to try again with HTTP 429. Provider waits don't hold a database connection. A failed request keeps earlier accepted package rows. `providers` reports the mode and saved-data status, while logs, metrics, and audit data record only source, outcome, bounded counts, origin, and error class. They don't include the local path, package identifier, requested version, or advisory payload. Changing the mode alone doesn't make a request, and viewing a finding, Project, report, or Atlas profile never acquires OSV data.
...
| `BROWSER_SESSION_IDLE_MINUTES` | Docker Compose, Flask app | Inactivity deadline for authenticated browser sessions. Defaults to `30` minutes |
| `BROWSER_SESSION_ABSOLUTE_HOURS` | Docker Compose, Flask app | Maximum browser-session lifetime from authentication. Defaults to `12` hours and must not be shorter than the idle limit. Team privilege changes and provider linking preserve this deadline; only fresh sign-in renews it |
...
Complete these checks before changing an existing installation's image or database:

1. **Check the identity schema.** The v3 runtime requires principal-owned data on both SQLite and Postgres. Databases that still contain retired identity tables or ownership columns are rejected at startup. The old conversion helper isn't included. A legacy `tok_` value can't sign in or authorize an API request, and renaming it doesn't turn it into a new credential. Confirm compatibility on a disposable restored copy before touching the live installation; an incompatible database needs a separate data transition or a fresh installation.
2. **Keep a usable access path.** Choose the [access profile](#restricted-browser-access), confirm HTTPS and proxy settings, and test a working credential or provider sign-in plus the local recovery path. For OIDC, verify the exact issuer, callback URI, client authentication, subject/provisioning policy, and signed authentication time needed for recent operator verification. Operator pages require an explicit grant and a recently verified browser session in every profile; Team roles don't supply either.
3. **Update integrations and configuration.** API v1 and the CLI require scoped PATs. Replace the old `--token`, `DARKLAB_TOKEN`, and `token` settings with `--pat`, `DARKLAB_PAT`, and `pat`; issue a new PAT for each integration. For 3.1.0, rename `diagnostics_allowed_cidrs` to `metrics_allowed_cidrs` in every YAML layer and update old audit URLs. Run the candidate's [configuration checker](#validating-instance-configuration) with `--strict` against the intended inputs. It checks configuration, not database compatibility or provider availability.
4. **Verify a complete backup and rollback path.** Stop writes and preserve the database, its matching secrets key, Files, `.env`, `conf/`, operator Compose settings, and release metadata together. Follow [Operator Backups](#operator-backups), check the archive checksums, and rehearse recovery on a separate target using the original release. Retain a separate copy of `compose.operator.yaml` when the older backup helper doesn't include it. Schema changes can be forward-only; rollback means restoring the matching pre-upgrade state, not just selecting an older image.
5. **Verify the candidate artifacts.** Check the exact tag's signed checksum manifest and image digest before startup. Compare the new release-owned files with the installation's configuration, then verify health, browser sign-in, API access, operator verification, and metrics scraping after the upgrade. Keep the pre-upgrade backup until those checks are complete.
...
`darklab-deploy` manages the release-owned base stack and automatically adds an existing `compose.operator.yaml` after it for lifecycle work. Backup, restore, database migration, upgrade validation, and removal all use the layered configuration; restore restarts and upgrade instructions do too. Managed backups include this file when it exists. Restore keeps the destination's current override; see [Operator Backups](#operator-backups) for manual recovery. Application log format remains separate: set `log_format: gelf` in installed `conf/config.local.yaml` only when the application itself should emit GELF-shaped JSON.
...
docker compose -f compose.dev.yaml build \
  --build-arg APP_VERSION=3.1.0-rc.1 \
...
Follow [operator access setup](#operator-settings-console) to grant a principal and sign in with recent verification. The grant covers diagnostics, audit, and settings from any network in every profile, including `open`. Anonymous visitors and network allowlists never confer operator access.
...
Starting with 3.1.0-rc.1, `diagnostics_allowed_cidrs` is no longer supported. Rename it to `metrics_allowed_cidrs` in every YAML layer before upgrading. Startup ignores the old key with an unknown-key warning, and `config check --strict` fails when it finds that warning. The old key never changes the effective allowlist: an existing canonical value stays in effect, and installations with only the old key use the empty default, which disables scrapes. Normal layer precedence applies to `metrics_allowed_cidrs`, including an explicit empty list. The checker and operator inventory show the canonical setting and its actual winning source. This setting grants no operator access and doesn't bypass AI limits. Apply YAML changes through the documented [container recreation workflow](#config-file-reload-behavior).
