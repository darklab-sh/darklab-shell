...
5. **Verify the release artifacts.** Check the exact tag's signed checksum manifest and image digest before startup. Compare the new release-owned files with the installation's configuration, then verify health, browser sign-in, API access, operator verification, and metrics scraping after the upgrade. Keep the pre-upgrade backup until those checks are complete.
