...
| `asset_bundle_mode` | `bundle` | Frontend asset rendering mode. `bundle` renders content-hashed files from `app/static/build/` and fails if the committed build output is missing or incomplete; `source` renders ordered source-file tags from the committed asset manifest for local edit-and-refresh work. `ASSET_BUNDLE_MODE` can also override this setting |
