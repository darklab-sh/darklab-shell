...
| `asset_bundle_mode` | `source` | Frontend asset rendering mode. `source` renders ordered source-file tags from the committed asset manifest for local development; `bundle` renders content-hashed files from `app/static/build/` and fails if the committed build output is missing or incomplete |
