...
`osv_advisory_mode: external` permits bounded, explicitly initiated lookups against `https://api.osv.dev/v1/query`. Each request sends one exact PURL and version; it never uploads an SBOM, discovered inventory, Project target list, or finding. The request uses the configured timeout, attempt, and response-size limits, rejects redirects, and keeps positive and negative results under SHA-256 lookup keys until their configured expiry. A failed request keeps earlier accepted package rows. `providers` reports the mode and saved-data status, while logs and metrics record only source, outcome, bounded counts, and error class. They don't include the local path, package identifier, requested version, or advisory payload. Changing the mode alone doesn't make a request, and viewing a finding, Project, report, or Atlas profile never acquires OSV data.
