...
Existing open-profile browsers exchange an older saved credential once before loading workspace data. The secret is removed from browser storage only after the protected session succeeds. Failed exchanges stay blocked; save the credential externally, correct HTTPS or cookie settings, and retry, or use Access to explicitly remove invalid saved access. If a profile change sends the browser straight to sign-in, **Use saved credential** performs the same exchange where credential sign-in is allowed. Link a provider before moving that workspace to `oidc_required`; a portable credential cannot override the profile's policy.

Sign-out returns to the shared sign-in page. In open mode, **Continue anonymously** leaves the saved principal workspace intact and returns to an unrelated anonymous workspace, or a new one after attachment. It never grants operator access. Changing a profile doesn't transfer ownership or grant authority.
