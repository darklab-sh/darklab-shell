...
| `workspace_enabled` | `false` | Server-side only. Enables app-managed personal and team Files, including the constrained `command > file`, `command >> file`, and final `command \| tee file` sinks. It does not enable raw shell navigation, file-descriptor redirects such as `2>`, or arbitrary redirection |
