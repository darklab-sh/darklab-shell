...
Each Assessment target heading shows its totals for the whole cycle, even when the check worklist spans several pages. Paging changes which checks are visible without changing the target's covered or review counts.

The maintained catalog offers Network, Web, API, TLS, and Combined cycles. TLS cycles use fixed certificate-chain and fast high-severity configuration checks for approved domains and IPs. Combined cycles collect every applicable check from the other four profiles in one frozen snapshot, so a Project with mixed targets can track the whole surface without starting separate cycles.

When you build an evidence package or engagement report, choose the Assessment cycle that explains the work. Earlier and archived cycles stay available, and **Current or newest saved cycle** follows the active cycle first, then the newest completed or archived cycle. The export keeps the frozen scope, coverage, exclusions, evidence references, fix-first priorities, comparison basis, and clear warnings when a source or screenshot file wasn't included.

External ZAP scans use the operator's scanner-side network guard as well as the app's own target check. The plan shows the enforced proxy, and preview, confirmation, or submission stops if ZAP's DNS view leaves the approved networks.

Expanded Assessment targets also show reviewed next steps for explicit Project service evidence, such as an HTTPS or SSH fingerprint. Nmap suggestions name the reviewed profile and the evidence it can collect; app-owned profiles don't accept custom NSE scripts, arguments, or argument files. The FTP profile can make one standard anonymous-login check with remote listing disabled, but it doesn't guess credentials or enable a broad authentication scan. Uncertain, unsupported, or conflicting service labels don't produce an action, and these read-only hints never start a run. Successful reviewed Nmap facts appear in Run Details and matching Assessment checks on desktop and mobile, while API clients can page through the same saved observations. These views stay inside the current personal or team scope and never include free-form NSE output or change a check or finding.

Projects also include a **Web Surface** tab for reviewing saved HTTPx screenshots on desktop or mobile. It shows the URL, page title, response details, capture role, and source run beside each image; unavailable or changed files stay visible with a clear explanation. Each capture says whether its visual hash changed from the nearest earlier capture of the same exact URL and HTTP role. You can filter or group by that comparison as well as target, HTTP status, technology, role, or exact visual hash. Full view keeps you in the gallery and lets you move through the current page with buttons, arrow keys, or a horizontal swipe. An available screenshot can go straight into the existing package wizard or report draft. Package handoff starts in Raw mode because an image can't be safely text-redacted; choosing Redacted omits its bytes, while reports keep the image behind signed-in storage and include its metadata. Screenshots load through the signed-in artifact path, and captured HTML is never opened inside darklab_shell. New captures also share the owner's Files count, size, and total storage limits, so a large screenshot run can't evict earlier evidence to make room.

The Assessment tab also manages reusable HTTP profiles on desktop and mobile. It shows each saved role, scope, request bounds, enabled state, and whether its referenced Secrets are available. **Manage Secrets** opens the existing Options panel for add-or-replace values, while viewers see redacted counts. Supported Curl, HTTPx, Katana, Nuclei, Dalfox, and detection-only SQLmap actions ask which available role to use before showing the redacted launch plan. When an operator connects an external OWASP ZAP service, a URL check can also select up to eight Project URLs and a credential-free anonymous profile, show the exact non-secret ZAP plan, and queue it only after a separate confirmation. The check keeps its latest status after a reload, shows bounded progress, supports cancellation, and links the finished JSON report to its exact Files path. **Review Atlas import** shows the report's counts, samples, warnings, and available choices in the existing Atlas review window. Scanner findings aren't saved until the assessor presses **Apply**. When saved technology, version-based CVE, service, or dangling-DNS evidence supports a Nuclei check for that exact target, the worklist labels the check as recommended and explains why. This is a read-only hint: it doesn't change the saved profile, contact a provider, or start a scan, and intrusive checks are never recommended automatically. Nuclei previews name the safe, standard, or separately enabled intrusive template profile, the checks it includes, what it excludes, and the installed template manifest's recorded release, SHA-256 revision, and entry count. The intrusive profile is limited to reviewed headless and low-aggression DAST checks for one target and asks for a fresh warning confirmation before every run. Missing or changed manifests require an explicit `nuclei -update-templates` command and a fresh review; opening the Assessment never updates templates. An API assessment can instead use a saved OpenAPI JSON artifact: choose the contract, review its GET/HEAD operation count and fixed request limits, then start the bounded Schemathesis run without exposing a direct schema command. The reviewed operation results stay with the Assessment as evidence. Safe failure summaries become linked findings, while a clean run counts as coverage only when every expected operation completed; missing operations stay untested. A Dalfox discovery run saves the parameter names and request locations it found for the exact URL. When maintained intrusive actions are enabled, an assessor can choose one saved query parameter, review the fixed payload, rate, request, and time bounds, and confirm a separate XSS validation run. Discovery rows stay as evidence, while a successful reviewed validation creates linked findings that keep confirmed browser execution, AST-only evidence, and reflection-only evidence at distinct confidence levels.

Saved CVEs also use release-pinned FIRST EPSS and CISA KEV data to explain which findings deserve attention first. Every saved finding has a stable observation reference that keeps its evidence method distinct. Assessors can create and edit their own findings from Project Findings, start one from an Assessment check or Project-scoped Atlas profile, or select saved output lines in Run Details and turn them into a finding or supporting evidence. Likely duplicates require a second confirmation, and later title or severity edits keep the original observation and remediation references. API clients can use the same saved-finding model. They can also save reusable HTTP assessment profiles for a Project, including named roles, exact host and path boundaries, request limits, and references to app-managed Secrets or Files without copying credential values into the profile. API clients can select one of those profiles when previewing and starting a supported Curl, HTTPx, Katana, Nuclei, Dalfox, or detection-only SQLmap assessment action; the visible command stays redacted and short-lived credential material is removed after the run. Project findings can retain typed references to supporting runs, transcript lines, output, workspace artifacts, screenshots, Atlas entities, targets, assessment checks, and retest runs; evidence packages keep those references with the finding, and removed sources are labeled unavailable instead of silently disappearing. Each Assessment check also offers its saved recommended action: darklab_shell previews the exact bounded command, target, policy, scope, limits, and credential use before asking for confirmation, then links the run to the Project and opens it in the normal terminal. The Assessment tab keeps the selected cycle, filters, paging, expanded targets, and reading position when you return. In a Project, the triage view explains whether a completed retest run matches the original tool, target, and frozen assessment check before the assessor attaches it or compares it with the original. When a finding points back to a saved assessment action, **Run verification** uses the same guarded preview; the completed run also stays linked to the Project and is retained as retest evidence. Compatible evidence can suggest **Needs retest** when the same issue appears again, or **Verified** when a successful check explicitly supports a clean result. The suggestion explains its basis and can fill the status field, but final verification stays a human decision and records when it was saved plus safe team-member context when available. When a cycle is completed, **Finding changes** compares matching checks with the newest compatible earlier cycle and separates new, persistent, no-longer-observed, regressed, and incomparable remediation groups, with links to both evidence sets. The same compact totals appear in Overview and Findings, and selected evidence packages and reports preserve the matching current and earlier references plus the reason a result couldn't be compared. Several observations of the same CVE or scanner rule on the same exact affected subject share one remediation identity, so fix-first views and cycle comparisons count the issue once without discarding its separate evidence or verification work. When two differently identified findings still represent one fix, an assessor can search for the other finding, preview every affected observation, and explicitly merge their remediation groups from the shared triage editor. Operators can add stored NVD advisory status, CVSS, and CWE context from a local dataset or an explicit Atlas CVE Intel refresh, and can independently load exact OSV package-version applicability from a local dataset or permit bounded exact package lookups. Once accepted, those OSV rules can be checked locally against one exact PURL and version without contacting OSV or changing a finding; the match remains inference evidence until a separate reviewed write uses it. An external OSV lookup sends only the requested PURL and version; it never uploads an SBOM or discovered inventory. Simply viewing a finding never contacts either source. Later NVD disputes, rejection, withdrawal, reinstatement, and material CVSS downgrades appear in Project Monitoring without rewriting the original finding. The `providers` command shows each configured advisory mode and saved-data status. Live bulk-feed, NVD advisory, and OSV package storage are off by default, so a fresh or disconnected install still has a dated EPSS/KEV baseline without making an outbound request.

For the maintained private blind-XSS check, desktop, mobile, and API clients can review the redacted bounded command, explicitly reserve its short-lived callback, and poll that exact reservation until the private worker has made it ready. The Assessment check recovers that state after a reload, shows accepted, duplicate, and rejected interaction counts with the callback and cleanup deadlines, and can open the bound source run in Run Details. A separate fresh confirmation starts the Project-linked run and binds that callback to it. Simply previewing the plan never contacts the OAST service, the ordinary recommended-action route can't launch it, and callback details stay out of browser storage, the public command, and the run response.

When external OSV mode is enabled, API clients start that one-package lookup with `POST /api/v1/advisories/osv/lookup`. Team viewers can't start it; roles with finding-triage permission can. The response and audit trail keep only the result and counts, not the submitted package or version.
...
Web assessment cycles include a separate **Subdomain takeover confirmation** for one approved hostname. It uses the app's reviewed provider fingerprint, makes one request without redirects or saved credentials, and never claims the provider resource. A successful match becomes a high-severity finding only when the same Project has compatible saved DNS source and negative-target results from the preceding 24 hours. The finding links the exact two DNS lines and reviewed web match, while incomplete or stale evidence stays unconfirmed.
