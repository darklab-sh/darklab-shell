...
Every available action shows its exact target, policy, scope, limits, and credential use before it starts. Reusable HTTP profiles keep role and scope settings while referring to darklab_shell Secrets and Files instead of copying credential values. Saved service evidence can suggest a fixed Nmap profile, but uncertain or conflicting fingerprints don't produce an action. The full list of maintained one-off actions and their safety boundaries is in [Project probes](FEATURES.md#project-probes).

The terminal can use the same reviewed actions without starting a cycle. `probe list` shows what's available for the active Project, or for the active Project named with `--project <project-slug-or-id>`; add `--target-type domain|ip|url` when you only want compatible actions. The catalog also explains unavailable profiles, service recommendations, and the actions kept outside the one-off probe boundary. `probe plan <action> <target> --project <project-slug-or-id>` previews one exact bounded command, and `probe run ...` pauses in that tab for confirmation before it starts. If your Team role can't launch that plan, the preview explains the missing permission without asking you to confirm it. The browser terminal completes active Project slugs for that option and offers only confirmed targets compatible with the selected action, while `darklab probe` accepts the same slug or stable `prj_...` id. Targets discovered from command input still need confirmation before their first probe; after you confirm one, later runs can observe it without returning it to the pending queue. Nuclei profile completion offers the intrusive choice only when intrusive Assessment and probe actions are enabled. Web probes format saved IPv6 targets as bracketed URL hosts, so the reviewed command stays valid. A confirmed probe is an ordinary Project-linked run, so it streams in the same tab, appears in History, and can produce normal structured evidence. Compose installations fill an empty persistent Nuclei template cache during startup. If that best-effort download can't finish, the app still starts and Nuclei plans name the missing template feature and explain that `nuclei -update-templates` is needed before you review them again. Existing templates change only through that explicit command. Headless Nuclei probes use the Chromium browser included in the image instead of downloading an executable into temporary storage. Add `--http-profile <profile-name-or-id>` when an authorized team member needs one protected Project HTTP profile. Autocomplete shows enabled profile names and inserts their stable ids; manually entered names work too, with quotes around names that contain spaces. The preview shows the profile's role, allowed hosts, roots, and included or excluded paths, while its Secret and Files values stay out of the command and transcript. Nuclei stays anonymous because its templates can generate requests beyond an HTTP profile's exact path boundary; combining Nuclei with `--http-profile` returns an unavailable plan. Listing and planning remain read-only, and launching a probe doesn't directly change an Assessment cycle.
