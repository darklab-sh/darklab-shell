...
The terminal can use the same reviewed actions without starting a cycle. `probe list` shows what's available for the active Project, `probe plan <action> <target> --project <project_id>` previews one exact bounded command, and `probe run ...` pauses in that tab for confirmation before it starts. A confirmed probe is an ordinary Project-linked run, so it streams in the same tab, appears in History, and can produce normal structured evidence. Add `--http-profile <profile_id>` when an authorized team member needs one protected Project HTTP profile; its Secret and Files values stay out of the command and transcript. Listing and planning remain read-only, and launching a probe doesn't directly change an Assessment cycle.
