...
Saved CVEs also use release-pinned FIRST EPSS and CISA KEV data to explain which findings deserve attention first. Every saved finding has a stable observation reference that keeps its evidence method distinct. API clients can record assessor-authored findings for confirmed Project targets when a parser or scanner doesn't know about the issue; likely duplicates require an explicit override, and later title or severity edits keep the original observation and remediation references. Project findings can retain typed references to supporting runs, transcript lines, output, workspace artifacts, screenshots, Atlas entities, targets, assessment checks, and retest runs; evidence packages keep those references with the finding, and removed sources are labeled unavailable instead of silently disappearing. Several observations of the same CVE or scanner rule on the same exact affected subject share one remediation identity, so fix-first views count the issue once and one review or remediation-guidance change keeps those matching observations aligned without discarding their separate evidence or verification work. When two differently identified findings still represent one fix, an assessor can search for the other finding, preview every affected observation, and explicitly merge their remediation groups from the shared triage editor. Operators can add stored NVD advisory status, CVSS, and CWE context from a local dataset or an explicit Atlas CVE Intel refresh; simply viewing a finding never contacts NVD. Later NVD disputes, rejection, withdrawal, reinstatement, and material CVSS downgrades appear in Project Monitoring without rewriting the original finding. The `providers` command shows the source mode, version, and age. Live bulk-feed and NVD advisory storage are off by default, so a fresh or disconnected install still has a dated EPSS/KEV baseline without making an outbound request.
