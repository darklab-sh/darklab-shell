...
- **Outbound notifications** — durable `tok_` sessions can send queued run, schedule, PTY, watcher, and test notifications to webhook, Slack, Discord, Telegram, Pushover, or email channels, with vault-backed secrets, masked list responses, retries, delivery audit rows, and test sends
