...
For the maintained private blind-XSS check, API clients can review the redacted bounded command, explicitly reserve its short-lived callback, and poll that exact reservation until the private worker has made it ready. A separate confirmation then starts the Project-linked run and binds that callback to it. Simply previewing the plan never contacts the OAST service, the ordinary recommended-action route can't launch it, and callback details stay out of the public command and run response.
