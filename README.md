...
For the maintained private blind-XSS check, API clients can review the redacted bounded command, explicitly reserve its short-lived callback, and poll that exact reservation until the private worker has made it ready. Simply previewing the plan never contacts the OAST service, and the ordinary recommended-action route can't launch it.
