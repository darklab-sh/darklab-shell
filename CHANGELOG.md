...
- **Workspace Access replaces session tokens with portable credentials, scoped API tokens, and optional provider sign-in.** Personal data stays with a stable workspace while its credentials and sign-in methods change. The feature and operator references group open workspaces, device and API credentials, restricted sign-in, provider linking, and session expiry under the same Access feature.
  ...
  - **Upgrade from v2:** The old token table, headers, routes, aliases, and compatibility owner fields are removed; old copied credentials no longer authenticate. The [managed cutover procedure](CONFIGURATION.md#v3-identity-cutover) stops application writers, verifies the old-state backup, stages and verifies the v3 image, and runs its one-off tool before starting the app. Preflight reads an untouched v2.9.2 schema. Conversion applies missing prerequisite migrations and ownership removal in one transaction under the migration lock, preserving workspace paths, SQLite row IDs, and History search; failure restores the old schema and removes incomplete secret output. SQLite supports a fresh reset with staged rollback or a narrow selected-operator conversion; PostgreSQL uses the selected conversion. A startup guard rejects incomplete cutovers. Backups must match the database backend and retain the corresponding vault key and files.
  - **Development cutover:** A checksum-verified development backup can be explicitly accepted by the cutover tool without becoming eligible for managed restore. PostgreSQL can discard separately reviewed test-owner records and memberships during selected conversion. It requires exact credential, row, and membership counts, reviewed Team-snapshot IDs and recent-value counts where applicable, and refuses unexpected links, other Team-scoped work, or unreviewed owners. The selected operator's entries stay intact, file cleanup remains separate, and the preview leaves saved database state unchanged.
  - **Validation:** SQLite and PostgreSQL cover credential states, scopes, ownership isolation, Team permissions, lifecycle transactions, revocation, storage-key containment, migration and rollback, and digest-only storage. Signed-provider tests cover invalid proofs, freshness and expiry boundaries, replay, binding conflicts, and access lost before callback without changing ownership or identities. Browser journeys exercise the Access lifecycle, protected sessions, provider linking, public-share policy, and recovery in source and bundled assets. Release-schema cutover cases start at both `0077` and `0081` and verify unchanged preflight state, rollback, retained search and files, and replacement authentication.
