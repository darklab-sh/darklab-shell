...
- **Findings now include offline EPSS and CISA KEV context, with feed-driven Project Monitoring alerts.**
  ...
  - **Tests:** Focused backend coverage exercises bundle and NVD dataset validation, refresh safety, positive/negative advisory caching, last-good fallback, stale and unavailable states, ranking, queue fairness and retries, escalation hysteresis, acknowledgement scope, report provenance, configuration bounds, and privacy-safe errors. Browser unit coverage pins compact KEV/EPSS/CVSS labels and Project Monitoring acknowledgement and digest behavior; route, SQLite/Postgres migration, architecture, OpenAPI, asset, and documentation checks cover the shared schema and public contracts.
