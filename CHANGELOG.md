...
- **Fresh-image Container Smoke Test** — added opt-in wrapper scripts for the live-browser corpus capture and the Docker-backed container regression. The fresh-image `tests/py/test_container_smoke_test.py` check still builds a fresh Docker image, starts the app, and runs every command from `app/conf/auto_complete.txt` through `/run` with per-command output assertions so missing apt/pip/go/gem tools or broken fake-command wiring show up before an image update lands; the fixture also streams Docker build/start/health progress so long builds do not look idle, mounts a temporary `/app/conf` overlay so `config.local.yaml` can be injected without tripping the read-only app bind mount, and writes a JUnit report to `test-results/container_smoke_test.xml` when run directly
...
- **Container Smoke Test DinD reachability** — the smoke-test fixture now detects `tcp://...` `DOCKER_HOST` values and probes the Docker daemon host instead of hard-coding `127.0.0.1`, which keeps GitLab DinD jobs talking to the published container port instead of the job container loopback; a small unit regression in the same module locks that behavior in
