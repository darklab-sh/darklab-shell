...
- **Container Smoke Test rate-limit overlay silently ignored in DinD** — the `container_smoke_test` fixture now copies the entire `app/` tree to a temp directory, writes `config.local.yaml` into the temp copy's `conf/` subdirectory, and substitutes that temp copy for the `../app:/app:ro` volume. Previously the fixture kept the original `../app:/app:ro` mount and added a second `{tmp_conf_dir}:/app/conf:ro` overlay on top of it. Docker does not reliably honour a more-specific bind mount (`/app/conf`) when a parent path (`/app`) is already mounted: on macOS Docker Desktop the overlay was applied; on the Linux DinD runtime used in GitLab CI it was silently ignored, leaving `rate_limit_enabled: true` active and causing every `/run` request after the fifth to return 429. Using a single `/app` mount that already contains `config.local.yaml` eliminates the overlapping-mount problem across all runtimes.
...
- **Scheduled dependency drift check** — GitLab CI now has a scheduled `dependency-version-check` job that runs the local version checker and publishes its output as a short-lived artifact
- **Fresh-image Container Smoke Test** — added opt-in wrapper scripts for the live-browser corpus capture and the Docker-backed container regression. The fresh-image `tests/py/test_container_smoke_test.py` check still builds a fresh Docker image, starts the app, and runs every command from `app/conf/auto_complete.txt` through `/run` with per-command output assertions so missing apt/pip/go/gem tools or broken fake-command wiring show up before an image update lands; the fixture also streams Docker build/start/health progress so long builds do not look idle, mounts a temporary `/app/conf` overlay so `config.local.yaml` can be injected without tripping the read-only app bind mount, includes a focused `_docker_reach_host()` regression for GitLab DinD, and writes a JUnit report to `test-results/container_smoke_test.xml` when run directly
- **CI smoke job for Container Smoke Test** — GitLab CI now has a `container-smoke-test` job that runs the same fresh-image command corpus check on schedule or manually when you want to verify Dockerfile/toolchain changes before merging
