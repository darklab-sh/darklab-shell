...
- **Operator diagnostics endpoint (`/diag`)** — a new IP-gated `/diag` route in `blueprints/assets.py` returns a themed HTML summary (or JSON with `?format=json`) of app version and name, operational config values (rate limits, timeouts, output limits, retention, proxy CIDRs, log settings), SQLite run/snapshot counts, Redis connectivity, vendor-asset source (built-in vs fallback), and per-tool availability derived from the allowlist. The route returns 404 unless the direct TCP peer IP falls within the `diagnostics_allowed_cidrs` list in `config.yaml`; the list defaults to empty so the endpoint is off by default. Because it uses `request.remote_addr` directly rather than the resolved `X-Forwarded-For` client IP, `X-Forwarded-For` spoofing cannot bypass the gate — only requests whose connection originates from an allowed address are served. Denied access is logged as `DIAG_DENIED` with the peer IP and configured CIDRs; allowed access is logged as `DIAG_VIEWED`. A new `ip_is_in_cidrs()` helper was added to `helpers.py`, reusing the existing `_trusted_proxy_networks` CIDR cache.
