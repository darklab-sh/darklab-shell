...
- **Operators can inspect settings, diagnostics, and audit activity through one protected console.** Desktop and mobile navigation connects the three read-only pages.
  ...
  - **Browser sessions:** All profiles exchange portable credentials for protected cookie sessions with CSRF checks. Open browsers retain anonymous use, migrate older saved credentials once, and can return anonymously after sign-out. Workspace attachment and session creation commit together; failed exchanges stay blocked.
