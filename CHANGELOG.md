...
- **Atlas Quick Lookup browser coverage pins its dedicated mode, outcome states, and shared profile handoff.** Unit checks cover the lazy bridge, personal and project request bodies, list-free first open, New lookup and result restoration, stale candidate cancellation, scoped profile rendering, no-record actions, bounded ambiguity choices, URL-parent handoff, command prefilling without execution, copy and explicit Intel refresh actions, and the explicit transition into ordinary Atlas without adding a second overlay.
