...
```json
{
  "error": {
    "code": "missing_pat",
    "message": "API v1 requires a personal access token."
  }
}
```

Other authentication failures use the same envelope. Malformed, unknown, expired, and revoked PATs return distinct `401` codes; a disabled principal also fails closed. A valid PAT without the operation's scope returns `403 insufficient_scope`.
...
| `GET` | `/api/v1/whoami` | PAT smoke test with principal, workspace, scopes, and bounded activity metadata, without echoing the PAT. |
| `GET` | `/api/v1/principal` | Current principal and PAT context. |
| `GET` | `/api/v1/credentials` | Safe metadata for the current PAT. |
| `POST` | `/api/v1/credentials/current/revoke` | Revoke the PAT used for this request. |
...
| `GET` | `/api/v1/teams` | List teams joined by the current principal. |
...
| `PATCH` | `/api/v1/teams/<team_id>` | Archive or reactivate a team when the principal has the required role; reactivation keeps archive-paused schedules and watchers paused. |
...
| `POST` | `/api/v1/teams/<team_id>/leave` | Leave a team as the current principal. |
...
| `GET` | `/api/v1/history` | Current principal's run history with `limit`, `offset`, `project_id`, `q`, `since`, `until`, `run_kind`, and `exit_code` filters. |
...
| `GET` | `/api/v1/history/<run_id>` | One current-principal run summary. |
...
| `POST` | `/api/v1/projects/<project_id>/assessment-batches/<batch_id>/retry-previews` | Build a fresh read-only preview from the terminal source batch's failed or unfinished work. Current cycle state can remove or reshape eligible items, while successful source items stay excluded. |
| `POST` | `/api/v1/projects/<project_id>/assessment-batches/<batch_id>/retry` | Confirm a retry preview and create a new immutable batch linked to the source. Empty retries are rejected, and selected standard work needs its separate acknowledgement. |
...
| `GET` | `/api/v1/schedules` | Scheduled-command page for the principal's active personal/team scope. |
...
| `GET` | `/api/v1/watchers` | Change-detection watcher page for the principal's active personal/team scope. |
...
| `GET` | `/api/v1/notification-channels` | List masked outbound notification channels for the active personal/team scope. |
...
| `GET` | `/api/v1/runs` | Active current-principal runs. |
...
| `POST` | `/api/v1/runs/<run_id>/cancel` | Cancel an active current-principal run, including browser-started runs. |
...
`variant` is `summary` or `next_commands`. `status` is `queued`, `in_progress`, `completed`, or `failed`. `GET /api/v1/runs/<run_id>/ai-assists` returns `{"assists":[...]}` with cached and in-flight assists visible to the current principal.
...
Run, history, artifact, project, AI assist, schedule, and watcher routes are scoped to the principal's active personal/team scope. Cross-scope IDs return `404` rather than confirming the object exists elsewhere.

### CVE risk feed status

Any PAT with `projects:read`, including one used by a Team viewer, can inspect the stored EPSS and KEV sources without starting a refresh:

```bash
curl -sS -H "Authorization: Bearer $DARKLAB_PAT" \
...
PATs with the automation scopes can manage scheduled commands through `/api/v1/schedules` and `darklab schedule`. Direct API callers can add `X-Team-ID` or `team_id` to work inside a team scope they belong to; otherwise schedules use personal scope. Schedule create and update calls use the same command-policy checks as browser schedules, so saved commands are validated before they can fire later. Due schedules launch through the same brokered run path as manual runs and keep the scope they were created in; history summaries mark those runs with `scheduled: true` and the originating `schedule_id`.
...
```bash
curl -N -H "Authorization: Bearer $DARKLAB_PAT" \
...
| Command | Purpose |
| --- | --- |
| `darklab whoami [--format text\|json]` | Check PAT authentication and show safe principal, workspace, scope, and activity metadata. |
| `darklab credential status [--format text\|json]` | Show the current principal and PAT context. |
| `darklab credential list [--format text\|json\|ndjson]` | Show safe metadata for the PAT used by this CLI. |
| `darklab credential revoke [--reason TEXT] [--format text\|json]` | Revoke the current PAT without placing it in command arguments. |
...
| `darklab active [--format text\|json\|ndjson]` | List active runs for the current principal. |
...
| `darklab cancel <run_id> [--format text\|json]` | Cancel an active run in the same personal or team scope. |
...
| `darklab assessment batch retry <batch_id> [selection and concurrency options] [--confirm] [--confirm-standard] [--format text\|json]` | Build and page a current preview for failed or unfinished source work. It remains read-only without `--confirm`, creates a new linked batch on confirmation, never selects successful source commands, and requires `--confirm-standard` when standard work is selected. |
...
| `darklab completion bash\|zsh\|fish` | Print static shell completion for subcommands, nested subcommands, option names, and fixed choices. This command reads only the local CLI parser and does not require `DARKLAB_API_URL` or `DARKLAB_PAT`. |
...
2. environment variables: `DARKLAB_API_URL`, `DARKLAB_PAT`, `DARKLAB_TEAM`, `DARKLAB_TIMEOUT`
