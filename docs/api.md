...
| `darklab probe run ACTION [TARGET\|--entity-id ENTITY_ID] --project PROJECT_ID [--http-profile PROFILE_NAME_OR_ID] [--nmap-profile PROFILE] [--nuclei-profile PROFILE] [--confirm] [--workspace-cwd PATH] [--format text\|json]` | Preview one probe by default. `--confirm` immediately submits the freshly returned digest and prints the Project-linked run. |
