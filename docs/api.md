...
| `darklab probe run ACTION [TARGET\|--entity-id ENTITY_ID] --project PROJECT_ID [--http-profile PROFILE_NAME_OR_ID] [--nmap-profile PROFILE] [--nuclei-profile PROFILE] [--confirm] [--workspace-cwd PATH] [--format text\|json]` | Preview one probe by default. In text mode, `--confirm` prints the freshly returned plan before submitting its digest and printing the Project-linked run. |
