...
Prometheus tracks active Assessment cycles by personal or team owner and a capped profile key, check transitions by bounded state and manual or derived source, derived evidence matches by type and outcome, action outcomes by fixed action and policy, parser outcomes, Project probe operations by fixed phase, outcome, and credential class, and ZAP/OAST operations and durations by fixed connector phase and outcome. The label contracts don't accept Project, assessment, check, run, job, correlation, workflow, target, command, CVE, callback, profile, or provider values. Unknown values fold into fixed fallback labels, and scrape-time profile keys are normalized and capped rather than growing with stored data. Probe activity is exposed as `darklab_probe_operations_total{phase,outcome,credential_use}`.
