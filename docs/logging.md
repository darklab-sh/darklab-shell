...
Prometheus tracks active Assessment cycles by personal or team owner and a capped profile key, check transitions by bounded state and manual or derived source, derived evidence matches by type and outcome, action outcomes by fixed action and policy, parser outcomes, Project probe operations by fixed phase, outcome, and credential class, Assessment batch activity, queue depth, retained outcomes, launch latency, revalidation rejection class, concurrency deferrals and recovery, and ZAP/OAST operations and durations by fixed connector phase and outcome. The label contracts don't accept Project, assessment, batch, check, run, job, correlation, workflow, target, command, CVE, callback, profile, or provider values. Unknown values fold into fixed fallback labels, and scrape-time profile keys are normalized and capped rather than growing with stored data. Probe activity is exposed as `darklab_probe_operations_total{phase,outcome,credential_use}`.
...
| DEBUG | `METRICS_ASSESSMENT_BATCH_STATE_COLLECT_FAILED` | Prometheus assessment-batch state collector | (+ traceback) |
...
| INFO | `PROJECT_ASSESSMENT_BATCH_STARTED` / `API_ASSESSMENT_BATCH_STARTED` | browser/API confirmed assessment-batch start | masked session, team_id, project_id, assessment_id, batch_id, item_count, launched_count |
| INFO | `PROJECT_ASSESSMENT_BATCH_RETRY_STARTED` / `API_ASSESSMENT_BATCH_RETRY_STARTED` | browser/API confirmed immutable assessment-batch retry | masked session, team_id, project_id, assessment_id, source_batch_id, batch_id, item_count, launched_count |
| INFO | `PROJECT_ASSESSMENT_BATCH_CANCEL_REQUESTED` / `API_ASSESSMENT_BATCH_CANCEL_REQUESTED` | browser/API assessment-batch cancellation | masked session, team_id, project_id, assessment_id, batch_id, batch_status, signal_failure_count |
| INFO | `ASSESSMENT_BATCH_COMPLETED` | one post-commit terminal assessment-batch outcome | batch_id, batch_status, duration_ms, fixed reason_code, and bounded succeeded, failed, unavailable, canceled, and could_not_cancel totals; no command, target, profile material, or finding content |
| DEBUG | `ASSESSMENT_BATCH_LAUNCH_DEFERRED` | one concurrency decision per assessment-batch launch-fill cycle | batch_id, fixed reason_code, launched_count; no target id, target value, command, or profile material |
| WARNING | `ACTIVE_ASSESSMENT_BATCH_MONITOR_ERROR` | optional owner-scoped Status Monitor batch read | request-safe context and traceback; private credentials, commands, and targets stay excluded |
| WARNING | `ASSESSMENT_BATCH_CANCEL_SIGNAL_FAILED` / `ASSESSMENT_BATCH_CANCEL_SIGNAL_REJECTED` | an active batch child couldn't be signaled or the run boundary explicitly rejected the request | batch_id, run_id, masked session, team_id, and exception class for raised failures; no command or target value |
| ERROR | `ASSESSMENT_BATCH_ITEM_MATERIALIZE_ERROR` / `ASSESSMENT_BATCH_FINALIZE_ERROR` | unexpected batch item preparation or coordinator-finalization failure | batch_id, bounded item index or run_id, stage, exception class, and traceback; commands, target values, and protected material stay excluded |
| ERROR | `ASSESSMENT_BATCH_NOTIFICATION_ERROR` | terminal assessment-batch summary couldn't be queued after coordinator settlement | batch_id, masked session, exception class, and traceback; commands, targets, profile details, and protected material stay excluded |
| INFO / WARNING / ERROR | `ASSESSMENT_BATCH_RECOVERY_COMPLETED` / `ASSESSMENT_BATCH_RECOVERY_CHILD_MISSING` / `ASSESSMENT_BATCH_RECOVERY_FAILED` / `ASSESSMENT_BATCH_RECOVERY_ERROR` | bounded startup recovery summary, missing child classification, durable failure, or per-row recovery error | bounded recovery counts or batch_id, reason code, attempt, run_id, and exception class where relevant; no command or target value |
| INFO | `ASSESSMENT_BATCHES_PRUNED` | startup retention cleanup for terminal assessment-batch coordinator state | count, retention_days |
| DEBUG / INFO / WARNING | `PROJECT_PROBE_OPERATION_COMPLETED` | Successful catalog and available-plan reads use DEBUG, and successful launches use INFO. A returned plan with `launchable: false` also uses this event with `probe_outcome=unavailable`: reviewed policy or target-type unavailability uses INFO, while operational degradation uses WARNING. This event doesn't report protected cleanup. | bounded source, request_id, masked session, team_id, project_id, entity_id, canonical action_id, stable availability error_code, policy_level, and successful run_id plus probe_phase, probe_outcome, and protected; invalid identifiers fold to safe empty or unknown values, and no availability reason, target value, profile id, command, or private material is logged |
| INFO / WARNING | `PROJECT_PROBE_OPERATION_REJECTED` | Raised validation, lookup, availability, and launch rejections use this event. Ordinary input and missing resources plus an intentionally disabled run broker use INFO; conflicts, rate limits, unavailable dependencies, and policy or capability rejections use WARNING. | bounded source, request_id, masked session, team_id, project_id, entity_id, canonical action_id, and stable error_code plus probe_phase, probe_outcome, and protected; broker_disabled and broker_dependency_unavailable distinguish configuration from an outage without recording the free-form reason, and no target value, profile id, command, or private material is logged |
| ERROR | `PROJECT_PROBE_OPERATION_FAILED` | run-spawn or unexpected Project probe service failure | probe_phase, probe_outcome, project_id, entity_id, action_id, protected, fixed error_code, error_class, sanitized origin-only traceback; no exception message, chained cause, target value, profile id, command, or private material |
| DEBUG | `PROJECT_PROBE_TARGET_RESOLVED` | exact Project probe target-value resolution | bounded source, request_id, masked session, team_id, project_id, entity_id, target_type, fixed selector_kind, candidate_count, and duration_ms; the submitted and canonical target values are excluded |
| INFO / WARNING | `PROJECT_PROBE_TARGET_REJECTED` | ordinary exact-target misses use INFO; ambiguity, archived state, and unsupported target state use WARNING | bounded source, request_id, masked session, team_id, project_id, fixed selector_kind, candidate_count, duration_ms, and stable error_code; target values are excluded |
| ERROR | `PROJECT_PROBE_TARGET_RESOLUTION_FAILED` | unexpected exact-target resolution failure | the same bounded target-resolution context and a sanitized origin-only traceback; exception messages and target values are excluded |
| DEBUG / ERROR | `PROJECT_PROBE_PROTECTED_CLEANUP_COMPLETED` / `PROJECT_PROBE_PROTECTED_CLEANUP_FAILED` | The only Project-probe cleanup events. They cover retryable protected launch-material cleanup from post-materialization validation through run completion; completion is emitted only after confirmed deletion. | bounded project_id, entity_id, action_id, profile_id, cleanup_stage, and error_class; invalid identifiers fold to safe empty or unknown values, and raised failure uses a sanitized origin-only traceback with no exception message, path, target, command, or private material |
| ERROR | `HTTP_PROFILE_RUNTIME_MATERIAL_CLEANUP_FAILED` | local or scanner-owned protected runtime cleanup that remains available to retry | material_count; no path, command, target, profile, or private material |
| WARNING / ERROR | `PROJECT_PROBE_CLIENT_REQUEST_FAILED` | browser-terminal probe catalog, exact-target resolution, plan, and confirmed-launch failures; network and HTTP failures use WARNING, while invalid client response shapes use ERROR | fixed page and source plus bounded phase, Project id, HTTP status, and error class; target values, commands, response bodies, profile ids, URLs, query strings, and private material are excluded |
| INFO | `PROJECT_PROBE_LAUNCHED` / `API_PROJECT_PROBE_LAUNCHED` | browser/API confirmed one-off probe launch | ip, request_id, source, masked session, team_id, project_id, entity_id, action_id, policy_level, run_id, HTTP-profile id when selected; browser also records whether an owner tab id was present |
...
| WARNING | `ASSESSMENT_PROFILE_CATALOG_RELOAD_REJECTED` | changed required shipped profile catalog rejected in favor of the last valid catalog | source, path, fixed error_code, error_class; no rejected values or traceback |
