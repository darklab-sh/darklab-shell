...
| DEBUG / INFO | `PROJECT_PROBE_OPERATION_COMPLETED` | Project probe catalog, plan, confirmation, launch, and protected cleanup services; launch uses INFO | bounded project_id, entity_id, canonical action_id, and stable error_code plus probe_phase, probe_outcome, and protected; invalid identifiers fold to safe empty or unknown values, and no target value, profile id, command, or private material is logged |
| INFO / WARNING | `PROJECT_PROBE_OPERATION_REJECTED` | ordinary Project probe input and missing-resource rejections use INFO; conflicts, rate limits, service unavailability, and policy or capability rejections use WARNING | bounded project_id, entity_id, canonical action_id, and stable error_code plus probe_phase, probe_outcome, and protected; invalid identifiers fold to safe empty or unknown values, and no target value, profile id, command, or private material is logged |
