...
| INFO | `PROJECT_PROBE_LAUNCHED` / `API_PROJECT_PROBE_LAUNCHED` | browser/API confirmed one-off probe launch | ip, session, team_id, project_id, entity_id, action_id, policy_level, run_id, HTTP-profile id when selected; browser also records whether an owner tab id was present |
