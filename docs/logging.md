...
| DEBUG / INFO | `PROJECT_PROBE_OPERATION_COMPLETED` | Project probe catalog, plan, confirmation, launch, and protected cleanup services; launch uses INFO | probe_phase, probe_outcome, project_id, entity_id, action_id, protected, error_code; no target value, profile id, command, or private material |
| INFO / WARNING | `PROJECT_PROBE_OPERATION_REJECTED` | ordinary Project probe input and missing-resource rejections use INFO; conflicts, rate limits, service unavailability, and policy or capability rejections use WARNING | probe_phase, probe_outcome, project_id, entity_id, action_id, protected, error_code; no target value, profile id, command, or private material |
...
| DEBUG / ERROR | `PROJECT_PROBE_PROTECTED_CLEANUP_COMPLETED` / `PROJECT_PROBE_PROTECTED_CLEANUP_FAILED` | retryable protected launch-material cleanup from post-materialization validation through run completion; completion is emitted only after confirmed deletion | project_id, entity_id, action_id, profile_id, cleanup_stage, bounded error_class; raised failure uses a sanitized origin-only traceback with no exception message, path, target, command, or private material |
