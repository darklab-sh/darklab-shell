...
| `nuclei` | Adds `-ud /tmp/nuclei-templates` when no update-directory flag is present, wraps normal ProjectDiscovery config state with `XDG_CONFIG_HOME=<active workspace>/tools` when Files are enabled, and declares workspace paths for response stores, Markdown/SARIF/JSON/JSONL exports, trace/error logs, resume files, and selected config/secret inputs. Explicit template updates don't inherit the workspace config marker. | Template storage must be writable under the read-only container filesystem, while useful evidence and logs should be visible in Files without exposing template caches as artifacts. Keeping update metadata with the temporary cache prevents retained workspace state from suppressing a required download after container recreation. |
