...
| `nuclei` | Adds `-ud /tmp/nuclei-templates/current` when no update-directory flag is present, wraps normal ProjectDiscovery config state with `XDG_CONFIG_HOME=<active workspace>/tools` when Files are enabled, and declares workspace paths for response stores, Markdown/SARIF/JSON/JSONL exports, trace/error logs, resume files, and selected config/secret inputs. | Template storage must be writable under the read-only container filesystem, while useful evidence and logs should be visible in Files without exposing template caches as artifacts. The managed cache and its update metadata stay together in the persistent Compose volume; normal command config and resume state remain owner-scoped. |
| `dalfox` | Uses the required v3 `scan` subcommand, adds `--only-discovery --skip-mining-dict` for normal runs, blocks server/file/pipe/callback/proxy/redirect/payload/remote-wordlist modes, and accepts protected Assessment headers only through a private app-owned config. | Parameter discovery is useful evidence, but active XSS payloads and scope-broadening features need a separate reviewed contract. Protected header values must stay out of visible commands and saved history. |
| `sqlmap` | Accepts one credential-free HTTP(S) URL and reviewed detection options only, then adds fixed detection level, risk, technique, request, thread, and time bounds. Assessment launches validate the same narrow carrier before private profile material is appended. | Detection can provide useful evidence, while extraction, file, script, configuration, multi-target, and takeover paths must stop before process launch. |
...
| `gau` | Declares `--config` as a Files input and `--o` as a Files output; custom proxy arguments remain blocked. | Historical URLs are passive leads that should stay owner-scoped and reviewable before a separate live/scope check sends them to an active scanner. |
...
| `httpx` | Wraps ProjectDiscovery config state with `XDG_CONFIG_HOME=<active workspace>/tools` when Files are enabled and declares workspace paths for list/raw-request inputs, normal outputs, response/screenshot store directories, and config files. Maintained screenshot commands use the image's Chromium directly. | Response stores and screenshots are high-value evidence, while config state should remain visible only to the active personal/team owner. Using the bundled browser avoids downloading executables into temporary storage at run time. |
...
`nuclei` still receives `-ud /tmp/nuclei-templates/current` unless the user provides an update directory. The managed cache lives in its Compose volume because it is reusable deployment state rather than session evidence.

Nuclei output metadata records template provenance for each run. The app classifies the source as the managed `/tmp/nuclei-templates/current` cache, an actual session workspace template path passed through `-t`, a pinned-looking `nuclei-templates` clone path, an operator-updated template set, or a custom update directory. Normal relative template selectors such as `http/`, `cves/`, or `ssl/...` are treated as managed-cache selectors, not workspace templates. Saved output, Run Details restores, and Nuclei JSONL Atlas imports keep this provenance with the line or import `source_detail`, so later review can tell which template source produced the finding.
