...
describe('project assessment controller', () => {
  ...
  it('renders the same truthful coverage and target worklist on desktop and mobile', async () => {
    ...
    for (const surface of [desktop, mobile]) {
      ...
      expect(surface.querySelector('[aria-label="Policy filter"]')?.textContent)
        .toBe('PolicyAll policiesSafeStandardIntrusiveDestructive')
      expect(surface.querySelector('[aria-label="Evidence filter"]')?.textContent)
        .toBe('EvidenceAll evidenceAvailableUnavailableNo evidence')
      expect(surface.querySelector('[aria-label="Policy filter"] .chip.is-active')?.textContent)
        .toBe('All policies')
      expect(surface.querySelector('[aria-label="Evidence filter"] .chip.is-active')?.textContent)
        .toBe('All evidence')
      expect(surface.querySelector('.project-assessment-clear-filters')?.disabled).toBe(true)
      expect(surface.querySelector('.project-assessment-check-filter-status')?.textContent)
  ...
  it('preserves cycle filters, paging, disclosure, and scroll state per project', async () => {
    ...
    await controller.setFilter('prj_1', 'policy_level', 'intrusive')
    await controller.setFilter('prj_1', 'evidence_state', 'unavailable')
    ...
    expect(detailUrls.at(-1)).toContain('policy_level=intrusive')
    expect(detailUrls.at(-1)).toContain('evidence_state=unavailable')
    ...
    expect(state.policyLevel).toBe('intrusive')
    expect(state.evidenceState).toBe('unavailable')
    ...
    controller.renderAssessment(container, 'prj_1')
    await controller.load('prj_1')
    controller.renderAssessment(container, 'prj_1')
    const clearFilters = container.querySelector('.project-assessment-clear-filters')
    expect(clearFilters.disabled).toBe(false)
    expect(container.querySelector('.project-assessment-check-filter-status').textContent)
      .toBe('4 filters applied')
    clearFilters.click()
    await vi.waitFor(() => expect(state.detailLoading).toBe(false))
    expect(state.category).toBe('')
    expect(state.checkState).toBe('')
    expect(state.policyLevel).toBe('')
    expect(state.evidenceState).toBe('')
    expect(state.offset).toBe(0)
    expect(state.checksScrollTop).toBe(0)
    expect(state.expandedTargets.size).toBe(0)
    const clearedUrl = projectWorkspaceRequest.mock.calls.map(([url]) => url).at(-1)
    expect(clearedUrl).not.toContain('category=')
    expect(clearedUrl).not.toContain('state=')
    expect(clearedUrl).not.toContain('policy_level=')
    expect(clearedUrl).not.toContain('evidence_state=')
  })
