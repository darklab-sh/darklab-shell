...
async function expectSplitCompareRendered(page, fixture, { projectId = '' } = {}) {
  ...
  await expect(overlay.locator('.history-compare-counts')).toContainText('unchanged 8')
  await expect(overlay.locator('.history-compare-counts')).toContainText('changed 1')
  await expect(overlay.locator('.history-compare-counts')).toContainText('added 1')
...
test.describe('history drawer', () => {
  ...
  test('run comparison split view works from history and project entry points', async ({ page }, testInfo) => {
    ...
    const historyCountsText = await compareOverlay.locator('.history-compare-counts').textContent()
    ...
    expect(await compareOverlay.locator('.history-compare-counts').textContent()).toBe(historyCountsText)
