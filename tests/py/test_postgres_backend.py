...
def test_postgres_rejects_legacy_identity_database_without_removing_credentials(postgres_schema, schema_head):
    from core.migrations import MIGRATIONS
    from core.migrations.runner import applied_versions, run_migrations_with_advisory_lock

    conn = postgres_schema.conn
    run_migrations_with_advisory_lock(conn, tuple(m for m in MIGRATIONS if m.version <= schema_head))
    conn.execute("INSERT INTO session_tokens (token, created, last_seen_at) VALUES (%s, %s, %s)",
                 ("tok_unsupported_backup", "2026-09-15", "2026-09-15"))
    conn.commit()
    psycopg = pytest.importorskip("psycopg")
    with pytest.raises(psycopg.errors.RaiseException, match="legacy session identities are not supported"):
        run_migrations_with_advisory_lock(conn, MIGRATIONS)
    assert "0082" not in applied_versions(conn)
    assert conn.execute("SELECT token FROM session_tokens").fetchone()["token"] == "tok_unsupported_backup"


@pytest.mark.postgres
@pytest.mark.parametrize("legacy_shape", ["table", "column"])
def test_postgres_startup_schema_rejects_retired_identity_state(postgres_schema, legacy_shape):
    ...
    validate_startup_schema(conn, DatabaseBackend.POSTGRES)
    if legacy_shape == "table":
        conn.execute("CREATE TABLE session_tokens (token TEXT PRIMARY KEY)")
    else:
        conn.execute("CREATE TABLE obsolete_owners (session_token_hash TEXT)")
