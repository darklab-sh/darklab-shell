...
def test_email_channel_rejects_missing_smtp_transport(monkeypatch):
    monkeypatch.setattr("services.notifications.channels.email.database.CFG", {"notifications": {}})
...
def test_email_channel_requires_recipients(monkeypatch):
    ...
    monkeypatch.setattr("services.notifications.channels.email.database.CFG", {"notifications": {"smtp": _smtp_config()}})
...
def test_email_channel_uses_smtp_ssl_without_starttls(monkeypatch):
    ...
    monkeypatch.setenv("DARKLAB_SMTP_PASSWORD", "smtp-secret")
    monkeypatch.setattr("services.notifications.channels.email.database.CFG", {"notifications": {"smtp": config}})
