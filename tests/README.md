...
External CLI coverage also pages typed Nmap service evidence, reads stored EPSS/KEV feed status, and starts one explicit exact-package OSV lookup. It distinguishes stale and unavailable sources, exercises each command's text, JSON, and NDJSON formats, limits OSV requests to PURL and version, rejects empty values locally, gives separate guidance for Team permission, disabled mode, and provider failure, preserves public API error status/code/details for lifecycle-aware handlers, and keeps focused parser, handler, schema, route, and shared error modules under architecture budgets. Feed-status tests also prove Team-viewer access and no refresh or feed-row writes on SQLite and PostgreSQL.
