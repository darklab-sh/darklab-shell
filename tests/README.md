...
Cutover backup checks keep managed archives as the default, allow an explicit development archive only in the cutover tool, and still reject it for managed restore. SQLite and PostgreSQL cutover cases start at both the v2.9.2 schema (`0077`) and the prepared schema (`0081`). They check unchanged preflight state, prerequisite migrations inside conversion, rollback to the original schema, stable History search and workspace files, and one-time replacement authentication. The PostgreSQL development preview also proves its temporary schema changes are rolled back.

Principal-persistence coverage exercises the SQLite and Postgres schema, one-time credential issuance, concurrent first-time SQLite upgrades under one verifier root, digest-only storage, encrypted verifier-root and browser-session signing-key rotation and rewrapping, immutable workspace keys, safe path resolution, credential, principal, and browser-session lifecycle operations, bounded last-used writes, backup and SQLite-to-Postgres copying, and rollback after injected anonymous-workspace attachment failures. Principal-access contract tests cover anonymous attachment, credential redemption, current-principal and safe-credential reads, every fail-closed credential state, the complete PAT scope set, route-level scope enforcement, PAT-only CLI configuration and self-revocation, terminal secret rejection, and container-only operator bootstrap, issuance, recovery, expiry, rotation, revocation, disable, and enable behavior. Restricted-profile cases pin the exact public allowlist, anonymous and issuance rejection, standalone sign-in, cookie flags, CSRF, fixation-safe rotation, idle and absolute expiry, logout, principal-wide revocation, parent-credential cascades, share policy, Team privilege changes, restart continuity, and open-profile compatibility. Ownership-cutover cases additionally prove that an authenticated context owns through its personal-workspace id while retaining separate principal and credential attribution, anonymous attachment keeps its existing directory, SQLite `runs.rowid` and FTS search remain stable, Postgres conversion rolls back and commits atomically under the migration lock while preserving History search, and the development-only discard review rejects unexpected data, requires exact Team-snapshot IDs and Team-recent-value counts, checks links, preserves selected-owner entries, and rolls back test-owner cleanup on failure. They also pin Team roles through principal membership, public share reads as capability-based, workspace-owned share mutations, and rotation or revocation without data movement.
