...
Principal-persistence coverage exercises the additive SQLite and Postgres schema, one-time credential issuance, digest-only storage, encrypted verifier-root rotation and rewrapping, immutable workspace keys, safe path resolution, credential and principal lifecycle operations, bounded last-used writes, backup and SQLite-to-Postgres copying, and rollback after injected anonymous-workspace attachment failures. The post-cutover schema guard is also covered in its inactive current state and its explicit legacy-schema rejection mode.
