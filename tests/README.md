...
Project Assessment coverage pins its lazy entry, safe profile picker, desktop/mobile rendering parity, truthful coverage states, target disclosures, filters, paging, per-Project scroll and expansion state, profile-driven cycle creation, and view-only permissions. Browser-route coverage separately checks that the picker receives only bounded profile summaries rather than complete live definitions.
