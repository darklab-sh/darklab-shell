...
Access coverage keeps one-time credentials masked until requested and removes them from the DOM on close. It also exercises invalid and out-of-order responses, safe metadata for every credential state, durable-work review before revocation, and replacement-first rotation.
