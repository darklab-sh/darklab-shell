...
Project, Assessment, and Atlas owner-query coverage applies the mixed fixture to strict empty-team personal rows, nullable legacy rows, unfiltered personal-only tables, team-owned data, and flattened metadata-owner keys on both databases. The review-ledger test accounts for all 156 original sites, requires the 116 adapter conversions to remain behavior-preserving, and keeps the 40 exact relational and key-shape exceptions named and audited. Broader subsystem cases cover reads, writes, imports, exports, evidence and report persistence, cleanup, retention, and worker-triggered updates through the shared owner boundary.
...
- Use the shared helpers in `tests/py/identity_helpers.py` for server-side requests that need an identity. `anonymous_session_id()` creates a valid anonymous UUID, `durable_session_token()` stores a real legacy token through the application service, and `identity_client_factory` keeps the same identity across related client requests. For principal-backed coverage, `principal_identity()` creates one personal workspace with a portable browser credential and a scoped PAT; use `browser_identity_headers()` for browser routes and `api_identity_headers()` for API v1 routes.
