...
Private OAST configuration coverage keeps the connector disabled by default, validates its private HTTPS origin, exact callback suffix, environment-only token reference, bounded retention, and privacy acknowledgement, and proves settings reads have no provider side effects.
