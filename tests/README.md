...
Options coverage includes preference loads that overlap control refreshes, newer workspace loads, local edits, and the sign-in handoff to Access. The Access browser tests simulate a shrinking and panning visual viewport to check that credential fields stay visible and the backdrop still covers the header. Provider-only mobile qualification also checks backdrop dismissal after credential creation. Desktop browser emulation doesn't open an iPhone keyboard, so verify focus zoom and keyboard dismissal on an actual iPhone when changing these behaviors.
...
Project workspace behavior follows the same split: pytest owns project routes, schema, migration, assessment profiles and cycle services, the shared bounded-action registry, exact Project target resolution, IPv6-safe probe URLs, typed probe OpenAPI payloads and examples, probe-plan digests, anonymous and protected probe launches, privacy-safe probe failure logs, observed and retryable protected-material cleanup across prelaunch and run lifecycle failures, cross-cycle finding reconciliation, shared active-cycle, coverage, fix-first, finding-change handoffs, assessment-batch parent/event schema, safe-default preview compilation, exact plan deduplication and frozen-check mappings, managed Nuclei template health, scanner-prefixed validation, protected-file-safe scan locking, locked refresh, launchable rebuild enforcement, snapshot-pinned batch start, aggregate failure diagnosis, atomic confirmed-start snapshots and replay, immutable retry previews and source lineage, serialized batch/target/owner/instance claim limits, fixed concurrency and chunking limits, single-cycle fairness deferral logs, exact item revalidation, Project-linked heterogeneous child launches, notification suppression, sanitized finalization events, post-commit terminal operator milestones, cross-chunk advancement, truthful active-run cancellation, failed-signal rollups, lifecycle cancellation conflicts and fresh-request behavior, kind-isolated startup recovery, abandoned-claim release, live-run attachment, bounded missing-run retries, scope, runtime, and permission-loss settlement, deterministic progress with distinct skipped outcomes, bounded list/detail/item/event routes, latest-attempt paging, owner-isolated read cursors, owner-scoped Status Monitor projections, bounded external ZAP jobs and prepared Atlas draft reads, private OAST reservation, ready-only launch, run binding, and redacted interaction evidence, Web Surface reads, filters, and visual comparisons, overview and monitoring payloads, packages, reports, history/share integration, public CVE risk and NVD advisory storage, Project/no-Project risk projections, and persistence edge cases; Vitest owns Projects modal, Entity select-mode scroll preservation, Assessment coverage, probe parsing, terminal output, per-tab confirmation, lifecycle invalidation, and same-tab run binding, private OAST recovery, polling, confirmation, and redacted status rendering, Assessment batch Nuclei preflight, update/retry controls, in-page draft restoration, in-place planner and polling refresh, and Status Monitor rendering, ZAP plan confirmation, job recovery, and explicit Atlas import review/apply, fix-first, finding-change rendering, the Web Surface gallery with its shared filters, comparison states, grouping, and full-image navigation, shared Overview/Findings summaries, exact-cycle and priority-filter navigation, desktop/mobile Monitoring tab rendering, compact finding-risk labels, history drawer, Files metadata, and package-wizard browser behavior; Playwright covers full user flows when focus, navigation, or live browser state is the important risk, including Project Entity row selection that retains its reading position, bounded Assessment preview and template-update loading states that retain their scroll position, start, managed Nuclei preflight in source and bundle modes, Status Monitor handoff, poll-safe scroll and focus, reload, cancellation, and retry lineage, lazy mobile-width probe planning, declined and reload-invalidated confirmations, a harmless confirmed probe that streams in the same tab and reaches History and its Project, authenticated Web Surface image retrieval and decoding with keyboard-safe full view and package handoff, plus mobile risk acknowledgement that stays separate from watcher triage. Interactive PTY behavior is split between pytest service/route coverage, Vitest browser-controller coverage, and focused Playwright checks for the real terminal modal path.
...
- Compose Postgres runs reuse a private dependency environment under `$DARKLAB_TEST_COMPOSE_VENV_PARENT/darklab-postgres-test-venvs` (the parent defaults to `/data`). Requirements, the preparation helper, Python/ABI, OS release, libc, and installed base-image package versions determine the cache key. A lock protects construction; interrupted installs are rebuilt, and completed environments can serve concurrent runs. Remove this test-only cache while no Compose test runs are using it to resolve dependency ranges again or reclaim old entries. The helper doesn't migrate the shared base database: fixtures own their isolated schemas or databases.
...
- Bundle-mode runs check `npm run assets:fresh` before starting servers. The recorded fingerprint includes all static inputs, build scripts, configuration, npm manifests, the installed esbuild version, and Node major compatibility. The gate checks generated-file membership and hashes plus decompressed sidecar integrity; changes require `npm run assets:sync`. CI lint still runs the full build and working-directory reproducibility checks.
- Browser CI caches package downloads and headless Chromium under a lockfile/CI-configuration key, with runner and platform separation. `npm ci` still installs the locked tree, and Playwright still installs required OS dependencies. A cold cache downloads the same pinned browser; headed local/capture workflows retain their normal browser installation.
...
- Spec weights come from measured CI durations, include the first open project's fixed qualification work, and warn when a new spec uses the fallback. Focused helper commands collect the requested selection first and start only its isolated servers. Project ports, profiles, and test assignment stay stable; a failed preselection falls back to the full server set and normal Playwright error reporting. Interactive UI/debug runs keep all configured servers available. Timing summaries distinguish the configured worker cap from peak busy workers.
...
Operator navigation, settings controls, and verification/revocation run as independent browser journeys. Access separates credential management, restoration across tabs, and invalid-input checks; restricted sign-out starts both independent browser sessions before testing revocation. The mobile keyboard journey holds the Access module download to verify that interaction waits for the disabled control to become usable. Comparison entry points, assessment planning/monitoring/retry, output-menu input methods, and logout recovery also run independently. Fresh browser contexts avoid unnecessary identity-reset reloads. Operator navigation waits for the settings response, and managed sign-in waits for navigation and shell readiness before checking Access. The assessment monitor test holds a scheduled poll, focuses its control, then releases the response and checks focus and scroll after the old monitor is replaced. These cases keep the existing timeout limits and CI's flaky-test guard.

Backup tests cover named-volume exports for app data and Files, including successful copies and rejection of missing or inaccessible sources before an export starts. Disposable-container cleanup removes attached anonymous volumes as well as containers; smoke-test recovery also handles anonymous volumes without Compose project labels.

Operator console qualification adds shared HTTP and failed-credential budgets for denied requests, exact page/data/verification denials, expired-form recovery and provider-freshness guidance, metrics independence, PAT and Team-role rejection, grant revocation, safe disclosure, `/diag` parity, and actor attribution on SQLite and Postgres. Real isolated processes prove loaded snapshots ignore later YAML/environment edits. Vitest covers literal rendering, asynchronous group synchronization, keyboard option updates, combined filters, URL restoration through verification, clearing, category counts and expansion restoration, nested disclosure and focus state during delayed refreshes, failed refreshes, safe returns, and out-of-order responses. `operator-console.spec.js` runs desktop/mobile credential and provider journeys in both asset modes and the PostgreSQL profile matrix. Styled-menu mouse and keyboard checks cover each filter with and without search, category and value disclosures, delayed-refresh focus/scroll, and dark/light captures. Header checks follow links between all three operator pages, omit the current page, preserve mobile touch targets, and verify that desktop settings and audit menu links leave the shell open without exposing a window opener. Audit menu checks cover desktop keyboard activation, mobile touch navigation, and absence for ineligible sessions. Its local fixture tool changes only each isolated server's database; it adds no HTTP fixture endpoint.

Operator diagnostics coverage checks every route representation, shared grant and session denials, network independence, private responses, safe verification returns, CSRF, and revocation during CSV/JSON streaming, including incomplete-CSV markers, safe ERROR records for unavailable live checks, and suppression of later rows and success events. Route tests supply deterministic command rows and verify source/bundle module loading; browser unit tests exercise click, Enter, and Space expansion after the cells are replaced by a refresh. Configuration cases cover the metrics alias, canonical empty lists, per-layer precedence, single-key inspection, truthful provenance, withheld-versus-unset diagnostic rendering, and warning redaction. Desktop/mobile browser journeys cover diagnostics layout and timezone handling, explicit AI probes and fair global/per-operator quota accounting, audit-filter preservation, real Project-link audit rows and actor details, guarded downloads, and clearing data after revocation in source and bundle modes on SQLite and Postgres.
...
The default Postgres helper includes the operator diagnostics module. Its CSV and JSON export cases revoke the grant or browser session, expire verification, or fail the authority lookup between pages; later rows never escape and no completion event is emitted.
...
`test_credential_throttle.py` covers failure limits before verification for portable credentials, PATs, browser cookies, redemption, and the sign-in form. It checks independent IP and lookup limits, reset windows, Redis and local counters, and successful requests that leave the allowance intact. Warning checks cover anonymous issuance, rejection after concurrent attempts exhaust a limit, exact HTTP status and policy fields, text/GELF redaction, concurrent sampling, bounded reason keys, and duplicate request boundaries.
...
The Team panel unit suite holds a list refresh open and checks that the active form, focused field, text selection, and Submit button remain usable. `team-mode.spec.js` types into the focused Team name during a delayed refresh, then exercises creation and invite redemption through real pointer input and checks the join response before reading the refreshed list.

Restricted Access browser checks exercise the local and session-wide sign-out buttons, including rejection of a second browser after session-wide sign-out. The revoked-tab journey revokes through an independent request context after starting a protected read, so an earlier background 401 can redirect the app without leaving the test clicking absent controls. It restores stale cookies on the sign-in page to verify explicit logout clears them. Unit checks cover provider-specific copy, recency-based provider controls, and recovery when link or unlink outlives the authentication window. Python request tests age the real session and verify reauthentication; the local-provider browser journey uses an older-session response fixture, then verifies real credential sign-in and return to Access.

Provider-only qualification creates an API token through Access at desktop and mobile widths. Request tests reject self-service portable issuance and both rotation modes, retain operator recovery issuance, and verify usable PAT creation and rotation.
...
Access panel unit tests cover disabled provider support, authentication and HTTP failures, network errors, malformed JSON and payloads, refresh recovery, and stale request suppression. `oidc-access.spec.js` exercises the retry controls on desktop and narrow screens in both asset modes and verifies safe diagnostics delivered through the real `/log` endpoint.
...
The trust-directory fork check runs in a fresh Python process so it doesn't inherit pytest's background threads. It exercises the real child-reset hook, verifies that the child preserves the parent's directory and that parent exit removes it, and treats deprecation warnings and child exceptions as failures. A timeout stops both processes if the check hangs.

`test_export_authorization_logging.py` checks every denied background authorization state before, during, and after package and report builds. It verifies persisted failure status, one terminal warning, separate safe errors for unavailable checks, and text/GELF privacy. Real package builds exercise typed exception propagation and ZIP cleanup, including principal disablement during the final progress callback; ordinary exporter failures retain their existing error records. Cleanup cases distinguish absent files from permission and I/O failures, verify private retry metadata and download denial, and exercise failed and successful retention and discard retries.

`test_credential_limiter_logging.py` injects Redis read, increment, conversion, and expiry failures across all fixed counter namespaces. It checks local enforcement, one warning during repeated or concurrent failures, recovery after every failed operation succeeds, fixed-size diagnostic state, quiet intentional local operation, and text/GELF privacy.

`test_oidc_link_boundaries.py` uses real signed provider tokens and a shared callback clock for request and session resolution. It checks missing or mismatched state cookies, expired flows, multi-audience authorized-party claims, subject validation, provider freshness, callback-time access changes, and exact valid boundaries. Rejected callbacks preserve existing identities, sessions, credentials, and workspace ownership. The PostgreSQL suite runs the same source-authorization and conflicting-binding matrix against its isolated database.

`test_oidc_logging.py` checks discovery and CA failures, real OAuth client response parsing and status capture, unusable signing keys, signed-token claim rejections, callback and link storage failures, warning sampling, and DEBUG stage completion. Its isolated databases keep provisioning tests independent. Text and GELF checks exclude provider details, credentials, state, responses, and exception chains.

`test_credential_lifecycle_logging.py` checks committed INFO milestones for anonymous upgrades, operator bootstrap and recovery, provider provisioning, credential creation and rotation, revocation, and principal status. Separate database connections verify that records see committed data. Commit and replacement failures emit no success records, repeated operations stay quiet, audit-disabled deployments retain milestones, and text/GELF checks exclude private input. A real PostgreSQL case checks committed visibility, concurrent revocation, and recovery rollback. Its helper verifies an authenticated context before exercising lifecycle operations; log-capture callbacks keep the `logging.Handler.emit(record)` signature.
...
`test_logging.py` imports configuration in fresh processes to verify one fatal startup record for invalid OIDC policy, scopes, allowlists, required settings, URLs, and inconsistent browser-session limits. Text and GELF cases check fixed reasons and source attribution while keeping private configuration values out of the output. Successful startup cases cover every access profile, each provisioning mode, and default or explicit sharing policy, including the session-limit values in the existing INFO summary.

`test_log_streams.py` checks every application severity and configured threshold in text and GELF, one emission after repeated configuration, exception routing, native Gunicorn output, transitions between console and file destinations, and explicit logging configuration in an isolated process. The configuration-startup cases in `test_logging.py` verify separate stdout/stderr replay and keep fatal fallback output on stderr. Deterministic container smoke checks capture both streams separately for real Gunicorn master and worker startup, a rejected HTTP request, and normal or failed notification-worker runs.
...
Principal-persistence coverage exercises the SQLite and Postgres schema, one-time credential issuance, concurrent first-time SQLite upgrades under one verifier root, digest-only storage, encrypted verifier-root and browser-session signing-key rotation and rewrapping, immutable workspace keys, safe path resolution, credential, principal, and browser-session lifecycle operations, bounded last-used writes, backup and SQLite-to-Postgres copying, and rollback after injected anonymous-workspace attachment failures. Principal-access contract tests cover anonymous attachment, credential redemption, current-principal and safe-credential reads, every fail-closed credential state, the complete PAT scope set, route-level scope enforcement, PAT-only CLI configuration and self-revocation, terminal secret rejection, and container-only operator bootstrap, issuance, recovery, expiry, rotation, revocation, disable, and enable behavior. Restricted-profile cases pin the exact public allowlist, anonymous and issuance rejection, standalone sign-in, cookie flags, CSRF, fixation-safe rotation, idle and absolute expiry, logout, principal-wide revocation, parent-credential cascades, share policy, Team privilege changes, restart continuity, and open-profile compatibility. Ownership-attachment cases prove that an authenticated context owns through its personal-workspace id while retaining separate principal and credential attribution, anonymous attachment keeps its existing directory, and SQLite `runs.rowid` and FTS search remain stable. Both database backends reject retired identity schemas and preserve their credential rows when migration is refused. They also pin Team roles through principal membership, public share reads as capability-based, workspace-owned share mutations, and rotation or revocation without data movement.

Owner-query adapter coverage uses the same mixed fixture on SQLite and Postgres: two personal owners, one team owner, one `NULL` team row, and one empty-team row. It pins the exact result sets for personal-only, team-only, team-capable, token-keyed, composite-key, and attribution helpers. A separate source check keeps `.tooling/owner-query-inventory.jsonl` aligned with direct ownership SQL and its reviewed exceptions.

History/run owner-query coverage applies that fixture to the first subsystem conversion. It pins personal and team History filters, the deliberately empty-only personal run lookup, snapshot deletion's existing session-only behavior, and Postgres query parity. The review-ledger test also keeps all 28 original sites classified and requires the six remaining row-to-row correlations to stay named inventory exceptions.

Project, Assessment, and Atlas owner-query coverage applies the mixed fixture to strict empty-team personal rows, nullable historical rows, unfiltered personal-only tables, team-owned data, and flattened metadata-owner keys on both databases. The review-ledger test accounts for all 156 original sites, requires the 116 adapter conversions to remain behavior-preserving, and keeps the 40 exact relational and key-shape exceptions named and audited. Broader subsystem cases cover reads, writes, imports, exports, evidence and report persistence, cleanup, retention, and worker-triggered updates through the shared owner boundary.

Files, workflows, and secrets owner-query coverage applies the mixed fixture to nullable and empty-team personal rows, direct team rows, exact personal-and-team composite keys, personal/team secret scopes, and the older flattened team metadata owner on both databases. The review-ledger test accounts for all 40 original sites, requires 39 adapter conversions to remain behavior-preserving, and keeps the one dual-representation workspace-metadata expression named and audited. Existing focused suites cover preference, variable, recent-value, star, workflow, secret, workspace metadata, migration, cleanup, retention, and worker paths through the shared owner boundary.

Automation and notification owner-query coverage applies the mixed fixture to the connectors' empty-team-only personal rows, the automation and notification stores' nullable-or-empty personal rows, direct team rows, exact notification-event keys, and unfiltered watcher run loading on both databases. The review-ledger test accounts for all 28 original sites, requires 27 adapter conversions to remain behavior-preserving, and keeps the ZAP assessment-to-profile correlation named and audited. Existing focused suites cover schedule, watcher, channel, event, OAST, and ZAP reads, writes, import linking, cleanup, retention, archived-team pausing, and worker paths through the shared owner boundary.

Remaining-surface owner-query coverage applies the mixed fixture to AI, shell built-in, CVE risk, finding-diff, and team-storage query shapes on both databases. It includes unfiltered personal rows, nullable-or-empty personal rows, exact composite keys, and team-only tables. The final review-ledger test accounts for all 52 original sites, requires 34 adapter conversions to remain behavior-preserving, keeps 18 principal-foundation, attribution, membership-hash, exact risk-key, and backfill exceptions named, and rejects any unclassified direct ownership predicate.

`tests/py/` covers backend contracts, route behavior, persistence, loaders, configuration/theme resolution, command validation, diagnostics gating, and structured logging. Atlas exact-lookup coverage pins canonical identity detection, single-host CIDR rejection, personal/team/project isolation, team visibility through runs and imports, bounded legacy ambiguity, URL-parent fallback after both misses and unreadable matches, privacy-safe browser/API completion, rejection, ambiguity, and degraded-profile logs, migration parity, and indexed SQLite/Postgres candidate plans without invoking providers, materializing records, or writing raw submitted URLs to audit data. Shared CVE risk coverage validates the bundled EPSS/KEV manifest, feed parsers, strict full-record OSV package normalization, configured local OSV startup loading and unchanged detection, bounded exact external OSV requests, redirect rejection, query-scoped hash caching, atomic SQLite/Postgres OSV applicability replacement and last-good rollback, bounded read-only SQLite/Postgres OSV package-version correlation, backend-owned feed status through the browser command catalog, privacy-safe OSV failure and provider status, normalized NVD status/CVSS/CWE parsing, bounded local-file acceptance, capability-checked explicit persistence, hash-only positive/negative caches, last-good and no-network refresh behavior, allowlisted redirects, committed short-lived refresh leases, concurrent SQLite writers during a blocked download, stale-owner publication rejection on SQLite/Postgres, origin, age, live-refresh and freshness states in enriched findings and the browser catalog, deterministic enrichment and ordering, exact remediation grouping, explicit remediation-group search/preview/apply, stale-preview rejection, target-winner disposition, shared review and remediation-guidance state, observation-specific verification, explicit guidance clearing, observation/evidence counts, personal/team and subject isolation, credential-stable ownership, EPSS hysteresis, owner batching, retry isolation, NVD transition thresholds and source-version preservation, complete read-only NVD version-inference candidate provenance, bounded structured-Nmap and HTTPx CPE extraction, capped reject-don't-evict materialization, parser-to-command agreement, cross-tool rejection, and explicit inference persistence with immutable source decisions on SQLite/Postgres, Project projections and acknowledgements, report snapshots, complete rendered source provenance, attribution, and privacy-safe failure logs. Project finding-evidence coverage pins all supported source types, exact-line snippets, idempotency, personal/Project scope, configurable quota semantics, unavailable-source reads, package preservation, audit events, and the browser/API OpenAPI contract. Its verification coverage also pins frozen assessment origins, bounded completed-run candidates, target/tool/profile compatibility, unavailable evidence, comparison pairs, privacy-safe final actors, audit events, and credential rotation without automatic dispositions. Project assessment coverage keeps SQLite and Postgres tables and indexes aligned, validates the shipped Network, Web, API, TLS, and Combined profiles plus the local catalog, and rejects multiple active cycles, ownerless cycles, invalid lifecycle/check/evidence states, duplicate checks or evidence, archived and out-of-scope Projects, incompatible targets, and quota overflow. Service, browser-route, API v1, and CLI tests also pin immutable profile and target snapshots, personal/team isolation, team-viewer read-only access, forward-only completion and archiving, completed-cycle immutability, archived-only deletion previews, source-record preservation, lifecycle and manual check/evidence audit events, exact versus host-and-descendant target matching, command/workflow/completion/version/structured-output compatibility, unrelated and cross-scope saved-source rejection, idempotent evidence links, transactional evidence quotas, reasoned manual states with safe actor metadata, decision clearing and state re-derivation, manual exclusion protection, bounded newest-first manual-link reads, shared desktop/mobile link and unlink controls, manual-link removal without source deletion, credential rotation without assessment ownership changes, audit-failure rollback, finding-aware state derivation, manual/active and auto-promoted completed-run matching, savepoint rollback, bounded cycle/check pages and filters, owner-scoped paged Nmap service-evidence API and browser reads, cross-owner browser rejection, capped Assessment-check attachment, free-form NSE omission, safe serialization, truthful coverage denominators, unavailable-evidence rollups, read-only target-scoped Nuclei recommendations from bounded technology/CVE/service/DNS signals, intrusive-profile abstention, guarded finding-verification plans, exact digest confirmation, current-target and frozen-action revalidation, safe/standard policy enforcement, broker handoff, Project linking, reviewed ZAP preview/queue/status/cancel routes, target and HTTP-profile digest revalidation, owner and nested-check isolation, public connector-state redaction, target/command-safe audit and logging, public error envelopes, privacy-safe OpenAPI responses, CLI request/output/completion shapes, idempotent SQLite/Postgres source tombstones across single, bulk, filtered History, and automatic-retention deletion, and explicit Project cleanup. The registered-route inventory also keeps browser and API Assessment mutations paired with their limiter and capability contracts. Its role matrix covers viewer, operator, admin, and owner behavior in personal, team, active, archived, and cross-team scopes, confirms denied handlers aren't called, and proves a protected action rechecks a downgraded member before creating private launch files. The app-owned helper registry coverage pins provider registration, duplicate rejection, lazy execution context, browser/server ownership, safe two-step workspace alias resolution, the complete root and exact-alias sets, and parity across execution, discovery, rich details, search, and autocomplete. Stable route modules reuse an application with a fresh client and reset Flask config for each test; application-factory, construction-time configuration and logging, import, and extension-isolation tests still create independent applications.

Principal background-authorization coverage runs the same persisted identities through SQLite and Postgres. It proves workers don't need request state or reusable secrets, current Team roles govern later launches and retries, disabled principals stop active and queued work, anonymous upgrades retain honest attribution, and credential rotation or deletion doesn't erase the trail used by the stolen-device review flow.

External CLI coverage also creates and edits assessor-authored findings, manages their typed supporting-evidence links, pages typed Nmap service evidence, reads stored EPSS/KEV feed status, starts one explicit exact-package OSV lookup, and drives the Project Assessment lifecycle. Finding cases pin Project slug resolution, bounded file and stdin JSON objects, exact create and optimistic-edit bodies, possible-duplicate overrides, stale revisions, Team permissions, typed source validation, zero-based run lines, duplicate-link no-ops, whole-collection text/JSON/NDJSON output, and exact link-id removal without deleting the source. Independently reported command-family cases cover Assessment lifecycle, batch planning and control, HTTP profiles, findings and evidence, runs and History, risk and advisories, and Project and Atlas reads; equivalent terminal outcomes use named parameters so CI identifies the failed contract directly. The broader CLI cases distinguish stale and unavailable sources, exercise each command's supported output formats, limit OSV requests to PURL and version, reject invalid values locally, give separate guidance for Team permission, disabled mode, provider failure, and pending batch cancellation, preserve public API error status/code/details for lifecycle-aware handlers, and keep focused parser, handler, formatter, confirmation, schema, route, structured-input, and shared error modules under architecture budgets. Assessment cases pin exact create and transition payloads, server-owned delete previews, archived-only confirmation, stable confirmed JSON output, source-record preservation, and personal/team capability handling. Feed-status tests also prove Team-viewer access and no refresh or feed-row writes on SQLite and PostgreSQL, while PostgreSQL finding coverage exercises the same evidence-link lifecycle as SQLite.
...
Record-aware `dig` and `nslookup` coverage pins command parsing, source-line provenance, positive A/AAAA/CNAME/PTR promotion, same-domain MX/NS/SRV boundaries, owner-only CAA handling, wildcard normalization, rejected-owner address suppression, malformed-owner rejection, multi-query and explicit-class dig commands, TXT and DNSSEC suppression, IPv4 and IPv6 resolvers and reverse lookups, NXDOMAIN and NODATA handling, short output, debug output, trace and nameserver-search modes, and final Atlas materialization without changing the raw transcript.

Target-aware WHOIS coverage pins option-aware IP and domain parsing, one-time metadata on the recognized record line, and exact entity sets from full ARIN, domain-registration, and RIPE transcripts. It rejects failed, rate-limited, empty, malformed, and unrelated replies, and covers IPv4/IPv6 allocations and multiline domain fields. SQLite and Postgres integration cases carry that one target through Atlas materialization, command-target discovery, and active-Project linking, and prove failed responses leave all three empty while keeping allocation ranges, provider infrastructure, nameservers, handles, referral links, and policy URLs in the transcript only.
...
Shortcuts overlay checks wait for the asynchronously loaded rows before counting them, then verify keyboard opening and dismissal.

Operator request tests cover portable and provider sessions across all four profiles on SQLite and Postgres, including grant and session revocation, disabled principals, direct-header rejection, CSRF, verified freshness, profile transitions, and private responses. Audit export tests interrupt output after authority is lost. Cookie replacement and recovery retain the original session deadline. The local status checks distinguish grant eligibility, CLI policy, and unobserved browser verification.

Operator browser journeys run on desktop and mobile in all four profiles, using both source and bundle assets on SQLite and Postgres. Open-profile setup creates principals through the disposable fixture helper, preserving the public signup limit. Anonymous menus stay unprivileged; granted sessions exercise navigation, probes, audit filters and downloads, expired proof, and grant revocation.

The open-profile Access journey exchanges legacy saved credentials and reloads into the authenticated page, requires explicit removal before anonymous recovery, keeps an anonymous workspace in a cookie session, proves the one-time secret leaves the DOM, manages safe credential metadata, rotates replacement-first, distinguishes browser sign-out from credential revocation, uses an existing credential, and exercises current- and last-credential warnings. It also checks that only the right actions are visible for anonymous, invalid-credential, and kept-workspace states, and that the Credentials section, card contents, and existing-credential form have room around them. Its mobile path checks the shared panel, action visibility and spacing, focus, touch targets, and horizontal containment. The dedicated restricted journey starts an isolated `token_required` server with an operator-bootstrapped credential, then checks the standalone sign-in, HttpOnly cookie exchange, the masked safe credential id in the HUD and **Kept** mobile access label, script-readable storage cleanup, CSRF rejection and success, safe credential rows, logout, session-wide revocation, invalid-credential redaction, and the mobile sign-in layout.
...
- [.tooling/playwright.parallel.config.js](../.tooling/playwright.parallel.config.js) is the normal CLI path and balances the open-profile suite across 5 isolated projects by default using measured per-file runtime weights. Separate `chromium-restricted` and `chromium-oidc` projects own the credential-gated and HTTPS-provider servers so their configuration and bootstrap state can't leak into other journeys. CI currently sets `PLAYWRIGHT_PROJECT_COUNT=3` for the open-profile suite. Each project has a one-worker limit. The parallel CLI suite allows up to seven workers locally and three in CI, shared across the open and dedicated access-profile projects. `PLAYWRIGHT_PROJECT_COUNT` controls the number of open-profile shards; it doesn't set the total worker limit. `--ci` uses the CI limit locally, and Playwright's `--workers` flag can override it for a focused investigation.
...
GitLab CI automatically requires `container-smoke-test-deterministic` when the Dockerfile, packaged dependencies, container helpers, command/workflow catalogs, wrapper, or smoke fixtures change. Scheduled pipelines run `container-smoke-test-public-network` with the normal retries and retain its JUnit, duration log, and JSONL retry evidence even when a later attempt passes. The broader `container-smoke-test` job remains available manually for the complete corpus. Ordinary branch image builds also retain a CycloneDX SBOM and full Grype report and fail on fixed Critical findings before a release tag is created. The build contract derives one runtime APT cache key per UTC day, carries the warmer's value through every scheduled Docker and Podman build, and keeps compiled-tool stages outside that invalidation boundary. Protected tags derive the same key from their recorded build time, build native AMD64 and ARM64 staging children, and run production-installation, bundled-tool, Syft, and Grype checks against each child before the canonical image index can exist. The ARM64 smoke also starts Redis and Postgres. Later evidence and signing jobs consume the retained platform contracts, SBOMs, reports, and effective build arguments. A protected branch rehearsal publishes a temporary index and repeats anonymous native pulls without promoting or creating release artifacts. Maintainers can use `release-image-recheck` to rerun production startup, bundled tools, and the scan against either an existing child digest or an index digest plus its selected platform without rebuilding it.

---

## History Seeding

`scripts/seed_history.py` populates the history database with realistic runs for a personal-workspace id or anonymous UUID. It's a manual-QA helper, not a test — use it when you want to exercise user-facing flows that only reveal themselves against a populated history: the history drawer, fuzzy history search, reverse-i-search, date/exit/star filters, and workspace-backed history views.
...
| `test-results/container-smoke-retries.jsonl` | public-network container smoke tier | One bounded record for every failed attempt, including timeouts later hidden by a successful retry |
...
- Set permission-test fixtures to their intended modes with `chmod` after creation. The process umask can strip permissions supplied to `mkdir`, turning a shared-directory rejection case into a valid private directory.
- Architecture, documentation, and ownership-inventory checks share a process-local syntax cache. It retains at most 64 MiB of serialized trees, checks current source contents on every read, and gives each visitor independent nodes. Syntax errors, changed files, and temporary test files retain normal diagnostics. The inventory result is checked in-process; a focused CLI case still checks the command interface.
- Use `copy_pristine_sqlite_database()` for an ordinary isolated test that only needs an empty current SQLite schema. It copies a production-built, integrity-checked template to a unique path, so test data remains isolated without replaying the migration ladder. For memory-only fixtures, `create_pristine_sqlite_connection()` backs up the same template into a fresh connection with `sqlite3.Row` rows and foreign keys enabled. The fixture owns commit, rollback, and closing; `foreign_keys=False` preserves fixtures whose existing contract disables enforcement. Clone tests cover the migration ledger, FTS triggers, deferred foreign keys, rollback, and isolation across current and later clones.
- Keep migration, schema reconciliation, startup, failure, and rollback coverage on a genuinely empty or purpose-built database through the real `db_init()` path.
...
- Keep owner-query conversions honest. Classify the existing result set before adopting an adapter, use one fixture with two personal owners plus team, `NULL`, and empty-team rows, and refresh and check `.tooling/owner-query-inventory.jsonl` whenever direct ownership SQL moves.
...
- Python and Playwright harnesses raise general app rate limits by default. Credential-issuance limits remain active, so unrelated tests shouldn't add extra issuance requests or retry failed issuance blindly.
