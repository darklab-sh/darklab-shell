...
`test_credential_lifecycle_logging.py` checks committed INFO milestones for anonymous upgrades, operator bootstrap and recovery, provider provisioning, credential creation and rotation, revocation, and principal status. Separate database connections verify that records see committed data. Commit and replacement failures emit no success records, repeated operations stay quiet, audit-disabled deployments retain milestones, and text/GELF checks exclude private input. A real PostgreSQL case checks committed visibility, concurrent revocation, and recovery rollback.
...
Autocomplete readiness coverage checks that a pending startup catalog becomes ready without a competing fetch and that browser checks read the shared app state. Startup recovery cases verify one retry after a network or server failure, one final error after persistent failure, and no retry for authorization or JSON errors. The browser case aborts the first real catalog request, then checks recovered positional hints and flags in source and bundled modes.
