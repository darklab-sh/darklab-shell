...
HTTP-profile CLI coverage resolves active Project slugs, lists profiles in text, JSON, and NDJSON, shows public composition counts, preserves the API's capability-aware JSON shape, and proves human-readable output doesn't expose protected reference names or Files paths. Viewer-shaped responses remain useful without those protected fields, and focused architecture budgets keep the new parser, handler, and formatter bounded.
