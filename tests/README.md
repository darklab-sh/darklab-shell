...
External CLI coverage also pages typed Nmap service evidence in text and JSON forms, distinguishes a valid empty page from a missing run, rejects oversized page requests locally, preserves public API error status/code/details for lifecycle-aware handlers, and keeps the new parser, handler, and shared error modules under architecture budgets.
