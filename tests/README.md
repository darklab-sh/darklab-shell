...
Schemathesis contract coverage keeps local OpenAPI review and generated API traffic inside the reviewed API base URL. It pins Project and owner scope, no-follow reads, saved size and digest checks, strict JSON parsing, internal references, fixed in-scope servers, read-operation and document limits, exact private schema and report material, partial-file cleanup, GET/HEAD-only negative generation, deterministic request bounds, and rejection of direct or drifted commands.
...
Dalfox evidence-mode coverage recognizes only the maintained discovery and active command shapes, validates the separate Web 1.4 discovery and validation rules, and proves clean negative evidence from either mode can't satisfy the other mode's check.
