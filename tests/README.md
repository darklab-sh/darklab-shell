...
`tests/js/e2e/` covers the browser UI against a live Flask server, including mobile behavior, kill/history/search/share flows, team scope switching, browser-visible output behavior, and startup resilience. Focused Quick Lookup flows cover the desktop rail and mobile menu, hostname and IP profiles, URL-parent recovery, app and cached-provider evidence, related-entity navigation, owner-scope refresh, Atlas handoff, and focus restoration in both bundled and source asset modes.
