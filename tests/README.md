...
External CLI coverage also pages typed Nmap service evidence, reads stored EPSS/KEV feed status, starts one explicit exact-package OSV lookup, and drives the Project Assessment lifecycle. It distinguishes stale and unavailable sources, exercises each command's text, JSON, and NDJSON formats, limits OSV requests to PURL and version, rejects empty values locally, gives separate guidance for Team permission, disabled mode, provider failure, and pending batch cancellation, preserves public API error status/code/details for lifecycle-aware handlers, and keeps focused parser, handler, formatter, confirmation, schema, route, and shared error modules under architecture budgets. Assessment cases pin slug resolution, exact create and transition payloads, server-owned delete previews, archived-only confirmation, stable confirmed JSON output, source-record preservation, and personal/team capability handling. Feed-status tests also prove Team-viewer access and no refresh or feed-row writes on SQLite and PostgreSQL.
