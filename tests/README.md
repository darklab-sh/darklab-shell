...
Project Assessment coverage pins its lazy entry, safe profile picker, desktop/mobile rendering parity, truthful coverage states, target disclosures, filters, paging, per-Project scroll and expansion state, active-tab return context, profile-driven cycle creation, remediation-group finding-change counts, comparison explanations, current and earlier finding links, contextual finding launches with typed check evidence, guarded recommended-action previews, exact digest confirmation, one-launch terminal handoff, focus restoration, forward-only lifecycle confirmations, preview-backed archived-cycle deletion, cancellation without mutation, touch-sized shared mobile action-sheet choices, and view-only permissions. The shared finding-change component also pins the same totals, cycle link, and distinct-remediation copy across desktop Overview, desktop Findings, and the mobile Findings sheet. Browser-route coverage separately checks that the picker receives only bounded profile summaries rather than complete live definitions; API and architecture coverage pin owner scope, frozen target/action revalidation, safe logging and audit fields, OpenAPI parity, and route/module inventories.
