...
- `test_autocomplete_container.py` - opt-in fresh-image smoke test that builds the Docker image via `docker compose` (using `examples/docker-compose.standalone.yml` as the base, with a unique image tag and a free port so the test build never collides with a running dev stack) and runs every command in `app/conf/auto_complete.txt` through `/run` with per-command output assertions; it also streams Docker build/start/health progress so long builds do not look idle and writes `test-results/autocomplete-container.xml` when you run that file directly or via [scripts/test_autocomplete_container.sh](../scripts/test_autocomplete_container.sh)
...
- `test_autocomplete_container.py` is the primary verification step after a Dockerfile or package upgrade. It reads `examples/docker-compose.standalone.yml`, resolves all relative paths to absolute, injects a unique image tag and a free localhost port, writes a temporary compose file, and builds with `docker compose build --pull`. It then runs every command from `app/conf/auto_complete.txt` through `/run` and checks each one against the stored expectations in `tests/py/fixtures/autocomplete_expectations.json`. A failure means a tool is missing, broken, or producing unexpected output in the upgraded image — the intent is to catch these before the image ships. Using the compose file (rather than bare `docker run`) ensures the test environment matches real deployments: tmpfs, Redis, and `init: true` are all present. It prints build/start/health progress while the container comes up, and writes `test-results/autocomplete-container.xml` when invoked directly or via [scripts/test_autocomplete_container.sh](../scripts/test_autocomplete_container.sh). If a tool's output has intentionally changed (e.g. a version bump changed its help text), re-capture the baseline with [scripts/capture_autocomplete_outputs.sh](../scripts/capture_autocomplete_outputs.sh) against a known-good running container before re-running this test. Before running the capture script, add `rate_limit_enabled: false` to `app/conf/config.local.yaml` so the per-session rate limiter does not interrupt the corpus run — remove it again before committing or deploying, as it is for local development only.
