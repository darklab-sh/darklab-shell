...
- `test_container_smoke_test.py` - opt-in Container Smoke Test that builds the Docker image via `docker compose` (using `examples/docker-compose.standalone.yml` as the base, with a unique image tag and a free port so the test build never collides with a running dev stack) and runs every command in `app/conf/auto_complete.txt` through `/run` with per-command output assertions; it also streams Docker build/start/health progress so long builds do not look idle and writes `test-results/container_smoke_test.xml` when you run that file directly or via [scripts/container_smoke_test.sh](../scripts/container_smoke_test.sh)
