...
`test_log_streams.py` checks every application severity and configured threshold in text and GELF, one emission after repeated configuration, exception routing, native Gunicorn output, and transitions between console and file destinations. The configuration-startup cases in `test_logging.py` verify separate stdout/stderr replay and keep fatal fallback output on stderr. Deterministic container smoke checks capture both streams separately for real Gunicorn master and worker startup, a rejected HTTP request, and normal or failed notification-worker runs.
