...
- `test_container_smoke_test.py` - opt-in Container Smoke Test that builds the Docker image via `docker compose` (using `examples/docker-compose.standalone.yml` as the base, with a unique image tag and a free port so the test build never collides with a running dev stack) and runs every command in `app/conf/auto_complete.txt` through `/run` with per-command output assertions; it also streams Docker build/start/health progress so long builds do not look idle, includes a focused `_docker_reach_host()` regression so GitLab DinD jobs keep probing the Docker daemon host instead of `127.0.0.1`, and writes `test-results/container_smoke_test.xml` when you run that file directly or via [scripts/container_smoke_test.sh](../scripts/container_smoke_test.sh); the fixture copies the whole `app/` tree to a temp directory, writes `config.local.yaml` into the temp copy, and substitutes that copy for the `../app:/app:ro` volume — this avoids an overlapping-bind-mount problem where Docker on Linux DinD silently ignored the more-specific `/app/conf` overlay when the parent `/app` path was already mounted
...
- `test_container_smoke_test.py` is the primary verification step after a Dockerfile or package upgrade. It reads `examples/docker-compose.standalone.yml`, copies the entire `app/` tree to a temp directory and writes `config.local.yaml` there, replaces the `../app:/app:ro` volume in the generated compose file with the temp copy (so the config overlay is inside the single `/app` bind mount rather than a second overlapping mount that Docker on Linux DinD ignores), injects a unique image tag and a free localhost port, writes a temporary compose file, and builds with `docker compose build --pull`. It then runs every command from `app/conf/auto_complete.txt` through `/run` and checks each one against the stored expectations in `tests/py/fixtures/container_smoke_test-expectations.json`. A failure means a tool is missing, broken, or producing unexpected output in the upgraded image — the intent is to catch these before the image ships. Using the compose file (rather than bare `docker run`) ensures the test environment matches real deployments: tmpfs, Redis, and `init: true` are all present. It prints build/start/health progress while the container comes up, and writes `test-results/container_smoke_test.xml` when invoked directly or via [scripts/container_smoke_test.sh](../scripts/container_smoke_test.sh). A small regression test in the same module locks `_docker_reach_host()` so GitLab DinD jobs keep probing the daemon host instead of hard-coding `127.0.0.1`. If a tool's output has intentionally changed (e.g. a version bump changed its help text), re-capture the baseline with [scripts/capture_container_smoke_test_outputs.sh](../scripts/capture_container_smoke_test_outputs.sh) against a known-good running container before re-running this test. Before running the capture script, add `rate_limit_enabled: false` to `app/conf/config.local.yaml` so the per-session rate limiter does not interrupt the corpus run — remove it again before committing or deploying, as it is for local development only.
