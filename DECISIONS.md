...
The checked ownership-query inventory now records 438 SQL sites across application and support code: 156 schema/migration sites, 12 already-principal credential-storage sites, six reviewed relational exceptions, and 264 unclassified session-era sites outside the adapter boundary. The query-shape view contains 74 personal-only sites, 239 team-capable sites, 17 token-keyed sites, 105 composite-key sites, and 3 attribution-only sites. The remaining subsystem grouping contains six History/run correlations, 156 Project/Assessment/Atlas sites, 40 Files/workflow/secret sites, 28 automation/notification sites, and 52 other application sites, including the 12 principal-foundation entries. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has five; the adopted adapters now have seven personal-only, five team-capable, and one composite production call sites.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-capable, token-keyed, composite-key, and attribution calls have distinct entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership remains the current session identity across this boundary. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code, the additive principal credential-storage boundary, and the six reviewed History/run row correlations are named direct-SQL exceptions; every other direct predicate has an assigned subsystem and a recorded principal-era replacement in `.tooling/owner-query-inventory.jsonl`. The completed History/run classification is recorded separately in `.tooling/owner-query-reviews/history-runs.json` so regenerated source facts cannot erase the review decision.
