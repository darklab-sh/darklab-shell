...
**Accepted for v3.0 on 2026-09-06. The additive principal store, typed authentication resolver, credential lifecycle routes, and scoped PAT model now enforce this contract. Existing product ownership remains session-backed through an explicit temporary adapter until the coordinated cutover.**
...
The checked ownership-query inventory now records 256 direct SQL sites across application and support code: 156 schema/migration sites, 12 already-principal credential-storage sites, 48 reviewed behavior-preserving exceptions, and 40 unclassified session-era sites outside the adapter boundary. The current direct-query shape contains 38 personal-only sites, 172 team-capable sites, 3 token-keyed sites, 40 composite-key sites, and 3 attribution-only sites. The remaining subsystem grouping contains six History/run correlations, 40 Project/Assessment/Atlas exceptions, one Files/workflow/secret exception, one automation/notification exception, and 52 other application sites, including the 12 principal-foundation entries. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has three; production code now calls the lower-level adapters through 23 personal-only, 22 team-capable, 29 composite, and 14 token-keyed sites.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-capable, token-keyed, composite-key, and attribution calls have distinct entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership remains the current session identity across this boundary. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code, the additive principal credential-storage boundary, and reviewed source-specific correlations are named direct-SQL exceptions; every other direct predicate has an assigned subsystem and a recorded principal-era replacement in `.tooling/owner-query-inventory.jsonl`. The completed classifications are recorded in `.tooling/owner-query-reviews/history-runs.json`, `.tooling/owner-query-reviews/projects-assessments-atlas.json`, `.tooling/owner-query-reviews/files-workflows-secrets.json`, and `.tooling/owner-query-reviews/automation-notifications.json` so regenerated source facts cannot erase the review decisions. The Project/Assessment/Atlas ledger accounts for all 156 original sites: 116 adapter conversions and 40 exact relational or key-shape exceptions. The Files/workflows/secrets ledger accounts for all 40 original sites: 39 adapter conversions and one exact workspace-metadata exception that preserves both direct team ownership and its legacy flattened owner representation. The automation/notifications ledger accounts for all 28 original sites: 27 adapter conversions and one exact ZAP assessment-to-profile correlation. None of these slices required a non-equivalent behavior change.
...
**Superseded for v3.0 on 2026-09-06 by [Pseudonymous Principals and Access Credentials](#pseudonymous-principals-and-access-credentials).** Legacy session-owned product surfaces still follow the historical constraints below through the explicitly temporary adapter; the central resolver already rejects invalid tokens instead of treating them as anonymous. These constraints are not compatibility requirements for the new schema.
