...
Operator authority depends on an active principal, an explicit grant, a protected browser session, and recent verification. The open profile permits anonymous workspace access without excluding verified operators. Credential and configured provider sign-in are available in open mode; restricted profiles retain their sign-in-method policies. Cookie CSRF, expiration, revocation, and privilege-change rotation apply independently of the anonymous-access policy. Neither a Team role nor a direct credential header or PAT grants operator access.
...
**Accepted for v3.0 on 2026-09-06. The principal store, typed authentication resolver, credential lifecycle routes, scoped PAT model, personal-workspace ownership, request-independent background authorization, principal-only schema guard, and restricted browser-session profile now enforce this contract. Authenticated product ownership resolves through the principal's workspace, and no earlier session identity is accepted as a compatibility path.**
...
Authenticated browsers use the same protected cookie session in every profile. Open mode retains anonymous UUID access, but a kept workspace no longer depends on a reusable secret in browser storage. Older saved credentials are exchanged through the POST-only redemption boundary and removed after success; failed exchange never becomes anonymous access. Browser cookies, portable headers, anonymous headers, and PAT bearer values remain mutually exclusive transports. `X-Team-ID` selects a requested scope while the server derives authority.

An explicit anonymous continuation preserves an unrelated anonymous workspace. Keeping that workspace retires its UUID and creates the principal and browser session atomically, so rollback cannot strand attached ownership without a session. Browser-only session markers and public identity metadata are navigation/cache hints, never authorization.
...
Product data belongs to a personal workspace or Team. Principal and credential ids provide actor attribution; credentials don't own data or determine workspace directory names. Browser sessions, background jobs, and API requests resolve the same ownership and capability rules.

The application rejects retired identity schemas at migration and startup boundaries. Positive Python and Postgres tests use canonical anonymous UUIDs or credentials issued through the real storage path; malformed values remain explicit rejection cases. Playwright obtains identities from the running application, so authentication coverage uses the production resolver.
