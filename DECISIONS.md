...
SQLite-to-Postgres data movement is a separate offline cutover, not an automatic startup conversion. Repository-free operators run `darklab-deploy migrate-to-postgres` as the deployment owner; it reaches the locked-down SQLite file through the managed Docker mount, copies data into a fresh bundled Postgres schema, validates counts and artifacts, and only then switches backend settings. Source checkouts and custom Postgres targets use `scripts/migrate_sqlite_to_postgres.py` with the guide in `docs/postgres-migration.md`. Schema management is shared by the app-owned migration runner; data migration remains an explicit maintenance workflow.
