...
For a live authenticated request, `credential_id` is required and `credential_type` is `portable` or `pat`. Authentication methods are `portable_header`, `pat_bearer`, or `browser_cookie`; a browser-cookie context keeps the parent portable credential id for attribution and revocation while carrying its separate browser-session id inside the server context. Operator tooling uses an explicit `operator` method. Background workers use `background`, carry no reusable credential, and keep an optional originating credential id only for audit and incident response. Before work starts, they rebuild authorization from the stable principal, workspace or team owner, current principal state, current membership, and required capabilities.
...
`token_required` is the restricted profile. It disables anonymous contexts and public principal issuance, and it gates every workspace route before cleanup or handler code can read scoped data. The complete identity-free surface is limited to static and built assets, favicon, health, status, CIDR-gated metrics, the standalone sign-in form, and credential redemption. Missing access redirects the root to sign-in and returns `401` elsewhere. `oidc_required` and `mixed` are reserved values, fail configuration validation, and provide no managed sign-in behavior.
...
No schema-changing choice remains in this decision. OIDC protocol behavior is outside this decision; the implemented profiles preserve these principal, credential, browser-session, PAT, recovery, share, and authorization contracts.
