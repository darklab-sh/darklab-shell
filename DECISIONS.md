...
Startup recovery dispatches workflow and assessment-batch coordinators separately even though both use the same child-attempt and checkpoint primitives. A batch validates its immutable parent, chunks, items, retry lineage, checkpoints, current Project, active assessment, runtime, and current owner permission before it can claim more work. Recovery releases only an abandoned unbound launch claim, replays a saved completion, leaves a live bound run attached, and routes a vanished run through the normal bounded retry policy. If the scope, authorization, runtime, or snapshot no longer permits execution, the coordinator stops claims and signals bound processes but remains `canceling` until their real outcomes settle; it then finishes as `failed` with the durable recovery reason. Recovery never turns an active process into a terminal row merely because startup couldn't stop it.
