...
The checked ownership-query inventory now records 283 direct SQL sites across application and support code: 156 schema/migration sites, 12 already-principal credential-storage sites, 47 reviewed behavior-preserving exceptions, and 68 unclassified session-era sites outside the adapter boundary. The current direct-query shape contains 38 personal-only sites, 189 team-capable sites, 9 token-keyed sites, 44 composite-key sites, and 3 attribution-only sites. The remaining subsystem grouping contains six History/run correlations, 40 Project/Assessment/Atlas exceptions, one Files/workflow/secret exception, 28 automation/notification sites, and 52 other application sites, including the 12 principal-foundation entries. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has three; production code now calls the lower-level adapters through 23 personal-only, 19 team-capable, 25 composite, and 8 token-keyed sites.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-capable, token-keyed, composite-key, and attribution calls have distinct entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership remains the current session identity across this boundary. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code, the additive principal credential-storage boundary, and reviewed source-specific correlations are named direct-SQL exceptions; every other direct predicate has an assigned subsystem and a recorded principal-era replacement in `.tooling/owner-query-inventory.jsonl`. The completed classifications are recorded in `.tooling/owner-query-reviews/history-runs.json`, `.tooling/owner-query-reviews/projects-assessments-atlas.json`, and `.tooling/owner-query-reviews/files-workflows-secrets.json` so regenerated source facts cannot erase the review decisions. The Project/Assessment/Atlas ledger accounts for all 156 original sites: 116 adapter conversions and 40 exact relational or key-shape exceptions. The Files/workflows/secrets ledger accounts for all 40 original sites: 39 adapter conversions and one exact workspace-metadata exception that preserves both direct team ownership and its legacy flattened owner representation. Neither slice required a non-equivalent behavior change.
