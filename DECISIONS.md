...
The checked ownership-query inventory now records 322 direct SQL sites across application and support code: 156 schema/migration sites, 12 already-principal credential-storage sites, 46 reviewed behavior-preserving exceptions, and 108 unclassified session-era sites outside the adapter boundary. The current direct-query shape contains 58 personal-only sites, 195 team-capable sites, 14 token-keyed sites, 52 composite-key sites, and 3 attribution-only sites. The remaining subsystem grouping contains six History/run correlations, 40 Project/Assessment/Atlas exceptions, 40 Files/workflow/secret sites, 28 automation/notification sites, and 52 other application sites, including the 12 principal-foundation entries. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has five; production code now calls the lower-level adapters through 14 personal-only, 15 team-capable, 17 composite, and 3 token-keyed sites.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-capable, token-keyed, composite-key, and attribution calls have distinct entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership remains the current session identity across this boundary. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code, the additive principal credential-storage boundary, and reviewed source-specific correlations are named direct-SQL exceptions; every other direct predicate has an assigned subsystem and a recorded principal-era replacement in `.tooling/owner-query-inventory.jsonl`. The completed classifications are recorded in `.tooling/owner-query-reviews/history-runs.json` and `.tooling/owner-query-reviews/projects-assessments-atlas.json` so regenerated source facts cannot erase the review decisions. The Project/Assessment/Atlas ledger accounts for all 156 original sites: 116 adapter conversions and 40 exact relational or key-shape exceptions, with no non-equivalent behavior change.
