...
The checked ownership-query inventory now records 460 SQL sites across application and support code: 156 schema/migration sites, 12 already-principal credential-storage sites, and 292 direct session-era sites outside the adapter boundary. The query-shape view contains 78 personal-only sites, 251 team-capable sites, 17 token-keyed sites, 111 composite-key sites, and 3 attribution-only sites. The subsystem grouping contains 28 History/run sites, 156 Project/Assessment/Atlas sites, 40 Files/workflow/secret sites, 28 automation/notification sites, and 40 other session-era application sites. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has five; the new adapters have no outside callers in this foundation change.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-capable, token-keyed, composite-key, and attribution calls have distinct entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership remains the current session identity across this boundary. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code and the additive principal credential-storage boundary are named direct-SQL exceptions; every other direct predicate has an assigned subsystem and a recorded principal-era replacement in `.tooling/owner-query-inventory.jsonl`.
