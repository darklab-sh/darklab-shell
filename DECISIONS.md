...
Go tools such as `nuclei`, `subfinder`, `httpx`, `dnsx`, `naabu`, `katana`, `tlsx`, `cdncheck`, `amass`, `assetfinder`, `gobuster`, `ffuf`, `tcping`, `trufflehog`, and `puredns` are installed with `ENV GOBIN=/usr/local/bin` in the Dockerfile. This puts binaries directly in `/usr/local/bin` with world-executable permissions, accessible to the `scanner` user. Versioned installs build through a short-lived main module that raises `golang.org/x/crypto` to the reviewed security baseline without changing each tool's pinned release. `gosu` is built from its pinned source with the same current Go toolchain, so its embedded standard library doesn't lag behind the rest of the image. Cached upstream executables that aren't installed tools are removed from both Go module-cache locations before the image is finalized, keeping them out of the runtime SBOM while preserving the module license notices. Without `GOBIN`, Go installs to `/root/go/bin` which is root-owned and inaccessible to `scanner`. Previous symlinks from `/root/go/bin/` to `/usr/local/bin/` also fail because symlinks inherit the target's permissions issue.
