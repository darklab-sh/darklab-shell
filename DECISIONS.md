...
**Accepted for v3.0 on 2026-09-06. The principal store, typed authentication resolver, credential lifecycle routes, scoped PAT model, personal-workspace ownership cutover, and request-independent background authorization now enforce this contract. Authenticated product ownership resolves through the principal's workspace; an explicit temporary adapter remains only for the legacy session interface until its coordinated removal.**
...
Every personal product owner is now stored as `personal_workspace_id`; team alternatives keep `team_id`. The central owner context, audit events, anonymous attachment, and Team membership carry principal and credential attribution separately. Migration `0080` adds nullable principal and credential companions for legacy private actor fields and attachment backfills them; those legacy fields remain transitional until the clean-removal pass. Digest-only credentials and PATs are the principal authentication store, and Team authorization resolves principal membership; `session_tokens` and legacy membership hashes remain temporary compatibility records. Scheduler `owner_kind`/`owner_id` remains a relationship to the owning automation object, while its personal owner is the stable workspace id. Composite keys, partial unique indexes, cleanup queries, and both backends follow the same mapping.

The checked ownership-query inventory now records 224 direct SQL sites across application and support code: 157 migration sites, 14 principal-foundation sites, and 53 reviewed behavior-preserving exceptions. No unclassified direct ownership predicate remains. The current direct-query shape contains 32 personal-only sites, 163 team-capable sites, one token-keyed site, 25 composite-key sites, and three attribution-only sites. The non-migration grouping contains six History/run correlations, 40 Project/Assessment/Atlas exceptions, one Files/workflow/secret exception, one automation/notification exception, and 19 other named exceptions. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has three; production code calls the lower-level adapters through 28 personal-only, 24 team-capable, 6 team-only, 39 composite, and 14 token-keyed sites.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-only, team-capable, token-keyed, composite-key, and attribution calls have distinct entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership uses a personal-workspace id for authenticated principals and a validated UUID for anonymous workspaces. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code, the principal credential-storage boundary, and reviewed source-specific correlations are named direct-SQL exceptions in `.tooling/owner-query-inventory.jsonl`; the completed Phase 3A classifications remain recorded in the five ledgers under `.tooling/owner-query-reviews/` as the review history for those conversions.
...
**Superseded for v3.0 on 2026-09-06 by [Pseudonymous Principals and Access Credentials](#pseudonymous-principals-and-access-credentials).** Product data is now owned by personal-workspace ids, while the explicitly temporary session-token interface adapts legacy requests into that model. The central resolver rejects invalid tokens instead of treating them as anonymous. The historical constraints below are not compatibility requirements for the new schema.
