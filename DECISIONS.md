...
One user-facing parent owns ordered chunks of at most 32 child items, preserving the workflow child ceiling without silently truncating a larger plan. A batch defaults to 128 selected items and always rejects more than 512. It defaults to eight active children, one per target, sixteen per owner, and thirty-two instance-wide; the hard limits are eight, one, thirty-two, and sixty-four. Operators can lower the item, active-batch, batch, owner, and instance limits, but they can't raise the product caps. The target limit isn't configurable because serial work against one target is part of the safety boundary.

Those four limits are checked inside the same short transaction that claims a child. PostgreSQL serializes assessment-batch claim decisions with one transaction advisory lock, while SQLite uses its immediate write transaction. The gate counts only active assessment-batch children and applies the most restrictive limit already in effect for an owner or instance. It scans pending items in stable order and may pass a temporarily busy target to keep an unrelated target moving; the skipped item remains pending and keeps its original ordinal.
...
Starting a batch rebuilds the current plan from the saved selection and concurrency, then requires the complete digest to match before it copies any selected item. Safe work needs one explicit confirmation; a selection containing standard work needs a second confirmation. The preview claim, parent, chunks, items, and check mappings commit together, so a failed copy leaves no partial coordinator. Once the claim succeeds, repeating the same owner-scoped preview and digest returns the existing batch—even if the preview later expires or the cycle changes—instead of launching a duplicate.

A retry never reopens or edits its source coordinator. It starts from a terminal batch, considers only the latest failed, interrupted, skipped, or canceled item outcomes, and uses their frozen-check mappings to compile a new current-state preview. Successful source items are never selected. Current evidence, manual decisions, target confirmation, feature readiness, policy, and profile changes can remove or reshape retry work, including leaving an empty explanatory preview. Starting a non-empty retry requires a fresh digest confirmation and any separate standard-policy confirmation, creates a new parent linked to the source, and records that lineage on both parents' immutable event history. Repeating the same confirmed retry preview returns that new parent rather than creating another one.

Copying an approved plan doesn't authorize a later launch by itself. Every claimed item goes back through the shared probe planner and must reproduce the same policy, digest, and public display command against the current confirmed target and feature state. The downstream child launcher accepts that complete launch specification directly, links its ordinary run to the parent Project, and never reconstructs an assessment command from a workflow template. Stale or unavailable items settle with fixed reason codes and no run. Per-child completion notifications stay suppressed because the batch is the user-facing unit, but History, evidence extraction, and assessment reconciliation remain ordinary run behavior. When the parent settles, channels using the existing run-complete preference receive one bounded summary for that batch id. Each immutable retry has a new id and sends its own summary.

Assessment-batch observability stays separate from workflow metrics even though both coordinators share lifecycle tables. Scrape-time gauges derive active parents, pending latest attempts, retained parent outcomes, and retained latest-attempt item outcomes from durable state, so recovery doesn't double-count them. Event metrics cover claim-to-launch latency, bounded stale/scope/target/profile/feature/policy rejection classes, fairness deferrals, operator lifecycle requests, and recovery outcomes. Their labels never include Project, assessment, batch, target, action, profile, command, or run values.

Cancellation records intent before it signals a child process. Pending and unbound work can settle immediately, but a bound run remains active until its ordinary completion path records the real exit. The batch therefore stays `canceling` until all children settle, doesn't retry after cancellation intent, and keeps separate succeeded, canceled, failed, unavailable, and `could_not_cancel` outcomes. This avoids turning a failed signal into a false claim that work stopped or erasing useful evidence from a child that completed during the request.

Assessment completion, archive, assessment deletion, and Project deletion never wait inside a request or save a deferred mutation. They lock the batch claim gate, record cancellation for every matching active batch, and return `assessment_batch_cancellation_pending` with the affected batch ids instead of changing lifecycle state. The operator must send the lifecycle request again after settlement. This makes the second request a fresh authorization and state decision, avoids a batch racing a deleted scope, and keeps ordinary child runs under the existing Project retention contract even when terminal coordinator records are cleaned up.

Startup recovery dispatches workflow and assessment-batch coordinators separately even though both use the same child-attempt and checkpoint primitives. A batch validates its immutable parent, chunks, items, retry lineage, checkpoints, current Project, active assessment, configured runtime, and current owner permission before it can claim more work. Recovery releases only an abandoned unbound launch claim, replays a saved completion, leaves a live bound run attached, and routes a vanished run through the normal bounded retry policy. If the scope, authorization, runtime, or snapshot no longer permits execution, the coordinator stops claims and signals bound processes but remains `canceling` until their real outcomes settle; it then finishes as `failed` with the durable recovery reason. Recovery never turns an active process into a terminal row merely because startup couldn't stop it. After recovery, bounded retention cleanup removes only old terminal retry-chain leaves. It can work back through an entirely expired lineage, but it preserves the source of any retained retry and never deletes the ordinary child runs or their evidence.
...
Greenbone remains operator-managed and integrates only through an explicit upload of its native GMP XML result report. darklab_shell doesn't start Greenbone, call its API, read its feeds, or own its database or scheduler. The report enters the existing owner-scoped Atlas preview/apply draft and batch: personal imports remain personal, while team imports use the current team and its existing capabilities. Each finding uses the exact affected host plus the NVT OID and normalized severity for duplicate identity; Greenbone's per-report result UUID remains occurrence provenance. Hosts and CVEs use the normal entity mapping, optional Project links and targets use the existing apply choices, and applied findings retain import origin and imported-assertion evidence so Assessment, packages, and reports can reuse the same links. No second external-findings store or connector lifecycle is created.

Private OAST begins with a disabled-by-default configuration boundary and no public service fallback. An enabled connector requires one credential-free HTTPS origin, an exact callback DNS suffix, an environment-variable token reference, a bounded retention period, and an explicit privacy acknowledgement. The normalized settings never contain the token value. Loading or reading them doesn't register a client, allocate a correlation domain, poll interactions, or write evidence; any caller must cross a separate explicit policy gate before performing those actions.

OAST correlation state belongs in the primary database because a callback can outlive the web request that approved it. A reservation is allowed only for the current owner-scoped Project, active assessment, applicable check, confirmed target, intrusive policy, and exact reviewed OAST action. It gets one random label, a short active window, a fixed purge deadline, and guarded binding to one source run per check. The row keeps only the allowed callback suffix and a hash of the private service origin; provider credentials, the origin URL, and interaction bodies remain outside it. Owner and check caps reject excess reservations without silently replacing active work. Creating or updating this state still doesn't register the label with a provider or poll for callbacks.

Accepted OAST interactions keep a redacted protocol summary in the primary database so evidence survives process restarts and stays tied to its source run and check. The app accepts only normalized DNS, HTTP, SMTP, or LDAP records during the exact active window, hashes provider ids, removes HTTP query strings and fragments, counts omitted fields, deduplicates within the correlation, and stops at 64 records without evicting earlier evidence. Raw headers, bodies, client addresses, provider metadata, and credentials are never stored. A finding association is explicit and requires the same Project and exact target; it reuses the normal run and assessment-check evidence links. The interaction rows share the correlation's fixed purge lifecycle. This storage contract stays independent of the adapter's polling schedule and process-private session material.

The private provider adapter follows Interactsh's default client protocol rather than adding deployment-specific identity knobs. Every callback label is 33 lowercase alphanumeric characters: a 20-character correlation id followed by the 13-character nonce recognized by the server. A service configured with different lengths is incompatible and must fail instead of letting the app guess. The adapter can call only registration, polling, and deregistration. It sends the deployment token in `Authorization`; the poll URL includes the correlation id and generated session secret because that is the provider's required client contract. Registration creates a local 2048-bit RSA session key, polling decrypts the returned AES-256 key with RSA-OAEP SHA-256 and each interaction with AES-CTR, and deregistration uses the same generated secret. Redirects, changed response URLs, shared service scopes, malformed encrypted records, cross-session callback ids, and over-limit responses never become evidence. Settings reads and database reservations still don't call the adapter.

The generated provider secret and RSA private key live in a short-lived encrypted app-data spool so polling can resume after a restart. They don't belong in the correlation row, logs, diagnostics, or public responses. The spool reuses the app master key and AES-GCM boundary, authenticates the exact correlation, callback, and service origin, uses private directory and file modes, and rejects path tricks, links, tampering, and key drift. It exposes only bounded old-file batches for lifecycle reconciliation and supports best-effort terminal or orphan cleanup.

One dedicated OAST worker owns provider calls under a deployment-wide advisory or file lock. It registers only durable reservations created by an explicit approved action, polls only rows activated for a source run, and sends every normalized result through the existing owner and active-window checks. Its encrypted spool is the recovery marker, so a restart reuses the same provider identity; corrupted or unstorable session material fails closed instead of registering a replacement. Provider-origin or callback-suffix drift, missing credentials, and temporary provider failures don't widen scope or expose secrets. Terminal rows are deregistered before local session removal when possible, while bounded orphan cleanup and the provider's accepted retention period cover interrupted or unreachable remote cleanup.
...
The shared fix-first order is explainable: KEV-listed remediation groups first, then EPSS probability and percentile, then CVSS, with finding age as the final stable tie-breaker. Owner-scoped Vulners and public-exploit references remain context and do not change that shared order.
...
The confirmation shows the profile, included families, exclusions, explicit-only update policy, and the installed managed-cache manifest's recorded release, SHA-256 revision, and entry count before the operator starts the run. The app reads that local manifest through fixed byte and entry limits and never contacts a scan target during preview. Assessment-plan preflight also validates the cache with the installed Nuclei binary and reports its refresh age and compatibility state. A stale but valid snapshot can be deliberately retained, while missing, invalid, unreadable, or incompatible templates block Nuclei work. Compose keeps the cache in a named volume, and the web entrypoint makes one bounded, non-fatal update attempt when the manifest is absent. It never refreshes an installed snapshot in the background. An explicit Assessment-plan refresh stages and validates a replacement under a deployment-wide lock, preserves the last good cache on failure, and rebuilds the full preview before approval. Opening or refreshing an Assessment by itself never updates templates. Safe, standard, and intrusive profiles have different exact evidence modes so one profile's successful clean run can't complete another profile's check. Unknown profile names fail closed to safe. The dedicated subdomain-takeover check remains a separate digest-pinned one-template contract.

`assessment_intrusive_actions_enabled` is the deployment gate for maintained intrusive Assessment actions and the reviewed intrusive Nuclei Project-probe profile, and it defaults to `false`. It is a necessary condition, not an authorization bypass: each maintained intrusive plan still has to prove its exact frozen check or probe selection, Project target, applicable source evidence, bounds, and fresh confirmation immediately before execution. The ordinary command registry remains on its existing policy, Project-probe Dalfox stays in parameter-discovery mode, and destructive actions have no launch path even when the gate is enabled.
...
Curl receives request headers and client-certificate paths through a short-lived, escaped config file and runs one HEAD request with ambient config, proxies, and redirects disabled. HTTPx receives request credentials through a short-lived ProjectDiscovery secrets file, Katana receives a short-lived header file, and Dalfox receives a short-lived JSON config containing protected headers with redirects disabled. Dalfox is limited to one target and parameter discovery with active XSS payloads and remote dictionaries disabled. Nuclei remains anonymous: its templates can generate requests beyond the selected target, and ProjectDiscovery's host-scoped credential material can't enforce an HTTP profile's exact scheme, port, root, included paths, and excluded paths for every request. Combining Nuclei with a profile therefore produces an unavailable plan before confirmation, materialization, or broker launch. The launch rechecks the profile revision, exact Project target, permissions, Secret and Files availability, and supported feature set immediately before execution. Redirect broadening, proxy use without an allowlist, login/token-capture workflows, and unsupported client-certificate combinations fail closed. The run worker removes private material on completion or start failure, and startup recovery removes abandoned directories without placing protected values or paths in user-visible records.
...
Assessment quotas are conservative capacity boundaries, not performance-sizing claims. One Project can keep 200 explicit targets, 25 assessment cycles, 50,000 target-specific checks, 250,000 typed evidence links, and 100,000 remediation deltas for one cycle by default. One personal or team owner can keep 100 cycles, 250,000 checks, and 1,000,000 assessment evidence links. These values leave room for repeated Combined cycles at the Project target limit while preventing one owner or Project from growing work tables without a bound; operators can lower them or set the documented limits to `0` when external capacity planning justifies an unlimited value. They are not presented as small-, medium-, or large-Project benchmarks. Reaching a hard limit rejects new work clearly instead of evicting assessment history. HTTPx screenshot output uses the existing per-owner Files count, per-file, and total-byte budgets: finalization retains new event-named images in order and removes the excess rather than deleting earlier files. Completed cycles stay until explicit deletion; rebuildable feed caches, acknowledged escalation events, temporary HTTP material, and connector callbacks follow documented retention policies.
...
**Accepted for v3.0 on 2026-09-06. The principal store, typed authentication resolver, credential lifecycle routes, scoped PAT model, personal-workspace ownership cutover, request-independent background authorization, clean-removal migration, and restricted browser-session profile now enforce this contract. Authenticated product ownership resolves through the principal's workspace, and no earlier session identity is accepted as a compatibility path.**
...
The SQLite default is therefore a fresh application-data set, with the verified old database and workspace tree retained for rollback. Postgres uses the supported one-off conversion so an existing production database doesn't need to be discarded. That conversion may attach only the operator data selected during rehearsal; it is not an automatic legacy-token migrator. It verifies a matching managed backup by default, holds the migration advisory lock, and commits the ownership change and legacy-schema removal in one transaction. A development checkout without `darklab-deploy` can explicitly opt into a checksum-verified archive made by the repository backup helper; managed restore remains strict. When extra development tokens are confirmed disposable, a separate Postgres-only confirmation can remove only the reviewed, isolated test-owner rows and safe Team memberships inside the selected conversion transaction. A Team snapshot needs its exact reviewed ID and no links; Team recent-value suggestions need an exact reviewed count. Both require an active selected-owned Team and test-member relationship. Other Team-scoped data still blocks conversion. The tool leaves files for later cleanup and refuses unknown references rather than guessing what to discard. Existing `tok_` values become invalid. Immediately before the cutover, the preflight must repeat the token and usage audit and verify a current verified backup. Any new or unexplained token use, or any known external user, stops the cutover and requires an approved broader migration design.
...
In the `open` profile, the browser temporarily keeps a portable access credential in `localStorage` and sends exactly one identity header per request: `X-Darklab-Credential` while authenticated or `X-Darklab-Anonymous-ID` while anonymous. `X-Team-ID` continues to express a requested team, but the server derives role and capabilities. Browser credential routes remain same-origin and do not emit permissive credential CORS headers. Supplying more than one identity header is malformed, and every supplied-but-invalid credential fails closed instead of falling back to anonymous state.

This script-readable browser secret is an accepted boundary for the anonymous-first profile, where portability without an account is the product goal. It remains exposed to a successful same-origin XSS attack.

#### Restricted access and browser sessions

`token_required`, `oidc_required`, and `mixed` are restricted profiles. They disable anonymous contexts and public principal issuance, and gate every workspace route before cleanup or handler code can read scoped data. The complete identity-free surface is limited to static and built assets, favicon, health, status, CIDR-gated metrics, the sign-in boundary, credential redemption where enabled, and OIDC start/callback when configured. Missing access redirects the root to sign-in and returns `401` elsewhere. `oidc_required` accepts only provider-backed browser sessions for the application, while scoped PATs remain available to `/api/v1/`; `mixed` offers both browser sign-in methods.

The sign-in form exchanges a portable credential for a shorter-lived database-backed session. The signed cookie is `Secure`, `HttpOnly`, and `SameSite=Strict`; the portable value isn't returned, retained by the page, or available to normal application JavaScript. Session ids rotate after authentication and successful Team privilege changes. Rows enforce both inactivity and absolute deadlines, and support current-session logout plus principal-wide revocation. Rotating or revoking a portable credential revokes every child session; disabling a principal or removing a Team member closes the relevant sessions without changing ownership.

Provider sign-in uses the authorization-code flow with PKCE S256 through [Authlib 1.7](https://docs.authlib.org/en/latest/oauth2/client/web/flask.html), a supported minor line under [Authlib's security policy](https://github.com/authlib/authlib/security). A one-use database state digest and `Secure`, `HttpOnly`, `SameSite=Lax` cookie bind the callback to the initiating browser; the flow also carries a nonce and short-lived PKCE verifier. Discovery must return the configured HTTPS issuer. Discovered endpoints may use other HTTPS hosts because providers commonly separate their token and key services, but redirects aren't followed. The app validates the ID token's signature, permitted algorithm, issuer, audience, nonce, issue time, and expiry, and doesn't persist provider access or refresh tokens. A valid provider subject links to one principal for that issuer, and one principal can link only one subject from that issuer. Email, names, and groups aren't identity keys or role inputs.

Linking requires a recent credential-backed browser session and fresh provider authentication; the callback rechecks the credential session before it commits. Unlinking requires recent credential proof and a usable portable credential, then revokes all sessions. Existing provider-backed sessions continue through a temporary provider outage until their own deadlines, but new sign-in and linking fail closed. Local logout doesn't terminate the provider's single-sign-on session. An issuer or subject change is a new identity, not an automatic transfer of the old workspace; an operator with host access can issue a one-time recovery credential and use `mixed` to repair the link. Provider compromise or subject reassignment can impersonate a linked principal, so provider account lifecycle and trusted CA management are operator responsibilities. Provider claims never grant Team roles, and background work still rechecks local principal, Team, and capability state.

Moving browser authentication into a cookie changes the cross-site request boundary. Restricted sessions therefore pair a readable same-site CSRF cookie with a stored digest and require the exact value in `X-Darklab-CSRF` for every unsafe cookie-authenticated request. A request with a valid PAT or portable header remains outside that cookie-specific check. This double-submit check doesn't make browser JavaScript trusted: a successful same-origin XSS can still perform actions as the user, but it can't read the portable recovery credential or the HttpOnly session cookie.

Session signing keys are versioned database rows encrypted by the existing vault master key. That choice keeps all Gunicorn workers and ordinary restarts on one keyring without introducing another plaintext file secret. Rotation activates a new key for future sessions and retains older versions for their bounded remaining lifetime; incident response combines rotation with targeted or principal-wide session revocation. A usable restore needs both the database and its matching vault master key.

There is no public bootstrap route. A container-only operator command succeeds only for an empty principal store in `token_required`, creates the first principal transactionally, and writes its one-time portable credential to a new owner-only file. Operator commands also rotate the signing key and revoke every browser session for a named principal. Host administrators and anyone controlling a complete backup remain trusted recovery actors.
...
Disabling a principal is the global stop. It rejects every credential, disconnects streams, terminates PTYs and active runs assigned to that principal, cancels queued launches, suppresses notification delivery and retries, and suspends that principal's personal or team-assigned durable definitions. Re-enabling the principal does not silently replay missed work or resume suspended definitions; an operator reviews the `status` command's suspended-work inventory and arranges explicit resumption through the usual controls. Stopped jobs need a new request. Existing manual pauses remain separate from operator suspension.

Background services carry stable principal, personal-workspace, team, and optional originating-credential ids. They never retain or reconstruct a bearer secret. Each launch, continuation, delivery, retry, or provider mutation resolves the principal again and checks the current Team membership and capability when the work is team-owned. Credential ids are historical and incident-response metadata only; deleting or rotating a credential doesn't erase attribution or change ownership.

Recovery stays pseudonymous and possession-based. Any remaining active portable credential may create another after recent-authentication confirmation. A linked managed sign-in may do the same when its access profile permits portable credentials. If neither path remains, there is no email, security-question, principal-id-only, or public self-service recovery. A deployment operator may run a local in-container recovery command, name the principal id, confirm the destructive action, revoke every prior credential and browser session, pause related schedules, watchers, notification channels, and Project digests, and receive one replacement secret exactly once. The action is audited. The UI protects the last credential from casual revocation but permits an explicit, warned lockout because the operator path is the final recovery boundary.
...
Unrecognized scopes fail issuance and authentication. PATs cannot create, rotate, revoke, or recover access credentials, cannot create another PAT, and can never read stored secret values. A PAT may revoke only itself through a dedicated endpoint; managing any other PAT requires a portable credential, browser session, or operator action. Scopes are immutable; changing them or rotating a PAT means issuing a replacement, saving it, and then revoking the old PAT. The default lifetime is 90 days, the minimum is one day, and the maximum is 365 days; deployments may lower but not raise that maximum, and there is no non-expiring PAT. Issuance starts with `identity:read`, `history:read`, and `runs:execute` unless the user deliberately adds scopes. Failed authentication is limited by client IP and public lookup id. Once a limit is reached, verification is skipped until the window resets, even for a correct secret; successful requests do not consume the failure allowance. Authenticated API limits use the PAT id and client IP.

Public share URLs remain capability links, not identities. They do not create either authentication context and do not grant access to the owner, Atlas, team, or API. The `open` profile keeps public shares available. Restricted profiles disable share creation and return `404` for share reads by default; an operator may explicitly enable unauthenticated public shares when that exception is intended. Authenticated share deletion follows the principal/team owner context in every profile.

#### Legacy identity inventory and replacement map

The 2026-09-06 pre-cutover inventory covered both schema heads and all 77 migrations that existed then. It found 59 `session`-named ownership or attribution columns across 45 application tables, plus the raw `session_tokens` identity table, 36 session-named indexes, and token/session constraints and foreign keys. Migration `0080` renamed product owner columns in place and added principal/credential attribution. On SQLite and Postgres, the selected conversion changes the reviewed operator's ownership in place and migration `0082` removes the retired identity table and token-keyed Team fields in the same transaction. The migration still fails closed when old identity rows remain. The historically affected surfaces were:
...
Every personal product owner is stored as `personal_workspace_id`; team alternatives keep `team_id`. The central owner context, audit events, anonymous attachment, and Team membership carry principal and credential attribution separately. Digest-only credentials and PATs are the only principal authentication store, and Team authorization and uniqueness use principal membership. Scheduler `owner_kind`/`owner_id` remains a relationship to the owning automation object, while its personal owner is the stable workspace id. Composite keys, partial unique indexes, cleanup queries, and both backends follow the same mapping.

The checked ownership-query inventory records 241 direct SQL sites across application and support code: 141 migration sites, 44 principal-foundation sites, and 56 reviewed behavior-preserving exceptions. No unclassified direct ownership predicate remains. The current direct-query shape contains 49 personal-only sites, 153 team-capable sites, two historical migration-only token-keyed sites, 31 composite-key sites, and six attribution-only sites. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has three; production code calls the lower-level adapters through 25 personal-only, 22 team-capable, six team-only, and 38 composite sites. No production path calls the retired token-keyed adapter.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-only, team-capable, composite-key, and attribution calls have distinct production entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership uses a personal-workspace id for authenticated principals and a validated UUID for anonymous workspaces. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code, the principal credential-storage boundary, and reviewed source-specific correlations are named direct-SQL exceptions in `.tooling/owner-query-inventory.jsonl`; the completed classifications remain recorded in the five ledgers under `.tooling/owner-query-reviews/` as the review history for those conversions.
...
The server-side test bypass is removed. Positive Python and Postgres tests use canonical anonymous UUIDs or credentials issued through the real storage path; malformed values remain labeled negative cases. Vitest values that only test request construction are not treated as server identities, and Playwright obtains identity from the running application. This keeps the production resolver under test without adding a test-only authentication path.
...
| XSS | Existing credentials are never returned and one-time reveals leave the DOM. CSP, output escaping, and redaction remain required. The open profile knowingly leaves the active portable credential script-readable. Restricted mode removes that value before the app starts and uses an HttpOnly session cookie, but same-origin XSS can still act through the user's live session. |
| CSRF | Restricted browser writes require the same random token in a same-site readable cookie, a request header, and the stored session digest. `SameSite=Strict` narrows ambient cookie use further. XSS remains outside the protection CSRF provides. |
...
| Unauthenticated issuance | Only `open` may upgrade the caller's validated anonymous workspace. Issuance cannot name another owner, is rate-limited, and is disabled in restricted profiles. Resource exhaustion remains an operator sizing and abuse-monitoring concern. |
...
The OIDC identity and flow tables extend this principal model without making a provider claim an owner key. The same browser-session, PAT, recovery, share, and authorization contracts continue to apply.

**Managed sign-in trust boundary.** The authorization-code client uses [Authlib's maintained OAuth client](https://docs.authlib.org/en/latest/oauth2/client/web/flask.html), pinned to its [supported 1.7.x security line](https://github.com/authlib/authlib/security), for the redirect and code exchange. The app validates one-use state, nonce, PKCE S256, the exact issuer and callback host/path, signed ID-token algorithm and key, audience, authorized party, issue and expiry times, and a recent provider `auth_time` for linking. Provider tokens are discarded after verification; only issuer and subject become a durable identity link. The requested scope defaults to `openid`, so names, email, groups, and profile claims aren't needed.

A compromised configured provider can authenticate as its subjects; this is the trust granted to any sign-in provider, not a role grant. Operators should disable the provider or principal, revoke browser sessions, and use the local recovery command when that trust is lost. A reused or changed subject is treated according to the provider's identity guarantee: a new subject doesn't silently inherit the old workspace, while reuse of the same issuer and subject can reach its existing link. Keep subject reuse disabled at the provider and review its account-recovery policy. A provider-only principal can be recovered by an operator-issued one-time portable credential; public recovery by knowing a subject or principal id does not exist.

Linking and unlinking require a recently authenticated credential session; linking also requires a fresh provider authentication and cannot bind one provider identity to two principals. Unlinking refuses to strand a principal without a usable portable credential and revokes all of that principal's browser sessions. Team roles are always resolved from darklab_shell membership, not provider groups or token claims. Background jobs keep stable principal ownership and recheck current local authority when they run; provider access tokens never authorize work. If the provider is unavailable, new sign-ins and links fail closed, while already issued app sessions retain their normal expiry and revocation behavior. Local logout does not clear the provider's single-sign-on session.

### Session Token Security

**Superseded for v3.0 on 2026-09-06 by [Pseudonymous Principals and Access Credentials](#pseudonymous-principals-and-access-credentials).** Product data is owned by personal-workspace ids, and the earlier session-token interface has been removed rather than adapted into the new model. The historical constraints below are not compatibility requirements for the new schema.
...
Header sync alone was not sufficient, though. Passive tabs also needed to refresh session-scoped UI such as recent-command chips, server-backed starred state, history results, saved Options state, and the options-panel token status. The v2.9 listener therefore called `reloadSessionHistory()`, `loadSessionPreferences()`, and `_updateOptionsSessionTokenStatus()` when those helpers were present, so visible UI followed the new session identity instead of lagging behind it. The v3 Access UI replaces that listener and token-status helper with explicit credential identity events and shared Access-panel refreshes.
...
In the released v2.9 model, `session-token revoke` deleted the token row from `session_tokens`. But that alone was not enough — any client still holding the token string could keep sending it as `X-Session-ID` and get data back, because the old data routes trusted any header value unconditionally. `get_session_id()` therefore looked up every `tok_`-prefixed header value against `session_tokens` on each request. A revoked or never-issued token returned `""` (anonymous), so the caller immediately lost access to session-scoped runs, snapshots, and stars without client-side cooperation. The v3 resolver keeps the indexed lookup but represents the same condition as an explicit authentication failure, so it can never reach an anonymous owner path.
...
Normal schedules and watcher-owned schedules share the physical `schedules` table. The relationship fields (`owner_kind` and `owner_id`) keep the behavior explicit, while personal-workspace or Team ownership controls access. Browser/API/CLI schedule lists expose only normal user-owned schedules; watcher-owned cadence rows stay tied to watcher state and cannot be edited as ordinary command schedules.
...
GitLab is the canonical registry because the source and release pipeline already live there. CI resolves the Python base index once, builds native AMD64 and ARM64 children from that snapshot, verifies and scans both staging digests, then assembles one OCI index only after every required gate passes. AMD64 uses one stable `buildcache-amd64` reference rather than a release-line name because BuildKit's content keys already invalidate entries when the Dockerfile, build arguments, or pinned base content changes. A validated `YYYY-MM-DD` build argument deliberately changes the runtime APT instruction once per UTC day; it isn't in any compiled-tool stage, so package freshness doesn't discard the expensive cache. Branch and scheduled builds derive it from the pipeline timestamp, scheduled consumers reuse the warmer's exported value, and protected publishers derive it from the immutable release build time for both architectures. A fixed resource lock serializes the scheduled warmer and release publisher that write the AMD64 reference. Every BuildKit path uses a tracked `HEAD` Git archive with a fixed tar umask as its context; that preserves Git's executable-bit contract while excluding checkout ownership, timestamps, ACLs, and xattrs such as SELinux labels from content keys. The scheduled warmer can run on any standard self-managed Docker runner; after it succeeds, every named Docker runner imports the recorded cache and pinned base digest without writing back. Cache portability is backed by an acceptance run that exported the release cache through a fresh BuildKit builder on `bael` and imported it through another fresh builder on `botis`; both jobs completed in under one minute with the expensive builder stages cached. CI doesn't keep those probe-only jobs because the production warmer, fanout, and release build exercise the lasting cache path. ARM64 stays uncached because registry-cache import and export for the bundled scanner and SecLists layers exceeds the hosted small runner's storage budget. Three uncached hosted-runner rehearsals completed in 1,790 to 1,827 seconds of a 3,600-second limit and retained 23.62 to 26.72 percent free daemon storage after export. The project accepts a 20 percent post-export floor for this isolated, cache-free lane; a cached or replacement runner must qualify separately. BuildKit cache mounts remain a builder-local optimization rather than cross-runner evidence.
...
Container starts as root → `entrypoint.sh` runs → fixes durable mount ownership and stages private configuration → sets `/tmp` to `1777` → dispatches an allowlisted connector-worker role or continues with web-only scanner setup → prepares the managed Nuclei cache and fills it when empty → installs scanner firewall rules → drops to `appuser` through `gosu` for the selected worker or Gunicorn.
...
**Why keep Nuclei bootstrap failure non-fatal?** A reviewed Nuclei plan needs an exact local manifest before it can calculate its approval digest. The Compose stacks persist that cache and the entrypoint runs `nuclei -update-templates` only when the manifest is absent. An unavailable provider or network leaves Nuclei plans unavailable but doesn't prevent the rest of the app from starting. Installed templates remain stable until an operator explicitly refreshes them from an Assessment-plan preflight or replaces the cache through deployment maintenance.
...
**The recording wrappers also provide an OBS-free playback check.** `--playback-only` runs the same seeded journey headlessly with frame capture disabled, including the container health check, session registration, workspace probe, scene timing, and UI assertions. This keeps the validation path close to the real recording path while making it practical to catch stale selectors or a stalled scene before opening OBS.
