...
**Accepted for v3.0 on 2026-09-06. The principal store, typed authentication resolver, credential lifecycle routes, scoped PAT model, personal-workspace ownership cutover, request-independent background authorization, and clean-removal migration now enforce this contract. Authenticated product ownership resolves through the principal's workspace, and no earlier session identity is accepted as a compatibility path.**
...
The checked ownership-query inventory records 232 direct SQL sites across application and support code: 141 migration sites, 37 principal-foundation sites, and 54 reviewed behavior-preserving exceptions. No unclassified direct ownership predicate remains. The current direct-query shape contains 43 personal-only sites, 153 team-capable sites, two historical migration-only token-keyed sites, 28 composite-key sites, and six attribution-only sites. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has three; production code calls the lower-level adapters through 25 personal-only, 22 team-capable, six team-only, and 38 composite sites. No production path calls the retired token-keyed adapter.
