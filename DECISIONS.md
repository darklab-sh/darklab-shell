...
The primary image has an approved compressed-size and cold-start budget. Small, pinned, multi-architecture tools required by maintained profiles may ship in it; protocol client packs and service stacks remain optional and operator-managed. Dalfox uses checksum-pinned Linux AMD64 and ARM64 musl release archives, keeps its reviewed MIT notice at a durable runtime path, and is covered by the normal bundled-tool smoke contract. A tool that exceeds the budget requires a separate packaging decision.
