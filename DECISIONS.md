...
The checked ownership-query inventory now records 222 direct SQL sites across application and support code: 156 schema/migration sites, 12 already-principal credential-storage sites, and 54 reviewed behavior-preserving exceptions. No unclassified direct ownership predicate remains. The current direct-query shape contains 30 personal-only sites, 161 team-capable sites, 3 token-keyed sites, 25 composite-key sites, and 3 attribution-only sites. The non-migration grouping contains six History/run correlations, 40 Project/Assessment/Atlas exceptions, one Files/workflow/secret exception, one automation/notification exception, and 18 other named exceptions, including the 12 principal-foundation entries. `personal_scope_predicate()` has no outside production caller and `shared_owner_predicate()` has three; production code calls the lower-level adapters through 28 personal-only, 24 team-capable, 6 team-only, 39 composite, and 14 token-keyed sites.

The inventory and adapters make result-set semantics reviewable. Personal-only, team-only, team-capable, token-keyed, composite-key, and attribution calls have distinct entry points, and every team-capable call names whether personal rows use `NULL`, an empty string, either, or no team filter. Personal ownership remains the current session identity across this boundary. An equivalent conversion preserves the recorded result set, while a conversion that adds or removes a personal/team condition is a separately tested behavior change. Migration code, the additive principal credential-storage boundary, and reviewed source-specific correlations are named direct-SQL exceptions; every direct predicate has a recorded principal-era replacement in `.tooling/owner-query-inventory.jsonl`. The completed classifications are recorded in the five ledgers under `.tooling/owner-query-reviews/` so regenerated source facts cannot erase the review decisions. The Project/Assessment/Atlas ledger accounts for all 156 original sites: 116 adapter conversions and 40 exact relational or key-shape exceptions. The Files/workflows/secrets ledger accounts for all 40 original sites: 39 adapter conversions and one exact workspace-metadata exception that preserves both direct team ownership and its legacy flattened owner representation. The automation/notifications ledger accounts for all 28 original sites: 27 adapter conversions and one exact ZAP assessment-to-profile correlation. The remaining-surfaces ledger accounts for all 52 original sites: 34 adapter conversions and 18 principal-foundation, attribution, membership-hash, exact risk-key, or backfill exceptions. None of these slices required a non-equivalent behavior change.
