...
8. install that payload with the bundled Postgres profile and normal process defaults, save and mutate API state around a repository-free backup/restore round trip, then anonymously verify the index signatures, selected child, evidence checksums, installer, and running stack; a final tag creates stable GitLab Release links only after the Postgres gate passes
