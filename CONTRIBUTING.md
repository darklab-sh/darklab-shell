...
**Owner-scoped SQL** — use `services.teams.ownership_queries` instead of assembling a new owner predicate at a call site. Choose the adapter that matches the table's actual key, and state the personal `team_id` representation explicitly for a team-capable table. Personal ownership uses the authenticated principal's workspace ID or a validated anonymous UUID; credentials provide access and attribution, not owner keys. Classify a query change before making it: an equivalent change must return the same rows, while a changed result set needs an explicit behavior decision and focused coverage. Refresh the checked exception list with `python scripts/development/owner_query_inventory.py --write`, review the entry's current result set and replacement path, then run the same command with `--check`. Also run the ownership review checks: `bash scripts/run_pytest.sh -c .tooling/pytest.ini --rootdir=. tests/py/test_owner_context_*.py -k review_covers_every_baseline_site -q`. The inventory freshness check alone does not reject an unclassified query.
...
- Browser runs retain a trace for every failed attempt, including the first attempt when a retry passes. CI retries once and still rejects flaky results and focused `test.only` cases.
...
`release-image-arm64` and its native verification jobs use GitLab's `saas-linux-small-arm64` runner. This is the release chain's one DinD path: the jobs start the version-matched Docker service on `tcp://docker:2375` with TLS disabled and a conservative `1360` MTU inside GitLab's isolated hosted environment. The publisher pushes an immutable ARM64 child without a registry cache because the complete scanner and SecLists cache exceeds the small runner's storage budget. The runtime stage inherits the SecLists asset stage so that large tree remains one shared layer instead of becoming a second BuildKit snapshot during assembly. The job artifact records wall time, job timeout, daemon storage before and after publication, and `docker system df -v`.

The accepted floor is completion within 75 percent of the 3,600-second job timeout with at least 20 percent free daemon storage after export. Rehearse the protected multi-platform path after Dockerfile or packaged-tool changes that can materially affect either limit. No self-managed runner configuration or `/certs/client` mount is used for this lane. Keep ARM64 caching disabled unless a replacement runner passes the same storage and timing checks with cache import and export enabled.
...
For a branch-image security recheck that needs fresh Debian packages, start a new web pipeline with `BRANCH_IMAGE_FRESH_APT=true`. The `docker-build` job then bypasses the runtime-stage cache while keeping the compiled tool stages cached. Leave the variable unset for ordinary builds. Check the job's vulnerability report to confirm the installed package versions and scan result.
