...
`release-image-arm64-smoke` uses GitLab's `saas-linux-small-arm64` runner. It builds in the publish stage beside the canonical AMD64 image, while Docker Hub promotion still waits for both when the gate is enabled. This is the one DinD path: the job starts the version-matched Docker service on `tcp://docker:2375` with TLS disabled, a conservative `1360` MTU, and Docker's containerd image store inside GitLab's isolated hosted environment. The default Buildx builder uses that shared store for both the candidate image and the portable registry cache. No self-managed runner configuration or `/certs/client` mount is used for this job.
