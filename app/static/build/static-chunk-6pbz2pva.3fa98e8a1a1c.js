...
var GRACEFUL_TERMINATION_EXIT_CODES = /* @__PURE__ */ new Set([-15]);
...
function formatElapsed(started) {
  ...
  const total = Math.max(0, Math.floor((Date.now() - start) / 1e3));
...
function seededUnit(seed) {
  const value = Math.sin(Number(seed) || 1) * 1e4;
