...
function createStatusMonitorResources({ core, svgEl, pathFromPoints }) {
  const resourceStateByRunId = /* @__PURE__ */ new Map();
  const resourceTrendByRunId = /* @__PURE__ */ new Map();
  const isTelemetryNumber = core.isTelemetryNumber;
  function runResourceUsage(run) {
    const runId = String(run?.run_id || run?.id || "");
    const usage = run?.resource_usage && typeof run.resource_usage === "object" ? run.resource_usage : {};
    const previous = runId ? resourceStateByRunId.get(runId) : null;
    const now = Date.now();
    const cpuSeconds = isTelemetryNumber(usage.cpu_seconds) ? Number(usage.cpu_seconds) : null;
    let cpuPercent = previous?.cpu_percent;
    if (cpuSeconds !== null && previous && isTelemetryNumber(previous.cpu_seconds)) {
      const elapsedSeconds = Math.max(0, (now - Number(previous.sampled_at || now)) / 1e3);
    ...
    const memoryBytes = isTelemetryNumber(usage.memory_bytes) ? Number(usage.memory_bytes) : previous?.memory_bytes;
    const resolved = {
    ...
    if (runId && (isTelemetryNumber(resolved.cpu_percent) || isTelemetryNumber(resolved.cpu_seconds) || isTelemetryNumber(resolved.memory_bytes))) {
  ...
  function trendPath(samples, key, width = 160, height = 34) {
    ...
    const max = key === "cpu" ? Math.max(100, ...values) : Math.max(...values, 1);
...
var DarklabStatusMonitorResources = { create: createStatusMonitorResources };
