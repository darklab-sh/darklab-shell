...
import {
  useMobileTerminalViewportMode as useMobileTerminalViewportMode2
...
var bindOutsideClickClose = (function(global) {
  "use strict";
  ...
  return bindOutsideClickClose2;
})(typeof window !== "undefined" ? window : globalThis);

// app/static/js/core/session_core.js
var DarklabSessionCore = (function(global) {
  ...
  function generateUUID2(preferredCrypto) {
    const primary = _cryptoApi(preferredCrypto);
    if (primary && typeof primary.randomUUID === "function") {
      try {
        return primary.randomUUID();
      } catch (_) {
      }
    }
    const fallback = primary && typeof primary.getRandomValues === "function" ? primary : _cryptoApi(null);
  ...
  function getOrCreateStorageValue2(storage, key, createValue) {
  ...
  function withSessionHeaders2(options = {}, sessionId, clientId) {
    return {
      ...options,
  ...
  const api = Object.freeze({
    ...
    getOrCreateStorageValue: getOrCreateStorageValue2,
    resolveSessionId: resolveSessionId2,
    maskSessionToken: maskSessionToken4,
    describeFetchError: describeFetchError4,
    withSessionHeaders: withSessionHeaders2
  });
  return api;
})(typeof window !== "undefined" ? window : globalThis);
var { describeFetchError, generateUUID, getOrCreateStorageValue, maskSessionToken, resolveSessionId, withSessionHeaders } = DarklabSessionCore;

// app/static/js/core/app_preferences_core.js
var DarklabPreferenceCore = null;
(function() {
  const SESSION_PREFERENCE_KEYS2 = Object.freeze([
  ...
  const WELCOME_INTRO_MODES2 = Object.freeze(["animated", "disable_animation", "remove"]);
  const SHARE_REDACTION_DEFAULT_MODES2 = Object.freeze(["unset", "redacted", "raw"]);
  const HUD_CLOCK_MODES2 = Object.freeze(["utc", "local"]);
  const COMPARE_VIEW_MODES2 = Object.freeze(["auto", "side_by_side", "unified", "changes_only", "findings_only"]);
  const COMPARE_CONTEXT_MODES2 = Object.freeze(["3", "10", "all"]);
  const OPTIONS_MODAL_TABS2 = Object.freeze(["preferences", "secrets", "teams", "notifications"]);
  function _coerceMode(value, modes, fallback) {
    return modes.includes(value) ? value : fallback;
  }
  function coerceTimestampMode2(value, modes = ["off", "elapsed", "clock"]) {
    return _coerceMode(value, modes, "off");
  }
  function coerceLineNumberMode2(value) {
    return value === "on" ? "on" : "off";
  }
  function coerceWelcomeIntroMode2(value) {
    return _coerceMode(value, WELCOME_INTRO_MODES2, "animated");
  }
  function coerceShareRedactionDefaultMode2(value) {
    return _coerceMode(value, SHARE_REDACTION_DEFAULT_MODES2, "unset");
  }
  function coerceRunNotifyMode2(value) {
    return value === "on" ? "on" : "off";
  }
  function coerceCommandOutcomeSummariesMode2(value) {
    return value === "off" ? "off" : "on";
  }
  function coerceConstellationFullDayMode2(value) {
    return value === "on" ? "on" : "off";
  }
  function coerceHudClockMode2(value) {
    return _coerceMode(value, HUD_CLOCK_MODES2, "utc");
  }
  function coerceCompareViewMode2(value) {
    return _coerceMode(value, COMPARE_VIEW_MODES2, "auto");
  }
  function coerceCompareContextMode2(value) {
    const normalized = String(value || "").trim().toLowerCase();
    return _coerceMode(normalized, COMPARE_CONTEXT_MODES2, "3");
  }
  function coerceOptionsModalTab2(value) {
    const normalized = String(value || "").trim().toLowerCase();
    return _coerceMode(normalized, OPTIONS_MODAL_TABS2, "preferences");
  }
  function normalizePromptUsername2(value) {
    const username = String(value || "").trim();
    return /^[A-Za-z0-9._-]{1,32}$/.test(username) ? username : "";
  }
  function coerceTourSeenVersion2(value) {
    if (value === null || typeof value === "undefined" || value === "") return "";
    const parsed = Number.parseInt(String(value).trim(), 10);
    return Number.isFinite(parsed) && parsed >= 1 ? parsed : "";
  }
  function defaultSessionPreferences2(defaultTheme = "darklab_obsidian.yaml") {
    return {
  ...
  function normalizeSessionPreferences2(raw, defaults, { timestampModes = ["off", "elapsed", "clock"] } = {}) {
    const prefs = { ...defaults || defaultSessionPreferences2() };
    ...
    prefs.pref_timestamps = coerceTimestampMode2(source.pref_timestamps, timestampModes);
    prefs.pref_line_numbers = coerceLineNumberMode2(source.pref_line_numbers);
    prefs.pref_welcome_intro = coerceWelcomeIntroMode2(source.pref_welcome_intro);
    prefs.pref_share_redaction_default = coerceShareRedactionDefaultMode2(source.pref_share_redaction_default);
    ...
    prefs.pref_run_notify = coerceRunNotifyMode2(source.pref_run_notify);
    prefs.pref_command_outcome_summaries = coerceCommandOutcomeSummariesMode2(source.pref_command_outcome_summaries);
    prefs.pref_hud_clock = coerceHudClockMode2(source.pref_hud_clock);
    prefs.pref_prompt_username = normalizePromptUsername2(source.pref_prompt_username);
    prefs.pref_compare_view_mode = coerceCompareViewMode2(source.pref_compare_view_mode);
    prefs.pref_compare_context = coerceCompareContextMode2(source.pref_compare_context);
    prefs.pref_options_modal_last_tab = coerceOptionsModalTab2(source.pref_options_modal_last_tab);
    prefs.pref_tour_seen_version = coerceTourSeenVersion2(source.pref_tour_seen_version);
    prefs.pref_constellation_full_day = coerceConstellationFullDayMode2(source.pref_constellation_full_day);
    return prefs;
  }
  function sessionPreferenceCacheKey2(sessionId = "") {
    return `session_pref_cache:${sessionId || ""}`;
  }
  DarklabPreferenceCore = Object.freeze({
    COMPARE_CONTEXT_MODES: COMPARE_CONTEXT_MODES2,
    COMPARE_VIEW_MODES: COMPARE_VIEW_MODES2,
    HUD_CLOCK_MODES: HUD_CLOCK_MODES2,
    OPTIONS_MODAL_TABS: OPTIONS_MODAL_TABS2,
    SESSION_PREFERENCE_KEYS: SESSION_PREFERENCE_KEYS2,
    SHARE_REDACTION_DEFAULT_MODES: SHARE_REDACTION_DEFAULT_MODES2,
    WELCOME_INTRO_MODES: WELCOME_INTRO_MODES2,
    coerceCommandOutcomeSummariesMode: coerceCommandOutcomeSummariesMode2,
    coerceCompareContextMode: coerceCompareContextMode2,
    coerceCompareViewMode: coerceCompareViewMode2,
    coerceConstellationFullDayMode: coerceConstellationFullDayMode2,
    coerceHudClockMode: coerceHudClockMode2,
    coerceLineNumberMode: coerceLineNumberMode2,
    coerceOptionsModalTab: coerceOptionsModalTab2,
    coerceRunNotifyMode: coerceRunNotifyMode2,
    coerceShareRedactionDefaultMode: coerceShareRedactionDefaultMode2,
    coerceTimestampMode: coerceTimestampMode2,
    coerceTourSeenVersion: coerceTourSeenVersion2,
    coerceWelcomeIntroMode: coerceWelcomeIntroMode2,
    defaultSessionPreferences: defaultSessionPreferences2,
    normalizePromptUsername: normalizePromptUsername2,
    normalizeSessionPreferences: normalizeSessionPreferences2,
    sessionPreferenceCacheKey: sessionPreferenceCacheKey2
  });
})();
var {
...
var DarklabRunOutputModel = (function(global) {
  "use strict";
  const LINE_EVENT_SCHEMA_VERSION2 = 1;
  const LINE_KIND_VALUES2 = Object.freeze(["info", "notice", "warn", "error"]);
  const LINE_ROLE_VALUES2 = Object.freeze([
    "body",
    "prompt-echo",
    "section-header",
    "kv",
  ...
  const LINE_SIGNAL_VALUES2 = Object.freeze(["findings", "warnings", "errors", "summaries"]);
  const LINE_NOISE_KIND_VALUES2 = Object.freeze(["progress", "status", "boilerplate"]);
  const LineKind2 = Object.freeze({
    INFO: "info",
    NOTICE: "notice",
    WARN: "warn",
    ERROR: "error"
  });
  const LineRole2 = Object.freeze({
    BODY: "body",
  ...
  const LineSignal2 = Object.freeze({
    FINDINGS: "findings",
    WARNINGS: "warnings",
    ERRORS: "errors",
    SUMMARIES: "summaries"
  });
  const LineNoiseKind2 = Object.freeze({
    PROGRESS: "progress",
    STATUS: "status",
  ...
  const LEGACY_KIND_BY_CLS = Object.freeze({
    "": LineKind2.INFO,
    output: LineKind2.INFO,
    out: LineKind2.INFO,
    cmd: LineKind2.INFO,
    notice: LineKind2.NOTICE,
    "builtin-note": LineKind2.NOTICE,
    "welcome-output": LineKind2.NOTICE,
    warn: LineKind2.WARN,
    warning: LineKind2.WARN,
    error: LineKind2.ERROR
  });
  const LEGACY_ROLE_BY_CLS = Object.freeze({
    "": LineRole2.BODY,
    output: LineRole2.BODY,
    out: LineRole2.BODY,
    notice: LineRole2.BODY,
    warn: LineRole2.BODY,
    warning: LineRole2.BODY,
    error: LineRole2.BODY,
    "builtin-note": LineRole2.BODY,
    "builtin-spacer": LineRole2.BODY,
    "welcome-output": LineRole2.BODY,
    cmd: LineRole2.PROMPT_ECHO,
    "prompt-echo": LineRole2.PROMPT_ECHO,
    "builtin-section": LineRole2.SECTION_HEADER,
    "builtin-kv": LineRole2.KV,
    "builtin-help-row": LineRole2.HELP_ROW,
    "builtin-faq-q": LineRole2.HELP_ROW,
    "builtin-faq-a": LineRole2.HELP_ROW,
    "pty-marker": LineRole2.PTY_MARKER,
    progress: LineRole2.PROGRESS,
    "status-line": LineRole2.STATUS_LINE,
    "builtin-success": LineRole2.SUCCESS,
    denied: LineRole2.DENIED,
    "exit-ok": LineRole2.EXIT_OK,
    "exit-fail": LineRole2.EXIT_FAIL
  });
  ...
  const NOISE_KIND_BY_ROLE = Object.freeze({
    progress: LineNoiseKind2.PROGRESS,
    "status-line": LineNoiseKind2.STATUS
  });
  ...
  function legacyClsToKind2(value) {
  ...
  function signalsFromWire(value, unknownCollector) {
    ...
    value.forEach((item) => {
      ...
      if (LINE_SIGNAL_VALUES2.includes(signal)) {
        signals.push(signal);
      } else {
  ...
  function legacyClsForEvent(event) {
    ...
    const role = event.role || LineRole2.BODY;
    if (role !== LineRole2.BODY) return ROLE_LEGACY_CLS[role] || "";
    return KIND_LEGACY_CLS[event.kind || LineKind2.INFO] || "";
  }
  function noiseKindForRole2(role) {
    return NOISE_KIND_BY_ROLE[role] || null;
  }
  function noiseKindForEvent2(event) {
    if (!event || Array.isArray(event.signals) && event.signals.length) return null;
    if (event.kind === LineKind2.WARN || event.kind === LineKind2.ERROR) return null;
    if (event.noise_kind) return event.noise_kind;
    return noiseKindForRole2(event.role || LineRole2.BODY);
  }
  function isNoiseLineEvent2(event) {
    return noiseKindForEvent2(event) !== null;
  }
  function legacyPayload(event) {
    ...
    const noiseKind = noiseKindForEvent2(event);
    if (noiseKind) {
  ...
  function fromWireLineEvent2(payload, unknownCollector) {
    const item = payload && typeof payload === "object" ? payload : {};
    const clsValue = item.cls || "";
    const kind = enumValue(LINE_KIND_VALUES2, item.kind, legacyClsToKind2(clsValue), "kind", unknownCollector);
    const role = enumValue(LINE_ROLE_VALUES2, item.role, legacyClsToRole2(clsValue), "role", unknownCollector);
    const noiseKind = enumValue(
      LINE_NOISE_KIND_VALUES2,
      item.noise_kind,
      noiseKindForRole2(role),
      "noise_kind",
      unknownCollector
    );
    return {
  ...
  function toLegacyWireLineEvent2(event) {
    return legacyPayload(event || {});
  }
  function toWireLineEvent2(event) {
    const payload = legacyPayload(event || {});
    payload.v = LINE_EVENT_SCHEMA_VERSION2;
    payload.kind = event && event.kind || LineKind2.INFO;
    payload.role = event && event.role || LineRole2.BODY;
    return payload;
  }
  function eventSearchText2(event) {
    return String(event && event.text || "");
  }
  const api = Object.freeze({
    LINE_EVENT_SCHEMA_VERSION: LINE_EVENT_SCHEMA_VERSION2,
    LINE_KIND_VALUES: LINE_KIND_VALUES2,
    LINE_NOISE_KIND_VALUES: LINE_NOISE_KIND_VALUES2,
    LINE_ROLE_VALUES: LINE_ROLE_VALUES2,
    LINE_SIGNAL_VALUES: LINE_SIGNAL_VALUES2,
    LineKind: LineKind2,
    LineNoiseKind: LineNoiseKind2,
    LineRole: LineRole2,
    LineSignal: LineSignal2,
    eventSearchText: eventSearchText2,
    fromWireLineEvent: fromWireLineEvent2,
    isNoiseLineEvent: isNoiseLineEvent2,
    legacyClsToKind: legacyClsToKind2,
    legacyClsToRole: legacyClsToRole2,
    noiseKindForEvent: noiseKindForEvent2,
    noiseKindForRole: noiseKindForRole2,
    toLegacyWireLineEvent: toLegacyWireLineEvent2,
    toWireLineEvent: toWireLineEvent2
  });
  return api;
})(typeof window !== "undefined" ? window : globalThis);
var {
...
var DarklabOutputCore = (function(global) {
  const OUTPUT_SIGNAL_SCOPES2 = Object.freeze(["findings", "warnings", "errors", "summaries"]);
  ...
  const OUTPUT_COMMAND_OUTCOME_SUMMARY_CLASSES2 = Object.freeze([
  ...
  const OUTPUT_SYNTHETIC_SUMMARY_CLASSES = Object.freeze([
    ...
    ...OUTPUT_COMMAND_OUTCOME_SUMMARY_CLASSES2
  ]);
  function promptIdentityPrefix3(rawPrefix = "") {
  ...
  function promptIdentityFromParts2(username = "", domain = "") {
  ...
  function normalizeWorkspaceCwd2(rawPath = "") {
    return String(rawPath || "").split("/").map((part) => String(part || "").trim()).filter(Boolean).join("/");
  }
  function workspaceDisplayPath2(path = "") {
    const normalized = normalizeWorkspaceCwd2(path);
    return normalized ? `/${normalized}` : "/";
  }
  function buildPromptLabel3(rawPrefix = "", path = "~") {
    return `${promptIdentityPrefix3(rawPrefix)}:${String(path || "~")} $`;
  }
  function buildPromptLabelFromParts2(username = "", domain = "", path = "~") {
    return `${promptIdentityFromParts2(username, domain)}:${String(path || "~")} $`;
  }
  function _escapeRegex(text) {
    return String(text || "").replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  }
  function stripPromptLabelFromEchoText3(text = "", currentLabel = "", identityPrefix = "") {
    const value = String(text || "");
    const current = String(currentLabel || "");
    if (current && value.startsWith(current)) return value.slice(current.length).replace(/^\s+/, "");
    const identity = promptIdentityPrefix3(identityPrefix);
  ...
  function formatOutputPrefix2(index, tsText, includeTimestamp, lineMode, timestampMode) {
    const parts = [];
  ...
  function emptySignalCounts2() {
    return { findings: 0, warnings: 0, errors: 0, summaries: 0 };
  }
  function isSignalSummaryClassName2(cls) {
    return OUTPUT_SIGNAL_SUMMARY_CLASSES.includes(cls);
  }
  function isSyntheticSummaryClassName2(cls) {
  ...
  function normalizeCommandOutcomeSummary2(raw) {
    if (!raw) return null;
  ...
  function _outcomeLines(lines) {
    const model = typeof DarklabRunOutputModel !== "undefined" && DarklabRunOutputModel || null;
    return (Array.isArray(lines) ? lines : []).filter((line) => line && typeof line === "object").filter((line) => {
      ...
      return !cls.some((name) => isSyntheticSummaryClassName2(name));
  ...
  function _parseDigOutcome(lines) {
    ...
    lines.forEach((line) => {
      ...
      if (statusMatch) status3 = statusMatch[1].toUpperCase();
    ...
    const items = [];
    _pushOutcomeItem(items, "Status", status3);
  ...
  function buildCommandOutcomeSummary2(command = "", rawLines = []) {
    const root = _outcomeCommandRoot(command);
    const lines = _outcomeLines(rawLines);
    if (!root || !lines.length) return null;
    try {
      if (root === "nmap") return normalizeCommandOutcomeSummary2(_parseNmapOutcome(lines));
      if (root === "dig") return normalizeCommandOutcomeSummary2(_parseDigOutcome(lines));
      if (root === "nslookup") return normalizeCommandOutcomeSummary2(_parseNslookupOutcome(lines));
      if (root === "curl") return normalizeCommandOutcomeSummary2(_parseCurlOutcome(lines));
      if (root === "openssl") return normalizeCommandOutcomeSummary2(_parseOpenSslOutcome(command, lines));
    } catch (_) {
      return null;
    }
    return null;
  }
  function lineHasClass2(rawLine, className) {
  ...
  function lineRole(rawLine) {
    ...
    return lineHasClass2(rawLine, "prompt-echo") ? "prompt-echo" : "body";
  }
  function isSignalCountableLine2(rawLine) {
    if (!rawLine || lineRole(rawLine) === "prompt-echo") return false;
    const classes = String(rawLine.cls || "").split(/\s+/).filter(Boolean);
    return !classes.some((cls) => isSyntheticSummaryClassName2(cls));
  }
  function isBuiltinCommandRoot2(root, builtinRoots = []) {
    return !!root && Array.isArray(builtinRoots) && builtinRoots.includes(root);
  }
  function normalizeSignals2(signals) {
    return Array.isArray(signals) ? signals.map((signal) => String(signal || "")).filter(Boolean) : [];
  }
  function normalizeEntities2(entities) {
  ...
  function countableSignalScopes2(rawLine, builtinRoots = []) {
    if (!isSignalCountableLine2(rawLine)) return [];
    const commandRoot = String(rawLine?.command_root || "").trim();
    if (isBuiltinCommandRoot2(commandRoot, builtinRoots)) return [];
    const signals = normalizeSignals2(rawLine?.signals);
    if (!signals.length) return [];
    const uniqueScopes = new Set(signals.filter((scope) => OUTPUT_SIGNAL_SCOPES2.includes(scope)));
    return Array.from(uniqueScopes);
  }
  const api = Object.freeze({
    OUTPUT_COMMAND_OUTCOME_SUMMARY_CLASSES: OUTPUT_COMMAND_OUTCOME_SUMMARY_CLASSES2,
    OUTPUT_SIGNAL_SCOPES: OUTPUT_SIGNAL_SCOPES2,
    buildCommandOutcomeSummary: buildCommandOutcomeSummary2,
    buildPromptLabel: buildPromptLabel3,
    buildPromptLabelFromParts: buildPromptLabelFromParts2,
    countableSignalScopes: countableSignalScopes2,
    emptySignalCounts: emptySignalCounts2,
    formatOutputPrefix: formatOutputPrefix2,
    isBuiltinCommandRoot: isBuiltinCommandRoot2,
    isSignalCountableLine: isSignalCountableLine2,
    isSignalSummaryClassName: isSignalSummaryClassName2,
    isSyntheticSummaryClassName: isSyntheticSummaryClassName2,
    lineHasClass: lineHasClass2,
    normalizeCommandOutcomeSummary: normalizeCommandOutcomeSummary2,
    normalizeEntities: normalizeEntities2,
    normalizeSignals: normalizeSignals2,
    normalizeWorkspaceCwd: normalizeWorkspaceCwd2,
    promptIdentityFromParts: promptIdentityFromParts2,
    promptIdentityPrefix: promptIdentityPrefix3,
    stripPromptLabelFromEchoText: stripPromptLabelFromEchoText3,
    workspaceDisplayPath: workspaceDisplayPath2
  });
  return api;
})(typeof window !== "undefined" ? window : globalThis);
var { OUTPUT_COMMAND_OUTCOME_SUMMARY_CLASSES, OUTPUT_SIGNAL_SCOPES, buildCommandOutcomeSummary, buildPromptLabel, buildPromptLabelFromParts, countableSignalScopes, emptySignalCounts, formatOutputPrefix, isBuiltinCommandRoot, isSignalCountableLine, isSignalSummaryClassName, isSyntheticSummaryClassName, lineHasClass, normalizeCommandOutcomeSummary, normalizeEntities, normalizeSignals, normalizeWorkspaceCwd, promptIdentityFromParts, promptIdentityPrefix, stripPromptLabelFromEchoText, workspaceDisplayPath } = DarklabOutputCore;
...
function openAtlas(...args) {
  return typeof atlasHandlers.openAtlas === "function" ? atlasHandlers.openAtlas(...args) : void 0;
}
function closeAtlas(...args) {
  return typeof atlasHandlers.closeAtlas === "function" ? atlasHandlers.closeAtlas(...args) : false;
}
function isAtlasOverlayOpen(...args) {
  return typeof atlasHandlers.isAtlasOverlayOpen === "function" ? !!atlasHandlers.isAtlasOverlayOpen(...args) : false;
}
function cycleAtlasTab(...args) {
  return typeof atlasHandlers.cycleAtlasTab === "function" ? atlasHandlers.cycleAtlasTab(...args) : false;
}

// app/static/js/core/autocomplete_core.js
var DarklabAutocompleteCore = (function(global) {
  const RECENT_VALUE_LIMIT2 = 10;
  ...
  function tokenContextFromText(value, cursorPos, offset = 0) {
    ...
    const containing = tokens.find((token) => cursor > token.start && cursor <= token.end) || tokens.find((token) => cursor === token.start && cursor === token.end);
  ...
  function _searchFields(item) {
    const fields = [itemInsertValue(item), itemText(item)].map((value) => String(value || "").trim()).filter(Boolean);
  ...
  function scoreItem(item, query, originalIndex) {
    ...
    const fieldScores = _searchFields(item).map((field, fieldIndex) => {
  ...
  function filterItems(items, query) {
    ...
    const filteredConcrete = concrete.map((item, index) => scoreItem(item, q, index)).filter(Boolean).sort((left, right) => left.score - right.score || left.index - right.index).map((result) => result.item);
  ...
  function isDomainValue(value) {
    ...
    return labels.length >= 2 && labels.every((label) => label.length >= 1 && label.length <= 63 && /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(label));
  }
  ...
  function normalizeRecentPortSet(value) {
    ...
    for (const part of parts) {
      ...
      if (!Number.isInteger(start) || !Number.isInteger(end) || start < 1 || start > 65535 || end < 1 || end > 65535 || start > end) {
        return "";
      }
  ...
  function normalizeRecentValueList(items) {
    ...
    (Array.isArray(items) ? items : []).forEach((item) => {
      ...
      if (next.filter((existing) => existing.kind === normalized.kind).length >= RECENT_VALUE_LIMIT2) return;
  ...
  function highlightedLabel(label, query) {
    ...
    if (substringIndex >= 0) {
      return escapeHtmlText(text.slice(0, substringIndex)) + '<span class="ac-match">' + escapeHtmlText(text.slice(substringIndex, substringIndex + q.length)) + "</span>" + escapeHtmlText(text.slice(substringIndex + q.length));
    }
    const fuzzyIndexes = fuzzyMatchIndexes(text, q);
  ...
  function sharedPrefix(items) {
    ...
    const completionPrefix = items[0] && typeof items[0] === "object" ? String(items[0].completionPrefix || "").trim() : "";
    if (completionPrefix && items.every((item) => item && typeof item === "object" && String(item.completionPrefix || "").trim() === completionPrefix)) {
  ...
  const api = Object.freeze({
    RECENT_VALUE_LIMIT: RECENT_VALUE_LIMIT2,
    escapeHtmlText,
    itemText,
    itemInsertValue,
    itemInsertText,
...
var {
  RECENT_VALUE_LIMIT: autocompleteRecentValueLimit,
  boundaryIndex: autocompleteBoundaryIndex,
  buildItem: autocompleteBuildItem,
  escapeHtmlText: autocompleteEscapeHtmlText,
  filterItems: autocompleteFilterItems,
  fuzzyMatchIndexes: autocompleteFuzzyMatchIndexes,
  highlightedLabel: autocompleteHighlightedLabel,
  isDomainValue: autocompleteIsDomainValue,
  isIPv4Address: autocompleteIsIPv4Address,
  isIPv6Address: autocompleteIsIPv6Address,
  itemDescription: autocompleteItemDescription,
  itemInsertText: autocompleteItemInsertText,
  itemInsertValue: autocompleteItemInsertValue,
  itemText: autocompleteItemText,
  limitItemsForDisplay: autocompleteLimitItemsForDisplay,
  normalizeRecentDomain: autocompleteNormalizeRecentDomain,
  normalizeRecentIp: autocompleteNormalizeRecentIp,
  normalizeRecentPortSet: autocompleteNormalizeRecentPortSet,
  normalizeRecentUrl: autocompleteNormalizeRecentUrl,
  normalizeRecentValue: autocompleteNormalizeRecentValue,
  normalizeRecentValueList: autocompleteNormalizeRecentValueList,
  normalizeWordlistCategories: autocompleteNormalizeWordlistCategories,
  scoreItem: autocompleteScoreItem,
  sharedPrefix: autocompleteSharedPrefix,
  tokenContextFromText: autocompleteTokenContextFromText
} = DarklabAutocompleteCore;

// app/static/js/runner_bridge.js
var RUNNER_BRIDGE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var warnedMissingRunnerHandlers = /* @__PURE__ */ new Set();
var runnerHandlers = RUNNER_BRIDGE_GLOBAL.__darklabRunnerHandlers || {
...
function _bridgeWarningsEnabled() {
  const config = RUNNER_BRIDGE_GLOBAL?.APP_CONFIG && typeof RUNNER_BRIDGE_GLOBAL.APP_CONFIG === "object" && !Array.isArray(RUNNER_BRIDGE_GLOBAL.APP_CONFIG) ? RUNNER_BRIDGE_GLOBAL.APP_CONFIG : {};
...
function _readRunErrorMessage(...args) {
  return _callRunnerHandler("_readRunErrorMessage", Promise.resolve(""), args);
}
function _recordSuccessfulLocalCommand(...args) {
  return _callRunnerHandler("_recordSuccessfulLocalCommand", void 0, args);
}
function _seedLocalStorageStarsToServer(...args) {
  return _callRunnerHandler("_seedLocalStorageStarsToServer", Promise.resolve(), args);
}
function _sseMessageFromChunk(...args) {
  return _callRunnerHandler("_sseMessageFromChunk", null, args);
}
function appendCommandEcho(...args) {
  return _callRunnerHandler("appendCommandEcho", void 0, args);
}
function attachActiveRunFromMonitor(...args) {
  return _callRunnerHandler("attachActiveRunFromMonitor", void 0, args);
}
function cancelPendingTerminalConfirm(...args) {
  return _callRunnerHandler("cancelPendingTerminalConfirm", void 0, args);
}
function confirmKill(...args) {
  return _callRunnerHandler("confirmKill", void 0, args);
}
function detachRunStreamForTab(...args) {
  return _callRunnerHandler("detachRunStreamForTab", void 0, args);
}
function doKill(...args) {
  return _callRunnerHandler("doKill", void 0, args);
}
function hasPendingTerminalConfirm(...args) {
  return _callRunnerHandler("hasPendingTerminalConfirm", false, args);
}
function interruptPromptLine(...args) {
  return _callRunnerHandler("interruptPromptLine", void 0, args);
}
function killActiveRunFromMonitor(...args) {
  return _callRunnerHandler("killActiveRunFromMonitor", void 0, args);
}
function pauseBackgroundRunStreamsForStatusMonitor(...args) {
  return _callRunnerHandler("pauseBackgroundRunStreamsForStatusMonitor", void 0, args);
}
function resumeBackgroundRunStreamsAfterStatusMonitor(...args) {
  return _callRunnerHandler("resumeBackgroundRunStreamsAfterStatusMonitor", void 0, args);
}
function runCommand(...args) {
  return _callRunnerHandler("runCommand", void 0, args);
}
function setStatus(...args) {
  return _callRunnerHandler("setStatus", void 0, args);
}
function submitCommand(...args) {
  return _callRunnerHandler("submitCommand", false, args);
}
function submitComposerCommand(...args) {
  return _callRunnerHandler("submitComposerCommand", false, args);
}
function submitVisibleComposerCommand(...args) {
  return _callRunnerHandler("submitVisibleComposerCommand", false, args);
}
if (RUNNER_BRIDGE_GLOBAL) {
...
var closeMajorOverlaysHandler = null;
...
function closeMajorOverlays(...args) {
  return typeof closeMajorOverlaysHandler === "function" ? closeMajorOverlaysHandler(...args) : void 0;
}

// app/static/js/features/workspace/workspace_autocomplete_cache.js
...
async function refreshWorkspaceFileCache() {
  ...
  const fetcher = typeof state.apiFetch === "function" ? state.apiFetch : typeof _workspaceApi().apiFetch === "function" ? _workspaceApi().apiFetch : null;
  const isEnabled = typeof state.isEnabled === "function" ? state.isEnabled : typeof _workspaceApi().isWorkspaceEnabled === "function" ? _workspaceApi().isWorkspaceEnabled : null;
  if (!isEnabled || !isEnabled() || !fetcher) return state.files || [];
  try {
    const resp = await fetcher("/workspace/files");
    const data = typeof state.parseJson === "function" ? await state.parseJson(resp) : await resp.json();
    const nextOwner = typeof state.ownerFromPayload === "function" ? state.ownerFromPayload(data) : {};
    const scopeKey = typeof state.activeScopeKeyFromOwner === "function" ? state.activeScopeKeyFromOwner : null;
    const previousScopeKey = scopeKey ? scopeKey(state.owner) : "personal";
    const nextScopeKey = scopeKey ? scopeKey(nextOwner) : previousScopeKey;
...
function getWorkspaceAutocompleteFileHints() {
  ...
  const formatBytes2 = typeof state.formatBytes === "function" ? state.formatBytes : (value) => `${Number(value) || 0} B`;
  return files.map((file) => {
    const path = String(file.path || "").trim();
    return {
      value: path,
      description: `${state.owner && state.owner.scope === "team" ? "team" : "personal"} file · ${formatBytes2(file.size)}`
    };
...
var DarklabHistoryCore = (function(global) {
  const GRACEFUL_TERMINATION_EXIT_CODES = /* @__PURE__ */ new Set([-15]);
  function normalizeFilterValue2(value) {
    return typeof value === "string" ? value.trim() : "";
  }
  function hasActiveServerFilters2(filters) {
    const state = filters || {};
    const type = state.type || "all";
    const exitCode = state.exitCode || "all";
    const dateRange = state.dateRange || "all";
    return Boolean(
      type !== "all" || state.q || state.commandRoot || state.signal && state.signal !== "all" || state.kind && state.kind !== "all" || state.entity || state.entityType && state.entityType !== "all" || exitCode !== "all" || dateRange !== "all" || state.projectId && state.projectId !== "all"
    );
  }
  function hasAnyFilters2(filters) {
    return hasActiveServerFilters2(filters) || !!(filters && filters.starredOnly);
  }
  function resetRunOnlyFilters2(filters) {
    return {
      ...filters || {},
      commandRoot: "",
      signal: "all",
  ...
  function labelForType2(type = "all") {
  ...
  function summaryLabel2(type = "all", totalCount = 0) {
  ...
  function commandRootsFromRuns2(runs) {
    const roots = /* @__PURE__ */ new Set();
    for (const run of Array.isArray(runs) ? runs : []) {
      const root = typeof run === "string" ? run.trim() : run && typeof run.command === "string" ? run.command.trim().split(/\s+/, 1)[0] : "";
      if (root) roots.add(root);
    }
    return [...roots].sort((a, b) => a.localeCompare(b));
  }
  function rootMatches2(suggestions, query, limit = 12) {
    const value = normalizeFilterValue2(query).toLowerCase();
    if (!value) return [];
    return (Array.isArray(suggestions) ? suggestions : []).filter((root) => String(root || "").toLowerCase().startsWith(value)).slice(0, Math.max(0, Number(limit) || 0));
  }
  function activeFilterItems2(filters) {
    const state = filters || {};
    const items = [];
    const type = state.type || "all";
    const exitCode = state.exitCode || "all";
    const dateRange = state.dateRange || "all";
    if (type !== "all") items.push({ key: "type", label: `type: ${labelForType2(type)}` });
  ...
  function buildRequestUrl2(filters, paging) {
    const state = filters || {};
  ...
  function historyLimit2(appConfig) {
    return Math.max(1, Number(appConfig && appConfig.recent_commands_limit) || 50);
  }
  function commandRecallHistory2(tab, globalHistory, limit) {
    const seen = /* @__PURE__ */ new Set();
    const local = tab && Array.isArray(tab.commandHistory) ? tab.commandHistory : [];
    return [...local, ...Array.isArray(globalHistory) ? globalHistory : []].map((cmd) => String(cmd || "")).filter((cmd) => {
      if (!cmd || seen.has(cmd)) return false;
      seen.add(cmd);
      return true;
    }).slice(0, Math.max(1, Number(limit) || 50));
  }
  function relativeTime2(startedAt, now = /* @__PURE__ */ new Date()) {
    if (!(startedAt instanceof Date) || Number.isNaN(startedAt.getTime())) return "";
    const diffSec = Math.round((now.getTime() - startedAt.getTime()) / 1e3);
    ...
    return startedAt.toLocaleDateString(void 0, { month: "short", day: "numeric" });
  }
  function exitCodeNumber2(exitCode) {
    if (exitCode === null || exitCode === void 0 || exitCode === "") return null;
    const number = Number(exitCode);
    return Number.isFinite(number) ? number : null;
  }
  function isGracefulTerminationExitCode2(exitCode) {
    const code = exitCodeNumber2(exitCode);
    return code !== null && GRACEFUL_TERMINATION_EXIT_CODES.has(code);
  }
  function isFailedExitCode2(exitCode) {
    const code = exitCodeNumber2(exitCode);
    return code !== null && code !== 0 && !GRACEFUL_TERMINATION_EXIT_CODES.has(code);
  }
  function exitLabel2(exitCode) {
    const code = exitCodeNumber2(exitCode);
    if (code === null) return "exit —";
    return isGracefulTerminationExitCode2(code) ? "terminated" : `exit ${code}`;
  }
  function exitClass2(exitCode) {
    const code = exitCodeNumber2(exitCode);
    if (code === 0) return "exit-ok";
    if (isFailedExitCode2(code)) return "exit-fail";
    return "exit-neutral";
  }
  function elapsedSeconds3(run) {
    ...
    return Math.max(0, (finished.getTime() - started.getTime()) / 1e3);
  }
  function elapsedLabel2(run) {
    const total = elapsedSeconds3(run);
  ...
  function compareDateGroupLabel2(value, now = /* @__PURE__ */ new Date()) {
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) return "Undated";
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const dateStart = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    const ageDays = Math.round((todayStart.getTime() - dateStart.getTime()) / 864e5);
    ...
    return date.toLocaleDateString(void 0, {
      ...
      month: "short",
      day: "numeric",
      year: date.getFullYear() === now.getFullYear() ? void 0 : "numeric"
    });
  }
  function compareFormatDuration2(seconds) {
    if (seconds === null || seconds === void 0 || Number.isNaN(Number(seconds))) return "n/a";
    const value = Number(seconds);
    if (value < 1) return `${Math.round(value * 1e3)}ms`;
  ...
  const api = Object.freeze({
    activeFilterItems: activeFilterItems2,
    buildRequestUrl: buildRequestUrl2,
    commandRecallHistory: commandRecallHistory2,
    commandRootsFromRuns: commandRootsFromRuns2,
    ...
    elapsedLabel: elapsedLabel2,
    elapsedSeconds: elapsedSeconds3,
    exitClass: exitClass2,
    exitCodeNumber: exitCodeNumber2,
    exitLabel: exitLabel2,
    hasActiveServerFilters: hasActiveServerFilters2,
    hasAnyFilters: hasAnyFilters2,
    historyLimit: historyLimit2,
    isFailedExitCode: isFailedExitCode2,
    isGracefulTerminationExitCode: isGracefulTerminationExitCode2,
    labelForType: labelForType2,
    normalizeFilterValue: normalizeFilterValue2,
    relativeTime: relativeTime2,
    resetRunOnlyFilters: resetRunOnlyFilters2,
    rootMatches: rootMatches2,
    summaryLabel: summaryLabel2
  });
  return api;
})(typeof window !== "undefined" ? window : globalThis);
var { activeFilterItems, buildRequestUrl, commandRecallHistory, commandRootsFromRuns, compareDateGroupLabel, compareFormatDate, compareFormatDelta, compareFormatDuration, elapsedLabel, elapsedSeconds, exitClass, exitCodeNumber, exitLabel, hasActiveServerFilters, hasAnyFilters, historyLimit, isFailedExitCode, isGracefulTerminationExitCode, labelForType, normalizeFilterValue, relativeTime, resetRunOnlyFilters, rootMatches, summaryLabel } = DarklabHistoryCore;

// app/static/js/features/history/history_recall.js
var HISTORY_RECALL_GLOBAL = typeof window !== "undefined" ? window : globalThis;
...
function _historyRecallComposerValue() {
  const getter = typeof getComposerValue !== "undefined" && getComposerValue || HISTORY_RECALL_GLOBAL.getComposerValue;
  const input = typeof cmdInput !== "undefined" && cmdInput || HISTORY_RECALL_GLOBAL.cmdInput;
...
function _historyRecallSetComposerValue(value) {
  const setter = typeof setComposerValue !== "undefined" && setComposerValue || HISTORY_RECALL_GLOBAL.setComposerValue;
  if (typeof setter === "function") setter(value);
}
function _historyRecallIsSearchMode() {
  const isMode = typeof isHistSearchMode !== "undefined" && isHistSearchMode || HISTORY_RECALL_GLOBAL.isHistSearchMode;
  return typeof isMode === "function" ? isMode() : false;
}
function _historyRecallExitSearch() {
  const exitSearch = typeof exitHistSearch !== "undefined" && exitHistSearch || HISTORY_RECALL_GLOBAL.exitHistSearch;
  if (typeof exitSearch === "function") exitSearch(false);
}
function _activeTabCommandHistoryState() {
  const getActive = typeof getActiveTab !== "undefined" && getActiveTab || (typeof HISTORY_RECALL_GLOBAL.APP_STATE_API?.getActiveTab === "function" ? HISTORY_RECALL_GLOBAL.APP_STATE_API.getActiveTab : null);
...
function addToRecentPreview(cmd) {
  ...
  _historyRecallSetRecentPreviewHistory(
    [cmd, ...previewHistory.filter((c) => c !== cmd)].slice(0, config.recent_commands_limit)
  );
  _historyRecallRenderHistory();
}
function hydrateCmdHistory(runs) {
  ...
  const seen = /* @__PURE__ */ new Set();
  const config = _historyRecallAppConfig();
  _historyRecallSetCmdHistory(items.map((run) => run && typeof run.command === "string" ? run.command : "").filter((cmd) => {
    if (!cmd || seen.has(cmd)) return false;
    seen.add(cmd);
    return true;
  }).slice(0, config.recent_commands_limit));
  const previewSeen = /* @__PURE__ */ new Set();
  _historyRecallSetRecentPreviewHistory(items.map((run) => run && typeof run.command === "string" ? run.command : "").filter((cmd) => {
...
var HISTORY_ACTIONS_GLOBAL = typeof window !== "undefined" ? window : globalThis;
...
function _historyActionsHydrateCmdHistory(runs) {
  const hydrate = typeof hydrateCmdHistory !== "undefined" && hydrateCmdHistory || (typeof HISTORY_ACTIONS_GLOBAL?.hydrateCmdHistory === "function" ? HISTORY_ACTIONS_GLOBAL.hydrateCmdHistory : null);
  if (typeof hydrate === "function") hydrate(runs);
}
function _historyActionsIsPanelOpen() {
  const isOpen = typeof isHistoryPanelOpen !== "undefined" && isHistoryPanelOpen || (typeof HISTORY_ACTIONS_GLOBAL?.isHistoryPanelOpen === "function" ? HISTORY_ACTIONS_GLOBAL.isHistoryPanelOpen : null);
  return typeof isOpen === "function" ? isOpen() : false;
}
function _historyActionsApiFetch(...args) {
  const fetcher = (typeof hasRuntimeHandler === "function" && hasRuntimeHandler("apiFetch") && typeof apiFetch === "function" ? apiFetch : null) || (typeof HISTORY_ACTIONS_GLOBAL?.apiFetch === "function" ? HISTORY_ACTIONS_GLOBAL.apiFetch : null);
  return fetcher ? fetcher(...args) : Promise.reject(new Error("apiFetch unavailable"));
}
function _historyActionsRefreshHistoryPanel() {
  const refresh = typeof HISTORY_ACTIONS_GLOBAL?.refreshHistoryPanel === "function" ? HISTORY_ACTIONS_GLOBAL.refreshHistoryPanel : null;
  if (refresh) refresh();
}
function _getStarred() {
  return _starredCache !== null ? _starredCache : /* @__PURE__ */ new Set();
}
...
function _positionHistoryActionMenu(wrap) {
  ...
  const top = belowTop + menuHeight <= viewportHeight - gutter ? belowTop : Math.max(gutter, aboveTop);
...
// app/static/js/features/projects/project_context_bridge.js
var PROJECT_CONTEXT_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var projectContextHandlers = PROJECT_CONTEXT_GLOBAL.__darklabProjectContextHandlers || {
...
function getActiveProjectContext(...args) {
  return typeof projectContextHandlers.getActiveProjectContext === "function" ? projectContextHandlers.getActiveProjectContext(...args) : null;
}
function refreshActiveProjectContext(...args) {
  return typeof projectContextHandlers.refreshActiveProjectContext === "function" ? projectContextHandlers.refreshActiveProjectContext(...args) : Promise.resolve(null);
}
function openProjectWorkspace(...args) {
  return typeof projectContextHandlers.openProjectWorkspace === "function" ? projectContextHandlers.openProjectWorkspace(...args) : Promise.resolve(false);
}
function closeProjectWorkspace(...args) {
  return typeof projectContextHandlers.closeProjectWorkspace === "function" ? projectContextHandlers.closeProjectWorkspace(...args) : void 0;
}
function cycleProjectWorkspaceTab(...args) {
  return typeof projectContextHandlers.cycleProjectWorkspaceTab === "function" ? projectContextHandlers.cycleProjectWorkspaceTab(...args) : false;
}
function refreshProjectWorkspace(...args) {
  return typeof projectContextHandlers.refreshProjectWorkspace === "function" ? projectContextHandlers.refreshProjectWorkspace(...args) : Promise.resolve(false);
}
function isProjectWorkspaceOpen(...args) {
  return typeof projectContextHandlers.isProjectWorkspaceOpen === "function" ? !!projectContextHandlers.isProjectWorkspaceOpen(...args) : false;
}
function openProjectAutoPromoteRuleFromAtlas(...args) {
  return typeof projectContextHandlers.openProjectAutoPromoteRuleFromAtlas === "function" ? projectContextHandlers.openProjectAutoPromoteRuleFromAtlas(...args) : Promise.resolve(false);
}
function openEntityMetadataEditor(...args) {
  return typeof projectContextHandlers.openEntityMetadataEditor === "function" ? projectContextHandlers.openEntityMetadataEditor(...args) : void 0;
}
...
function refreshOptionsSecrets(...args) {
  return typeof secretsHandlers.refreshOptionsSecrets === "function" ? secretsHandlers.refreshOptionsSecrets(...args) : void 0;
}
function invalidateOptionsSecrets(...args) {
  return typeof secretsHandlers.invalidateOptionsSecrets === "function" ? secretsHandlers.invalidateOptionsSecrets(...args) : void 0;
}
function closeProviderStatusModal(...args) {
  return typeof secretsHandlers.closeProviderStatusModal === "function" ? secretsHandlers.closeProviderStatusModal(...args) : void 0;
}
function isProviderStatusModalOpen(...args) {
  return typeof secretsHandlers.isProviderStatusModalOpen === "function" ? !!secretsHandlers.isProviderStatusModalOpen(...args) : false;
}
function handleSecretCommand(...args) {
  return typeof secretsHandlers.handleSecretCommand === "function" ? secretsHandlers.handleSecretCommand(...args) : void 0;
}

// app/static/js/features/team_scope.js
var DarklabTeamScope = null;
(function initTeamScope(global) {
  ...
  const apiFetchImpl = typeof apiFetch !== "undefined" && apiFetch || null;
  ...
  const blurVisibleComposerInputIfMobile3 = typeof blurVisibleComposerInputIfMobile !== "undefined" && blurVisibleComposerInputIfMobile || null;
  const logClientErrorImpl = typeof logClientError !== "undefined" && logClientError || null;
  const loadRecentValuesImpl = typeof loadRecentValues !== "undefined" && loadRecentValues || null;
  const refocusComposerAfterAction3 = typeof refocusComposerAfterAction !== "undefined" && refocusComposerAfterAction || null;
  const reloadSessionHistoryImpl = typeof reloadSessionHistory !== "undefined" && reloadSessionHistory || null;
  const refreshWorkspaceFileCacheImpl = typeof refreshWorkspaceFileCache !== "undefined" && refreshWorkspaceFileCache || null;
  const refreshActiveProjectContextImpl = typeof refreshActiveProjectContext !== "undefined" && refreshActiveProjectContext || null;
  const refreshActiveRunsImpl = typeof global.refreshActiveRuns === "function" ? global.refreshActiveRuns : null;
  const refreshStatusMonitorImpl = typeof refreshStatusMonitor !== "undefined" && refreshStatusMonitor || null;
  const invalidateOptionsSecretsImpl = typeof invalidateOptionsSecrets !== "undefined" && invalidateOptionsSecrets || null;
  ...
  function storageKey() {
    const sessionId = typeof getSessionId === "function" && getSessionId() || "anonymous";
  ...
  function getActiveTeamId2() {
    return activeTeamId || "";
  }
  function getActiveTeam2() {
    if (!activeTeamId) return null;
    return teams.find((item) => item.id === activeTeamId) || null;
  }
  function getActiveTeamCapabilities2() {
    const team = getActiveTeam2();
  ...
  function activeTeamScopeCan2(capability) {
    ...
    return getActiveTeamCapabilities2().includes(wanted);
  }
  function teamScopeDeniedMessage2(action = "make this change") {
  ...
  function renderMenuButton(team = null) {
    ...
    button.addEventListener("click", (event) => {
      ...
      setActiveTeamId2(button.dataset.teamScopeMenuOption || "", { source: "selector" });
      closeScopeMenu();
    });
    return button;
  }
  ...
  function setActiveTeamId2(teamId, { persist = true, emit = true, allowPending = false, source = "direct" } = {}) {
  ...
  function ensureScopeMenu() {
    ...
    menuList = section;
    menuNote = note;
    if (typeof bindOutsideClickClose2 === "function") {
  ...
  function closeTeamScopeSelector2({ refocus = true } = {}) {
    closeScopeMenu();
    if (!overlay) return;
    ...
    if (refocus && typeof refocusComposerAfterAction3 === "function") {
      refocusComposerAfterAction3({ defer: true });
    }
  }
  function showTeamScopeSelector() {
    ...
    if (typeof blurVisibleComposerInputIfMobile3 === "function") blurVisibleComposerInputIfMobile3();
  ...
  function toggleScopeMenu(event) {
    ...
    if (typeof closeMajorOverlays === "function") closeMajorOverlays();
    closeTeamScopeSelector2({ refocus: false });
    ensureScopeMenu();
    render();
  ...
  function bindModalDismissal() {
    ...
    if (typeof bindDismissible2 === "function") {
      bindDismissible2(overlay, {
        level: "modal",
        isOpen,
        onClose: closeTeamScopeSelector2,
        closeButtons
      });
    } else {
      closeButtons.forEach((button) => {
        button.addEventListener("click", () => closeTeamScopeSelector2());
      });
      overlay.addEventListener("click", (event) => {
        if (event.target === overlay) closeTeamScopeSelector2();
      });
    }
    if (typeof bindMobileSheet2 === "function" && modal) {
      bindMobileSheet2(modal, { onClose: closeTeamScopeSelector2 });
    } else {
      grabHandle?.addEventListener("click", () => closeTeamScopeSelector2());
    }
  }
  function normalizeTeams(payload) {
    ...
    return rows.map((team) => ({
      ...
      capabilities: Array.isArray(team.member?.capabilities) ? team.member.capabilities.map((item) => String(item || "")).filter(Boolean) : []
    })).filter((team) => team.id);
  }
  function replaceTeamScopes2(payload) {
  ...
  async function refreshTeamScopes2() {
    ...
    refreshing = apiFetchImpl("/session/teams", { cache: "no-store" }).then(async (resp) => {
      if (resp.status === 401) {
        teams = [];
        teamScopesResolved = true;
        scopeLoadError = false;
        showStatus("");
        setActiveTeamId2("", { persist: true, emit: false });
        render();
        return teams;
      }
      const payload = await resp.json().catch(() => ({}));
      if (!resp.ok) throw new Error(payload.error || resp.statusText || resp.status);
      return replaceTeamScopes2(payload);
    }).catch((err) => {
  ...
  listEl?.addEventListener("click", (event) => {
    ...
    if (!option) return;
    event.preventDefault();
    if (setActiveTeamId2(option.dataset.teamScopeOption || "", { source: "selector" })) closeTeamScopeSelector2();
  });
  const openTeamScopeSelector2 = () => {
  ...
  DarklabTeamScope = {
    getActiveTeamId: getActiveTeamId2,
    getActiveTeam: getActiveTeam2,
    getActiveTeamCapabilities: getActiveTeamCapabilities2,
    activeTeamScopeCan: activeTeamScopeCan2,
    deniedMessage: teamScopeDeniedMessage2,
    setActiveTeamId: setActiveTeamId2,
    refreshTeamScopes: refreshTeamScopes2,
    replaceTeamScopes: replaceTeamScopes2,
    isOpen,
    open: openTeamScopeSelector2,
    close: closeTeamScopeSelector2
  };
  ...
  window.addEventListener("storage", (event) => {
    if (event.key === storageKey()) {
      ...
      if (nextTeamId !== activeTeamId) {
        setActiveTeamId2(nextTeamId, { persist: false, allowPending: true, source: "storage" });
      }
    }
  });
...
var getActiveTeamId = DarklabTeamScope ? DarklabTeamScope.getActiveTeamId : null;
var getActiveTeam = DarklabTeamScope ? DarklabTeamScope.getActiveTeam : null;
var getActiveTeamCapabilities = DarklabTeamScope ? DarklabTeamScope.getActiveTeamCapabilities : null;
var activeTeamScopeCan = DarklabTeamScope ? DarklabTeamScope.activeTeamScopeCan : null;
var teamScopeDeniedMessage = DarklabTeamScope ? DarklabTeamScope.deniedMessage : null;
var setActiveTeamId = DarklabTeamScope ? DarklabTeamScope.setActiveTeamId : null;
var refreshTeamScopes = DarklabTeamScope ? DarklabTeamScope.refreshTeamScopes : null;
var replaceTeamScopes = DarklabTeamScope ? DarklabTeamScope.replaceTeamScopes : null;
var isTeamScopeSelectorOpen = DarklabTeamScope ? DarklabTeamScope.isOpen : null;
var openTeamScopeSelector = DarklabTeamScope ? DarklabTeamScope.open : null;
var closeTeamScopeSelector = DarklabTeamScope ? DarklabTeamScope.close : null;

// app/static/js/output_bridge.js
var OUTPUT_BRIDGE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var warnedMissingOutputHandlers = /* @__PURE__ */ new Set();
var outputHandlers = {
...
function _bridgeWarningsEnabled2() {
  const config = OUTPUT_BRIDGE_GLOBAL?.APP_CONFIG && typeof OUTPUT_BRIDGE_GLOBAL.APP_CONFIG === "object" && !Array.isArray(OUTPUT_BRIDGE_GLOBAL.APP_CONFIG) ? OUTPUT_BRIDGE_GLOBAL.APP_CONFIG : {};
  return config.frontend_bridge_warnings === true || config.debug === true || config.dev_mode === true || config.environment === "development" || config.env === "development";
}
function _warnMissingOutputHandler(name) {
  if (warnedMissingOutputHandlers.has(name) || !_bridgeWarningsEnabled2()) return;
...
function _logOutputBridgeDiagnostic(level, event, details = {}) {
  if (!_bridgeWarningsEnabled2()) return;
...
function resetAnsiRendererForTab(...args) {
  return _callOutputHandler("resetAnsiRendererForTab", void 0, args);
}
function syncOutputPrefixes(...args) {
  return _callOutputHandler("syncOutputPrefixes", void 0, args);
}
function _resetTabOutputSignalCounts(...args) {
  return _callOutputHandler("_resetTabOutputSignalCounts", void 0, args);
}
function _cancelPendingOutputBatch(...args) {
  return _callOutputHandler("_cancelPendingOutputBatch", void 0, args);
}
function _stickOutputToBottom(...args) {
  return _callOutputHandler("_stickOutputToBottom", void 0, args);
}
function _restoreOutputTailAfterLayout(...args) {
  return _callOutputHandler("_restoreOutputTailAfterLayout", void 0, args);
}
function isTabSessionRestoreInProgress(...args) {
  return _callOutputHandler("isTabSessionRestoreInProgress", false, args);
}

// app/static/js/tabs_bridge.js
var TABS_BRIDGE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var warnedMissingTabHandlers = /* @__PURE__ */ new Set();
var tabHandlers = TABS_BRIDGE_GLOBAL.__darklabTabHandlers || {
...
function _bridgeWarningsEnabled3() {
  const config = TABS_BRIDGE_GLOBAL?.APP_CONFIG && typeof TABS_BRIDGE_GLOBAL.APP_CONFIG === "object" && !Array.isArray(TABS_BRIDGE_GLOBAL.APP_CONFIG) ? TABS_BRIDGE_GLOBAL.APP_CONFIG : {};
  return config.frontend_bridge_warnings === true || config.debug === true || config.dev_mode === true || config.environment === "development" || config.env === "development";
}
function _warnMissingTabHandler(name) {
  if (warnedMissingTabHandlers.has(name) || !_bridgeWarningsEnabled3()) return;
...
function _logTabBridgeDiagnostic(level, event, details = {}) {
  if (!_bridgeWarningsEnabled3()) return;
...
function _clearTabRunningLabelTimer(...args) {
  return _callTabHandler("_clearTabRunningLabelTimer", void 0, args);
}
function _getNeighborTabIdAfterClose(...args) {
  return _callTabHandler("_getNeighborTabIdAfterClose", null, args);
}
function _getTabEl(...args) {
  return _callTabHandler("_getTabEl", null, args);
}
function _getTabPanelEl(...args) {
  return _callTabHandler("_getTabPanelEl", null, args);
}
function activateTab(...args) {
  return _callTabHandler("activateTab", void 0, args);
}
function clearTab(...args) {
  return _callTabHandler("clearTab", void 0, args);
}
function createDefaultTabLabel(...args) {
  return _callTabHandler("createDefaultTabLabel", `shell ${args[0] || ""}`.trim(), args);
}
function createTab(...args) {
  return _callTabHandler("createTab", null, args);
}
function ensureActiveTabVisible(...args) {
  return _callTabHandler("ensureActiveTabVisible", void 0, args);
}
function getOutput(...args) {
  return _callTabHandler("getOutput", null, args);
}
function mountShellPrompt(...args) {
  return _callTabHandler("mountShellPrompt", void 0, args);
}
function setTabLabel(...args) {
  return _callTabHandler("setTabLabel", void 0, args);
}
function setTabStatus(...args) {
  return _callTabHandler("setTabStatus", void 0, args);
}
function syncTabOrderFromDom(...args) {
  return _callTabHandler("syncTabOrderFromDom", void 0, args);
}
function unmountShellPrompt(...args) {
  return _callTabHandler("unmountShellPrompt", void 0, args);
}
function updateNewTabBtn(...args) {
  return _callTabHandler("updateNewTabBtn", void 0, args);
}
function updateOutputFollowButton(...args) {
  return _callTabHandler("updateOutputFollowButton", void 0, args);
}
function updateTabScrollButtons(...args) {
  return _callTabHandler("updateTabScrollButtons", void 0, args);
}
if (TABS_BRIDGE_GLOBAL) {
...
var TAB_DRAG_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var _tabDragSuppressClickUntil = 0;
var _touchDragState = null;
var _TOUCH_TAB_DRAG_THRESHOLD = 14;
var _TOUCH_TAB_DRAG_HOLD_MS = 180;
var _POINTER_TAB_DRAG_THRESHOLD = 6;
function _tabDragTabsBar() {
  return typeof tabsBar !== "undefined" && tabsBar || TAB_DRAG_GLOBAL.tabsBar || null;
}
...
function _tabDragUpdateTabScrollButtons() {
  const update = typeof updateTabScrollButtons !== "undefined" && updateTabScrollButtons || (typeof TAB_DRAG_GLOBAL.updateTabScrollButtons === "function" ? TAB_DRAG_GLOBAL.updateTabScrollButtons : null);
  if (typeof update === "function") update();
}
function _tabDragSyncTabOrderFromDom() {
  const sync = typeof syncTabOrderFromDom !== "undefined" && syncTabOrderFromDom || (typeof TAB_DRAG_GLOBAL.syncTabOrderFromDom === "function" ? TAB_DRAG_GLOBAL.syncTabOrderFromDom : null);
  if (typeof sync === "function") sync();
}
function _tabDragEnsureActiveTabVisible(tabId) {
  const ensureVisible = typeof ensureActiveTabVisible !== "undefined" && ensureActiveTabVisible || (typeof TAB_DRAG_GLOBAL.ensureActiveTabVisible === "function" ? TAB_DRAG_GLOBAL.ensureActiveTabVisible : null);
  if (typeof ensureVisible === "function") ensureVisible(tabId);
}
function _tabDragRefocusComposerAfterAction() {
  const refocus = typeof refocusComposerAfterAction !== "undefined" && refocusComposerAfterAction || (typeof TAB_DRAG_GLOBAL.refocusComposerAfterAction === "function" ? TAB_DRAG_GLOBAL.refocusComposerAfterAction : null);
...
function _reorderDraggedTab(dragged, target, clientX) {
  ...
  const noChange = after ? target.nextSibling === dragged : target === dragged.nextSibling;
  if (noChange) {
...
function _cleanupTouchDrag() {
  ...
  if (_touchDragState.tab && typeof _touchDragState.pointerId === "number" && typeof _touchDragState.tab.releasePointerCapture === "function") {
    try {
      _touchDragState.tab.releasePointerCapture(_touchDragState.pointerId);
    } catch (_) {
    }
  }
...
function _onTouchDragEnd(e) {
  ...
  if (typeof window !== "undefined") {
    if (state.id === currentActiveTabId) _tabDragRefocusComposerAfterAction();
  }
}
...
var DETACHED_ACTIVE_RUNS_STORAGE_PREFIX = "detached_active_runs";
var RUNNER_ACTIVE_RESTORE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
...
function _writeDetachedActiveRunIds(detached) {
  if (typeof localStorage === "undefined") return;
  const entries = Object.entries(detached || {}).filter(([runId]) => String(runId || "").trim());
  try {
    if (!entries.length) {
...
function _pruneDetachedActiveRunRestoreIds(activeRunIds) {
  const activeIds = activeRunIds instanceof Set ? activeRunIds : /* @__PURE__ */ new Set();
  const detached = _readDetachedActiveRunIds();
  let changed = false;
...
var updateOptionsSessionTokenStatusHandler = null;
...
function updateOptionsSessionTokenStatus(...args) {
  return typeof updateOptionsSessionTokenStatusHandler === "function" ? updateOptionsSessionTokenStatusHandler(...args) : void 0;
}

// app/static/js/session.js
var SessionCore = typeof DarklabSessionCore !== "undefined" && DarklabSessionCore ? DarklabSessionCore : null;
var SESSION_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _sessionCore() {
  return typeof DarklabSessionCore !== "undefined" && DarklabSessionCore || SessionCore || SESSION_GLOBAL && SESSION_GLOBAL.DarklabSessionCore || null;
}
...
function _refreshWorkspaceFileCache() {
  const refresh = typeof refreshWorkspaceFileCache !== "undefined" && refreshWorkspaceFileCache || SESSION_GLOBAL.refreshWorkspaceFileCache;
  if (typeof refresh === "function") return refresh();
  return null;
}
function _sessionLogEvent(context, event, level, details = {}) {
  logClientError2(context, null, {
    event,
    level,
    ...details
  });
...
function _sessionCallAsync(name, reason = "session-update") {
  ...
  const fn = typeof importedFn === "function" ? importedFn : SESSION_GLOBAL && typeof SESSION_GLOBAL[name] === "function" ? SESSION_GLOBAL[name] : null;
  if (!fn) return;
  const result = fn();
  if (result && typeof result.catch === "function") {
    result.catch((err) => {
...
function _sessionRefreshOptionsSecretsIfOpen(reason = "session-update") {
  ...
  const refresh = (typeof hasSecretsHandler === "function" && hasSecretsHandler("refreshOptionsSecrets") && typeof refreshOptionsSecrets === "function" ? refreshOptionsSecrets : null) || (typeof SESSION_GLOBAL.refreshOptionsSecrets === "function" ? SESSION_GLOBAL.refreshOptionsSecrets : null);
...
function _sessionInvalidateOptionsSecrets() {
  const invalidate = (typeof hasSecretsHandler === "function" && hasSecretsHandler("invalidateOptionsSecrets") && typeof invalidateOptionsSecrets === "function" ? invalidateOptionsSecrets : null) || (typeof SESSION_GLOBAL.invalidateOptionsSecrets === "function" ? SESSION_GLOBAL.invalidateOptionsSecrets : null);
  if (invalidate) invalidate();
}
function _sessionUpdateOptionsSessionTokenStatus() {
  if (typeof updateOptionsSessionTokenStatus === "function") {
...
function maskSessionToken2(token) {
  return _sessionCore().maskSessionToken(token);
}
function apiFetch2(url, options = {}) {
  _ensureSessionIdentity();
...
function describeFetchError2(err, context = "server") {
  return _sessionCore().describeFetchError(err, context);
}
function logClientError2(context, err, details = null) {
  if (typeof console !== "undefined" && typeof console.warn === "function") {
...
if (typeof window !== "undefined") {
  if (typeof setRuntimeHandlers === "function") {
    setRuntimeHandlers({
      ...
      logClientError: logClientError2
    });
  }
}
_ensureSessionIdentity();
...
function _themeSelectEl() {
  return typeof themeSelect !== "undefined" && themeSelect || (typeof document !== "undefined" ? document.getElementById("theme-select") : null);
}
function _themeLogClientError(message, err) {
  const log = typeof logClientError2 === "function" ? logClientError2 : null;
...
function _getThemeRegistry() {
  ...
  const currentThemeVars = typeof window !== "undefined" && window.ThemeCssVars && window.ThemeCssVars.current && typeof window.ThemeCssVars.current === "object" ? window.ThemeCssVars.current : {};
  return {
    current: currentThemeName ? { name: currentThemeName, label: currentThemeName, source: "fallback", vars: currentThemeVars } : null,
    themes: []
  };
}
...
function _persistThemeEntry(entry) {
  if (!entry) return;
  try {
    if (typeof _persistCurrentSessionPreferences === "function") {
...
function _savedThemeName() {
  ...
  return (getPreference2 ? getPreference2("pref_theme_name") : "") || localStorage.getItem("theme") || (getPreference2 ? getPreference2("pref_theme") : "") || "";
}
...
function _buildThemePreviewCard(theme) {
  ...
  prompt.textContent = typeof buildPromptLabel2 === "function" ? buildPromptLabel2() : "anon@darklab.sh:~ $";
...
function renderThemeSelectionOptions() {
  const themeSelect2 = _themeSelectEl();
  if (!themeSelect2 || themeSelect2.dataset.wired === "1") return;
  ...
  if (!themes.length) {
    ...
    themeSelect2.appendChild(empty);
    themeSelect2.dataset.wired = "1";
    return;
  }
  ...
  themeSelect2.style.setProperty("--theme-picker-columns", String(desktopColumns));
  const mobileColumns = Math.max(1, Math.min(maxColumns, 2));
  themeSelect2.style.setProperty("--theme-picker-columns-mobile", String(mobileColumns));
  ...
  themes.forEach((theme) => {
    ...
    if (themeGroup !== currentGroup) {
      ...
      themeSelect2.appendChild(groupSection);
    }
...
function syncThemeSelectionControls() {
  ...
  const themeSelect2 = _themeSelectEl();
  if (!themeSelect2) return;
  themeSelect2.dataset.theme = themeName;
  themeSelect2.querySelectorAll("[data-theme-name]").forEach((card) => {
...
var THEME_GLOBAL = typeof window !== "undefined" ? window : globalThis;
if (THEME_GLOBAL) {
  THEME_GLOBAL.applyThemeSelection = applyThemeSelection;
  THEME_GLOBAL.syncThemeSelectionControls = syncThemeSelectionControls;
}

// app/static/js/features/projects/project_hud_bridge.js
var renderHudClockHandler = null;
...
function renderHudClock(...args) {
  return typeof renderHudClockHandler === "function" ? renderHudClockHandler(...args) : void 0;
}

// app/static/js/features/preferences/preferences.js
var PreferenceCore = typeof DarklabPreferenceCore !== "undefined" && DarklabPreferenceCore ? DarklabPreferenceCore : null;
var PREFERENCE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var _welcomeIntroModes = PreferenceCore.WELCOME_INTRO_MODES;
var _shareRedactionDefaultModes = PreferenceCore.SHARE_REDACTION_DEFAULT_MODES;
var _hudClockModes = PreferenceCore.HUD_CLOCK_MODES;
...
function _preferenceAppConfig() {
  return typeof APP_CONFIG !== "undefined" && APP_CONFIG || PREFERENCE_GLOBAL?.APP_CONFIG || {};
}
function _preferenceDefaultThemeEntry() {
  const defaultThemeEntry = typeof _defaultThemeEntry !== "undefined" && _defaultThemeEntry || _preferenceGlobalFunction("_defaultThemeEntry");
  return typeof defaultThemeEntry === "function" ? defaultThemeEntry() : null;
}
function _preferenceSavedThemeName() {
  const savedThemeName = typeof _savedThemeName !== "undefined" && _savedThemeName || _preferenceGlobalFunction("_savedThemeName");
...
function _preferenceApiFetch(...args) {
  const fetcher = typeof apiFetch === "function" && apiFetch || _preferenceGlobalFunction("apiFetch");
  if (!fetcher) throw new Error("apiFetch is not available");
  return fetcher(...args);
}
function _preferenceLogClientError(message, err) {
  const log = typeof logClientError === "function" && logClientError || _preferenceGlobalFunction("logClientError");
  if (log) log(message, err);
}
function _preferenceShowToast(message) {
  const toast = typeof showToast !== "undefined" && showToast || _preferenceGlobalFunction("showToast");
  if (toast) toast(message);
}
function _preferenceSyncAppSelect(select) {
  const sync = typeof syncAppSelect !== "undefined" && syncAppSelect || _preferenceGlobalFunction("syncAppSelect");
  if (sync) sync(select);
}
function _preferenceComposerPromptMode() {
  const readMode = typeof getComposerPromptMode === "function" && getComposerPromptMode || _preferenceGlobalFunction("getComposerPromptMode");
  return typeof readMode === "function" ? readMode() : null;
}
function _preferenceHidePromptUsernameSavedIndicator() {
  const hide = typeof hidePromptUsernameSavedIndicator === "function" && hidePromptUsernameSavedIndicator || _preferenceGlobalFunction("hidePromptUsernameSavedIndicator");
  if (typeof hide === "function") hide();
}
function _preferenceShowPromptUsernameSavedIndicator() {
  const show = typeof showPromptUsernameSavedIndicator === "function" && showPromptUsernameSavedIndicator || _preferenceGlobalFunction("showPromptUsernameSavedIndicator");
  if (typeof show === "function") show();
}
function _preferenceSetComposerPromptMode(mode) {
  const setMode = typeof setComposerPromptMode === "function" && setComposerPromptMode || _preferenceGlobalFunction("setComposerPromptMode");
...
function _preferenceSyncShellPrompt() {
  const sync = typeof syncShellPrompt === "function" && syncShellPrompt || _preferenceGlobalFunction("syncShellPrompt");
  if (typeof sync === "function") sync();
}
function _preferenceTimestampMode() {
  const readTimestampMode = (typeof getTimestampMode === "function" ? getTimestampMode : null) || _preferenceGlobalFunction("getTimestampMode");
  let mode = readTimestampMode ? readTimestampMode() : typeof PREFERENCE_GLOBAL?.tsMode === "string" ? PREFERENCE_GLOBAL.tsMode : "off";
...
function _preferenceLineNumberMode() {
  const mode = typeof getLineNumberMode === "function" ? getLineNumberMode() : typeof PREFERENCE_GLOBAL?.lnMode === "string" ? PREFERENCE_GLOBAL.lnMode : "off";
  return mode === "on" ? "on" : "off";
}
var PREF_COOKIE_MAX_AGE = 60 * 60 * 24 * 365;
var _sessionPreferenceKeys = PreferenceCore.SESSION_PREFERENCE_KEYS;
var _sessionPreferenceOverrides = null;
var _sessionPreferencePersistQueue = Promise.resolve();
var _sessionPreferenceLocalRevision = 0;
var _tourOpenedRecordedThisSession = false;
...
function _defaultSessionPreferences() {
  const defaultTheme = _preferenceDefaultThemeEntry()?.name || _preferenceAppConfig().default_theme || "darklab_obsidian.yaml";
...
function _buildCurrentSessionPreferenceSnapshot() {
  const defaultTheme = _preferenceDefaultThemeEntry()?.name || _preferenceAppConfig().default_theme || "darklab_obsidian.yaml";
  const currentThemeName = document.body && document.body.dataset && document.body.dataset.theme || _preferenceSavedThemeName() || defaultTheme;
  const rawPrefs = {
...
function _persistCurrentSessionPreferences() {
  ...
  const persist = _sessionPreferencePersistQueue.catch(() => {
...
async function loadSessionPreferences() {
  ...
  try {
    ...
    const localFallback = sessionId && !sessionId.startsWith("tok_") ? _buildCookieSessionPreferenceSnapshot() : null;
    let prefs = null;
    try {
      const resp = await _preferenceApiFetch("/session/preferences");
...
function activateOptionsTab(tab, { persist = true, focus = false } = {}) {
  ...
  if (persist) {
    try {
      ...
      _preferenceLogClientError("failed to persist options tab preference", err);
    }
  }
...
async function recordTourOpened() {
  ...
  if (resp && resp.ok === false) {
    ...
    try {
      ...
      detail = data2 && data2.error ? `: ${data2.error}` : "";
    } catch (_) {
    }
...
async function applyRunNotifyPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      await _persistCurrentSessionPreferences();
    } catch (err) {
      _preferenceLogClientError("failed to persist run notify preference", err);
    }
  } else {
    _primePreferenceValue("pref_run_notify", nextMode);
  }
...
function applyHudClockPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist HUD clock preference", err);
    }
  } else {
    _primePreferenceValue("pref_hud_clock", nextMode);
  }
  syncOptionsControls();
  if (typeof renderHudClock === "function") renderHudClock();
}
...
function applyThemePreference(theme, persist = true) {
  const applyThemeSelection2 = typeof applyThemeSelection !== "undefined" && applyThemeSelection || _preferenceGlobalFunction("applyThemeSelection");
  if (applyThemeSelection2) applyThemeSelection2(theme, persist);
}
function applyTimestampPreference(mode, persist = true) {
  ...
  const setTimestampMode = typeof _setTsMode !== "undefined" && _setTsMode || _preferenceGlobalFunction("_setTsMode");
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist timestamp preference", err);
    }
  }
  syncOptionsControls();
}
function applyLineNumberPreference(mode, persist = true) {
  ...
  const setLineNumberMode = typeof _setLnMode !== "undefined" && _setLnMode || _preferenceGlobalFunction("_setLnMode");
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist line-number preference", err);
    }
  }
  syncOptionsControls();
}
function applyWelcomeIntroPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist welcome-intro preference", err);
    }
  }
  syncOptionsControls();
}
function applyShareRedactionDefaultPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist share-redaction preference", err);
    }
  }
  syncOptionsControls();
}
function applyProjectAutoLinkExternalRunsPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist project auto-link preference", err);
    }
  } else {
    _primePreferenceValue("pref_project_auto_link_external_runs", nextMode);
  }
...
function applyProjectAutoLinkRunEntitiesPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist project entity auto-link preference", err);
    }
  } else {
    _primePreferenceValue("pref_project_auto_link_run_entities", nextMode);
  }
...
function applyCommandOutcomeSummariesPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist command outcome summaries preference", err);
    }
  } else {
    _primePreferenceValue("pref_command_outcome_summaries", nextMode);
  }
  syncOptionsControls();
  const refreshSummaries = typeof refreshCommandOutcomeSummaries !== "undefined" && refreshCommandOutcomeSummaries || _preferenceGlobalFunction("refreshCommandOutcomeSummaries");
...
function applyConstellationFullDayPreference(mode, persist = true) {
  ...
  if (persist) {
    try {
      ...
      _preferenceLogClientError("failed to persist constellation full-day preference", err);
    }
  }
  return nextMode;
}
function applyCompareViewModePreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist compare view preference", err);
    }
  } else {
    _primePreferenceValue("pref_compare_view_mode", nextMode);
  }
...
function applyCompareContextPreference(mode, persist = true) {
  ...
  if (persist) {
    ...
    try {
      ...
      _preferenceLogClientError("failed to persist compare context preference", err);
    }
  } else {
    _primePreferenceValue("pref_compare_context", nextMode);
  }
...
function applyPromptUsernamePreference(username, persist = true) {
  ...
  if (persist) {
    ...
    try {
      void _persistCurrentSessionPreferences().then(() => _preferenceShowPromptUsernameSavedIndicator()).catch((err) => {
        _preferenceHidePromptUsernameSavedIndicator();
...
var COMMAND_REGISTRY_BRIDGE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var commandRegistryBridgeState = COMMAND_REGISTRY_BRIDGE_GLOBAL.__darklabCommandRegistryBridge || {
...
function openCommandRegistry(...args) {
  return typeof commandRegistryBridgeState.openCommandRegistryHandler === "function" ? commandRegistryBridgeState.openCommandRegistryHandler(...args) : void 0;
}
function closeCommandRegistry(...args) {
  return typeof commandRegistryBridgeState.closeCommandRegistryHandler === "function" ? commandRegistryBridgeState.closeCommandRegistryHandler(...args) : void 0;
}
...
function isCommandRegistryOverlayOpen(...args) {
  return typeof commandRegistryBridgeState.isCommandRegistryOverlayOpenHandler === "function" ? !!commandRegistryBridgeState.isCommandRegistryOverlayOpenHandler(...args) : false;
}
function renderCommandRegistry(...args) {
  return typeof commandRegistryBridgeState.renderCommandRegistryHandler === "function" ? commandRegistryBridgeState.renderCommandRegistryHandler(...args) : void 0;
}
...
var faqHandleRegistry = [];
var FAQ_CATEGORY_ORDER = [
  "Getting started",
  "Core features",
  "Privacy & sessions",
  "Keyboard & controls",
...
var FAQ_CATEGORY_OTHER = "Other";
var FAQ_GLOBAL = typeof window !== "undefined" ? window : globalThis;
...
function _faqBody() {
  if (typeof faqBody !== "undefined" && faqBody) return faqBody;
...
function _faqGetAutocompleteState() {
  const getter = typeof getAutocompleteState === "function" ? getAutocompleteState : _faqGlobalFunction("getAutocompleteState");
  return getter ? getter() : {};
}
function _faqSetAutocompleteState(next) {
  const setter = typeof setAutocompleteState === "function" ? setAutocompleteState : _faqGlobalFunction("setAutocompleteState");
  if (setter) setter(next);
}
function _faqAcHide() {
  const hide = (typeof acHide === "function" ? acHide : null) || _faqGlobalFunction("acHide");
  if (hide) hide();
}
function _faqAcShow(items) {
  const show = (typeof acShow === "function" ? acShow : null) || _faqGlobalFunction("acShow");
  if (show) show(items);
}
function _faqIsActiveTabRunning() {
  const isRunning = typeof isActiveTabRunning === "function" && isActiveTabRunning || _faqGlobalFunction("isActiveTabRunning");
  return !!(isRunning && isRunning());
}
function _faqGetVisibleComposerInput() {
  const getInput = (typeof getVisibleComposerInput === "function" ? getVisibleComposerInput : null) || _faqGlobalFunction("getVisibleComposerInput");
  return getInput ? getInput() : null;
}
function _faqGetAutocompleteMatches(value, cursor) {
  const getMatches = (typeof getAutocompleteMatches === "function" ? getAutocompleteMatches : null) || _faqGlobalFunction("getAutocompleteMatches");
  return getMatches ? getMatches(value, cursor) : [];
}
function _faqLimitAutocompleteMatchesForDisplay(matches, limit) {
  const limitMatches = (typeof limitAutocompleteMatchesForDisplay === "function" ? limitAutocompleteMatchesForDisplay : null) || _faqGlobalFunction("limitAutocompleteMatchesForDisplay");
...
function _faqSetComposerValue(value, start, end, options) {
  const setValue = (typeof setComposerValue === "function" ? setComposerValue : null) || _faqGlobalFunction("setComposerValue");
  if (setValue) setValue(value, start, end, options);
}
function _faqRefocusComposerAfterAction(options) {
  const refocus = (typeof refocusComposerAfterAction === "function" ? refocusComposerAfterAction : null) || _faqGlobalFunction("refocusComposerAfterAction");
  if (refocus) refocus(options);
}
function _faqUseMobileTerminalViewportMode() {
  const useMobile = typeof useMobileTerminalViewportMode2 === "function" ? useMobileTerminalViewportMode2 : _faqGlobalFunction("useMobileTerminalViewportMode");
  return !!(useMobile && useMobile());
}
function _faqIsOverlayOpen() {
  const isOpen = typeof isFaqOverlayOpen === "function" ? isFaqOverlayOpen : _faqGlobalFunction("isFaqOverlayOpen");
  return !!(isOpen && isOpen());
}
function _faqHideOverlay() {
  const hideFaq = typeof hideFaqOverlay === "function" ? hideFaqOverlay : _faqGlobalFunction("hideFaqOverlay");
  if (hideFaq) hideFaq();
}
...
function _faqBindDisclosure(trigger, options) {
  const bind = typeof bindDisclosure === "function" ? bindDisclosure : _faqGlobalFunction("bindDisclosure");
  return bind ? bind(trigger, options) : null;
}
function _faqBindPressable(el, options) {
  const bind = typeof bindPressable === "function" ? bindPressable : _faqGlobalFunction("bindPressable");
...
function _readFaqAutocompleteState() {
  ...
  return {
    filtered: Array.isArray(apiState.filtered) ? apiState.filtered : Array.isArray(globalState.acFiltered) ? globalState.acFiltered : [],
    index: Number.isFinite(Number(apiState.index)) ? Number(apiState.index) : Number.isFinite(Number(globalState.acIndex)) ? Number(globalState.acIndex) : -1
  };
}
function _writeFaqAutocompleteState(next = {}) {
  ...
  if (globalState && typeof setAutocompleteState !== "function" && !_faqGlobalFunction("setAutocompleteState")) {
...
function faqSlug(value) {
  return String(value || "").trim().toLowerCase().replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "item";
}
...
function applyFaqHashTarget() {
  ...
  if (target.kind === "section") {
    const section = [...body.querySelectorAll("[data-faq-section]")].find((el) => el.dataset.faqSection === target.slug);
  ...
  const rows = [
    {
      ...
      value: timeout > 0 ? `<strong>${_fmtDuration(timeout)}</strong> - commands are automatically killed after this time; a notice appears inline in the output` : "<strong>None</strong> - commands run until they finish or you click Kill"
    },
    {
      label: "Output line limit",
      value: maxLines > 0 ? `<strong>${maxLines.toLocaleString()} lines</strong> per tab - older lines are dropped from the top when this is reached` : "<strong>Unlimited</strong>"
    },
    {
      label: "Permalink & history retention",
      value: retention > 0 ? `<strong>${retention} day${retention === 1 ? "" : "s"}</strong> - run history and share links are deleted after this period` : "<strong>Unlimited</strong> - run history and share links are kept indefinitely"
    }
  ];
...
function setAllowedCommandsFaqData2(data) {
  allowedCommandsFaqData = data;
  if (typeof setAllowedCommandsFaqData === "function") {
...
function activateFaqCommandChip(cmd) {
  ...
  const fallbackInput = _faqUseMobileTerminalViewportMode() && typeof document !== "undefined" ? document.getElementById("mobile-cmd") : _faqGetVisibleComposerInput();
  ...
  const closeMajorOverlays2 = typeof closeMajorOverlays === "function" && closeMajorOverlays || _faqGlobalFunction("_closeMajorOverlays");
  if (closeMajorOverlays2) closeMajorOverlays2();
...
function wireCommandRegistryOpenButtons(root = document) {
  ...
  root.querySelectorAll("[data-command-registry-open]").forEach((btn) => {
    ...
    btn.addEventListener("click", () => {
      const openRegistry = typeof openCommandRegistry === "function" ? openCommandRegistry : _faqGlobalFunction("openCommandRegistry");
...
function renderAllowedCommandsFaq(data) {
  setAllowedCommandsFaqData2(data);
  ...
  intro.textContent = count ? `Open the Command Registry to browse ${count} supported command${count === 1 ? "" : "s"}, examples, flags, and subcommands.` : "Open the Command Registry to browse supported commands, examples, flags, and subcommands.";
...
// app/static/js/core/search_core.js
var DarklabSearchCore = (function(global) {
  const SEARCH_SCOPE_LABELS2 = Object.freeze({
    text: "text",
    findings: "findings",
    warnings: "warnings",
    errors: "errors",
  ...
  const SEARCH_SUMMARY_LIMIT2 = 25;
  function searchScopeButtonLabel2(scope, count) {
    if (scope === "text") return "text";
    return `${SEARCH_SCOPE_LABELS2[scope]} (${count})`;
  }
  function searchScopeUnitLabel2(scope, count) {
    if (scope === "summaries") return count === 1 ? "summary" : "summaries";
    const base = SEARCH_SCOPE_LABELS2[scope] || "matches";
    return count === 1 ? base.replace(/s$/, "") : base;
  }
  function formatFindingSummary2(counts) {
    const state = counts || {};
    const parts = [];
  ...
  function searchNoMatchesLabel2(scope) {
    return scope === "text" ? "no matches" : `no ${SEARCH_SCOPE_LABELS2[scope] || "matches"}`;
  }
  function searchInputPlaceholder2(scope) {
    if (scope === "text") return "Search output…";
    if (scope === "summaries") return "Jump between summary lines…";
    return `Jump between ${SEARCH_SCOPE_LABELS2[scope] || "matches"}…`;
  }
  function normalizeSignalCounts2(counts) {
    return {
  ...
  function summaryCommandRoot2(command) {
    return String(command || "").trim().split(/\s+/, 1)[0]?.toLowerCase() || "";
  }
  function summarySectionsTotal2(sections) {
    return (Array.isArray(sections) ? sections : []).reduce((sum, [, lines]) => sum + (Array.isArray(lines) ? lines.length : 0), 0);
  }
  function summaryCompactLines2(lines) {
  ...
  function summaryMergeSections2(items) {
    return ["findings", "warnings", "errors", "summaries"].map((scope) => {
      const lines = [];
      (Array.isArray(items) ? items : []).forEach((item) => {
        const section = Array.isArray(item?.sections) ? item.sections.find(([candidate]) => candidate === scope) : null;
        if (section && Array.isArray(section[1])) lines.push(...section[1]);
      });
      return [scope, lines];
    });
  }
  function summaryGroupedItems2(items) {
    const groups = [];
    (Array.isArray(items) ? items : []).forEach((item) => {
  ...
  function summaryCommandLabels2(items) {
    const counts = /* @__PURE__ */ new Map();
    const ordered = [];
    (Array.isArray(items) ? items : []).forEach((item) => {
  ...
  const api = Object.freeze({
    SEARCH_SCOPE_LABELS: SEARCH_SCOPE_LABELS2,
    SEARCH_SUMMARY_LIMIT: SEARCH_SUMMARY_LIMIT2,
    formatFindingSummary: formatFindingSummary2,
    normalizeSignalCounts: normalizeSignalCounts2,
    searchInputPlaceholder: searchInputPlaceholder2,
    searchNoMatchesLabel: searchNoMatchesLabel2,
    searchScopeButtonLabel: searchScopeButtonLabel2,
    searchScopeUnitLabel: searchScopeUnitLabel2,
    summaryCommandLabels: summaryCommandLabels2,
    summaryCommandRoot: summaryCommandRoot2,
    summaryCompactLines: summaryCompactLines2,
    summaryGroupedItems: summaryGroupedItems2,
    summaryMergeSections: summaryMergeSections2,
    summarySectionsTotal: summarySectionsTotal2
  });
  return api;
})(typeof window !== "undefined" ? window : globalThis);
var {
...
var SEARCH_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _searchGlobalFunction(name) {
  const fn = SEARCH_GLOBAL && SEARCH_GLOBAL[name];
  return typeof fn === "function" ? fn : null;
}
function _searchGlobalValue(name) {
  return SEARCH_GLOBAL ? SEARCH_GLOBAL[name] : void 0;
}
var _searchCore = typeof DarklabSearchCore !== "undefined" && DarklabSearchCore ? DarklabSearchCore : _searchGlobalValue("DarklabSearchCore") || null;
var _SEARCH_SCOPE_LABELS = _searchCore.SEARCH_SCOPE_LABELS;
var _SEARCH_SUMMARY_LIMIT = _searchCore.SEARCH_SUMMARY_LIMIT;
var _TERMINAL_SEARCH_DELAY_MS = 200;
var _TERMINAL_LARGE_SEARCH_DELAY_MS = 600;
var _TERMINAL_LARGE_SEARCH_LINE_THRESHOLD = 2e3;
var _TERMINAL_LARGE_SEARCH_CHAR_THRESHOLD = 5e5;
var _TERMINAL_LARGE_SEARCH_MIN_CHARS = 3;
var _searchTogglePulseTimer = null;
var _searchDiscoverabilityRefreshTimer = null;
var _searchDiscoverabilityRefreshTimerType = "";
var _terminalSearchTimer = null;
var _terminalLazyHighlightedMatch = null;
var _terminalSearchLazyMode = false;
function _stateValue(name, fallback = void 0) {
  const state = typeof getAppState === "function" ? getAppState() : _searchGlobalValue("APP_STATE") || null;
...
function _searchInputEl() {
  return typeof searchInput !== "undefined" && searchInput ? searchInput : _searchDomRef(null, "searchInput");
}
function _searchCountEl() {
  return typeof searchCount !== "undefined" && searchCount ? searchCount : _searchDomRef(null, "searchCount");
}
function _searchCaseButton() {
  return typeof searchCaseBtn !== "undefined" && searchCaseBtn ? searchCaseBtn : _searchDomRef(null, "searchCaseBtn");
}
function _searchRegexButton() {
  return typeof searchRegexBtn !== "undefined" && searchRegexBtn ? searchRegexBtn : _searchDomRef(null, "searchRegexBtn");
}
function _searchToggleButton() {
  return typeof searchToggleBtn !== "undefined" && searchToggleBtn ? searchToggleBtn : _searchDomRef(null, "searchToggleBtn");
}
function _searchSignalSummaryEl() {
  return typeof searchSignalSummary !== "undefined" && searchSignalSummary ? searchSignalSummary : _searchDomRef(null, "searchSignalSummary");
}
function _searchSummaryButton() {
  return typeof searchSummaryBtn !== "undefined" && searchSummaryBtn ? searchSummaryBtn : _searchDomRef(null, "searchSummaryBtn");
}
function _searchScopeButtonList() {
  const buttons = typeof searchScopeButtons !== "undefined" ? searchScopeButtons : null;
...
function _getOutputForTab(tabId) {
  if (typeof getOutput2 === "function") return getOutput2(tabId);
...
function _appendSearchLine(text, cls, tabId) {
  const append = typeof appendLine === "function" ? appendLine : _searchGlobalFunction("appendLine");
  if (append) append(text, cls, tabId);
}
function _escapeSearchRegex(value) {
  if (typeof escapeRegex === "function") return escapeRegex(value);
...
function _bindSearchPressable(el, opts) {
  const bind = typeof bindPressable === "function" ? bindPressable : _searchGlobalFunction("bindPressable");
  return bind ? bind(el, opts) : null;
}
function _isSearchOpen() {
  const check = typeof isSearchBarOpen === "function" ? isSearchBarOpen : _searchGlobalFunction("isSearchBarOpen");
  return !!(check && check());
}
function _updateSearchTabbarChromeFit() {
  const update = typeof updateTabbarChromeFit === "function" ? updateTabbarChromeFit : _searchGlobalFunction("updateTabbarChromeFit");
  if (update) update();
}
...
function _clearSearchDiscoverabilityPulse() {
  ...
  const searchToggleBtn2 = _searchToggleButton();
  if (searchToggleBtn2) {
    searchToggleBtn2.classList.remove("search-discover-pulse");
  }
}
...
function syncSearchScopeUi() {
  ...
  const searchInput2 = _searchInputEl();
  if (searchInput2) {
    searchInput2.disabled = scope !== "text";
    searchInput2.placeholder = _searchInputPlaceholder(scope);
  }
  const searchCaseBtn2 = _searchCaseButton();
  const searchRegexBtn2 = _searchRegexButton();
  if (searchCaseBtn2) searchCaseBtn2.disabled = scope !== "text";
  if (searchRegexBtn2) searchRegexBtn2.disabled = scope !== "text";
...
function _lineMatchesSearchScopeForRoot(line, scope, root = null) {
  ...
  if (line.classList.contains("builtin-signal-summary-header") || line.classList.contains("builtin-signal-summary-section") || line.classList.contains("builtin-signal-summary-row") || line.classList.contains("builtin-signal-summary-note") || line.classList.contains("builtin-signal-summary-sep")) {
    return false;
  }
...
function refreshSearchDiscoverabilityUi() {
  if (_searchDiscoverabilityRefreshTimer) {
    if (_searchDiscoverabilityRefreshTimerType === "idle" && typeof cancelIdleCallback === "function") {
  ...
  if (searchToggleBtn2) {
    ...
    searchToggleBtn2.dataset.searchLabel = buttonLabel;
    searchToggleBtn2.textContent = `⌕ ${buttonLabel}`;
    searchToggleBtn2.title = summary ? `Search output (${summary} available)` : "Search output (Alt+S)";
    searchToggleBtn2.setAttribute(
      "aria-label",
      summary ? `Search output, ${summary} available` : "Search output"
    );
    const shouldPulse = counts.findings > 0 && !_stateValue("searchDiscoverabilityPrompted", false) && !_isSearchOpen();
    if (shouldPulse) {
      _setStateValue("searchDiscoverabilityPrompted", true);
      searchToggleBtn2.classList.add("search-discover-pulse");
  ...
  const searchSignalSummary2 = _searchSignalSummaryEl();
  if (searchSignalSummary2) {
    const hasSignals = counts.findings > 0 || counts.warnings > 0 || counts.errors > 0 || counts.summaries > 0;
    searchSignalSummary2.classList.toggle("u-hidden", !hasSignals);
    _renderCompactSignalSummary(searchSignalSummary2, counts);
    searchSignalSummary2.title = hasSignals ? _formatFindingSummary(counts) : "";
    searchSignalSummary2.setAttribute("aria-label", hasSignals ? _formatFindingSummary(counts) : "");
    if (hasSignals) {
      searchSignalSummary2.querySelectorAll("[data-search-signal-scope]").forEach((btn) => {
        const activate = () => {
          const externalOpen = SEARCH_GLOBAL && (SEARCH_GLOBAL.__darklabOpenSearchFromSignalOverride || SEARCH_GLOBAL.openSearchFromSignal);
          const open = typeof externalOpen === "function" && externalOpen !== openSearchFromSignal ? externalOpen : openSearchFromSignal;
  ...
  const searchSummaryBtn2 = _searchSummaryButton();
  if (searchSummaryBtn2) {
    const hasSignals = counts.findings > 0 || counts.warnings > 0 || counts.errors > 0 || counts.summaries > 0;
    const tabRunning = _isActiveSearchTabRunning();
    searchSummaryBtn2.disabled = !hasSignals || tabRunning;
    searchSummaryBtn2.title = tabRunning ? "Summary is unavailable while this tab has a running command" : hasSignals ? "Summarize findings, warnings, errors, and summary lines" : "No findings, warnings, errors, or summary lines yet";
    searchSummaryBtn2.setAttribute(
      "aria-label",
      tabRunning ? "Summary is unavailable while this tab has a running command" : hasSignals ? "Summarize findings, warnings, errors, and summary lines" : "No findings, warnings, errors, or summary lines yet"
    );
  }
  _searchScopeButtonList().forEach((btn) => {
    ...
    btn.title = scope === "text" ? "Text search" : `Jump between ${count} ${_searchScopeUnitLabel(scope, count)}`;
  });
...
function openSearchFromSignal(scope) {
  ...
  const alreadyActive = _getSearchScope() === normalizedScope && _getSearchMatches().length > 0;
  if (alreadyActive) {
    navigateSearch(1);
    return;
  }
...
function createTextSearchController({
  ...
  const highlight = () => {
    if (lazyHighlight) {
      ...
      const current2 = _highlightLazySearchMatch(currentMatch, matchIdx);
      lazyHighlightedMatch = currentMatch || null;
      if (current2 && typeof current2.scrollIntoView === "function") {
        current2.scrollIntoView({ block: "center" });
      }
      return;
    }
  ...
  const run = () => {
    ...
    const result = lazyHighlight ? _collectLazyTextSearchMatches(targetRoot, query, { caseSensitive, regexMode, lineSelector, lineTextSelector }) : _collectTextSearchMatches(targetRoot, query, { caseSensitive, regexMode, lineSelector });
    if (result.error) {
...
function _summaryBuildItems(blocks, tab) {
  ...
  blocks.forEach((block, index) => {
    ...
    const targetBuckets = /* @__PURE__ */ new Map();
    ...
    if (targetBuckets.size) {
      targetBuckets.forEach((lines, target) => {
        const sections2 = _summaryBlockSections({ command: commandLabel, lines }, root);
        items.push({
          command: commandLabel,
          sections: sections2,
          total: _summarySectionsTotal(sections2),
          root,
          target
        });
      });
      if (ungroupedLines.length) {
        const sections2 = _summaryBlockSections({ command: commandLabel, lines: ungroupedLines }, root);
        items.push({
          command: commandLabel,
          sections: sections2,
          total: _summarySectionsTotal(sections2),
...
function _terminalUsesLargeSearchMode(out) {
  ...
  return lineCount >= _TERMINAL_LARGE_SEARCH_LINE_THRESHOLD || charCount >= _TERMINAL_LARGE_SEARCH_CHAR_THRESHOLD;
}
...
function runSearch() {
  ...
  const scope = _getSearchScope();
  if (scope !== "text") {
    const matches2 = _setSearchMatches(_collectScopedSearchMatches(out, scope));
    _setSearchCount(matches2.length ? `1 / ${matches2.length}` : _searchNoMatchesLabel(scope));
    if (matches2.length) {
  ...
  const result = useLargeSearch ? _collectLazyTextSearchMatches(out, q, {
...
function highlightCurrent() {
  ...
  if (_terminalSearchLazyMode) {
    ...
    _scrollTerminalLazySearchMatchIntoView(currentMatch, current2);
    return;
  }
  matches.forEach((group, i) => {
    group.forEach((node) => node.classList.toggle("current", i === matchIdx));
...
var isSearchBarOpen2 = (...args) => typeof globalThis.isSearchBarOpen === "function" ? globalThis.isSearchBarOpen(...args) : false;

// app/static/js/output_mode_bridge.js
var OUTPUT_MODE_BRIDGE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var setTimestampModeHandler = null;
...
function _bridgeWarningsEnabled4() {
  const config = OUTPUT_MODE_BRIDGE_GLOBAL?.APP_CONFIG && typeof OUTPUT_MODE_BRIDGE_GLOBAL.APP_CONFIG === "object" && !Array.isArray(OUTPUT_MODE_BRIDGE_GLOBAL.APP_CONFIG) ? OUTPUT_MODE_BRIDGE_GLOBAL.APP_CONFIG : {};
  return config.frontend_bridge_warnings === true || config.debug === true || config.dev_mode === true || config.environment === "development" || config.env === "development";
}
function _logOutputModeBridgeDiagnostic(level, event, details = {}) {
  if (!_bridgeWarningsEnabled4()) return;
...
// app/static/js/core/workspace_core.js
var DarklabWorkspaceCore = (function(global) {
  ...
  function normalizeDir2(path = "") {
    const parts = String(path || "").split("/").map((part) => part.trim()).filter(Boolean);
    return parts.join("/");
  }
  function normalizeCommandPath2(path = "", cwd = "") {
    const raw = String(path ?? "").trim();
    const baseParts = raw.startsWith("/") ? [] : normalizeDir2(cwd).split("/").filter(Boolean);
  ...
  function displayPath2(path = "") {
    const normalized = normalizeDir2(path);
    return normalized ? `/${normalized}` : "/";
  }
  function parentDir2(path = "") {
    const parts = normalizeDir2(path).split("/").filter(Boolean);
    parts.pop();
    return parts.join("/");
  }
  ...
  const api = Object.freeze({
    ...
    normalizeDir: normalizeDir2,
    normalizeCommandPath: normalizeCommandPath2,
...
var { basename, displayPath, formatBytes, normalizeCommandPath, normalizeDir, parentDir } = DarklabWorkspaceCore;

// app/static/js/features/tabs/tab_session_state.js
var TAB_SESSION_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _tabSessionGlobalFunction(name) {
  const fn = TAB_SESSION_GLOBAL && TAB_SESSION_GLOBAL[name];
  return typeof fn === "function" ? fn : null;
}
var TAB_SESSION_STATE_KEY = `tab_session_state:${TAB_SESSION_GLOBAL.SESSION_ID || "session"}`;
var _tabSessionPersistTimer = null;
var _tabSessionRestoreInProgress = false;
if (TAB_SESSION_GLOBAL && typeof Object.defineProperty === "function" && !Object.prototype.hasOwnProperty.call(TAB_SESSION_GLOBAL, "_tabSessionRestoreInProgress")) {
...
function _tabSessionTabsBar() {
  return typeof tabsBar !== "undefined" && tabsBar || TAB_SESSION_GLOBAL.tabsBar || null;
}
function _tabSessionTabPanels() {
  return typeof tabPanels !== "undefined" && tabPanels || TAB_SESSION_GLOBAL.tabPanels || null;
}
function _tabSessionGetTabs() {
  const getTabs2 = _tabSessionGlobalFunction("getTabs");
  if (getTabs2) return getTabs2();
...
function _tabSessionSetTabs(nextTabs) {
  ...
  if (typeof setTabs !== "undefined" && typeof setTabs === "function") {
    setTabs(nextTabs);
    return;
  }
}
...
function _tabSessionSetActiveTabId(tabId) {
  ...
  if (typeof setActiveTabId !== "undefined" && typeof setActiveTabId === "function") {
...
function _tabSessionGetTab(tabId) {
  ...
  if (typeof getTab !== "undefined" && typeof getTab === "function") return getTab(tabId);
  return null;
}
...
function _tabSessionSetWelcomeState(nextState) {
  ...
  if (typeof setWelcomeState !== "undefined" && typeof setWelcomeState === "function") {
...
function _tabSessionCreateDefaultTabLabel(index) {
  ...
  if (typeof createDefaultTabLabel !== "undefined" && typeof createDefaultTabLabel === "function") {
    return createDefaultTabLabel(index);
  }
  return `shell ${index}`;
}
function _tabSessionCreateTab(label) {
  ...
  if (typeof createTab !== "undefined" && typeof createTab === "function") return createTab(label);
  return null;
}
function _tabSessionRenderRestoredTabOutput(tabId, rawLines) {
  const render = typeof renderRestoredTabOutput !== "undefined" && renderRestoredTabOutput || _tabSessionGlobalFunction("renderRestoredTabOutput");
  if (typeof render === "function") render(tabId, rawLines);
}
function _tabSessionSetTabStatus(tabId, status3) {
  const setStatus3 = typeof setTabStatus !== "undefined" && setTabStatus || _tabSessionGlobalFunction("setTabStatus");
  if (typeof setStatus3 === "function") setStatus3(tabId, status3);
}
function _tabSessionActivateTab(tabId, options) {
  const activate = typeof activateTab !== "undefined" && activateTab || _tabSessionGlobalFunction("activateTab");
  if (typeof activate === "function") activate(tabId, options);
}
function _tabSessionMountShellPrompt(tabId, force = false) {
  const mount = typeof mountShellPrompt !== "undefined" && mountShellPrompt || _tabSessionGlobalFunction("mountShellPrompt");
  if (typeof mount === "function") mount(tabId, force);
}
function _tabSessionUnmountShellPrompt() {
  const unmount = typeof unmountShellPrompt !== "undefined" && unmountShellPrompt || _tabSessionGlobalFunction("unmountShellPrompt");
  if (typeof unmount === "function") unmount();
}
function _tabSessionGetOutput(tabId) {
  const getOutputForTab = typeof getOutput !== "undefined" && getOutput || _tabSessionGlobalFunction("getOutput");
...
function _tabSessionRestoreOutputTailAfterLayout(outputEl, tab) {
  const restoreTail = typeof _restoreOutputTailAfterLayout2 !== "undefined" && _restoreOutputTailAfterLayout2 || _tabSessionGlobalFunction("_restoreOutputTailAfterLayout");
  if (typeof restoreTail === "function") restoreTail(outputEl, tab);
}
function _tabSessionOutputCore() {
  return typeof DarklabOutputCore !== "undefined" && DarklabOutputCore || TAB_SESSION_GLOBAL.DarklabOutputCore || null;
}
function _snapshotTabRawLines(rawLines) {
  ...
  return rawLines.map((line) => ({
    ...
    line_index: Number.isInteger(line && line.line_index) ? line.line_index : void 0,
    ...
    entities: _tabSessionOutputCore() && typeof _tabSessionOutputCore().normalizeEntities === "function" ? _tabSessionOutputCore().normalizeEntities(line && line.entities) : []
  }));
}
function _snapshotTabCommandOutcomeSummary(summary) {
  if (!summary) return null;
  const outputCore = _tabSessionOutputCore();
  const normalized = outputCore && typeof outputCore.normalizeCommandOutcomeSummary === "function" ? outputCore.normalizeCommandOutcomeSummary(summary) : null;
  return normalized ? {
...
function _tabSessionSnapshot() {
  ...
  const persisted = allTabs.filter((tab) => tab && !tab.closing).map((tab) => ({
    label: String(tab.label || ""),
    command: String(tab.command || ""),
    renamed: !!tab.renamed,
    workspaceCwd: String(tab.workspaceCwd || ""),
    draftInput: String(tab.draftInput || ""),
    commandHistory: Array.isArray(tab.commandHistory) ? tab.commandHistory.map((cmd) => String(cmd || "")).filter(Boolean) : [],
...
function restoreTabSessionState() {
  ...
  try {
    ...
    parsed.tabs.forEach((item, index) => {
      const label = String(item && item.label || _tabSessionCreateDefaultTabLabel(index + 1));
      ...
      tab.commandHistory = Array.isArray(item && item.commandHistory) ? item.commandHistory.map((cmd) => String(cmd || "")).filter(Boolean) : [];
      tab.historyNavIndex = -1;
      tab.historyNavDraft = "";
      ...
      tab.currentRunStartIndex = Number.isFinite(Number(item && item.currentRunStartIndex)) ? Number(item.currentRunStartIndex) : null;
      ...
      const status3 = typeof item?.st === "string" && item.st !== "running" ? item.st : "idle";
      _tabSessionSetTabStatus(tabId, status3);
...
var RUNNER_WORKSPACE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _runnerWorkspaceConfig() {
  if (typeof getAppConfig !== "undefined" && typeof getAppConfig === "function") {
    return getAppConfig() || {};
  }
...
function _runnerWorkspaceActiveTabId() {
  ...
  return typeof RUNNER_WORKSPACE_GLOBAL.APP_STATE_API?.getActiveTabId === "function" ? RUNNER_WORKSPACE_GLOBAL.APP_STATE_API.getActiveTabId() : null;
}
function _runnerWorkspaceGetTab(tabId) {
  if (typeof getTab !== "undefined" && typeof getTab === "function") return getTab(tabId);
  return typeof RUNNER_WORKSPACE_GLOBAL.APP_STATE_API?.getTab === "function" ? RUNNER_WORKSPACE_GLOBAL.APP_STATE_API.getTab(tabId) : null;
}
function _runnerWorkspaceDisplayPath(path = "") {
  const display = typeof displayPath !== "undefined" && displayPath || RUNNER_WORKSPACE_GLOBAL.workspaceDisplayPath;
...
function _runnerWorkspaceNormalizeCommandPath(path = "", cwd = "") {
  const normalize = typeof normalizeCommandPath !== "undefined" && normalizeCommandPath || RUNNER_WORKSPACE_GLOBAL.normalizeWorkspaceCommandPath;
  return typeof normalize === "function" ? normalize(path, cwd) : null;
}
function _runnerWorkspaceSchedulePersistTabSessionState() {
  const schedulePersist = typeof schedulePersistTabSessionState !== "undefined" && schedulePersistTabSessionState || RUNNER_WORKSPACE_GLOBAL.schedulePersistTabSessionState;
  if (typeof schedulePersist === "function") schedulePersist();
}
function _workspaceCacheApi() {
  return {
    getDirectoryHints: typeof getWorkspaceAutocompleteDirectoryHints !== "undefined" && getWorkspaceAutocompleteDirectoryHints || RUNNER_WORKSPACE_GLOBAL.getWorkspaceAutocompleteDirectoryHints,
    getFileHints: typeof getWorkspaceAutocompleteFileHints !== "undefined" && getWorkspaceAutocompleteFileHints || RUNNER_WORKSPACE_GLOBAL.getWorkspaceAutocompleteFileHints,
    refresh: typeof refreshWorkspaceFileCache !== "undefined" && refreshWorkspaceFileCache || RUNNER_WORKSPACE_GLOBAL.refreshWorkspaceFileCache
  };
}
function _workspaceDeleteCommand(cmd) {
  ...
  const usage = root === "file" ? `Usage: file ${fileAction} [-r|-f|-rf] <file-or-folder>` : "Usage: rm [-r|-f|-rf] <file-or-folder>";
...
function _workspaceListCommand(parts) {
  const root = (parts[0] || "").toLowerCase();
  const parseListArgs = (args, usage2) => {
    let long = false;
    let recursive = false;
    const targets = [];
    let invalid = false;
...
function _setWorkspaceCwd(tabId, path = "") {
  ...
  const syncPrompt = typeof hasComposerPromptHandler === "function" && hasComposerPromptHandler("syncShellPrompt") ? syncShellPrompt : RUNNER_WORKSPACE_GLOBAL.syncShellPrompt;
...
function _workspacePathExists(path = "", kind = "any") {
  ...
  if (kind === "directory" || kind === "any") {
    ...
    const dirHints = typeof getDirectoryHints === "function" ? getDirectoryHints() : [];
...
function _outputCoreApi() {
  return typeof DarklabOutputCore !== "undefined" && DarklabOutputCore || _outputGlobalValue("DarklabOutputCore") || null;
}
...
function _outputGetOutput(tabId) {
  if (typeof getOutput === "function") return getOutput(tabId);
  const readOutput = _outputGlobalFunction("getOutput");
  return readOutput ? readOutput(tabId) : null;
}
function _outputShellPromptWrap() {
  return typeof shellPromptWrap !== "undefined" && shellPromptWrap || _outputGlobalValue("shellPromptWrap") || null;
}
function _outputWorkspaceCwd(tabId = _outputActiveTabId()) {
  if (typeof _workspaceCwd === "function") return _workspaceCwd(tabId);
...
function _outputOpenAtlas(options) {
  const openAtlas2 = typeof openAtlas === "function" && openAtlas || _outputGlobalFunction("openAtlas");
  return openAtlas2 ? openAtlas2(options) : null;
}
function _outputCanOpenAtlas() {
  return typeof openAtlas === "function" || typeof _outputGlobalFunction("openAtlas") === "function";
}
function _outputLogClientError(context, err, details = null) {
  const logClientError4 = (typeof hasRuntimeHandler === "function" && hasRuntimeHandler("logClientError") && typeof logClientError === "function" ? logClientError : null) || _outputGlobalFunction("logClientError");
  if (logClientError4) logClientError4(context, err, details);
}
function _outputIsTabSessionRestoreInProgress({ allowBridge = true } = {}) {
  const bridgeRead = allowBridge && typeof isTabSessionRestoreInProgress === "function" ? isTabSessionRestoreInProgress : null;
  return !!(bridgeRead && bridgeRead() || _outputGlobalValue("_tabSessionRestoreInProgress"));
}
function _outputSearchRefresh(tabId) {
  if (tabId !== _outputActiveTabId()) return;
  const refresh = typeof refreshSearchDiscoverabilityUi === "function" && refreshSearchDiscoverabilityUi || _outputGlobalFunction("refreshSearchDiscoverabilityUi");
  if (!refresh) return;
  const isOpen = typeof isSearchBarOpen2 === "function" && isSearchBarOpen2 || _outputGlobalFunction("isSearchBarOpen");
  const run = typeof runSearch === "function" && runSearch || _outputGlobalFunction("runSearch");
  const schedule = typeof scheduleSearchDiscoverabilityRefresh === "function" && scheduleSearchDiscoverabilityRefresh || _outputGlobalFunction("scheduleSearchDiscoverabilityRefresh");
...
function _outputUpdateFollowButton(tabId) {
  const update = typeof updateOutputFollowButton === "function" && updateOutputFollowButton || _outputGlobalFunction("updateOutputFollowButton");
  if (update) update(tabId);
}
function _outputMountShellPrompt(tabId, force = false) {
  const mount = typeof mountShellPrompt === "function" && mountShellPrompt || _outputGlobalFunction("mountShellPrompt");
...
function createAnsiUpRenderer() {
  ...
  return {
    use_classes: false,
    ansi_to_html(text) {
      const escape = typeof escapeHtml === "function" && escapeHtml || _outputGlobalFunction("escapeHtml");
      if (typeof escape === "function") return escape(String(text ?? ""));
      return String(text ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;");
    }
  };
}
var _ansiRenderersByTab = /* @__PURE__ */ new Map();
...
function resetAnsiRendererForTab2(tabId) {
...
var _OUTPUT_SYNC_BURST_LIMIT = 60;
var _OUTPUT_BATCH_SIZE = 300;
var _OUTPUT_APPEND_LINES_CHUNK_SIZE = 300;
var _OUTPUT_RESTORE_TAIL_DELAYS = [0, 16, 64, 160, 320];
var _OUTPUT_COALESCED_LINE_ROLES = /* @__PURE__ */ new Set(["progress", "status-line"]);
var _pendingOutputBatches = /* @__PURE__ */ new Map();
var _OUTPUT_SIGNAL_SCOPES = _outputCoreApi().OUTPUT_SIGNAL_SCOPES;
var _HIGH_VOLUME_OUTPUT_DEFAULT_LINE_THRESHOLD = 5e4;
var _HIGH_VOLUME_OUTPUT_DEFAULT_STATUS_INTERVAL_LINES = 5e4;
function _promptUsernameOverride() {
  let value = "";
  const getPreference2 = typeof getPreference === "function" && getPreference || _outputGlobalFunction("getPreference");
  if (typeof getPreference2 === "function") {
    value = getPreference2("pref_prompt_username");
  }
  if (typeof document !== "undefined") {
...
function _configuredPromptUsername() {
  ...
  const configured = config && typeof config.prompt_username === "string" ? config.prompt_username : "anon";
  return _promptUsernameOverride() || String(configured || "anon").trim() || "anon";
}
function _configuredPromptDomain() {
  const config = _outputAppConfig();
  const configured = config && typeof config.prompt_domain === "string" ? config.prompt_domain : "darklab.sh";
  return String(configured || "darklab.sh").trim() || "darklab.sh";
}
function promptIdentityPrefix2(rawPrefix = null) {
...
function currentPromptWorkspacePath() {
  const config = _outputAppConfig();
  if (config && config.workspace_enabled === true) {
...
function buildPromptLabel2(rawPrefix = null, path = null) {
...
function stripPromptLabelFromEchoText2(text = "") {
  return _outputCoreApi().stripPromptLabelFromEchoText(text, buildPromptLabel2(), promptIdentityPrefix2());
}
function _outputPromptPrefix() {
  const config = _outputAppConfig();
  const shellPromptWrap2 = _outputShellPromptWrap();
  if (config && config.workspace_enabled === true && !(shellPromptWrap2 && shellPromptWrap2.classList.contains("shell-prompt-confirm"))) {
    return buildPromptLabel2();
  }
  const promptPrefix = shellPromptWrap2 ? shellPromptWrap2.querySelector(".prompt-prefix") : document.querySelector("#shell-prompt-wrap .prompt-prefix");
  const text = promptPrefix ? String(promptPrefix.textContent || "").trim() : "";
  return text || buildPromptLabel2();
}
...
function _promptTimestampPrefix() {
  ...
  if (tsMode === "clock") return (/* @__PURE__ */ new Date()).toTimeString().slice(0, 8);
  return "";
}
function _prefixWidthForOutput(out) {
  ...
  const timestampWidth = tsMode === "clock" ? 8 : tsMode === "elapsed" ? 8 : 0;
...
function _countableOutputSignalScopes(rawLine) {
  const builtinRoots = Array.isArray(_outputAppState().acBuiltinCommandRoots) ? _outputAppState().acBuiltinCommandRoots : [];
...
function _resetTabOutputSignalCounts2(tab, rawLines = []) {
  if (!tab) return;
  tab._outputSignalCounts = _emptyOutputSignalCounts();
  tab._outputSignalCountsValid = true;
...
function _isSyntheticSummaryLine(line) {
  if (!line || !line.classList) return false;
  const isSyntheticClass = typeof _outputCoreApi().isSyntheticSummaryClassName === "function" ? _outputCoreApi().isSyntheticSummaryClassName : _outputCoreApi().isSignalSummaryClassName;
...
function _cancelPendingOutputBatch2(tabId) {
...
function discardPendingOutputBatch(tabId) {
  _cancelPendingOutputBatch2(tabId);
}
...
function _outputEntityRanges(text, entities) {
  ...
  (Array.isArray(entities) ? entities : []).forEach((entity) => {
    ...
    const candidate = _outputEntityRangeCandidates(source, entity).find((range) => range.start >= 0 && range.end > range.start && range.end <= source.length);
...
var _outputEntityMenu = null;
var _outputEntityMenuOpener = null;
...
function _showOutputEntityMenu(token, x, y) {
  ...
  menu.addEventListener("click", (event) => {
    const action = event.target && event.target.closest ? event.target.closest("[data-output-entity-action]")?.dataset.outputEntityAction : "";
    if (!action) return;
    event.preventDefault();
    event.stopPropagation();
    ...
    if (action === "copy-value") {
      const copyTextToClipboard2 = typeof copyTextToClipboard === "function" && copyTextToClipboard || _outputGlobalFunction("copyTextToClipboard");
      const showToast3 = typeof showToast === "function" && showToast || _outputGlobalFunction("showToast");
      if (typeof copyTextToClipboard2 !== "function") return;
      copyTextToClipboard2(String(token.dataset.atlasEntityValue || "")).then(() => showToast3?.("Entity copied")).catch(() => showToast3?.("Failed to copy entity", "error"));
    }
...
var _outputEntityLongPressTimer = null;
...
function _runOutputModel() {
  if (typeof DarklabRunOutputModel !== "undefined" && DarklabRunOutputModel) return DarklabRunOutputModel;
  if (_outputGlobalValue("DarklabRunOutputModel")) return _outputGlobalValue("DarklabRunOutputModel");
  return null;
}
var _runOutputModelMissingReported = false;
...
function _lineEventModelStub(payload) {
  ...
  return {
    ...
    line_number: Number.isInteger(item.line_number) ? item.line_number : void 0,
    command_root: String(item.command_root || ""),
    target: String(item.target || ""),
...
function _mergeLineEventMetadata(payload, metadata = null) {
  ...
  sources.forEach((source) => {
    ...
    [
      ...
      if (source[key] !== void 0 && source[key] !== null && source[key] !== "") merged[key] = source[key];
    });
  });
...
function _lineEventPayload(text, cls, metadata = null) {
  if (text && typeof text === "object") {
    const payload2 = { ...text };
    if (payload2.metadata && typeof payload2.metadata === "object") {
      ["kind", "role", "signals", "line_index", "line_number", "command_root", "target", "entities", "noise_kind", "noise_reason"].forEach((key) => {
        if (payload2[key] === void 0 && payload2.metadata[key] !== void 0) payload2[key] = payload2.metadata[key];
      });
    }
    if (metadata && typeof metadata === "object") {
      ["kind", "role", "signals", "line_index", "line_number", "command_root", "target", "entities", "noise_kind", "noise_reason"].forEach((key) => {
        if (metadata[key] !== void 0) payload2[key] = metadata[key];
      });
    }
    return payload2;
  }
  ...
  if (metadata && typeof metadata === "object") {
    ["kind", "role", "signals", "line_index", "line_number", "command_root", "target", "entities", "noise_kind", "noise_reason"].forEach((key) => {
      if (metadata[key] !== void 0) payload[key] = metadata[key];
    });
  }
  return payload;
}
...
function _rawOutputLine(event, tabId, now, runStart, metadata = null) {
  ...
  const tsE = runStart ? "+" + ((now - runStart) / 1e3).toFixed(1) + "s" : "+0.0s";
...
function _bindHighVolumeOutputResumeButton(button) {
  ...
  const bind = typeof bindPressable === "function" && bindPressable || _outputGlobalFunction("bindPressable");
...
function _activateOutputCommandChip(command) {
  const activate = typeof activateFaqCommandChip === "function" && activateFaqCommandChip || _outputGlobalFunction("activateFaqCommandChip");
...
function _buildOutputLine(event, tabId, now, runStart, metadata = null) {
  ...
  if (runStart) {
    span.dataset.tsE = "+" + ((now - runStart) / 1e3).toFixed(1) + "s";
  } else {
...
function _appendOutputSpan(out, span) {
  const shellPromptWrap2 = _outputShellPromptWrap();
  const prompt = shellPromptWrap2 && shellPromptWrap2.parentElement === out ? shellPromptWrap2 : null;
  if (prompt) out.insertBefore(span, prompt);
  else out.appendChild(span);
}
function _commandOutcomeSummariesEnabled() {
  const readPreference = typeof getCommandOutcomeSummariesPreference === "function" && getCommandOutcomeSummariesPreference || _outputGlobalFunction("getCommandOutcomeSummariesPreference") || null;
...
function _commandOutcomeRawLinesForTab(tab) {
  ...
  const start = Number.isFinite(Number(tab && tab.currentRunStartIndex)) ? Math.max(0, Math.min(rawLines.length, Number(tab.currentRunStartIndex))) : 0;
...
function renderCommandOutcomeSummary(tabId, outcome = null) {
  ...
  const explicitSummary = outcome !== null && outcome !== void 0 ? _normalizeCommandOutcomeSummary(outcome) : null;
  ...
  const cachedSummary = !explicitSummary && !builtSummary && !hasCurrentOutput ? _normalizeCommandOutcomeSummary(tab.commandOutcomeSummary) : null;
...
function _stickOutputToBottom2(out, tab) {
  if (!out) return;
  if (tab) {
...
function _restoreOutputTailAfterLayout2(out, tab) {
  if (!out) return;
...
function _followOutputAfterAppend(out, tab, { afterLargeBatch = false } = {}) {
  ...
  if (afterLargeBatch && _isMobileTerminalMode()) {
    _restoreOutputTailAfterLayout2(out, tab);
    return;
  }
  setTimeout(() => _stickOutputToBottom2(out, tab), 0);
}
...
function _appendRestoredOutputSpan(out, rawLine, tabId) {
  ...
  if (_isPromptEchoLineEvent(event)) {
    ...
    prefixEl.textContent = prefix;
    content.appendChild(prefixEl);
    const bodyText = stripPromptLabelFromEchoText2(text);
...
function renderRestoredTabOutput(tabId, rawLines) {
  ...
  out.innerHTML = "";
  resetAnsiRendererForTab2(tabId);
  tab.rawLines = lines;
  _resetTabOutputSignalCounts2(tab, lines);
  lines.forEach((line) => _appendRestoredOutputSpan(out, line, tabId));
  renderCommandOutcomeSummary(tabId, restoredSummary);
  syncOutputPrefixes2(out);
  if (lines.length) {
    tab.followOutput = true;
    if (_isMobileTerminalMode()) _stickOutputToBottom2(out, tab);
    else _restoreOutputTailAfterLayout2(out, tab);
  }
...
function _refreshFollowingOutputsAfterLayout() {
  _outputTabs().forEach((tab) => {
    ...
    if (!out) return;
    setTimeout(() => _restoreOutputTailAfterLayout2(out, tab), 16);
  });
}
...
function syncOutputPrefixes2(scope = document) {
  const isElement = scope && typeof scope.querySelectorAll === "function";
  const looksLikeOutput = scope === document || isElement && (scope.classList?.contains?.("output") || scope.querySelector?.(".line") || scope.querySelector?.("#shell-prompt-wrap"));
  const outputs = scope === document ? [...document.querySelectorAll(".output")] : looksLikeOutput ? [scope] : [...scope?.querySelectorAll?.(".output") || []];
...
function appendLine(text, cls, tabId, metadata = null) {
  ...
  if (_shouldSkipLiveOutputRender(tab, normalized.metadata)) {
    _flushPendingOutputBeforeHighVolumeSkip(id);
    const state2 = _ensureHighVolumeOutputState(tab);
    const rawLine2 = _rawOutputLine(event, id, now, runStart, normalized.metadata);
    _assignRawOutputLineNumber(out, tab, rawLine2);
    _syncTabRawLines(tab, rawLine2);
    if (state2) {
      state2.skippedLines += 1;
      _appendHighVolumeOutputNotice(id, state2, { force: state2.skippedLines === 1 });
    }
    return;
  }
...
if (typeof window !== "undefined") {
  ...
  if (typeof setOutputModeHandlers === "function") {
    setOutputModeHandlers({ setTimestampMode: _setTsMode });
  }
  if (typeof setOutputHandlers === "function") {
    setOutputHandlers({
      resetAnsiRendererForTab: resetAnsiRendererForTab2,
      dropAnsiRendererForTab,
      hasPendingOutputBatch,
      _maybeMountDeferredPrompt,
      syncOutputPrefixes: syncOutputPrefixes2,
      _resetTabOutputSignalCounts: _resetTabOutputSignalCounts2,
      _cancelPendingOutputBatch: _cancelPendingOutputBatch2,
      _stickOutputToBottom: _stickOutputToBottom2,
      _restoreOutputTailAfterLayout: _restoreOutputTailAfterLayout2,
      appendLine,
      appendLines,
...
var DarklabRunnerCore = (function(global) {
  function formatElapsed2(totalSecs) {
  ...
  function _parseStage(stageTokens) {
    ...
    if (helper === "head" || helper === "tail") {
      ...
      if (normalizedStageTokens.length !== 3 || normalizedStageTokens[1] !== "-n" || !/^\d+$/.test(normalizedStageTokens[2])) {
        return null;
      }
  ...
  function parseSyntheticPostFilterCommand2(cmd) {
    ...
    const pipeIndexes = tokens.map((token, index) => token === "|" ? index : -1).filter((index) => index !== -1);
  ...
  function _syntheticPostFilterLineLimit(options) {
    const configured = options && options.maxOutputLines !== void 0 ? options.maxOutputLines : global.APP_CONFIG && global.APP_CONFIG.max_output_lines;
    const limit = Number.parseInt(configured, 10);
    return Number.isFinite(limit) && limit > 0 ? limit : 0;
  }
  function applySyntheticPostFilterLines2(lineItems, spec, options = {}) {
    ...
    function textOf(item) {
      return String(item && item.text !== void 0 ? item.text : item || "");
    }
  ...
  function _selectJsonValues(value, selector) {
    ...
    if (selector.op === "field") return [_jsonPathValue(value, selector.path)].filter((item) => item !== void 0);
    ...
    if (selector.op === "filter_has") {
      return _jsonPathValue(value, selector.path) !== void 0 ? [value] : [];
    }
  ...
  function _jsonPathValue(value, path) {
    ...
    for (const part of path || []) {
      ...
      if (!Object.prototype.hasOwnProperty.call(current, part)) return void 0;
  ...
  function isSyntheticPostFilterCommand2(cmd) {
    return !!parseSyntheticPostFilterCommand2(cmd);
  }
  function isSyntheticSortCommand2(cmd) {
    const parsed = parseSyntheticPostFilterCommand2(cmd);
    return !!(parsed && parsed.kind === "sort");
  }
  function isSyntheticUniqCommand2(cmd) {
    const parsed = parseSyntheticPostFilterCommand2(cmd);
    return !!(parsed && parsed.kind === "uniq");
  }
  function isSyntheticGrepCommand2(cmd) {
    const parsed = parseSyntheticPostFilterCommand2(cmd);
    return !!(parsed && parsed.kind === "grep");
  }
  function isSyntheticHeadCommand2(cmd) {
    const parsed = parseSyntheticPostFilterCommand2(cmd);
    return !!(parsed && parsed.kind === "head");
  }
  function isSyntheticTailCommand2(cmd) {
    const parsed = parseSyntheticPostFilterCommand2(cmd);
    return !!(parsed && parsed.kind === "tail");
  }
  function isSyntheticWcLineCountCommand2(cmd) {
    const parsed = parseSyntheticPostFilterCommand2(cmd);
    return !!(parsed && parsed.kind === "wc_l");
  }
  function isSyntheticJqCommand2(cmd) {
    const parsed = parseSyntheticPostFilterCommand2(cmd);
    return !!(parsed && parsed.kind === "jq");
  }
  const api = Object.freeze({
    formatElapsed: formatElapsed2,
    parseSyntheticPostFilterCommand: parseSyntheticPostFilterCommand2,
    applySyntheticPostFilterLines: applySyntheticPostFilterLines2,
    isSyntheticPostFilterCommand: isSyntheticPostFilterCommand2,
    isSyntheticSortCommand: isSyntheticSortCommand2,
    isSyntheticUniqCommand: isSyntheticUniqCommand2,
    isSyntheticGrepCommand: isSyntheticGrepCommand2,
    isSyntheticHeadCommand: isSyntheticHeadCommand2,
    isSyntheticTailCommand: isSyntheticTailCommand2,
    isSyntheticWcLineCountCommand: isSyntheticWcLineCountCommand2,
    isSyntheticJqCommand: isSyntheticJqCommand2
  });
  return api;
})(typeof window !== "undefined" ? window : globalThis);
var { applySyntheticPostFilterLines, formatElapsed, isSyntheticGrepCommand, isSyntheticHeadCommand, isSyntheticJqCommand, isSyntheticPostFilterCommand, isSyntheticSortCommand, isSyntheticTailCommand, isSyntheticUniqCommand, isSyntheticWcLineCountCommand, parseSyntheticPostFilterCommand } = DarklabRunnerCore;

// app/static/js/welcome_bridge.js
var cancelWelcomeHandler = null;
var welcomeOwnsTabHandler = null;
...
function cancelWelcome(...args) {
  return typeof cancelWelcomeHandler === "function" ? cancelWelcomeHandler(...args) : void 0;
}
function welcomeOwnsTab(...args) {
  return typeof welcomeOwnsTabHandler === "function" ? !!welcomeOwnsTabHandler(...args) : false;
}

// app/static/js/features/workspace/workspace_viewer_formats.js
var DarklabWorkspaceViewerFormats = null;
(function initWorkspaceViewerFormats(global) {
  ...
  function parseDelimited(text = "", delimiter = ",", { tableLimit = 250 } = {}) {
    const rows = String(text || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n").split("\n").filter((line) => line.length > 0).slice(0, tableLimit + 1).map((line) => parseDelimitedLine(line, delimiter));
  ...
  function parseHttpResponse(text = "") {
    ...
    return {
      ...
      headers: lines.slice(1).map((line) => {
        ...
        return index >= 0 ? { name: line.slice(0, index).trim(), value: line.slice(index + 1).trim() } : { name: line.trim(), value: "" };
      }),
      body: bodyParts.join("\n\n")
    };
  }
  ...
  function viewerPayload2(path = "", text = "", options = {}) {
  ...
  function viewerRawText2(payload) {
    return String(payload?.rawText ?? payload?.text ?? "");
  }
  DarklabWorkspaceViewerFormats = {
    viewerPayload: viewerPayload2,
    viewerRawText: viewerRawText2
  };
})(typeof window !== "undefined" ? window : globalThis);
var viewerPayload = DarklabWorkspaceViewerFormats ? DarklabWorkspaceViewerFormats.viewerPayload : null;
var viewerRawText = DarklabWorkspaceViewerFormats ? DarklabWorkspaceViewerFormats.viewerRawText : null;

// app/static/js/ui/ui_entity_metadata.js
var DarklabEntityMetadata = null;
(function initEntityMetadataClient(global) {
  function parseLabelInput2(value) {
    const seen = /* @__PURE__ */ new Set();
    return String(value || "").split(",").map((item) => item.trim()).filter(Boolean).filter((item) => {
  ...
  function entityMetadataUrl2(entityType, entityId, suffix = "") {
  ...
  function _metadataFetch(apiFetchImpl) {
    ...
    if (typeof apiFetch2 === "function") return apiFetch2;
    throw new Error("Entity metadata requests are unavailable");
  }
  async function entityMetadataRequest2(url, options = {}, { apiFetchImpl = null, onWrite = null } = {}) {
  ...
  function _requestForOptions(options = {}) {
    ...
    return (url, requestOptions = {}) => entityMetadataRequest2(url, requestOptions, options);
  }
  function _labelValues(labels) {
    return (Array.isArray(labels) ? labels : []).map((label) => String(label && typeof label === "object" ? label.label : label || "").trim()).filter(Boolean);
  }
  async function syncEntityLabels2(entityType, entityId, nextLabels, options = {}) {
    const request = _requestForOptions(options);
    const labelsUrl = entityMetadataUrl2(entityType, entityId, "labels");
  ...
  async function syncEntityNote2(entityType, entityId, body, options = {}) {
    const request = _requestForOptions(options);
    const noteUrl = entityMetadataUrl2(entityType, entityId, "note");
  ...
  DarklabEntityMetadata = {
    parseLabelInput: parseLabelInput2,
    entityMetadataUrl: entityMetadataUrl2,
    entityMetadataRequest: entityMetadataRequest2,
    syncEntityLabels: syncEntityLabels2,
    syncEntityNote: syncEntityNote2
  };
})(typeof window !== "undefined" ? window : globalThis);
var {
...
var _workspaceFiles = [];
var _workspaceDirs = [];
var _workspaceLimits = {};
var _workspaceLoaded = false;
var _workspaceCurrentDir = "";
var _workspaceCurrentScopeKey = "personal";
var _workspaceDirByScope = /* @__PURE__ */ new Map();
var _workspaceOwner = {
  scope: "personal",
  team_id: "",
  label: "Personal",
...
function _workspaceApiFetch() {
  if (typeof apiFetch2 !== "function") throw new Error("apiFetch is not available");
  return apiFetch2;
}
function _publishWorkspaceState() {
  ...
  Object.defineProperties(global.DarklabWorkspaceState, {
    files: {
      ...
      get: () => _workspaceFiles,
      set: (value) => {
        _workspaceFiles = Array.isArray(value) ? value : [];
      }
    },
    dirs: {
      configurable: true,
      get: () => _workspaceDirs,
      set: (value) => {
        _workspaceDirs = Array.isArray(value) ? value : [];
      }
    },
    limits: {
      configurable: true,
      ...
      set: (value) => {
        _workspaceLimits = value && typeof value === "object" ? value : {};
      }
    },
    loaded: {
      configurable: true,
    ...
    owner: {
      ...
      get: () => _workspaceOwner,
      set: (value) => {
        _workspaceOwner = value && typeof value === "object" ? value : _workspaceOwner;
      }
    },
    currentDir: {
      configurable: true,
      ...
      set: (value) => {
        _workspaceCurrentDir = _normalizeWorkspaceDir(value);
      }
    },
    currentScopeKey: {
      configurable: true,
      ...
      set: (value) => {
        _workspaceCurrentScopeKey = String(value || "personal");
      }
    },
    apiFetch: {
      configurable: true,
...
var _workspaceViewedPath = "";
var _workspaceViewerPayloadCache = null;
var _workspaceViewerSearchController = null;
var _workspaceViewerRefreshTimer = null;
var _workspaceViewerAutoRefreshSeconds = 0;
var _workspaceViewerRefreshSpinTimer = null;
var _workspaceViewerRefreshInFlight = false;
var _workspaceViewerAutoRefreshEnabled = false;
var _workspaceViewedSize = null;
function WorkspaceCore() {
  return typeof DarklabWorkspaceCore !== "undefined" && DarklabWorkspaceCore || null;
}
function _workspaceEntityMetadataClient() {
  return typeof DarklabEntityMetadata !== "undefined" && DarklabEntityMetadata || {};
}
function _workspaceViewerFormats() {
  return typeof DarklabWorkspaceViewerFormats !== "undefined" && DarklabWorkspaceViewerFormats || {};
}
function _workspaceFileCacheApi() {
  return {
    getDirectoryEntries: typeof getWorkspaceDirectoryEntries !== "undefined" && getWorkspaceDirectoryEntries || null,
    getDirectoryHints: typeof getWorkspaceAutocompleteDirectoryHints !== "undefined" && getWorkspaceAutocompleteDirectoryHints || null,
    getFileHints: typeof getWorkspaceAutocompleteFileHints !== "undefined" && getWorkspaceAutocompleteFileHints || null,
    refresh: typeof refreshWorkspaceFileCache !== "undefined" && refreshWorkspaceFileCache || null
  };
}
var WORKSPACE_PREVIEW_LINE_LIMIT = 1e4;
var WORKSPACE_PREVIEW_TABLE_LIMIT = 250;
var WORKSPACE_VIEWER_AUTO_REFRESH_MS = 5e3;
var WORKSPACE_VIEWER_AUTO_REFRESH_MAX_BYTES = 1024 * 1024;
var WORKSPACE_VIEWER_BOTTOM_THRESHOLD = 24;
var WORKSPACE_VIEWER_REFRESH_SPINNER_MS = 650;
var WORKSPACE_VIEWER_SEARCH_DELAY_MS = 250;
var WORKSPACE_VIEWER_LARGE_SEARCH_DELAY_MS = 600;
var WORKSPACE_VIEWER_LARGE_SEARCH_LINE_THRESHOLD = 2e3;
var WORKSPACE_VIEWER_LARGE_SEARCH_CHAR_THRESHOLD = 5e5;
var WORKSPACE_VIEWER_LARGE_SEARCH_SIZE_THRESHOLD = 1024 * 1024;
var WORKSPACE_VIEWER_LARGE_SEARCH_MIN_CHARS = 3;
var WORKSPACE_MANAGE_CAPABILITY = "manage_workspace_files";
...
function _workspaceViewerIsOpen() {
  return !!(workspaceViewer && !workspaceViewer.classList.contains("u-hidden") && (typeof workspaceViewerOverlay === "undefined" || !workspaceViewerOverlay || !workspaceViewerOverlay.classList.contains("u-hidden")));
}
...
function _workspaceSyncViewerAutoRefreshToggle() {
  ...
  workspaceViewerAutoRefreshToggle.title = disabledReason || (_workspaceViewerAutoRefreshEnabled ? "Disable viewer auto refresh" : "Enable viewer auto refresh");
  const label = typeof workspaceViewerAutoRefreshLabel !== "undefined" && workspaceViewerAutoRefreshLabel ? workspaceViewerAutoRefreshLabel : workspaceViewerAutoRefreshToggle.querySelector("span:last-child");
  if (label) {
    label.textContent = _workspaceViewerAutoRefreshEnabled ? `Auto - ${Math.max(1, _workspaceViewerAutoRefreshSeconds || Math.ceil(WORKSPACE_VIEWER_AUTO_REFRESH_MS / 1e3))}s` : "Auto - off";
  }
}
function _workspaceStartViewerAutoRefresh() {
  _workspaceStopViewerAutoRefresh();
  _workspaceViewerAutoRefreshSeconds = Math.ceil(WORKSPACE_VIEWER_AUTO_REFRESH_MS / 1e3);
  ...
  _workspaceViewerRefreshTimer = setInterval(() => {
    ...
    refreshWorkspaceViewedFile({ auto: true }).catch(() => {
    }).finally(() => {
...
function _workspaceViewerPayload(path = "", text = "") {
  const formatter = typeof viewerPayload !== "undefined" && viewerPayload || _workspaceViewerFormats().viewerPayload;
  return formatter(path, text, {
    tableLimit: WORKSPACE_PREVIEW_TABLE_LIMIT
  });
}
function _workspaceViewerRawText(payload) {
  const formatter = typeof viewerRawText !== "undefined" && viewerRawText || _workspaceViewerFormats().viewerRawText;
  return formatter(payload);
}
function _workspaceUsesLargeSearchMode({ lineCount = 0, charCount = 0, size = null } = {}) {
  const numericSize = size == null ? NaN : Number(size);
  return Number(lineCount) >= WORKSPACE_VIEWER_LARGE_SEARCH_LINE_THRESHOLD || Number(charCount) >= WORKSPACE_VIEWER_LARGE_SEARCH_CHAR_THRESHOLD || Number.isFinite(numericSize) && numericSize >= WORKSPACE_VIEWER_LARGE_SEARCH_SIZE_THRESHOLD;
}
...
function _workspaceRenderHttpPreview(payload) {
  ...
  status3.className = "workspace-http-status";
  status3.textContent = data.status || "HTTP response";
  wrap.appendChild(status3);
...
function showWorkspaceEditor(path = "", text = "", { readOnlyPath = false, labels = [], noteBody = "" } = {}) {
  ...
  setTimeout(() => {
    ...
    const editorHost = typeof workspaceEditorOverlay !== "undefined" && workspaceEditorOverlay ? workspaceEditorOverlay : workspaceEditor;
...
async function refreshWorkspaceViewedFile({ auto = false, suppressErrorToast = false } = {}) {
  ...
  try {
    ...
    _workspaceFlashViewerRefreshSpinner(auto && typeof workspaceViewerAutoRefreshToggle !== "undefined" ? workspaceViewerAutoRefreshToggle : null);
    return data;
  } catch (err) {
...
async function handleWorkspaceFileAction(action, path) {
  try {
    ...
    if (action === "open-folder") {
      ...
      const note = count ? `This will also delete ${count} ${count === 1 ? "file" : "files"} in this folder.` : "This only removes the empty workspace folder.";
      const confirmed = typeof showConfirm === "function" ? await showConfirm({
...
workspaceViewerRefreshBtn?.addEventListener("click", () => {
...
if (typeof window !== "undefined") {
  if (isWorkspaceEnabled()) setTimeout(() => {
    _workspaceFileCacheApi().refresh?.();
  }, 0);
}
...
function createRunnerPersistence({
  apiFetch: apiFetch4,
  maskSessionToken: maskSessionToken4,
  isHistoryPanelOpen: isHistoryPanelOpen3,
  refreshHistoryPanel: refreshHistoryPanel3,
  logClientError: logClientError4
} = {}) {
  function historySafeCommand(cmd) {
    const value = String(cmd || "").trim();
    if (!value) return "";
    const mask = typeof maskSessionToken4 === "function" ? maskSessionToken4 : (token) => token;
  ...
  function persistClientSideRun(command, lineItems, statusValue, tabId = "") {
    ...
    if (!safeCommand || typeof apiFetch4 !== "function") return;
    const lines = (Array.isArray(lineItems) ? lineItems : []).map((line) => {
      const entry = {
        text: String(line && line.text !== void 0 ? line.text : line || ""),
        cls: String(line && line.cls || "")
      };
    ...
    apiFetch4("/run/client", {
      ...
      if (typeof isHistoryPanelOpen3 === "function" && isHistoryPanelOpen3() && typeof refreshHistoryPanel3 === "function") {
        refreshHistoryPanel3();
      }
    }).catch((err) => {
      if (typeof logClientError4 === "function") {
        logClientError4("client-side run persistence failed", err);
      }
    });
  }
  return {
...
var LOCAL_COMMAND_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _cliGlobalFunction(name) {
  const fn = LOCAL_COMMAND_GLOBAL?.[name];
  return typeof fn === "function" ? fn : null;
}
function _cliAppendLineFn() {
  return typeof appendLine !== "undefined" && appendLine || _cliGlobalFunction("appendLine");
}
function _cliGetTab(id) {
  const getTab3 = typeof getTab !== "undefined" && getTab || _cliGlobalFunction("getTab");
  return typeof getTab3 === "function" ? getTab3(id) : null;
}
function _cliGetOutput(id) {
  const getOutput3 = typeof getOutput2 !== "undefined" && getOutput2 || _cliGlobalFunction("getOutput");
  return typeof getOutput3 === "function" ? getOutput3(id) : null;
}
...
function _cliAppendLine(text, cls = "", tabId = null, metadata = null) {
  const appendLine3 = _cliAppendLineFn();
  if (appendLine3) appendLine3(text, cls, tabId, metadata);
}
...
function _cliPreserveOutputTail(tabId = null, shouldPreserve = true) {
  ...
  const stickOutputToBottom = typeof _stickOutputToBottom2 !== "undefined" && _stickOutputToBottom2 || _cliGlobalFunction("_stickOutputToBottom");
  if (out && stickOutputToBottom) {
    stickOutputToBottom(out, tab);
  } else if (out) {
    out.scrollTop = out.scrollHeight;
  }
  const updateOutputFollowButton3 = typeof updateOutputFollowButton2 !== "undefined" && updateOutputFollowButton2 || _cliGlobalFunction("updateOutputFollowButton");
  if (updateOutputFollowButton3) updateOutputFollowButton3(id);
}
function _cliSetStatus(statusValue) {
  const setStatus3 = typeof setStatus === "function" && setStatus || _cliGlobalFunction("setStatus");
  if (setStatus3) setStatus3(statusValue);
}
function _cliAppendCommandEcho(command, tabId = null) {
  const appendCommandEcho3 = typeof appendCommandEcho === "function" && appendCommandEcho || _cliGlobalFunction("appendCommandEcho");
  if (appendCommandEcho3) appendCommandEcho3(command, tabId);
}
...
function _cliPreferenceCore() {
  return typeof DarklabPreferenceCore !== "undefined" && DarklabPreferenceCore || {};
}
function _cliTimestampModes() {
  return Array.isArray(LOCAL_COMMAND_GLOBAL?._tsModes) ? LOCAL_COMMAND_GLOBAL._tsModes.slice() : ["off", "elapsed", "clock"];
}
function _cliTimestampMode() {
  const mode = typeof getTimestampMode === "function" ? getTimestampMode() : typeof LOCAL_COMMAND_GLOBAL?.tsMode === "string" ? LOCAL_COMMAND_GLOBAL.tsMode : "off";
  return _cliTimestampModes().includes(mode) ? mode : "off";
}
function _cliLineNumberMode() {
  const mode = typeof getLineNumberMode === "function" ? getLineNumberMode() : typeof LOCAL_COMMAND_GLOBAL?.lnMode === "string" ? LOCAL_COMMAND_GLOBAL.lnMode : "off";
  return mode === "on" ? "on" : "off";
}
function _cliPreferenceFunction(importedFn, name) {
  return typeof importedFn !== "undefined" && importedFn || _cliGlobalFunction(name);
}
function _cliThemeSlug(entry) {
  const normalizeThemeName = typeof _normalizeThemeName !== "undefined" && _normalizeThemeName || _cliGlobalFunction("_normalizeThemeName");
  return typeof normalizeThemeName === "function" ? normalizeThemeName(entry?.name || entry?.filename || "") : String(entry?.name || entry?.filename || "").trim().replace(/\.yaml$/, "");
}
function _cliThemeEntries() {
  const getThemeThemes = typeof _getThemeThemes !== "undefined" && _getThemeThemes || _cliGlobalFunction("_getThemeThemes");
  const compareThemeEntries = typeof _compareThemeEntries !== "undefined" && _compareThemeEntries || _cliGlobalFunction("_compareThemeEntries");
  const entries = typeof getThemeThemes === "function" ? getThemeThemes() : [];
  const compare = typeof compareThemeEntries === "function" ? compareThemeEntries : void 0;
...
function _cliCurrentThemeEntry() {
  const resolveThemeEntry = typeof _resolveThemeEntry !== "undefined" && _resolveThemeEntry || _cliGlobalFunction("_resolveThemeEntry");
  const savedThemeName = typeof _savedThemeName !== "undefined" && _savedThemeName || _cliGlobalFunction("_savedThemeName");
...
function _formatCliTableRow(values, widths) {
  ...
  return values.map((value, index) => index === lastIndex ? String(value || "") : String(value || "").padEnd(widths[index] || 0)).join("  ");
}
...
async function handleThemeCommand(cmd, tabId = null) {
  ...
  if (parts.length === 1 || sub === "list") {
    ...
    ["dark", "light", "other"].forEach((scheme) => {
      ...
      entries.forEach((entry2) => {
        const slug = _cliThemeSlug(entry2);
        const marker = slug === _cliCurrentThemeSlug() ? "*" : " ";
        _cliAppendLine(`  ${marker} ${slug.padEnd(24)}  ${String(entry2.label || slug)}`, "builtin-help-row builtin-plain", tabId);
      });
    });
  ...
  const findThemeEntry = typeof _findThemeEntry !== "undefined" && _findThemeEntry || _cliGlobalFunction("_findThemeEntry");
  ...
  const applyThemeSelection2 = typeof applyThemeSelection !== "undefined" && applyThemeSelection || _cliGlobalFunction("applyThemeSelection");
  if (typeof applyThemeSelection2 === "function") applyThemeSelection2(entry.name);
  _cliAppendLine(`theme set: ${_cliThemeDescription(entry)}`, "", tabId);
  _cliRecordSuccess(cmd);
  _cliSetStatus("ok");
  return true;
}
var _cliConfigValueLabels = {
  animated: "animated",
  static: "static",
  off: "off",
  ask: "ask"
...
function _cliConfigEntries() {
  ...
  const hudClockModes = Array.isArray(preferenceCore.HUD_CLOCK_MODES) ? preferenceCore.HUD_CLOCK_MODES : ["utc", "local"];
  return [
    {
      key: "line-numbers",
      ...
      set: (value) => {
        const applyLineNumberPreference2 = _cliPreferenceFunction(
          typeof applyLineNumberPreference !== "undefined" ? applyLineNumberPreference : null,
          "applyLineNumberPreference"
        );
        return typeof applyLineNumberPreference2 === "function" ? applyLineNumberPreference2(value) : void 0;
      }
    },
    {
      key: "timestamps",
      ...
      set: (value) => {
        const applyTimestampPreference2 = _cliPreferenceFunction(
          typeof applyTimestampPreference !== "undefined" ? applyTimestampPreference : null,
          "applyTimestampPreference"
        );
        return typeof applyTimestampPreference2 === "function" ? applyTimestampPreference2(value) : void 0;
      }
    },
    {
      key: "welcome",
      ...
      get: function getWelcomeCliValue() {
        const getWelcomeIntroPreference2 = _cliPreferenceFunction(
          typeof getWelcomeIntroPreference !== "undefined" ? getWelcomeIntroPreference : null,
          "getWelcomeIntroPreference"
        );
        const value = typeof getWelcomeIntroPreference2 === "function" ? getWelcomeIntroPreference2() : "animated";
        return this.fromStored[value] || "animated";
      },
      set: function setWelcomeCliValue(value) {
        const applyWelcomeIntroPreference2 = _cliPreferenceFunction(
          typeof applyWelcomeIntroPreference !== "undefined" ? applyWelcomeIntroPreference : null,
          "applyWelcomeIntroPreference"
        );
        if (typeof applyWelcomeIntroPreference2 === "function") applyWelcomeIntroPreference2(this.toStored[value] || value);
      }
    },
    {
      key: "share-redaction",
      ...
      get: function getShareRedactionCliValue() {
        const getShareRedactionDefaultPreference2 = _cliPreferenceFunction(
          typeof getShareRedactionDefaultPreference !== "undefined" ? getShareRedactionDefaultPreference : null,
          "getShareRedactionDefaultPreference"
        );
        const value = typeof getShareRedactionDefaultPreference2 === "function" ? getShareRedactionDefaultPreference2() : "unset";
        return this.fromStored[value] || "ask";
      },
      set: function setShareRedactionCliValue(value) {
        const applyShareRedactionDefaultPreference2 = _cliPreferenceFunction(
          typeof applyShareRedactionDefaultPreference !== "undefined" ? applyShareRedactionDefaultPreference : null,
          "applyShareRedactionDefaultPreference"
        );
        if (typeof applyShareRedactionDefaultPreference2 === "function") applyShareRedactionDefaultPreference2(this.toStored[value] || value);
      }
    },
    {
      ...
      values: ["on", "off"],
      get: () => {
        const getProjectAutoLinkExternalRunsPreference2 = _cliPreferenceFunction(
          typeof getProjectAutoLinkExternalRunsPreference !== "undefined" ? getProjectAutoLinkExternalRunsPreference : null,
          "getProjectAutoLinkExternalRunsPreference"
        );
        return typeof getProjectAutoLinkExternalRunsPreference2 === "function" ? getProjectAutoLinkExternalRunsPreference2() : "on";
      },
      set: (value) => {
        const applyProjectAutoLinkExternalRunsPreference2 = _cliPreferenceFunction(
          typeof applyProjectAutoLinkExternalRunsPreference !== "undefined" ? applyProjectAutoLinkExternalRunsPreference : null,
          "applyProjectAutoLinkExternalRunsPreference"
        );
        return typeof applyProjectAutoLinkExternalRunsPreference2 === "function" ? applyProjectAutoLinkExternalRunsPreference2(value) : void 0;
      }
    },
    {
      ...
      values: ["on", "off"],
      get: () => {
        const getProjectAutoLinkRunEntitiesPreference2 = _cliPreferenceFunction(
          typeof getProjectAutoLinkRunEntitiesPreference !== "undefined" ? getProjectAutoLinkRunEntitiesPreference : null,
          "getProjectAutoLinkRunEntitiesPreference"
        );
        return typeof getProjectAutoLinkRunEntitiesPreference2 === "function" ? getProjectAutoLinkRunEntitiesPreference2() : "on";
      },
      set: (value) => {
        const applyProjectAutoLinkRunEntitiesPreference2 = _cliPreferenceFunction(
          typeof applyProjectAutoLinkRunEntitiesPreference !== "undefined" ? applyProjectAutoLinkRunEntitiesPreference : null,
          "applyProjectAutoLinkRunEntitiesPreference"
        );
        return typeof applyProjectAutoLinkRunEntitiesPreference2 === "function" ? applyProjectAutoLinkRunEntitiesPreference2(value) : void 0;
      }
    },
    {
      ...
      values: ["on", "off"],
      get: () => {
        const getRunNotifyPreference3 = _cliPreferenceFunction(
          typeof getRunNotifyPreference !== "undefined" ? getRunNotifyPreference : null,
          "getRunNotifyPreference"
        );
        return typeof getRunNotifyPreference3 === "function" ? getRunNotifyPreference3() : "off";
      },
      set: (value) => {
        const applyRunNotifyPreference2 = _cliPreferenceFunction(
          typeof applyRunNotifyPreference !== "undefined" ? applyRunNotifyPreference : null,
          "applyRunNotifyPreference"
        );
        return typeof applyRunNotifyPreference2 === "function" ? applyRunNotifyPreference2(value) : void 0;
      }
    },
    {
      ...
      values: ["on", "off"],
      get: () => {
        const getCommandOutcomeSummariesPreference2 = _cliPreferenceFunction(
          typeof getCommandOutcomeSummariesPreference !== "undefined" ? getCommandOutcomeSummariesPreference : null,
          "getCommandOutcomeSummariesPreference"
        );
        return typeof getCommandOutcomeSummariesPreference2 === "function" ? getCommandOutcomeSummariesPreference2() : "on";
      },
      set: (value) => {
        const applyCommandOutcomeSummariesPreference2 = _cliPreferenceFunction(
          typeof applyCommandOutcomeSummariesPreference !== "undefined" ? applyCommandOutcomeSummariesPreference : null,
          "applyCommandOutcomeSummariesPreference"
        );
        return typeof applyCommandOutcomeSummariesPreference2 === "function" ? applyCommandOutcomeSummariesPreference2(value) : void 0;
      }
    },
    {
      key: "hud-clock",
      ...
      get: () => {
        const getHudClockPreference2 = _cliPreferenceFunction(
          typeof getHudClockPreference !== "undefined" ? getHudClockPreference : null,
          "getHudClockPreference"
        );
        return typeof getHudClockPreference2 === "function" ? getHudClockPreference2() : "utc";
      },
      set: (value) => {
        const applyHudClockPreference2 = _cliPreferenceFunction(
          typeof applyHudClockPreference !== "undefined" ? applyHudClockPreference : null,
          "applyHudClockPreference"
        );
        return typeof applyHudClockPreference2 === "function" ? applyHudClockPreference2(value) : void 0;
      }
    },
    {
      key: "compare-view",
      ...
      get: function getCompareViewCliValue() {
        const getCompareViewModePreference2 = _cliPreferenceFunction(
          typeof getCompareViewModePreference !== "undefined" ? getCompareViewModePreference : null,
          "getCompareViewModePreference"
        );
        const value = typeof getCompareViewModePreference2 === "function" ? getCompareViewModePreference2() : "auto";
        return this.fromStored[value] || "auto";
      },
      set: function setCompareViewCliValue(value) {
        const applyCompareViewModePreference2 = _cliPreferenceFunction(
          typeof applyCompareViewModePreference !== "undefined" ? applyCompareViewModePreference : null,
          "applyCompareViewModePreference"
        );
        if (typeof applyCompareViewModePreference2 === "function") applyCompareViewModePreference2(this.toStored[value] || value);
      }
    },
    {
      key: "compare-context",
      ...
      get: () => {
        const getCompareContextPreference2 = _cliPreferenceFunction(
          typeof getCompareContextPreference !== "undefined" ? getCompareContextPreference : null,
          "getCompareContextPreference"
        );
        return typeof getCompareContextPreference2 === "function" ? getCompareContextPreference2() : "3";
      },
      set: (value) => {
        const applyCompareContextPreference2 = _cliPreferenceFunction(
          typeof applyCompareContextPreference !== "undefined" ? applyCompareContextPreference : null,
          "applyCompareContextPreference"
        );
        return typeof applyCompareContextPreference2 === "function" ? applyCompareContextPreference2(value) : void 0;
      }
    },
    {
      key: "prompt-username",
      ...
      get: () => {
        const getPromptUsernamePreference2 = _cliPreferenceFunction(
          typeof getPromptUsernamePreference !== "undefined" ? getPromptUsernamePreference : null,
          "getPromptUsernamePreference"
        );
        return (typeof getPromptUsernamePreference2 === "function" ? getPromptUsernamePreference2() : "") || "default";
      },
      normalize: _cliNormalizePromptUsernameValue,
      set: (value) => {
        const applyPromptUsernamePreference2 = _cliPreferenceFunction(
          typeof applyPromptUsernamePreference !== "undefined" ? applyPromptUsernamePreference : null,
          "applyPromptUsernamePreference"
        );
        return typeof applyPromptUsernamePreference2 === "function" ? applyPromptUsernamePreference2(value) : void 0;
      }
    }
  ];
}
...
function _normalizeCliConfigEntryValue(entry, value) {
  if (typeof entry.normalize === "function") {
    const normalized2 = entry.normalize(value);
    return normalized2 === "" || normalized2 ? normalized2 : null;
  }
  const normalized = _cliNormalizeValue(value);
  const aliased = entry.aliases && Object.prototype.hasOwnProperty.call(entry.aliases, normalized) ? entry.aliases[normalized] : normalized;
...
async function handleConfigCommand(cmd, tabId = null) {
  ...
  if (sub === "get") {
    const key2 = parts[2] || "";
    const entry2 = _findCliConfigEntry(key2);
    if (!entry2) {
      _cliAppendLine(`config: unknown option '${key2}'`, "exit-fail", tabId);
      _cliAppendLine("run 'config list' to see available options", "", tabId);
      _cliSetStatus("fail");
      return true;
    }
    _printCliConfigEntry(entry2, tabId);
    _cliRecordSuccess(cmd);
    _cliSetStatus("ok");
    return true;
  }
...
var TOUR_CLI_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _tourGlobalFunction(name) {
  const fn = TOUR_CLI_GLOBAL && TOUR_CLI_GLOBAL[name];
  return typeof fn === "function" ? fn : null;
}
function _tourConfig() {
  const readConfig = typeof getAppConfig !== "undefined" ? getAppConfig : null;
  if (typeof readConfig === "function") return readConfig();
  return TOUR_CLI_GLOBAL.APP_CONFIG || {};
}
function _tourActiveTabId() {
  const readActiveTabId = typeof getActiveTabId !== "undefined" && getActiveTabId || _tourGlobalFunction("getActiveTabId");
  if (typeof readActiveTabId === "function") return readActiveTabId();
  return TOUR_CLI_GLOBAL.activeTabId || null;
}
function _tourAppendLine(text, cls = "", tabId = null, options = void 0) {
  const append = typeof _cliAppendLine !== "undefined" && _cliAppendLine || _tourGlobalFunction("_cliAppendLine");
  if (typeof append === "function") append(text, cls, tabId, options);
}
function _tourPreserveOutputTail(tabId = null, preserveTail = true) {
  const preserve = typeof _cliPreserveOutputTail !== "undefined" && _cliPreserveOutputTail || _tourGlobalFunction("_cliPreserveOutputTail");
  if (typeof preserve === "function") preserve(tabId, preserveTail);
}
function _tourRecordSuccess(cmd) {
  const record = typeof _cliRecordSuccess !== "undefined" && _cliRecordSuccess || _tourGlobalFunction("_cliRecordSuccess");
  if (typeof record === "function") record(cmd);
}
function _tourSetStatus(status3) {
  const setCliStatus = typeof _cliSetStatus !== "undefined" && _cliSetStatus || _tourGlobalFunction("_cliSetStatus");
  if (typeof setCliStatus === "function") setCliStatus(status3);
}
function _tourShouldPreserveOutputTail(tabId = null) {
  const shouldPreserve = typeof _cliShouldPreserveOutputTail !== "undefined" && _cliShouldPreserveOutputTail || _tourGlobalFunction("_cliShouldPreserveOutputTail");
  return typeof shouldPreserve === "function" ? shouldPreserve(tabId) : true;
}
function _tourAppendCommandEcho(cmd, tabId = null) {
  const appendEcho = typeof appendCommandEcho2 !== "undefined" && appendCommandEcho2 || _tourGlobalFunction("appendCommandEcho");
  if (typeof appendEcho === "function") appendEcho(cmd, tabId);
}
function _tourUseMobileTerminalViewportMode() {
  const useMobile = typeof useMobileTerminalViewportMode2 !== "undefined" && useMobileTerminalViewportMode2 || _tourGlobalFunction("useMobileTerminalViewportMode");
  return typeof useMobile === "function" && useMobile();
}
function _tourGetOutput(tabId) {
  const readOutput = typeof getOutput2 !== "undefined" && getOutput2 || _tourGlobalFunction("getOutput");
  return typeof readOutput === "function" ? readOutput(tabId) : null;
}
function _tourCreateTab() {
  const create = typeof createTab2 !== "undefined" && createTab2 || _tourGlobalFunction("createTab");
  return typeof create === "function" ? create() : null;
}
function _tourActivateTab(tabId) {
  const activate = typeof activateTab2 !== "undefined" && activateTab2 || _tourGlobalFunction("activateTab");
  if (typeof activate === "function") activate(tabId);
}
function _tourSetComposerValue(value, start, end, options) {
  const setValue = typeof setComposerValue !== "undefined" && setComposerValue || _tourGlobalFunction("setComposerValue");
  if (typeof setValue === "function") setValue(value, start, end, options);
}
function _tourRefocusComposerAfterAction(options) {
  const refocus = typeof refocusComposerAfterAction !== "undefined" && refocusComposerAfterAction || _tourGlobalFunction("refocusComposerAfterAction");
  if (typeof refocus === "function") refocus(options);
}
function _tourShellPromptWrap() {
  return typeof shellPromptWrap !== "undefined" && shellPromptWrap || TOUR_CLI_GLOBAL.shellPromptWrap || null;
}
...
var TOUR_CLI_TYPE_CHARS_PER_FRAME = 10;
...
function _cliAppendTourDomLine(text = "", cls = "", tabId = null) {
  ...
  const prompt = shellPrompt && shellPrompt.parentElement === out ? shellPrompt : null;
...
function _openTourSampleInNewTab(command, sourceTabId = null) {
  ...
  if (typeof createTab2 !== "undefined" || _tourGlobalFunction("createTab")) {
...
function _tourWaitForContinue(tabId = null, isLastChapter = false) {
  const prompt = isLastChapter ? "Press any key to finish, or press q to quit the tour." : "Press any key to continue, or press q to quit the tour.";
...
async function handleTourCommand(cmd, tabId = null) {
  ...
  const recordOpened = typeof _recordTourOpenedOnceThisSession === "function" && _recordTourOpenedOnceThisSession || _tourGlobalFunction("_recordTourOpenedOnceThisSession");
...
var WORKFLOW_BRIDGE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var warnedMissingWorkflowHandlers = /* @__PURE__ */ new Set();
var workflowHandlers = {
...
function _workflowBridgeWarningsEnabled() {
  const config = WORKFLOW_BRIDGE_GLOBAL?.APP_CONFIG && typeof WORKFLOW_BRIDGE_GLOBAL.APP_CONFIG === "object" && !Array.isArray(WORKFLOW_BRIDGE_GLOBAL.APP_CONFIG) ? WORKFLOW_BRIDGE_GLOBAL.APP_CONFIG : {};
  return config.frontend_bridge_warnings === true || config.debug === true || config.dev_mode === true || config.environment === "development" || config.env === "development";
}
...
function renderWorkflowItems(...args) {
  return _callWorkflowHandler("renderWorkflowItems", void 0, args);
}
function reloadWorkflowCatalog(...args) {
  return _callWorkflowHandler("reloadWorkflowCatalog", void 0, args);
}
function ensureWorkflowCatalogLoaded(...args) {
  return _callWorkflowHandler("ensureWorkflowCatalogLoaded", void 0, args);
}
...
var _stalledTimeouts = /* @__PURE__ */ new Map();
var _stalledRuns = /* @__PURE__ */ new Set();
var _runStreamStateByTabId = /* @__PURE__ */ new Map();
var _streamRecoveryTimers = /* @__PURE__ */ new Map();
var RUNNER_GLOBAL = typeof window !== "undefined" ? window : globalThis;
var timerStart = 0;
...
function _runnerValue(name, imported = void 0) {
  return imported !== void 0 ? imported : RUNNER_GLOBAL ? RUNNER_GLOBAL[name] : void 0;
}
function _runnerIgnoreFailure(value) {
  return Promise.resolve(value).catch(() => {
  });
}
function _runnerEl(name, imported = void 0) {
  return _runnerValue(name, imported) || null;
}
function _runnerCurrentSessionId() {
  if (typeof getSessionId2 === "function") return getSessionId2();
  return SESSION_ID2 || _runnerValue("SESSION_ID") || "";
}
function _runnerState() {
  const state = _runnerFn("getAppState", getAppState)?.();
  return state || _runnerValue("APP_STATE") || {};
}
function _runnerActiveTabId() {
  const id = _runnerFn("getActiveTabId", getActiveTabId)?.();
  return id || _runnerState().activeTabId || null;
}
function _runnerTabs() {
  const tabs = _runnerFn("getTabs", getTabs)?.();
  if (Array.isArray(tabs)) return tabs;
  const state = _runnerState();
  return Array.isArray(state.tabs) ? state.tabs : [];
}
var cmdInput2 = _runnerEl("cmdInput", cmdInput);
var mobileCmdInput2 = _runnerEl("mobileCmdInput", mobileCmdInput);
var runBtn2 = _runnerEl("runBtn", runBtn);
var runTimer2 = _runnerEl("runTimer", runTimer);
var status2 = _runnerEl("status", status);
var apiFetch3 = (...args) => _runnerFn("apiFetch", apiFetch2)?.(...args);
var appendLine2 = (...args) => _runnerFn("appendLine", appendLine)?.(...args);
var appendLines2 = (...args) => _runnerFn("appendLines", appendLines)?.(...args);
var getTab2 = (...args) => _runnerFn("getTab", getTab)?.(...args);
var getActiveTab2 = (...args) => _runnerFn("getActiveTab", getActiveTab)?.(...args);
var activateTab3 = (...args) => _runnerFn("activateTab", activateTab2)?.(...args);
var clearTab3 = (...args) => _runnerFn("clearTab", clearTab2)?.(...args);
var createTab3 = (...args) => _runnerFn("createTab", createTab2)?.(...args);
var createDefaultTabLabel3 = (...args) => _runnerFn("createDefaultTabLabel", createDefaultTabLabel2)?.(...args);
var closeTab2 = (...args) => _runnerFn("closeTab", closeTab)?.(...args);
var finalizeClosingTab2 = (...args) => _runnerFn("finalizeClosingTab", finalizeClosingTab)?.(...args);
var setTabStatus3 = (...args) => _runnerFn("setTabStatus", setTabStatus2)?.(...args);
var setTabRunningCommand2 = (tabId, command) => {
  const fn = _runnerFn("setTabRunningCommand", setTabRunningCommand);
  if (typeof fn === "function") return fn(tabId, command);
  const tab = typeof getTab2 === "function" ? getTab2(tabId) : null;
  if (tab) tab.command = command;
  return void 0;
};
var setTabLabel3 = (...args) => _runnerFn("setTabLabel", setTabLabel2)?.(...args);
var hideTabKillBtn2 = (...args) => _runnerFn("hideTabKillBtn", hideTabKillBtn)?.(...args);
var showTabKillBtn2 = (...args) => _runnerFn("showTabKillBtn", showTabKillBtn)?.(...args);
var emitUiEvent2 = (...args) => _runnerFn("emitUiEvent", emitUiEvent)?.(...args);
var logClientError3 = (...args) => _runnerFn("logClientError", logClientError2)?.(...args);
var maskSessionToken3 = (...args) => _runnerFn("maskSessionToken", maskSessionToken2)?.(...args);
var updateSessionId2 = (...args) => {
  const fn = _runnerFn("updateSessionId", updateSessionId);
  const result = typeof fn === "function" ? fn(...args) : void 0;
  if (typeof getSessionId2 === "function") SESSION_ID2 = getSessionId2();
  else if (args.length) SESSION_ID2 = args[0] || SESSION_ID2;
  return result;
};
var describeFetchError3 = (...args) => _runnerFn("describeFetchError", describeFetchError2)?.(...args);
var isHistoryPanelOpen2 = (...args) => _runnerFn("isHistoryPanelOpen", isHistoryPanelOpen)?.(...args);
var refreshHistoryPanel2 = (...args) => {
  const bridge = typeof hasHistoryPanelHandler === "function" && hasHistoryPanelHandler("refreshHistoryPanel") && typeof refreshHistoryPanel === "function" ? refreshHistoryPanel : null;
  return _runnerFn("refreshHistoryPanel", bridge)?.(...args);
};
var refocusComposerAfterAction2 = (...args) => _runnerFn("refocusComposerAfterAction", refocusComposerAfterAction)?.(...args);
var setComposerValue2 = (...args) => _runnerFn("setComposerValue", setComposerValue)?.(...args);
var getComposerValue2 = (...args) => _runnerFn("getComposerValue", getComposerValue)?.(...args);
var blurVisibleComposerInputIfMobile2 = (...args) => _runnerFn("blurVisibleComposerInputIfMobile", blurVisibleComposerInputIfMobile)?.(...args);
var syncRunButtonDisabled2 = (...args) => _runnerFn("syncRunButtonDisabled", syncRunButtonDisabled)?.(...args);
var appendHighVolumeOutputFinalSummary2 = (...args) => _runnerFn("appendHighVolumeOutputFinalSummary", appendHighVolumeOutputFinalSummary)?.(...args);
var disableHighVolumeOutputResumeControls2 = (...args) => _runnerFn("disableHighVolumeOutputResumeControls", disableHighVolumeOutputResumeControls)?.(...args);
var discardPendingOutputBatch2 = (...args) => _runnerFn("discardPendingOutputBatch", discardPendingOutputBatch)?.(...args);
var recordLiveOutputCoalescedLines2 = (...args) => _runnerFn("recordLiveOutputCoalescedLines", recordLiveOutputCoalescedLines)?.(...args);
var renderCommandOutcomeSummary2 = (...args) => _runnerFn("renderCommandOutcomeSummary", renderCommandOutcomeSummary)?.(...args);
var resetHighVolumeOutputState2 = (...args) => _runnerFn("resetHighVolumeOutputState", resetHighVolumeOutputState)?.(...args);
var addToHistory2 = (...args) => _runnerFn("addToHistory", addToHistory)?.(...args);
var addToRecentPreview2 = (...args) => _runnerFn("addToRecentPreview", addToRecentPreview)?.(...args);
var hydrateCmdHistory2 = (...args) => _runnerFn("hydrateCmdHistory", hydrateCmdHistory)?.(...args);
var _runnerEnsureWorkspaceCacheAdapter = (...args) => _runnerFn("_ensureWorkspaceCache", _ensureWorkspaceCache)?.(...args);
var _runnerIsWorkspaceDeleteCommandAdapter = (...args) => _runnerFn("_isWorkspaceDeleteCommand", _isWorkspaceDeleteCommand)?.(...args);
var _runnerIsWorkspaceDownloadCommandAdapter = (...args) => _runnerFn("_isWorkspaceDownloadCommand", _isWorkspaceDownloadCommand)?.(...args);
var _runnerIsWorkspaceEditorCommandAdapter = (...args) => _runnerFn("_isWorkspaceEditorCommand", _isWorkspaceEditorCommand)?.(...args);
var _runnerIsWorkspaceMoveCommandAdapter = (...args) => _runnerFn("_isWorkspaceMoveCommand", _isWorkspaceMoveCommand)?.(...args);
var _runnerIsWorkspaceTerminalCommandAdapter = (...args) => _runnerFn("_isWorkspaceTerminalCommand", _isWorkspaceTerminalCommand)?.(...args);
var _runnerResolveExistingWorkspaceCommandPathAdapter = (...args) => _runnerFn("_resolveExistingWorkspaceCommandPath", _resolveExistingWorkspaceCommandPath)?.(...args);
var _runnerResolveWorkspaceCommandPathAdapter = (...args) => _runnerFn("_resolveWorkspaceCommandPath", _resolveWorkspaceCommandPath)?.(...args);
var _runnerSetWorkspaceCwdAdapter = (...args) => _runnerFn("_setWorkspaceCwd", _setWorkspaceCwd)?.(...args);
var _runnerWorkspaceCommandTokensAdapter = (...args) => _runnerFn("_workspaceCommandTokens", _workspaceCommandTokens)?.(...args);
var _runnerWorkspaceCwdAdapter = (...args) => _runnerFn("_workspaceCwd", _workspaceCwd)?.(...args);
var _runnerWorkspaceDeleteCommandAdapter = (...args) => _runnerFn("_workspaceDeleteCommand", _workspaceDeleteCommand)?.(...args);
var _runnerWorkspaceDisplayPathAdapter = (...args) => _runnerFn("_workspaceDisplayPath", _workspaceDisplayPath)?.(...args);
var _runnerWorkspaceDownloadTargetAdapter = (...args) => _runnerFn("_workspaceDownloadTarget", _workspaceDownloadTarget)?.(...args);
var _runnerWorkspaceEditorCommandAdapter = (...args) => _runnerFn("_workspaceEditorCommand", _workspaceEditorCommand)?.(...args);
var _runnerWorkspaceExpandPathPatternAdapter = (...args) => _runnerFn("_workspaceExpandPathPattern", _workspaceExpandPathPattern)?.(...args);
var _runnerWorkspaceListCommandAdapter = (...args) => _runnerFn("_workspaceListCommand", _workspaceListCommand)?.(...args);
var _runnerWorkspaceMoveCommandAdapter = (...args) => _runnerFn("_workspaceMoveCommand", _workspaceMoveCommand)?.(...args);
var _runnerWorkspacePathExistsAdapter = (...args) => _runnerFn("_workspacePathExists", _workspacePathExists)?.(...args);
var _runnerWorkspacePathHasGlobAdapter = (...args) => _runnerFn("_workspacePathHasGlob", _workspacePathHasGlob)?.(...args);
var _formatWorkspaceBytes2 = (...args) => _runnerFn("_formatWorkspaceBytes")?.(...args);
var _maybeMountDeferredPrompt2 = (...args) => _runnerFn("_maybeMountDeferredPrompt", _maybeMountDeferredPrompt)?.(...args);
var _runnerActiveTeamScopeCanAdapter = (...args) => {
  const fn = _runnerFn("activeTeamScopeCan", activeTeamScopeCan);
...
var _runnerCopyTextToClipboardAdapter = (...args) => {
  const fn = _runnerFn("copyTextToClipboard", copyTextToClipboard);
  return typeof fn === "function" ? fn(...args) : Promise.reject(new Error("clipboard unavailable"));
};
var createWorkspaceDirectory2 = (...args) => {
  const fn = _runnerFn("createWorkspaceDirectory", createWorkspaceDirectory);
  if (typeof fn !== "function") throw new Error("workspace folder creation is unavailable");
  return fn(...args);
};
var dismissMobileKeyboardAfterSubmit2 = (...args) => {
  const fn = typeof hasMobileShellLayoutHandler === "function" && hasMobileShellLayoutHandler("dismissMobileKeyboardAfterSubmit") ? dismissMobileKeyboardAfterSubmit : _runnerFn("dismissMobileKeyboardAfterSubmit");
  return typeof fn === "function" ? fn(...args) : void 0;
};
var downloadWorkspaceFile2 = (...args) => _runnerFn("downloadWorkspaceFile")?.(...args);
var flushRecentValues2 = _runnerFn("flushRecentValues", flushRecentValues);
var getRunNotifyPreference2 = (...args) => _runnerFn("getRunNotifyPreference")?.(...args);
var _runnerHandleConfigCommandAdapter = (...args) => {
  const fn = typeof handleConfigCommand === "function" && handleConfigCommand || _runnerFn("handleConfigCommand");
  return typeof fn === "function" ? fn(...args) : false;
};
var _runnerHandleSecretCommandAdapter = (...args) => {
  const fn = typeof hasSecretsHandler === "function" && hasSecretsHandler("handleSecretCommand") && typeof handleSecretCommand === "function" ? handleSecretCommand : _runnerFn("handleSecretCommand");
  return typeof fn === "function" ? fn(...args) : false;
};
var _runnerHandleThemeCommandAdapter = (...args) => {
  const fn = typeof handleThemeCommand === "function" && handleThemeCommand || _runnerFn("handleThemeCommand");
  return typeof fn === "function" ? fn(...args) : false;
};
var _runnerHandleTourCommandAdapter = (...args) => {
  const fn = typeof handleTourCommand === "function" && handleTourCommand || _runnerFn("handleTourCommand");
  return typeof fn === "function" ? fn(...args) : false;
};
var _runnerHandleWorkflowTerminalCommandAdapter = (...args) => {
  const fn = typeof hasWorkflowHandler === "function" && hasWorkflowHandler("handleWorkflowTerminalCommand") ? handleWorkflowTerminalCommand : _runnerFn("handleWorkflowTerminalCommand");
  return typeof fn === "function" ? fn(...args) : false;
};
var hideRunTimer = (...args) => _runnerFn("hideRunTimer")?.(...args);
var isInteractivePtyCommand = (...args) => _runnerFn("isInteractivePtyCommand")?.(...args);
var isProjectWorkspaceOpen2 = (...args) => _runnerFn("isProjectWorkspaceOpen")?.(...args);
var isRunButtonDisabled = (...args) => _runnerFn("isRunButtonDisabled")?.(...args);
var loadRecentValues2 = (...args) => _runnerFn("loadRecentValues", loadRecentValues)?.(...args);
var loadStarredFromServer2 = (...args) => _runnerFn("loadStarredFromServer", loadStarredFromServer)?.(...args);
var moveWorkspacePath2 = (...args) => {
  const fn = typeof moveWorkspacePath === "function" && moveWorkspacePath || _runnerFn("moveWorkspacePath");
  return typeof fn === "function" ? fn(...args) : void 0;
};
var notifyProjectWorkspaceChanged = (...args) => _runnerFn("notifyProjectWorkspaceChanged")?.(...args);
var openWorkspaceEditorFromCommand2 = (...args) => {
  const fn = typeof openWorkspaceEditorFromCommand === "function" && openWorkspaceEditorFromCommand || _runnerFn("openWorkspaceEditorFromCommand");
  return typeof fn === "function" ? fn(...args) : void 0;
};
var readWorkspaceFile2 = (...args) => {
  const fn = typeof readWorkspaceFile === "function" && readWorkspaceFile || _runnerFn("readWorkspaceFile");
  return typeof fn === "function" ? fn(...args) : void 0;
};
var refreshActiveProjectContext2 = (...args) => _runnerFn("refreshActiveProjectContext")?.(...args);
var refreshProjectWorkspace2 = (...args) => _runnerFn("refreshProjectWorkspace")?.(...args);
var refreshWorkspaceFiles2 = (...args) => {
  const fn = typeof refreshWorkspaceFiles === "function" && refreshWorkspaceFiles || _runnerFn("refreshWorkspaceFiles");
  return typeof fn === "function" ? fn(...args) : void 0;
};
var reloadSessionHistory2 = (...args) => _runnerFn("reloadSessionHistory", reloadSessionHistory)?.(...args);
var reloadWorkflowCatalog2 = (...args) => {
  const fn = typeof hasWorkflowHandler === "function" && hasWorkflowHandler("reloadWorkflowCatalog") ? reloadWorkflowCatalog : _runnerFn("reloadWorkflowCatalog");
  return typeof fn === "function" ? fn(...args) : void 0;
};
var rememberRecentValuesFromCommand2 = (...args) => {
  const fn = _runnerFn("rememberRecentValuesFromCommand", rememberRecentValuesFromCommand);
  return typeof fn === "function" ? fn(...args) : void 0;
};
var restoreHistoryRunIntoTab = (...args) => _runnerFn("restoreHistoryRunIntoTab")?.(...args);
var setComposerPromptMode2 = (...args) => {
  const fn = typeof hasComposerPromptHandler === "function" && hasComposerPromptHandler("setComposerPromptMode") ? setComposerPromptMode : _runnerFn("setComposerPromptMode");
  return typeof fn === "function" ? fn(...args) : void 0;
};
var setRunButtonDisabled = (...args) => {
  const fn = _runnerFn("setRunButtonDisabled");
  if (typeof fn === "function") return fn(...args);
  if (runBtn2) runBtn2.disabled = !!args[0];
  const mobileRunBtn2 = _runnerEl("mobileRunBtn", mobileRunBtn);
  if (mobileRunBtn2) mobileRunBtn2.disabled = !!args[0];
  return void 0;
};
var showConfirm2 = (...args) => _runnerFn("showConfirm", showConfirm)?.(...args);
var showRunTimer = (...args) => _runnerFn("showRunTimer")?.(...args);
var showToast2 = (...args) => _runnerFn("showToast")?.(...args);
...
var _runnerTeamScopeDeniedMessageAdapter = (...args) => {
  const fn = _runnerFn("teamScopeDeniedMessage", teamScopeDeniedMessage);
  return typeof fn === "function" ? fn(...args) : `View-only team members can't ${args[0] || "run commands in team scope"}. Switch to Personal or ask for operator access.`;
};
...
var CLIENT_ID2 = _runnerValue("CLIENT_ID") || "";
var SESSION_ID2 = typeof getSessionId2 === "function" && getSessionId2() || _runnerValue("SESSION_ID") || "";
function _runnerAppConfig() {
  const config = _runnerValue("APP_CONFIG");
  return config && typeof config === "object" && !Array.isArray(config) ? config : {};
}
function _runnerCore() {
  return typeof DarklabRunnerCore !== "undefined" && DarklabRunnerCore || null;
}
function _welcomeApi(name) {
  if (name === "cancelWelcome" && typeof cancelWelcome === "function") return cancelWelcome;
  if (name === "welcomeOwnsTab" && typeof welcomeOwnsTab === "function") return welcomeOwnsTab;
  return typeof window !== "undefined" && window[name] || typeof globalThis !== "undefined" && globalThis[name] || null;
}
function _isWelcomeActive() {
  const welcomeState = _runnerFn("getWelcomeState", getWelcomeState)?.();
  return !!(_runnerValue("_welcomeActive") || welcomeState && welcomeState.active);
}
function _isWelcomeDone() {
  const welcomeState = _runnerFn("getWelcomeState", getWelcomeState)?.();
  return !!(_runnerValue("_welcomeDone") || welcomeState && welcomeState.done);
}
...
var _RUN_STREAM_MESSAGE_BATCH_LIMIT = 750;
var _RUN_STREAM_MESSAGE_BATCH_MS = 12;
var _runnerPersistence = null;
var _activeRunPollTimer = null;
...
var _pendingTerminalConfirm = null;
function _runnerPersistenceHelpers() {
  if (!_runnerPersistence) {
    const createPersistence = typeof createRunnerPersistence !== "undefined" && createRunnerPersistence || null;
    ...
    _runnerPersistence = createPersistence({
      apiFetch: apiFetch3,
      maskSessionToken: maskSessionToken3,
      isHistoryPanelOpen: typeof isHistoryPanelOpen2 === "function" ? isHistoryPanelOpen2 : null,
      refreshHistoryPanel: typeof refreshHistoryPanel2 === "function" ? refreshHistoryPanel2 : null,
      logClientError: typeof logClientError3 === "function" ? logClientError3 : null
    });
  }
  return _runnerPersistence;
}
function _runnerIsActiveRunDetachedForRestore(runId) {
  const isDetached = typeof _isActiveRunDetachedForRestore !== "undefined" && _isActiveRunDetachedForRestore || null;
  return typeof isDetached === "function" ? isDetached(runId) : false;
}
function _runnerPruneDetachedActiveRunRestoreIds(activeRunIds) {
  const prune = typeof _pruneDetachedActiveRunRestoreIds !== "undefined" && _pruneDetachedActiveRunRestoreIds || null;
  if (typeof prune === "function") prune(activeRunIds);
}
function _runnerClearActiveRunDetachedForRestore(runId) {
  const clear = typeof clearActiveRunDetachedForRestore !== "undefined" && clearActiveRunDetachedForRestore || null;
...
function _recoverStalledRun(tabId) {
  ...
  _stalledRuns.delete(tabId);
  appendLine2("[connection re-established — live output resumed]", "exit-ok", tabId);
  const t = getTab2(tabId);
  if (!t || t.killed) return;
  ...
  setTabStatus3(tabId, "running");
  _setRunButtonDisabled(true);
  showTabKillBtn2(tabId);
}
...
function _isRunStillActive(runId) {
  if (!runId || typeof apiFetch3 !== "function") return Promise.resolve(false);
...
function _fetchActiveRun(runId, { includeScheduled = false } = {}) {
  if (!runId || typeof apiFetch3 !== "function") return Promise.resolve(null);
  return apiFetch3(_activeRunsUrl({ includeScheduled })).then((r) => r && r.ok !== false && typeof r.json === "function" ? r.json() : null).then((data) => {
    const normalized = String(runId || "");
    return (Array.isArray(data && data.runs) ? data.runs : []).find((run) => String(run && run.run_id || "") === normalized) || null;
  }).catch((err) => {
...
function _markStalledButRunning(tabId) {
  ...
  if (firstNotice) {
    appendLine2("[stream quiet — no output or heartbeat reached the browser for 45s]", "notice", tabId);
    appendLine2("[process is still running; Kill remains available and live output will continue here if the stream resumes]", "notice", tabId);
  }
  if (tabId === _runnerActiveTabId()) {
    setStatus2("running");
    syncActiveRunTimer(tabId);
  ...
  _setRunButtonDisabled(true);
  const t = getTab2(tabId);
  if (t) showTabKillBtn2(tabId);
}
function _markStalledAndInactive(tabId) {
  const firstNotice = !_stalledRuns.has(tabId);
  _stalledRuns.add(tabId);
  if (firstNotice) {
    appendLine2("[connection stalled — no stream activity arrived from the server for 45s]", "denied", tabId);
  }
  appendLine2("[process is no longer listed as active; check the history panel for the final result]", "denied", tabId);
  if (tabId === _runnerActiveTabId()) setStatus2("fail");
  setTabStatus3(tabId, "fail");
  stopTimer();
  _setRunButtonDisabled(false);
  hideTabKillBtn2(tabId);
}
function _handleStreamEndedWithoutExit(tabId) {
  _clearStalledTimeout(tabId);
  const runId = _tabRunGeneration(tabId);
  const existingTab = getTab2(tabId);
      ...
      if (_activeRunIsInteractivePty(activeRun)) {
        if (typeof attachInteractivePtyCommand === "function") {
          attachInteractivePtyCommand(activeRun, tabId).catch((err) => {
            appendLine2(`[server error] ${err.message || "Interactive PTY reattach failed"}`, "exit-fail", tabId);
            setTabStatus3(tabId, "fail");
          });
        }
        return;
...
function _scheduleActiveRunStreamRecovery(run, tabId, { after = "", mode = "reconnected" } = {}) {
  ...
  const delayMs = Math.min(1e3 * 2 ** Math.max(0, attempts - 2), 5e3);
...
function setStatus2(s) {
  status2 = status2 || _runnerEl("status", status);
  if (status2) {
    status2.className = "status-pill " + s;
    status2.textContent = s === "running" ? "RUNNING" : "IDLE";
  }
  if (typeof emitUiEvent2 === "function") {
    emitUiEvent2("app:status-changed", { status: s });
    if (s === "ok") emitUiEvent2("app:last-exit-changed", { value: 0 });
    else if (s === "fail") emitUiEvent2("app:last-exit-changed", { value: 1 });
    else if (s === "killed") emitUiEvent2("app:last-exit-changed", { value: "killed" });
  }
}
function setLastExit(value) {
  if (typeof emitUiEvent2 === "function") emitUiEvent2("app:last-exit-changed", { value });
  if (typeof emitUiEvent === "function" && emitUiEvent !== emitUiEvent2) {
    emitUiEvent("app:last-exit-changed", { value });
  }
}
function _maybeNotify(command, codeOrStatus, elapsed) {
  if (typeof getRunNotifyPreference2 !== "function" || getRunNotifyPreference2() !== "on") return;
  if (typeof Notification === "undefined" || Notification.permission !== "granted") return;
  const root = (command || "").split(/\s+/)[0] || "";
  const title = root ? "$ " + root : "$";
  const body = codeOrStatus === "killed" ? elapsed ? `killed after ${elapsed}` : "killed" : elapsed ? `exit ${codeOrStatus} in ${elapsed}` : `exit ${codeOrStatus}`;
  try {
...
function startTimer(startMs = Date.now()) {
  ...
  timerInterval = setInterval(() => {
    runTimer2.textContent = _formatElapsed((Date.now() - timerStart) / 1e3);
  }, 100);
}
function stopTimer() {
...
function _startedAtLabel(started) {
  ...
  return Number.isNaN(startedAt.getTime()) ? "unknown start time" : startedAt.toLocaleString();
}
...
function _activeRunReattachNotice(run, { afterStreamRecovery = false } = {}) {
  ...
  return [
    afterStreamRecovery ? "[reattached to active run after stream recovery]" : "[reattached to active run after reload]",
    `[active run started at ${startedLabel}]`,
    "[live output will continue here]"
  ];
}
function _reattachActiveRunToTab(run, tabId, {
  after = "",
  ...
  if (!preserveTranscript) clearTab3(tabId);
  const t = getTab2(tabId);
  if (!t) return false;
  if (typeof setTabRunningCommand2 === "function") {
    setTabRunningCommand2(tabId, run.command);
  } else {
    if (!t.renamed) setTabLabel3(tabId, run.command);
  ...
  t.currentRunStartIndex = Number.isFinite(Number(t.currentRunStartIndex)) ? Number(t.currentRunStartIndex) : Array.isArray(t.rawLines) ? t.rawLines.length : 0;
  t.followOutput = true;
  ...
  if (!preserveTranscript || !Array.isArray(t.rawLines) || t.rawLines.length === 0) {
    appendCommandEcho2(run.command, tabId);
  }
  const streamRecoveryNoticeShown = afterStreamRecovery && t.streamRecoveryNoticeRunId === runId;
  if (!streamRecoveryNoticeShown) {
    _activeRunReattachNotice(run, { afterStreamRecovery }).forEach((line) => appendLine2(line, "notice", tabId));
  }
...
function restoreActiveRunsAfterReload(runs) {
  _runnerPruneDetachedActiveRunRestoreIds(new Set((Array.isArray(runs) ? runs : []).map((run) => run && run.run_id).filter(Boolean)));
  ...
  items.forEach((run, index) => {
    ...
    if (originalTab) {
      ...
      if (_activeRunIsInteractivePty(run) && typeof attachInteractivePtyCommand === "function") {
        attachInteractivePtyCommand(run, originalTab.id).catch((err) => {
          appendLine2(`[server error] ${err.message || "Interactive PTY reattach failed"}`, "exit-fail", originalTab.id);
          setTabStatus3(originalTab.id, "fail");
        });
        return;
      }
    ...
    const canReuseBootstrapTab = !!(bootstrapTab && bootstrapTab.st === "idle" && !bootstrapTab.renamed && !bootstrapTab.command && !bootstrapTab.historyRunId && !bootstrapTab.draftInput && Array.isArray(bootstrapTab.rawLines) && bootstrapTab.rawLines.length === 0);
    const tabId = canReuseBootstrapTab ? bootstrapTab.id : createTab3();
    if (!tabId) return;
  ...
  activateTab3(firstRestoredTabId);
...
function _attachActiveRunToTab(run, tabId, { mode = "attached" } = {}) {
  ...
  appendLine2("[restored available output; live output will continue here]", "notice", tabId);
  setTabStatus3(tabId, "running");
  if (tabId === _runnerActiveTabId()) {
    setStatus2("running");
    syncActiveRunTimer(tabId);
...
function attachActiveRunFromMonitor2(run) {
  ...
  const tabId = createTab3();
  if (!tabId) return Promise.resolve(false);
  activateTab3(tabId, { focusComposer: false });
...
function _tabForActiveRunId(runId) {
  ...
  return _runnerTabs().find((tab) => tab && (tab.runId === normalized || tab.historyRunId === normalized)) || null;
}
function _requestKillActiveRun(run, tabId = "") {
  const runId = String(run?.run_id || run?.id || "").trim();
  if (!runId || typeof apiFetch3 !== "function") return Promise.resolve(false);
  return apiFetch3("/kill", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ run_id: runId, tab_id: tabId })
  }).then((resp) => {
    if (resp && resp.ok) return true;
    return _readRunErrorMessage2(resp || {}).then((message) => {
      throw new Error(message || "Could not kill this run.");
    });
  });
}
function killActiveRunFromMonitor2(run) {
  const runId = String(run?.run_id || run?.id || "").trim();
  ...
  if (typeof showConfirm2 !== "function") return kill();
  return showConfirm2({
    body: {
...
function _restoreCompletedReconnectedRun(tab, run) {
  const restore = typeof window !== "undefined" && window.restoreHistoryRunIntoTab || typeof globalThis !== "undefined" && globalThis.restoreHistoryRunIntoTab || (typeof restoreHistoryRunIntoTab !== "undefined" ? restoreHistoryRunIntoTab : null);
  if (!tab || !run || typeof restore !== "function") return Promise.resolve();
  return restore(run, { targetTabId: tab.id, hidePanelOnSuccess: false }).then(() => {
    const refreshed = getTab2(tab.id);
    if (refreshed) refreshed.reconnectedRun = false;
    if (tab.id === _runnerActiveTabId()) stopTimer();
  }).catch(() => {
    appendLine2("[reconnected run finished, but the saved result could not be restored automatically]", "notice", tab.id);
    appendLine2("[open the history panel to load the completed run]", "notice", tab.id);
    setTabStatus3(tab.id, "fail");
    const refreshed = getTab2(tab.id);
    if (refreshed) refreshed.reconnectedRun = false;
    if (tab.id === _runnerActiveTabId()) stopTimer();
  });
...
function _markReconnectedRunUnavailable(tab) {
  if (!tab) return;
  appendLine2("[reconnected run is no longer active]", "denied", tab.id);
  appendLine2("[no saved result is available; the app may have restarted while the command was running]", "denied", tab.id);
  setTabStatus3(tab.id, "fail");
  const refreshed = getTab2(tab.id);
  if (refreshed) {
...
function pollActiveRunsAfterReload() {
  ...
  return apiFetch3("/history/active").then((r) => r.json()).then((data) => {
    ...
    return Promise.all(reconnectTabs.map((tab) => {
      ...
      return apiFetch3(`/history/${tab.historyRunId}?json&preview=1`).then((r) => r.ok ? r.json() : Promise.reject(new Error("run not ready"))).then((run) => _restoreCompletedReconnectedRun(tab, run)).catch(() => _markReconnectedRunUnavailable(tab));
    }));
  }).finally(() => {
...
function elapsedSeconds2() {
  return timerStart ? (Date.now() - timerStart) / 1e3 : null;
}
function _setRunButtonDisabled(disabled) {
  if (disabled) {
    if (runBtn2) runBtn2.disabled = true;
    const mobileRunBtn3 = _runnerEl("mobileRunBtn", mobileRunBtn);
    if (mobileRunBtn3) mobileRunBtn3.disabled = true;
  }
  ...
  const mobileRunBtn2 = _runnerEl("mobileRunBtn", mobileRunBtn);
  if (mobileRunBtn2) mobileRunBtn2.disabled = false;
}
function _syncMobileRunButtonAfterRunSettled() {
  ...
  const mobileRunBtn2 = _runnerEl("mobileRunBtn", mobileRunBtn);
  const mobileCmd = _runnerEl("mobileCmdInput", mobileCmdInput);
  if (!mobileRunBtn2 || !mobileCmd) return;
  if (typeof syncRunButtonDisabled2 === "function") {
    syncRunButtonDisabled2();
    return;
  }
  mobileRunBtn2.disabled = !String(mobileCmd.value || "").trim();
}
function _describeRunnerFetchError(err, context = "server") {
  return describeFetchError3(err, context);
}
function _logRunnerError(context, err, details = null) {
  logClientError3(context, err, details);
}
...
function _handleKillRequestFailure(err, tabId) {
  ...
  showToast2("Failed to send kill request; command may still be running");
  appendLine2("[kill request failed] " + _describeRunnerFetchError(err), "notice", tabId);
}
function _handleKillRequestDenied(message, tabId, runId) {
  ...
  appendLine2(`[kill request denied] ${message || "The server could not kill this run."}`, "notice", tabId);
  if (tabId === _runnerActiveTabId()) {
    setStatus2("running");
    _setRunButtonDisabled(true);
  }
...
function _currentClientId() {
  return typeof CLIENT_ID2 !== "undefined" ? String(CLIENT_ID2 || "") : "";
}
function _handleRunOwnerChanged(msg, tabId) {
  ...
  if (ownedByThisClient) {
    ...
    if (t.st === "running") showTabKillBtn2(tabId);
    return;
  }
...
function _handleRunKilled(msg, tabId) {
  ...
  t.killed = true;
  t.pendingKill = false;
  if (typeof discardPendingOutputBatch2 === "function") discardPendingOutputBatch2(tabId);
  if (!killedByThisTab) {
    appendLine2(
  ...
  setTabStatus3(tabId, "killed");
  if (typeof disableHighVolumeOutputResumeControls2 === "function") {
    disableHighVolumeOutputResumeControls2(tabId);
  }
  hideTabKillBtn2(tabId);
  if (tabId === _runnerActiveTabId()) {
    setStatus2("killed");
...
function _markTabKilledByUser(tabId, secs, { suppressTranscript = false } = {}) {
  ...
  if (!t.closing && !suppressTranscript) {
    appendLine2(`[killed by user${secs != null ? " after " + _formatElapsed(secs) : ""}]`, "exit-fail", tabId);
  }
  _maybeNotify(t.command, "killed", secs != null ? _formatElapsed(secs) : null);
  if (typeof emitUiEvent2 === "function") emitUiEvent2("app:last-exit-changed", { value: "killed" });
  setTabStatus3(tabId, "killed");
  if (typeof disableHighVolumeOutputResumeControls2 === "function") {
    disableHighVolumeOutputResumeControls2(tabId);
  }
  ...
  if (typeof _maybeMountDeferredPrompt2 === "function") {
    _maybeMountDeferredPrompt2(tabId);
  }
}
function _handleRunTransportFailure(err, tabId) {
  _logRunnerError("run request failed", err);
  appendLine2("[connection error] " + _describeRunnerFetchError(err), "exit-fail", tabId);
  if (tabId === _runnerActiveTabId()) setStatus2("fail");
  setTabStatus3(tabId, "fail");
  if (typeof disableHighVolumeOutputResumeControls2 === "function") {
    disableHighVolumeOutputResumeControls2(tabId);
...
function _appendHighVolumeOutputFinalSummary(tabId) {
  if (typeof appendHighVolumeOutputFinalSummary2 === "function") {
    appendHighVolumeOutputFinalSummary2(tabId);
  }
}
async function _readRunErrorMessage2(res) {
...
function _runTeamScopeDeniedMessage(action) {
  return typeof _runnerTeamScopeDeniedMessageAdapter === "function" ? _runnerTeamScopeDeniedMessageAdapter(action) : `View-only team members can't ${action}. Switch to Personal or ask for operator access.`;
}
...
function _streamUnknownCollector(streamState) {
  ...
  if (!state.unknownLineEventValues) state.unknownLineEventValues = /* @__PURE__ */ new Set();
  return (family, value) => {
...
function _typedRunStreamLineMessage(msg, streamState) {
  ...
  const hasTypedFields = msg.v !== void 0 || msg.kind !== void 0 || msg.role !== void 0 || msg.signals !== void 0;
  ...
  const legacy = typeof model.toLegacyWireLineEvent === "function" ? model.toLegacyWireLineEvent(event) : { text: event.text || "", cls: event.legacy_cls || "" };
  return {
    ...msg,
    ...legacy,
...
function _appendStreamLine(text, cls, tabId, msg, options = {}) {
  ...
  if (metadata) appendLine2(text, cls, tabId, metadata);
  else appendLine2(text, cls, tabId);
}
...
function _recordRunOutputCoalescing(msg, tabId) {
  ...
  if (count && typeof recordLiveOutputCoalescedLines2 === "function") {
    recordLiveOutputCoalescedLines2(tabId, count);
  }
}
...
function _handleRunOutputBatch(msg, tabId, streamState = null) {
  ...
  if (entries.length && typeof appendLines2 === "function") {
    appendLines2(entries, tabId);
  }
}
...
function appendCommandEcho2(cmd, tabId) {
  appendLine2(cmd, "prompt-echo", tabId);
}
function appendPromptNewline(tabId) {
  appendLine2("", "prompt-echo", tabId);
}
...
function _sseMessageFromChunk2(part) {
  let eventId = "";
  const dataLines = [];
  String(part || "").split(/\r?\n/).forEach((line) => {
...
function _markTabRunStarted(tabId, runId) {
  ...
  if (!sameRun && typeof resetHighVolumeOutputState2 === "function") {
    resetHighVolumeOutputState2(tabId);
  }
...
function _handleRunStreamMessage(msg, tabId, streamState = null) {
  ...
  if (msg.type === "schema" || msg.event === "schema") {
    ...
    const notifyProjectChange = typeof globalThis.notifyProjectWorkspaceChanged === "function" ? globalThis.notifyProjectWorkspaceChanged : typeof notifyProjectWorkspaceChanged === "function" ? notifyProjectWorkspaceChanged : null;
    ...
    if (msg.project_auto_promoted) {
      ...
      if (typeof emitUiEvent2 === "function") {
        emitUiEvent2("app:project-auto-promoted", {
    ...
    if (msg.project_targets_discovered) {
      ...
      if (typeof emitUiEvent2 === "function") {
        emitUiEvent2("app:project-target-discovered", {
          project_id: msg.project_id || "",
    ...
    clearTab3(tabId);
    const t2 = getTab2(tabId);
    if (t2) t2.syntheticClear = true;
    ...
    msg = _typedRunStreamLineMessage(msg, streamState);
    const t2 = getTab2(tabId);
    if (t2 && t2.killed) return;
    if (t2 && typeof msg.text === "string" && /^Unsupported built-in command: /.test(msg.text)) {
      t2.unknownCommand = true;
    }
    _forEachStreamTextLine(msg.text, (line) => {
      if (!_shouldSuppressStreamOutputLine(t2, line)) {
    ...
    const t2 = getTab2(tabId);
    if (t2) {
      t2.exitCode = msg.code;
      t2.runId = null;
      t2.reconnectedRun = false;
      t2.lastEventId = "";
      t2.attachMode = "";
      t2.streamRecoveryAttempts = 0;
      t2.streamRecoveryNoticeRunId = "";
      t2.deferPromptMount = true;
      t2.previewTruncated = !!msg.preview_truncated;
      t2.fullOutputAvailable = !!msg.full_output_available;
      t2.fullOutputLoaded = !msg.preview_truncated;
    }
    if (t2 && t2.killed) {
      t2.killed = false;
      if (typeof discardPendingOutputBatch2 === "function") discardPendingOutputBatch2(tabId);
      ...
      const finalizeTabClose2 = typeof finalizeClosingTab === "function" ? finalizeClosingTab : typeof finalizeClosingTab2 === "function" ? finalizeClosingTab2 : null;
      if (t2.closing && finalizeTabClose2) {
        finalizeTabClose2(tabId);
        if (isHistoryPanelOpen2()) refreshHistoryPanel2();
      }
      if (isHistoryPanelOpen2()) refreshHistoryPanel2();
      if (!(t2 && t2.closing) && typeof _maybeMountDeferredPrompt2 === "function") {
        _maybeMountDeferredPrompt2(tabId);
      }
      return;
    }
    ...
    if (msg.preview_truncated) {
      appendLine2(_previewTruncationNotice(msg.output_line_count, msg.full_output_available), "notice", tabId);
    }
    if (msg.code === 0) {
      if (!(t2 && t2.syntheticClear)) appendLine2(`[process exited with code 0${dur}]`, "exit-ok", tabId);
      _appendHighVolumeOutputFinalSummary(tabId);
      if (typeof renderCommandOutcomeSummary2 === "function") renderCommandOutcomeSummary2(tabId);
      if (tabId === _runnerActiveTabId()) setStatus2("ok");
      setTabStatus3(tabId, "ok");
      if (typeof disableHighVolumeOutputResumeControls2 === "function") {
        disableHighVolumeOutputResumeControls2(tabId);
      }
    } else {
      appendLine2(`[process exited with code ${msg.code}${dur}]`, "exit-fail", tabId);
      _appendHighVolumeOutputFinalSummary(tabId);
      if (typeof renderCommandOutcomeSummary2 === "function") renderCommandOutcomeSummary2(tabId);
      if (tabId === _runnerActiveTabId()) setStatus2("fail");
      setTabStatus3(tabId, "fail");
    ...
    if (typeof addToRecentPreview2 === "function" && t2 && t2.command && !t2.unknownCommand) {
      addToRecentPreview2(t2.command);
    }
    if (t2 && t2.command) _refreshProjectContextAfterCommand(t2.command, msg.code);
    if (t2 && /^var(?:\s|$)/i.test(String(t2.command || "")) && typeof loadSessionVariables === "function") {
      loadSessionVariables().catch(() => {
      });
    }
    if (t2) t2.syntheticClear = false;
    _maybeNotify(t2 ? t2.command : "", msg.code, msg.elapsed ? msg.elapsed + "s" : null);
    if (typeof emitUiEvent2 === "function") emitUiEvent2("app:last-exit-changed", { value: msg.code });
    _setRunButtonDisabled(false);
    hideTabKillBtn2(tabId);
    _syncMobileRunButtonAfterRunSettled();
    const finalizeTabClose = typeof finalizeClosingTab === "function" ? finalizeClosingTab : typeof finalizeClosingTab2 === "function" ? finalizeClosingTab2 : null;
    if (t2 && t2.closing && finalizeTabClose) {
    ...
    if (typeof _maybeMountDeferredPrompt2 === "function") _maybeMountDeferredPrompt2(tabId);
  } else if (msg.type === "error") {
    _clearStalledTimeout(tabId);
    appendLine2("[error] " + msg.text, "exit-fail", tabId);
    if (tabId === _runnerActiveTabId()) setStatus2("fail");
    setTabStatus3(tabId, "fail");
    if (typeof disableHighVolumeOutputResumeControls2 === "function") {
      disableHighVolumeOutputResumeControls2(tabId);
...
function _sameTabRunStillActive(tabId, runId) {
  const t = getTab2(tabId);
  return !!(t && t.st === "running" && runId && (t.runId === runId || t.historyRunId === runId));
}
...
function detachRunStreamForTab2(tabId) {
...
function pauseBackgroundRunStreamsForStatusMonitor2() {
...
function resumeBackgroundRunStreamsAfterStatusMonitor2() {
  _runStreamStateByTabId.forEach((state, tabId) => {
...
function _streamRunResponse(res, tabId, state = null) {
  if (!res.body || typeof res.body.getReader !== "function") {
    appendLine2("[server error] The server returned an invalid streaming response.", "exit-fail", tabId);
    setStatus2("fail");
    setTabStatus3(tabId, "fail");
    stopTimer();
    _setRunButtonDisabled(false);
  ...
  function read() {
    reader.read().then(({ done, value }) => {
      ...
      parts.forEach((part) => {
        try {
          const msg = _sseMessageFromChunk2(part);
      ...
      _runStreamStateByTabId.delete(tabId);
      appendLine2(`[network error] ${_describeRunnerFetchError(err, "server")}`, "exit-fail", tabId);
      if (tabId === _runnerActiveTabId()) setStatus2("fail");
      setTabStatus3(tabId, "fail");
      stopTimer();
      _setRunButtonDisabled(false);
...
function _subscribeRunStream(runId, tabId, { streamUrl = "", after = "" } = {}) {
  if (!runId || !tabId || typeof apiFetch3 !== "function") return Promise.resolve(false);
  ...
  _runStreamStateByTabId.set(tabId, streamState);
  return apiFetch3(_brokerStreamUrl(runId, tabId, streamUrl, after)).then((streamRes) => {
    if (streamState.detached) return false;
    if (!streamRes.ok) {
      _runStreamStateByTabId.delete(tabId);
      return _readRunErrorMessage2(streamRes).then((message) => {
        const suffix = message ? ` ${message}` : "";
        appendLine2(`[server error] The server could not stream the command.${suffix}`, "exit-fail", tabId);
        if (tabId === _runnerActiveTabId()) setStatus2("fail");
        setTabStatus3(tabId, "fail");
        stopTimer();
        _setRunButtonDisabled(false);
...
function _clearComposerInputs() {
  let cleared = false;
  for (const input of [cmdInput2, mobileCmdInput2]) {
    if (!input) continue;
    input.value = "";
  ...
  if (typeof setComposerValue2 === "function") setComposerValue2("", 0, 0, { allowDuringRun: true });
  if (typeof syncRunButtonDisabled2 === "function") syncRunButtonDisabled2();
  return cleared;
}
function interruptPromptLine2(tabId = _runnerActiveTabId()) {
  const t = getTab2(tabId);
  if (t && t.st === "running") return false;
  appendPromptNewline(tabId);
  _clearComposerInputs();
  refocusComposerAfterAction2();
  if (tabId === _runnerActiveTabId()) setStatus2("idle");
  return true;
}
function confirmKill2(tabId) {
  ...
  showConfirm2({
    ...
    if (result === "confirm" && targetId) doKill2(targetId);
  });
}
function doKill2(tabId) {
  const t = getTab2(tabId);
  if (!t || t.st !== "running") return;
  const secs = elapsedSeconds2();
...
function _isExactSpecialBuiltInCommand(cmd) {
  ...
  const known = Array.isArray(rawKnown) ? rawKnown : Object.values(rawKnown).flatMap((entry) => {
...
function _recordSuccessfulLocalCommand2(cmd) {
  if (typeof addToRecentPreview2 !== "function") return;
  const value = _historySafeCommand(cmd);
  if (value) addToRecentPreview2(value);
}
...
function _broadcastProjectWorkspaceChanged(cmd) {
  if (typeof emitUiEvent2 === "function") {
    emitUiEvent2("app:project-workspace-changed", { command: String(cmd || "") });
  }
  ...
  try {
    localStorage.setItem(_runnerProjectWorkspaceSyncStorageKey(), JSON.stringify({
      session_id: typeof SESSION_ID2 !== "undefined" ? SESSION_ID2 : "",
      command: String(cmd || ""),
      changed_at: Date.now()
    }));
  } catch (_) {
...
function _refreshProjectContextAfterCommand(cmd, exitCode) {
  ...
  const refreshWorkspace = typeof refreshProjectWorkspace2 === "function" && typeof isProjectWorkspaceOpen2 === "function" && isProjectWorkspaceOpen2() ? refreshProjectWorkspace2 : null;
  const refreshActive = refreshWorkspace || (typeof refreshActiveProjectContext2 === "function" ? refreshActiveProjectContext2 : null);
...
function _finalizeClientSideCommandStatus(tabId, statusValue) {
  ...
  if (tabId === _runnerActiveTabId()) setStatus2(finalStatus);
  setTabStatus3(tabId, finalStatus);
  if (typeof emitUiEvent2 === "function") emitUiEvent2("app:last-exit-changed", { value: exitCode });
}
...
async function _doSessionMigration(fromId, toId, tabId) {
  ...
  try {
    ...
    const data = await resp.json().catch(() => ({}));
    if (resp.ok && data.ok) {
      appendLine2(_sessionMigrationResultText(data), "", tabId);
      succeeded = true;
    } else {
      appendLine2(`[migration failed] ${data.error || resp.status}`, "exit-fail", tabId);
    }
  } catch (err) {
    appendLine2(`[migration failed] ${err.message || "network error"}`, "exit-fail", tabId);
    logClientError3("session-token migrate", err);
  }
  return succeeded;
}
async function _seedLocalStorageStarsToServer2() {
  let localStars;
  try {
    localStars = new Set(JSON.parse(localStorage.getItem("starred") || "[]"));
  } catch {
    localStars = /* @__PURE__ */ new Set();
  }
  if (!localStars.size) {
  ...
  const results = await Promise.allSettled(cmds.map(async (cmd) => {
    const resp = await apiFetch3("/session/starred", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ command: cmd })
    });
  ...
  if (typeof loadStarredFromServer2 === "function") await loadStarredFromServer2();
}
function _setPendingTerminalConfirm(config) {
  _pendingTerminalConfirm = config || null;
  if (typeof setComposerPromptMode2 === "function") {
    setComposerPromptMode2(_pendingTerminalConfirm ? "confirm" : null);
  }
}
function hasPendingTerminalConfirm2() {
  return !!_pendingTerminalConfirm;
}
async function _runPendingTerminalConfirmHandler(promptTabId, handler) {
  const originalSetStatus = setStatus2;
  let finalStatus = "idle";
  try {
    setStatus2 = (statusValue) => {
      finalStatus = statusValue;
      originalSetStatus(statusValue);
    };
    await Promise.resolve(typeof handler === "function" ? handler() : void 0);
  } finally {
    setStatus2 = originalSetStatus;
  }
  _finalizeClientSideCommandStatus(promptTabId, finalStatus);
}
function cancelPendingTerminalConfirm2(tabId = _runnerActiveTabId()) {
  ...
  const cancelHandler = typeof pending.onCancel === "function" ? pending.onCancel : typeof pending.onNo === "function" ? pending.onNo : null;
  if (pending.kind === "text") {
    Promise.resolve(typeof cancelHandler === "function" ? cancelHandler() : void 0).catch((err) => {
      appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", promptTabId);
      setStatus2("fail");
    });
    refocusComposerAfterAction2();
    return true;
...
function _appendSessionTokenSetLines(token, tabId) {
  appendLine2(`session token set: ${maskSessionToken3(token)}`, "", tabId);
  appendLine2("reload other tabs to apply the new session token", "", tabId);
}
function _clearVisibleSessionHistoryState() {
  if (typeof hydrateCmdHistory2 === "function") hydrateCmdHistory2([]);
}
async function _activateSessionTokenIdentity(token) {
  localStorage.setItem("session_token", token);
  updateSessionId2(token);
  if (typeof loadRecentValues2 === "function") await _runnerIgnoreFailure(loadRecentValues2());
  await _seedLocalStorageStarsToServer2();
  if (typeof reloadSessionHistory2 === "function") await _runnerIgnoreFailure(reloadSessionHistory2());
  if (typeof refreshWorkspaceFiles2 === "function") void _runnerIgnoreFailure(refreshWorkspaceFiles2());
  else _runnerWorkspaceCacheApi().refresh?.()?.catch?.(() => {
  });
  if (typeof reloadWorkflowCatalog2 === "function") void _runnerIgnoreFailure(reloadWorkflowCatalog2());
}
async function _sessionTokenGenerate(tabId) {
  const oldSessionId = _runnerCurrentSessionId();
  try {
    const resp = await apiFetch3("/session/token/generate");
    if (!resp.ok) {
      const data2 = await resp.json().catch(() => ({}));
      appendLine2(`[error] Failed to generate session token — ${data2.error || resp.status}`, "exit-fail", tabId);
      setStatus2("fail");
      return;
    }
    const data = await resp.json();
    const newToken = data.session_token;
    if (typeof flushRecentValues2 === "function") {
      await _runnerIgnoreFailure(flushRecentValues2());
    }
    let runCount = 0;
    ...
    try {
      const countResp = await apiFetch3("/session/run-count");
      if (countResp.ok) {
    ...
    appendLine2(`session token generated:  ${maskSessionToken3(newToken)}`, "", tabId);
    appendLine2("stored in localStorage as session_token", "", tabId);
    appendLine2("use session-token set <value> on another device to continue your session", "", tabId);
    appendLine2("warning: your session token grants full access to your session history — treat it like a password", "notice", tabId);
    if (runCount > 0 || workspaceFileCount > 0 || workflowCount > 0 || recentValueCount > 0) {
      ...
      _setPendingTerminalConfirm({
        ...
        onYes: async () => {
          ...
          if (migrated) {
            ...
            updateSessionId2(newToken);
            await _seedLocalStorageStarsToServer2();
            if (typeof reloadSessionHistory2 === "function") await _runnerIgnoreFailure(reloadSessionHistory2());
            if (typeof reloadWorkflowCatalog2 === "function") void _runnerIgnoreFailure(reloadWorkflowCatalog2());
            _recordSuccessfulLocalCommand2("session-token generate");
            _persistSessionTokenRun("session-token generate", [
              { text: `session token generated:  ${maskSessionToken3(newToken)}` },
        ...
        onNo: async () => {
          ...
          if (typeof reloadWorkflowCatalog2 === "function") void _runnerIgnoreFailure(reloadWorkflowCatalog2());
          _recordSuccessfulLocalCommand2("session-token generate");
          appendLine2("History, file, workflow, and recent-value migration skipped.", "", tabId);
          _persistSessionTokenRun("session-token generate", [
            { text: `session token generated:  ${maskSessionToken3(newToken)}` },
      ...
      updateSessionId2(newToken);
      await _seedLocalStorageStarsToServer2();
      if (typeof reloadSessionHistory2 === "function") reloadSessionHistory2().catch(() => {
      });
      _recordSuccessfulLocalCommand2("session-token generate");
      _persistSessionTokenRun("session-token generate", [
        { text: `session token generated:  ${maskSessionToken3(newToken)}` },
    ...
    appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", tabId);
    logClientError3("session-token generate", err);
    setStatus2("fail");
  }
}
async function _sessionTokenSet(value, tabId) {
  if (!value) {
    appendLine2("usage: session-token set <token>", "", tabId);
    setStatus2("fail");
    return;
  }
  ...
  if (!isTok && !isUuid) {
    appendLine2(`[error] invalid session token format — expected tok_... or a UUID`, "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  if (isTok) {
    let verifyErr = null;
    try {
      const vResp = await apiFetch3("/session/token/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (verifyErr !== null) {
      appendLine2(`[error] ${verifyErr}`, "exit-fail", tabId);
      setStatus2("fail");
      return;
    }
  }
    ...
    _setPendingTerminalConfirm({
      ...
      onYes: async () => {
        ...
        if (migrated) {
          ...
          _recordSuccessfulLocalCommand2(`session-token set ${value}`);
          _persistSessionTokenRun(`session-token set ${value}`, [
            { text: `session token set: ${maskSessionToken3(value)}` },
      ...
      onCancel: async () => {
        appendLine2("Session token set canceled.", "", tabId);
        setStatus2("idle");
      }
    });
    setStatus2("idle");
...
async function _sessionTokenCopy(tabId) {
  ...
  if (!token) {
    appendLine2("no session token is set — already using an anonymous session", "", tabId);
    setStatus2("idle");
    return;
  }
  try {
    await _runnerCopyTextToClipboardAdapter(token);
    appendLine2(`session token copied to clipboard: ${maskSessionToken3(token)}`, "", tabId);
    _recordSuccessfulLocalCommand2("session-token copy");
    _persistSessionTokenRun("session-token copy", [
      { text: `session token copied to clipboard: ${maskSessionToken3(token)}` }
    ]);
    setStatus2("ok");
  } catch (err) {
    appendLine2("[error] failed to copy the session token to clipboard", "exit-fail", tabId);
    logClientError3("session-token copy", err);
    setStatus2("fail");
  }
}
async function _sessionTokenClear(tabId) {
  ...
  appendLine2("warning: clearing the active session token removes it from this browser", "notice", tabId);
  appendLine2("run 'session-token copy' first if you want to save the current token before clearing it", "notice", tabId);
  appendLine2("clear the active session token and revert to an anonymous session?", "", tabId);
  _setPendingTerminalConfirm({
    tabId,
    onYes: async () => {
      localStorage.removeItem("session_token");
      const uuid = localStorage.getItem("session_id") || _runnerCurrentSessionId();
      updateSessionId2(uuid);
      _clearVisibleSessionHistoryState();
      if (typeof reloadSessionHistory2 === "function") await _runnerIgnoreFailure(reloadSessionHistory2());
      if (typeof reloadWorkflowCatalog2 === "function") void _runnerIgnoreFailure(reloadWorkflowCatalog2());
      appendLine2(`session token cleared — reverted to anonymous session (${maskSessionToken3(uuid)})`, "", tabId);
      appendLine2("your session token data remains in the server database", "", tabId);
      _recordSuccessfulLocalCommand2("session-token clear");
      _persistSessionTokenRun("session-token clear", [
        { text: `session token cleared — reverted to anonymous session (${maskSessionToken3(uuid)})` },
        { text: "your session token data remains in the server database" }
      ]);
      setStatus2("ok");
    },
    onNo: async () => {
      appendLine2("Session token clear canceled.", "", tabId);
      setStatus2("idle");
    }
  });
  setStatus2("idle");
...
async function _sessionTokenRotate(tabId) {
  ...
  try {
    ...
    if (!migrateResp.ok || !migrateData.ok) {
      appendLine2(`[error] migration failed — session token NOT rotated: ${migrateData.error || migrateResp.status}`, "exit-fail", tabId);
      appendLine2("your previous session token is still active", "", tabId);
      setStatus2("fail");
      return;
    }
    localStorage.setItem("session_token", newToken);
    ...
    if (typeof reloadWorkflowCatalog2 === "function") void _runnerIgnoreFailure(reloadWorkflowCatalog2());
    appendLine2(`session token rotated: ${maskSessionToken3(newToken)}`, "", tabId);
    appendLine2(_sessionMigrationResultText(migrateData), "", tabId);
    appendLine2("old session token is now inactive — reload other tabs to use the new token", "", tabId);
    _recordSuccessfulLocalCommand2("session-token rotate");
    _persistSessionTokenRun("session-token rotate", [
      { text: `session token rotated: ${maskSessionToken3(newToken)}` },
    ...
    appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", tabId);
    logClientError3("session-token rotate", err);
    setStatus2("fail");
  }
}
async function _sessionTokenList(tabId) {
  try {
    const resp = await apiFetch3("/session/token/info");
    if (!resp.ok) {
      appendLine2("[error] failed to load session token info", "exit-fail", tabId);
      setStatus2("fail");
      return;
    }
    const data = await resp.json();
    ...
    if (data.token) {
      appendLine2(kv("session token", maskSessionToken3(data.token)), "builtin-kv", tabId);
      appendLine2(kv("status", "active"), "builtin-kv", tabId);
      if (data.created) appendLine2(kv("created", data.created + " UTC"), "builtin-kv", tabId);
      appendLine2(kv("storage", "localStorage (session_token)"), "builtin-kv", tabId);
    } else {
      appendLine2(kv("session", maskSessionToken3(_runnerCurrentSessionId())), "builtin-kv", tabId);
      appendLine2(kv("status", "anonymous (no session token set)"), "builtin-kv", tabId);
      appendLine2(kv("tip", "run 'session-token generate' to create a persistent token"), "builtin-kv", tabId);
    }
    _recordSuccessfulLocalCommand2("session-token list");
    _persistSessionTokenRun("session-token list", [
      { text: data.token ? `session token  ${maskSessionToken3(data.token)}` : `session  ${maskSessionToken3(_runnerCurrentSessionId())}`, cls: "builtin-kv" },
      { text: data.token ? "status          active" : "status          anonymous (no session token set)", cls: "builtin-kv" }
    ]);
    setStatus2("ok");
  } catch (err) {
    appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", tabId);
    logClientError3("session-token list", err);
    setStatus2("fail");
  }
}
async function _sessionTokenRevoke(token, tabId) {
  if (!token) {
    appendLine2("usage: session-token revoke <token>", "", tabId);
    setStatus2("fail");
    return;
  }
  if (!token.startsWith("tok_")) {
    appendLine2("[error] only tok_ tokens can be revoked", "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  appendLine2(`revoke session token ${maskSessionToken3(token)}?`, "", tabId);
  appendLine2(
  ...
  _setPendingTerminalConfirm({
    ...
    onNo: async () => {
      appendLine2("Session token revoke canceled.", "", tabId);
      setStatus2("idle");
    },
    onCancel: async () => {
      appendLine2("Session token revoke canceled.", "", tabId);
...
async function _sessionTokenRevokeConfirmed(token, tabId) {
  try {
    const resp = await apiFetch3("/session/token/revoke", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const data = await resp.json().catch(() => ({}));
    if (!resp.ok) {
      appendLine2(`[error] ${data.error || resp.status}`, "exit-fail", tabId);
      setStatus2("fail");
      return;
    }
    const isCurrentToken = token === _runnerCurrentSessionId();
    appendLine2(`session token revoked: ${maskSessionToken3(token)}`, "", tabId);
    if (isCurrentToken) {
      localStorage.removeItem("session_token");
      const uuid = localStorage.getItem("session_id") || _runnerCurrentSessionId();
      updateSessionId2(uuid);
      ...
      appendLine2(`reverted to anonymous session (${maskSessionToken3(uuid)})`, "", tabId);
    } else {
      appendLine2("token removed from server — any device using it is now on an empty anonymous session", "", tabId);
    }
    _recordSuccessfulLocalCommand2(`session-token revoke ${token}`);
    _persistSessionTokenRun(`session-token revoke ${token}`, [
      { text: `session token revoked: ${maskSessionToken3(token)}` }
    ]);
    setStatus2("ok");
  } catch (err) {
    appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", tabId);
    logClientError3("session-token revoke", err);
    setStatus2("fail");
  }
}
...
function _workspaceFormatBytes(size) {
  const formatted = typeof _formatWorkspaceBytes2 === "function" ? _formatWorkspaceBytes2(size) : null;
...
function _workspaceDirectListEntries(entries, base = "") {
  ...
  const addDirectFolder = (path, fallbackName = "") => {
    const relative = _workspaceRelativeListName(path, normalizedBase, false) || String(fallbackName || "").replace(/\/+$/, "");
    const parts = relative.split("/").filter(Boolean);
...
function _workspaceRecursiveEntries(base = "") {
  ...
  while (queue.length) {
    ...
    const entries = typeof getDirectoryEntries === "function" ? getDirectoryEntries(current) : { folders: [], files: [] };
    (entries.folders || []).forEach((folder) => {
...
async function _workspaceReadLines(path) {
  const data = await readWorkspaceFile2(path);
...
async function _runWorkspaceListCommand(parts, tabId) {
  ...
  if (_runnerWorkspacePathHasGlobAdapter(rawTarget)) {
    ...
    const entries2 = {
      folders: matches.filter((item) => item.kind === "directory").map((item) => ({
        path: item.path,
        name: _workspaceRelativeListName(item.path, _runnerWorkspaceCwdAdapter(tabId), true).replace(/\/+$/, "")
      })),
      files: matches.filter((item) => item.kind === "file").map((item) => ({
        ...item.item || {},
        path: item.path,
        name: _workspaceRelativeListName(item.path, _runnerWorkspaceCwdAdapter(tabId), false)
      }))
    };
    return parsed.long ? _workspaceListLines(entries2, "") : _workspaceShortListLines(entries2);
  }
  ...
  const getDirectoryEntries = _runnerWorkspaceCacheApi().getDirectoryEntries;
  const entries = typeof getDirectoryEntries === "function" ? getDirectoryEntries(target) : { folders: [], files: [] };
  const isRoot = !target;
...
async function _handleWorkspaceTerminalCommand(cmd, tabId) {
  ...
  try {
    ...
    if (root === "pwd") {
      ...
      const data = await createWorkspaceDirectory2(target);
    ...
    outputLines.forEach((line) => appendLine2(line.text, line.cls || "", tabId));
    _recordSuccessfulLocalCommand2(cmd);
    _persistClientSideRun(cmd, outputLines, "ok");
    setStatus2("ok");
  } catch (err) {
    appendLine2(`[error] ${err.message || "workspace command failed"}`, "exit-fail", tabId);
    logClientError3("workspace terminal command", err);
    setStatus2("fail");
  }
}
async function _handleSessionTokenCommand(cmd, tabId) {
  ...
  if (sub === "generate") {
    ...
    appendLine2(`session-token: unknown subcommand '${sub}'`, "exit-fail", tabId);
    appendLine2("usage: session-token [generate | copy | set <value> | clear | rotate | list | revoke <token>]", "", tabId);
    setStatus2("fail");
  }
}
async function _handleWorkspaceDeleteCommand(cmd, tabId) {
  ...
  if (!_workspaceTerminalCanWrite("delete Files")) {
    appendLine2(`[error] ${_workspaceTerminalDeniedMessage("delete Files")}`, "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  if (!target) {
    appendLine2(_workspaceDeleteUsageForCommand(parsedDelete), "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  let targetInfo = null;
  ...
  try {
    await _runnerEnsureWorkspaceCacheAdapter();
    const expandedTargets = _runnerWorkspacePathHasGlobAdapter(target) ? _runnerWorkspaceExpandPathPatternAdapter(target, { cwd: _runnerWorkspaceCwdAdapter(tabId), kind: "any" }) : [{ path: _runnerResolveExistingWorkspaceCommandPathAdapter(target, { cwd: _runnerWorkspaceCwdAdapter(tabId), kind: "any" }) }];
    ...
    for (const expandedTarget of expandedTargets) {
      ...
      const existsResp = await apiFetch3(`/workspace/files/info?path=${encodeURIComponent(path)}`);
      if (!existsResp.ok) {
    ...
    appendLine2(`[error] ${err.message || "file or folder was not found"}`, "exit-fail", tabId);
    logClientError3("file rm validate", err);
    setStatus2("fail");
    return;
  }
  ...
  if (directoryInfos.length && !(parsedDelete && parsedDelete.recursive)) {
    ...
    appendLine2(`[error] ${matchedText}; use rm -r ${target} or file delete -r ${target}`, "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  appendLine2(
    isMultiDelete ? `delete ${targetInfos.length} matched session items for '${target}'?` : `delete session ${isDirectory ? "folder" : "file"} '${target}'?`,
    "",
    tabId
  );
  if (directoryInfos.length && fileCount > 0) {
    appendLine2(`warning: this will also delete ${fileCount} ${fileCount === 1 ? "file" : "files"} in this folder.`, "warning", tabId);
  }
  _setPendingTerminalConfirm({
    tabId,
    onYes: async () => {
      try {
        ...
        for (const info of targetInfos) {
          ...
          const resp = await apiFetch3(`/workspace/files?path=${encodeURIComponent(path)}`, { method: "DELETE" });
          if (!resp.ok) {
            const data = await resp.json().catch(() => ({}));
        ...
        appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", tabId);
        logClientError3("file rm", err);
        setStatus2("fail");
      }
    },
    onNo: async () => {
      appendLine2(`Session ${isMultiDelete ? "items" : isDirectory ? "folder" : "file"} delete canceled.`, "", tabId);
      setStatus2("idle");
    }
  });
  setStatus2("idle");
...
async function _handleWorkspaceMoveCommand(cmd, tabId) {
  ...
  if (!_workspaceTerminalCanWrite("move Files")) {
    appendLine2(`[error] ${_workspaceTerminalDeniedMessage("move Files")}`, "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  if (!parsed || parsed.invalid || !parsed.source || !parsed.destination) {
    appendLine2(parsed?.usage || "Usage: file move <source> <destination>", "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  if (typeof moveWorkspacePath2 !== "function") {
    appendLine2("[error] Files move is not ready — reload the page and try again", "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  try {
    await _runnerEnsureWorkspaceCacheAdapter();
    const sources = _runnerWorkspacePathHasGlobAdapter(parsed.source) ? _runnerWorkspaceExpandPathPatternAdapter(parsed.source, { cwd: _runnerWorkspaceCwdAdapter(tabId), kind: "any" }) : [{ path: _runnerResolveExistingWorkspaceCommandPathAdapter(parsed.source, { cwd: _runnerWorkspaceCwdAdapter(tabId), kind: "any" }) }];
    if (!sources.length) {
    ...
    for (const sourceEntry of sources) {
      ...
      const data = await moveWorkspacePath2(source, destination);
    ...
    appendLine2(`[error] ${err.message || "file move failed"}`, "exit-fail", tabId);
    logClientError3("file move", err);
    setStatus2("fail");
  }
}
async function _handleWorkspaceEditorCommand(cmd, tabId) {
  ...
  if (!_workspaceTerminalCanWrite(writeAction)) {
    appendLine2(`[error] ${_workspaceTerminalDeniedMessage(writeAction)}`, "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  if (!parsed || parsed.invalid || parsed.action === "edit" && !parsed.target) {
    ...
    appendLine2(`Usage: file ${action} ${operand}`, "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  if (typeof openWorkspaceEditorFromCommand2 !== "function") {
    appendLine2("[error] Files panel is not ready — reload the page and try again", "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  try {
    if (parsed.action === "edit") await _runnerEnsureWorkspaceCacheAdapter();
    const target = parsed.target ? parsed.action === "edit" ? _runnerResolveExistingWorkspaceCommandPathAdapter(parsed.target, { cwd: _runnerWorkspaceCwdAdapter(tabId), kind: "file" }) : _runnerResolveWorkspaceCommandPathAdapter(parsed.target, { cwd: _runnerWorkspaceCwdAdapter(tabId) }) : "";
    const opened = await openWorkspaceEditorFromCommand2(parsed.action, target);
    if (opened === false) throw new Error(_workspaceTerminalDeniedMessage(writeAction));
    const targetLabel = target ? ` ${target}` : "";
    appendLine2(`file: opened${targetLabel} in the file editor`, "", tabId);
    _recordSuccessfulLocalCommand2(cmd);
    _persistClientSideRun(cmd, [{ text: `file: opened${targetLabel} in the file editor` }], "ok");
    setStatus2("ok");
  } catch (err) {
    appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", tabId);
    logClientError3(`file ${parsed.action}`, err);
    setStatus2("fail");
  }
}
async function _handleWorkspaceDownloadCommand(cmd, tabId) {
  ...
  appendCommandEcho2(cmd);
  if (!target) {
    appendLine2("Usage: file download <file>", "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  if (typeof downloadWorkspaceFile2 !== "function") {
    appendLine2("[error] Files download is not ready — reload the page and try again", "exit-fail", tabId);
    setStatus2("fail");
    return;
  }
  try {
    ...
    const downloaded = await downloadWorkspaceFile2(target);
    ...
    appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", tabId);
    logClientError3("file download", err);
    setStatus2("fail");
  }
}
async function _runClientSideCommandWithOptionalPipe(cmd, tabId, runBaseCommand) {
  ...
  const capturedLines = [];
  const originalAppendLine = appendLine2;
  const originalAppendCommandEcho = appendCommandEcho2;
  const originalRecordSuccessfulLocalCommand = _recordSuccessfulLocalCommand2;
  ...
  if (typeof setTabStatus3 === "function") setTabStatus3(tabId, "running");
  appendCommandEcho2(cmd, tabId);
  try {
    _recordSuccessfulLocalCommand2 = () => {
    };
    ...
    appendCommandEcho2 = (echoCommand, echoTabId = null) => {
    ...
    appendLine2 = (text, cls = "", lineTabId = null, metadata = null) => {
    ...
    appendLine2 = originalAppendLine;
    appendCommandEcho2 = originalAppendCommandEcho;
    _recordSuccessfulLocalCommand2 = originalRecordSuccessfulLocalCommand;
  ...
  if (!_pendingTerminalConfirm) {
    ...
    if (finalStatus !== "fail") _recordSuccessfulLocalCommand2(cmd);
    _persistClientSideRun(cmd, outputLines, finalStatus, tabId);
  } else if (typeof setTabStatus3 === "function") {
    setTabStatus3(tabId, finalStatus === "fail" ? "fail" : "idle");
  }
}
function submitCommand2(rawCmd) {
  ...
  if (!cmd) {
    ...
    const _activeTab = getActiveTab2();
  ...
  if (_pendingTerminalConfirm) {
    ...
    appendCommandEcho2(cmd, promptTabId);
    if (pending.kind === "text") {
      _setPendingTerminalConfirm(null);
      Promise.resolve(typeof pending.onAnswer === "function" ? pending.onAnswer(cmd) : void 0).catch((err) => {
        appendLine2(`[error] ${err.message || "network error"}`, "exit-fail", promptTabId);
        setStatus2("fail");
      });
      return true;
    ...
    if (answer !== "yes" && answer !== "y" && answer !== "no" && answer !== "n") {
      appendLine2("please answer yes or no", "notice", promptTabId);
      return true;
    }
    _setPendingTerminalConfirm(null);
  ...
  if (_welcomeOwns(_runnerActiveTabId())) {
    ...
    clearTab3(_runnerActiveTabId());
  }
  const activeTab = getActiveTab2();
  if (activeTab && activeTab.st === "running") {
    const newId = createTab3(typeof createDefaultTabLabel3 === "function" ? createDefaultTabLabel3() : "shell " + (_runnerTabs().length + 1));
    if (!newId) return false;
  }
  const shellOps = /&&|\|\|?|;;?|`|\$\(|>>?|</;
  if (shellOps.test(cmd) && !_isSyntheticPostFilterCommand(cmd) && !_isExactSpecialBuiltInCommand(cmd)) {
    appendCommandEcho2(cmd);
    appendLine2("[denied] Shell operators (&&, |, ;, >, etc.) are not permitted.", "denied");
    setStatus2("fail");
    setLastExit(1);
    setTabStatus3(_runnerActiveTabId(), "fail");
    return false;
  }
  if (/(?<![\w:\/])\/data\b/.test(cmd) || /(?<![\w:\/])\/tmp\b/.test(cmd)) {
    appendCommandEcho2(cmd);
    appendLine2("[denied] Access to /data and /tmp is not permitted.", "denied");
    setStatus2("fail");
    setLastExit(1);
    setTabStatus3(_runnerActiveTabId(), "fail");
    return false;
  ...
  if (_isTabCloseCommand(cmd)) {
    const closeActiveTab = typeof closeTab === "function" ? closeTab : typeof closeTab2 === "function" ? closeTab2 : null;
    if (closeActiveTab) closeActiveTab(_runnerActiveTabId());
    return true;
  }
  addToHistory2(_historySafeCommand(cmd));
  if (typeof rememberRecentValuesFromCommand2 === "function") {
    try {
      rememberRecentValuesFromCommand2(cmd);
    } catch (_) {
    }
  }
  ...
  if (_isSessionTokenSubcommand(cmd)) {
    void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _handleSessionTokenCommand(baseCommand, _runnerActiveTabId()));
    return true;
  }
  if (_isClientSideSecretSetCommand(cmd)) {
    ...
    void _runClientSideCommandWithOptionalPipe(safeCommand, _runnerActiveTabId(), (baseCommand) => {
      ...
      appendLine2("[error] secret prompt is not ready — reload the page and try again", "exit-fail", _runnerActiveTabId());
      setStatus2("fail");
      return true;
    });
    return true;
  ...
  if (_runnerIsWorkspaceTerminalCommandAdapter(cmd)) {
    void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _handleWorkspaceTerminalCommand(baseCommand, _runnerActiveTabId()));
    return true;
  }
  if (_runnerIsWorkspaceDeleteCommandAdapter(cmd)) {
    void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _handleWorkspaceDeleteCommand(baseCommand, _runnerActiveTabId()));
    return true;
  }
  if (_runnerIsWorkspaceMoveCommandAdapter(cmd)) {
    void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _handleWorkspaceMoveCommand(baseCommand, _runnerActiveTabId()));
    return true;
  }
  if (_runnerIsWorkspaceEditorCommandAdapter(cmd)) {
    void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _handleWorkspaceEditorCommand(baseCommand, _runnerActiveTabId()));
    return true;
  }
  if (_runnerIsWorkspaceDownloadCommandAdapter(cmd)) {
    void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _handleWorkspaceDownloadCommand(baseCommand, _runnerActiveTabId()));
    return true;
  }
  if (String(cmd || "").trim().toLowerCase().split(/\s+/, 1)[0] === "workflow") {
    ...
    appendCommandEcho2(cmd);
    appendLine2("[error] workflow command is not ready — reload the page and try again", "exit-fail", _runnerActiveTabId());
    setStatus2("fail");
    return true;
  }
  if (_isClientSideUiCommand(cmd)) {
    ...
    if (root === "theme" && typeof _runnerHandleThemeCommandAdapter === "function") {
      void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _runnerHandleThemeCommandAdapter(baseCommand, _runnerActiveTabId()));
      return true;
    }
    if (root === "config" && typeof _runnerHandleConfigCommandAdapter === "function") {
      void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _runnerHandleConfigCommandAdapter(baseCommand, _runnerActiveTabId()));
      return true;
    }
    if (root === "tour" && typeof _runnerHandleTourCommandAdapter === "function") {
      void _runClientSideCommandWithOptionalPipe(cmd, _runnerActiveTabId(), (baseCommand) => _runnerHandleTourCommandAdapter(baseCommand, _runnerActiveTabId()));
      return true;
    }
    appendCommandEcho2(cmd);
    appendLine2(`[error] ${root} command is not ready — reload the page and try again`, "exit-fail", _runnerActiveTabId());
    setStatus2("fail");
    return true;
  }
  if (!_runActiveTeamScopeCan("run_commands")) {
    appendCommandEcho2(cmd);
    appendLine2(`[denied] ${_runStartDeniedMessage()}`, "denied", _runnerActiveTabId());
    setStatus2("fail");
    setTabStatus3(_runnerActiveTabId(), "fail");
    return true;
  }
  const _runTab = getActiveTab2();
  if (typeof setTabRunningCommand2 === "function") {
    setTabRunningCommand2(_runnerActiveTabId(), cmd);
  } else {
    if (!_runTab || !_runTab.renamed) setTabLabel3(_runnerActiveTabId(), cmd);
  ...
  setStatus2("running");
  setTabStatus3(_runnerActiveTabId(), "running");
  _setRunButtonDisabled(true);
  showTabKillBtn2(_runnerActiveTabId());
  startTimer();
  ...
  apiFetch3("/runs", {
    ...
    if (res.status === 403) {
      return res.json().then((data) => {
        const message = data && data.error === "team_forbidden" ? _runStartDeniedMessage() : data.error || data.message || "Command not allowed.";
        appendLine2("[denied] " + message, "denied", tabId);
        setStatus2("fail");
        setTabStatus3(tabId, "fail");
        stopTimer();
        _setRunButtonDisabled(false);
    ...
    if (res.status === 429) {
      appendLine2("[rate limited] Too many requests. Please wait a moment.", "denied", tabId);
      setStatus2("fail");
      setTabStatus3(tabId, "fail");
      stopTimer();
      _setRunButtonDisabled(false);
    ...
    if (!res.ok) {
      return _readRunErrorMessage2(res).then((message) => {
        const suffix = message ? ` ${message}` : "";
        appendLine2(`[server error] The server could not start the command.${suffix}`, "exit-fail", tabId);
        setStatus2("fail");
        setTabStatus3(tabId, "fail");
        stopTimer();
        _setRunButtonDisabled(false);
    ...
    return res.json().then((data) => {
      ...
      if (!runId) {
        appendLine2("[server error] The server did not return a run id.", "exit-fail", tabId);
        setStatus2("fail");
        setTabStatus3(tabId, "fail");
        stopTimer();
        _setRunButtonDisabled(false);
...
function submitComposerCommand2(rawCmd, { dismissKeyboard = false, focusAfterSubmit = true } = {}) {
  const result = submitCommand2(rawCmd);
  if (result === true) {
    _clearComposerInputs();
    if (focusAfterSubmit) refocusComposerAfterAction2();
    if (dismissKeyboard && typeof dismissMobileKeyboardAfterSubmit2 === "function") {
      dismissMobileKeyboardAfterSubmit2();
    }
...
function submitVisibleComposerCommand2({ rawCmd = null, dismissKeyboard = false, focusAfterSubmit = true } = {}) {
  const value = typeof rawCmd === "string" ? rawCmd : typeof getComposerValue2 === "function" ? getComposerValue2() : "";
  return submitComposerCommand2(value, { dismissKeyboard, focusAfterSubmit });
}
function runCommand2() {
  if (typeof isRunButtonDisabled === "function" && isRunButtonDisabled()) return;
  const value = typeof getComposerValue2 === "function" ? getComposerValue2() : cmdInput2 ? cmdInput2.value : "";
  submitComposerCommand2(value, { dismissKeyboard: true });
}
if (typeof setRunnerHandlers === "function") {
  setRunnerHandlers({
    _readRunErrorMessage: _readRunErrorMessage2,
    _recordSuccessfulLocalCommand: _recordSuccessfulLocalCommand2,
    _seedLocalStorageStarsToServer: _seedLocalStorageStarsToServer2,
    _sseMessageFromChunk: _sseMessageFromChunk2,
    appendCommandEcho: appendCommandEcho2,
    attachActiveRunFromMonitor: attachActiveRunFromMonitor2,
    cancelPendingTerminalConfirm: cancelPendingTerminalConfirm2,
    confirmKill: confirmKill2,
    detachRunStreamForTab: detachRunStreamForTab2,
    doKill: doKill2,
    hasPendingTerminalConfirm: hasPendingTerminalConfirm2,
    interruptPromptLine: interruptPromptLine2,
    killActiveRunFromMonitor: killActiveRunFromMonitor2,
    pauseBackgroundRunStreamsForStatusMonitor: pauseBackgroundRunStreamsForStatusMonitor2,
    resumeBackgroundRunStreamsAfterStatusMonitor: resumeBackgroundRunStreamsAfterStatusMonitor2,
    runCommand: runCommand2,
    setStatus: setStatus2,
    submitCommand: submitCommand2,
...
function _welcomeGetOutput(tabId) {
  if (typeof getOutput2 === "function") return getOutput2(tabId);
  const getOutput3 = _welcomeGlobalFunction("getOutput");
  return getOutput3 ? getOutput3(tabId) : null;
}
function _welcomeMountShellPrompt(tabId, force = false) {
  const mountShellPrompt3 = typeof mountShellPrompt2 === "function" && mountShellPrompt2 || _welcomeGlobalFunction("mountShellPrompt");
  if (!mountShellPrompt3) return;
  if (force) {
    mountShellPrompt3(tabId, true);
    return;
  }
  mountShellPrompt3(tabId);
}
function _welcomeUnmountShellPrompt(tabId) {
  const unmountShellPrompt3 = typeof unmountShellPrompt2 === "function" && unmountShellPrompt2 || _welcomeGlobalFunction("unmountShellPrompt");
  if (unmountShellPrompt3) unmountShellPrompt3(tabId);
}
function _welcomeRefocusComposer(...args) {
  const refocus = typeof refocusComposerAfterAction === "function" && refocusComposerAfterAction || _welcomeGlobalFunction("refocusComposerAfterAction");
  if (refocus) refocus(...args);
}
function _welcomeAppendPromptNewline(tabId) {
  const append = typeof appendPromptNewline === "function" && appendPromptNewline || _welcomeGlobalFunction("appendPromptNewline");
  if (append) append(tabId);
}
function _welcomeSetComposerValue(value) {
  const setComposerValue3 = typeof setComposerValue === "function" && setComposerValue || _welcomeGlobalFunction("setComposerValue");
  if (setComposerValue3) setComposerValue3(value);
}
function _welcomeBindPressable(el, opts) {
  const bind = typeof bindPressable === "function" && bindPressable || _welcomeGlobalFunction("bindPressable");
  return bind ? bind(el, opts) : null;
}
function _welcomeApiFetch(...args) {
  const apiFetch4 = (typeof hasRuntimeHandler === "function" && hasRuntimeHandler("apiFetch") && typeof apiFetch === "function" ? apiFetch : null) || _welcomeGlobalFunction("apiFetch");
  if (!apiFetch4) throw new Error("apiFetch is unavailable");
  return apiFetch4(...args);
}
function _welcomeLogClientError(context, err, details = null) {
  const logClientError4 = (typeof hasRuntimeHandler === "function" && hasRuntimeHandler("logClientError") && typeof logClientError === "function" ? logClientError : null) || _welcomeGlobalFunction("logClientError");
  if (logClientError4) logClientError4(context, err, details);
}
function _welcomeCmdInput() {
  return typeof cmdInput !== "undefined" && cmdInput || _welcomeGlobalValue("cmdInput") || null;
}
function _welcomeShellPromptWrap() {
  return typeof shellPromptWrap !== "undefined" && shellPromptWrap || _welcomeGlobalValue("shellPromptWrap") || null;
}
var _welcomeActive = false;
var _welcomeDone = false;
var _welcomeTabId = null;
var _welcomeBanner = null;
var _welcomeLiveLine = null;
var _welcomeHintNode = null;
var _welcomeStatusNodes = [];
var _welcomePlan = null;
var _welcomeNextBlockIndex = 0;
var _welcomeSettleRequested = false;
var _welcomePromptAfterSettle = false;
var _welcomeBootPending = true;
var _welcomeWaiters = /* @__PURE__ */ new Set();
var _welcomePrompt = typeof buildPromptLabel2 === "function" ? buildPromptLabel2() : "anon@darklab.sh:~ $";
var _welcomeGroupOrder = ["basics", "dns", "web", "recon", "advanced"];
var _welcomeStatusFrames = ["initializing /", "initializing -", "initializing \\", "initializing |"];
var _welcomeStatusPendingText = "initializing...";
var _welcomeStatusReadyText = "initialized";
function _getWelcomeIntroMode() {
  const readPreference = typeof getWelcomeIntroPreference === "function" && getWelcomeIntroPreference || _welcomeGlobalFunction("getWelcomeIntroPreference");
  if (readPreference) return readPreference();
  return "animated";
}
function _getEffectiveWelcomeStatusLabels() {
  const config = _welcomeAppConfig();
  const statusLabels = Array.isArray(config.welcome_status_labels) ? config.welcome_status_labels.map((label) => String(label || "").trim().toUpperCase()).filter(Boolean).slice(0, 6) : [];
  return statusLabels.length ? statusLabels : ["CONFIG", "RUNNER", "HISTORY", "LIMITS", "AUTOCOMPLETE"];
}
function _shouldUseMobileWelcomeSequence() {
  const useMobile = typeof useMobileTerminalViewportMode2 === "function" && useMobileTerminalViewportMode2 || _welcomeGlobalFunction("useMobileTerminalViewportMode");
...
function _welcomeTourSeenVersion() {
  const readPreference = typeof getTourSeenVersionPreference === "function" && getTourSeenVersionPreference || _welcomeGlobalFunction("getTourSeenVersionPreference");
...
function _writeWelcomeState(next = {}) {
  const setWelcomeState2 = typeof setWelcomeState === "function" && setWelcomeState || _welcomeGlobalFunction("setWelcomeState");
  if (setWelcomeState2) setWelcomeState2(next);
  ...
  const getWelcomeState2 = typeof getWelcomeState === "function" && getWelcomeState || _welcomeGlobalFunction("getWelcomeState");
  return getWelcomeState2 ? getWelcomeState2() : {
...
function _readWelcomeState() {
  const getWelcomeState2 = typeof getWelcomeState === "function" && getWelcomeState || _welcomeGlobalFunction("getWelcomeState");
  const apiState = getWelcomeState2 ? getWelcomeState2() : null;
  return {
...
function welcomeOwnsTab2(tabId) {
...
function _appendWelcomeStatusRow(label, value = _welcomeStatusPendingText) {
  ...
  valueSpan.className = isReady ? "welcome-status-loaded" : "welcome-status-loading";
...
async function _runWelcomeStatusSequence(labels, intervalMs, staggerMs = null) {
  ...
  const STAGGER_MS = staggerMs == null ? Math.max(140, Math.floor(intervalMs * 0.32)) : staggerMs;
...
function cancelWelcome2(tabId = null) {
  ...
  if (tabId === _welcomeActiveTabId()) {
    ...
    setTimeout(() => {
      const shellPromptWrap2 = _welcomeShellPromptWrap();
...
async function _typeWelcomeCommand(tabId, cmd, { charMs, jitterMs, postMs, startDelayMs = 0, commentText = null, interactive = true } = {}) {
  ...
  typingSpan.innerHTML = `<span class="wlc-prompt">${_welcomePrompt}</span><span class="wlc-command-text welcome-command-text"></span><span class="wlc-cursor"></span>`;
...
function _renderWelcomeAsciiStream(tabId, asciiArt) {
  const out = _welcomeGetOutput(tabId);
  if (!out) return;
  const artLines = (asciiArt || (_welcomeAppConfig().app_name || "darklab_shell")).split("\n").map((line) => line.replace(/\s+$/g, "")).filter((line) => line.length > 0);
  ...
  if (motdText) {
    ...
    const renderMotd2 = typeof renderMotd === "function" && renderMotd || _welcomeGlobalFunction("renderMotd");
    if (renderMotd2) {
      operatorBody.innerHTML = renderMotd2(motdText);
    } else {
...
function _appendWelcomeCommand(tabId, cmd, commentText = null, { interactive = true } = {}) {
  ...
  line.innerHTML = `<span class="wlc-prompt">${_welcomePrompt}</span><span class="wlc-command-text welcome-command-text"></span>`;
  ...
  function loadCommand() {
    ...
    if (_welcomeIsActive() && welcomeOwnsTab2(tabId)) settleWelcome(tabId);
    _welcomeRefocusComposer();
    _welcomeSetComposerValue(cmd, cmd.length, cmd.length, { dispatch: false });
    setTimeout(() => {
      const cmdInput3 = _welcomeCmdInput();
      if (cmdInput3 && typeof cmdInput3.dispatchEvent === "function") cmdInput3.dispatchEvent(new Event("input"));
    }, 0);
  }
  if (interactive) {
...
function _appendWelcomeTourCta(tabId) {
  ...
  const openTourModal = _welcomeGlobalFunction("openTourModal");
  const useMobile = typeof useMobileTerminalViewportMode2 === "function" && useMobileTerminalViewportMode2 || _welcomeGlobalFunction("useMobileTerminalViewportMode");
  if (!(useMobile && useMobile()) && openTourModal) {
...
function _shouldRotateWelcomeHints(tabId) {
  const state = _readWelcomeState();
  return state.active && !state.settleRequested && welcomeOwnsTab2(tabId) && _welcomeActiveTabId() === tabId && (!_welcomeCmdInput() || !_welcomeCmdInput().value.trim());
}
...
function _ensureWelcomeFinalHint(tabId, hints) {
  ...
  if (Array.isArray(hints) && hints.length) {
    const finalHint = _pickRandomHint(hints, /* @__PURE__ */ new Set()) || hints[0];
    const line = document.createElement("span");
...
function settleWelcome(tabId = _welcomeActiveTabId()) {
  if (!welcomeOwnsTab2(tabId)) return false;
  const out = _welcomeGetOutput(tabId);
  if (!out) return false;
  _flushWelcomeWaiters();
  _clearWelcomeLiveLine();
  const asciiArt = _welcomePlan && typeof _welcomePlan.asciiArt === "string" ? _welcomePlan.asciiArt : "";
  if (!_welcomeBanner) _renderWelcomeAsciiStream(tabId, asciiArt);
  const statusLabels = _welcomePlan && Array.isArray(_welcomePlan.statusLabels) && _welcomePlan.statusLabels.length ? _welcomePlan.statusLabels : ["CONFIG", "RUNNER", "HISTORY", "LIMITS", "AUTOCOMPLETE"];
...
async function runWelcome() {
  ...
  if (_shouldUseMobileWelcomeSequence()) {
    const [asciiArt2, hintData2] = await Promise.all([
    ...
    const hints2 = hintData2 && Array.isArray(hintData2.items) ? hintData2.items : [];
...
if (typeof window !== "undefined") {
  if (typeof setWelcomeHandlers === "function") {
    setWelcomeHandlers({ cancelWelcome: cancelWelcome2, welcomeOwnsTab: welcomeOwnsTab2 });
  }
}

// app/static/js/features/tabs/tab_close_lifecycle.js
var TAB_CLOSE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
...
function _tabCloseEmitUiEvent(name, detail = {}) {
  const emit = typeof emitUiEvent !== "undefined" && emitUiEvent || _tabCloseGlobalFunction("emitUiEvent");
  if (typeof emit === "function") emit(name, detail);
}
function _tabCloseBlurActiveElement() {
  const blur = typeof blurActiveElement !== "undefined" && blurActiveElement || _tabCloseGlobalFunction("blurActiveElement");
  if (typeof blur === "function") blur();
}
function _tabCloseBlurVisibleComposerInputIfMobile() {
  const blur = typeof blurVisibleComposerInputIfMobile !== "undefined" && blurVisibleComposerInputIfMobile || _tabCloseGlobalFunction("blurVisibleComposerInputIfMobile");
  if (typeof blur === "function") blur();
}
function _tabCloseConfirm() {
  return typeof showConfirm !== "undefined" && showConfirm || _tabCloseGlobalFunction("showConfirm");
}
...
function _tabCloseMarkActiveRunDetachedForRestore(runId) {
  const markDetached = typeof markActiveRunDetachedForRestore !== "undefined" && markActiveRunDetachedForRestore || _tabCloseGlobalFunction("markActiveRunDetachedForRestore");
  if (typeof markDetached === "function") markDetached(runId);
}
function _tabCloseSyncRunButtonDisabled() {
  const sync = typeof syncRunButtonDisabled !== "undefined" && syncRunButtonDisabled || _tabCloseGlobalFunction("syncRunButtonDisabled");
  if (typeof sync === "function") sync();
}
function _tabCloseUseMobileTerminalViewportMode() {
  const useMobile = typeof useMobileTerminalViewportMode2 !== "undefined" && useMobileTerminalViewportMode2 || _tabCloseGlobalFunction("useMobileTerminalViewportMode");
  return typeof useMobile === "function" && useMobile();
}
function _tabCloseResetAnsiRendererForTab(tabId) {
  const resetAnsi = typeof resetAnsiRendererForTab2 !== "undefined" && resetAnsiRendererForTab2 || _tabCloseGlobalFunction("resetAnsiRendererForTab");
  if (typeof resetAnsi === "function") resetAnsi(tabId);
}
function _tabCloseDropAnsiRendererForTab(tabId) {
  const dropAnsi = typeof dropAnsiRendererForTab !== "undefined" && dropAnsiRendererForTab || _tabCloseGlobalFunction("dropAnsiRendererForTab");
  if (typeof dropAnsi === "function") dropAnsi(tabId);
}
function _tabCloseCancelPendingOutputBatch(tabId) {
  const cancelBatch = typeof _cancelPendingOutputBatch2 !== "undefined" && _cancelPendingOutputBatch2 || _tabCloseGlobalFunction("_cancelPendingOutputBatch");
  if (typeof cancelBatch === "function") cancelBatch(tabId);
}
function _tabCloseClearTabRunningLabelTimer(tab) {
  const clearTimer = typeof _clearTabRunningLabelTimer !== "undefined" && _clearTabRunningLabelTimer || _tabCloseGlobalFunction("_clearTabRunningLabelTimer");
  if (typeof clearTimer === "function") clearTimer(tab);
}
function _tabCloseGetNeighborTabIdAfterClose(idx, id) {
  const getNeighbor = typeof _getNeighborTabIdAfterClose !== "undefined" && _getNeighborTabIdAfterClose || _tabCloseGlobalFunction("_getNeighborTabIdAfterClose");
  return typeof getNeighbor === "function" ? getNeighbor(idx, id) : null;
}
function _tabCloseGetTabEl(id) {
  const getEl = typeof _getTabEl !== "undefined" && _getTabEl || _tabCloseGlobalFunction("_getTabEl");
  return typeof getEl === "function" ? getEl(id) : null;
}
function _tabCloseGetTabPanelEl(id) {
  const getEl = typeof _getTabPanelEl !== "undefined" && _getTabPanelEl || _tabCloseGlobalFunction("_getTabPanelEl");
  return typeof getEl === "function" ? getEl(id) : null;
}
function _tabCloseActivateTab(id, options) {
  const activate = typeof activateTab !== "undefined" && activateTab || _tabCloseGlobalFunction("activateTab");
  if (typeof activate === "function") activate(id, options);
}
function _tabCloseUpdateNewTabBtn() {
  const update = typeof updateNewTabBtn !== "undefined" && updateNewTabBtn || _tabCloseGlobalFunction("updateNewTabBtn");
  if (typeof update === "function") update();
}
function _tabCloseUpdateTabScrollButtons() {
  const update = typeof updateTabScrollButtons !== "undefined" && updateTabScrollButtons || _tabCloseGlobalFunction("updateTabScrollButtons");
  if (typeof update === "function") update();
}
function _tabCloseSchedulePersistTabSessionState() {
  const schedule = typeof schedulePersistTabSessionState !== "undefined" && schedulePersistTabSessionState || _tabCloseGlobalFunction("schedulePersistTabSessionState");
  if (typeof schedule === "function") schedule();
}
function _tabCloseDetachRunStreamForTab(id) {
  const detach = typeof detachRunStreamForTab2 !== "undefined" && detachRunStreamForTab2 || _tabCloseGlobalFunction("detachRunStreamForTab");
...
function _tabCloseDoKill(id) {
  const kill = typeof doKill2 !== "undefined" && doKill2 || _tabCloseGlobalFunction("doKill");
  if (typeof kill === "function") kill(id);
}
function _tabCloseClearTab(id) {
  const clear = typeof clearTab !== "undefined" && clearTab || _tabCloseGlobalFunction("clearTab");
  if (typeof clear === "function") clear(id);
}
function _tabCloseDefaultTabLabel(index) {
  const createLabel = typeof createDefaultTabLabel !== "undefined" && createDefaultTabLabel || _tabCloseGlobalFunction("createDefaultTabLabel");
  return typeof createLabel === "function" ? createLabel(index) : `Tab ${index}`;
}
function _tabCloseSetTabLabel(id, label) {
  const setLabel = typeof setTabLabel !== "undefined" && setTabLabel || _tabCloseGlobalFunction("setTabLabel");
  if (typeof setLabel === "function") setLabel(id, label);
}
function _tabCloseSyncMountedPromptLineNumber(id) {
  const getOutput3 = _tabCloseGlobalFunction("getOutput");
  const out = typeof getOutput3 === "function" ? getOutput3(id) : null;
...
function _tabCloseCancelWelcome(id) {
  const cancel = typeof cancelWelcome2 !== "undefined" && cancelWelcome2 || _tabCloseGlobalFunction("cancelWelcome");
...
function _activateNeighborAfterClose(idx, id) {
  ...
  if (typeof document !== "undefined" && document.body && document.body.classList && document.body.classList.contains("mobile-terminal-mode") && typeof window !== "undefined" && typeof window.scrollTo === "function") {
    setTimeout(() => {
      try {
...
function confirmCloseRunningTab(id) {
  ...
  return _tabCloseShowConfirm({
    body: {
      ...
      note: canDetach ? "Keep running detaches this tab only. The command keeps running and can be reopened from Status Monitor." : "The command is still starting, so it cannot be detached yet."
    },
    tone: "danger",
    actions
  }).then((result) => {
...
function finalizeClosingTab(id) {
  ...
  if (currentTabs.length === 1) {
    ...
    _tabCloseSetTabLabel(id, _tabCloseDefaultTabLabel(1));
    _tabCloseSyncMountedPromptLineNumber(id);
    if (typeof document !== "undefined" && document.body && document.body.classList && document.body.classList.contains("mobile-terminal-mode")) {
      setTimeout(() => _tabCloseBlurVisibleComposerInputIfMobile(), 0);
    }
    _tabCloseSchedulePersistTabSessionState();
    return true;
...
var confirmPermalinkRedactionChoiceHandler = null;
...
function hasShareRedactionHandler(name) {
  return name === "confirmPermalinkRedactionChoice" && typeof confirmPermalinkRedactionChoiceHandler === "function";
}
function confirmPermalinkRedactionChoice(...args) {
  return typeof confirmPermalinkRedactionChoiceHandler === "function" ? confirmPermalinkRedactionChoiceHandler(...args) : void 0;
}

// app/static/js/features/tabs/tab_exports.js
var TAB_EXPORT_GLOBAL = typeof window !== "undefined" ? window : globalThis;
...
function _tabExportGetTab(id) {
  const getTabFn = typeof getTab !== "undefined" && getTab || _tabExportGlobalFunction("getTab");
  return typeof getTabFn === "function" ? getTabFn(id) : null;
}
function _tabExportShowToast(...args) {
  const show = typeof showToast !== "undefined" && showToast || _tabExportGlobalFunction("showToast");
  if (typeof show === "function") show(...args);
}
function _tabExportRefocusComposer(options = { preventScroll: true }) {
  const refocus = typeof refocusComposerAfterAction !== "undefined" && refocusComposerAfterAction || _tabExportGlobalFunction("refocusComposerAfterAction");
  if (typeof refocus === "function") refocus(options);
}
function _tabExportCopyTextToClipboard(text) {
  const copy = typeof copyTextToClipboard !== "undefined" && copyTextToClipboard || _tabExportGlobalFunction("copyTextToClipboard");
...
function _tabExportDownloadBlobAsAttachment(blob, filename) {
  const download = typeof downloadBlobAsAttachment !== "undefined" && downloadBlobAsAttachment || _tabExportGlobalFunction("downloadBlobAsAttachment");
  if (typeof download === "function") download(blob, filename);
}
function _tabExportEscapeHtml(text) {
  const escape = typeof escapeHtml !== "undefined" && escapeHtml || _tabExportGlobalFunction("escapeHtml");
  return typeof escape === "function" ? escape(text) : String(text ?? "");
}
function _tabExportRedactLineEntries(lines, rules) {
  const redact = typeof redactLineEntries !== "undefined" && redactLineEntries || _tabExportGlobalFunction("redactLineEntries");
...
function _tabExportOmitRawOnlyLineEntries(lines) {
  const omit = typeof omitRawOnlyLineEntries !== "undefined" && omitRawOnlyLineEntries || _tabExportGlobalFunction("omitRawOnlyLineEntries");
  return typeof omit === "function" ? omit(lines) : Array.isArray(lines) ? lines : [];
}
function _tabExportCommandOutcomeSummariesPreference() {
  const getPreference2 = _tabExportGlobalFunction("getCommandOutcomeSummariesPreference") || typeof getCommandOutcomeSummariesPreference !== "undefined" && getCommandOutcomeSummariesPreference;
  return typeof getPreference2 === "function" ? getPreference2() : null;
}
function _tabExportCreateAnsiUpRenderer() {
  const createRenderer = typeof createAnsiUpRenderer !== "undefined" && createAnsiUpRenderer || _tabExportGlobalFunction("createAnsiUpRenderer");
...
function _tabExportTeamScopeDeniedMessage(action) {
  const message = typeof teamScopeDeniedMessage !== "undefined" && teamScopeDeniedMessage || null;
  return typeof message === "function" ? message(action) : "";
}
function _tabExportShareUrl(url) {
  const share = typeof shareUrl !== "undefined" && shareUrl || _tabExportGlobalFunction("shareUrl");
  return typeof share === "function" ? share(url) : Promise.reject(new Error("share unavailable"));
}
function _tabExportApiFetch(url, options) {
  const api = typeof apiFetch === "function" && apiFetch || _tabExportGlobalFunction("apiFetch");
...
function _tabExportConfirmRedactionChoice() {
  const confirm = typeof hasShareRedactionHandler === "function" && hasShareRedactionHandler("confirmPermalinkRedactionChoice") ? confirmPermalinkRedactionChoice : _tabExportGlobalFunction("confirmPermalinkRedactionChoice");
...
function _tabExportLineNumberMode() {
  const readMode = typeof getLineNumberMode !== "undefined" && getLineNumberMode || _tabExportGlobalFunction("getLineNumberMode");
  return typeof readMode === "function" ? readMode() : TAB_EXPORT_GLOBAL.lnMode;
}
function _tabExportTimestampMode() {
  const readMode = typeof getTimestampMode !== "undefined" && getTimestampMode || _tabExportGlobalFunction("getTimestampMode");
...
function _getExportableRawLines(tab) {
  ...
  return tab.rawLines.filter((line) => {
    ...
    const cls = ExportHtmlUtils && typeof ExportHtmlUtils.lineLegacyClass === "function" ? ExportHtmlUtils.lineLegacyClass(ExportHtmlUtils.lineEventFromWire(line)) : String(line.cls || "");
...
function _getShareRedactionRules() {
  ...
  return config && Array.isArray(config.share_redaction_rules) ? config.share_redaction_rules : [];
}
...
function copyTab(id) {
  ...
  _tabExportCopyTextToClipboard(text).then(() => _tabExportShowToast("Copied to clipboard")).catch(() => _tabExportShowToast("Failed to copy", "error")).finally(() => _refocusAfterTabAction());
}
function saveTab(id) {
  _refreshTabExportHtmlUtils();
  const t = _tabExportGetTab(id);
...
function _exportPrefix(line, zeroBasedIndex) {
  if (ExportHtmlUtils && typeof ExportHtmlUtils.isCommandOutcomeSummaryLine === "function" && ExportHtmlUtils.isCommandOutcomeSummaryLine(line)) {
    return "";
  }
  const parts = [];
...
function _normalizeTabTranscriptLine(line) {
  ...
  if (line && typeof line.text === "string") {
    return ExportHtmlUtils && typeof ExportHtmlUtils.lineEventFromWire === "function" ? ExportHtmlUtils.lineEventFromWire(line) : {
      text: line.text,
      cls: String(line.cls || ""),
      tsC: String(line.tsC || ""),
      tsE: String(line.tsE || ""),
      line_number: Number.isInteger(line.line_number) ? line.line_number : void 0
    };
  }
  return null;
}
function _normalizeTabTranscriptLines(lines, { stripTruncationNotices = false } = {}) {
  ...
  return (Array.isArray(lines) ? lines : []).map(_normalizeTabTranscriptLine).filter((line) => {
...
function _buildTabExportModel(tab, { createdText = null, omitRawOnly = false } = {}) {
  const config = _tabExportConfig();
  const normalizedCreatedText = String(createdText || (/* @__PURE__ */ new Date()).toLocaleString());
...
async function exportTabPdf(id) {
  ...
  try {
    ...
    const jsPDF = loadJsPdf ? await loadJsPdf() : await pdfUtils.loadJsPdf();
    const exportModel = _buildTabExportModel(t, { omitRawOnly: true });
    const ansiRenderer = _tabExportCreateAnsiUpRenderer();
...
function _extractLatestFullRunShareContent(tab, fullRun) {
  ...
  const runStartIndex = typeof tab.currentRunStartIndex === "number" && tab.currentRunStartIndex >= 0 ? tab.currentRunStartIndex : rawLines.length;
  const exitIndex = (() => {
    for (let i = rawLines.length - 1; i >= 0; i -= 1) {
      const event = ExportHtmlUtils && typeof ExportHtmlUtils.lineEventFromWire === "function" ? ExportHtmlUtils.lineEventFromWire(rawLines[i]) : { role: String(rawLines[i] && rawLines[i].role || rawLines[i] && rawLines[i].cls || "") };
      if (["exit-ok", "exit-fail"].includes(String(event.role || ""))) return i;
    }
    return rawLines.length;
  })();
  const fullOutput = Array.isArray(fullRun && fullRun.output_entries) ? fullRun.output_entries : _shareLinesWithoutTruncationNotices(fullRun && fullRun.output);
  return [
...
function _shareSnapshotDeniedMessage() {
  return _tabExportTeamScopeDeniedMessage("create team history snapshots") || "View-only team members can't create team history snapshots. Switch to Personal or ask for operator access.";
}
async function permalinkTab(id) {
  ...
  }).catch(() => _tabExportShowToast("Failed to create permalink", "error")).finally(() => {
    _refocusAfterTabAction();
  });
}
...
function _syncTabShellPrompt() {
  const syncPrompt = typeof hasComposerPromptHandler === "function" && hasComposerPromptHandler("syncShellPrompt") ? syncShellPrompt : _tabGlobalFn("syncShellPrompt");
  if (typeof syncPrompt === "function") syncPrompt();
}
...
function _tabGlobalValue(name, imported = void 0) {
  const global = _tabGlobal();
...
function _setTabsList(next) {
  setTabs(Array.isArray(next) ? next : []);
}
function _appConfig() {
  return _tabGlobalValue("APP_CONFIG") || {};
}
function _tabEl(name, imported = void 0) {
  return _tabGlobalValue(name, imported) || null;
}
function _isTabSessionRestoreInProgress() {
  return !!_tabGlobalValue("_tabSessionRestoreInProgress", _tabSessionRestoreInProgress);
}
function _schedulePersistTabSessionState() {
  _tabGlobalFn("schedulePersistTabSessionState", schedulePersistTabSessionState)?.();
}
function _blurActiveElement(...args) {
  return _tabGlobalFn("blurActiveElement", blurActiveElement)?.(...args);
}
function _blurVisibleComposerInputIfMobile(...args) {
  return _tabGlobalFn("blurVisibleComposerInputIfMobile", blurVisibleComposerInputIfMobile)?.(...args);
}
function _focusElement(...args) {
  return _tabGlobalFn("focusElement", focusElement)?.(...args);
}
function _getComposerValue(...args) {
  return _tabGlobalFn("getComposerValue", getComposerValue)?.(...args);
}
function _refocusComposerAfterAction(...args) {
  return _tabGlobalFn("refocusComposerAfterAction", refocusComposerAfterAction)?.(...args);
}
function _setComposerValue(...args) {
  return _tabGlobalFn("setComposerValue", setComposerValue)?.(...args);
}
function _showToast(...args) {
  return _tabGlobalFn("showToast", showToast)?.(...args);
}
function _syncRunButtonDisabled(...args) {
  return _tabGlobalFn("syncRunButtonDisabled", syncRunButtonDisabled)?.(...args);
}
function _tabWelcomeApi(name) {
  return _tabGlobalValue(name, null);
}
function _tabWelcomeActive() {
  const apiState = getWelcomeState();
  return !!(_tabGlobalValue("_welcomeActive") || apiState && apiState.active);
}
function _tabWelcomeBootPending() {
  const apiState = getWelcomeState();
  return !!(_tabGlobalValue("_welcomeBootPending") || apiState && apiState.bootPending && (apiState.active || apiState.done || apiState.tabId));
}
...
function _clearAutocompleteFilteredState() {
  setAutocompleteState({ filtered: [], index: -1 });
}
function _getTabEl2(id) {
  const bar = _tabEl("tabsBar", tabsBar);
  return bar ? bar.querySelector(`.tab[data-id="${id}"]`) : null;
}
function _getTabPanelEl2(id) {
  const panels = _tabEl("tabPanels", tabPanels);
  return panels ? panels.querySelector(`.tab-panel[data-id="${id}"]`) : null;
}
function _getTabStatusEl(id) {
  return _getTabEl2(id)?.querySelector(".tab-status") || null;
}
function _getTabLabelEl(id) {
  return _getTabEl2(id)?.querySelector(".tab-label") || null;
}
function _nextDefaultTabNumber() {
  const numbers = _tabsList().map((tab) => String(tab && tab.label || "").trim().match(/^shell\s+(\d+)$/i)).filter(Boolean).map((match) => Number(match[1])).filter(Number.isFinite);
  return numbers.length ? Math.max(...numbers) + 1 : 1;
}
function createDefaultTabLabel2(index = null) {
  const explicitIndex = index !== null && index !== void 0 && index !== "";
...
function _clearTabRunningLabelTimer2(tab) {
...
function _getTabOutputEl(id) {
  return _getTabPanelEl2(id)?.querySelector(".output") || null;
}
...
function _getNeighborTabIdAfterClose2(idx, closingId) {
...
function updateTabScrollButtons2() {
  updateTabbarChromeFit();
  const leftBtn = _tabEl("tabsScrollLeftBtn", tabsScrollLeftBtn);
  const rightBtn = _tabEl("tabsScrollRightBtn", tabsScrollRightBtn);
  const bar = _tabEl("tabsBar", tabsBar);
...
var PREF_TABBAR_CHROME = "pref_tabbar_chrome";
var _TABBAR_CHROME_FIT_BUFFER = 12;
var _tabbarChromeFullWidth = 0;
...
function _applyTabbarChromeState(barEl, collapsed, pref, autoWouldCollapse = collapsed) {
  ...
  const toggle = _tabEl("tabbarChromeToggle", tabbarChromeToggle);
  if (!toggle) return;
  ...
  toggle.title = collapsed ? "Show search and display controls" : "Collapse toolbar to make room for tabs";
}
function updateTabbarChromeFit() {
  const chrome = _tabEl("tabbarChrome", tabbarChrome);
  if (!chrome) return;
...
function ensureActiveTabVisible2(tabId) {
  const tabEl = _getTabEl2(tabId);
...
function scrollTabsBar(direction) {
  ...
  setTimeout(updateTabScrollButtons2, 180);
  _refocusComposerAfterAction({ defer: true });
}
function setupTabScrollControls() {
  ...
  bar.addEventListener("scroll", updateTabScrollButtons2, { passive: true });
  window.addEventListener("resize", updateTabScrollButtons2);
  const toggle = _tabEl("tabbarChromeToggle", tabbarChromeToggle);
  if (toggle) {
    toggle.addEventListener("click", _toggleTabbarChrome);
  }
  const chrome = _tabEl("tabbarChrome", tabbarChrome);
  const barEl = chrome ? chrome.closest(".terminal-bar") : null;
  if (barEl && typeof ResizeObserver === "function") {
    new ResizeObserver(() => updateTabScrollButtons2()).observe(barEl);
  }
  _tabsScrollControlsBound = true;
  updateTabScrollButtons2();
}
function syncTabOrderFromDom2() {
  const bar = _tabEl("tabsBar", tabsBar);
  if (!bar) return;
  ...
  emitUiEvent("app:tab-order-changed", {
    ...
    activeTabId: _activeTabId2()
  });
}
function unmountShellPrompt2() {
  const promptWrap = _tabEl("shellPromptWrap", shellPromptWrap);
...
function mountShellPrompt2(tabId, force = false) {
  const promptWrap = _tabEl("shellPromptWrap", shellPromptWrap);
  if (!promptWrap) return;
  ...
  const panel = _getTabPanelEl2(tabId);
  if (!panel) return;
...
function updateNewTabBtn2() {
  const btn = _tabEl("newTabBtn", newTabBtn);
  if (!btn) return;
...
function _createTabHeader(id, label) {
  ...
  status3.className = "tab-status idle";
  tab.appendChild(status3);
...
function _tabShareSnapshotDeniedTitle() {
  ...
  return deniedMessage ? deniedMessage("create team history snapshots") : "View-only team members can't create team history snapshots. Switch to Personal or ask for operator access.";
}
function _tabOutputFn(name, imported = null) {
  const hasBridgeHandler = typeof hasOutputHandler === "function" && hasOutputHandler(name);
...
function _tabSyncOutputPrefixes(...args) {
  return _tabOutputFn("syncOutputPrefixes", syncOutputPrefixes)?.(...args);
}
function _tabCancelPendingOutputBatch(...args) {
  return _tabOutputFn("_cancelPendingOutputBatch", _cancelPendingOutputBatch)?.(...args);
}
function _tabResetAnsiRendererForTab(...args) {
  return _tabOutputFn("resetAnsiRendererForTab", resetAnsiRendererForTab)?.(...args);
}
function _tabResetOutputSignalCounts(...args) {
  return _tabOutputFn("_resetTabOutputSignalCounts", _resetTabOutputSignalCounts)?.(...args);
}
...
function _getOutputFollowButton(id) {
  return _getTabPanelEl2(id)?.querySelector(".output-follow-btn") || null;
}
...
function updateOutputFollowButton2(id) {
  const tab = _tabById(id);
  const out = getOutput2(id);
...
function _createTabPanel(id) {
  ...
  followBtn.addEventListener("click", () => {
    ...
    if (!tab || !out) return;
    tab.followOutput = true;
    const stickOutputToBottom = _tabOutputFn("_stickOutputToBottom", _stickOutputToBottom);
    if (stickOutputToBottom) {
      stickOutputToBottom(out, tab);
    } else {
      out.scrollTop = out.scrollHeight;
    }
    updateOutputFollowButton2(id);
  });
  ...
  const bindOutsideClickClose2 = _tabGlobalFn("bindOutsideClickClose", bindOutsideClickClose);
  if (bindOutsideClickClose2) {
    bindOutsideClickClose2(saveWrap, {
      triggers: saveBtn,
...
function createTab2(label) {
  ...
  const stableLabel = String(label || createDefaultTabLabel2());
  const { tab, labelEl } = _createTabHeader(id, stableLabel);
  const closeCurrentTab = typeof closeTab === "function" ? closeTab : _tabGlobalFn("closeTab");
  const bindDragReorder = typeof bindTabDragReorder === "function" ? bindTabDragReorder : _tabGlobalFn("bindTabDragReorder");
  const copyCurrentTab = typeof copyTab === "function" ? copyTab : _tabGlobalFn("copyTab");
  const permalinkCurrentTab = typeof permalinkTab === "function" ? permalinkTab : _tabGlobalFn("permalinkTab");
  const saveCurrentTab = typeof saveTab === "function" ? saveTab : _tabGlobalFn("saveTab");
  const exportCurrentTabHtml = typeof exportTabHtml === "function" ? exportTabHtml : _tabGlobalFn("exportTabHtml");
  const exportCurrentTabPdf = typeof exportTabPdf === "function" ? exportTabPdf : _tabGlobalFn("exportTabPdf");
  ...
  const newTabButton = _tabEl("newTabBtn", newTabBtn);
  const bar = _tabEl("tabsBar", tabsBar);
  ...
  panel.querySelectorAll("[data-action]").forEach((btn) => {
    ...
    const bindPressable2 = _tabGlobalFn("bindPressable", bindPressable);
    bindPressable2?.(btn, {
      ...
      onActivate: () => {
        ...
        if (action === "kill") {
          const confirmKill3 = typeof confirmKill === "function" && confirmKill || _tabGlobalFn("confirmKill");
          confirmKill3?.(id);
        }
        if (action === "clear") {
          _tabCancelWelcome(id);
          clearTab2(id, { preserveRunState: true });
        }
  ...
  _tabEl("tabPanels", tabPanels)?.appendChild(panel);
  const list = _tabsList();
  list.push({
    id,
    label: stableLabel,
  ...
  emitUiEvent("app:tab-created", { id, label: stableLabel, activeTabId: _activeTabId2() });
  return id;
}
function activateTab2(id, { focusComposer = true } = {}) {
  if (_tabGlobalFn("isHistSearchMode")?.()) {
    _tabGlobalFn("exitHistSearch")?.(false);
  }
  const prevId = _activeTabId2();
  if (!_isTabSessionRestoreInProgress() && prevId && prevId !== id) {
    const prevTab = _tabById(prevId);
    if (prevTab && prevTab.st === "running") {
      prevTab.draftInput = "";
    } else if (prevTab) {
      const input = _tabEl("cmdInput", cmdInput);
      prevTab.draftInput = _getComposerValue?.() || (input ? input.value : "");
    }
  }
  _setActiveTabId(id);
  _tabEl("tabsBar", tabsBar)?.querySelectorAll(".tab").forEach((t2) => t2.classList.toggle("active", t2.dataset.id === id));
  _tabEl("tabPanels", tabPanels)?.querySelectorAll(".tab-panel").forEach((p) => p.classList.toggle("active", p.dataset.id === id));
  mountShellPrompt2(id);
  const t = _tabById(id);
  const setStatus3 = typeof setStatus === "function" && setStatus || _tabGlobalFn("setStatus");
  setStatus3?.(t ? t.st || "idle" : "idle");
  if (t && t.followOutput !== false) {
    const out = getOutput2(id);
    const restoreTail = _tabOutputFn("_restoreOutputTailAfterLayout", _restoreOutputTailAfterLayout);
    const stickBottom = _tabOutputFn("_stickOutputToBottom", _stickOutputToBottom);
    if (out && restoreTail) {
      restoreTail(out, t);
  ...
  _schedulePersistTabSessionState();
  emitUiEvent("app:tab-activated", { id, prevId, activeTabId: _activeTabId2() });
}
function setTabStatus2(id, st) {
  ...
  updateOutputFollowButton2(id);
  _schedulePersistTabSessionState();
  emitUiEvent("app:tab-status-changed", { id, status: st, activeTabId: _activeTabId2() });
}
function setTabLabel2(id, label) {
  const t = _tabById(id);
  if (t) {
    t.label = String(label || "");
    _renderTabLabel(id);
  }
  if (id === _activeTabId2()) ensureActiveTabVisible2(id);
  _schedulePersistTabSessionState();
}
...
function clearTab2(id, { preserveRunState = false } = {}) {
  ...
  if (id === _activeTabId2() && (!preserveRunState || !wasRunning)) {
    mountShellPrompt2(id);
    _syncMountedPromptLineNumber(id);
  }
  if (id === _activeTabId2() && (!preserveRunState || !wasRunning) && !(typeof document !== "undefined" && document.body && document.body.classList && document.body.classList.contains("mobile-terminal-mode"))) {
    _setComposerValue("", 0, 0);
  }
  if (!preserveRunState || !wasRunning) {
    setTabStatus2(id, "idle");
    if (id === _activeTabId2()) {
      const setStatus3 = typeof setStatus === "function" && setStatus || _tabGlobalFn("setStatus");
      setStatus3?.("idle");
      _tabGlobalFn("clearSearch")?.();
  ...
  if (typeof document !== "undefined" && document.body && document.body.classList && document.body.classList.contains("mobile-terminal-mode") && _tabGlobalFn("blurVisibleComposerInputIfMobile", blurVisibleComposerInputIfMobile)) {
...
if (typeof setTabHandlers === "function") {
  setTabHandlers({
    _clearTabRunningLabelTimer: _clearTabRunningLabelTimer2,
    _getNeighborTabIdAfterClose: _getNeighborTabIdAfterClose2,
    _getTabEl: _getTabEl2,
    _getTabPanelEl: _getTabPanelEl2,
    activateTab: activateTab2,
    clearTab: clearTab2,
    createDefaultTabLabel: createDefaultTabLabel2,
    createTab: createTab2,
    ensureActiveTabVisible: ensureActiveTabVisible2,
    getOutput: getOutput2,
    mountShellPrompt: mountShellPrompt2,
    setTabLabel: setTabLabel2,
    setTabStatus: setTabStatus2,
    syncTabOrderFromDom: syncTabOrderFromDom2,
    unmountShellPrompt: unmountShellPrompt2,
    updateNewTabBtn: updateNewTabBtn2,
    updateOutputFollowButton: updateOutputFollowButton2,
    updateTabScrollButtons: updateTabScrollButtons2
  });
}

// app/static/js/features/autocomplete/runtime_context.js
var RUNTIME_CONTEXT_GLOBAL = typeof window !== "undefined" ? window : globalThis;
...
function _runtimeApiFetch(url, options) {
  const api = typeof apiFetch === "function" && typeof hasRuntimeHandler === "function" && hasRuntimeHandler("apiFetch") ? apiFetch : _runtimeGlobalFunction("apiFetch");
  if (typeof api === "function") return options === void 0 ? api(url) : api(url, options);
  return Promise.reject(new Error("apiFetch unavailable"));
}
function _runtimeLogClientError(context, err, details = {}) {
  const logger = typeof logClientError === "function" && typeof hasRuntimeHandler === "function" && hasRuntimeHandler("logClientError") ? logClientError : _runtimeGlobalFunction("logClientError");
  if (typeof logger === "function") logger(context, err, details);
}
function _runtimeState() {
  const importedApi = typeof APP_STATE_API !== "undefined" ? APP_STATE_API : null;
...
function _runtimeWorkspaceCacheApi() {
  return {
    getDirectoryEntries: typeof getWorkspaceDirectoryEntries !== "undefined" && getWorkspaceDirectoryEntries || _runtimeGlobalFunction("getWorkspaceDirectoryEntries"),
    getDirectoryHints: typeof getWorkspaceAutocompleteDirectoryHints !== "undefined" && getWorkspaceAutocompleteDirectoryHints || _runtimeGlobalFunction("getWorkspaceAutocompleteDirectoryHints"),
    getFileHints: typeof getWorkspaceAutocompleteFileHints !== "undefined" && getWorkspaceAutocompleteFileHints || _runtimeGlobalFunction("getWorkspaceAutocompleteFileHints")
  };
}
...
function _runtimeActiveTabId() {
  const readActiveTabId = typeof getActiveTabId !== "undefined" && getActiveTabId || _runtimeGlobalFunction("getActiveTabId");
  if (typeof readActiveTabId === "function") return readActiveTabId();
  return RUNTIME_CONTEXT_GLOBAL.activeTabId || null;
}
function _runtimeGetTab(tabId) {
  const readTab = typeof getTab !== "undefined" && getTab || _runtimeGlobalFunction("getTab");
  return typeof readTab === "function" ? readTab(tabId) : null;
}
function _runtimeGetOutput(tabId) {
  const readOutput = typeof getOutput2 !== "undefined" && getOutput2 || _runtimeGlobalFunction("getOutput");
  return typeof readOutput === "function" ? readOutput(tabId) : null;
}
...
function _runtimeMergeHints(baseHints = {}, overlayHints = {}) {
  ...
  Object.entries(overlayHints || {}).forEach(([trigger, hints]) => {
    ...
    const seenInserts = /* @__PURE__ */ new Map();
...
function _runtimeCommandLookupHints(baseRegistry = {}, descriptionForExternal = "manual page") {
  const builtinNames = new Set(
    _runtimeActiveBuiltinRoots(baseRegistry).filter((root) => _runtimeSpecEnabledForFeatures(root, baseRegistry[root]))
  );
  const externalRoots = new Set(
    Object.keys(baseRegistry || {}).filter((root) => _runtimeSpecEnabledForFeatures(root, baseRegistry[root]))
  );
...
function _runtimeWorkspaceCwd() {
  ...
  const readWorkspaceCwd = typeof _workspaceCwd === "function" && _workspaceCwd || _runtimeGlobalFunction("_workspaceCwd");
  if (readWorkspaceCwd) return readWorkspaceCwd(tabId);
  if (tabId) {
    const tab = _runtimeGetTab(tabId);
    const parts = String(tab && tab.workspaceCwd || "").split("/").map((part) => String(part || "").trim()).filter(Boolean);
    return parts.join("/");
  }
  return "";
}
...
function _runtimeWorkspaceDirectHintFromPath(item, cwd = "", kind = "file") {
  ...
  const value = kind === "directory" && relative !== "/" && !relative.endsWith("/") ? `${relative}/` : relative;
  return _runtimeHint(value, item && item.description || "");
}
function _runtimeNormalizeWorkspaceCommandPath(path = "", cwd = "") {
  const normalize = typeof normalizeCommandPath === "function" && normalizeCommandPath || _runtimeGlobalFunction("normalizeWorkspaceCommandPath");
  if (normalize) {
...
function _runtimeWorkspaceAllHints(kind = "file") {
  ...
  return kind === "directory" ? typeof workspaceCache.getDirectoryHints === "function" ? workspaceCache.getDirectoryHints() : [] : typeof workspaceCache.getFileHints === "function" ? workspaceCache.getFileHints() : [];
}
function _runtimeWorkspaceHintDescription(path = "", kind = "file") {
  const normalized = String(path || "").split("/").filter(Boolean).join("/");
  const allHints = _runtimeWorkspaceAllHints(kind);
  const existing = (Array.isArray(allHints) ? allHints : []).find((item) => String(item && item.value || "").split("/").filter(Boolean).join("/") === normalized);
  return existing && existing.description ? existing.description : kind === "directory" ? "session folder" : "session file";
}
...
function _runtimeWorkspaceEntryHints(kind = "file") {
  ...
  if (typeof workspaceCache.getDirectoryEntries === "function") {
    ...
    const allHints = kind === "directory" ? typeof workspaceCache.getDirectoryHints === "function" ? workspaceCache.getDirectoryHints() : [] : typeof workspaceCache.getFileHints === "function" ? workspaceCache.getFileHints() : [];
    return (Array.isArray(source) ? source : []).map((entry) => {
      ...
      const value = kind === "directory" && name && name !== "/" && !name.endsWith("/") ? `${name}/` : name;
  ...
  if (kind === "directory") {
    ...
    return workspaceCache.getDirectoryHints().map((item) => _runtimeWorkspaceDirectHintFromPath(item, cwd, "directory")).filter(Boolean);
  }
  if (typeof workspaceCache.getFileHints !== "function") return [];
  return workspaceCache.getFileHints().map((item) => _runtimeWorkspaceDirectHintFromPath(item, cwd)).filter(Boolean);
}
...
function _runtimeWorkspaceMoveDestinationHintsForSource(source, destinationHints) {
  ...
  return (Array.isArray(destinationHints) ? destinationHints : []).filter((hint) => {
    ...
    return value === "/" || normalizedValue && normalizedValue !== normalizedSource && !normalizedValue.startsWith(`${normalizedSource}/`);
  });
}
...
function getWorkspaceAutocompleteFlagFileHints(token = "") {
  return String(token || "").includes("/") ? _runtimeWorkspaceScopedHints("file", token) : _runtimeWorkspaceFilePathHints();
}
...
function _runtimeThemeContext() {
  const themeEntries = (typeof _cliThemeEntries === "function" ? _cliThemeEntries : null) || _runtimeGlobalFunction("_cliThemeEntries");
  const themeSlug = (typeof _cliThemeSlug === "function" ? _cliThemeSlug : null) || _runtimeGlobalFunction("_cliThemeSlug");
  const themeDescription = (typeof _cliThemeDescription === "function" ? _cliThemeDescription : null) || _runtimeGlobalFunction("_cliThemeDescription");
  const themeHints = typeof themeEntries === "function" && typeof themeSlug === "function" && typeof themeDescription === "function" ? themeEntries().map((entry) => _runtimeHint(themeSlug(entry), themeDescription(entry))) : [];
  const argHints = {
    list: [],
    current: [],
    set: themeHints,
  ...
  themeHints.forEach((item) => {
    argHints[item.value] = [];
  });
  return _runtimeContextSpec({ expectsValue: ["set"], argHints });
}
function _runtimeConfigContext() {
  const configEntries = (typeof _cliConfigEntries === "function" ? _cliConfigEntries : null) || _runtimeGlobalFunction("_cliConfigEntries");
  ...
  entries.forEach((entry) => {
    sequenceArgHints[`set ${entry.key}`] = Array.isArray(entry.values) ? entry.values.map((value) => _runtimeHint(value, entry.description)) : [_runtimePlaceholderHint(entry.valueHelp || "<value>", entry.description)];
    sequenceArgHints[`get ${entry.key}`] = [];
    if (Array.isArray(entry.values)) entry.values.forEach((value) => {
      argHints[value] = [];
    });
  });
...
function _runtimeVarContext() {
  ...
  const starterHints = starterNames.filter((name) => !currentNames.has(name)).map((name) => _runtimeHint(name, `Common ${name.toLowerCase()} value`));
  const sequenceArgHints = {};
...
function _runtimeProjectRefHints(statuses = []) {
  const wanted = new Set((Array.isArray(statuses) ? statuses : []).map((status3) => String(status3 || "").toLowerCase()));
  const readProjects = typeof _readAutocompleteProjects !== "undefined" && _readAutocompleteProjects || _runtimeGlobalFunction("_readAutocompleteProjects");
  const projects = readProjects ? readProjects() : [];
  return projects.filter((project) => {
    const status3 = String(project && project.status || "").toLowerCase();
    return !wanted.size || wanted.has(status3);
  }).map((project) => {
    const value = String(project && project.value || "").trim();
    if (!value) return null;
    const name = String(project && project.name || "").trim();
    const status3 = String(project && project.status || "").trim();
    const suffix = status3 ? ` · ${status3}` : "";
...
function _runtimeProjectContext(baseSpec = {}) {
  ...
  const setProjectHints = (name, hints) => {
    const subSpec = spec.subcommands[name] && typeof spec.subcommands[name] === "object" ? _cloneRuntimeSpec(spec.subcommands[name]) : {};
    if (hints.length) {
...
function _runtimeScheduleHints() {
  const readSchedules = typeof _readAutocompleteSchedules !== "undefined" && _readAutocompleteSchedules || _runtimeGlobalFunction("_readAutocompleteSchedules");
...
function _runtimeWatcherHints() {
  const readWatchers = typeof _readAutocompleteWatchers !== "undefined" && _readAutocompleteWatchers || _runtimeGlobalFunction("_readAutocompleteWatchers");
...
function getRuntimeAutocompleteContext(baseRegistry = {}) {
  ...
  const workflowContext = typeof hasWorkflowHandler === "function" && hasWorkflowHandler("_runtimeWorkflowContext") ? _runtimeWorkflowContext : _runtimeGlobalFunction("_runtimeWorkflowContext");
...
var GREP_OUTPUT_TOKEN_LIMIT = 12;
...
function extractGrepOutputTokens(text, maxItems = GREP_OUTPUT_TOKEN_LIMIT) {
  ...
  _GREP_TOKEN_KINDS.forEach((def) => {
    ...
    const kindTokens = [...counts.values()].filter((entry) => entry.count >= def.minCount).sort((a, b) => b.count - a.count || a.text.localeCompare(b.text));
...
function _activeTabOutputText() {
  ...
  if (!out || typeof out.querySelectorAll !== "function") return "";
  return Array.from(out.querySelectorAll(".line")).filter((line) => line instanceof Element && !line.classList.contains("prompt-echo")).map((line) => line.textContent || "").join("\n");
}
...
var AUTOCOMPLETE_SUGGESTIONS_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _autocompleteGlobalFunction(name) {
  const fn = AUTOCOMPLETE_SUGGESTIONS_GLOBAL && AUTOCOMPLETE_SUGGESTIONS_GLOBAL[name];
  return typeof fn === "function" ? fn : null;
}
var autocompleteCore = typeof DarklabAutocompleteCore !== "undefined" && DarklabAutocompleteCore ? DarklabAutocompleteCore : AUTOCOMPLETE_SUGGESTIONS_GLOBAL.DarklabAutocompleteCore;
function _autocompleteState() {
  const api = APP_STATE_API || AUTOCOMPLETE_SUGGESTIONS_GLOBAL.APP_STATE_API || null;
  if (api && typeof api.getState === "function") return api.getState();
  return AUTOCOMPLETE_SUGGESTIONS_GLOBAL.APP_STATE || {};
}
function _autocompleteApiFetch(url, options) {
  const api = typeof apiFetch === "function" && apiFetch || _autocompleteGlobalFunction("apiFetch");
  if (typeof api !== "function") return Promise.reject(new Error("apiFetch unavailable"));
  return options === void 0 ? api(url) : api(url, options);
}
function _autocompleteLogClientError(context, err) {
  const log = typeof logClientError === "function" && logClientError || _autocompleteGlobalFunction("logClientError");
  if (typeof log === "function") log(context, err);
}
function _autocompleteSessionId() {
  if (typeof getSessionId === "function") return String(getSessionId() || "");
  return String(AUTOCOMPLETE_SUGGESTIONS_GLOBAL.SESSION_ID || "");
}
function _isAutocompleteBlockedByActiveRun() {
  const isRunning = typeof isActiveTabRunning === "function" && isActiveTabRunning || _autocompleteGlobalFunction("isActiveTabRunning");
  return typeof isRunning === "function" && isRunning();
}
function _autocompleteWorkspaceCacheApi() {
  return {
    getDirectoryHints: typeof getWorkspaceAutocompleteDirectoryHints !== "undefined" && getWorkspaceAutocompleteDirectoryHints || _autocompleteGlobalFunction("getWorkspaceAutocompleteDirectoryHints"),
    getFileHints: typeof getWorkspaceAutocompleteFileHints !== "undefined" && getWorkspaceAutocompleteFileHints || _autocompleteGlobalFunction("getWorkspaceAutocompleteFileHints")
  };
}
function _autocompleteOnUiEvent(name, handler) {
  const onEvent = typeof onUiEvent !== "undefined" && onUiEvent || _autocompleteGlobalFunction("onUiEvent");
  if (typeof onEvent === "function") onEvent(name, handler);
}
function _autocompleteWorkspacePathHints(kind, token) {
  const readHints = typeof getWorkspaceAutocompletePathHints !== "undefined" && getWorkspaceAutocompletePathHints || _autocompleteGlobalFunction("getWorkspaceAutocompletePathHints");
  return typeof readHints === "function" ? readHints(kind, token) : [];
}
function _autocompleteWorkspaceFlagFileHints(token) {
  const readHints = typeof getWorkspaceAutocompleteFlagFileHints !== "undefined" && getWorkspaceAutocompleteFlagFileHints || _autocompleteGlobalFunction("getWorkspaceAutocompleteFlagFileHints");
...
function _autocompleteRuntimeContext(registry) {
  const readContext = typeof getRuntimeAutocompleteContext !== "undefined" && getRuntimeAutocompleteContext || _autocompleteGlobalFunction("getRuntimeAutocompleteContext");
  return typeof readContext === "function" ? readContext(registry) : {};
}
function _autocompleteRuntimeItems(ctx) {
  const readItems = typeof getRuntimeAutocompleteItems !== "undefined" && getRuntimeAutocompleteItems || _autocompleteGlobalFunction("getRuntimeAutocompleteItems");
  return typeof readItems === "function" ? readItems(ctx, autocompleteCore.buildItem, autocompleteCore.filterItems) : [];
}
function _autocompleteGrepOutputSuggestions(ctx) {
  const readItems = typeof getGrepOutputSuggestions !== "undefined" && getGrepOutputSuggestions || _autocompleteGlobalFunction("getGrepOutputSuggestions");
  return typeof readItems === "function" ? readItems(ctx, autocompleteCore.buildItem, autocompleteCore.filterItems) : [];
}
...
function _autocompletePipeContext(value, cursorPos) {
  ...
  const invalidPriorStage = priorStages.some((stage) => {
    const stageText2 = String(stage || "").trim();
    if (!stageText2 || shellControlRe.test(stageText2)) return true;
    const stageRoot = stageText2.split(/\s+/, 1)[0].toLowerCase();
...
function _hintsToItems(hints, ctx, options = {}) {
  ...
  return (Array.isArray(hints) ? hints : []).map((item) => {
    ...
    const hintOnly = isObject && item.hintOnly != null ? item.hintOnly : isObject && item.hint_only != null ? item.hint_only : null;
...
var RECENT_VALUE_LIMIT = autocompleteCore.RECENT_VALUE_LIMIT;
var RECENT_VALUE_KINDS = ["domain", "ip", "url", "port_set"];
var RECENT_VALUE_CAPTURE_TYPES = ["domain", "host", "target", "ip", "url", "port_set"];
var acRecentValues = {
  domain: [],
  ip: [],
  url: [],
...
var acRecentValuePersistPromises = /* @__PURE__ */ new Set();
var acPendingRecentValueCommands = [];
var acProjectTargets = [];
var acProjects = [];
var acSchedules = [];
var acWatchers = [];
...
function setProjectAutocompleteTargets(items) {
  const seen = /* @__PURE__ */ new Set();
  acProjectTargets = (Array.isArray(items) ? items : []).map((item) => {
...
function setProjectAutocompleteProjects(items) {
  const seen = /* @__PURE__ */ new Set();
  acProjects = (Array.isArray(items) ? items : []).map((item) => {
    if (!item || typeof item !== "object") return null;
...
function setScheduleAutocompleteSchedules(items) {
  const seen = /* @__PURE__ */ new Set();
  acSchedules = (Array.isArray(items) ? items : []).map((item) => {
    if (!item || typeof item !== "object") return null;
    const id = String(item.id || "").trim();
    if (!id) return null;
...
function setWatcherAutocompleteWatchers(items) {
  const seen = /* @__PURE__ */ new Set();
  acWatchers = (Array.isArray(items) ? items : []).map((item) => {
    if (!item || typeof item !== "object") return null;
    const id = String(item.id || "").trim();
    if (!id) return null;
    const label = String(item.label || item.command_text || id).trim();
...
function loadProjectAutocompleteTargets() {
  const projectListRequest = _autocompleteApiFetch("/projects?include_archived=1", { cache: "no-store" }).then((resp) => resp && resp.ok && typeof resp.json === "function" ? resp.json() : { projects: [] }).then((data) => setProjectAutocompleteProjects(data && data.projects));
  const activeTargetRequest = _autocompleteApiFetch("/projects/active", { cache: "no-store" }).then((resp) => resp && resp.ok && typeof resp.json === "function" ? resp.json() : { project: null }).then((data) => {
    ...
    return _autocompleteApiFetch(`/projects/${encodeURIComponent(projectId)}/targets?limit=200`, { cache: "no-store" }).then((resp) => resp && resp.ok && typeof resp.json === "function" ? resp.json() : { targets: [] }).then((targetData) => setProjectAutocompleteTargets(targetData && targetData.targets));
  });
  return Promise.all([projectListRequest, activeTargetRequest]).then(() => _readProjectTargets()).catch((err) => {
...
function loadScheduleAutocompleteHints() {
  return _autocompleteApiFetch("/schedules", { cache: "no-store" }).then((resp) => resp && resp.ok && typeof resp.json === "function" ? resp.json() : { schedules: [] }).then((data) => setScheduleAutocompleteSchedules(data && data.schedules)).catch((err) => {
...
function loadWatcherAutocompleteHints() {
  return _autocompleteApiFetch("/watchers", { cache: "no-store" }).then((resp) => resp && resp.ok && typeof resp.json === "function" ? resp.json() : { watchers: [] }).then((data) => setWatcherAutocompleteWatchers(data && data.watchers)).catch((err) => {
...
function loadRecentValues() {
  return _autocompleteApiFetch("/session/recent-values").then((resp) => resp && typeof resp.json === "function" ? resp.json() : {}).then((data) => {
...
function _persistRecentValues(items) {
  ...
  }).then((resp) => resp && typeof resp.json === "function" ? resp.json() : null).then((data) => {
    if (data && data.values && typeof data.values === "object") _setRecentValuesByKind(data.values);
    return data;
  }).catch((err) => {
...
var AUTOCOMPLETE_VALUE_TYPE_HANDLERS = {
  domain: {
    emptySlot: false,
  ...
  wordlist: {
    ...
    applySuggestions: (ctx, baseItems, slot) => slot && slot.active ? _withWordlistSuggestions(ctx, baseItems, slot.categories) : baseItems
  },
  target: {
    emptySlot: false,
...
function _valueTypeSlotFromHints(type, hints) {
  const handler = _valueTypeHandler(type);
  return handler && typeof handler.slotFromHints === "function" ? handler.slotFromHints(hints) : _emptyValueTypeSlot(type);
}
function _valueTypeSourceHints(type, spec, hints) {
  const handler = _valueTypeHandler(type);
  return handler && typeof handler.sourceHints === "function" ? handler.sourceHints(spec, hints) : null;
}
...
function _argHintTriggersForValueType(spec, type) {
  ...
  return Object.entries(argHints).filter(([trigger, hints]) => trigger !== "__positional__" && Array.isArray(hints) && hints.some((hint) => _itemValueTypeIs(hint, type))).map(([trigger]) => String(trigger || ""));
}
...
function _concreteAutocompleteTokens(spec) {
  return new Set((spec && Array.isArray(spec.flags) ? spec.flags : []).map((flag) => String(flag && flag.value || "").toLowerCase()).filter((value) => value && !value.startsWith("-") && !value.startsWith("+")));
}
function _filterFlagItems(items, query) {
  const q = String(query || "");
  if (!q) return (Array.isArray(items) ? items : []).slice();
  return (Array.isArray(items) ? items : []).filter((item) => autocompleteCore.itemInsertValue(item).startsWith(q));
}
...
function _positionalHintsForSlot(spec, slotIndex) {
  const hints = spec && spec.arg_hints && Array.isArray(spec.arg_hints.__positional__) ? spec.arg_hints.__positional__ : [];
...
function _positionalHintSlotsForValueType(spec, type) {
  ...
  return Array.from({ length: maxPosition }, (_, index) => _valueTypeSlotFromHints(type, _positionalHintsForSlot(spec, index)));
}
function _walkAutocompletePositionalValues(ctx, spec, contextSpec = {}, visitor = () => {
}, options = {}) {
  const expectsValue = Array.isArray(spec && spec.expects_value) ? spec.expects_value : [];
  const expectsExact = new Set(expectsValue.map((token) => String(token || "")));
  const concreteTokens = options.skipConcreteTokens ? _concreteAutocompleteTokens(spec) : /* @__PURE__ */ new Set();
  const subTokens = contextSpec && Array.isArray(contextSpec.subcommandTokens) ? contextSpec.subcommandTokens : contextSpec && contextSpec.subcommandToken ? [contextSpec.subcommandToken] : [];
  const tokens = Array.isArray(options.tokens) ? options.tokens : ctx.tokens.filter((token) => token.end <= ctx.tokenStart);
...
function _recentValueKindForToken(type, tokenValue) {
  ...
  const candidates = normalizedType === "host" ? ["domain", "ip"] : normalizedType === "target" ? ["domain", "ip", "url", "port_set"] : [normalizedType];
...
function _positionalHintSlotsForRecentValues(spec) {
  const perTypeSlots = RECENT_VALUE_CAPTURE_TYPES.map((type) => _positionalHintSlotsForValueType(spec, type).map((slot, index) => slot && _recentValuePositionalSlotCapturable(spec, index)));
...
function _hintLooksLikeFileInput(hint) {
  ...
  return /(^|[\s_-])(file|files|wordlist|path|paths)([\s_-]|$)/.test(text) || /\bone [\w/-]+ per line\b/.test(text) || /<[^>]*(?:file|list|path|wordlist)[^>]*>/.test(text);
}
function _recentValueHintsCapturable(hints) {
  const list = Array.isArray(hints) ? hints : [];
  return list.some((hint) => RECENT_VALUE_CAPTURE_TYPES.some((type) => _itemValueTypeIs(hint, type))) && !list.some(_hintLooksLikeFileInput);
}
function _recentValueTriggerCapturable(spec, trigger) {
  const key = String(trigger || "");
  if (!key) return false;
  const workspaceFlags = new Set((Array.isArray(spec && spec.workspace_file_flags) ? spec.workspace_file_flags : []).map((flag) => String(flag || "")).filter(Boolean));
  if (workspaceFlags.has(key)) return false;
  const hints = spec && spec.arg_hints && Object.prototype.hasOwnProperty.call(spec.arg_hints, key) ? spec.arg_hints[key] : [];
...
function _storeRecentValues(found) {
  ...
  const next = [];
  found.concat(RECENT_VALUE_KINDS.flatMap((kind) => _readRecentValues(kind).map((value) => ({ kind, value })))).forEach((item) => {
...
function _recentValueAutocompleteItems(ctx, kinds = []) {
  return (Array.isArray(kinds) ? kinds : []).flatMap((kind) => _readRecentValues(kind).map((value) => ({ kind, value }))).map((item) => autocompleteCore.buildItem({
    value: item.value,
...
function _projectTargetAutocompleteItems(ctx, allowedTypes = []) {
  const allowed = new Set((Array.isArray(allowedTypes) ? allowedTypes : []).map((type) => String(type || "").trim().toLowerCase()).filter(Boolean));
  return _readProjectTargets().filter((target) => !allowed.size || allowed.has(target.type)).map((target) => autocompleteCore.buildItem({
    value: target.value,
    description: [
      "Project target",
      target.type,
...
function _workspaceAutocompleteHintsForFlag(spec, trigger, ctx = null) {
  ...
  if (ctx) {
    const hints2 = _autocompleteWorkspaceFlagFileHints(ctx.currentToken);
    return Array.isArray(hints2) ? hints2 : [];
  }
  const getFileHints = _autocompleteWorkspaceCacheApi().getFileHints;
...
function _workspaceAutocompleteEntryHints() {
  ...
  const fileHints = typeof workspaceCache.getFileHints === "function" ? workspaceCache.getFileHints() : [];
  const directoryHints = typeof workspaceCache.getDirectoryHints === "function" ? workspaceCache.getDirectoryHints() : [];
  return [
...
function _autocompleteWorkspacePathKind(ctx, spec) {
  ...
  for (let index = 1; index < completedTokens.length; index += 1) {
    ...
    const argIndex = completedTokens.slice(index + 1).filter((token) => {
...
function _resolveAutocompleteHintSource(ctx, spec, baseHints, options = {}) {
  const workspaceFlag = Object.prototype.hasOwnProperty.call(options, "workspaceFlag") ? options.workspaceFlag : null;
  const workspaceHints = workspaceFlag != null ? _workspaceAutocompleteHintsForFlag(spec, workspaceFlag, ctx) : null;
  ...
  const workspaceTargetHints = allowWorkspaceTarget && !_autocompleteSpecHasWorkspacePathKinds(spec) ? _workspaceAutocompleteHintsForTargetSlot(spec, baseHints) : null;
  return {
...
function _contextClosedByTokenArity(ctx, spec) {
  const closeAfter = spec && spec.close_after && typeof spec.close_after === "object" ? spec.close_after : {};
...
function _mergeAutocompleteSpecForSubcommand(baseSpec, subSpec) {
  ...
  const expectsValue = [];
  const seenValueTokens = /* @__PURE__ */ new Set();
...
function _autocompleteSpecForContext(ctx, spec) {
  ...
  for (let index = 1; index < ctx.tokens.length; index += 1) {
    const subcommands = activeSpec && activeSpec.subcommands && typeof activeSpec.subcommands === "object" ? activeSpec.subcommands : {};
...
function _buildUniqueSubcommandExampleAutocomplete(ctx, rootSpec) {
  const subcommands = rootSpec && rootSpec.subcommands && typeof rootSpec.subcommands === "object" ? rootSpec.subcommands : {};
...
function _buildContextAutocomplete(ctx) {
  ...
  if (contextSpec.activeSubcommand && spec.examples && spec.examples.length) {
    ...
    const subcommandIsCurrentToken = contextSpec.subcommandToken && ctx.tokenStart === contextSpec.subcommandToken.start && ctx.tokenEnd === contextSpec.subcommandToken.end;
  ...
  const argHints = spec.arg_hints || {};
  const argumentLimit = Number.isInteger(spec.argument_limit) && spec.argument_limit > 0 ? spec.argument_limit : null;
  const allowPositionalHints = !argumentLimit || _countCompletedPositionalArgs(ctx, spec) < argumentLimit;
  const directHints = Object.prototype.hasOwnProperty.call(argHints, ctx.previousToken || "") ? argHints[ctx.previousToken || ""] : null;
  const completedTokens = ctx.tokens.filter((token) => token.end <= ctx.tokenStart);
  const sequenceArgHints = spec.sequence_arg_hints || {};
  const priorToken = completedTokens.length >= 2 ? String(completedTokens[completedTokens.length - 2].value || "").toLowerCase() : "";
  const previousLower = String(ctx.previousToken || "").toLowerCase();
  const sequenceKey = `${priorToken} ${previousLower}`.trim();
  const sequenceHints = Object.prototype.hasOwnProperty.call(sequenceArgHints, sequenceKey) ? sequenceArgHints[sequenceKey] : null;
  ...
  if (allowPositionalHints) {
    const concreteCommandTokens = (spec.flags || []).filter((flag) => {
  ...
  const positionalHints = Object.prototype.hasOwnProperty.call(argHints, "__positional__") ? _positionalHintsForSlot(
    spec,
    _countCompletedPositionalValues(ctx, spec, contextSpec)
  ) : [];
  if (!ctx.currentToken || currentIsFlag) {
    const usedFlags = new Set(
      ctx.tokens.filter((token) => token.start !== ctx.tokenStart).map((token) => String(token.value || "")).filter((token) => token.startsWith("-") || token.startsWith("+"))
    );
    const flags = (spec.flags || []).filter((flag) => !usedFlags.has(String(flag.value || ""))).map((flag) => autocompleteCore.buildItem({
      value: flag.value,
      description: flag.description || "",
      replaceStart: ctx.tokenStart,
      replaceEnd: ctx.tokenEnd,
  ...
  const resolvedWorkspacePathHints = allowPositionalHints ? _resolveAutocompleteHintSource(ctx, spec, [], { allowWorkspaceTarget: false }) : null;
...
function _buildPipeCommandAutocomplete(ctx, registry) {
  const items = Object.entries(registry).filter(([, spec]) => spec && spec.pipe_command).map(([root, spec]) => autocompleteCore.buildItem({
...
function getAutocompleteMatches(value, cursorPos) {
  ...
  const singleItemIsFlag = String(singleItem && singleItem.value || "").startsWith("-") || String(singleItem && singleItem.value || "").startsWith("+");
  if (items.length === 1 && !singleItem.hintOnly && !singleItemIsFlag && autocompleteCore.itemInsertValue(singleItem).toLowerCase() === ctx.currentToken.toLowerCase()) {
    return [];
  }
  return items;
}
...
var AUTOCOMPLETE_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _autocompleteGlobalFunction2(name) {
  const fn = AUTOCOMPLETE_GLOBAL && AUTOCOMPLETE_GLOBAL[name];
  return typeof fn === "function" ? fn : null;
}
var autocompleteDropdownCore = typeof DarklabAutocompleteCore !== "undefined" && DarklabAutocompleteCore ? DarklabAutocompleteCore : AUTOCOMPLETE_GLOBAL.DarklabAutocompleteCore;
var _autocompleteDom = {
  acDropdown: typeof acDropdown !== "undefined" && acDropdown || AUTOCOMPLETE_GLOBAL.acDropdown || null,
  cmdInput: typeof cmdInput !== "undefined" && cmdInput || AUTOCOMPLETE_GLOBAL.cmdInput || null,
  mobileCmdInput: typeof mobileCmdInput !== "undefined" && mobileCmdInput || AUTOCOMPLETE_GLOBAL.mobileCmdInput || null,
  mobileComposerHost: typeof mobileComposerHost !== "undefined" && mobileComposerHost || AUTOCOMPLETE_GLOBAL.mobileComposerHost || null,
  mobileComposerRow: typeof mobileComposerRow !== "undefined" && mobileComposerRow || AUTOCOMPLETE_GLOBAL.mobileComposerRow || null,
  shellPromptWrap: typeof shellPromptWrap !== "undefined" && shellPromptWrap || AUTOCOMPLETE_GLOBAL.shellPromptWrap || null
};
function _autocompleteGetState() {
  const readState = typeof getAutocompleteState !== "undefined" && getAutocompleteState || _autocompleteGlobalFunction2("getAutocompleteState");
  return typeof readState === "function" ? readState() : {};
}
function _autocompleteSetState(next) {
  const writeState = typeof setAutocompleteState !== "undefined" && setAutocompleteState || _autocompleteGlobalFunction2("setAutocompleteState");
  if (typeof writeState === "function") writeState(next);
}
function _autocompleteComposerState() {
  const readState = typeof getComposerState !== "undefined" && getComposerState || _autocompleteGlobalFunction2("getComposerState");
  return typeof readState === "function" ? readState() : {};
}
function _autocompleteComposerValue() {
  const readValue = typeof getComposerValue !== "undefined" && getComposerValue || _autocompleteGlobalFunction2("getComposerValue");
...
function _autocompleteSetComposerValue(value, start = null, end = null) {
  const writeValue = typeof setComposerValue !== "undefined" && setComposerValue || _autocompleteGlobalFunction2("setComposerValue");
  if (typeof writeValue === "function") writeValue(value, start, end);
}
function _autocompleteVisibleInput() {
  const readInput = typeof getVisibleComposerInput !== "undefined" && getVisibleComposerInput || _autocompleteGlobalFunction2("getVisibleComposerInput");
  return typeof readInput === "function" ? readInput() : _autocompleteDom.cmdInput;
}
function _autocompleteHideDropdown() {
  const hideDropdown = typeof hideAcDropdown !== "undefined" && hideAcDropdown || _autocompleteGlobalFunction2("hideAcDropdown");
  if (typeof hideDropdown === "function") hideDropdown();
}
function _autocompleteShowDropdown() {
  const showDropdown = typeof showAcDropdown !== "undefined" && showAcDropdown || _autocompleteGlobalFunction2("showAcDropdown");
  if (typeof showDropdown === "function") showDropdown();
}
function _autocompleteRefocusComposer() {
  const refocus = typeof refocusComposerAfterAction !== "undefined" && refocusComposerAfterAction || _autocompleteGlobalFunction2("refocusComposerAfterAction");
  if (typeof refocus === "function") refocus({ preventScroll: true });
}
function _autocompleteEscapeHtml(value) {
  const escape = typeof escapeHtml !== "undefined" && escapeHtml || _autocompleteGlobalFunction2("escapeHtml");
  return typeof escape === "function" ? escape(value) : String(value || "");
}
function _isAutocompleteBlockedByTerminalConfirm() {
  const hasPending = typeof hasPendingTerminalConfirm === "function" && hasPendingTerminalConfirm || _autocompleteGlobalFunction2("hasPendingTerminalConfirm");
  return typeof hasPending === "function" && hasPending();
}
function _isAutocompleteBlockedByActiveRun2() {
  const isRunning = typeof isActiveTabRunning === "function" && isActiveTabRunning || _autocompleteGlobalFunction2("isActiveTabRunning");
  return typeof isRunning === "function" && isRunning();
}
...
function _autocompleteTokenContextForValue(value, cursor) {
  const tokenContext = typeof _autocompleteTokenContext !== "undefined" && _autocompleteTokenContext || _autocompleteGlobalFunction2("_autocompleteTokenContext");
  return typeof tokenContext === "function" ? tokenContext(value, cursor) : { currentToken: String(value || "") };
}
function _positionAutocomplete(itemsCount) {
  const acDropdown2 = _autocompleteDom.acDropdown;
  if (!acDropdown2) return false;
  const parentWrap = acDropdown2.parentElement;
  const wrap = parentWrap && parentWrap.classList?.contains("shell-prompt-wrap") ? parentWrap : _autocompleteDom.shellPromptWrap || parentWrap;
  ...
  if (!mobileTerminalMode && anchor && typeof anchor.getBoundingClientRect === "function") {
    ...
    if (anchorRect.width <= 0 && anchorRect.height <= 0 && anchorRect.top === 0 && anchorRect.bottom === 0 && visibleInput) {
      anchor = visibleInput;
    }
  }
  const prefix = anchor === wrap && wrap && wrap.querySelector ? wrap.querySelector(".prompt-prefix") : null;
  acDropdown2.classList.toggle("ac-mobile", mobileTerminalMode);
  if (mobileTerminalMode) {
    const rect2 = anchor && typeof anchor.getBoundingClientRect === "function" ? anchor.getBoundingClientRect() : { top: 0 };
    const rowH2 = 44;
    const desired2 = Math.min(8, Math.max(1, itemsCount)) * rowH2 + 10;
    const maxHeight2 = Math.max(88, Math.min(360, desired2));
    acDropdown2.style.position = "absolute";
    acDropdown2.style.left = "0";
    acDropdown2.style.right = "0";
    acDropdown2.style.width = "100%";
    acDropdown2.style.minWidth = "0";
    acDropdown2.style.maxHeight = `${Math.round(maxHeight2)}px`;
    acDropdown2.style.top = "auto";
    acDropdown2.style.bottom = "calc(100% + 4px)";
    acDropdown2.classList.add("ac-up");
    acDropdown2.classList.add("dropdown-up");
    return true;
  }
  acDropdown2.classList.remove("ac-mobile");
  ...
  acDropdown2.style.position = "fixed";
  acDropdown2.style.left = `${Math.max(0, Math.round((wrapRect ? wrapRect.left : 0) + prefixOffset))}px`;
  acDropdown2.style.right = "auto";
  acDropdown2.style.minWidth = mobileComposerMode ? "0" : "24ch";
  acDropdown2.style.width = mobileComposerMode && wrapRect ? `${Math.max(220, Math.round(wrapRect.width || 0))}px` : "";
  if (!anchor || typeof anchor.getBoundingClientRect !== "function") {
    acDropdown2.classList.remove("ac-up");
    acDropdown2.classList.remove("dropdown-up");
    return false;
  }
  const rect = anchor.getBoundingClientRect();
  const rowH = 22;
  ...
  acDropdown2.classList.toggle("ac-up", showAbove);
  acDropdown2.classList.toggle("dropdown-up", showAbove);
  ...
  acDropdown2.style.maxHeight = `${Math.round(maxHeight)}px`;
  if (showAbove) {
    acDropdown2.style.top = "auto";
    acDropdown2.style.bottom = `${Math.max(8, Math.round(window.innerHeight - rect.top + 2))}px`;
  } else {
    acDropdown2.style.top = `${Math.max(8, Math.round(rect.bottom + 2))}px`;
    acDropdown2.style.bottom = "auto";
  }
  return showAbove;
}
function _scrollAutocompleteActiveItem() {
  const acDropdown2 = _autocompleteDom.acDropdown;
  if (!acDropdown2) return;
  const activeItem = acDropdown2.querySelector(".ac-item.ac-active");
  if (!activeItem) return;
  const viewHeight = acDropdown2.clientHeight || 0;
  ...
  if (viewHeight > 0 && itemTop !== null && itemHeight !== null) {
    ...
    const viewTop = acDropdown2.scrollTop || 0;
    const viewBottom = viewTop + viewHeight;
    const padding = 4;
    if (itemTop < viewTop + padding) {
      acDropdown2.scrollTop = Math.max(0, itemTop - padding);
    } else if (itemBottom > viewBottom - padding) {
      acDropdown2.scrollTop = Math.max(0, itemBottom - viewHeight + padding);
    }
    return;
  }
...
function acSelectableIndexes(items) {
  return (Array.isArray(items) ? items : []).map((item, index) => acIsHintOnly(item) ? -1 : index).filter((index) => index >= 0);
}
...
function acShow(items) {
  if (_isAutocompleteBlockedByTerminalConfirm() || _isAutocompleteBlockedByActiveRun2()) {
    acHide();
    return;
  }
  const acDropdown2 = _autocompleteDom.acDropdown;
  if (!acDropdown2) return;
  acDropdown2.innerHTML = "";
  if (!items.length) {
  ...
  const cmdInput3 = _autocompleteDom.cmdInput;
  const currentCursor = typeof composerState.selectionStart === "number" ? composerState.selectionStart : cmdInput3 && typeof cmdInput3.selectionStart === "number" ? cmdInput3.selectionStart : currentValue.length;
  ...
  const maxExampleLabelLen = items.reduce((max, s) => s && s.isExample ? Math.max(max, autocompleteDropdownCore.itemText(s).length) : max, 0);
  ...
  items.forEach((s, i) => {
    ...
    const div = document.createElement("div");
    div.className = "ac-item dropdown-item dropdown-item-dense" + (!hintOnly && i === nextIndex ? " ac-active dropdown-item-active" : "") + (s && s.isExample ? " ac-example" : "") + (hintOnly ? " ac-hint-only" : "") + (hintSeparated ? " ac-hint-separated" : "");
    ...
    const val = s && typeof s === "object" && s.matchQuery != null ? String(s.matchQuery || "") : String(matchValue || "");
    const main = document.createElement("span");
    main.className = "ac-item-main";
    if (s && s.isExample && maxExampleLabelLen > 0) main.style.minWidth = maxExampleLabelLen + "ch";
    main.innerHTML = hintOnly ? _autocompleteEscapeHtml(label) : autocompleteDropdownCore.highlightedLabel(label, val);
...
var _histSearchMode = false;
var _histSearchQuery = "";
var _histSearchIndex = -1;
var _histSearchPreDraft = "";
var _histSearchRuns = null;
var _histSearchFetchTimer = null;
var HISTORY_SEARCH_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _historySearchApiFetch(...args) {
  const fetcher = (typeof hasRuntimeHandler === "function" && hasRuntimeHandler("apiFetch") && typeof apiFetch === "function" ? apiFetch : null) || (typeof HISTORY_SEARCH_GLOBAL.apiFetch === "function" ? HISTORY_SEARCH_GLOBAL.apiFetch : null);
...
function _historySearchCmdHistory() {
  const state = typeof getAppState === "function" ? getAppState() : typeof HISTORY_SEARCH_GLOBAL.APP_STATE_API?.getState === "function" ? HISTORY_SEARCH_GLOBAL.APP_STATE_API.getState() : HISTORY_SEARCH_GLOBAL.APP_STATE || null;
  if (state && Array.isArray(state.cmdHistory)) return state.cmdHistory;
  return [];
}
function _historySearchCmdInput() {
  return typeof cmdInput !== "undefined" && cmdInput || HISTORY_SEARCH_GLOBAL.cmdInput || null;
}
function _historySearchDropdown() {
  return typeof histSearchDropdown !== "undefined" && histSearchDropdown || HISTORY_SEARCH_GLOBAL.histSearchDropdown || null;
}
function _historySearchPromptWrap() {
  return typeof shellPromptWrap !== "undefined" && shellPromptWrap || HISTORY_SEARCH_GLOBAL.shellPromptWrap || null;
}
function _historySearchGetComposerValue() {
  const getter = typeof getComposerValue !== "undefined" && getComposerValue || HISTORY_SEARCH_GLOBAL.getComposerValue;
  const input = _historySearchCmdInput();
  return typeof getter === "function" ? getter() : input ? input.value : "";
}
function _historySearchSetComposerValue(...args) {
  const setter = typeof setComposerValue !== "undefined" && setComposerValue || HISTORY_SEARCH_GLOBAL.setComposerValue;
  if (typeof setter === "function") setter(...args);
}
function _historySearchHideAutocomplete() {
  const hide = typeof acHide !== "undefined" && acHide || HISTORY_SEARCH_GLOBAL.acHide;
  if (typeof hide === "function") hide();
}
function isHistSearchMode() {
  return _histSearchMode;
}
...
function _histSearchFetch(q) {
  ...
  _historySearchApiFetch(url).then((r) => r.json()).then((data) => {
    ...
    _histSearchRuns = Array.isArray(data.runs) ? [...new Set(data.runs.map((r) => r.command))] : [];
...
function _renderHistSearch() {
  ...
  if (!matches.length) {
    ...
    matches.forEach((cmd, i) => {
      const item = document.createElement("div");
      item.className = "hist-search-item dropdown-item dropdown-item-compact" + (i === _histSearchIndex ? " active dropdown-item-active" : "");
      if (_histSearchQuery) {
  ...
  if (promptWrap) {
    ...
    dropdown.style.top = showAbove ? `${Math.max(8, Math.round(rect.top - visibleHeight - 4))}px` : `${Math.max(8, Math.round(rect.bottom + 4))}px`;
  }
}
...
function exitHistSearch(accept, { keepCurrent = false } = {}) {
  ...
  _histSearchPreDraft = "";
  _histSearchRuns = null;
  if (_histSearchFetchTimer) {
    clearTimeout(_histSearchFetchTimer);
...
export {
  ...
  openWorkflowEditor,
  closeWorkflowEditor,
  setStatus2 as setStatus,
  restoreActiveRunsAfterReload,
  attachActiveRunFromMonitor2 as attachActiveRunFromMonitor,
  killActiveRunFromMonitor2 as killActiveRunFromMonitor,
  appendCommandEcho2 as appendCommandEcho,
  _markTabRunStarted,
  pauseBackgroundRunStreamsForStatusMonitor2 as pauseBackgroundRunStreamsForStatusMonitor,
  resumeBackgroundRunStreamsAfterStatusMonitor2 as resumeBackgroundRunStreamsAfterStatusMonitor,
  interruptPromptLine2,
  confirmKill2,
  _recordSuccessfulLocalCommand2 as _recordSuccessfulLocalCommand,
  _finalizeClientSideCommandStatus,
  _persistClientSideRun,
  _seedLocalStorageStarsToServer2 as _seedLocalStorageStarsToServer,
  _setPendingTerminalConfirm,
  hasPendingTerminalConfirm2 as hasPendingTerminalConfirm,
  cancelPendingTerminalConfirm2,
  submitComposerCommand2,
  submitVisibleComposerCommand2,
  runCommand2,
  welcomeOwnsTab2 as welcomeOwnsTab,
  cancelWelcome2,
  ...
  exportTabPdf,
  permalinkTab,
  _getTabPanelEl2 as _getTabPanelEl,
  createDefaultTabLabel2 as createDefaultTabLabel,
  setupTabScrollControls,
  updateNewTabBtn2 as updateNewTabBtn,
  updateOutputFollowButton2 as updateOutputFollowButton,
  createTab2 as createTab,
  activateTab2 as activateTab,
  setTabStatus2 as setTabStatus,
  setTabLabel2 as setTabLabel,
  ...
  applyFaqHashTarget,
  renderFaqLimits,
  setAllowedCommandsFaqData2 as setAllowedCommandsFaqData,
  ...
  buildPromptLabel2 as buildPromptLabel,
  hasPendingOutputBatch,
  ...
  getClientId,
  maskSessionToken2,
  apiFetch2 as apiFetch,
  logClientError2 as logClientError,
  DarklabEntityMetadata
};
