// app/static/js/features/projects/project_details.js
var exportedDarklabProjectDetails = null;
(function projectDetailsModule(global) {
  ...
  function createProjectDetailsController(context) {
    ...
    let notesSaveSeq = 0;
    const targetPageState = /* @__PURE__ */ new Map();
    const targetSearchTimers = /* @__PURE__ */ new Map();
    const targetTypes = [
      { id: "", label: "All" },
    ...
    function syncForms(project = ctx.selectedProject()) {
      ...
      if (ctx.projectLabelsInput && document.activeElement !== ctx.projectLabelsInput) {
        ...
        ctx.projectLabelsInput.placeholder = hasProject ? `Labels for ${ctx.projectDisplayName(project)}` : "Select a project to edit labels";
      }
      ...
      if (ctx.projectNotesInput && document.activeElement !== ctx.projectNotesInput && !hasPendingNotesEdit) {
        ...
        ctx.projectNotesInput.placeholder = hasProject ? `Notes for ${ctx.projectDisplayName(project)}` : "Select a project to edit notes";
      }
    }
    ...
    function cacheNotes(projectId, notes, updatedProject = null) {
      ...
      const replacement = updatedProject && typeof updatedProject === "object" ? updatedProject : null;
    ...
    function cacheLabels(projectId, labels) {
      ...
      if (!normalizedProjectId) return;
      const update = (project) => String(project && project.id || "") === normalizedProjectId ? { ...project, labels: labelItems } : project;
    ...
    function normalizeTargetCountsByType(counts) {
      ...
      return Object.fromEntries(
        Object.entries(counts).map(([key, value]) => [String(key || ""), Math.max(0, Number(value || 0))]).filter(([key]) => !!key)
      );
    }
    ...
    function seedTargetPageFromSummary(summary, state) {
      const existingEmptyPage = state.page && Array.isArray(state.page.targets) && state.page.targets.length === 0 && Number(state.page.total || 0) === 0;
      ...
      const hasExplicitTargetCounts = Object.prototype.hasOwnProperty.call(counts, "targets") || Object.prototype.hasOwnProperty.call(counts, "pending_targets");
    ...
    function renderTargetTypeTabs(projectId, summary, state) {
      ...
      targetTypes.forEach((item) => {
        ...
        const count = item.id ? Number(counts[item.id] || 0) : Object.prototype.hasOwnProperty.call(counts, "all") ? Number(counts.all || 0) : Object.values(counts).reduce((sum, value) => sum + Number(value || 0), 0);
