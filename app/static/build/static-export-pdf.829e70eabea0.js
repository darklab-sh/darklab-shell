...
// app/static/js/export_pdf.js
var ExportHtmlUtils = typeof window !== "undefined" ? window.ExportHtmlUtils : null;
var ExportPdfUtils = null;
(function() {
  ...
  function themeColors2() {
    const themeVars = ExportHtmlUtils && typeof ExportHtmlUtils.getThemeExportVars === "function" ? ExportHtmlUtils.getThemeExportVars() : null;
    ...
    return {
      bg: parseCssColor2(v("--bg")),
      surface: parseCssColor2(v("--surface")),
      border: parseCssColor2(v("--border")),
      panelBorder: parseCssColor2(v("--theme-panel-border") || v("--border")),
      text: parseCssColor2(v("--text")),
      muted: parseCssColor2(v("--muted")),
      green: parseCssColor2(v("--green")),
      greenDim: parseCssColor2(v("--green-dim") || v("--green")),
      red: parseCssColor2(v("--red")),
      amber: parseCssColor2(v("--amber")),
      blue: parseCssColor2(v("--blue"))
    };
  }
  ...
  function parseAnsiSegments(rawText, defaultColor, ansiToHtml) {
    ...
    for (const node of div.childNodes) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE && node.style.color) {
        segments.push({ text, color: parseCssColor2(node.style.color), fontStyle: "normal" });
      } else {
  ...
  function buildPdfLineSegments({ text, cls, kind, role, colors, ansiToHtml }) {
    ...
    const lineEvent = exportHtmlUtils && typeof exportHtmlUtils.lineEventFromWire === "function" ? exportHtmlUtils.lineEventFromWire({ text, cls, kind, role }) : { text: String(text || ""), kind: String(kind || (cls === "notice" ? "notice" : "info")), role: String(role || (["prompt-echo", "denied", "exit-ok", "exit-fail"].includes(cls) ? cls : "body")) };
  ...
  async function buildTerminalExportPdf2({ jsPDF, appName, metaLine, runMeta, rawLines, getPrefix, ansiToHtml }) {
    ...
    const headerModel = ExportHtmlUtils && typeof ExportHtmlUtils.buildExportHeaderModel === "function" ? ExportHtmlUtils.buildExportHeaderModel({ appName, metaLine, runMeta }) : {
    ...
    const summary = ExportHtmlUtils && typeof ExportHtmlUtils.buildExportLineSummary === "function" ? ExportHtmlUtils.buildExportLineSummary(rawLines) : null;
    if (summary) {
      const summaryParts = [];
    ...
    const checkPage = () => {
      if (y + leading > pageH - outputBoxPadBottom) newPage();
    };
  ...
  ExportPdfUtils = {
    loadJsPdf: loadJsPdf2,
    parseCssColor: parseCssColor2,
    themeColors: themeColors2,
    buildTerminalExportPdf: buildTerminalExportPdf2
  };
})();
var {
