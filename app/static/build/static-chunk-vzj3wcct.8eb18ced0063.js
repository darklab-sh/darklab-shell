import {
  exportedDarklabProjectFindingsData
} from "./static-chunk-wkdqs5l5.75c18d0d56e7.js";
import {
  DarklabFindingTriageEditor
} from "./static-chunk-aj3py647.674015cfb0e1.js";
import {
  restoreHistoryRunIntoTab
} from "./static-chunk-ftlux7er.217f2e307607.js";
import {
  DarklabTeamScope,
  activeTeamScopeCan,
  apiFetch,
...
// app/static/js/features/findings/findings_board_modal.js
var exportedOpenFindingsBoard = null;
var exportedCloseFindingsBoard = null;
var exportedIsFindingsBoardOpen = null;
(function findingsBoardModalModule(global) {
  ...
  const emitUiEvent2 = typeof emitUiEvent !== "undefined" && emitUiEvent || null;
  const findingTriageEditor = typeof DarklabFindingTriageEditor !== "undefined" && DarklabFindingTriageEditor || null;
  const projectFindingsData = typeof exportedDarklabProjectFindingsData !== "undefined" && exportedDarklabProjectFindingsData || {};
  ...
  function canTriageFindings() {
    return teamScope && typeof teamScope.activeTeamScopeCan === "function" ? teamScope.activeTeamScopeCan("triage_findings") : true;
  }
  function triageDeniedMessage() {
    return teamScope && typeof teamScope.deniedMessage === "function" ? teamScope.deniedMessage("triage team findings") : "View-only team members can't triage team findings. Switch to Personal or ask for operator access.";
  }
  ...
  function encodeParams(params = {}) {
    ...
    Object.entries(params).forEach(([key, value]) => {
      if (Array.isArray(value)) {
        value.forEach((item) => {
          if (item !== void 0 && item !== null && String(item) !== "") query.append(key, String(item));
        });
        return;
      }
  ...
  function setFindingReviewState(findingId, reviewState) {
    const normalized = String(findingId || "");
    state.findings = state.findings.map((finding) => String(finding && finding.id || "") === normalized ? { ...finding, review_state: reviewState } : finding);
  }
  function updateFindingTriage(findingId, triage) {
    const normalized = String(findingId || "");
    const compact = triage && typeof triage === "object" ? triage : {};
    state.findings = state.findings.map((finding) => String(finding && finding.id || "") === normalized ? {
      ...finding,
      triage: compact,
  ...
  function render() {
    ...
    const columns = boardApi().boardColumnsFromFindings ? boardApi().boardColumnsFromFindings(state.findings, { limit: COLUMN_LIMIT }).columns.map((column) => {
  ...
  async function loadColumnPageFindings() {
    const requests = COLUMN_STATE_QUERIES.flatMap(({ column, states }) => states.map((reviewState) => ({ column, endpoint: endpointWithReviewState(state.endpoint, reviewState) })));
  ...
  async function loadFindings() {
    ...
    try {
      ...
      if (typeof logClientError === "function") logClientError("failed to load findings board", err);
    } finally {
      setBusy(false);
      render();
    }
  ...
  async function persistReviewState(findingId, reviewState, previousReviewState) {
    ...
    try {
      ...
      const resp = await api()(
        projectScoped ? `/projects/${encodeURIComponent(state.projectId)}/findings/review` : `/findings/${encodeURIComponent(findingId)}/review`,
        {
          method: projectScoped ? "POST" : "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(
            projectScoped ? { finding_ids: [findingId], review_state: reviewState } : { review_state: reviewState }
          )
        }
      );
      if (!resp.ok) {
        ...
        try {
          ...
          if (data2 && data2.error === "team_forbidden") message = triageDeniedMessage();
          else if (data2 && typeof data2.message === "string" && data2.message.trim()) message = data2.message.trim();
        } catch (_) {
        }
        throw new Error(message);
      }
      const data = await resp.json().catch(() => null);
      const authoritativeReviewState = data && data.finding ? data.finding.review_state : data?.review_state;
      ...
      if (typeof emitUiEvent2 === "function") {
        emitUiEvent2("app:project-workspace-mutated", {
      ...
      if (typeof logClientError === "function") logClientError("failed to update findings board review state", err);
      render();
    }
  }
  async function openFindingTriage(findingId) {
    ...
    await findingTriageEditor.open(finding, {
      ...
      onSaved: async (triage) => {
        const compact = typeof findingTriageEditor.compactTriage === "function" ? findingTriageEditor.compactTriage(triage) : triage;
  ...
  bodyEl?.addEventListener("click", (event) => {
    ...
    if (action.dataset.findingsBoardAction === "open-run") {
      event.preventDefault();
      let opened = false;
      if (typeof restoreHistoryRunIntoTab2 === "function") {
        const lineIndex = Number(action.dataset.lineIndex || "");
        void restoreHistoryRunIntoTab2({
...
export {
  ...
  exportedCloseFindingsBoard,
  exportedIsFindingsBoardOpen
};
