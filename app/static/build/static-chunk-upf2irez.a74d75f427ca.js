...
var DarklabStatusMonitorData = /* @__PURE__ */ (() => {
  ...
  function create({ apiFetch }) {
    ...
    async function refreshDashboardData({ cachedInsights = null, includeInsights = false } = {}) {
      ...
      return {
        status: status.status === "fulfilled" ? status.value : { error: status.reason?.message || "Unavailable" },
        workspace: workspace.status === "fulfilled" ? workspace.value : { enabled: false, error: workspace.reason?.message || "Unavailable" },
        stats: stats.status === "fulfilled" ? stats.value : { error: stats.reason?.message || "Unavailable" },
        insights: shouldLoadInsights ? insights.status === "fulfilled" ? insights.value : { error: insights.reason?.message || "Unavailable" } : cachedInsights,
