// app/static/js/features/projects/project_workspace_renderer.js
var exportedDarklabProjectWorkspaceRenderer = null;
(function projectWorkspaceRendererModule(global) {
  ...
  function createProjectWorkspaceRendererController(context) {
    ...
    function renderExplorer() {
      ...
      const filterBar = ["runs", "entities", "findings", "artifacts"].includes(activeTab) ? ctx.renderProjectFilterBar(projectId, summary) : null;
      body.append(header);
      ...
      if (ctx.workspaceTab() === "findings" || ["runs", "artifacts"].includes(ctx.workspaceTab()) || ctx.projectPackageWizardActive(projectId)) {
    ...
    function renderWorkspace() {
      if (ctx.projectWorkspaceSubtitle) {
        ...
        ctx.projectWorkspaceSubtitle.textContent = count ? `${count} project workspace${count === 1 ? "" : "s"} in this session.` : "Select a project to review its targets, runs, findings, artifacts, and packages.";
      }
