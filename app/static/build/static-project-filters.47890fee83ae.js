// app/static/js/features/projects/project_filters.js
var exportedDarklabProjectFilters = null;
(function projectFiltersModule(global) {
  ...
  function createProjectFiltersController(context) {
    ...
    const statusFilters = /* @__PURE__ */ new Map();
    const labelFilters = /* @__PURE__ */ new Map();
    const noteStateFilters = /* @__PURE__ */ new Map();
    const orphanFilters = /* @__PURE__ */ new Map();
    const findingSort = /* @__PURE__ */ new Map();
    ...
    function findingCommandFilterSet(projectId = "") {
      ...
      if (!commandFilters.has(normalized)) {
        commandFilters.set(normalized, /* @__PURE__ */ new Set());
      }
    ...
    function findingCommandOptions(projectId = "", summary = ctx.projectSummary?.(projectId)) {
      ...
      ctx.projectFindingItems(projectId).forEach((finding) => {
        const root = String(finding && finding.command_root || "").trim().toLowerCase() || findingCommandRoot(finding && finding.run_command);
        if (root) roots.add(root);
      });
      return [...roots].sort((left, right) => left.localeCompare(right, void 0, { sensitivity: "base" }));
    }
    ...
    function findingSeverityFilterSet(projectId = "") {
      ...
      if (!severityFilters.has(normalized)) {
        severityFilters.set(normalized, /* @__PURE__ */ new Set());
      }
    ...
    function findingScopeFilterSet(projectId = "") {
      ...
      if (!scopeFilters.has(normalized)) {
        scopeFilters.set(normalized, /* @__PURE__ */ new Set());
      }
    ...
    function findingStatusFilterSet(projectId = "") {
      ...
      if (!statusFilters.has(normalized)) {
        statusFilters.set(normalized, /* @__PURE__ */ new Set());
      }
    ...
    function findingLabelFilterSet(projectId = "") {
      ...
      if (!labelFilters.has(normalized)) {
        labelFilters.set(normalized, /* @__PURE__ */ new Set());
      }
    ...
    function findingLabelOptions(projectId = "") {
      ...
      return [...labels].sort((left, right) => left.localeCompare(right, void 0, { sensitivity: "base" }));
    }
    ...
    function filteredFindingItems(projectId = "", summary = ctx.projectSummary?.(projectId)) {
      ...
      return key && ctx.hasProjectFilteredFindingsKey?.(key) ? ctx.projectFilteredFindingItems(key) : ctx.projectFindingItems(projectId);
    }
    function findingTargetText(summary, finding) {
      const targetIds = [...findingTargetIds(finding)];
      if (!targetIds.length) return "";
      return targetIds.map((targetId) => ctx.projectTargetLabel(summary, targetId)).filter(Boolean).join(", ");
    }
    ...
    function compareProjectFindingText(left, right) {
      return String(left || "").localeCompare(String(right || ""), void 0, { sensitivity: "base", numeric: true });
    }
    ...
    function filteredFindings(projectId, summary) {
      ...
      if (!ctx.hasProjectFilteredFindingsKey?.(findingFilteredKey(projectId, summary))) {
        ...
        const runFilters2 = new Set(runFilterIds(projectId, summary));
        if (runFilters2.size) {
          findings = findings.filter((finding) => runFilters2.has(String(finding && finding.run_id || "")));
        }
    ...
    function filteredArtifacts(projectId, summary) {
      ...
      const runFilters2 = new Set(runFilterIds(projectId, summary));
      if (runFilters2.size) {
        artifacts = artifacts.filter((artifact) => runFilters2.has(String(artifact && artifact.run_id || "")));
      }
      const filterIds = targetFilterIds(projectId, summary);
      if (!filterIds.length) return artifacts;
      const matchingRunIds = runIdsMatchingTargets(projectId, filterIds);
      const matchingDirectRunIds = /* @__PURE__ */ new Set();
      ctx.projectRunItems(summary).forEach((run) => {
    ...
    function scheduleFilterSortDividerSync(root) {
      const schedule = typeof global.requestAnimationFrame === "function" ? global.requestAnimationFrame.bind(global) : typeof window.requestAnimationFrame === "function" ? window.requestAnimationFrame.bind(window) : window.setTimeout.bind(window);
