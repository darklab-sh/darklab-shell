// app/static/js/features/projects/project_mobile_shell.js
var exportedDarklabProjectMobileShell = null;
(function projectMobileShellModule(global) {
  ...
  function createProjectMobileShellController(context) {
    ...
    function selectProject(projectId, tab = "") {
      ...
      Promise.resolve(ctx.ensureProjectSummary?.(nextProjectId)).then(() => ctx.renderProjectWorkspace()).catch(() => {
      });
    }
    function projectActions(project) {
      ...
      actions.push(ctx.projectIsArchived(project) ? { label: "Unarchive", action: "unarchive" } : { label: "Archive", action: "archive" });
    ...
    function renderMobile() {
      ...
      if (ctx.projectMobileSummary) {
        ...
        ctx.projectMobileSummary.textContent = count ? `${count} project${count === 1 ? "" : "s"} in this session` : "Create a project to group related work";
      }
