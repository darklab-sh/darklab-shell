...
var exportedDarklabProjectEntityEditor = null;
(function projectEntityEditorModule(global) {
  ...
  function createProjectEntityEditorController(context) {
    ...
    function titleize(value) {
      return String(value || "Activity").replace(/[._-]+/g, " ").replace(/\b\w/g, (char) => char.toLocaleUpperCase());
    }
    ...
    async function loadActivity() {
      ...
      try {
        ...
        if (!resp.ok) {
          const err = typeof ctx.projectResponseError === "function" ? await ctx.projectResponseError(resp, "Could not load recent activity.") : new Error("Could not load recent activity.");
          throw err;
        }
