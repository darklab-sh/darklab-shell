// app/static/js/features/projects/project_list.js
var exportedDarklabProjectList = null;
(function projectListModule(global) {
  ...
  function createProjectListController(context) {
    ...
    function renderListRow(project, activeId) {
      ...
      row.type = "button";
      row.className = "control-row project-workspace-row" + (projectId === activeId ? " is-active" : "") + (projectId === ctx.selectedProjectId() ? " is-selected" : "");
      ...
      const statusText = projectId === activeId ? "active" : isArchived(project) ? "archived" : "";
      if (statusText) {
        const status = document.createElement("span");
      ...
      ctx.projectCountEntries(summary).filter((item) => ["runs", "findings", "artifacts", "packages"].includes(item.id)).forEach((item) => {
    ...
    function mobileCountEntries(summary) {
      return ctx.projectCountEntries(summary).filter((item) => item.id !== "artifacts" || ctx.projectArtifactsVisible()).filter((item) => ["runs", "entities", "findings", "artifacts", "targets", "packages"].includes(item.id));
    }
    function renderMobileListRow(project, activeId) {
      const projectId = String(project.id || "");
      const summary = ctx.projectSummary(projectId);
      const row = document.createElement("article");
      row.className = "panel-row project-mobile-row" + (projectId === activeId ? " is-active" : "") + (projectId === ctx.selectedProjectId() ? " is-selected" : "");
      row.dataset.projectId = projectId;
