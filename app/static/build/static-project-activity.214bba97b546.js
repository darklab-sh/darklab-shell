...
(function projectActivityModule(global) {
  ...
  // Only target types that can carry a project_id appear in project-scoped audit
  // events; team/account-config targets (notification, schedule, watcher, secret,
  // team) never do, so they are intentionally excluded from this filter.
  const targetTypes = [
    ['', 'All targets'],
    ['project', 'Project'],
    ['finding', 'Finding'],
  ...
  // Target types whose objects open inside the project workspace. Runs and
  // snapshots keep their server-route hrefs; these map to in-workspace tabs so an
  // activity row can jump straight to the related object.
  ...
  function createProjectActivityController(context) {
    ...
    function detailValueText(value) {
      ...
      if (value && typeof value === 'object') {
        const parts = Object.entries(value)
          .filter(([, item]) => item !== undefined && item !== null && item !== '')
          .slice(0, 4)
          .map(([key, item]) => `${key.replace(/_/g, ' ')}: ${item && typeof item === 'object' ? '[object]' : String(item)}`);
    ...
    function renderFilters(projectId, st) {
      ...
      const apply = ctx.makeProjectButton
        ? ctx.makeProjectButton('Apply', 'activity-apply', projectId)
        : document.createElement('button');
      apply.type = 'button';
      apply.textContent = 'Apply';
      apply.dataset.projectActivityAction = 'apply';
      apply.dataset.projectId = projectId;
      const clear = ctx.makeProjectButton
        ? ctx.makeProjectButton('Clear', 'activity-clear', projectId, 'secondary')
        : document.createElement('button');
      clear.type = 'button';
    ...
    function renderEmpty(st) {
      const message = hasFilters(st)
        ? 'No activity matches these filters.'
        : 'No project activity yet.';
      const panel = ctx.emptyProjectPanel
        ? ctx.emptyProjectPanel(message)
    ...
    function renderPager(projectId, st) {
      ...
      const prev = ctx.makeProjectButton
        ? ctx.makeProjectButton('Previous', 'activity-prev', projectId, 'secondary')
        : document.createElement('button');
      prev.type = 'button';
      ...
      const next = ctx.makeProjectButton
        ? ctx.makeProjectButton('Next', 'activity-next', projectId, 'secondary')
        : document.createElement('button');
      next.type = 'button';
    ...
    function render(projectId, summary, { mobile = false } = {}) {
      ...
      if (st.loading) {
        ...
        const retry = ctx.makeProjectButton
          ? ctx.makeProjectButton('Retry', 'activity-retry', projectId, 'secondary')
          : document.createElement('button');
        retry.type = 'button';
