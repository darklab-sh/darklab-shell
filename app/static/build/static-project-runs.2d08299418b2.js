...
// app/static/js/features/projects/project_runs.js
var exportedDarklabProjectRuns = null;
(function projectRunsModule(global) {
  ...
  function createProjectRunsController(context) {
    ...
    const runPages = /* @__PURE__ */ new Map();
    ...
    function runFindingCount(projectId, runId, run = null) {
      ...
      return ctx.projectFindingItems(projectId).filter((finding) => String(finding && finding.run_id || "") === normalizedRunId).length;
    }
    function runArtifactCount(summary, runId, run = null) {
      ...
      return ctx.projectArtifactItems(summary).filter((artifact) => String(artifact && artifact.run_id || "") === normalizedRunId).length;
    }
    ...
    function compareOptionLabels(option) {
      return Array.isArray(option && option.labels) ? option.labels.map((label) => String(label || "").trim()).filter(Boolean) : [];
    }
    ...
    function syncCompareMode(wrap) {
      ...
      const options = mode === "baseline" ? compareDatasetOptions(container, "projectCompareLabelOptions") : compareDatasetOptions(container, "projectCompareRunOptions");
      const savedValue = mode === "baseline" ? targetSelect.dataset.projectCompareBaselineValue : targetSelect.dataset.projectCompareRunValue;
    ...
    function compareRuns(projectId, leftId, mode, targetValue, controls = null) {
      ...
      const compareFn = (...args) => {
        const bridged = typeof fetchAndRenderHistoryComparison === "function" ? fetchAndRenderHistoryComparison(...args) : void 0;
        if (bridged !== void 0) return bridged;
        return typeof global.fetchAndRenderHistoryComparison === "function" ? global.fetchAndRenderHistoryComparison(...args) : void 0;
      };
    ...
    function renderRuns(container, projectId, summary) {
      ...
      const runs = useServerPage && canUseLoadedPage ? pagination.runs : ctx.filteredProjectRuns(projectId, summary);
      const comparableRuns = useServerPage && canUseLoadedPage ? runs.filter((run) => run && run.id) : ctx.projectComparableRuns(summary);
      ...
      if (!runs.length) {
        ...
        const pager2 = useServerPage && canUseLoadedPage ? renderPagination(projectId, pagination, runs, "bottom") : null;
