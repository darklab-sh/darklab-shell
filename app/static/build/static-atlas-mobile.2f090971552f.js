...
} from "./static-chunk-yhnwdnjm.e0842c7c529b.js";
import "./static-chunk-vzj3wcct.8eb18ced0063.js";
import "./static-chunk-wkdqs5l5.75c18d0d56e7.js";
import "./static-chunk-ssxb75px.e5dcd6664439.js";
import "./static-chunk-c3rnwntr.1e4d0c9ab5a9.js";
import "./static-chunk-ex5y6xtw.bfde46123a35.js";
import "./static-chunk-dil5yyjg.6d28df9092db.js";
import {
  DarklabFindingTriageEditor
} from "./static-chunk-aj3py647.674015cfb0e1.js";
import "./static-chunk-ftlux7er.217f2e307607.js";
import "./static-chunk-3jpzlov4.47e7ebc68e55.js";
import "./static-chunk-jdmxosyg.f57815afbc1f.js";
import "./static-chunk-iqwevzke.cb2ae1f8735e.js";
import {
  DarklabTeamScope,
  getActiveProjectContext,
  openEntityMetadataEditor,
  teamScopeDeniedMessage
} from "./static-chunk-7zwykdnz.d0aafed7faec.js";
import {
  copyTextToClipboard,
  showToast
...
(function initAtlasMobile(global) {
  const controller = typeof exportedDarklabAtlasOverlay !== "undefined" && exportedDarklabAtlasOverlay || null;
  ...
  const bindTabStripEdgeListener2 = typeof bindTabStripEdgeListener !== "undefined" && bindTabStripEdgeListener || null;
  const closeActionSheet2 = typeof closeActionSheet !== "undefined" && closeActionSheet || null;
  ...
  const openActionSheet2 = typeof openActionSheet !== "undefined" && openActionSheet || null;
  const showToast2 = typeof showToast !== "undefined" && showToast || null;
  const syncActiveTabStripScroll2 = typeof syncActiveTabStripScroll !== "undefined" && syncActiveTabStripScroll || null;
  ...
  function setView(name) {
    ...
    if (name === "list") {
      window.requestAnimationFrame(() => {
        try {
          listHost.scrollTop = listHost.scrollTop;
        } catch (_) {
        }
      });
  ...
  function renderTabs(state) {
    ...
    tabs.forEach((tab) => {
      ...
      button.addEventListener("click", () => {
        if (typeof closeActionSheet2 === "function") closeActionSheet2({ restoreFocus: false });
    ...
    if (typeof syncActiveTabStripScroll2 === "function") {
      syncActiveTabStripScroll2(tabsHost, tabStripEdgeOpts);
    }
  }
  ...
  function buildToolsOnce() {
    ...
    runSearch.addEventListener("input", () => {
      ...
      runSearchTimer = setTimeout(() => {
        Promise.resolve(controller.loadRunOptions?.({ query: controller.state.runOptionsQuery, force: true })).then(() => renderMobile(controller.state));
      }, 180);
    });
    ...
    if (typeof bindDisclosure2 === "function") {
      view.filtersDisclosure = bindDisclosure2(filterToggle, {
        panel,
        openClass: "open",
  ...
  function syncTools(state) {
    ...
    if (mobileOrphanFilter && mobileOrphanFilter.value !== state.orphanFilter) {
      ...
      if (typeof syncAppSelect2 === "function") syncAppSelect2(mobileOrphanFilter);
    }
    if (mobileSuppressionFilter && mobileSuppressionFilter.value !== state.suppressionFilter) {
      ...
      if (typeof syncAppSelect2 === "function") syncAppSelect2(mobileSuppressionFilter);
    }
    ...
    if (mobileRunFilterSelect) {
      ...
      const currentValues = Array.from(mobileRunFilterSelect.options || []).map((option2) => option2.value).join("\n");
      ...
      if (typeof syncAppSelect2 === "function") syncAppSelect2(mobileRunFilterSelect);
    }
    if (mobileFindingStatusFilter) {
      ...
      if (mobileFindingStatusFilter.value !== state.findingStatus) {
        ...
        if (typeof syncAppSelect2 === "function") syncAppSelect2(mobileFindingStatusFilter);
      }
    }
    if (mobileSavedViewSelect) {
      const selectedId = controller.state.selectedSavedViewId || "";
      const values = ["", ...(controller.state.savedViews || []).map((view2) => String(view2.id || ""))].join("\n");
      const currentValues = Array.from(mobileSavedViewSelect.options || []).map((option2) => option2.value).join("\n");
      if (values !== currentValues) {
        ...
        (controller.state.savedViews || []).forEach((view2) => {
          mobileSavedViewSelect.appendChild(option(view2.name || "Saved view", view2.id || ""));
        });
      ...
      if (typeof syncAppSelect2 === "function") syncAppSelect2(mobileSavedViewSelect);
    }
  ...
  function renderOrphanChip(state) {
    ...
    if (String(state.orphanFilter || "hide") === "only") {
      chips.push(["orphans only · clear", () => {
        controller.state.orphanFilter = "hide";
      }]);
    }
    if (String(state.suppressionFilter || "hide") === "only") {
      chips.push(["suppressed only · clear", () => {
        controller.state.suppressionFilter = "hide";
      }]);
    }
  ...
  function copyText(value, label = "Copied") {
    ...
    if (!text) return;
    const copier = typeof copyTextToClipboard2 === "function" ? copyTextToClipboard2(text) : navigator.clipboard?.writeText?.(text);
    Promise.resolve(copier).then(() => {
      if (typeof showToast2 === "function") showToast2(label, "success");
    }).catch(() => {
      if (typeof showToast2 === "function") showToast2("Copy failed", "error");
    });
  }
  ...
  function openMetadataEditorForEntity(entity) {
    if (typeof openEntityMetadataEditor !== "function") return;
    openEntityMetadataEditor("atlas_entity", entity, {
  ...
  function openListOverflowSheet(returnFocus) {
    if (typeof openActionSheet2 !== "function") return;
  ...
  function openEntityActionSheet(entity, returnFocus, title = "Atlas entity actions") {
    if (!entity || typeof openActionSheet2 !== "function") return;
  ...
  function openFindingActionSheet(finding, returnFocus, title = "Atlas finding actions") {
    if (!finding || typeof openActionSheet2 !== "function") return;
    view.lastActionSheetTab = controller.currentTab().id;
    openActionSheet2({
      title,
  ...
  function renderBulkBar(state) {
    ...
    if (typeof enhanceAppSelects2 === "function") enhanceAppSelects2(bulkBar);
  }
  function renderEntityRow(entity, state) {
    ...
    const row = entityRowApi.renderAtlasEntityRow({
      entity,
      selected: state.selectMode ? state.selectedEntityIds.has(String(entity.id || "")) : String(entity.id) === String(state.selectedId),
      selectMode: state.selectMode,
      mobile: true,
      text: controller.text,
      countLabel: controller.countLabel,
      badge: controller.badge,
      appendSelectionControl: state.selectMode ? (targetRow) => appendMobileSelectionCheckbox(targetRow, entity, state, `Select entity: ${entity.canonical_value || entity.id}`) : null,
  ...
  function renderFindingRow(finding, state) {
    ...
    if (triage) {
      ...
      if (verificationStatus && verificationStatus !== "not_started") {
        const label = findingTriageEditor?.verificationStatusLabel?.(verificationStatus) || verificationStatus.replace(/_/g, " ");
  ...
  function renderEntityTopbar(state) {
    ...
    more.addEventListener("click", () => {
      const entity2 = state.detail?.entity || state.detail || {};
      openEntityActionSheet(entity2, more, "Atlas entity actions");
    });
  ...
  function renderEntityFooter(state) {
    ...
    const activeLink = activeId ? links.find((link) => String(link.project_id || "") === activeId) : null;
    const refresh = document.createElement("button");
    refresh.type = "button";
  ...
  function renderFindingFooter(state) {
    ...
    if (controller.detailApi && typeof controller.detailApi.reviewStateSelect === "function") {
      ...
      if (select) {
        ...
        if (typeof enhanceAppSelects2 === "function") {
          enhanceAppSelects2(findingFooter);
        }
      }
    }
  ...
  function syncRequestedView(state) {
    ...
    if (started && Date.now() - started > 1500) {
      ...
      if (typeof showToast2 === "function") {
        showToast2("Entity not found in Atlas — showing list", "warning");
      }
    }
  }
  ...
  if (typeof bindTabStripEdgeListener2 === "function") {
    bindTabStripEdgeListener2(tabsHost, tabStripEdgeOpts);
  }
  bindRowLongPress();
  ...
  if (typeof setAtlasMobileHandlers === "function") {
    setAtlasMobileHandlers({
