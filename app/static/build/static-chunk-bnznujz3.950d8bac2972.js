import {
  ...
  compareFormatDate
} from "./static-chunk-xdnhi6ng.03705073697e.js";
import {
  _ensureHistoryCompareOverlay,
  _openHistoryCompareOverlay
} from "./static-chunk-tda3zjlz.ba4d349f2998.js";
import {
...
var HISTORY_COMPARE_LAUNCHER_GLOBAL = typeof window !== "undefined" ? window : globalThis;
function _historyCompareLauncherFormatDate(value) {
  return typeof compareFormatDate === "function" ? compareFormatDate(value) : "";
}
function _historyCompareLauncherDateGroupLabel(value) {
  return typeof compareDateGroupLabel === "function" ? compareDateGroupLabel(value) : "Other";
}
...
function _historyCompareLauncherOpenOverlay() {
  const open = typeof _openHistoryCompareOverlay !== "undefined" && _openHistoryCompareOverlay || HISTORY_COMPARE_LAUNCHER_GLOBAL._openHistoryCompareOverlay;
  if (typeof open === "function") open();
}
function _historyCompareLauncherEnsureOverlay() {
  const ensure = typeof _ensureHistoryCompareOverlay !== "undefined" && _ensureHistoryCompareOverlay || HISTORY_COMPARE_LAUNCHER_GLOBAL._ensureHistoryCompareOverlay;
  return typeof ensure === "function" ? ensure() : null;
}
function _historyCompareLauncherApiFetch() {
  const fetcher = (typeof hasRuntimeHandler === "function" && hasRuntimeHandler("apiFetch") && typeof apiFetch === "function" ? apiFetch : null) || (typeof HISTORY_COMPARE_LAUNCHER_GLOBAL.apiFetch === "function" ? HISTORY_COMPARE_LAUNCHER_GLOBAL.apiFetch : null);
...
function _historyCompareLauncherFetchAndRender(leftId, rightId, options) {
  const bridged = typeof fetchAndRenderHistoryComparison === "function" ? fetchAndRenderHistoryComparison(leftId, rightId, options) : void 0;
  ...
  return typeof globalFetchAndRender === "function" ? globalFetchAndRender(leftId, rightId, options) : void 0;
}
function _historyCompareRunCard(run, label, extra = "") {
  ...
  if (run && run.exit_code !== void 0 && run.exit_code !== null) parts.push(`exit ${run.exit_code}`);
...
var _historyCompareManualTimer = null;
function _loadHistoryCompareManualCandidates(source, query = "") {
  ...
  window._historyCompareState.manualCollapsedGroups = /* @__PURE__ */ new Set();
...
function _fetchHistoryCompareManualCandidates(source, query = "", { requestId = null, page = 1, append = false } = {}) {
  ...
  _historyCompareLauncherApiFetch()(`/history?${params.toString()}`).then((resp) => resp.json()).then((data) => {
    ...
    const existing = append ? new Set((window._historyCompareState.manualCandidates || []).map((item) => item.id)) : /* @__PURE__ */ new Set();
    const manualItems = items.filter((item) => item && item.type !== "snapshot" && item.id && item.id !== source.id && !existing.has(item.id)).map((item) => ({
      ...item,
      confidence_label: seenRanked.has(item.id) ? (ranked.find((candidate) => candidate.id === item.id) || {}).confidence_label || "" : ""
    }));
    window._historyCompareState.manualCandidates = append ? [...window._historyCompareState.manualCandidates || [], ...manualItems] : manualItems;
...
function _renderHistoryCompareCandidateList() {
  ...
  const sourceCandidates = window._historyCompareState.manualLoaded ? window._historyCompareState.manualCandidates || [] : window._historyCompareState.candidates || [];
  const candidates = sourceCandidates.filter((item) => item && item.id && item.id !== source.id);
  ...
  groups.forEach((group) => {
    ...
    group.items.forEach((candidate) => {
      ...
      meta.textContent = [
        ...
        candidate.exit_code !== void 0 && candidate.exit_code !== null ? `exit ${candidate.exit_code}` : ""
      ].filter(Boolean).join(" · ");
...
function openHistoryCompareLauncher(run) {
  ...
  _historyCompareLauncherApiFetch()(`/history/${encodeURIComponent(run.id)}/compare-candidates`).then((resp) => resp.json()).then((data) => {
...
if (typeof setHistoryCompareHandlers === "function") {
  setHistoryCompareHandlers({ openHistoryCompareLauncher });
}
