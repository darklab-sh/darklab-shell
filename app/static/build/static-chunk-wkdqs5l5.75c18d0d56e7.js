// app/static/js/features/projects/project_findings_data.js
var exportedBoardColumnLimit = null;
var exportedBoardWorkflowStates = null;
var exportedBoardCardFromFinding = null;
var exportedBoardColumnsFromFindings = null;
var exportedBoardWorkflowState = null;
var exportedDarklabProjectFindingsData = null;
(function projectFindingsDataModule(global) {
  ...
  function boardWorkflowState(value) {
    const reviewState = typeof value === "object" && value !== null ? value.review_state : value;
  ...
  function boardLabels(finding = {}) {
    return Array.isArray(finding.labels) ? finding.labels.map((label) => String(label || "")).filter(Boolean) : [];
  }
  ...
  function createProjectFindingsDataController(context) {
    const ctx = context || {};
    let findings = /* @__PURE__ */ new Map();
    let findingsPagination = /* @__PURE__ */ new Map();
    let filteredFindings = /* @__PURE__ */ new Map();
    let filteredFindingsPagination = /* @__PURE__ */ new Map();
    ...
    function collapsedGroupLabels(projectId = "") {
      const labels = typeof ctx.collapsedFindingGroupLabels === "function" ? ctx.collapsedFindingGroupLabels(projectId) : [];
      return Array.isArray(labels) ? labels.map((label) => String(label || "")).filter(Boolean) : [];
    }
    function collapsedGroupSignature(projectId = "") {
      return collapsedGroupLabels(projectId).join("");
    }
    ...
    function mergedGroupOrder(previousOrder = [], nextOrder = []) {
      ...
      [...Array.isArray(previousOrder) ? previousOrder : [], ...Array.isArray(nextOrder) ? nextOrder : []].map((label) => String(label || "")).filter(Boolean).forEach((label) => {
    ...
    function collapsedCountsFromPage(projectId, pageData = {}) {
      ...
      const counts = {};
      const currentCollapsedCounts = pageData.collapsed_group_counts && typeof pageData.collapsed_group_counts === "object" ? pageData.collapsed_group_counts : {};
      const currentGroupCounts = pageData.group_counts && typeof pageData.group_counts === "object" ? pageData.group_counts : {};
    ...
    function invalidateFiltered(projectId = "") {
      ...
      if (!normalized) {
        filteredFindings = /* @__PURE__ */ new Map();
        filteredFindingsPagination = /* @__PURE__ */ new Map();
    ...
    function invalidate(projectId = "") {
      ...
      if (normalized) {
        ...
        findingsPagination = /* @__PURE__ */ new Map();
        invalidateFiltered();
      }
    }
    function setCachedReviewState(projectId, findingId, reviewState) {
      ...
      filteredFindings.forEach((items2, key) => {
        if (!String(key).startsWith(`${normalized}::`)) return;
        filteredFindings.set(key, items2.map((finding) => {
          if (String(finding && finding.id || "") !== normalizedFindingId) return finding;
          return { ...finding, review_state: reviewState };
        }));
      });
    ...
    function updateCachedFinding(projectId, findingId, updates) {
      ...
      const current = findings.get(normalized);
      if (!normalized || !normalizedFindingId || !Array.isArray(current)) return;
      const updateRow = (finding) => String(finding && finding.id || "") === normalizedFindingId ? { ...finding, ...updatePayload } : finding;
      findings.set(normalized, current.map(updateRow));
      filteredFindings.forEach((items2, key) => {
        if (!String(key).startsWith(`${normalized}::`)) return;
        filteredFindings.set(key, items2.map(updateRow));
      });
    }
    async function load(projectId, options = {}) {
      ...
      if (findings.has(normalized) && currentPage.offset === offset && currentPage.loaded && currentPage.collapsed_signature === collapsedSignature) return;
      ...
      findingsLoadingPromise = Promise.resolve().then(async () => {
        ...
        try {
          ...
          const collapsedCounts = {
            ...
            ...data.collapsed_group_counts && typeof data.collapsed_group_counts === "object" ? data.collapsed_group_counts : {}
          };
    ...
    async function loadFiltered(projectId, summary = ctx.projectSummary?.(projectId), options = {}) {
      ...
      const collapsedSignature = collapsedGroupSignature(normalized);
      if (filteredFindings.has(key) && currentPage.offset === offset && currentPage.loaded && currentPage.collapsed_signature === collapsedSignature) return;
...
export {
  exportedBoardColumnLimit,
  exportedBoardWorkflowStates,
  exportedBoardCardFromFinding,
  exportedBoardColumnsFromFindings,
  exportedBoardWorkflowState,
