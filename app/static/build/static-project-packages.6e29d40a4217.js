...
// app/static/js/features/projects/project_packages.js
var exportedDarklabProjectPackages = null;
(function initProjectPackages(global) {
  if (typeof document === "undefined") return;
  function createProjectPackagesController(context) {
    const ctx = context || {};
    let wizard = null;
    const downloadTimers = /* @__PURE__ */ new WeakMap();
    ...
    function normalizePackagePresetEntry(entry) {
      ...
      return {
        ...
        labels: Array.isArray(entry.labels) ? entry.labels.map((label) => safePackagePresetText(label)).filter(Boolean) : [],
        notes: safePackagePresetText(entry.notes),
        selection: {
          ...packagePresetSelectionDefaults,
          ...Object.fromEntries(
            Object.entries(selection).map(([key, value]) => [key, safePackagePresetText(value).toLowerCase()]).filter(([key, value]) => key in packagePresetSelectionDefaults && value)
          )
        }
      };
    }
    ...
    function packagePresets() {
      return packagePresetCatalog.length ? packagePresetCatalog : defaultPackagePresets.map((preset) => ({ ...preset, selection: { ...preset.selection } }));
    }
    ...
    async function ensurePackagePresetCatalog() {
      ...
      packagePresetCatalogPromise = ctx.apiFetch("/projects/package-presets", { cache: "no-store" }).then((resp) => readJsonResponse(resp, "Unable to load package presets.")).then((data) => {
    ...
    function metaText(pkg) {
      ...
      const estimate = manifest.estimated_archive && typeof manifest.estimated_archive === "object" ? Number(manifest.estimated_archive.estimated_uncompressed_bytes || 0) : 0;
      const includeArtifacts = pkg && Object.prototype.hasOwnProperty.call(pkg, "include_artifacts") ? !!pkg.include_artifacts : !!manifest.options?.raw_artifacts;
    ...
    function packageChips(pkg) {
      ...
      if (summary?.chips?.length) {
        const provenanceChip = summary.chips.find((chip) => String(chip.label || "").startsWith("source:")) || summary.chips[0];
    ...
    function openManifest(pkg) {
      ...
      if (ctx.manifestSummary) {
        ...
        ctx.manifestSummary.appendChild(
          ctx.projectProvenanceSummaryElement?.(manifest, { fallbackKind: "evidence_package" }) || document.createElement("div")
        );
      }
    ...
    function presetDefaults(preset, summary, findings) {
      ...
      return {
        ...
        collapsedRunIds: /* @__PURE__ */ new Set(),
        selection: {
          ...
          targetIds: new Set(
            selectionPolicy(presetConfig, "targets") === "all" ? targets.map((target) => String(target.id || "")).filter(Boolean) : []
          )
        }
      };
    }
    ...
    function renderWizardModal({ focus = false, scrollTop = null } = {}) {
      ...
      const enhanceAppSelects2 = typeof enhanceAppSelects !== "undefined" && enhanceAppSelects || null;
      if (typeof enhanceAppSelects2 === "function") {
        enhanceAppSelects2(ctx.wizardBody);
      }
    ...
    function suggestedName(project, preset) {
      const slug = String(project && (project.slug || project.name) || "project").trim().toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "") || "project";
    ...
    function reapplyPresetDefaults(projectId, { preserveTouched = true } = {}) {
      ...
      refreshed.collapsedRunIds = previous.collapsedRunIds || /* @__PURE__ */ new Set();
    ...
    function openWizardFromPackage(projectId, pkg) {
      ...
      const redactionMode = String(pkg?.redaction_mode || manifest.redaction_mode || "raw") === "redacted" ? "redacted" : "raw";
      const options = manifest.options && typeof manifest.options === "object" ? manifest.options : {};
      const includeArtifacts = !ctx.projectFilesEnabled() ? false : Boolean(pkg?.include_artifacts ?? options.raw_artifacts ?? options.redacted_artifact_derivatives);
    ...
    function runChildIds(runId, projectId, summary) {
      ...
      const findings = ctx.projectFindingItems(projectId).filter((finding) => String(finding && finding.run_id || "") === normalizedRunId);
      const artifacts = ctx.projectArtifactItems(summary).filter((artifact) => String(artifact && artifact.run_id || "") === normalizedRunId);
      return {
        ...
        selectableArtifactIds: artifacts.filter((artifact) => ctx.projectArtifactStatus(artifact) === "available").map((artifact) => String(artifact.id || "")).filter(Boolean)
      };
    }
    ...
    function selectionKind(kind, config) {
      return {
        ...
        missing(projectId, summary) {
          const current = new Set((config.items(projectId, summary) || []).map((item) => String(item && item.id || "")).filter(Boolean));
    ...
    function setFor(kind) {
      ...
      if (!wizard || !config) return /* @__PURE__ */ new Set();
      return config.set(wizard) || /* @__PURE__ */ new Set();
    }
    ...
    function skippedPreview(projectId, summary) {
      ...
      if (wizard?.includeArtifacts) {
        ctx.projectArtifactItems(summary).filter((artifact) => wizard.selection.artifactIds.has(String(artifact.id || ""))).filter((artifact) => ctx.projectArtifactStatus(artifact) !== "available").forEach((artifact) => {
          skipped.push({
            kind: "artifact",
        ...
        if (wizard.redactionMode === "redacted") {
          ctx.projectArtifactItems(summary).filter((artifact) => wizard.selection.artifactIds.has(String(artifact.id || ""))).filter((artifact) => ctx.projectArtifactStatus(artifact) === "available").map((artifact) => ({ artifact, reason: redactedArtifactWarning(artifact) })).filter((item) => item.reason).forEach(({ artifact, reason }) => {
            skipped.push({
              kind: "artifact",
              id: String(artifact.id || ""),
              label: artifact.display_name || artifact.workspace_path || artifact.id,
    ...
    function renderPreset(wrap, projectId, summary) {
      ...
      note.textContent = `${ctx.projectRunItems(summary).length} runs, ${ctx.projectFindingItems(projectId).length} findings, ${ctx.projectArtifactItems(summary).length} artifacts, and ${ctx.projectTargetItems(summary).length} targets are available.`;
    ...
    function renderMetadata(wrap) {
      ...
      artifactsStrong.textContent = wizard.redactionMode === "redacted" ? "Include selected artifact derivatives" : "Include selected raw artifacts";
      const artifactsSmall = document.createElement("small");
      artifactsSmall.textContent = !ctx.projectFilesEnabled() ? "Raw artifact files are unavailable because Files are disabled on this instance." : wizard.redactionMode === "redacted" ? "Text and JSON artifacts are sanitized into derivative files; binary or unknown types are skipped." : "Static HTML, Markdown, and selected raw artifacts are included in the archive.";
    ...
    function manifestPreview(projectId, summary) {
      ...
      const selectedEntityIds = {
        ...
        transcript_run_ids: Array.from(wizard.selection.transcriptRunIds).filter((runId) => wizard.selection.runIds.has(String(runId || ""))),
    ...
    function estimatePackage(summary) {
      const selectedRuns = ctx.projectRunItems(summary).filter((run) => wizard.selection.runIds.has(String(run.id || "")));
      const selectedTranscriptRuns = selectedRuns.filter((run) => wizard.selection.transcriptRunIds.has(String(run.id || "")));
      const selectedArtifacts = ctx.projectArtifactItems(summary).filter((artifact) => wizard.selection.artifactIds.has(String(artifact.id || "")));
      const selectedFindings = ctx.projectFindingItems(wizard.projectId).filter((finding) => wizard.selection.findingIds.has(String(finding.id || "")));
      const selectedTargets = ctx.projectTargetItems(summary).filter((target) => wizard.selection.targetIds.has(String(target.id || "")));
      const rawArtifactBytes = wizard.includeArtifacts && wizard.redactionMode !== "redacted" ? selectedArtifacts.filter((artifact) => ctx.projectArtifactStatus(artifact) === "available").reduce((total, artifact) => total + Math.max(0, Number(artifact.byte_size || 0)), 0) : 0;
      const redactedArtifactItems = wizard.includeArtifacts && wizard.redactionMode === "redacted" ? selectedArtifacts.filter((artifact) => ctx.projectArtifactStatus(artifact) === "available").filter((artifact) => !redactedArtifactWarning(artifact)) : [];
      const redactedArtifactBytes = redactedArtifactItems.reduce((total, artifact) => total + Math.max(0, Number(artifact.byte_size || 0)), 0);
      const skippedArtifactCountEstimate = wizard.includeArtifacts ? selectedArtifacts.filter((artifact) => ctx.projectArtifactStatus(artifact) !== "available" || wizard.redactionMode === "redacted" && !!redactedArtifactWarning(artifact)).length : 0;
      const config = appConfig() || {};
      const maxLines = Math.max(1, Number(config.max_output_lines || 5e3));
      ...
      const requiredArchiveBytes = rawArtifactBytes + redactedArtifactBytes + transcriptHtmlBytes + metadataBytes;
      ...
      const estimatedCompressedArchiveBytes = rawArtifactBytes + estimateTextZipBytes(redactedArtifactBytes) + estimateTextZipBytes(transcriptHtmlBytes) + estimateTextZipBytes(metadataBytes);
      const estimatedCompressedArchiveBytesWithOptionalCompanions = estimatedCompressedArchiveBytes + estimateTextZipBytes(transcriptTextCompanionBytes);
      return {
    ...
    function renderPreview(wrap, projectId, summary) {
      ...
      const note = document.createElement("p");
      note.className = "project-package-wizard-note";
      const zipCapText = Number(estimate.max_compressed_archive_bytes || estimate.max_archive_bytes || 0) > 0 ? ` ZIP cap: ${ctx.formatBytes(estimate.max_compressed_archive_bytes || estimate.max_archive_bytes)}.` : "";
      const expandedCapText = Number(estimate.max_uncompressed_archive_bytes || 0) > 0 ? ` Expanded cap: ${ctx.formatBytes(estimate.max_uncompressed_archive_bytes)}.` : "";
      note.textContent = `Best-guess ZIP size: ${ctx.formatBytes(estimate.estimated_compressed_archive_bytes || 0)}. Expanded content: ${ctx.formatBytes(estimate.estimated_uncompressed_bytes || 0)}.${zipCapText}${expandedCapText}`;
      wrap.appendChild(note);
      if (Number(estimate.transcript_html_bytes || 0) > 0 || Number(estimate.transcript_text_companion_bytes || 0) > 0) {
        ...
        transcriptEstimate.textContent = `Transcript HTML estimate: ${ctx.formatBytes(estimate.transcript_html_bytes || 0)}` + (Number(estimate.transcript_text_companion_bytes || 0) > 0 ? ` Some capped transcripts can also produce full-text fallback files, up to ${ctx.formatBytes(estimate.transcript_text_companion_bytes || 0)} total; those fallback files are added only when they fit.` : ".");
      ...
      if (Number(estimate.max_compressed_archive_bytes || estimate.max_archive_bytes || 0) > 0 && Number(estimate.estimated_compressed_archive_bytes || 0) > Number(estimate.max_compressed_archive_bytes || estimate.max_archive_bytes || 0)) {
      ...
      if (Number(estimate.max_uncompressed_archive_bytes || 0) > 0 && Number(estimate.estimated_archive_bytes || 0) > Number(estimate.max_uncompressed_archive_bytes || 0)) {
        const overLimit = document.createElement("p");
        overLimit.className = "project-package-wizard-note is-warning";
    ...
    function renderWizard(container, projectId, summary) {
      ...
      if (wizard.notice) {
        ...
        message.className = "project-workspace-message project-package-wizard-message" + (wizard.noticeError ? " is-error" : "");
    ...
    async function createFromWizard(projectId) {
      ...
      const summary = summaryFor(projectId);
      const missingSelections = skippedPreview(projectId, summary).filter((item) => item.kind !== "artifact" || String(item.reason || "").includes("No longer linked"));
    ...
    async function downloadPackage(projectId, pkg) {
      ...
      if (!downloadResp.ok) {
        const data2 = await downloadResp.json().catch(() => ({}));
        throw new Error(data2.error || "Unable to download package.");
      }
    ...
    function setDownloadBusy(button, busy) {
      ...
      const update = () => {
        button.textContent = packageBuildStatusText(Math.floor((Date.now() - started) / 1e3));
      };
      update();
      downloadTimers.set(button, setInterval(update, 1e3));
      ctx.setProjectWorkspaceMessage?.(
    ...
    function renderPackages(container, projectId, summary) {
      ...
      packageItems.forEach((pkg) => {
        const counts = countsText(pkg);
        const updated = ctx.formatDate(pkg.updated);
        const detail = [pkg.description || "", counts, updated ? `Updated ${updated}` : ""].filter(Boolean).join(" · ");
    ...
    async function handleAction(btn) {
      ...
      if (action === "package-wizard-transcripts-all" || action === "package-wizard-transcripts-clear" || action === "package-wizard-bulk-selection") {
        if (wizard) {
    ...
    async function handleWizardOverlayClick(event) {
      ...
      if (runToggle && wizard) {
        ...
        if (runId) {
          if (!wizard.collapsedRunIds) wizard.collapsedRunIds = /* @__PURE__ */ new Set();
