...
function renderLabels(labels) {
  ...
  const values = (Array.isArray(labels) ? labels : []).map((label) => text(label && typeof label === "object" ? label.label : label)).filter(Boolean);
  if (!values.length) {
...
function formatIntelKey(key) {
  const normalized = text(key).replace(/_/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2").trim();
...
function renderIntelDataValue(value, depth = 0) {
  ...
  if (isRecord(value)) {
    const entries = Object.entries(value).filter(([, entryValue]) => entryValue !== void 0 && entryValue !== null && entryValue !== "");
...
function renderRelatedUrls(urls, meta = null, onOpenEntity = null) {
  ...
  rows.forEach((urlEntity) => {
    ...
    title.className = typeof onOpenEntity === "function" ? "btn btn-ghost btn-compact atlas-source-command atlas-related-url-open" : "atlas-source-command";
...
function renderMetadataEditor(entity, handlers = {}) {
  ...
  labelInput.value = (Array.isArray(entity.labels) ? entity.labels : []).map((label) => text(label && typeof label === "object" ? label.label : label)).filter(Boolean).join(", ");
