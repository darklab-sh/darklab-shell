...
var exportedRefreshOptionsTeams = null;
(function initOptionsTeamsPanel(global) {
  ...
  let _bound = false;
  const _activityStates = /* @__PURE__ */ new Map();
  const _recentActivityStates = /* @__PURE__ */ new Map();
  ...
  function _apiFetch() {
    return typeof apiFetch === "function" ? apiFetch : global.fetch.bind(global);
  }
  function _tokenSessionActive() {
    return String(typeof getSessionId === "function" ? getSessionId() : "").startsWith("tok_");
  }
  ...
  function _toast(text, tone = "success") {
    _msg("");
    const toast = typeof showToast !== "undefined" && showToast || null;
    if (toast) {
      toast(text, tone);
      return;
    }
  ...
  function _clipboardWriter() {
    if (typeof copyTextToClipboard !== "undefined" && copyTextToClipboard) {
  ...
  function _logTeamClientError(event, action, error, context = {}) {
    const logError = typeof logClientError === "function" ? logClientError : null;
    if (!logError) return;
  ...
  function _renderTopForm() {
    ...
    const existingValues = existingMode === _formMode && existingForm ? _formValues(existingForm) : {};
    _clear(host);
  ...
  function _renderMembers(section) {
    ...
    const activeOwnerCount = members.filter(
      (member) => member && member.role === "owner" && member.status === "active" && !member.removed_at
    ).length;
    members.forEach((member) => {
      ...
      const canEditRole = !removed && !isOnlyCurrentOwner && (member.role === "owner" ? canManageOwner : canManageMember);
  ...
  function _activityDetailValue(value) {
    ...
    if (value && typeof value === "object") {
      const parts = Object.entries(value).filter(([, item]) => item !== void 0 && item !== null && item !== "").slice(0, 4).map(([key, item]) => `${key.replaceAll("_", " ")}: ${item && typeof item === "object" ? "[object]" : String(item)}`);
  ...
  function _renderDetail() {
    ...
    const enhanceSelects = typeof enhanceAppSelects !== "undefined" && enhanceAppSelects || null;
    if (enhanceSelects) {
      host.querySelectorAll("select.form-select").forEach((select) => {
  ...
  async function _confirm(body, { tone = "warning", confirmLabel = "Continue", destructive = false } = {}) {
    const confirmModal = typeof showConfirm !== "undefined" && showConfirm || null;
    if (confirmModal) {
    ...
    return typeof global.confirm === "function" ? global.confirm(typeof body === "string" ? body : body.text || confirmLabel) : false;
  }
  async function _submitTopForm(form) {
    ...
    try {
      if (mode === "create") {
        ...
        const response2 = await _jsonRequest("/session/teams", { method: "POST", body: payload });
        _formMode = "";
        _oneTimeCode = { teamId: response2.team?.id || "", label: "Recovery code", code: response2.recovery_code || "" };
        await refreshOptionsTeams();
        await _loadTeamDetail(response2.team?.id || "");
        _toast("Team created");
        return;
      }
      ...
      const action = mode === "create" ? "create_team" : mode === "recover" ? "redeem_recovery_code" : "join_team";
  ...
  async function _handleDetailAction(action, target) {
    ...
    if (action === "activity-apply" || action === "activity-clear" || action === "activity-prev" || action === "activity-next" || action === "activity-retry") {
      const st = _activityState(teamId);
    ...
    if (action === "archive-team" || action === "reactivate-team") {
      ...
      const ok = await _confirm({
        ...
        note: status === "archived" ? "Team schedules and watchers pause while the team is archived." : "Team members can use this team again, but schedules and watchers paused by archival stay paused until you resume them."
...
export {
  exportedRefreshOptionsTeams as refreshOptionsTeams
};
