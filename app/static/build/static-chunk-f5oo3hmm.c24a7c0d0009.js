...
function _resolvedWorkspaceContext(resolved) {
  return !!(resolved && (resolved.workspaceFlagActive || resolved.workspacePathActive || resolved.workspaceValueActive));
}
...
function _resolveAutocompleteHintSource(ctx, spec, baseHints, options = {}) {
  ...
  const workspaceValueHints = allowWorkspaceValue && !_autocompleteSpecHasWorkspacePathKinds(spec) ? _workspaceAutocompleteHintsForTypedSlot(spec, baseHints) : null;
  return {
...
function _buildContextAutocomplete(ctx) {
  ...
  const resolvedWorkspacePathHints = allowPositionalHints ? _resolveAutocompleteHintSource(ctx, spec, [], { allowWorkspaceValue: false }) : null;
