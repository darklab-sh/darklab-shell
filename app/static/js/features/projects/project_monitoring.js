...
(function projectMonitoringModule(global) {
  ...
  const timelineFireKindsByState = {
    active: new Set(['baseline_accepted', 'baseline_created', 'no_change', 'recovered', 'unclassified']),
    changed: new Set(['changed']),
    failed: new Set(['failed']),
    quiet: new Set(['no_change', 'recovered']),
  ...
  function createProjectMonitoringController(context) {
    ...
    function monitorSeverity(monitor) {
      const severities = [monitor.latest_fire, ...(Array.isArray(monitor.fires) ? monitor.fires : [])]
        .map(fireSeverity)
        .filter(Boolean);
    ...
    function monitorClassifier(monitor) {
      return fireClassifier(monitor.latest_fire)
        || (Array.isArray(monitor.fires) ? monitor.fires.map(fireClassifier).find(Boolean) : '')
        || String(monitor.monitor_group?.key || '');
    }

    function itemTargetValues(item) {
      return (Array.isArray(item?.linked_targets) ? item.linked_targets : [])
        .map(target => String(target?.value || ''))
        .filter(Boolean);
    }
    ...
    function createdWithin(value, days) {
      ...
      return parsed >= Date.now() - (count * 24 * 60 * 60 * 1000);
    }
    ...
    function renderDigestSettings(projectId, st, { mobile = false } = {}) {
      ...
      section.className = mobile
        ? 'project-monitoring-digest is-mobile'
        : 'project-monitoring-digest';
      ...
      status.className = settings.enabled
        ? 'badge badge-tone-green project-monitoring-digest-status'
        : 'badge badge-tone-muted project-monitoring-digest-status';
      ...
      cadence.value = ['hourly', 'daily', 'weekly'].includes(settings.cadence_preset)
        ? settings.cadence_preset
        : 'daily';
    ...
    function makeActionButton(label, projectId, role = 'secondary') {
      const normalizedRole = ['primary', 'secondary', 'ghost', 'destructive'].includes(String(role || ''))
        ? String(role || '')
        : 'secondary';
    ...
    async function confirmAcceptBaseline() {
      const confirm = typeof ctx.showConfirm === 'function'
        ? ctx.showConfirm
    ...
    async function openWatcherSettings(watcherId) {
      const open = typeof global.openWatchersModal === 'function'
        ? global.openWatchersModal
        : (typeof globalThis.openWatchersModal === 'function' ? globalThis.openWatchersModal : null);
    ...
    async function handleClick(event) {
      ...
      try {
        ...
        if (name === 'ack') {
          ...
          const note = [...(row?.querySelectorAll?.('[data-project-monitoring-note]') || [])]
            .find(item => item.dataset.projectMonitoringNote === String(action.dataset.fireId || ''))?.value || '';
