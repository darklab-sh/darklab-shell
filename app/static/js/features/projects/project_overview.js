  ...
  function createProjectOverviewController(context) {
    ...
    function renderRollups(st) {
      ...
      const certs = source.certificate_statuses && typeof source.certificate_statuses === 'object'
        ? source.certificate_statuses
        : {};
      const severities = source.finding_severities && typeof source.finding_severities === 'object'
        ? source.finding_severities
        : {};
    ...
    function portDivergence(target) {
      const provenance = target?.port_provenance && typeof target.port_provenance === 'object'
        ? target.port_provenance
        : {};
      const divergence = provenance.divergence && typeof provenance.divergence === 'object'
    ...
    function makeTabButton(projectId, label, tab, hints, role = 'secondary') {
      const button = typeof ctx.makeProjectButton === 'function'
        ? ctx.makeProjectButton(label, 'overview-open', projectId, role)
    ...
    function renderRecentState(st) {
      ...
      if (!changes.length) {
        const text = document.createElement('span');
        text.textContent = state === 'not-monitored' ? 'No monitoring window' : 'No recent target changes';
        wrap.appendChild(text);
        return wrap;
      }
      const text = document.createElement('span');
