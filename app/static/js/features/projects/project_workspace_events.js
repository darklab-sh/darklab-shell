(function projectWorkspaceEventsModule(global) {
  ...
  function createProjectWorkspaceEventsController(context) {
    ...
    function selectedEntityIds() {
      return ctx.selectedEntityIds?.() || new Set();
    }
    ...
    async function handleChange(event) {
      ...
      if (targetFilterControl) {
        event.stopPropagation();
        const projectId = String(targetFilterControl.dataset.projectId || selectedProjectId() || '');
      ...
      if (runFilterControl) {
        event.stopPropagation();
        const projectId = String(runFilterControl.dataset.projectId || selectedProjectId() || '');
      ...
      if (statusFilterControl) {
        event.stopPropagation();
        const projectId = String(statusFilterControl.dataset.projectId || selectedProjectId() || '');
      ...
      if (labelFilterControl) {
        event.stopPropagation();
        const projectId = String(labelFilterControl.dataset.projectId || selectedProjectId() || '');
    ...
    async function handleClick(event) {
      ...
      if (mobileAction) {
        ...
        if (action === 'project-menu') {
          const project = projectRows().find(item => String(item.id || '') === mobileProjectId)
            || ctx.projectSummary?.(mobileProjectId)?.project
            || null;
          if (!project) return;
      ...
      if (
        mobileProjectRow
        && ctx.projectMobileListView
        && ctx.projectMobileListView.contains(mobileProjectRow)
