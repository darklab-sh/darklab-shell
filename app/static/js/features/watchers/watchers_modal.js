...
const WATCHERS_CADENCE_PRESETS = [
  ['hourly', 'Every hour'],
  ['daily', 'Daily'],
  ['weekly', 'Weekly'],
];
const WATCHERS_COMMON_TIMEZONES = [
  'UTC',
  'America/New_York',
  'America/Chicago',
  'America/Denver',
...
function _watcherCurrentComparable() {
  ...
  return form
    ? _normalizeWatcherComparable(_collectWatcherDraft(form))
    : _normalizeWatcherComparable(_watchersState.draft || {});
}
...
function _watcherNewDraftHasMeaningfulInput(draft) {
  ...
  return !!(
    ...
    || data.timezone !== _watcherDefaultTimezone()
    || data.enabled === false
...
function _renderWatcherDiffSummary(summary = {}, { kind = '', truncated = false, titleText = 'Last diff' } = {}) {
  ...
  ].forEach(([titleText, items, sign]) => {
    if (Array.isArray(items) && items.length) wrap.appendChild(_renderWatcherSignalSection(titleText, items, sign));
  });
  return wrap;
}
...
function _renderWatchersDetail() {
  ...
  if (_watchersState.mode === 'missing') {
    ...
    missing.textContent = _watchersState.missingWatcherId
      ? `Watcher ${_watchersState.missingWatcherId} was not found. It may have been deleted.`
      : 'Watcher was not found. It may have been deleted.';
