...
const CONFIG_GLOBAL = typeof window !== 'undefined' ? window : globalThis;
...
function _configLogClientEvent(context, err, details) {
  if (
    ...
    importedLogClientError(context, err, details);
...
function _configSourceDetails(config, source) {
  const lazyAssetUrls = config && typeof config.lazy_asset_urls === 'object' && !Array.isArray(config.lazy_asset_urls)
    ? config.lazy_asset_urls
    : {};
  return {
    source,
  ...
  if (typeof document !== 'undefined') {
    ...
    if (node && node.textContent) {
      try {
        ...
        const fallback = CONFIG_GLOBAL?.APP_CONFIG
          && typeof CONFIG_GLOBAL.APP_CONFIG === 'object'
          && !Array.isArray(CONFIG_GLOBAL.APP_CONFIG)
          ? 'window.APP_CONFIG'
          : 'empty';
...
_configLogClientEvent('app config loaded', null, {
  ...
  level: 'info',
  ..._configSourceDetails(APP_CONFIG, (
    CONFIG_GLOBAL?.APP_CONFIG
    && typeof CONFIG_GLOBAL.APP_CONFIG === 'object'
    && !Array.isArray(CONFIG_GLOBAL.APP_CONFIG)
