...
from services.runs.output_model import LineRole, from_wire, line_event_from_legacy, to_legacy_entry

_PTY_TRANSIENT_LINE_PATTERNS = (
    re.compile(r"^rate:\s+.*\bdone\b.*\bfound=\d+\b", re.IGNORECASE),
    re.compile(r"^::\s*Progress:\s*\[", re.IGNORECASE),
)
_SCROLLBACK_FINDINGS_EXCLUDED_ROLES = {LineRole.prompt_echo, LineRole.pty_marker}


def normalize_pty_entry(entry) -> dict[str, object]:
    if isinstance(entry, dict):
        return to_legacy_entry(from_wire(entry), include_timestamps=False)
...
def filter_scrollback_finding_entries(entries: list[dict[str, object]]) -> list[dict[str, object]]:
    return [
        entry for entry in entries
        if _entry_role(entry) not in _SCROLLBACK_FINDINGS_EXCLUDED_ROLES
        and not is_transient_pty_line(str(entry.get("text", "")))
    ]
...
def shape_completed_pty_entries(synthesized_lines, transcript_mode: object) -> list[dict[str, object]]:
    ...
    if mode == "scrollback_findings":
        shaped = filter_scrollback_finding_entries(scrollback)
        if shaped:
            return shaped
        return filter_scrollback_finding_entries(final_frame or entries)
