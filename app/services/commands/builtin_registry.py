...
class BuiltinExecutionContext:
    ...
    _command_registry: object = field(default=_UNSET, init=False, repr=False)
    ...
    @property
    def command_registry(self) -> "BuiltinCommandRegistry":
        """Return the registry handling this execution.

        Registry-backed handlers use this for discovery and classification
        without reaching back through the aggregate executor module.
        """
        if self._command_registry is _UNSET:
            raise RuntimeError("Built-in execution context is not bound to a registry")
        return cast("BuiltinCommandRegistry", self._command_registry)

    def _bind_registry(self, registry: "BuiltinCommandRegistry") -> None:
...
class BuiltinCommandSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.match_strategy is BuiltinMatchStrategy.ROOT and not root and not self.exact_aliases:
            raise ValueError(f"Built-in command {handler_key!r} must declare a root or exact alias")
        if self.match_strategy is BuiltinMatchStrategy.WORKSPACE_ALIAS and not root:
            raise ValueError(f"Workspace-alias command {handler_key!r} must declare a root")
        if self.match_strategy is BuiltinMatchStrategy.FORK_BOMB and root:
            raise ValueError(f"Fork-bomb command {handler_key!r} cannot declare a root")
        exact_aliases = tuple(
            dict.fromkeys(
                " ".join(str(alias or "").strip().lower().split()) for alias in self.exact_aliases if str(alias or "").strip()
            )
        )
        required_features = tuple(
            dict.fromkeys(str(feature or "").strip().lower() for feature in self.feature_required if str(feature or "").strip())
        )
        ...
        if self.execution_owner is BuiltinExecutionOwner.BROWSER and not self.browser_fallback_stub:
            raise ValueError(f"Browser-owned command {handler_key!r} must declare an explicit fallback stub")
    ...
    def catalog_entry(self) -> dict[str, object]:
        ...
        if self.autocomplete:
            ...
            entry.update(
                {
                    "arguments": [],
                    "examples": (list(raw_examples) if isinstance(raw_examples, (list, tuple)) else []),
                    "flags": [],
                    "subcommands": [],
                }
            )
...
BuiltinCommandProvider: TypeAlias = Callable[[], Sequence[BuiltinCommandSpec]]


def build_builtin_command_spec(
    autocomplete_entry: Mapping[str, object] | None,
    *,
    handler_key: str,
    handler: BuiltinHandler,
    name: str,
    ...
    root: str | None = None,
    exact_aliases: tuple[str, ...] = (),
    feature_required: tuple[str, ...] | None = None,
    execution_owner: BuiltinExecutionOwner = BuiltinExecutionOwner.SERVER,
    browser_owned_subcommands: tuple[str, ...] = (),
    browser_fallback_stub: bool = False,
    match_strategy: BuiltinMatchStrategy = BuiltinMatchStrategy.ROOT,
    ...
    """Build one command spec from module-local catalog and grammar data."""
    entry = autocomplete_entry or {}
    autocomplete_root = str(entry.get("root") or "").strip().lower()
    raw_autocomplete = entry.get("autocomplete")
    autocomplete = normalize_autocomplete_spec(
        autocomplete_root,
        dict(raw_autocomplete) if isinstance(raw_autocomplete, Mapping) else {},
    )
    if feature_required is None:
        raw_features = entry.get("feature_required") or entry.get("requires_feature") or entry.get("feature")
        if isinstance(raw_features, (list, tuple, set, frozenset)):
            resolved_features = tuple(str(value) for value in raw_features)
        elif raw_features:
            resolved_features = (str(raw_features),)
        else:
            resolved_features = ()
    else:
        resolved_features = feature_required
    resolved_root = autocomplete_root if root is None else root
    return BuiltinCommandSpec(
        ...
        autocomplete_description=str(entry.get("description") or ""),
        exact_aliases=exact_aliases,
        feature_required=resolved_features,
        execution_owner=execution_owner,
        browser_owned_subcommands=browser_owned_subcommands,
        browser_fallback_stub=browser_fallback_stub,
...
class BuiltinCommandRegistry:
    ...
    def register(self, spec: BuiltinCommandSpec) -> None:
        ...
        if spec.handler_key in self._specs_by_key:
            raise ValueError(f"Duplicate built-in handler key: {spec.handler_key}")
        if spec.root and spec.root in self._specs_by_root:
            other = self._specs_by_root[spec.root]
            raise ValueError(f"Duplicate built-in root {spec.root!r}: {other.handler_key!r} and {spec.handler_key!r}")
        if spec.autocomplete_root and spec.autocomplete_root in self._specs_by_autocomplete_root:
            other = self._specs_by_autocomplete_root[spec.autocomplete_root]
            raise ValueError(
                f"Duplicate built-in autocomplete root {spec.autocomplete_root!r}: {other.handler_key!r} and {spec.handler_key!r}"
            )
        for alias in spec.exact_aliases:
            if alias in self._specs_by_exact_alias:
                other = self._specs_by_exact_alias[alias]
                raise ValueError(f"Duplicate built-in exact alias {alias!r}: {other.handler_key!r} and {spec.handler_key!r}")
        if spec.match_strategy is BuiltinMatchStrategy.FORK_BOMB:
            if self._fork_bomb_spec is not None:
                raise ValueError(
                    f"Duplicate built-in fork-bomb matcher: {self._fork_bomb_spec.handler_key!r} and {spec.handler_key!r}"
                )
            if self._fork_bomb_matcher is None:
                raise ValueError("Fork-bomb matcher strategy has no configured matcher")
        if spec.match_strategy is BuiltinMatchStrategy.WORKSPACE_ALIAS and self._workspace_alias_validator is None:
    ...
    def register_provider(self, provider: BuiltinCommandProvider) -> None:
        """Register every spec returned by one explicitly reviewed provider."""
        if self._frozen:
            raise RuntimeError("Built-in command registry is frozen")
        provider_name = f"{provider.__module__}.{provider.__qualname__}"
        specs = tuple(provider())
        if not specs:
            raise ValueError(f"Built-in command provider returned no specs: {provider_name}")
        for spec in specs:
            try:
                self.register(spec)
            except (TypeError, ValueError) as exc:
                raise ValueError(f"Invalid built-in command provider {provider_name}: {exc}") from exc
        self._provider_names.append(provider_name)
    ...
    def provider_names(self) -> tuple[str, ...]:
        return tuple(self._provider_names)
    ...
    def roots(
        ...
        *,
        ...
        roots = {root for root, spec in self._specs_by_root.items() if self._enabled(spec, cfg=cfg)}
    ...
    def registered_roots(
        ...
        *,
        ...
        if include_exact_alias_roots:
            roots.update(alias.split()[0] for alias in self._specs_by_exact_alias if alias)
        return tuple(sorted(roots))

    def exact_aliases(self, *, cfg: Mapping[str, Any] | None = None) -> tuple[str, ...]:
        return tuple(sorted(alias for alias, spec in self._specs_by_exact_alias.items() if self._enabled(spec, cfg=cfg)))

    def catalog(self, *, cfg: Mapping[str, Any] | None = None) -> list[dict[str, object]]:
        entries = [
            spec.catalog_entry() for spec in self._specs_by_key.values() if spec.user_facing and self._enabled(spec, cfg=cfg)
        ]
    ...
    def documented_commands(
        ...
        *,
        ...
        """Return the concise help rows declared by active user-facing specs."""
        commands: list[dict[str, object]] = []
        for spec in self._specs_by_key.values():
            if not spec.user_facing or not self._enabled(spec, cfg=cfg):
                continue
            entry: dict[str, object] = {
                "name": spec.name,
            ...
            if spec.root:
                entry["root"] = spec.root
            elif spec.exact_aliases:
                entry["exact"] = spec.exact_aliases[0]
            commands.append(entry)
        return commands

    def autocomplete_registry_data(self) -> dict[str, object]:
        """Return the normalizer-compatible autocomplete source from specs."""
        commands: list[dict[str, object]] = []
        for spec in self._specs_by_key.values():
            if not spec.autocomplete_root or not spec.autocomplete:
                continue
            entry: dict[str, object] = {
                "root": spec.autocomplete_root,
                "description": spec.autocomplete_description or spec.description,
                "autocomplete": _thaw_value(spec.autocomplete),
            }
    ...
    def autocomplete_context(
        ...
        *,
        ...
        for spec in self._specs_by_key.values():
            if not spec.autocomplete_root or not spec.autocomplete or not self._enabled(spec, cfg=cfg):
                continue
