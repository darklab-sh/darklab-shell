...
import config_paths
from config_builder import AppConfig as AppConfig, ConfigLoadError, PROJECT_NAME
...
def __getattr__(name):
    # Existing explicit imports retain their model/helper API during extraction.
    return getattr(_builder, name)


def _config_log_path(value):
    return _builder._config_log_path(value)


def _record_config_load_failure(**kwargs):
    extra = {"phase": kwargs["phase"], "source": _config_log_path(kwargs["source"]), "key": kwargs.get("key", "")}
    error = kwargs.get("error")
    if error is not None:
        extra["error"] = _builder._config_log_value(type(error).__name__ if isinstance(error, BaseException) else error, 120)
    log.error("CONFIG_LOAD_FAILED", extra=extra)


def get_config_load_summary():
...
def load_config(conf_dir=None, local_conf_dir=None):
    """Read startup inputs, evaluate them once, and replay structured startup events."""
    CONFIG_LOAD_WARNINGS.clear()
    CONFIG_LOAD_SUMMARY.clear()
    roots = config_paths.config_roots(conf_dir if conf_dir is not None else APP_CONF_DIR or None,
                                    local_conf_dir if local_conf_dir is not None else APP_LOCAL_CONF_DIR or None)
    base_path = roots.shipped / "config.yaml"
    local_path = config_paths.config_asset_paths("config.yaml", shipped_conf_dir=roots.shipped,
                                                local_conf_dir=roots.local).local
    CONFIG_LOAD_SUMMARY.update({
        "conf_dir": _config_log_path(roots.shipped), "local_conf_dir": _config_log_path(roots.local),
        "local_overlay": local_path.exists(),
        "supported_local_overlays": len(config_paths.supported_overlay_assets(shipped_conf_dir=roots.shipped)),
        "present_local_overlays": list(config_paths.present_local_overlays(
            shipped_conf_dir=roots.shipped, local_conf_dir=roots.local)),
        "overlays": [], "env_keys": [],
    })
    log.debug("CONFIG_SOURCE_SELECTED", extra={key: CONFIG_LOAD_SUMMARY[key]
              for key in ("conf_dir", "local_conf_dir", "local_overlay")})
    layers = []
    display = _builder.config_defaults()
    allowed = _builder._flatten_config_paths(display) | {"full_output_max_bytes"}
    for path in (base_path, local_path):
        overlay = _load_yaml_config(path)
        layers.append((str(path), overlay))
        _builder._merge_dict_data(display, overlay)
        configure_config_log_fallback(log, log_format=display.get("log_format", "text"),
                                      app_name=display.get("app_name", PROJECT_NAME), app_version=APP_VERSION)
        known, unknown = _builder._overlay_path_counts(overlay, allowed)
        extra = {"source": _config_log_path(path), "known_keys": known, "unknown_keys": unknown}
        if known:
            CONFIG_LOAD_SUMMARY["overlays"].append(extra)
            log.debug("CONFIG_OVERLAY_APPLIED", extra=extra)
        else:
            log.debug("CONFIG_OVERLAY_CHECKED", extra={**extra, "present": path.exists()})
    try:
        result = _builder.build_config(layers, dict(os.environ))
    except ConfigLoadError as exc:
        CONFIG_LOAD_WARNINGS.extend(getattr(exc, "warnings", ()))
        for level, event, extra in getattr(exc, "events", ()):
            getattr(log, level)(event, extra=extra)
        raise
    CONFIG_LOAD_WARNINGS.extend(result.warnings)
    CONFIG_LOAD_SUMMARY.update(result.summary)
...
CFG = load_config()
_LOADED_CONFIG_SNAPSHOT = {
    "values": deepcopy(CFG.model_dump()), "provenance": dict(CFG._provenance),
    "warnings": deepcopy(CONFIG_LOAD_WARNINGS), "app_version": APP_VERSION,
    "loaded_at": datetime.now(timezone.utc).isoformat(), "load_pid": os.getpid(),
}
