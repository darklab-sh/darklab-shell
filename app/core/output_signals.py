...
from core.output_command_entities import CommandEntityState, command_entity_state
...
def _extract_entities_for_command(
    ...
    *,
    ...
    entity_state: CommandEntityState | None = None,
) -> list[dict[str, object]]:
    stripped = normalized_text if normalized_text is not None else _normalize_signal_text(text)
    plain = ansi_stripped_text if ansi_stripped_text is not None else _strip_ansi_codes(str(text or "")).rstrip("\n\r")
    if entity_state is not None:
        return entity_state.entities_for_line(stripped, source_line)
...
class OutputSignalClassifier:
    ...
    def __post_init__(self) -> None:
        ...
        self.command_entities = command_entity_state(self.root, self.command, self.target, self.extra_domain_suffixes)
        self.url_template = ""
        if self.root == "ffuf":
    ...
    def classify_line(self, text: str, cls: str = "") -> dict[str, object]:
        ...
        if can_extract_entities:
            entities = _extract_entities_for_command(
                ...
                entity_state=self.command_entities,
            )
            if entities:
                if historical:
