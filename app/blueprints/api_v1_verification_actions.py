...
"""API v1 preview and launch routes for guarded finding verification."""
...
def api_project_finding_verification_action_preview(project_id, finding_id, check_id):
    ...
    except (ProjectWorkspaceError, VerificationActionError, TeamPermissionDenied) as exc:
        return _error(exc)
...
@api_routes.require_api_auth
def api_project_finding_verification_action_launch(project_id, finding_id, check_id):
    try:
        session_id = api_routes._require_session_id()
        owner_scope = api_routes._api_request_scope()
    ...
    if not api_routes.broker_available():
        response, status = api_routes._api_json_error(
            "broker_unavailable", api_routes.broker_unavailable_reason(), 503
        )
    ...
    api_routes.log.info("API_PROJECT_VERIFICATION_ACTION_LAUNCHED", extra={
        "ip": get_client_ip(),
