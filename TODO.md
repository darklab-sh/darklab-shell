...
- **Replace frontend compatibility globals with explicit imports.**
  - Goal: remove implicit app-to-app dependencies on `window.*` and bare browser globals while keeping intentional browser boundaries such as server bootstrap data (`window.APP_CONFIG`, `window.PermData`), classic vendor globals, documented test/debug hooks, and lazy-loader placeholders that genuinely need to exist before a lazy module downloads. Most app files are still IIFE/script-style modules loaded through ESM entry points, so this is an IIFE-to-ESM migration for most files, not just a global-read-to-import swap.
  - Prioritize completeness and consistency over churn: replace app-owned global reads with explicit imports or explicit state APIs first while leaving temporary compatibility publishes in place, then remove the publishes once each ownership area is fully migrated. Work bottom-up from producer modules toward hub consumers so `app.js` and `controller.js` do not need to be revisited repeatedly.
  - Start by improving `scripts/inventory_frontend_modules.mjs` so it classifies globals by purpose: intentional bootstrap data, vendor global, lazy placeholder, test hook, compatibility export, and compatibility read. Add an allowlist file with owner, reason, and removal target, then tighten `npm run assets:inventory:check` in stages so non-allowlisted app reads and later non-allowlisted app publishes fail.
  - Establish the low-risk named-export pattern in modules that do not own reassigned live state: `core/utils.js`, `core/dom.js`, `core/session_core.js`, `core/history_core.js`, `core/output_core.js`, `core/runner_core.js`, `core/workspace_core.js`, and the shared `ui/` helpers. Use named exports from the owner module and avoid broad barrel files until a cluster has settled.
  - Treat `core/config.js` as a small special case: migrate internal helper reads through explicit imports, but keep `window.APP_CONFIG` as an intentional server-bootstrap global and record that in the allowlist so it is not removed during cleanup.
  - Treat `core/state.js` as its own high-risk state-boundary slice, not as a low-risk named-export swap. The module's `tabs`, `activeTabId`, autocomplete flags, composer flags, welcome flags, timers, and pending-kill state are live `Object.defineProperty` globals that many files directly reassign; ESM imports are read-only live bindings, so `import { activeTabId }` followed by assignment would fail. Convert direct global reads/writes to the existing `APP_STATE_API`, `getTabs`/`setTabs`, `getActiveTabId`/`setActiveTabId`, `getComposerState`/`setComposerState`, or a deliberately imported mutable state object. Decide and document the write path before touching callers.
  - Migrate producer clusters before hub consumers. Start with self-contained shared producers, then `tabs.js` and `features/tabs/*`, then `output.js`, then `runner.js` and `features/runner/*`, then `pty.js`. Keep temporary compatibility exports until each whole cluster imports cleanly because this area touches active runs, SSE, tab lifecycle, exports, welcome state, workspace commands, and PTY attach/recovery.
  - Untangle shared UI, composer, and hub files after the producers they consume are stable: `ui_helpers.js`, `features/terminal/composer_controller.js`, `features/terminal/local_commands.js`, `features/mobile/mobile_shell_layout.js`, `features/mobile/mobile_menu_actions.js`, then the heavier hubs `app.js` and `controller.js`. Split this into composer state, overlay helpers, mobile layout helpers, local command dispatch, and final hub cleanup so each slice can carry focused tests.
  - Keep `app.js` and `controller.js` late unless a narrow helper extraction is clearly independent of runtime-core producers. They are heavy consumers of tabs, runner, output, welcome, autocomplete, theme, history, workspace, and lazy-loader contracts, so migrating them early would force compatibility shims in both directions.
  - Convert welcome/autocomplete state alongside the state-boundary and composer slices, not as incidental hub cleanup. Direct assignments such as `acIndex = ...`, `acSuppressInputOnce = ...`, `_welcomeActive = ...`, `_welcomePromptAfterSettle = ...`, and `_welcomeBootPending = ...` need explicit setter/state-object paths before their globals are removed.
  - Convert history and search after the tabs/output/state producers they consume are stable: `history.js`, `features/history/*`, `search.js`, and `features/run-comparison/*`. Work from pure row/action helpers toward top-level history and comparison controllers, with coverage for restore, permalink, delete/star, compare, and project-filter flows.
  - Convert lazy feature clusters one at a time: Atlas, Projects workspace, Findings Board, Preferences panels, Command Registry, Workflows, Schedules, Watchers, Status Monitor, and Tour. Each cluster should export its public entry points, let `core/lazy_assets.js` keep and return the loaded module API, replace direct `window.someFeatureFunction()` calls with that API, then remove the lazy module's broad `window.*` publishes when direct callers are gone.
  - Redesign `core/lazy_assets.js` as the main boundary. Loader functions should return module API objects such as `loadAtlasOverlay()`, `loadProjectWorkspace()`, and `loadOptionsPanels()`. Keep placeholder globals only for user-facing commands or open functions that truly must work before a lazy module has loaded.
  - Convert shared export and permalink modules last: `export_html.js`, `export_pdf.js`, `permalink.js`, and `permalink.entry.js`. Replace `ExportHtmlUtils` and `ExportPdfUtils` globals with named exports consumed by tabs, permalink, and lazy PDF loading, while keeping real vendor boundaries such as `jspdf` separate.
  - Final cleanup: run the inventory report, mark every remaining app-owned `window.*` publish as permanent, test-only, lazy placeholder, or removable, then delete anything not allowlisted. Update `ARCHITECTURE.md` so the compatibility-globals section becomes intentional browser globals, update `CHANGELOG.md`, and remove this TODO when complete.
  - Verification should include `npm run assets:inventory:check`, `npm run assets:check`, focused Vitest coverage for touched clusters, and focused route tests when templates/bootstrap/lazy manifest behavior changes. Playwright source and bundle smoke coverage is mandatory, not optional, for the state boundary, tabs, runner, lazy loader, PTY, and other runtime clusters where removed publishes can break active runs, SSE, tab lifecycle, lazy feature loading, or attach/recovery paths without a static inventory signal.
