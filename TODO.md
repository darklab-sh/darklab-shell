...
- [ ] Extend the finding model so assessors can record issues that no parser or import knows about:
  - Move saved remediation guidance onto the stable remediation identity already returned by finding reads. Require an explicit human merge when affected subjects or vulnerability/rule identities do not match exactly.
