...
- **Structured output model**
  - Goal
    ...
    - Each phase is independently shippable, but phases land in numerical order. The remaining work starts from typed producers/consumers, headered artifact storage, structured search-text derivation, and upgrade-on-read compatibility for historical artifacts. Phase 6 changes the output payload shape while keeping SSE/NDJSON event types stable.
