...
- **Interactive PTY mode follow-ups after the first pass**
  - **Current state:** `mtr --interactive <host>` has a guarded PTY path behind `interactive_pty_enabled`, uses dedicated `/pty/runs` start/stream/input/resize routes, brokers PTY events through Redis in multi-worker deployments, renders the browser terminal with vendored xterm.js, and persists completed PTY runs through the normal history/output path using server-side terminal capture.
