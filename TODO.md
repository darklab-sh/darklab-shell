...
- **Open product decisions** — resolve these before coding the related phase items so the work does not churn mid-implementation. Decisions are grouped by the phase they gate; resolved decisions carry an inline `Decision:` line.
  ...
  - **Phase 3 decisions**
    ...
    - **Diagnostics split axis** — which buckets does the activity section split into (counts vs status vs config, or some other axis)? (gates Phase 3 diagnostics item)
      - Decision (proposed): split by **concept type**, not by sub-system, into three sub-blocks each with its own H3 header: **Counts** (runs total, runs failed, runs killed, cache hits, FTS corpus size — all pure numeric metrics), **Status** (OK/warn/fail indicators — Redis health, DB health, vendor assets, transport state; consumes the Workstream D semantic colors via `.diag-ok` / `.diag-warn` / `.diag-fail` / `.diag-muted`), **Config** (runtime settings — share-redaction enabled, custom-rule count, persistence flag, session-token mode, notification flag). Put the generated-at timestamp at the top of the Activity section (already planned separately in the Phase 3 list). Rationale: the current mixed block forces the reader to visually re-sort entries to answer "is anything wrong?" vs. "what are the numbers?"; a concept-based split answers each question in one glance and aligns color usage to the Workstream D rule (status has semantic color, counts and config do not).
  - **Phase 4 decisions**
