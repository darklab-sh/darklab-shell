...
- **Best-case project evidence packages**
  - **Current state:** `build_evidence_package_archive` (`project_workspace.py`) builds a zip with `manifest.json` and any included artifacts under `artifacts/`. The Packages tab lists package rows with `Download`, `Delete`, and `View manifest` actions, while package creation is still a single name/description/redaction-mode/include-artifacts row. `redaction_mode` is metadata-only — "redacted" and "raw" produce byte-identical archives.
  ...
  - **Wizard UX (four-stage in-modal stepper inside the existing project modal, not a nested modal)**
    ...
    - **Stage 4 — Preview and build.** Manifest tree with counts and estimated archive size. List of items that would be skipped and why (artifact missing, transcript over cap, run no longer linked). Confirm → backend builds → row appears in the Packages tab with `Download` / `Delete` / `View manifest` actions visible immediately.
  - **Package contents (zip layout)**
    ...
    - `findings/findings.json` and `findings/findings.md` — full finding rows with title, scope/severity, raw line, review state, source run id/command, source line number, associated target, labels, and annotations.
  ...
  - **Phase 1 — Wizard with manifest-only output.** Build the four-stage wizard described above against the existing `build_evidence_package_archive` payload. Selection tree, presets, preview, async build, list refresh. No `index.html` yet, no redaction, and the redaction control stays hidden. Server-side: extend the manifest schema to record explicit selected entity ids, `package_format_version`, package options, and private-notes inclusion state, then respect those selections during archive build.
  - **Phase 2 — Temp-file-backed archive builds.** Replace `BytesIO` with a temp-file-backed build and `send_file(path)` before rendered HTML/transcripts make normal packages heavier. Keep `evidence_package_max_mb` and artifact-count caps, but ensure package build memory does not scale with archive size.
  - **Phase 3 — `index.html` + permalink renderer reuse.** Render `runs/<run-id>.html` through `permalink_base.html` at build time. Generate `index.html` with inlined CSS/fonts, sortable findings table, target chips, run list, artifact links. Redaction control stays hidden and generated HTML/transcripts remain raw-only until Phase 5. This is where the package starts to feel like a darklab_shell-native artifact instead of a generic JSON dump.
  - **Phase 4 — `README.md` companion and honest skipped-items reporting.** Render the same content as `index.html` in plain Markdown. Surface skip reasons on both surfaces ("3 artifacts referenced but not included; see metadata") so consumers know what is and is not in the bundle. Redaction control still stays hidden; Markdown and metadata outputs remain raw-only until Phase 5.
  - **Phase 5 — Redaction pass and the "redacted" preset.** Implement redaction across manifest contents AND rendered HTML transcripts via `redact_line_entries`. Only after that lands, expose the redaction control in the wizard and add a `Redacted Evidence` preset.
  - **Phase 6 — "Re-package with same selection" action.** Action on existing package rows that re-runs the wizard pre-filled with the prior package's selections. After the user reviews findings or re-runs scans, the natural workflow is "re-export with the same shape"; without this, the wizard becomes a tax instead of a feature.
  - **Trade-offs and gaps**
    - **Redaction must precede the redacted preset.** Until manifest contents and rendered HTML transcripts both flow through `redact_line_entries`, the wizard's redaction control stays hidden. Shipping a "redacted" preset that is not actually redacted is the kind of mistake users would not catch until it had already gone outside the org.
    - **Build cost.** Rendering HTML for many runs is real work. Cap rendered transcript size per run (config-driven), keep the existing total package size cap, and stream to a temp file (Phase 2) so the build cost shows up as latency rather than RAM.
    ...
    - **Versioned packages.** Same project will produce many packages over time. Date-stamp list rows, allow same-name with different timestamps, and let "Re-package with same selection" branch from any prior package.
