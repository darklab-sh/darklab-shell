...
- **Best-case project evidence packages**
  ...
  - **Package contents (zip layout)**
    ...
    - `targets/targets.json` — landed first pass: selected target records with type/value/notes/source metadata and references to selected findings and runs that mention each target. Still future: inline labels and richer derived references.
    ...
    - `notes/` — landed first pass: `notes/annotations.json` exports selected-entity annotations only when `Include private notes/annotations` is enabled; otherwise the file records that private annotations were excluded. Still future: project notes and per-entity note Markdown.
    - `metadata/` — landed first pass: `metadata/labels.json` exports selected-entity labels for the package project, selected runs, findings, and artifacts. Still future: richer provenance metadata and round-trip import hints.
