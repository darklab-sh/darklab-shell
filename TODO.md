...
- **Phase 3: add a reusable disclosure helper** — ✅ **Done (2026-04-19)**
  - `bindDisclosure(trigger, { panel, openClass, hiddenClass, initialOpen, onToggle, stopPropagation, ...pressableOpts })` added in `app/static/js/ui_disclosure.js`. Composes `bindPressable` from Phase 2 for the trigger: the pressable owns click + `Enter`/`Space` + blur + optional refocus; the disclosure owns aria-expanded sync + panel class lifecycle + onToggle emission. Follows the `bindMobileSheet()` / `bindPressable()` shape: one function, element + options bag, idempotent via a `data-disclosure-bound` guard. Loaded in `index.html` immediately after `ui_pressable.js` so it's available to every module that needs it.
  - Behavior contract: aria-expanded is kept in sync with the open flag on every state change; `openClass` toggles on the panel when open (default `'open'`, pass `null` to disable); `hiddenClass` inverse-toggles when closed (for `u-hidden`-style panels); `panel: null` lets the caller own class mutation entirely (used by the rail sections where `applySectionsState()` is the sole class writer); `onToggle(open, { trigger, panel })` fires on user transitions and on imperative open/close/toggle calls that actually change state, but is deliberately suppressed on the initial sync so binding doesn't trigger side effects; the returned handle exposes `isOpen() / open() / close() / toggle()` for imperative coordination; `stopPropagation: true` opts into `e.stopPropagation()` on the activation event so disclosures can coexist with ambient outside-click dismissal (Phase 4b territory).
  - Migrated in this phase:
    - FAQ question toggles in `app.js` (5 aria-only wiring removed; handle-list collected so `faqHandles[0].open()` replaces the hand-rolled `faq-open` class + aria-expanded on the first item)
    - desktop rail Recent / Workflows section headers in `shell_chrome.js` (used `panel: null, openClass: null` so the helper only syncs aria-expanded; `onToggle(open)` updates `ui.recentOpen` / `ui.workflowsOpen`, writes prefs, and calls `applySectionsState()` — the sole writer of the `.closed` class — matching prior behavior)
    - mobile timestamps sub-menu in `mobile_chrome.js` (uses `hiddenClass: 'u-hidden'`; the returned handle's `close()` is called from `openMenuSheet()` so the sheet always opens with the submenu collapsed)
    - mobile recents advanced filters toggle in `mobile_chrome.js` (uses `hiddenClass: 'u-hidden'`; `onToggle` closes the recents filter dropdowns on collapse and re-syncs the filter UI)
  - `controller.js` dispatcher for `ts-toggle` simplified: the disclosure in `mobile_chrome.js` now owns the aria + submenu visibility toggle via the pressable's own click handler, so the dispatcher becomes a targeted no-op that only preserves "don't close the sheet on ts-toggle" semantics (the single exception to the menu-close-after-action rule).
  - Test coverage: new `tests/js/unit/ui_disclosure.test.js` with 23 cases covering initial aria state, custom openClass, hiddenClass inverse, panel-null caller-owns semantics, onToggle emit-only-on-transitions invariant, handle isOpen/open/close/toggle idempotency, data-disclosure-bound guard, stopPropagation opt-in, null guards (missing trigger / opts / bindPressable), refocusComposer forwarding, clearPressStyle forwarding, and Enter/Space activation via the underlying pressable. Test harness (`tests/js/unit/helpers/extract.js`) extended to concatenate `ui_disclosure.js` after `ui_pressable.js` into every `fromDomScript`/`fromDomScripts` bundle. The `app.test.js` `ts-toggle does not close the mobile sheet` case was rewritten to assert only the controller dispatcher's remaining contract (sheet does not close on ts-toggle); the submenu toggle behavior is covered in `ui_disclosure.test.js`.
  - Net delta: 518 Vitest (+23), 836 pytest, 171 Playwright — all green.
  - **Intentionally not migrated** (deferred to Phase 4b so one migration can fix the outside-click coupling):
    - save-menu dropdowns — three near-identical copies at `tabs.js:621`, `permalink.js:115`, `shell_chrome.js:383–384`. Their disclosure state is tightly coupled to outside-click dismiss (each site re-implements `closest()`/`contains()` in a slightly different way). Migrating the disclosure in Phase 3 alone would duplicate state between the helper and the legacy outside-click wiring; Phase 4b's `bindOutsideClickClose` will migrate both together so the save-menu becomes a single disclosure + dismissible pair with the trigger-exemption contract.
...
- **Phase 4: add two dismissible-surface helpers** (splitting what was one phase — scrim-backed overlays and ambient-click menus need different behavior contracts and are not interchangeable today)
  ...
  - **4a. Scrim overlay / modal helper** — ✅ **Done (2026-04-19)**
    - `bindDismissible(el, { level, isOpen, onClose, closeButtons, backdropEl, closeOnBackdrop })` added in `app/static/js/ui_dismissible.js`. Composes `bindPressable` from Phase 2 for close buttons (click + Enter/Space + blur + skip-refocus). Shape follows `bindMobileSheet()` / `bindPressable()` / `bindDisclosure()` prior art: one function, element + options bag, idempotent via a `data-dismissible-bound` guard. Loaded in `index.html` immediately after `ui_disclosure.js` so every module sees `bindDismissible` and `closeTopmostDismissible` as globals.
    - Behavior contract: `level` is one of `'modal' | 'panel' | 'sheet'`; `isOpen` and `onClose` are required; backdrop click defaults to `el` itself (click where `e.target === el` closes), or takes a separate `backdropEl` (lets sheets route dismissal through a detached scrim) — passing `closeOnBackdrop: false` or `backdropEl: null` disables backdrop wiring entirely; `closeButtons` accepts a single element or array (null/undefined entries are ignored); if a close button is already marked `data-pressable-bound='1'` the helper falls back to a plain click listener rather than double-binding. The returned handle exposes `isOpen() / close() / dispose()` for coordination; `dispose()` unregisters from the cascade registry AND clears `data-dismissible-bound` so the element can rebind.
    - `closeTopmostDismissible()` is the shared dispatcher invoked by the global Escape listener in `controller.js`. Level priority is `modal > sheet > panel`; within a level the most recently registered entry wins. Returns `true` if it closed something, `false` otherwise — so the Escape branch can `e.preventDefault(); return;` on a hit.
    - Migrated in this phase: FAQ overlay, Workflows overlay, Theme overlay, Options overlay, Shortcuts overlay, history panel (closeOnBackdrop:false — ambient outside-click is Phase 4b territory), kill confirmation modal, history-delete confirmation modal, share-redaction modal, mobile menu sheet (backdropEl: scrim), mobile recents sheet (backdropEl: scrim). Setup lives in a new `setupDismissibleOverlays()` function in `controller.js` called right after `setupMobileSheetDragClose()`. Removed from `controller.js`: eight individual backdrop click handlers (workflows, faq, shortcuts, theme, options, histDel, shareRedaction, kill), eight pure close-button handlers, the per-overlay Escape if-chain at the top of the keydown handler, and the redundant secondary Escape fallback (dismissibles are already handled by the unified dispatch — only the search-bar and search-term clears remain). Removed from `mobile_chrome.js`: the separate Escape-to-close handler for the menu/recents sheets, the scrim click handlers, and the close-button click handlers.
    - Test coverage: new `tests/js/unit/ui_dismissible.test.js` with 29 cases covering the guard rails (null el / missing opts / unknown level / missing onClose / idempotency), backdrop semantics (target === el, child clicks ignored, closeOnBackdrop:false, isOpen:false skip, backdropEl override, backdropEl:null), close-button semantics (single element, array, falsy tolerance, isOpen gating, pressable composition, pressable-absent fallback, pre-existing-pressable fallback), the handle API (isOpen/close/dispose + rebinding after dispose), and the cascade dispatcher (no-op when nothing is open, modal > sheet > panel, within-level recency wins, skip closed, close-at-most-one). The Vitest harness (`tests/js/unit/helpers/extract.js`) was extended to concatenate `ui_dismissible.js` after `ui_disclosure.js` into every `fromDomScript`/`fromDomScripts` bundle.
    - Net delta: 547 Vitest (+29), 836 pytest, 171 Playwright — all green.
  - **4b. Ambient outside-click menu helper** — ✅ **Done (2026-04-19)**
    - `bindOutsideClickClose(panel, { triggers, isOpen, onClose, exemptSelectors, scope })` added in `app/static/js/ui_outside_click.js`. Loaded in both `index.html` and `permalink_base.html` immediately after `ui_dismissible.js` so every module sees `bindOutsideClickClose` as a global. Shape follows `bindMobileSheet()` / `bindPressable()` / `bindDisclosure()` / `bindDismissible()` prior art: one function, element + options bag, null-guards its preconditions, returns a `{ dispose }` handle.
    - Behavior contract: `panel` may be `null` for sibling-set cases (multiple peer dropdowns sharing a parent — e.g. the two recents filter dropdowns on mobile), in which case containment is decided purely by `triggers` / `exemptSelectors`; `triggers` accepts a single element or array and is matched via `.contains()` (so clicks anywhere inside a trigger do not close); `exemptSelectors` is matched via `.closest()` against the click target (so clicks landing on or inside any matching ancestor do not close — used for `.hist-chip-overflow` + `[data-action="history"]` on the history panel, `.ac-dropdown` + `#mobile-composer` for autocomplete, `[data-recents-dropdown]` for the recents filter dropdowns); `scope` defaults to `document` but accepts any element (the recents sheet passes itself so only sheet-scoped clicks are evaluated, keeping outer-document clicks from reaching the handler); the handler short-circuits when `isOpen()` returns false so a no-op close is never triggered.
    - Trigger-exemption contract: the pressable(s) passed as `triggers` are exempt from outside-click dismissal even though they sit outside the panel element — the helper treats a click that lands on a registered trigger as "inside". This replaces the hand-rolled `e.stopPropagation()` pattern every pre-refactor outside-click site used — trigger clicks now flow naturally and the helper recognises them as part of the surface rather than as "outside" clicks.
    - Migrated in this phase (six sites): the three near-identical save-menu dropdowns (`tabs.js` per-tab save-menu inside `_createTabPanel`, `shell_chrome.js` HUD save-menu, `permalink.js` save-menu) each collapse a hand-rolled document click listener plus a hand-rolled `e.stopPropagation()` on the trigger into one `bindOutsideClickClose` call with `triggers: saveBtn`; `controller.js` global-dismiss block (three stacked document click listeners for autocomplete, history panel, and mobile menu) collapses to two `bindOutsideClickClose` calls — history panel with `triggers: histBtn` + `exemptSelectors: ['.hist-chip-overflow', '[data-action="history"]']`, autocomplete anchored on `shellPromptWrap` with `exemptSelectors: ['.ac-dropdown', '#mobile-composer']` — with the mobile-menu outside-click deliberately removed as redundant now that the Phase 4a scrim (via `bindDismissible`) covers the viewport and handles dismissal there; `mobile_chrome.js` recents filter dropdowns (two sibling dropdowns on one sheet) collapse to a single `bindOutsideClickClose(null, { scope: recentsSheet, exemptSelectors: ['[data-recents-dropdown]'], ... })` with `panel: null` because there is no single containing element.
    ...
    - Test coverage: new `tests/js/unit/ui_outside_click.test.js` with 19 cases covering guard paths (`panel: null` accepted, missing `opts` / `isOpen` / `onClose` each return `null`, handle exposes `dispose`), outside-click dismissal (closes outside, skips inside, skips on element itself, respects `isOpen`), trigger exemption (single, array, nested via `.contains()`, falsy entries ignored), exempt selectors (single, nested via `.closest()`, array), scope override, and handle `dispose()` on document and scope-override paths. The Vitest harness (`tests/js/unit/helpers/extract.js`) was extended with `UI_OUTSIDE_CLICK_SRC` bundled after `UI_DISMISSIBLE_SRC` in both `fromDomScript` and `fromDomScripts`.
    - Net delta: 566 Vitest (+19), 836 pytest, 171 Playwright — all green.

- **Phase 5: narrow the direct focus API** — ✅ **Done (2026-04-19)**
  - Two new helpers in `app/static/js/ui_helpers.js` are now the canonical wrappers for raw DOM focus/blur across the UI modules: `focusElement(el, { preventScroll })` collapses the repeated `try { el.focus({ preventScroll: true }) } catch (_) { el.focus() }` pattern into one helper (the try/catch fallback covers engines that reject the options arg), null-guards non-focusable targets, and returns a bool; `blurActiveElement()` blurs `document.activeElement` if it is blurrable, returning a bool so callers can branch cleanly. `focusComposerInput` was refactored to delegate to `focusElement`, eliminating the duplicate try/catch inside the helper module itself.
  - Migrated in this phase: two `searchInput.focus()` sites in `controller.js` (rail action dispatcher + search toggle button), the session-token-set modal's `input.focus()` in `controller.js`, the tab-rename input focus in `tabs.js:1085`, the history-filter root-input `focus({ preventScroll: true })` in `history.js:149`, and both theme-selector focus targets in `app.js:546–562` (selected card first, fallback to `themeSelect` — two stacked try/catch blocks collapsed to two `focusElement(target, { preventScroll: true })` calls). The `focusCommandInputFromGesture` function in `app.js` had a dead defensive fallback (`typeof focusComposerInput !== 'function'`) re-implementing the same try/catch in place — removed, since `ui_helpers.js` always loads before `app.js` in the template and the branch was unreachable. The `_blurActiveElement()` local in `tabs.js` (plus its two call sites inside `createTab` and `closeTab`) and the `chip.blur()` in `history.js:478` were migrated to `blurActiveElement()`.
  - Remaining direct focus/blur calls outside helper internals: `utils.js:83` (the hidden textarea inside the clipboard `execCommand('copy')` fallback — single-shot, self-contained, does not belong in a UI-module helper) and `ui_pressable.js:55` (the helper-internal blur after activation, which is what Phase 2's `bindPressable` owns). These are intentional and not migration targets.
  - Test coverage: new `tests/js/unit/ui_focus_helpers.test.js` with 9 cases covering `focusElement` (null-guard, non-focusable-guard, live-focus path, `preventScroll` forwarding, default no-args path, `preventScroll`-throws fallback) and `blurActiveElement` (null `activeElement` via `Object.defineProperty` override, non-blurrable `activeElement`, live-blur path). The test uses a `withActiveElement(value, fn)` helper that captures and restores the original `activeElement` property descriptor so per-test overrides don't leak.
  - Net delta: 575 Vitest (+9), 836 pytest, 171 Playwright — all green.

- **Phase 6: test and documentation follow-through** — ✅ **Done (2026-04-19)**
  - New `tests/js/e2e/interaction-contract.spec.js` asserts the contract the five helpers (`refocusComposerAfterAction`, `focusElement` / `blurActiveElement`, `bindPressable`, `bindDisclosure`, `bindDismissible`, `bindOutsideClickClose`) jointly encode — verified against real mounted chrome surfaces rather than helper fixtures. Eight Playwright cases across three describe blocks:
    - **Scrim overlays (5 parametric tests, one per overlay: FAQ, theme, options, workflows, shortcuts)** — each test opens the overlay with `#cmd` focused, then exercises all three close paths in sequence: explicit close button, backdrop click at `(10, 10)` outside the panel, `Escape` through the shared `closeTopmostDismissible()` dispatcher. After every close path the test asserts (a) the overlay is `hidden`/`u-hidden` and (b) `document.activeElement` is `#cmd` — verifying invariants 1 (refocus-composer) and 5 (scrim close via button / backdrop / Escape) simultaneously for every modal surface.
    - **Disclosures (2 tests)** — one FAQ-question case (scoped to `#faq-overlay .faq-body .faq-item` / `.faq-q` so the share-redaction modal's `.faq-item.modal-inline-field` doesn't false-positive) verifies `aria-expanded` stays in sync with the `.faq-open` class across a full open/close/open cycle; one rail-Workflows-header case verifies the `panel: null` caller-owns-visibility path (aria-expanded stays in sync with the section's `.closed` class via `applySectionsState()`, confirming the helper honours the caller-owned-visibility branch end-to-end).
    - **Ambient outside-click (1 HUD save-menu test)** — verifies the `bindOutsideClickClose` trigger-exemption contract end-to-end: trigger click opens, trigger click toggles closed (trigger is exempt from outside-click dismissal), reopen, inside-panel click stays open (helper treats clicks inside the panel element as non-dismissing — dispatched via `document.querySelector('.hud-save-wrap.open .save-menu').dispatchEvent(...)` so Playwright doesn't auto-scroll or force-focus), outside click closes (dispatched via `document.body.dispatchEvent(new MouseEvent('click', { bubbles: true }))` — target-agnostic, not dependent on viewport positioning of any specific element).
  - Coverage audit (performed before writing tests): invariants 2 (press-style clears) and 3 (`Enter` / `Space` activation) are thoroughly covered at the Vitest unit level — `ui_pressable.test.js` has explicit blur-after-activate and `clearPressStyle` double-rAF cases, plus Enter/Space native-vs-non-native gating cases. Duplicating those at the Playwright level would add cost without adding coverage, so no E2E cases were added for invariants 2 or 3. Invariants 1, 4, 5, 6 had no systematic E2E coverage and were the primary target of the new spec.
  - `ARCHITECTURE.md` gained a dedicated "UI Interaction Helpers" section in the Frontend Architecture block documenting each of the five helpers + two focus utilities: purpose, signature, idempotency guard (e.g. `data-pressable-bound`, `data-disclosure-bound`, `data-dismissible-bound`), and composition with neighbours (e.g. `bindDisclosure` composes `bindPressable`, `bindDismissible` composes `bindPressable` for close buttons). The six-point shared contract the family enforces was written out alongside so future readers can see invariants 1–6 in one place.
  - The ARCHITECTURE.md JS module load order listing was refreshed to include the four `ui_*` sibling modules in their canonical load order (`ui_pressable.js` → `ui_disclosure.js` → `ui_dismissible.js` → `ui_outside_click.js`), plus `export_pdf.js`, `mobile_sheet.js`, and `mobile_chrome.js` which were missing from the prior snapshot.
  - Test counts updated in all five tracked locations (per memory): `tests/README.md`, `CONTRIBUTING.md`, `ARCHITECTURE.md`, `~/git_docs/v1.5-merge-request.md`, `~/git_docs/v1.5-release-notes.md`. README.md file tree in the tests section gained the new `interaction-contract.spec.js` entry; tests/README.md appendix got an 8-row parametric table.
  - No user-visible behavior changes — the Phase 6 work is verification-and-docs only. No shortcut/help text updates were needed.
  - Net delta: 179 Playwright (+8), 575 Vitest, 836 pytest — all green.
  - Sixth and final phase of the UI Interaction Helper Refactor Plan; every scoped phase is now ✅ done and the `ui_interaction_refactor` branch is ready to merge back to `v1.5`.
