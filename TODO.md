...
- **Documentation structure refactor for scannability and navigation** — several core docs now preserve the right level of detail, but some sections still read as dense prose blocks or long mixed-purpose sections that are harder to browse than they need to be. Treat this as documentation information-architecture debt: keep the same substance and audience contract, but restructure content into clearer summary lines, flat bullets, short subsections, and more predictable section shapes so users can skim, jump, and compare changes faster.

### Documentation Structure Refactor Plan

- **Goal** — improve browsing, navigation, and reference usability across the main docs without reducing detail or collapsing nuanced rationale into shallow bullet lists. The target outcome is "same information, easier to scan".

- **Restructuring rules to apply consistently**
  - Lead long sections with a short framing sentence or summary bullet, then break detail into flat bullets.
  - Prefer one concept, behavior, constraint, or implementation detail per bullet instead of dense mixed paragraphs.
  - Split mixed-purpose sections into predictable sub-blocks when needed, such as `What it is`, `How it works`, `Operator notes`, `Developer notes`, `Risks`, or `Related files`.
  - Preserve prose where narrative sequencing actually matters, especially in architectural request flows and decision-history sections.
  - Avoid nested bullet sprawl; if a section starts growing hierarchy, promote the next level into a short subsection instead.
  - Keep tables of contents, per-section headings, and appendix anchors stable unless the new structure materially improves discoverability.
  - **When in doubt, leave as prose.** If a section's current prose has been stable and isn't mixing concerns, skip it — the cost of a bad bullet-ification is higher than the cost of a paragraph that reads fine today.

- **Completion heuristics (so "done" is falsifiable)**
  - No single bullet exceeds ~6 rendered lines; if it does, split or promote to a subsection.
  - No bullet bundles user-visible behavior + implementation notes + test/docs follow-through in one paragraph (release entries especially).
  - Release entries lead with a one-sentence outcome a reader can skim before deciding whether to expand the details.
  - Reference-shaped sections (module inventories, config tables, appendix listings) render as lookup tables or flat bullets, not prose walkthroughs.
  - Anchors referenced by other docs, tests, or `tests/py/test_docs.py` still resolve.

- **Canonical templates (promote from existing gold-standard sections — don't design in a vacuum)**
  - **Release-entry umbrella shape** (for multi-phase refactors under `### Changed` / `### Added`) — umbrella bullet stating scope + motivation, then one nested bullet per phase or subsystem, each with a repeatable sub-bullet set (`Contract / Migrated / Removed / Tests / Net delta` or a similar recurring shape). Working example: the `UI Interaction Helper Refactor` entry in `CHANGELOG.md` under `[1.5] — Unreleased` → `### Changed`.
  - **Release-entry single-shot shape** (for one-shot features and fixes) — single bullet leading with a one-sentence outcome, remaining detail in one follow-on paragraph or flat sub-bullets. Don't force the umbrella pattern onto single-concept changes.
  - **Merge-request doc shape** — `Summary / Validation / Risks / Docs`, dictated by `.gitlab/merge_request_templates/Default.md`. This contract is non-negotiable for `~/git_docs/*-merge-request.md`; restructure inside these sections, not across them. Working example: `~/git_docs/mobile-ui-refactor-merge-request.md`, `~/git_docs/ui-interaction-refactor-merge-request.md`.
  - **Architecture section shape** — short framing paragraph, then inventory or contract bullets; keep request-flow narratives as prose.
  - **Feature inventory shape** — consistent order across entries (e.g. `Purpose / Behavior / Limits / Configuration / Related files`).
  - **Testing overview shape** — `Suite purpose / Recommended workflow / Artifacts and where they land`.

- **Doc-by-doc scope**
  - `CHANGELOG.md`
    - Apply the release-entry umbrella shape to multi-phase `### Changed` entries; apply the single-shot shape to one-shot `### Added` / `### Fixed` / `### Removed` entries. Specify which shape applies before editing a section so `Added` / `Changed` / `Fixed` / `Removed` don't drift into inconsistency.
    - Split mixed bullets that currently bundle user-visible behavior, implementation notes, and test/docs follow-through into separate bullets.
    - Keep chronology and version semantics intact; this is formatting and scannability work, not release-note summarisation.
  - `README.md`
    - Review long feature and operator sections for paragraph-heavy blocks that should become summary + bullets.
    - Keep the end-user/operator voice, but separate `what the app does`, `how to run it`, and `where to look next` more clearly.
    - Revisit project-structure and configuration sections so they read as lookup references rather than prose walkthroughs.
  - `FEATURES.md`
    - Keep the high-detail feature inventory, but normalize section shape where possible so readers can skim feature purpose, behavior, limits, and configuration in a repeatable order.
    - Break up dense feature sections that currently mix examples, behavior notes, and authoring rules into clearer blocks.
  - `ARCHITECTURE.md`
    - Preserve the deep technical detail, but convert dense explanatory paragraphs into shorter chunks with bullet lists where the content is inventory-, contract-, or layer-shaped.
    - Keep request-flow narratives and rationale prose where sequence matters; do not flatten the whole document into bullets.
  - `CONTRIBUTORS.md`
    - Keep it concise, but verify that the contributor path and linked-doc map are cleanly chunked and scannable.
  - `DECISIONS.md`
    - Preserve rationale-first writing, but review older entries for places where tradeoffs, consequences, and implementation notes should be separated into labeled bullet groups for faster review.
  - `tests/README.md`
    - Keep the appendix exhaustive, but improve any paragraph-heavy overview sections so suite purpose, recommended workflow, and artifact guidance scan more like a handbook.
    - Ensure the appendix and overview follow a consistent "summary first, details second" pattern.
    - Every structural edit must still pass `tests/py/test_docs.py` (21 meta-tests pin test counts and appendix entries).
  - `THEME.md`
    - Review theme authoring and system internals for dense instructional prose that would read better as short steps, rules, and lookup bullets.
  - `~/git_docs/*-merge-request.md` and `~/git_docs/v1.5-release-notes.md`
    - Apply the summary-plus-bullets structure **inside** the fixed MR-template sections (`Summary / Validation / Risks / Docs`); never rewrite across section boundaries or drop required sections.
    - Separate user-facing impact, implementation scope, and verification notes more cleanly within each section.

- **Execution phases**
  - **Phase 1: promote templates from existing gold-standard sections**
    - Pick the working examples listed under "Canonical templates" above and lift their shape into a short templates reference (either inline in this plan or as a sibling doc).
    - Explicitly name one worked example per template so reviewers have something concrete to compare against.
    - Do not design templates from scratch — use what's already working.
  - **Phase 2: audit and classify against the templates**
    - Identify sections that already conform to a template and should be left alone.
    - Mark sections that are paragraph-heavy but fundamentally reference-shaped (template candidates).
    - Mark sections where prose is correct and should stay prose because sequence or rationale matters.
  - **Phase 3: pilot on `~/git_docs/` release and branch notes**
    - Lowest blast radius (smallest reader base, shortest shelf life) — ideal to validate the templates before touching long-lived docs.
    - Bring `~/git_docs/*-merge-request.md` and `~/git_docs/v1.5-release-notes.md` into the template model, inside the fixed GitLab-template sections.
    - If the templates need adjusting, revise them now before moving on.
  - **Phase 4: refactor high-traffic overview sections**
    - Apply to `README.md`, `CHANGELOG.md`, `tests/README.md`, `FEATURES.md` — where readers skim rather than read linearly.
    - Track `tests/py/test_docs.py` as a gate for every `tests/README.md` and `CHANGELOG.md` edit.
  - **Phase 5: refactor deep reference sections**
    - Rework long feature subsections, architecture inventories, theme reference sections, and older decision entries using the agreed section shapes.
    - Preserve anchors and cross-links where practical so other docs and tests do not drift.
  - **Phase 6: consistency pass**
    - Verify headings, list style, emphasis style, code/reference formatting, and related-doc links are consistent across the docs set.
    - Re-run `tests/py/test_docs.py` end-to-end; confirm all 21 meta-tests still pass.
    - Check that restructuring did not remove important rationale, caveats, or operator/developer guidance.

- **Guardrails**
  - Do not reduce technical detail just to make sections shorter.
  - Do not over-bullet architecture narratives or decision rationale that genuinely need prose.
  - Do not let "cleaner structure" drift into duplicating content across docs; improve shape first, then consolidate only where duplication is clearly harmful.
  - Preserve each document's audience boundary: end-user/operator docs should not quietly turn into developer docs, and vice versa.
  - MR docs under `~/git_docs/` must keep the `Summary / Validation / Risks / Docs` section contract from `.gitlab/merge_request_templates/Default.md` intact.
  - When bullet-vs-prose is a judgment call, leave as prose and flag for reviewer rather than converting speculatively.
