...
- **Best-case project evidence packages**
  - **Current state:** `build_evidence_package_archive` (`project_workspace.py`) builds a temp-file-backed zip with `manifest.json`, `README.md`, `index.html`, per-selected-run `runs/<run-id>.html` transcript pages, `findings/findings.json`, `findings/findings.md`, `targets/targets.json`, `metadata/labels.json`, `notes/annotations.json`, and any included artifacts under `artifacts/`, and the download route streams that path with cleanup after the response closes. The Packages tab lists package rows with `Download`, `Re-package`, `Delete`, and `View manifest` actions, and package creation uses a first-pass four-stage wizard for preset, entity selection, metadata, redaction mode, and package preview. The backend stores explicit selected entity ids, `package_format_version`, package options, private-notes inclusion state, and inline labels/annotations for selected package entities in the manifest, then limits archive artifacts to the selected artifact ids. Missing artifacts, missing selected runs, and transcript caps are surfaced through `index.html`, `README.md`, `skipped-items.json`, and the compatibility `skipped-artifacts.json` file. The static `index.html` and `README.md` include project notes, inline finding labels/annotations, and a Package Exports section that links recipients to the manifest, finding exports, target export, label/annotation metadata, and skipped-item report when present; the `index.html` findings table can sort offline by finding, severity, review state, and run. Redacted packages now apply share/export redaction rules to manifest contents, `index.html`, `README.md`, finding exports, target exports, metadata exports, run transcript pages, and package filenames; raw artifact bytes are excluded from redacted packages until artifact-content redaction exists.
  ...
  - **Package contents (zip layout)**
    ...
    - `README.md` — same human-readable summary as `index.html`, plain Markdown so it renders in code-review tools and terminals, with direct links to machine-readable package exports.
    - `index.html` — self-contained app-styled landing page (see polish notes below) with a Package Exports section for the manifest, findings, targets, labels, annotations, and skipped-item metadata.
    ...
    - `findings/findings.json` and `findings/findings.md` — landed first pass: selected finding rows with title, scope/severity, raw line, review state, source run id/command, source line number, associated target details when present, inline labels/private annotations when selected, and transcript links when the source run transcript is included. Still future: richer per-entity note Markdown.
    - `targets/targets.json` — landed first pass: selected target records with type/value/notes/source metadata, inline labels/private annotations when selected, and references to selected findings and runs that mention each target. Still future: richer derived references.
    ...
    - `notes/` — landed first pass: `notes/annotations.json` exports selected-entity annotations only when `Include private notes/annotations` is enabled; otherwise the file records that private annotations were excluded. Project notes are included in `index.html`, `README.md`, and `manifest.json`. Still future: per-entity note Markdown.
    - `metadata/` — landed first pass: `metadata/labels.json` exports selected-entity labels for the package project, selected runs, findings, targets, and artifacts. Still future: richer provenance metadata and round-trip import hints.
  ...
  - **Phase 1 — `index.html` + run transcript pages.** Landed first pass: packages now include an app-styled `index.html`, selected run transcript pages under `runs/`, selected artifact links, target chips, project notes, sortable findings, findings with inline labels/private annotations when selected, skipped-artifact reporting, and raw-only transcript output capped by the existing output-line setting. Still future: fuller live-permalink template reuse and stronger CSS/font snapshotting.
  - **Phase 2 — `README.md` companion and expanded skipped-items reporting.** Landed first pass: packages now include a human-readable `README.md` companion, and skipped artifacts, missing selected runs, and capped transcripts are surfaced in HTML, Markdown, and `skipped-items.json`. Still future: skipped finding-specific reasons, richer Markdown parity for entity-specific note files, and pre-build size estimation in the wizard. Redaction control still stays hidden; Markdown and metadata outputs remain raw-only until Phase 3.
