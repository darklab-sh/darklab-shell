...
- **Phase 2 - Storage and master key bootstrap**
  ...
  - `app/services/secrets/vault.py` lazily generates `<data_dir>/.secrets_master_key` (mode `0600`) on first vault use if `SECRETS_MASTER_KEY` is unset; emits a one-time `MASTER_KEY_GENERATED` log line. Docker uses the same app-owned lazy bootstrap path rather than a parallel entrypoint-only flow.
  - Existing key files are accepted only if they decode to 32 bytes. If the file exists with broader permissions, the vault repairs it to `0600` when possible before use.
  - Operator can override via `SECRETS_MASTER_KEY` env to use an externally managed key (KMS, Vault, manual operator key).
  - Operator key-source changes are process-start decisions: the derived wrapping key is cached per app process, so changing `SECRETS_MASTER_KEY` or the key file requires an app restart unless a purpose-built rotation helper clears and replaces the key as part of its own process.
  - Session-token rotate/migrate moves secret rows to the new session token but does not change ciphertext; migration is idempotent and leaves existing destination rows untouched.
  - **Phase 2 findings:**
    - The app-owned lazy bootstrap path is the only bootstrap path. Docker and non-Docker installs both use `resolve_data_dir()` and generate the key file only when the vault is first used.
    - Existing key files with broader permissions are repaired to `0600` before decoding. Invalid or unreadable key files raise `MasterKeyError` and surface through the secrets route as `vault_unavailable`.
    - Repeated `db_init()` runs create the `secrets` table and `idx_secrets_session_updated` index idempotently.
...
- **Phase 5 - Feedback and tests**
  ...
  - Frontend coverage: panel CRUD, replace-only value entry, no-reveal contract, mobile sheet parity, Options modal integration, missing-secret pre-launch error rendering.
...
- **Open Decisions** (recommended answers below; review before implementation starts)
  - **Master-key env-var format.** Question: is `SECRETS_MASTER_KEY` the base64-encoded form or raw bytes? **Recommend:** base64-encoded; service decodes and rejects startup if the decoded length is not exactly 32 bytes. Document in CONFIGURATION.md.
  - **HKDF salt and info constants.** Question: what salt + info parameters does HKDF-SHA256 use to derive the wrapping key? **Recommend:** hardcoded module constants — `HKDF_SALT = b"darklab_shell/secrets/v1"`, `HKDF_INFO = b"vault-wrap"`. Single per-deployment context; not per-secret (per-secret nonce already protects individual rows).
  - **Non-Docker dev bootstrap.** Question: how does the local dev path get a master key when `SECRETS_MASTER_KEY` is unset? **Recommend:** `app/services/secrets/vault.py` generates a one-time key file at `<data_dir>/.secrets_master_key` (mode `0600`) on first vault use; emits `MASTER_KEY_GENERATED` log line. Normal app startup already refuses an explicitly configured unwritable `data_dir` through the shared database/data-dir resolver, so the vault does not need a separate startup hook. Docker uses the same app-owned lazy bootstrap path.
  - **Env-vs-file precedence.** Question: when both `SECRETS_MASTER_KEY` and `<data_dir>/.secrets_master_key` exist, which wins? **Recommend:** env wins; log a warning `MASTER_KEY_FILE_IGNORED` once per startup so the discrepancy is visible.
  - **Secret name validation.** Question: allowed characters, length, case sensitivity for `name`? **Recommend:** accept names case-insensitively, normalize to uppercase before storing and returning, and intentionally treat lowercase/uppercase variants as the same secret. Regex after uppercasing: `^[A-Z][A-Z0-9_]{0,63}$`. Reject with `400 {error: "invalid_name"}` otherwise.
  - **Secret value size cap.** Question: max bytes for a single secret value? **Recommend:** 4096 bytes UTF-8 encoded. Covers JWT-style bearers; rejects truly oversized inputs that would balloon ciphertext.
  - **Secrets-routes rate limit.** Question: what dedicated limit blocks brute-force enumeration? **Recommend:** 30 req/min per session token across all `/session/secrets*` routes (matches `_project_write_limit` pattern). Return `429 {error: "rate_limited", retry_after: <seconds>}`.
  - **`secret set NAME` value entry.** Question: how is the value collected without echoing it into the transcript? **Recommend:** the built-in immediately opens a modal prompt (browser-owned built-in, same lifecycle as `theme` / `config` / `session-token`); transcript shows only `secret set NAME`. Pasting the value into the modal sets it via `POST /session/secrets`. No stdin-style entry in v1.
  - **`consumer_envs` shape and default.** Question: what does `consumer_envs` carry, and what's the default? **Recommend:** JSON array of env-var names this secret satisfies; default `[NAME]` (the secret's own name is its env-var binding). Users can set `consumer_envs: ["VT_API_KEY", "VIRUSTOTAL_TOKEN"]` if one credential satisfies multiple registry env declarations.
  - **Reveal vs replace-only.** Question: can a user view a stored secret value from the Options panel? **Recommend:** **no reveal in v1.** Edit modal supports "Replace" (set a new value) and "Delete" only. Eliminates the side-channel risk and the masked-input/reveal-toggle complexity. The reveal toggle described in Phase 4 should be removed unless this decision is overridden.
  - **Master key rotation procedure.** Question: how does the operator rotate the master key globally? **Recommend:** a new CLI helper (`python -m app.services.secrets rotate-master-key --new-key-file <path>` or `--prompt`) that walks every session's rows, re-wraps each with the new key, and atomically replaces `<data_dir>/.secrets_master_key`. The new key must never be passed as a command-line argument because argv can leak through shell history and process listings. The per-session `POST /session/secrets/rotate` route stays for the case where a single session's rows need re-wrapping under the current master.
  - **Help-line nudge from `var set`.** Question: do we steer users away from putting API keys into plaintext session variables? **Recommend:** if `var set NAME VALUE` is called with a NAME matching `*_API_KEY`, `*_TOKEN`, or `*_SECRET`, print a one-line hint: `Hint: API-key-shaped values should use "secret set NAME" instead — they stay encrypted at rest and out of transcripts.` Non-blocking.
