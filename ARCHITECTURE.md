...
`zap_connector` is the disabled-by-default operator boundary for an external OWASP ZAP service. Startup accepts only an HTTP(S) origin without credentials or request paths, an environment-variable reference for the API key, canonical IPv4/IPv6 target networks, verified TLS by default, and fixed concurrency, job-lifetime, and report-byte bounds. Enabling it requires every scope field; malformed or incomplete settings stop startup instead of leaving a partly active connector. `services/connectors/zap_config.py` exposes only normalized non-secret settings, rechecks the HTTP or HTTPS scheme at the connector boundary, and resolves the API key from the named environment variable only when a connector call needs it. `zap_scope.py` accepts one credential-free HTTP(S) URL, applies the shared unambiguous path normalization, caps DNS results at 16 distinct addresses, and requires every answer to fall inside the configured networks before returning an immutable reviewed target. Plan generation repeats that normalization for targets, roots, includes, and exclusions before it builds scope patterns. Disabled state, missing credentials, malformed targets, ambiguous paths, resolver failures, excess answers, and out-of-scope addresses use bounded machine codes. Loading configuration alone never resolves a target, contacts ZAP, or submits work.
...
`services/assessments/zap_connector.py` is the owner-scoped review and submission boundary used by the Project browser and API v1 routes. Preview accepts confirmed Project URL entity ids rather than caller-supplied target URLs, rechecks the active cycle and current anonymous HTTP profile, resolves every selected hostname against the configured networks, and returns the exact generated YAML plus a non-secret summary. Its digest covers the plan hash, selected entity ids, HTTP profile revision, policy, and exclusions. Submission repeats those reads and DNS checks, requires explicit confirmation and the fresh digest, and queues the exact rebuilt bytes without resolving the ZAP API key in a web process. Status and cancellation require the same personal or team owner and the exact Project, assessment, and check path. Public job state omits session and actor identity, private spool paths, target values, report bodies, and worker-only import identity; ready jobs add only their Files path and Atlas draft id. Submit and cancel audit events contain ids, policy, state, and counts without plan YAML or target values.
