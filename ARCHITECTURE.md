...
darklab_shell is a web-based shell for running network diagnostics and vulnerability scanning commands against remote targets. It uses Flask + Gunicorn on the backend, an ES module browser frontend with compatibility globals for older cross-file contracts, SQLite by default or Postgres for larger deployments, Redis for shared live-run state, and SSE for live output.
...
The frontend now enters through ES module entry points, but the app still keeps a small compatibility layer of browser globals for older cross-file contracts. The architecture relies on a deliberate import order:
...
**Compatibility globals.** App-owned JavaScript now enters through ES module entry points, but many modules still publish selected APIs onto `window` so existing lazy loaders, tests, and older cross-file call sites keep working. Those globals are compatibility contracts, not the delivery format. New module-to-module code should prefer explicit imports when it can do so without forcing a large unrelated rewrite. `npm run assets:inventory:check` keeps that bridge honest by failing when an app-level bare read points at a name that exists in another file but has no publish path.
