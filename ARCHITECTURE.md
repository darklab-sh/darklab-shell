...
FIRST EPSS and CISA KEV are shared public datasets, not owner-scoped Intel provider results. Migration `0046` stores accepted source metadata in `cve_risk_sources` and current per-CVE signals in `cve_risk_records`. Release images carry checksum-pinned compressed snapshots under `app/resources/cve_risk`; post-schema maintenance loads them only when the database does not already hold newer accepted data. The bundled import is a non-alerting baseline. Reads from `intel cve`, Atlas, Projects, reports, and evidence packages query the stored rows and never refresh a source or contact a provider.

Live bulk-feed refresh is disabled by default. When enabled, the scheduler claims a database lease per source, uses conditional HTTPS requests against fixed allowlisted URLs, honors standard proxy handling, caps downloads and attempts, validates the complete feed before acceptance, and leaves the previous snapshot in place after a failure. Accepted source rows retain the origin, source URL, version, model/catalog version, publication/retrieval/acceptance times, checksum, conditional-request metadata, record count, attribution, and terms link. `providers` exposes the current source age and opt-in state. Logs and metrics use only bounded source/outcome/count labels; audit rows do not contain requested CVEs, Project values, targets, packages, or provider payloads.

`finding_cve_links` keeps current public signals separate from owner-scoped finding observations. Atlas and Project finding reads enrich linked rows with explicit KEV, EPSS, source-version, and freshness fields. Server-side lists order KEV-listed findings first, then EPSS probability and percentile, then age and finding id; the Python ranking helper inserts a structured CVSS score before age when a caller already has one. The UI shows short KEV, EPSS, and stale-source labels instead of a composite score. API v1 exposes the same nested risk objects. Report and evidence-package builds snapshot only the selected CVEs together with source dates, checksums, attribution, and non-endorsement text, so later feed changes do not rewrite an existing export.

Accepted feed changes enqueue only linked CVEs in `cve_risk_work_items`. The worker processes a bounded number of owners through a durable cursor, isolates failures with savepoints and retry state, and creates owner-scoped `risk_escalations` plus observation and Project projection links. A bundled baseline cannot enqueue work. KEV additions activate immediately; EPSS uses configurable activation/reset thresholds with an armed/active state so scores around the boundary do not flap. Canonical events deduplicate by owner, remediation identity, source, feed version, and transition. One acknowledgement updates all Project projections, while digest delivery stays opt-in per Project. De-list, reset, and downgrade transitions remain in history without being presented as new actionable alerts.
...
- `cve_risk_sources` and `cve_risk_records` — shared, rebuildable FIRST EPSS and CISA KEV source metadata plus current per-CVE signals. Source rows keep version, publication/retrieval/acceptance times, checksum, origin, conditional-request metadata, attribution, terms, and record count. Record rows keep source-native signal data and source version without copying it into each owner-scoped finding.
...
- `cve_risk_refresh_leases` and `cve_risk_work_items` — shared coordination for one accepted feed refresh per source and bounded, resumable processing of changed CVEs. Work items retain owner cursors, retry state, and completion metadata so a failed owner doesn't replay finished work or starve later owners.
- `risk_escalation_states`, `risk_escalations`, `risk_escalation_observations`, and `risk_escalation_projects` — owner-scoped EPSS hysteresis state, canonical feed-driven escalation history, contributing findings, and Project projections. Acknowledgement belongs to the canonical event; Project digests opt in separately and deduplicate projections inside each Project.
...
- Prometheus counters, histograms, label normalizers, cardinality policies, and multiprocess registry setup live in `app/services/metrics/__init__.py`; workflow outcome, duration, capture-failure, cancellation, and recovery metrics live in `app/services/metrics/workflows.py`; CVE risk feed and escalation metrics live in `app/services/metrics/cve_risk.py`; scrape-time collectors for database, Postgres pool state, Redis, AI Redis coordination key pressure, broker mode, workspace, intel cache size, Atlas, findings, snapshots, AI queue health, and provider-secret health live in `app/services/metrics/collectors.py`
