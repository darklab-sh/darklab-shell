...
Operator backups are owned by `scripts/backup_system.py`, outside the Flask request path. The script resolves the same app config, writes SQLite backups through SQLite's online backup API or Postgres backups through `pg_dump`, copies filesystem-backed `data_dir` state, and records both logical app paths and physical Docker/host sources in its manifest so restores can line up database rows with run artifacts and workspace files. Unreadable host bind sources fail with operator guidance instead of a raw Python permission error.
