...
Product/package-version correlation is inference-only and fail-closed. A package observation must provide an exact PURL and version, either separately or as one version-bearing PURL. The package identity must match the cached advisory exactly, and the version must be explicitly listed or fall inside a supported SEMVER event range. A product observation must provide a complete CPE 2.3 identifier with an exact part, vendor, product, and version. CPE criteria are accepted only when an upstream normalizer marks the clause as complete, vulnerable, and non-negated; exact versions, explicitly declared all-version clauses, and numeric dotted NVD limits can match. A criterion that depends on an update, target software, hardware, or another environment field doesn't match when the observation doesn't contain that same field. Conflicting versions, generic product names, malformed events, incomplete NVD `AND` context, and ecosystem-specific ranges without an app-owned comparator don't match. Materialized candidates retain the source observation, target, run/import context, matching rule, affected range, advisory source, and data version, and they remain separate from findings confirmed by an active probe.
...
- `package_advisories` and `package_advisory_ranges` — shared schema reserved for the explicit OSV/local advisory acquisition modes described in the open assessment plan. The pure correlation service can evaluate normalized records supplied by a caller, but the current EPSS/KEV path neither populates nor queries these tables, and viewing a finding never performs package-advisory lookups.
