...
Live Web Review is the maintained scalar path for one approved URL. Its HTTPx step writes only the screenshot directory needed by the existing verified-artifact finalizer, and its Dalfox step streams discovery-only JSONL into the existing bounded parameter-observation parser. The workflow doesn't copy screenshot paths or parameter values between steps; durable transitions link the two normal runs while Files owns the image and structured run output owns the inventory.
