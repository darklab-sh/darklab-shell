...
OSV package applicability has a separate normalization and persistence boundary for operator-supplied full-record JSON arrays. It requires unique advisory ids, supported OSV 1.x schema versions, timezone-aware source dates, exact versionless PURLs, and either explicit versions or supported SEMVER event ranges. Duplicate ids, malformed supported ranges, and configured size, record, version, or range overflow reject the dataset. Withdrawn records, missing or versioned PURLs, and unsupported ECOSYSTEM or GIT ranges are counted and omitted instead of being guessed. Parsing performs no network request or database write. Explicitly accepted local data replaces the stored OSV snapshot inside one savepoint, retaining source advisory and schema versions, exact affected versions, SEMVER events, checksum, attribution, acceptance time, and expiry. Any failed replacement rolls back to the last complete accepted snapshot.
