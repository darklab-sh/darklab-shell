...
`nmap_script_evidence_catalog.py` maps every exact informational service-script selector to one evidence family and proves that the map stays aligned with the reviewed profile catalog while remaining disjoint from vulnerability scripts. `nmap_service_observations.py` accepts bounded, entity-safe Nmap XML for open ports and reads only structured `table` and `elem` children from those exact scripts. Each observation keeps the canonical port, bounded service name, script and evidence ids, source run, Nmap/parser versions, aware timestamp, and bounded path/value fields. The free-form `output` attribute, host scripts, output-only rows, unknown scripts, closed ports, and vulnerability-profile scripts are omitted. Depth, value, field, observation, byte, and XML-element limits report truncation or fail closed; parsing is read-only and labels every accepted row informational.
