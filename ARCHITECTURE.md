...
The browser's active product identity remains a two-tier compatibility model managed in `app/static/js/session.js`:
...
`SESSION_ID` is initialised at page load by preferring `session_token` over `session_id`. `updateSessionId(newId)` still switches the legacy browser identity at runtime without a page reload while the old Options controls remain active. Browser calls send that compatibility identity as `X-Session-ID` via `apiFetch()`. History, stars, saved Options state, and app-managed workspace files are scoped to the resolved owner; clearing a legacy browser token reverts to the UUID rather than losing the anonymous session. The supported terminal root is `credential`: status and list render safe metadata, while secret-bearing subcommands open the browser-owned controls and strip extra values from persistence.
...
**Server-side compatibility validation:** the central resolver validates `tok_`-prefixed legacy browser headers against `session_tokens` on every request and returns a typed authentication failure for a revoked or unknown token. That adapter is limited to the still-live browser session interface; API v1 no longer consumes it. Canonical anonymous UUIDs remain the only unpersisted personal identities accepted by the browser adapter. A valid principal credential resolves product ownership to its `personal_workspace_id`; the credential secret and id are never converted into owner keys.
