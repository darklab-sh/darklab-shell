...
| `GET` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/recommended-action` | Recomputes a secret-free launch plan from one saved assessment check and its current confirmed target. |
| `POST` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/recommended-action` | Starts the freshly confirmed safe or standard command action and links its run to the Project. |
...
| `GET` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/recommended-action` | Recomputes the browser's secret-free launch preview from one saved assessment check. |
| `POST` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/recommended-action` | Starts the freshly confirmed bounded command and hands the Project-linked run to the normal terminal flow. |
...
**Assessment action launches.** `services/assessments/action_plans.py` owns the shared secret-free plan contract for direct Assessment recommendations and finding verification. Every preview re-reads the frozen profile action and policy, confirms that the exact frozen target is still a confirmed Project entity, and returns the display command, target, policy, Project-only fan-out, request/time bounds, empty HTTP-profile context, credential-use declaration, availability reason, and a digest over the complete preview. Launch accepts only `confirmed: true` plus that current digest, recomputes the plan immediately, and permits only maintained safe or standard command templates. Archived Projects or cycles, changed targets, stale digests, workflows without a frozen input map, unsupported commands, and intrusive or destructive policy fail closed.
