...
`probe_targets.py` resolves either one exact entity id or one exact value to a confirmed, non-suppressed Atlas entity linked to the active owner-scoped Project. Missing, archived, unconfirmed, cross-owner, cross-Project, and ambiguous selections fail before command planning. `probe_plans.py` then composes that target with the ordinary-action registry and reviewed profiles without starting a run or contacting a provider. The catalog and planner treat a missing feature inventory as empty, so a feature-gated action fails closed unless its runtime availability was checked explicitly. The public plan keeps the exact command, target, policy, feature gates, evidence expectations, availability, and request, time, credential, and fan-out bounds. Its versioned digest projection intentionally omits labels and help text while covering every execution field, including a selected HTTP profile's identity and revision. Human-readable terminal and CLI previews label the first 12 digest characters as the approval digest; API and JSON responses keep the complete value used for confirmation. The older Assessment adapter continues hashing its complete payload, so extracting the shared confirmation check doesn't change existing Assessment digests.
