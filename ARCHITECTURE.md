...
`builtins_probe.py` registers the browser-owned `probe list`, `probe plan`, and `probe run` metadata. The lazy `probe_terminal.js` adapter calls the Project-scoped routes directly. Catalog reads use the active Project; plans and launches require an explicit Project and resolve exact terminal values to an entity id before planning. List and plan suppress History and Recent-command persistence, remain available to viewers for anonymous actions, and don't create a run, connector job, callback reservation, audit event, Assessment evidence, or separate probe record.
