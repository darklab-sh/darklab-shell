...
| `nuclei` | Adds `-ud /tmp/nuclei-templates`; uses owner-scoped `XDG_CONFIG_HOME=<workspace>/tools` when Files are enabled, except for explicit template updates | Redirects template storage to a `scanner:appuser` tmpfs directory that the scanner can update and the web process can inspect for an exact managed-template snapshot. Normal ProjectDiscovery config/resume state stays under the active personal/team workspace's `tools/` folder. Template updates keep their version marker in the same temporary lifecycle as the cache, so a retained workspace marker can't suppress downloading after container recreation. Output metadata records the template source for later Run Details, Atlas import, and evidence review. Silent. |
