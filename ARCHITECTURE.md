...
| Method | Endpoint | Description |
| -------- | ---------- | ------------- |
| `GET` | `/atlas` | Returns current-session Atlas entity counts by entity type, honoring optional source-run, orphan-source, and suppression filters. |
...
| `GET` | `/atlas/runs` | Returns recent or searched current-session source runs that contribute Atlas rows for the Atlas run filter. |
| `GET` | `/atlas/entities` | Returns a paginated current-session entity list, with optional `type`, text search, project, source-run, orphan-source, suppression, limit, and offset parameters. |
| `GET` | `/atlas/entities/export` | Downloads current-session Atlas entity rows as CSV or JSONL, honoring optional type, text, project, source-run, orphan-source, suppression, and limit filters. |
...
| `GET` | `/atlas/runs/<run_id>/cleanup-preview` | Previews disposable and curated single-source Atlas rows that can be removed when cleaning one source run while keeping the run transcript. |
| `POST` | `/atlas/runs/<run_id>/cleanup` | Detaches one current-session run from Atlas, deletes disposable rows that only came from that run, optionally deletes curated single-source rows, and recalculates remaining entity/finding counts. |
...
| `POST` | `/atlas/entities/suppression` | Suppresses or restores selected current-session Atlas entities without deleting their source data. |
| `GET` | `/atlas/entities/<entity_id>/delete-preview` | Previews related Atlas cleanup before deleting an entity. |
| `DELETE` | `/atlas/entities/<entity_id>` | Deletes one Atlas entity and its attached findings, with optional same-source cleanup for disposable or curated single-source siblings. |
| `PUT` | `/atlas/entities/<entity_id>/suppression` | Suppresses or restores one current-session Atlas entity without deleting its source data. |
| `GET` | `/atlas/findings` | Returns the paginated Atlas Findings queue with optional text, project, source-run, review-state, orphan-source, suppression, limit, and offset filters. |
...
| `POST` | `/atlas/findings/suppression` | Suppresses or restores selected current-session Atlas findings without deleting their source data. |
| `GET` | `/atlas/findings/<finding_id>/delete-preview` | Previews same-source cleanup before deleting a finding. |
| `DELETE` | `/atlas/findings/<finding_id>` | Deletes one Atlas finding, with optional same-source cleanup for disposable or curated single-source siblings. |
| `PUT` | `/atlas/findings/<finding_id>/suppression` | Suppresses or restores one current-session Atlas finding without deleting its source data. |
...
| `POST` | `/projects/<project_id>/findings/review` | Updates the review state for selected current-session findings that are visible through the project. |
