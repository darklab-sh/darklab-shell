...
The Project Web Surface read boundary pages only those verified HTTPx image artifacts from external runs linked to the selected Project. It joins each artifact to bounded structured-event metadata and the exact URL and host entities observed in the same run, while keeping the artifact id as the only path to authenticated image retrieval. Missing metadata, conflicting records for one artifact path, and missing or changed files remain visible as explicit states rather than being guessed or silently dropped. The response never includes captured markup or binary image data.
