...
**Asset composition.** CSS is split across ordered source files under `static/css/`, with `styles.css` kept as a compatibility entrypoint. JavaScript stays in classic scripts under `static/js/`, grouped into coarse shell bundles plus a self-contained permalink bundle. Runtime templates render CSS and JavaScript through the manifest-backed `asset_bundle()` helper. In source mode, Flask emits ordered source links from `app/static/build/manifest.json` with version query strings for local edit-and-refresh work. In bundle mode, it emits committed content-hashed files from `app/static/build/`. `assets.config.json` owns bundle membership and order, `npm run assets:sync` regenerates the committed output, and `npm run assets:check` fails when the build output, source coverage, or bundle order drifts.
