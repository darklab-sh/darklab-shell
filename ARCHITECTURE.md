...
`blueprints/projects_assessments.py` exposes the browser list, create, detail, update, deletion-preview, and deletion routes through the existing Projects blueprint. Reads use the same personal/team scope as the shared model. Mutations require `MUTATE_PROJECTS`, use the normal Project write limit, reject archived Projects, record assessment-specific audit events, and log only ids, profile versions, state transitions, and counts. Assessment deletion and its fail-closed audit record share one transaction.
