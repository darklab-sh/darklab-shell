...
Package-version correlation is inference-only and fail-closed. A structured observation must provide an exact PURL and version, either separately or as one version-bearing PURL. The package identity must match the cached advisory exactly, and the version must be explicitly listed or fall inside a supported SEMVER event range. Conflicting versions, generic product names, malformed events, and ecosystem-specific ranges without an app-owned comparator don't match. Materialized candidates retain the source observation, target, run/import context, matching rule, affected range, advisory source, and data version, and they remain separate from findings confirmed by an active probe.
