...
The package stays decomposed into profile loading, contracts, storage, evidence matching, coverage derivation, serialization, and read-model modules, all covered by the module-size ratchet. Quota failures use the existing Project workspace quota contract, and every query applies the same Project personal/team ownership predicate before returning or changing cycle state.
