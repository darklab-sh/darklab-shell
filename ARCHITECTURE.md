...
- Prometheus counters, histograms, label normalizers, cardinality policies, and multiprocess registry setup live in `app/services/metrics/__init__.py`; Project assessment transition, evidence, action, parser, and connector metrics live in `app/services/metrics/assessments.py`; workflow outcome, duration, capture-failure, cancellation, and recovery metrics live in `app/services/metrics/workflows.py`; CVE risk feed, NVD advisory acquisition, and escalation metrics live in `app/services/metrics/cve_risk.py`; scrape-time collectors for active assessment cycles, database, Postgres pool state, Redis, AI Redis coordination key pressure, broker mode, workspace, intel cache size, Atlas, findings, snapshots, AI queue health, and provider-secret health live in `app/services/metrics/collectors.py`
