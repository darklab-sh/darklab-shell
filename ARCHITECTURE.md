...
**Finding verification action launches.** `services/projects/verification_actions.py` owns one plan contract for the browser, API v1, and CLI. Every preview resolves the finding through Project scope, requires its exact typed `assessment_check` evidence link, re-reads the frozen profile action and policy, and confirms that the exact frozen target is still a confirmed Project entity. The plan contains the display command, target, policy, Project-only fan-out, request/time bounds, empty HTTP-profile context, credential-use declaration, availability reason, and a digest over the complete preview. Launch accepts only `confirmed: true` plus that current digest, recomputes the plan immediately, and permits only the maintained safe or standard command templates. Archived Projects or cycles, changed targets, stale digests, workflows, unsupported commands, and intrusive or destructive policy fail closed. The ordinary broker starts and Project-links the external run, so History, terminal streaming, and run cleanup keep their existing ownership. Once persistence and Project linkage complete, the launch-specific finalization hook idempotently attaches the run as typed `retest_run` evidence for the same Project, finding, and originating check. A failed or stale evidence link is logged without changing run finalization. `finding_triage_editor.js` uses the shared button and structured-confirmation primitives, prevents a second launch while one is pending, explains any evidence-backed suggestion, and only copies an accepted suggestion into the ordinary status control. Starting or completing the run never changes the finding's verification disposition; an authorized person must use the existing triage save. `assessment.action_launch` records ids and policy/action metadata while application logs and audit details omit the command and target value.
