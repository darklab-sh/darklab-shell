...
Release images are built once for `linux/amd64` from protected final or `-rc.N` semantic-version tags, with unchanged toolchain layers reused through a registry-backed BuildKit cache. The release job resolves the tagged Python base to an exact AMD64 manifest digest, passes that immutable reference to BuildKit, and records it in the image labels. Before the build, CI requires every versioned Dockerfile component and every top-level apt, pip, and Git install to map to the release-specific license inventory and required notices. Nmap 7.95 has its own NPSL 0.95 record and hash-pinned license text; public image publication also requires that record to show an upstream waiver or OEM authorization. Image smoke validation then resolves every declared notice path and compares the complete installed RubyGem set with the generated WPScan dependency manifest. CI pushes the canonical image to the GitLab Container Registry, verifies repository-free startup and architecture, then copies the already-verified `image@digest` manifest byte-for-byte to Docker Hub without routing it through a daemon image store. The Docker Hub digest must match before the protected pipeline generates a CycloneDX SBOM and Grype report from the pulled image, gates fixed Critical findings, emits SLSA provenance for the tag/commit/pipeline/digest relationship, and keylessly signs both immutable registry references. The generated deployment then starts with its normal four-worker, four-thread, notification-worker, and scheduler defaults against the bundled Postgres profile. That gate saves API state, creates a repository-free backup, changes the state, restores through `darklab-deploy`, and requires the original state to return before a final GitLab Release can be created. The public-image check separately verifies those signatures, the signed checksum manifest, and the independent issuer/identity pattern in the Docker Hub overview; it installs the deployment payload, pulls through its Compose file, requires both the HTTP and Compose health checks to pass, restarts the app, and confirms `/data` survives. Final and candidate tags run that same artifact chain, but only a final tag creates a GitLab Release. The final release job runs a versioned GitLab CLI image pinned to its multi-architecture manifest digest, and that reference plus the CI configuration hash are recorded in the signed release evidence. Each exact app tag is immutable within the workflow; the production Compose file uses a readable exact tag rather than `latest`.

Protected-tag compatibility gates share the repository-free runtime verifier across Docker and Podman, accept only `amd64` or `arm64`, and can add private SELinux relabeling to the external mounts. The verifier binds test-owned host directories at `/config`, `/data`, and `/workspaces`, loads an overlay, writes durable app and workspace markers, restarts the container, and executes an unprivileged Nmap SYN probe with the production capabilities. A native GitLab-hosted ARM64 runner builds with an isolated Docker-in-Docker service and executes the bundled tools as the scanner user; dedicated self-managed runner tags cover SELinux-enforcing Docker and rootless Podman startup. Each gate is disabled until its matching `RELEASE_*_COMPATIBILITY_ENABLED` project variable and any required self-managed runner are ready; enabling it makes the check blocking for that release. Production remains Linux AMD64 on Docker Compose until the broader lanes have passed and the published support matrix changes.
...
The deterministic deployment archive contains `compose.yaml`, `.env.example`, safe local-overlay starters and non-active replacement examples, the project's GNU AGPLv3 license, third-party notices, checksums, a pulled-image verifier, and a release-specific POSIX lifecycle command. The surrounding release payload adds the POSIX installer, CycloneDX SBOM, Grype report, SLSA provenance, build-input inventory, evidence index, and a keyless Sigstore bundle for `SHA256SUMS`. The inventory hashes the source-controlled build inputs, records effective Docker arguments and the pinned Python base manifest, and lists every network-fetching Dockerfile instruction. It explicitly distinguishes the byte-stable deployment archive from the image, whose apt and transitive dependency resolution prevents an honest byte-reproducibility claim; the signed image digest and SBOM capture the released result. The installer requires Docker Compose 2.20.0 or newer, verifies every downloaded file, validates the resolved Compose model, generates only the Postgres password, and records both registry references, digests, and measured image sizes. Archive handling requires a successful complete listing and rejects unsafe member paths before extraction; the lifecycle help labels its verified-bundle `install` entry point as the internal handoff used by `setup.sh`. Online upgrades download the signed checksum manifest and bundle first, verify the exact GitLab tag identity through a multi-architecture Cosign image pinned by digest, and accept only the archive checksum named once in that authenticated manifest. The explicit offline `--archive` path checks its adjacent checksum file and leaves publisher-signature verification to the operator. After an operator pulls the image, the installed verifier requires the registry digests to agree, matches `.env` to the reviewed image reference, and checks the local image's repository digest before startup. Volatile pull timing stays in a separate CI artifact so retries produce identical release archive bytes. The installer doesn't pull or start containers.
