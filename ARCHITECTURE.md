...
- `finding_remediation_dispositions` — one canonical review state for each owner-scoped exact affected subject plus normalized CVE or stable scanner rule. Review writes mirror the state to current matching finding rows for bounded filters and counts, while reads attach the same disposition to every matching observation reference without merging their validation, verification, confidence, or evidence.
