...
`project_assessment.js` owns per-Project Assessment cycle, filter, paging, disclosure, and scroll state plus the browser request flow. `project_assessment_renderer.js` owns the shared desktop/mobile coverage, category, target, check, empty, and error rendering. The entry controller and `project-assessment.css` load together on first use, so adding the tab doesn't increase the initial Projects payload.
...
`services/assessments/read_model.py` is the shared scope-aware read contract for cycle lists and detail. It returns bounded, deterministic check pages; validates state, category, target-type, policy, and evidence-availability filters; and calculates the cycle and per-category totals from the saved check rows. Rollups keep total and applicable denominators separate and report covered, awaiting-review, untested, excluded, and unavailable-evidence counts without treating `not_applicable` work as completed coverage. Serializers omit personal session ids and retain only the safe team and member context needed by callers. Evidence matching and lifecycle mutations remain separate from reads, so loading this model never runs a command, contacts a provider, or changes a check.
