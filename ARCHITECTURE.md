...
Bounded CycloneDX JSON components can supply exact package observations to that stored-OSV boundary. The parser accepts only CycloneDX 1.x documents with a timezone-aware observation time and a PURL whose embedded and explicit versions agree. It deduplicates and caps components without evicting earlier observations, gives accepted rows deterministic ids, and preserves the component name, type, BOM reference, source batch, format version, parser version, and observation time. The correlation wrapper returns unsaved candidates with the matched advisory provenance. It doesn't persist SBOM inventory, contact OSV, or create or update a finding; full preview/apply import remains a separate boundary.
