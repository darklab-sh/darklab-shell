...
These additive routes establish and manage pseudonymous principal credentials. Existing browser and product routes still use the legacy session owner adapter until their stored ownership moves to personal workspaces.
...
`.tooling/owner-query-inventory.jsonl` is the reviewed exception list for direct owner-related SQL outside that adapter boundary. Each entry records the source location, table and operation, key shape, schema and predicate team behavior, current result set, assigned subsystem, and principal-era replacement. `python scripts/development/owner_query_inventory.py --check` fails when source and inventory drift, while `--write` refreshes generated facts and preserves reviewed annotations for unchanged sites. The current boundary deliberately keeps the existing session identity as the personal owner; it does not mix session and personal-workspace ownership.
...
Projects, Assessments, and Atlas use the same boundary across records, metadata, findings, evidence, imports, packages, reports, cleanup, retention, and worker support. Their ledger in `.tooling/owner-query-reviews/projects-assessments-atlas.json` classifies the original 156 sites: 116 use the shared adapters, while 40 exact row correlations, flattened metadata keys, compound keys, conflict targets, ranking expressions, and attribution hashes remain named exceptions because the generic adapters do not represent those shapes. Every conversion is result-set equivalent; none changes the stored owner or the meaning of a personal session.

Files, workflows, secrets, and their supporting session state use the same boundary for preferences, variables, recent values, starred commands, legacy session migration, workflow definitions and executions, encrypted vault records, and workspace-file metadata. Their ledger in `.tooling/owner-query-reviews/files-workflows-secrets.json` classifies the original 40 sites: 39 use the shared adapters, while one workspace-metadata expression remains a named exception because team metadata can use either a direct `team_id` or the older flattened `session_id = team_id` owner key. That exception has an explicit principal-era replacement; every conversion is result-set equivalent and keeps existing session and vault-scope owners unchanged.

Schedules, watchers, notifications, OAST correlations, and ZAP connector jobs use that boundary for their browser, lifecycle, cleanup, retention, and worker paths. Their ledger in `.tooling/owner-query-reviews/automation-notifications.json` classifies the original 28 sites: 27 use the shared adapters, while the ZAP creation check remains a named exception because it correlates ownership across the assessment and HTTP-profile tables. Connector personal rows remain empty-team-only, automation and notification personal rows continue to accept `NULL` or empty team markers, and token migration and watcher diff reads keep their existing unfiltered shapes. Every conversion is result-set equivalent and keeps the current session or durable token as the personal owner.

The remaining application surfaces use the same boundary for AI assists and context, shell built-ins, CVE risk escalation, structured finding diffs, and team storage. Their ledger in `.tooling/owner-query-reviews/remaining-surfaces.json` classifies the original 52 sites: 34 use the shared adapters, including the explicit team-only shape used by team storage, while 18 remain named exceptions. Those exceptions are the 12 additive principal-foundation queries, two legacy membership-hash lookups, two exact risk-state compound keys, one attribution-only audit filter, and one cross-table watcher backfill. Every conversion is result-set equivalent, personal ownership still means the current session identity, and the checked inventory has no unclassified direct ownership predicate.
...
**Server-side compatibility validation:** the central resolver validates `tok_`-prefixed legacy headers against `session_tokens` on every request and returns a typed authentication failure for a revoked or unknown token. API v1's temporary token adapter consumes that same typed result and uses a secret-free compatibility key for per-token limits instead of parsing or retaining the bearer value again. Canonical anonymous UUIDs remain the only unpersisted personal identities accepted by the adapter. A principal credential is never converted into a session owner: legacy product routes return `409 principal_cutover_pending` until their ownership columns and queries use the authenticated personal-workspace id.

Session-owned routes reject missing or invalid identities instead of reading or writing under an empty namespace. This includes the main browser project, session, history, workspace, secrets, notification, schedule, watcher, and Atlas paths.
