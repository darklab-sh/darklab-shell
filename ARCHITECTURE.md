...
`blueprints/projects_assessments.py` exposes the browser list, create, detail, update, deletion-preview, and deletion routes through the existing Projects blueprint. `blueprints/api_v1_assessments.py` and `api_v1_assessment_checks.py` expose the matching token-authenticated cycle, manual-state, and evidence operations. `projects_assessment_oast.py` and `api_v1_assessment_oast.py` add matching nested reads and explicit reservation writes for the blind-XSS check. Both OAST surfaces rebuild the current provider-free plan before accepting its digest, keep list and reservation responses redacted, disclose a callback only from one exact ready correlation, and never resolve the provider token or call the service. All Assessment surfaces reuse the same safe serializers and personal/team scope. Cycle and check mutations require `MUTATE_PROJECTS`; reservations require `RUN_COMMANDS`. Writes use the normal Project limit, reject archived Projects, record assessment-specific audit events, and log only ids, profile versions, state transitions, and counts. Assessment deletion and its fail-closed audit record share one transaction.
