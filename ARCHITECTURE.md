...
One multi-stage Dockerfile owns both runtime modes and produces one self-contained runtime image. Independent ProjectDiscovery, other Go, checksum-pinned binary, native, Ruby, isolated Python scanner, and script-asset builders feed the final stage. The SecLists asset stage is the final stage's base, so the large wordlist tree stays in one shared layer instead of being copied into a second BuildKit snapshot. The result contains the bundled tools and complete `/app` tree without their compilers, development packages, Go toolchain and caches, package installers, apt indexes, or source trees. Schemathesis uses its own exact-pinned virtual environment under `/opt/schemathesis`, so its scanner dependencies cannot change the application runtime. The source-mounted development stack builds that same image, mounts `./app` at `/opt/darklab-source/app:ro`, and stages a permission-normalized snapshot into an ephemeral `/app` tmpfs before startup; the production stack pulls `docker.io/darklabsh/darklab-shell:<version>` and never mounts `/app`.

Protected final and `-rc.N` tags publish one OCI image index with native `linux/amd64` and `linux/arm64` children. An early job resolves the Python base tag once, records its immutable index plus both platform manifests, and gives that same snapshot to the two publishers. Each child carries its platform-base and shared base-index digests in labels and artifacts. AMD64 builds share one content-addressed registry cache behind a serialized writer. Every BuildKit cache writer and reader builds from the same tracked `HEAD` Git archive with fixed file modes, so checkout ownership, timestamps, ACLs, and SELinux labels never become cache-key inputs. A validated UTC date key is consumed only by the runtime APT installation, so the first build on a new day refreshes Debian packages while compiled Go, native-tool, and downloaded-asset stages keep their existing keys. Scheduled fanout starts with a cache warmer on any standard self-managed Docker runner, exports that day's key with the cache reference, and gives both to every Docker and rootless Podman build. Branch builds derive the same key from their pipeline timestamp; protected AMD64 and ARM64 publishers derive it from the unchanged release build timestamp. The ARM64 release build stays uncached because importing and exporting the bundled scanner and SecLists layers exceeds the hosted small runner's storage budget. Builder-local cache mounts aren't treated as cross-runner evidence. Volatile release labels remain the final Dockerfile instruction. The pipeline pins VirusTotal CLI, Nikto, SecLists, and other direct tool inputs, inventories build inputs and licenses, validates required notices and packaged dependencies, and treats every attempt, child anchor, and canonical app tag as immutable. Nmap 7.95 has its own NPSL 0.95 inventory record and hash-pinned license text, both of which remain part of public-image validation.
...
| `GET` | `/api/v1/risk/feeds` | Returns stored EPSS/KEV source status to any authenticated token without refreshing or changing a feed. |
| `GET` | `/api/v1/whoami` | Authenticates a scoped PAT and returns principal, workspace, and safe credential metadata without echoing the PAT. |
| `GET` | `/api/v1/principal` | Returns the current principal, personal workspace, authentication method, credential id, and PAT scopes. |
| `GET` | `/api/v1/credentials` | Returns safe metadata for the PAT used by the current request. |
| `POST` | `/api/v1/credentials/current/revoke` | Revokes the PAT used by the current request. |
| `GET` | `/api/v1/teams` | Returns teams joined by the current principal. |
...
| `POST` | `/api/v1/teams/<team_id>/leave` | Removes the current principal from a team. |
...
| `GET` | `/api/v1/history` | Returns the current principal's run history with the browser-aligned offset page envelope, search filters, and batched History/Run Details counts. |
...
| `GET` | `/api/v1/history/<run_id>` | Returns one current-principal run summary with artifact, finding, Atlas, label, and note counts. |
...
| `GET` | `/api/v1/history/<run_id>/artifacts` | Lists workspace artifacts for one current-principal run by stable artifact id. |
| `GET` | `/api/v1/history/<run_id>/artifacts/<artifact_id>` | Streams one current-principal run artifact while rejecting cross-run artifact ids. |
...
| `GET` | `/api/v1/projects` | Returns a read-only paged project list for the current principal. |
| `GET` | `/api/v1/projects/<project_id>` | Returns one read-only project record for the current principal. |
...
| `GET` | `/api/v1/projects/<project_id>/probes` | Lists the reviewed one-off probe actions and local profiles for a Project without starting a run. |
| `POST` | `/api/v1/projects/<project_id>/probes/targets/resolve` | Resolves one exact value to one confirmed owner-scoped Project target id. |
| `POST` | `/api/v1/projects/<project_id>/probes/plan` | Builds one bounded redacted probe plan for a confirmed Project entity id without creating History, audit, or Assessment state. |
| `POST` | `/api/v1/projects/<project_id>/probes/run` | Rebuilds and confirms one probe plan before starting its ordinary Project-linked run. |
...
| `POST` | `/api/v1/projects/<project_id>/assessment-batches/<batch_id>/retry-previews` | Builds a fresh current-state preview for the terminal source batch's failed or unfinished work without changing either batch. |
| `POST` | `/api/v1/projects/<project_id>/assessment-batches/<batch_id>/retry` | Confirms a retry preview, creates a new immutable batch linked to its terminal source, and fills the currently fair launch slots. |
...
| `GET` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations` | Returns the ten newest redacted private-OAST reservations for the exact owner and nested assessment check. |
| `POST` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations` | Rebuilds and confirms the provider-free blind-XSS plan before reserving one short-lived callback; team scope requires command-run permission. |
| `GET` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations/<correlation_id>` | Returns one exact reservation's status and exposes its callback only after the private worker has made it ready; team scope requires command-run permission. |
| `POST` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations/<correlation_id>/launch` | Rebuilds the redacted plan, requires the exact ready reservation, starts its Project-linked run, and binds the callback without returning it in the run command. |
| `POST` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-plan` | Reviews confirmed Project URLs and the current anonymous HTTP profile, then returns the exact non-secret ZAP plan and digest without contacting ZAP. |
| `GET` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs` | Returns the ten newest bounded ZAP jobs for the exact owner and nested assessment check so clients can resume status tracking after reload. |
| `POST` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs` | Rebuilds and confirms the reviewed ZAP plan before durably queueing its exact bytes for the external worker. |
| `GET` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs/<job_id>` | Returns bounded status, progress, error, and completed Files/Atlas handoff state for the exact owner and nested assessment check. |
| `DELETE` | `/api/v1/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs/<job_id>` | Requests cancellation for an active owner-scoped ZAP job when the team role can run commands. |
...
| `GET` | `/api/v1/schedules` | Returns a paged list of normal scheduled commands for the principal's active personal/team scope. |
...
| `GET` | `/api/v1/schedules/<schedule_id>` | Returns one normal scheduled command in the principal's active personal/team scope. |
...
| `DELETE` | `/api/v1/schedules/<schedule_id>` | Deletes one normal scheduled command in the principal's active personal/team scope; team scope requires automation-management permission. |
...
| `GET` | `/api/v1/watchers` | Returns a paged list of change-detection watchers for the principal's active personal/team scope. |
...
| `GET` | `/api/v1/watchers/<watcher_id>` | Returns one change-detection watcher in the principal's active personal/team scope. |
...
| `GET` | `/api/v1/notification-channels` | Returns masked outbound notification channel metadata for the current principal or selected team. |
...
| `DELETE` | `/api/v1/notification-channels/<channel_id>` | Deletes one outbound notification channel from the current principal or selected team. |
...
| `GET` | `/api/v1/runs` | Returns active current-principal runs for CLI and script visibility. |
...
| `GET` | `/api/v1/runs/<run_id>` | Returns active broker status or completed history status for a current-principal run. |
| `GET` | `/api/v1/runs/<run_id>/output` | Returns stored run output as plain text or JSON, with optional line ranges. |
| `GET` | `/api/v1/runs/<run_id>/service-evidence` | Pages through typed informational service facts saved with a current-scope run. |
...
| `POST` | `/api/v1/runs/<run_id>/projects/<project_id>` | Links a completed external run to an active project for the current principal. |
| `DELETE` | `/api/v1/runs/<run_id>/projects/<project_id>` | Removes a completed external run link from an active project for the current principal. |
| `GET` | `/api/v1/runs/<run_id>/stream` | Streams a current-principal run as SSE by default or NDJSON when `format=ndjson`. |
| `POST` | `/api/v1/runs/<run_id>/cancel` | Cancels any active run for the current principal, including browser-started runs. |
...
| `GET` | `/runs/<run_id>/service-evidence` | Pages through typed informational Nmap service facts for Run Details in the active personal or team scope without returning free-form NSE output. |
...
| `GET` | `/atlas/views` | Lists saved Atlas views for the current personal workspace. |
| `POST` | `/atlas/views` | Saves a named Atlas view with its tab, search, filter, review-state, project-scope, and source-run state. |
| `PUT` | `/atlas/views/<view_id>` | Updates a saved Atlas view for the current personal workspace. |
| `DELETE` | `/atlas/views/<view_id>` | Deletes a saved Atlas view for the current personal workspace. |
...
| `GET` | `/atlas/imports/drafts/<draft_id>` | Reopens an owner-scoped, reviewable import draft with bounded counts, samples, warnings, and current apply options. |
...
`GET /atlas/imports/drafts/<draft_id>` reopens the same bounded review shape for
a prepared draft, including drafts created by an external scanner worker. It
requires the current personal or team owner, rejects applied or expired drafts,
...
These routes establish and manage pseudonymous principal credentials. Authenticated browser and product requests resolve ownership through the principal's personal workspace. Anonymous requests use a separately validated UUID context and cannot represent failed authentication.
...
| `GET` | `/session/teams` | Lists teams available to the current principal. |
| `POST` | `/session/teams` | Creates a team for the current principal and returns the one-time recovery code. |
| `GET` | `/session/teams/<team_id>` | Returns team detail for a team the current principal belongs to. |
...
| `POST` | `/session/teams/join` | Redeems a team invite code for the current principal. |
...
| `GET` | `/session/notification-events` | Lists recent outbound notification delivery audit rows for the current personal workspace or team. |
| `GET` | `/session/notification-channels` | Lists masked outbound notification channel metadata for the current personal workspace or team. |
...
| `DELETE` | `/session/notification-channels/<channel_id>` | Removes one outbound notification channel from the current personal workspace or team. |
...
| Method | Endpoint | Description |
| -------- | ---------- | ------------- |
| `POST` | `/workflow-executions` | Compiles a scoped workflow, validates typed inputs, snapshots the active workspace/project context, and starts a durable scalar or collection workflow. |
...
Workflow execution is bounded by `workflow_active_execution_limit` per personal/team owner and `workflow_execution_max_runtime_seconds` per execution. Every coordinator row carries an explicit `workflow` or `assessment_batch` execution kind. Workflow routes, quotas, recovery, metrics, audit labels, and History provenance filter to `workflow`, so a batch can't appear or recover as a saved playbook even though both kinds share lifecycle storage. The assessment-batch storage contract adds one parent row for cycle identity, preview identity, retry lineage, item count, and concurrency limits. Server-owned preview rows keep bounded selection and summary metadata, exact candidate plans, and one-to-many frozen-check mappings; confirmed execution rows copy only selected public plans and mappings, then bind each immutable item identity to one value-free workflow child ordinal. It partitions at most 512 items into ordered workflow steps of no more than 32 children, keeps target concurrency fixed at one, and stores a separate immutable event stream under a monotonically increasing parent sequence. Operators can lower the selected-item ceiling, active-parent quota, batch/owner/instance concurrency, maximum runtime, and terminal-state retention through the validated `assessment_batches` config section without raising the fixed product caps. Public progress is derived from child attempts and fixed error classifications rather than copied into a second mutable item state. Postgres takes an owner-scoped transaction advisory lock around the active-count claim and insert. Fan-out child claims lock the private child, parent step, and active execution together, recheck the parent policy's parallel-child ceiling, then move the same ordinal from pending to running in the parent checkpoint before committing. Each claimed child can bind one run exactly once; completion records a bounded result and advances that checkpoint once, while recovery can return only an unbound launching child to pending without changing its identity. The workflow-only fan-out adapter renders its shared command template and private collection substitution into a complete child launch specification before the claim; the downstream run-binding path accepts that specification without re-rendering it, which keeps command display, protected values, and heterogeneous batch children behind one launch boundary. A retryable failure closes its current attempt before creating the next bounded private row and returning that ordinal to pending. Terminal failures count by ordinal, and reaching the parent policy's limit moves pending and unbound launching rows into a durable skipped state without changing completed children or bound active runs. The final terminal child closes its parent step in the same transaction: failure-limit outcomes take the saved failure transition, while continue-mode partial results keep their failed-child summary and take the success transition. Cancellation locks the same execution and child state, preserves terminal child evidence, marks every unfinished attempt canceled, records those ordinals as skipped in the private checkpoint, and passes only bound child run ids to the existing process signal path. Before each step, the engine rechecks durable personal tokens plus the current team, initiating token, membership, and run capability. Create, list, detail, and cancel responses use one fixed public serializer that omits the definition snapshot, input and variable maps, workspace context, owner scope, actor context, and browser ownership hints for every role. Web startup follows an immutable `(created, id)` cursor through bounded pages of active workflow rows after stale run metadata cleanup: unbound launch claims return to pending, completed linked runs advance from saved normalized output, live broker runs remain attached, and missing runs fail with bounded recovery metadata. A workflow-hook exception marks only the execution failed; the completed run remains saved.

Assessment-batch previews stream one active cycle's frozen checks through explicit row and memory limits, classify excluded work, and compile each eligible check through one shared probe-readiness snapshot. Safe plans are selected automatically, supported standard plans stay separately visible, and explicit target/category filters remain the assessor's scope decision. Exact execution matches deduplicate across checks without merging their coverage mappings. Browser and API adapters cap preview request streams at 64 KiB and lifecycle mutation streams at 16 KiB before JSON decoding, including chunked requests without `Content-Length`. The stored preview validates every credential-free public plan and its digest, hashes the complete stable-ordered snapshot, expires after 15 minutes, and returns complete items in pages capped at 100 entries and 1 MiB. Creating or reading a preview doesn't create a coordinator or run row. Managed Nuclei health checks run through the same configured scanner prefix as template updates and assessment commands. After an update, the server rebuilds the whole preview and returns success only when the new Nuclei preflight is ready and launchable.
...
Retry previews begin with a terminal owner-scoped batch from the same Project and cycle. They read only the latest failed, skipped, or canceled child outcomes, recover those items' frozen-check mappings, and pass the eligible checks back through the current bounded compiler. Succeeded source items stay excluded, while new evidence, manual decisions, changed targets, unavailable actions, and policy or profile changes can reduce or reshape the result. The complete preview digest includes its source batch id. Confirmed start verifies that lineage again, rejects an empty current retry, and atomically creates a new parent with a source link and one `retry_created` event on the immutable source. Repeating the same preview and digest returns the same retry parent.

The external CLI remains an adapter over those same routes. `assessment batch plan` and `start` submit only the bounded selection and then page the server-owned preview; confirmed start sends only its preview id, complete digest, confirmation, and separate standard-policy acknowledgement. `assessment batch retry` first resolves the source batch, prints and pages a fresh linked preview, and stays read-only unless the caller supplies `--confirm`; selected standard work still needs `--confirm-standard`. List, show, and follow consume the durable read model without reconstructing coordinator state, and follow advances only from event sequences it has printed. Ctrl+C returns `130` and reports the exact resume cursor without changing the batch. A terminal follow returns `0` only when every item succeeded, `3` for a completed batch with partial outcomes, `4` for canceled work, and `1` for a failed parent or API/contract error. Cancel first reads the owner-scoped parent and does not mutate it unless the caller supplies `--confirm`.

The assessment claim adapter takes the next pending ordinal through the shared fan-out claim transition without using workflow template expansion. Before that transition, one short serialized transaction enforces the saved batch limit, the most restrictive active owner and instance limits, and one active item per exact target. A busy target doesn't block unrelated work later in the stable pending order, and the deferred item never changes identity. Each launch-fill cycle that reaches one of those ceilings emits one DEBUG decision with the batch id, fixed reason, and number already launched in that cycle, without identifying the blocked target. The same transaction activates the current chunk, advances the authoritative checkpoint, and appends the sanitized parent, chunk, and item events. A claim alone never creates a run.

After a claim, the assessment adapter rebuilds that item's probe plan from its persisted action, target, and public profile identity. The current Project, assessment, target confirmation, feature readiness, policy, plan digest, and public display command must still match before launch. The resulting child specification carries the execution command, exact public command, trusted private arguments, cleanup hook, and notification context through the same downstream launcher used by workflow fan-out, with the parent Project id supplied explicitly. Batch rows never receive private values or paths. A stale or unavailable item settles without a run and may let the next independent item proceed; a terminal settlement stops the fill loop. Each completed run first persists through the ordinary History, structured-evidence, and Project-link pipeline, then a kind-aware finalization hook advances only its batch child and refills newly available slots. Browser and API History, Project runs and comparisons, Assessment evidence, reports, and evidence packages join that run back to its owner-scoped batch child at read time. The public ancestry is limited to batch, Project, assessment, source-batch, batch status and creation time plus item index, step, attempt, status, run id, exit code, and mapped-check count. It never copies a command, target, profile, plan, output, private launch value, or workflow definition into the response. Evidence reads accept the ancestry only when its assessment id matches the exact frozen cycle, so every mapped check still earns coverage through its own ordinary evidence rule. API `RunSummary` exposes the same nullable object as `assessment_batch` with the convenience fields `assessment_batch_id` and `assessment_batch_item_index`; non-batch and cross-owner runs return an empty id, null item index, and null object. Child completion notifications are suppressed for this context, while ordinary runs keep their existing notification behavior. Once completion, cancellation, recovery, or failure isolation commits a terminal parent, the app emits one structured operator milestone with the terminal state, duration, fixed reason, and bounded outcome totals, then idempotently queues one bounded batch summary for channels already subscribed to `run_complete`. Neither path includes commands or targets. The notification payload identifies the batch, Project, cycle, terminal state, retry source, outcome counts, and batch link. Both actions run after coordinator commit, so observability or delivery storage can't roll back truthful settlement; retry batches have distinct ids and therefore distinct summaries.
...
Web startup recovers assessment batches through a kind-filtered immutable `(created, id)` cursor after workflow recovery. Each batch snapshot must prove that its parent count, chunks, persisted items, latest child attempts, retry lineage, and checkpoints agree before scheduling resumes. An unbound launching child returns to its original pending ordinal with a sanitized recovery event; a completed run replays ordinary batch finalization; a live broker run stays attached; and a vanished run enters the existing bounded retry path. Recovery rechecks the current Project, active assessment, configured runtime, token, membership, and run capability before any new claim. Scope loss, expiry, permission loss, or malformed state stops new work and signals every bound run, but the parent remains `canceling` until those processes actually settle and then finishes `failed` with the saved recovery reason. After recovery, retention cleanup deletes only old terminal retry-chain leaves, preserves any source that still has a retained retry, and leaves ordinary child runs and evidence untouched. Dedicated metrics expose active batches, launch queue depth, retained parent and item outcomes, claim-to-launch latency, bounded revalidation rejections, concurrency deferrals, operator actions, and recovery outcomes without changing workflow metrics or using stored ids, targets, profiles, or commands as labels.
...
| `POST` | `/projects/<project_id>/assessment-batches/<batch_id>/retry-previews` | Builds a fresh current-state preview for the terminal source batch's failed or unfinished work without changing either batch. |
| `POST` | `/projects/<project_id>/assessment-batches/<batch_id>/retry` | Confirms a retry preview, creates a new immutable batch linked to its terminal source, and fills the currently fair launch slots. |
...
| `GET` | `/assessment-batches/<batch_id>` | Returns one durable batch with current aggregate progress, bounded batch-level diagnostics, and immutable launch context. |
...
| `POST` | `/projects/<project_id>/assessments/<assessment_id>/retest-groups/<group_id>` | Revalidates and starts one safe, credential-free command shared by two to ten exactly matched finding retests. |
| `GET` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations` | Returns the ten newest redacted private-OAST reservations for this exact assessment check. |
| `POST` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations` | Confirms the current provider-free blind-XSS plan and reserves one short-lived callback for an actor who can run commands. |
| `GET` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations/<correlation_id>` | Returns one exact reservation's status and reveals its ready callback only to an actor who can run commands. |
| `POST` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/oast-correlations/<correlation_id>/launch` | Rebuilds the redacted plan, requires the exact ready reservation, starts its Project-linked run, and binds the callback without returning it in the run command. |
| `POST` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-plan` | Reviews confirmed Project URLs and the current anonymous HTTP profile for the browser's exact non-secret ZAP plan confirmation. |
| `GET` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs` | Returns the ten newest owner-scoped ZAP jobs for this check so the browser can resume status tracking after reload. |
| `POST` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs` | Rebuilds and confirms the browser-reviewed ZAP plan before queueing its exact bytes. |
| `GET` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs/<job_id>` | Returns bounded owner-scoped ZAP job status and completed Files/Atlas handoff state. |
| `DELETE` | `/projects/<project_id>/assessments/<assessment_id>/checks/<check_id>/zap-jobs/<job_id>` | Requests cancellation for an active owner-scoped ZAP job when the team role can run commands. |
...
| `GET` | `/projects/<project_id>/probes` | Lists the reviewed one-off probe actions and local profiles available to the current Project owner without starting a run. |
| `POST` | `/projects/<project_id>/probes/targets/resolve` | Resolves one exact confirmed Project target from a narrow JSON body for the terminal's read-only planner. |
| `GET` | `/projects/<project_id>/probes/plan` | Builds one bounded, redacted probe preview from a confirmed Project entity id without adding History or assessment evidence. |
| `POST` | `/projects/<project_id>/probes/run` | Rebuilds and confirms one probe plan, starts the ordinary Project-linked run, and binds a terminal launch to its validated origin tab. |
...
The options modal is part of that same browser-owned layer. It does not change backend config; it owns user-specific UX preferences (timestamp/line-number quick toggles, welcome-intro behavior, snapshot redaction defaults, project run/entity capture toggles, run-notification state, HUD clock timezone mode), workspace access, encrypted secret management, team membership management, and outbound notification channels for the active personal workspace. The modal is split into a **Preferences** tab for display, identity, run, and compare controls, an **Access** tab for anonymous attachment and credential lifecycle work, a **Secrets** tab for Provider Status, add/refresh actions, and the dynamic secret list, a **Teams** tab for create/join/member/invite/recovery flows plus personal/team scope switching, and a **Notifications** tab for workspace-owned delivery destinations; the selected tab is saved with the workspace preference snapshot. The modal feeds preference changes back into the browser runtime during boot and identity changes. The terminal-native `config` command calls the same preference application path as the modal, so terminal and modal changes stay equivalent. Browser-owned terminal commands (`theme`, `config`, `workflow`, `secret set`, `credential`, and Files helpers) keep DOM, storage, clipboard, download, and confirmation work in the browser. They return normalized output and completion state to the same coordinator that settles brokered `/runs` after their live SSE output finishes. That coordinator owns final transcript lines, tab and HUD status, eligible recents, notifications, refresh effects, and exactly-once browser persistence through `/run/client`; server-owned runs remain server-persisted. A successful `/run/client` response includes the saved run summary, and browser-owned recents use its command while an open History panel reloads from the same backend record. Commands are masked before either path reaches the transcript or recents, while prompt history deliberately remains a submit-time concern after client preflight and recents remain a completion-time concern. `secret set NAME` opens the same replace-only Options value prompt and never accepts the value on the command line. `workflow run` uses that local command path for catalog lookup and input prompting. Legacy workflows continue queuing their rendered commands through the browser and hand tab status to the first queued command. Explicit v2 and v3 playbooks instead submit `POST /workflow-executions`, then the server owns step or bounded-child launch and advancement and returns a durable id plus a `workflow status` hint without claiming that the initial work remains current. Panel-started durable playbooks keep their progress in the Workflows surface and do not add partial state to the active terminal. Preferences persist server-side under the personal-workspace owner, while browser cookies/local storage remain the local cache and anonymous-session fallback layer. On mobile, that same shared Options surface hides the desktop-only `HUD Clock` and `Run Notifications` rows even though the underlying preference set remains shared with desktop.
...
`project_assessment.js` owns per-Project Assessment cycle, category, state, policy, evidence-availability, and risk filters, independent paging, disclosure, and scroll state plus the browser request and forward-only lifecycle flow. Cycle creation and lifecycle mutations await their authoritative forced reload before the controller clears the action state and performs its final render. Those reloads supersede older list and detail requests, generation checks keep late responses from replacing the current cycle state, and the core cycle view doesn't wait for the optional HTTP-profile request before it renders. Lifecycle preview and mutation responses must succeed before the controller reports completion or changes its local selection. Complete and archive changes use the shared warning confirmation. Archived-cycle deletion loads the server preview before showing the destructive confirmation, and its copy distinguishes assessment-owned rows from preserved source records. Each target-specific check can launch the shared manual-finding editor with the confirmed target and an `assessment_check` evidence reference already selected; the normal finding-triage permission still controls the write. `project_assessment_evidence.js` provides the shared active-cycle evidence editor: it offers only source types accepted by the frozen check, links one saved Project source by opaque id, lists the newest bounded manual links returned with the read model, and removes only a selected manual link after explaining that its source is preserved. Link and unlink responses trigger an authoritative detail reload so coverage stays current. The read model returns cycle-wide target rollups separately from the bounded check page, so a target keeps the same truthful denominator and covered/review counts on every page. `project_assessment_batch.js` owns the browser's in-page target/category selection, server-owned preview reference, durable batch list, item pages, event cursor, visibility-aware polling, and reload recovery. General Project-list refreshes keep an unstarted planner draft and its rebuilt preview when the workspace closes and reopens in the same page, while explicit Assessment invalidation still discards it. The browser bootstrap exposes the server's effective item, batch, owner, and instance ceilings; the planner defaults to those values and filters every preset to the matching ceiling. It never rebuilds a command in the browser: `project_assessment_batch_renderer.js` shows the server's exact public plans, keeps safe and separately confirmed standard selection visible to viewers, disables mutations without `RUN_COMMANDS`, opens ordinary child runs through History, and describes cancellation as a request that preserves completed work. A preview ends with one state-aware sticky decision bar whose primary action follows the server-owned readiness state; the Run action remains visible with its blocking reason. Exact commands and exclusion reasons use the shared disclosure helper and count summaries, while disclosure state survives planner-only rerenders. On mobile, the cycle action footer gives up its sticky position whenever the preview decision bar is present, leaving one bottom action surface. Preview creation, Nuclei template refresh, and planner selection changes replace only the planner panel; progress polling does the same for the batch monitor and its error panel. Both paths leave the surrounding desktop or mobile Assessment tree mounted and restore its scroll offsets and keyed focus after the update. This keeps each loading label, limit control, and large monitor at the reader's current place instead of moving them back through the workspace. `project-assessment-batch.css` applies the same responsive panel, badge, button, form, theme-token, focus, and reduced-motion contracts on desktop and mobile. `project_assessment_renderer.js` owns the shared desktop/mobile coverage, category, finding-change, target, check, bounded evidence preview and recent-evidence, typed Nmap service-evidence, empty, error, and lifecycle rendering. The read-only evidence blocks identify opaque saved sources and availability without copying source content; available runs can open the existing Run Details surface. The service-evidence block is shared with Run Details, so both surfaces show the same bounded informational fields and omit free-form NSE output. `project_assessment_risk_renderer.js` owns the shared fix-first filters, remediation rows, observation disclosures, and paging. It uses the same public-risk labels and ranking response as other finding surfaces while keeping the coverage denominator separate. Finding-change and fix-first rows use the shared finding editor for their individual observations; desktop and mobile render the same remediation-level counts without flattening the observation evidence. Desktop uses shared button primitives while mobile delegates lifecycle choices to the shared action sheet. The entry controller, base Assessment stylesheet, and batch-plan stylesheet load together on first use, so adding the tab doesn't increase the initial Projects payload.

`project_http_profiles.js` owns the Assessment HTTP-profile list, permission-aware actions, reference-health presentation, and mutations. Its request generations let forced reloads supersede older work and keep late responses from replacing current profile state; completion renders the card independently from the core cycle list. `project_http_profile_editor.js` builds the shared desktop/mobile editor. It reads Secret metadata through the same personal/team request scope as the Project, offers stored names through the app-native select, and never reads Secret values; **Manage Secrets** opens the existing Options → Secrets surface. Existing unavailable references remain visible while editing so they can be repaired instead of silently dropped. Team members without `MANAGE_SECRETS` receive only the route's redacted summary and cannot open mutations. `project_assessment_actions.js` offers enabled, available profiles only for supported Curl, HTTPx, Katana, Dalfox, and detection-only SQLmap checks, then sends the chosen profile id through both the fresh preview and digest-confirmed launch. Nuclei doesn't show the profile picker because template-generated requests can't enforce the saved scheme, port, root, include-path, and exclude-path boundary. The confirmation names the role and credential categories without rendering protected values.
...
The enhancement targets `select.form-select` and History drawer filter selects. `ui_helpers.js` also watches the DOM and automatically enhances matching selects inserted after startup, so dynamically rendered modal, sheet, and detail controls do not fall back to browser-native dropdown chrome. Selecting an item in the app-native menu updates the real select and dispatches a normal bubbling `change` event, so existing Options and History logic does not need a parallel API. Long app-native menus are height-capped and scroll internally; their option rows don't flex-shrink to fit the cap. Selects inside dialogs, modals, and sheets automatically portal their menus to the page layer so provider, command, and Options lists stay reachable on desktop and mobile. When code changes a select value programmatically, it must call `syncAppSelect(select)` or `syncAppSelects()` after writing `.value` / `.disabled`; `syncOptionsControls()` and `_syncHistoryFilterControls()` are the canonical examples.
...
Recent target autocomplete follows the active personal or team workspace. `autocomplete.js` keeps a page-local cache for immediate suggestions, while `GET`/`POST /session/recent-values` persist normalized domains, IPs, URLs, and port sets under that workspace owner. Each value kind is capped at 10 entries. Capture and suggestions still require explicit value-type metadata in the command registry; placeholder text and descriptions are display-only and do not make a slot record recent targets. URLs are saved without query strings or fragments so pasted tokens do not become suggestions.
...
App-level modal confirmations — kill, history-delete, history-clear, share-redaction, Access credential actions, Options Secrets replace/delete prompts, delete-all — go through a single imperative primitive, `showConfirm()` in `app/static/js/ui/ui_confirm.js`. Surfaces do not hand-roll confirm markup, do not wire their own Escape handler, and do not manage their own backdrop. Transcript-native terminal confirmations are the intentional separate category: `probe run` keeps the reviewed context beside the prompt and uses the per-tab pending-confirm coordinator in `runner.js` instead of a modal surface. One confirmation may be pending in each tab; input from another tab cannot settle it, and tab reset, close, reload, identity replacement, logout, or owner-scope change clears the matching state. A second probe that reaches confirmation in the same tab stops with an operator-facing instruction and a warning-level diagnostic instead of replacing the first prompt or reporting an application error. After an answer is accepted, that origin tab remains in a private settling state until its asynchronous action and completion hook finish, so another command can't overtake the confirmed action. Other tabs remain independent, and a late completion updates global run controls only when its origin tab is still active. Changing the active Project cancels Project-bound probe confirmations without disturbing unrelated prompts in another tab; reloading the same active Project refreshes its display state without canceling the prompt or refetching probe autocomplete data.
...
- **Long forms stay inside the card.** A form can widen the shared compact card through a content-specific `:has()` selector, but the card remains bounded to the viewport and the form itself must use the available width. The HTTP profile editor keeps its own themed vertical scroll area, uses two columns only when they fit, and collapses to one column on narrow screens so its fields and action row remain reachable.
...
Owner-scoped SQL has a second, narrower boundary in `services.teams.ownership_queries`. Its adapters cover personal-only tables, team-only tables, team-capable tables, composite owner keys, and actor attribution. A team-capable call must state whether a personal row uses `NULL`, an empty string, either representation, or no team filter; the call can't quietly add a team condition while presenting itself as a behavior-preserving refactor. The compatibility helpers in `services.teams.scope` delegate to these adapters and keep their existing SQL and parameters.

`.tooling/owner-query-inventory.jsonl` is the reviewed exception list for direct owner-related SQL outside that adapter boundary. Each entry records the source location, table and operation, key shape, schema and predicate team behavior, current result set, assigned subsystem, and replacement. `python scripts/development/owner_query_inventory.py --check` fails when source and inventory drift, while `--write` refreshes generated facts and preserves reviewed annotations for unchanged sites. The boundary uses `personal_workspace_id` for product ownership and keeps principal and credential ids as separate attribution rather than owner keys.

History and run storage use that boundary for behavior-preserving owner checks. Their review ledger in `.tooling/owner-query-reviews/history-runs.json` classifies the original 28 direct sites: 22 use the shared adapters, while six column-to-column correlations remain named exceptions because they compare already-scoped rows rather than a row with a bound owner value. Those exceptions stay in the checked inventory for the principal cutover instead of being hidden behind an adapter with different semantics.

Projects, Assessments, and Atlas use the same boundary across records, metadata, findings, evidence, imports, packages, reports, cleanup, retention, and worker support. Their ledger in `.tooling/owner-query-reviews/projects-assessments-atlas.json` classifies the original 156 sites: 116 use the shared adapters, while exact row correlations, flattened metadata keys, compound keys, conflict targets, ranking expressions, and attribution hashes remain named exceptions because the generic adapters do not represent those shapes. The ownership cutover changes the personal key from session identity to personal-workspace id without changing team result sets.

Files, workflows, secrets, and their supporting state use the same boundary for preferences, variables, recent values, starred commands, workflow definitions and executions, encrypted vault records, and workspace-file metadata. Their ledger in `.tooling/owner-query-reviews/files-workflows-secrets.json` classifies the original 40 sites: 39 use the shared adapters, while one workspace-metadata expression remains a named exception because team metadata can use either a direct `team_id` or the older flattened personal-owner key. Personal records use the workspace id, while Team vault records continue to use the team id as their explicit owner scope.

Schedules, watchers, notifications, OAST correlations, and ZAP connector jobs use that boundary for their browser, lifecycle, cleanup, retention, and worker paths. Their ledger in `.tooling/owner-query-reviews/automation-notifications.json` classifies the original 28 sites: 27 use the shared adapters, while the ZAP creation check remains a named exception because it correlates ownership across the assessment and HTTP-profile tables. Connector personal rows remain empty-team-only, automation and notification personal rows continue to accept `NULL` or empty team markers, and watcher diff reads keep their existing unfiltered shapes. Personal ownership is the captured personal-workspace id rather than a reusable credential.

The remaining application surfaces use the same boundary for AI assists and context, shell built-ins, CVE risk escalation, structured finding diffs, and team storage. Their ledger in `.tooling/owner-query-reviews/remaining-surfaces.json` classifies the original 52 sites, including the explicit team-only shape used by team storage. Principal-foundation queries, exact risk-state compound keys, attribution-only audit filters, and cross-table backfills remain named exceptions where generic owner predicates do not represent the operation. The checked inventory has no unclassified direct ownership predicate.
...
Legacy workflows remain browser-owned linear queues. Explicit v2 and v3 workflows are server-owned durable executions. `services.workflows.compiler` validates the complete graph, rejects capture use unless every incoming path has produced the value, and validates typed values. Runtime rendering fails closed if a saved snapshot still lacks a referenced variable. `services.workflows.storage` saves an immutable definition snapshot and one pending row per stable step id.

Version 3 definitions add bounded collection captures and `for_each` policy to that compiled snapshot while keeping v1 and v2 scalar behavior unchanged. The shared editor exposes the same contract on desktop and mobile: capture shape and item bounds stay with the producer, while the consumer selects only a prior collection and sets the validated failure, retry, parallelism, and failure-count policy. It emits version 3 only when collection behavior is present or already belongs to the saved definition. The same personal/team execution-create route accepts saved v3 definitions, snapshots the initiating scope and context, and returns the fixed public projection used for scalar executions. Scalar and collection output selectors use separate bounded accumulators, then save their results together under one execution-level byte ceiling; list values remain private and recovery rebuilds the same state from durable run output. Private `workflow_execution_children` rows reserve stable ordinal and attempt identity, run linkage, status, exit code, bounded error code, and timestamps without storing a collection value, rendered command, or free-form error detail. The server rebuilds each child plan from the private collection, claims no more than the saved parallel limit, and sends every rendered command through the normal policy, scope, ownership, workspace, and brokered-run boundary. A completed child refills the next available slot or its bounded retry, while an optional empty collection completes without creating child rows. Startup recovery initializes a claimed parent that stopped before its child rows existed, returns unbound launching children to pending with the same identity, reconciles completed linked runs, leaves active linked runs alone, and turns a vanished linked run into a bounded failure before refilling available slots. Invalid child or parent recovery state fails the execution closed. Initial rows and the parent checkpoint are created idempotently from the validated `for_each` step, and public execution payloads expose only the checkpoint-derived count summary. The Workflows surface renders that summary as finished, pending, active, succeeded, failed, and skipped counts with the bounded failure-code sample; it never receives child values or rendered child commands. The shared run-finalization hook resolves a child link before the scalar step path, advances one child checkpoint exactly once, and emits or launches only the final parent transition.
...
Bounded Subdomain Assessment is the maintained built-in v3 path. Subfinder's normalized domain entities feed one private 16-item collection; DNSx and HTTPx fan-out at four children, while shallow Katana and safe-profile Nuclei fan-out at two. It uses no intermediate Files entry, keeps tool-specific request, time, rate, retry, and output controls in the saved definition, and relies on the same count-only child projection as user-authored collection workflows. Built-in, deployment-file, personal, and team v3 definitions all cross the same compiler and durable start boundary.

Live Web Review is the maintained scalar path for one approved URL. Its HTTPx step uses the release image's Debian Chromium package with the system-browser flag and the same container-required `--no-sandbox` browser option as headless Nuclei, so it never downloads Rod browser or launcher executables into the non-executable temporary filesystem. The step writes only the screenshot directory needed by the existing verified-artifact finalizer. HTTPx reports screenshot paths relative to its fixed `screenshot/` child directory, so normalization restores that segment before Files verifies and registers the image. Its Dalfox step streams discovery-only JSONL into the existing bounded parameter-observation parser. The workflow doesn't copy screenshot paths or parameter values between steps; durable transitions link the two normal runs while Files owns the image and structured run output owns the inventory.

Port Service Review is the maintained scalar path from one known TCP port to service-aware enumeration. Both Nmap runs use connect mode against the exact typed host and port. The first records the fingerprint; the second uses the reviewed `default` selector so Nmap chooses only applicable default scripts, with fixed script, retry, and host timeouts. Broader protocol profiles remain separate Assessment recommendations derived from explicit service evidence. API schema testing follows the same separation for a stronger reason: its maintained Assessment action privately revalidates one Project-linked OpenAPI artifact, inventories only GET/HEAD operations, and supplies bounded Schemathesis material that generic workflows are intentionally unable to construct.

The Workflows surface keeps catalog identity/loading in `features/workflows/workflow_catalog.js`, typed values, source pickers, remembered state, and previews in `features/workflows/workflow_parameters.js`, definition authoring in `features/workflows/workflow_editor.js`, and durable requests plus Executions-tab rendering in `features/workflows/workflow_executions.js`. The remaining controller coordinates those modules with the terminal and modal bridges. The lazy terminal bridge preserves the runner's command-execution object while loading that controller, so the first workflow command after page or container startup follows the same transcript and completion lifecycle as later commands. Every browser entry point opens one workspace: the **Workflows** tab uses a searchable, source-filtered catalog with `panel-row` navigation and an unframed selected detail, while the **Executions** tab owns active-scope execution history and polling. Desktop keeps the catalog and detail side by side; mobile drills from the same catalog into the same detail and back without a separate workflow implementation. Selecting a rail workflow changes workspace selection only, so execution history never inherits stale definition filters. Deleting a definition refreshes the shared catalog/rail while leaving the workspace and historical executions available. The editor owns typed parameter rows, reorderable steps with stable IDs, success/failure destinations, repeatable exact-exit-code routes, bounded capture selectors, and field-level server errors. Route destinations follow step renames, remain stable across reordering, and show a missing-step choice after destination deletion so the user can repair or remove the route. Client validation rejects blank, non-integer, and duplicate exact codes before save; the compiler canonicalizes integer keys and applies the same field-level validation to every source. Sensitive fields use masked controls; terminal answers use the shared secret composer mode without transcript echo, and inline sensitive flags are rejected after redaction. Browser command previews render known inputs only; references to earlier captures remain visible and non-runnable until the server-owned playbook produces them. The shared desktop modal and mobile sheet read the same active-scope execution list, poll only while the Executions tab is visible, refresh on reopen and scope changes, and stop polling when that view closes without canceling server-owned work. Active linked runs attach through the runner's Status Monitor path; finished linked runs open through the shared Run Details boundary.
...
2. The route layer checks feature flags, completed-run ownership in the active personal or team scope, useful context, Redis-backed per-workspace/global write limits, and a short enqueue lock.
3. `services.ai.storage` reuses a completed cache hit when the owner scope, context hash, variant, prompt version, and model match; otherwise it writes a queued `ai_run_assists` row with the personal-workspace id plus the team id when team scope is active.
...
Generic command-output entity extraction keeps URL, scanner-specific, import, and project-target paths on their stronger parsers, and command-scoped exclusions keep documented example targets in tool boilerplate from becoming saved entities. Bare free-text hostname candidates pass through an offline Public Suffix List gate before becoming domain metadata. The gate uses the `tldextract` package snapshot pinned in `app/requirements.txt`, includes private PSL suffixes so shared-hosting names such as `foo.github.io` stay distinct, and never fetches a live list during normal extraction. Operators can add deployment-local suffixes such as `.local` or `.corp` with `output_entity_extra_domain_suffixes`; that setting is threaded per extraction call, matching the existing private-IP capture policy.

`dig` and `nslookup` bypass that generic pass. A per-command DNS state machine reads every queried subject, record type, selected resolver, output section, and specialized mode before attaching entity metadata. Positive A, AAAA, CNAME, and PTR answers stay tied to the query chain; MX, NS, and SRV targets must share its registrable domain through the same offline suffix data. Wildcard owners normalize to their parent domain, CAA keeps its owner only, and non-canonical owners fail closed before their RDATA is considered. Address lines also require an owner already accepted into the query chain, preventing rejected provider infrastructure from re-entering on a later line. SOA contacts, TXT contents, DNSSEC records, reverse-zone owners, authority and additional sections, resolver echoes, trace delegations, nameserver-search responses, and negative subjects remain transcript evidence without becoming Atlas entities. Signal classification remains independent, so suppressing entity metadata never removes the original line or its finding, warning, summary, search, comparison, or export behavior.

`whois` also bypasses generic entity extraction. The command parser selects its first positional lookup target after recognized option values, and the output state emits that target once only when it is a canonical public IP address or a domain accepted by the same offline suffix data. Allocation boundaries, CIDR values, registry and registrar infrastructure, nameservers, contact handles, RDAP and referral URLs, and policy links remain registration evidence in the transcript instead of becoming Atlas entities. Finding and summary classification stays independent, so the complete response remains available to History, search, comparison, Project evidence, and export.
...
`app/blueprints/api_v1.py` exposes `/api/v1` for scripts and the bundled `darklab` CLI. The route layer stays intentionally thin: it authenticates a scoped PAT through `app/services/api_v1/auth.py`, shapes responses through `app/services/api_v1/serialization.py`, and then reuses the same command preparation, brokered run worker, history search, artifact storage, and project query services used by the browser.

The API accepts only a scoped personal access token through `Authorization: Bearer`. Browser portable credentials and anonymous UUIDs are rejected at the API boundary. The central policy maps every protected method and route to one required PAT scope and fails closed if a new route has no scope declaration. API errors use the stable `{"error":{"code":"...","message":"..."}}` envelope, and app-level 429/500 handlers preserve that envelope for `/api/v1/*`.
...
Manual-finding and finding-evidence API writes reuse the same Project services as the browser. They recheck the active Project, exact finding, confirmed target, typed source ownership, and `TRIAGE_FINDINGS` capability; optimistic revisions and possible-duplicate conflicts remain server-owned. Removing a finding-evidence link deletes only the relationship, not the referenced source record.
...
| `nuclei` | Adds `-ud /tmp/nuclei-templates/current`; uses owner-scoped `XDG_CONFIG_HOME=<workspace>/tools` when Files are enabled, except for explicit template updates | Redirects template storage to a `scanner:appuser` snapshot inside a Compose named volume. Container startup fills the cache only when its manifest is missing and treats download failure as non-fatal. Keeping the snapshot below the volume mount point permits a locked atomic replacement and one-time migration of older cache layouts. The web process can inspect the resulting exact snapshot, while normal ProjectDiscovery config and resume state stay under the active personal/team workspace's `tools/` folder. Output metadata records the template source for later Run Details, Atlas import, and evidence review. Silent. |
| `dalfox` | Inserts the v3 `scan` subcommand for a direct URL and adds `--only-discovery --skip-mining-dict` unless help or version output is requested | Keeps direct and assessment launches compatible with the bundled CLI and in bounded parameter-discovery mode without sending XSS payloads or loading remote mining dictionaries. Silent. |
...
`app/conf/commands.yaml` (plus optional `commands.local.yaml` overlays) is the single source of truth for every external-command surface: allow/deny policy, autocomplete grammar, runtime rewrites, workspace flags, `requires_secrets`, and the user-facing reference catalog. A normalized registry has one entry per root, doesn't share roots between commands and pipe helpers, and marks every pipe helper as enabled; ambiguous or incomplete registries fail during load. `command_catalog_from_registry()` projects each allow-listed entry into a normalized catalog — root, category, description, examples, flags, subcommands, `feature_required`, and an optional `knowledge` block — that feeds the discovery built-ins and the `/commands/catalog` route. `pipe_catalog_from_registry()` projects the `pipe_helpers` section the same way.
...
Live bulk-feed refresh is disabled by default. When enabled, the scheduler claims and commits a database lease per source before making a conditional HTTPS request against a fixed allowlisted URL. Download, retry, backoff, and full-feed validation run without a database write transaction. The effective lease covers the configured request window and bounded parse allowance; retry and pre-parse renewals are short transactions. Publishing then renews the exact owner's unexpired lease as the first write and atomically stores the accepted feed or failure state, audit event, and lease release. A stale worker can't publish or clear a successor's lease, SQLite's single-writer lock isn't held across provider latency, and PostgreSQL follows the same ownership contract. Downloads and attempts stay capped, standard proxy handling remains available, and a failure leaves the previous snapshot in place. Accepted source rows retain the origin, source URL, version, model/catalog version, publication/retrieval/acceptance times, checksum, conditional-request metadata, record count, attribution, and terms link. The command catalog returns that bounded feed status to the Options Provider Status modal, while `providers` exposes the same current source age and opt-in state in the terminal. The authenticated `GET /api/v1/risk/feeds` route and `darklab risk status` CLI reuse this read service for personal and Team-viewer tokens. They return a fixed EPSS/KEV collection and never call the scheduler, acquire a refresh lease, contact a provider, or write feed rows.
...
`osv_advisory_mode: external` is an explicit one-package boundary, not a scheduler or read hook. `POST /api/v1/advisories/osv/lookup` and `darklab advisory osv` require finding-triage permission in team scope and supply one normalized versionless PURL plus one exact version. The CLI sends only those two fields, rejects empty values before making a request, and distinguishes permission, disabled-mode, busy-provider, and provider-failure results. Acquisition posts only that pair to the fixed `https://api.osv.dev/v1/query` endpoint, requires the host in the outbound allowlist, rejects redirects before following them, and applies the shared timeout, attempt, response-size, record, and cache-expiry limits. The route uses the API Team write limit. A shared provider guard admits at most ten cache misses per minute and two provider calls at once, while one hash-keyed lease prevents duplicate requests for the same package and version. Redis coordinates those limits across workers; the supported single-worker fallback keeps equivalent in-process limits. Cache reads, accepted results, failed-attempt state, and audit rows use separate short database operations, so provider requests and retry backoff don't retain a database connection. Positive and negative cache entries retain only a SHA-256 lookup key. Accepted positive rows keep the normalized package identity needed for later correlation, use query-scoped advisory ids, and replace only rows for the same lookup hash. A failed request marks the source attempt without deleting earlier accepted rows. Logs, metrics, and the `cve_advisory.refresh` audit row carry source, outcome, counts, origin, attempts, and error classes, never the PURL, version, or provider payload. No imported SBOM, discovered inventory, or read surface can invoke this boundary automatically.
...
`services/assessments/profiles.py` owns the read-only, versioned profile contract used to snapshot assessment cycles. The shipped `assessment_profiles.yaml` catalog defines maintained Network, Web, API, TLS, and Combined profiles. The Combined definition reuses the other four profiles' exact check definitions, and cycle creation still expands only checks compatible with each confirmed target. Each profile, check, and evidence rule has a stable key and version; checks also declare applicable target types, policy level, a validated `command:` or `workflow:` recommendation or the one exact reviewed private-OAST action, completion guidance, and explicit evidence compatibility. That OAST action is valid only for an intrusive check with URL targets. Evidence rules bind accepted evidence kinds to command roots, workflow actions, structured output kinds, target matching, completion conditions, compatible tool/profile versions, and whether a clean negative result is meaningful. A generic Project link or command prefix on its own is not coverage.
...
`services/assessments/read_model.py` is the shared scope-aware read contract for cycle lists and detail. It returns bounded, deterministic check and fix-first pages and validates state, category, target-type, policy, evidence-availability, and public-risk filters. Cycle and check query builders add only the filters that were selected, which lets SQLite and PostgreSQL use the Project/status and Assessment/state indexes for bounded pages. The CVE-risk work queue, changed-CVE observation lookup, and Project risk projection follow the same reusable-query contract; high-cardinality query-plan tests require the supported indexes on both backends without pinning full planner text. `services/assessments/evidence_read.py` adds up to three newest evidence links to each returned check and the 20 newest links across the whole cycle, including only safe source ids, check/target context, availability, match origin, and timestamps. `services/assessments/summary.py` owns the shared cycle and category rollup queries plus the compact active-cycle coverage and top-three fix-first projection used by Project Overview, so both surfaces count the same saved check and remediation rows. Rollups keep total and applicable denominators separate and report covered, awaiting-review, untested, excluded, and unavailable-evidence counts without treating `not_applicable` work as completed coverage. `services/assessments/finding_worklist.py` admits only current observations stored by the selected cycle's compatible finding comparison, then delegates remediation grouping and deterministic KEV, EPSS, CVSS, and age ordering to the shared CVE-risk service. Its filter rollups count remediation groups, while each row preserves the observation and evidence counts, strongest validation method, contextual confidence and exposure, last-seen time, and expandable observation summaries. The detail model also includes the bounded, stored finding comparison from `services/assessments/reconciliation_read.py`; its rollup counts each remediation group once and expands the current and earlier observations only in item detail. Serializers omit personal session ids and retain only the safe team and member context needed by callers; the compact Overview projection also omits the profile snapshot and actor fields it doesn't display. Evidence matching and lifecycle mutations remain separate from reads, so loading either model never runs a command, contacts a provider, or changes a check.

`services/assessments/retest_queue.py` adds a finding-centered queue to the cycle detail without mutating findings or launching work during a read. It considers at most 50 findings in `ready_to_verify` or `needs_retest`, rebuilds each finding's current saved verification action, and groups only exact Project target, Assessment check, action, and HTTP role/profile matches. A shared group is launchable only for two to ten safe, credential-free plans with the same exact display command. The group digest covers every finding id, action-plan digest, and current verification status, so confirmation fails when the work changes. `projects_retest_queue.py` applies the ordinary command capability and write-rate boundary, revalidates that digest, and starts one Project-linked brokered run. `retest_finalization.py` then links the completed run to every finding independently; one stale or unavailable row is logged and skipped without rolling back links that succeeded. Comparison summaries remain suggestions, and the human triage service continues to own final `verified`, `needs_retest`, or `not_applicable` decisions.

`services/assessments/service_actions.py` and `service_action_catalog.py` own the central, conservative service-to-action registry. Every reviewed action declares its rationale, policy, accepted target types, command or evidence-family reference, required features, expected evidence, and conditions that make the suggestion unsupported. `service_action_recommendations.py` reads only bounded app-captured port entities tied to the same owner and Project, then attaches matching metadata to the Assessment check page. It never infers a service from a port number. Unknown services stay unsupported, ambiguous labels stay in review, and multiple saved service labels for the same socket produce a conflict that suppresses all actions. The Assessment renderer shows this metadata once per expanded target on desktop and mobile without adding a launch path; existing frozen check actions remain the only route to preview and start work.

`base_action_catalog.py` is the shared ordinary-action boundary beneath Assessment recommendations and Project-scoped probe planning. It defines the canonical action id, one bounded mode, target compatibility, policy floor, feature requirements, expected evidence, and exclusions for every command emitted by `command_plan()`. `command_target_urls.py` gives every URL-bearing command planner the same bracketed-host form for IPv6 literals. A selected Nmap or Nuclei profile may narrow compatibility or raise that floor, but it can't lower it. Dalfox exposes only its standard parameter-discovery command through this boundary; XSS payloads, OAST, takeover confirmation, Schemathesis, ZAP, and evidence-only correlation stay outside it.

`probe_targets.py` resolves either one exact entity id or one exact value to a confirmed, non-suppressed Atlas entity linked to the active owner-scoped Project. Missing, archived, unconfirmed, cross-owner, cross-Project, and ambiguous selections fail before command planning. Exact-value resolution records its fixed selector, candidate count, duration, result type, and safe request context without recording the submitted or canonical value; ordinary misses remain informational while ambiguity and invalid Project state retain warning visibility. `probe_plans.py` then composes that target with the ordinary-action registry and reviewed profiles without starting a run or contacting a provider. The catalog and planner treat a missing feature inventory as empty, so a feature-gated action fails closed unless its runtime availability was checked explicitly. Protected probe requests resolve an exact scoped profile id first, then a unique case-insensitive saved name, and keep the canonical id in the returned plan and digest. The public plan keeps the exact command, target, policy, feature gates, evidence expectations, availability, and request, time, credential, and fan-out bounds. A protected profile adds only its reviewable identity, revision, role, credential classes, and saved allowed-host, root, include-path, and exclude-path boundary; header definitions, Secret references and values, Files references and paths, generated arguments, and environment data stay private. Its versioned digest projection intentionally omits labels and help text while covering every execution field, including that protected scope. Human-readable terminal and CLI previews label the first 12 digest characters as the approval digest; API and JSON responses keep the complete value used for confirmation. `openapi_probe_schemas.py` defines strict reusable catalog, profile, plan, target, scope, bounds, availability, and started-run shapes, while `openapi_probe_examples.py` keeps representative success, unavailable, and stable-error examples beside those contracts. Probe failure records classify ordinary run-start and preparation rejections without tracebacks, while spawn and unexpected failures replace the caught exception chain with a bounded origin-only traceback so protected values can't return through logging. Probe log identifiers, action names, stable codes, and error classes pass through bounded grammars before they reach structured fields; control characters and oversized or unknown request values fold to safe empty or fixed fallback values. Browser and API adapters pass the generated request id, masked session, owner, and fixed source into the shared service record, while successful results add only the reviewed policy and new run id. Audit events prefer that same generated request id and accept a validated caller id only when server context isn't available. The older Assessment adapter continues hashing its complete payload, so extracting the shared confirmation check doesn't change existing Assessment digests.

`builtins_probe.py` registers the browser-owned `probe list`, `probe plan`, and `probe run` metadata. Built-in and external command metadata pass through the same recursive feature filter after their autocomplete contexts are merged, so the intrusive Nuclei value is suggested only while `assessment_intrusive_actions_enabled` is active; explicit input still reaches the fail-closed planner. Each probe action's autocomplete metadata carries the target types declared by the shared action catalog. Runtime suggestions then expose only compatible confirmed targets from the active Project. The same runtime context supplies active, owner-scoped Project slugs for every probe `--project` value and omits archived Projects; enabled active-Project HTTP profiles are labeled by saved name but insert their stable ids. The lazy `probe_terminal.js` adapter resolves a supplied active slug to its canonical Project id before calling the Project-scoped routes; stable ids continue to pass through directly. Catalog reads accept an explicit Project and otherwise use the active Project, and they can narrow the catalog to domain-, IP-, or URL-compatible actions. Plans and launches require an explicit Project and resolve exact terminal values through a narrow POST body before planning by entity id, keeping those values out of request URLs and routine access logs. List and plan suppress History and Recent-command persistence, remain available to viewers, and don't create a run, connector job, callback reservation, audit event, Assessment evidence, or separate probe record. Every plan carries caller-specific launch authorization derived from the shared probe capability contract. The terminal renders a denied reason and stops before opening its yes-or-no prompt; the launch route independently rechecks the same capabilities. Authorization isn't part of the approval digest because a role change is enforced at launch rather than treated as command-plan drift. Browser request diagnostics classify network and HTTP failures separately from invalid response shapes and report only a fixed phase, source, bounded Project id, status, and error class; the displayed target, command, response body, profile, and request URL never enter that telemetry. A protected launch audit keeps only the saved profile id and bounded role and credential-use classes alongside the ordinary Project, action, entity, policy, source, and run references; profile names, target values, headers, tokens, commands, Files paths, and private runtime paths are excluded. Protected cleanup is observed as soon as private material exists, so context validation, preparation, spawn, cancellation, and normal completion use the same hook. It reports success only after its private runtime directory is gone; an incomplete local or scanner-owned removal stays retryable by the run hook and startup stale-runtime recovery, and a cleanup failure never replaces the primary launch error.

`probe_execution.py` rebuilds the plan after transcript or API confirmation and sends one complete launch specification through the normal brokered-run service with trusted `link_project_id`. Browser launches also carry a validated `owner_tab_id`, then bind the returned stream to that same tab without clearing the preview or briefly settling the terminal to idle; the binding path owns the transition to `running`. `RUN_COMMANDS` gates every launch. Selecting a protected Project HTTP profile additionally requires `MANAGE_SECRETS`; `probe_protected_launch.py` rechecks its owner, revision, enabled state, references, and scope immediately before `probe_broker_launch.py` creates server-only arguments and private files. Completion and failed-spawn hooks clean that material without exposing it in public responses, audit details, metrics labels, logs, History, or the saved display command. The resulting record is an ordinary Project-linked run, so normal structured-evidence finalization and cycle reconciliation may later match it; the probe launch itself never writes Assessment evidence or check state. API v1 and the modular `darklab probe` client call the same catalog, planning, exact-target resolution, confirmation, and launch services. The CLI resolves an active Project slug through its paged Project list and sends only the canonical id to those API routes.

`nmap_profile_contracts.py`, `nmap_profile_catalog.py`, and `nmap_profiles.py` own the app-reviewed NSE profile boundary used by those suggestions and maintained command plans. Category profiles can select only `safe`, `default`, `version`, or `discovery`; service profiles and the separately confirmed vulnerability profile use fixed script names. The public contract includes the selector type, exact selectors, expected evidence kinds, policy, blocked broad category selectors, fixed app-owned arguments, and confirmation requirement. App-owned profiles reject operator-provided script arguments and Files-backed argument files. The FTP profile combines `ftp-syst` with the exact `ftp-anon` script, fixes `ftp-anon.maxlist=0`, and requires confirmation. This makes one standard anonymous login attempt without opening the script's directory-listing connection. The broad `auth`, `brute`, `dos`, `exploit`, `external`, `fuzzer`, and `intrusive` category selectors remain unavailable; individual service scripts are reviewed by exact name and policy instead of inheriting an entire category. Third-party scripts are not resolved or installed through this boundary.

`nmap_script_evidence_catalog.py` maps every exact informational service-script selector to one evidence family and proves that the map stays aligned with the reviewed profile catalog while remaining disjoint from vulnerability scripts. `nmap_service_observations.py` accepts bounded, entity-safe Nmap XML for open ports and normally reads only structured `table` and `elem` children from those exact scripts. The one output-only adapter in `nmap_exact_output_evidence.py` accepts only the exact `ftp-anon` 230 success line produced by the no-listing profile and turns it into access, account, and reply-code fields. Changed, denied, or listing-bearing output is omitted. Each observation keeps the canonical port, bounded service name, script and evidence ids, source run, Nmap/parser versions, aware timestamp, and bounded path/value fields. Other free-form `output` attributes, host scripts, output-only rows, unknown scripts, closed ports, and vulnerability-profile scripts are omitted. Depth, value, field, observation, byte, and XML-element limits report truncation or fail closed; parsing is read-only and labels every accepted row informational. Successful completed Nmap runs read one unambiguous marked XML artifact once, then isolate service-evidence persistence and version inference in separate savepoints. `nmap_service_observations` stores only those bounded informational fields in the run owner's scope with an idempotent run, target, and script identity. A failed or conflicting write rolls back that optional stage without changing the run, another Nmap evidence consumer, or any finding state. `nmap_service_evidence_read.py` rechecks the personal or team owner when it pages one run's observations. Assessment reads join only available run-evidence links from the already scoped cycle, cap each check's newest-first collection, and publish the same typed serializer through the API. The browser `/runs/<run_id>/service-evidence` route applies the same personal or team scope for Run Details, and `nmap_service_evidence.js` renders that page or the attached check page through one read-only desktop/mobile component. No read path loads raw output or derives a finding or check transition.

The external `darklab evidence services` command calls the owner-scoped service-evidence API directly. Its focused parser and handler preserve the bounded page for JSON and NDJSON consumers while presenting the same typed fields in text, and they don't grow the main CLI module or load scanner output. Finding evidence stays under the same grouped `evidence` noun: `list` returns the route's bounded whole collection, `link` validates the typed source shape before sending it, and `unlink` requires the stable relationship id. Repeated links remain explicit no-ops, and no command copies or deletes the source record.
...
`services/assessments/export_context.py` owns the complete saved-cycle handoff for reports and evidence packages. A draft may name one cycle in the same Project, including archived history; an empty selection chooses the active cycle first, then the newest completed or archived cycle. The owner-scoped read returns the frozen profile, target/check snapshots, category and coverage rollups, manual exclusions, evidence references, source-run timing and command mode, stored Nmap or Schemathesis versions when available, methodology, unavailable-source and omitted-screenshot warnings, fix-first rows, finding changes, and explicit redaction boundaries. Report and package composition apply their existing redaction and private-note rules to that context before rendering or archiving it. The selector never replays a command, rereads a scanner provider, or changes the cycle.

### HTTP Assessment Profile Services

`project_http_profiles` stores reusable, Project-scoped HTTP testing context without storing credential values. A profile contains a name, role, base URL and scope roots, exact allowed hosts, header names, app-managed Secret references, validated Files references for client credentials, a proxy origin, login-workflow reference, token-capture rules, include/exclude paths, rate and concurrency limits, enabled state, and optimistic revision. URL paths use one RFC 3986 dot-segment normalization boundary before storage or comparison. Encoded separators, backslashes, malformed escapes, and nested encodings that could be decoded differently by a client or server are rejected. Protected local adapters compare and launch only the normalized target, so a request can't leave a saved root or cross an excluded prefix when a downstream client normalizes the URL. The shared SQLite/Postgres migration uses JSON/JSONB only for bounded structured fields, deletes profiles with their Project, and indexes owner, Project, enabled state, and normalized name.

`services/assessments/http_profiles.py` applies the same personal/team owner predicate as the rest of Projects. `http_profile_scope.py` derives usable scope from exact hosts on confirmed, non-suppressed Project entities and skips malformed legacy values with one count-only warning. `http_profile_secret_references.py` resolves an entered Secret name or unique consumer env binding inside that owner scope, gives an exact Secret name precedence over an alias, and canonicalizes saved references to the stored name. Missing-reference errors identify only the affected profile fields, not Secret values. Every Secret, workflow, and Files reference is validated before a write, archived Projects are rejected, and the per-Project quota doesn't evict older profiles. One malformed saved entity therefore can't hide profiles or remove valid hosts from the usable scope. Secret values are never read into the profile model. Team members with `MANAGE_SECRETS` can read reference names and availability and make changes; other Project viewers receive only header names, credential-use categories, proxy presence, and reference counts. The browser and API routes share that service, use optimistic revisions for updates, and commit mutations with fail-closed audit records. Logs and audit details contain only owner-safe ids, role, enabled state, and counts. Session-token migration moves personal ownership and private creator/editor references together. Saving or reading a profile doesn't launch a scanner, inject credentials, or persist reusable scanner configuration.

`oast_connector` is the disabled-by-default configuration boundary for a private Interactsh-compatible service. Startup accepts only a credential-free HTTPS origin, an environment-variable token reference, an exact callback DNS suffix without wildcards, verified TLS by default, a 5-minute to 30-day callback-retention policy, and an explicit privacy acknowledgement. Enabling an incomplete or malformed connector stops startup. `services/connectors/oast_config.py` returns an immutable non-secret settings value and resolves the token only at an explicit connector call boundary. Configuration loading and settings reads don't contact the service, allocate a correlation domain, poll callbacks, or write evidence.

Production runs the private OAST loop through the profile-gated `oast-worker` Compose service. The image entrypoint's `DARKLAB_PROCESS_ROLE=oast-worker` branch dispatches directly to `services.connectors.oast_worker` after staging the shared configuration, app-data, and workspace mounts and before any web-only scanner firewall or Gunicorn setup. The service uses the same database and Redis dependencies as the web app, receives the same master-key source and fixed `DARKLAB_OAST_TOKEN` environment binding, has no published port or scanner capabilities, and proves its role through a tmpfs health marker. An unknown process role stops startup.

`oast_correlations` is the durable SQLite/PostgreSQL reservation boundary. `services/connectors/oast_correlations.py` creates one random callback label only after re-reading an owner-scoped active assessment, applicable check, confirmed target link, intrusive policy, and exact reviewed OAST action. A row keeps the owner, Project, assessment, check, target, actor, fixed active and purge deadlines, and a SHA-256 service-origin identity; it doesn't keep the service URL, token reference, token value, callback payloads, or evidence. Per-owner and per-check quotas reject excess work without evicting a live reservation. `oast_correlation_lifecycle.py` binds the reservation to one source run per check with guarded state changes, then closes, fails, expires, or purges it at its fixed boundaries. Personal rows stay with their Project when credentials change, and Project deletion removes them before assessment checks. Reserving or changing this app-owned state doesn't contact the private service or make the callback reachable there.

`oast_interaction_review.py` accepts a provider-independent normalized DNS, HTTP, SMTP, or LDAP record and produces only a bounded protocol summary. It hashes any provider event id, strips HTTP queries and fragments, counts unknown or truncated detail fields, and rejects oversized or malformed input. `oast_interactions.py` rechecks the owner, callback identity, active correlation window, per-correlation limit, and duplicate fingerprint before saving that summary. One accepted interaction preserves the originating run as assessment evidence and moves the check to `needs_review` unless a protected manual state applies. `oast_interaction_findings.py` can associate the interaction with one finding only after re-reading the finding in the same Project and proving that its target matches exactly; it then uses the existing finding-evidence service for the run and assessment-check links. Purging a correlation or deleting its Project removes the interaction summaries. These services don't call a provider or expose a callback endpoint.

`oast_provider_transport.py` implements the fixed private Interactsh client boundary. It uses the default 33-character alphanumeric callback identity: the first 20 characters are the registered correlation id and the remaining 13 are the request nonce. Registration and deregistration are fixed POSTs; polling is the one fixed GET required by the provider protocol. The deployment token stays in the `Authorization` header, while the provider-required correlation id and session secret are the only poll query values. Session RSA private keys and secrets are excluded from object representations and aren't part of the durable correlation row. Polling decrypts the provider AES key with RSA-OAEP SHA-256, decrypts each record with AES-CTR, requires the full callback identity to match exactly, and returns only a provider-independent protocol summary. Raw headers, bodies, responses, client addresses, SMTP senders, and provider metadata don't cross that normalization boundary; shared `extra` and `tlddata` scopes are counted and ignored. The HTTP seam permits only `/register`, `/poll`, and `/deregister`, rejects redirects and changed response URLs, caps control and poll bodies before JSON parsing, and uses bounded errors without URLs, tokens, secrets, or response bodies. No settings read or correlation reservation invokes this transport.

`oast_provider_spool.py` keeps the generated provider session available across worker restarts without adding its secret or RSA private key to the database. It serializes only the fixed session contract, encrypts it with AES-GCM through the existing app master-key boundary, and authenticates the durable correlation id, full callback label, and service-origin hash as associated data. The private app-data directory is forced to `0700`, atomically replaced session files are `0600`, reads don't follow symlinks, and ids must match the generated correlation shape before they can become paths. Loading rejects a different correlation identity, service origin, app key, payload shape, or modified ciphertext. Old regular files are listed in bounded batches for worker reconciliation; terminal cleanup is best effort and never logs session material.

`oast_worker.py` is the only process that crosses from an explicit durable reservation into private provider traffic. A deployment-wide advisory or file lock permits one worker. Each bounded tick expires old correlations, reconciles old spool files, and prioritizes active polling over new registration. Reserved rows can register their exact preapproved callback label but aren't polled until a source run activates them. Active rows recover the encrypted session after restart, reject changed provider origins or callback suffixes, count provider-side omissions, and pass normalized interactions back through the existing owner, check, callback, and time-window validation before anything is saved. Missing credentials and temporary provider failures remain retryable until the fixed deadline. Corrupt or unstorable private session material fails the correlation instead of silently creating another provider identity. Terminal sessions are deregistered before their local material is removed; unknown old files are discarded in bounded batches, and the provider's configured retention remains the backstop when a remote cleanup can't be confirmed. Successful provider calls record fixed phases, timings, attempts, and counts at DEBUG, while provider-ready, positive ingestion, and confirmed cleanup transitions record count-only INFO milestones. Those records never include provider, callback, credential, key, payload, or spool values.

`project_assessment_oast.js` owns the shared desktop/mobile browser sequence. Loading an applicable check recovers its bounded redacted history and polls only the exact reserved or active correlation while that state needs attention. The first confirmation reserves the reviewed provider-free plan; launch requires a fresh redacted plan and a second confirmation after readiness. The module keeps only safe correlation state, never stores or renders the returned callback, and shows accepted, duplicate, and rejected counts with the fixed active and purge deadlines. A launched or recovered source run opens through the existing Run Details handoff. Poll timers are cleared when the Project controller is invalidated.

`zap_connector` is the disabled-by-default operator boundary for an external OWASP ZAP service. Startup accepts only an HTTP(S) ZAP origin without credentials or request paths, an API-key environment reference, canonical IPv4/IPv6 target networks, an exact HTTPS scanner-side policy endpoint and token reference, one policy identity, one enforcing proxy address, verified ZAP TLS by default, and fixed concurrency, job-lifetime, and report-byte bounds. Enabling it requires every scope field; malformed or incomplete settings stop startup instead of leaving a partly active connector. `services/connectors/zap_config.py` exposes only normalized non-secret settings, rechecks the ZAP and scope-policy endpoints at the connector boundary, and resolves either secret only when its connector call needs it. `zap_scope.py` accepts one credential-free HTTP(S) URL, applies the shared unambiguous path normalization, caps the app's DNS results at 16 distinct addresses, and requires every answer to fall inside the configured networks before returning an immutable reviewed target. Plan generation repeats that normalization for targets, roots, includes, and exclusions before it builds scope patterns. Disabled state, missing credentials, malformed targets, ambiguous paths, resolver failures, excess answers, and out-of-scope addresses use bounded machine codes. Loading configuration alone never resolves a target, contacts either service, or submits work.

Production runs the ZAP loop through the profile-gated `zap-worker` Compose service. `DARKLAB_PROCESS_ROLE=zap-worker` follows the same fail-closed entrypoint dispatch and shared durable mounts as the OAST worker, then executes `services.connectors.zap_worker` without web-only scanner capabilities. The service receives the fixed `DARKLAB_ZAP_API_KEY` and `DARKLAB_ZAP_SCOPE_POLICY_TOKEN` bindings, shares database and Redis dependencies with the app, publishes no port, and exposes only a tmpfs process-role health marker. Operator Compose overrides attach the shell and worker to the ZAP API and policy network; the ZAP scanner's target route remains limited to the independently enforcing proxy.

`services/connectors/zap_scope_policy.py` proves that the network making ZAP's requests sees and enforces the same boundary. Its fixed HTTPS request binds a fresh nonce, policy id, canonical CIDR digest, proxy host and port, and up to eight distinct target hosts. The bounded response must echo those values, attest a CIDR-enforcing proxy with per-connection DNS checks, and return no more than 16 valid addresses per host, all inside the configured networks. Redirects, ambient HTTP proxies, changed identities, replayed nonces, malformed or oversized responses, and mixed-scope answers fail closed. Every generated Automation Framework plan sets that exact proxy. The web process attests at preview and confirmation, and the worker attests again immediately before submission. The operator deployment must make the policy service and proxy one scanner-side boundary and remove ZAP's direct route to assessment targets; the proxy re-resolves and enforces CIDRs for each connection, so a later DNS change can't bypass the earlier review.

`services/connectors/zap_plan.py` turns one to eight reviewed URLs plus one current HTTP profile into portable, non-secret Automation Framework YAML and an immutable operator summary. It rechecks profile hosts, roots, included paths, saved exclusions, and up to 50 additional path exclusions. Safe plans use scope-only passive scanning, single-threaded non-form spidering, capped response bodies and alerts, a bounded passive-queue wait, and a Traditional JSON report. Intrusive plans require the separate intrusive-action operator gate and add one low-strength, single-threaded active scan with fixed request delay, rule time, total time, and alert caps. Both modes fail on framework errors, suppress stdout progress, derive phase caps from the configured job lifetime, and carry that lifetime in the review summary. Profiles with Secrets, headers, client certificates, proxies, login workflows, or token capture are rejected because the generated document cannot contain or imply protected material. Plan generation does not contact ZAP or create a remote job.

`services/assessments/zap_connector.py` is the owner-scoped review and submission boundary used by the Project browser and API v1 routes. Preview accepts confirmed Project URL entity ids rather than caller-supplied target URLs, rechecks the active cycle and current anonymous HTTP profile, resolves every selected hostname against the configured networks, requires a matching scanner-side policy attestation, and returns the exact generated YAML plus a non-secret summary. Its digest covers the plan hash, selected entity ids, HTTP profile revision, policy, exclusions, policy identity, CIDR digest, and proxy binding. Submission repeats those reads, app DNS checks, and scanner attestation, requires explicit confirmation and the fresh digest, and queues the exact rebuilt bytes without resolving the ZAP API key in a web process. Status and cancellation require the same personal or team owner and the exact Project, assessment, and check path. Public job state omits session and actor identity, private spool paths, target values, report bodies, and worker-only import identity; ready jobs add only their Files path and Atlas draft id. Submit and cancel audit events contain ids, policy, state, and counts without plan YAML or target values.

`project_assessment_zap.js` owns the desktop/mobile browser sequence without widening the connector contract. An explicit URL-check action first reads the bounded newest-first history, restores an active or completed job inline, and polls only that exact nested job while its saved state remains active. New work accepts at most eight URL entity ids already present in the current Assessment page, disables HTTP profiles that aren't anonymous and credential-free, and sends the same selection to preview and confirmation. The second confirmation renders the returned plan summary and exact YAML; queueing adds only `confirmed: true` and the returned digest. Inline state shows bounded progress and errors, protects cancellation with the shared confirmation, and exposes the server-provided Files path. A ready job offers **Review Atlas import**, which passes only its draft id into the shared Atlas modal. That modal reads the owner-scoped bounded preview and requires the existing explicit Apply action; reading job status never applies the draft. Poll timers are cleared when the Project controller is invalidated.

`zap_connector_jobs` is the durable SQLite/PostgreSQL lifecycle record for work that crosses the ZAP service boundary. A row binds one job to its personal or team owner, Project, active assessment check, current HTTP profile revision, actor, policy, fixed expiry, reviewed plan summary, and report identity. Personal jobs stay with their Project when credentials change. `services/connectors/zap_jobs.py` rechecks that ownership and current profile boundary before creating a queued row. `zap_job_lifecycle.py` uses guarded compare-and-set transitions for submission, remote execution, cancellation intent, download, import readiness, failure, and expiry, so a stale worker can't replace newer state. A confirmed transition to `failed` emits one sanitized terminal ERROR with the safe job id, bounded state, phase, code, class, and traceback; a concurrent transition conflict emits no false failure record. `zap_remote_progress.py` accepts only the expected numeric plan id and aware timestamps, keeps total information/warning/error counts, and retains no more than the eight newest whitespace-normalized 500-character messages at each level. Plan YAML, API keys, report bodies, and full ZAP logs aren't stored in the job table. Creating or updating a lifecycle record does not contact ZAP.

`services/connectors/zap_http.py` permits only the Automation Framework plan start, progress and stop endpoints plus the core transfer upload and download endpoints. It sends the API key only through `X-ZAP-API-Key`, rejects redirects and changed response URLs, honors the connector TLS choice, caps every response before parsing, and turns remote and network failures into bounded codes without logging a URL, key, request body, or response body. `zap_transport.py` gives each durable job a traversal-free transfer subdirectory, uploads the already reviewed UTF-8 plan, starts its `${XFER}` path, validates the returned numeric plan id and stop confirmation, passes progress through the bounded reviewer, and hashes the size-limited downloaded report. These calls don't accept caller-selected endpoints or filesystem paths. The report body stays in memory at this boundary until the worker hands it to Files; it isn't written to the lifecycle row.

`zap_job_artifacts.py` keeps the exact reviewed plan in a private app-data spool with a `0700` directory and `0600` files until submission succeeds or the job reaches a terminal state. Old spools are reconciled in bounded batches against queued and submitting database rows so a crash between file creation and row creation doesn't leave private plan material behind. Failed plan deletion emits a sanitized error, while stale-spool inspection failures are aggregated and rate-limited; neither record includes paths, plan content, targets, authentication roles, or reports. `zap_job_queue.py` owns the database-and-spool queue boundary, while `zap_worker.py` is a dedicated runtime process that expires overdue work, respects the configured deployment-wide remote-job ceiling, re-attests queued targets from the scanner-side policy, submits one reviewed plan, polls bounded progress, requests cancellation, and downloads a named completed report. `zap_worker_support.py` keeps recovery and scope checks separate from the worker loop, and `zap_worker_telemetry.py` reports successful provider calls at DEBUG, durable state changes at INFO, one bounded WARNING per repeated retry condition, and one per-tick concurrency summary at DEBUG. Those records use safe job ids, fixed phases and outcomes, attempts, timings, counts, and bounded error metadata; they omit remote ids, target values, credentials, provider payloads, plan content, and reports. `zap_worker_lock.py` holds a deployment-wide PostgreSQL advisory lock or SQLite file lock for that process, so an accidentally duplicated worker exits without racing the concurrency ceiling. A fresh submitting or downloading claim gets a five-minute recovery window; an abandoned submission fails as uncertain rather than risking a second remote scan, while an abandoned download can retry against its stable job identity.

The worker saves the report through the existing owner-scoped Files quota at `assessments/zap/<job id>/<report filename>` and sends those exact bytes through the existing `zap_json` Atlas preview path. The preview uses a deterministic draft id so download recovery replaces the same draft instead of creating duplicates. The bounded draft-read route rechecks owner, expiry, and row-set digest without returning the complete normalized rows. The lifecycle stays `ready` until the owner applies that draft through Atlas; the Atlas apply transaction records its import batch on the ZAP job and moves the job to `imported`. Neither the worker nor a read route applies remote findings silently.

`services/assessments/http_profile_execution.py` owns protected Curl, HTTPx, Katana, Dalfox, and SQLmap launch validation. Preview and launch re-read the scoped profile, and launch rechecks its revision, enabled state, exact Project target, team capability, Secret and Files references, and supported features. Materialization reuses that same scoped profile row instead of issuing a second identical lookup. The service resolves Secret values only after broker availability is confirmed. `services/assessments/http_profile_material.py` builds each tool's private context: Curl receives an escaped config for request headers and client-certificate paths, HTTPx receives a ProjectDiscovery secrets file, Katana receives a header file, Dalfox receives a JSON config containing only protected headers and a no-redirect setting, and SQLmap receives a private header configuration. Katana's crawl expressions keep the exact saved scheme, effective port, root, included paths, and excluded paths around every request that receives those headers. Nuclei HTTP profiles fail closed during preview because domain-scoped ProjectDiscovery material can't enforce that same boundary for every template-generated request; the materializer and command builder independently reject that combination as defense in depth. Anonymous HTTPx recommendations keep the same default request-rate and concurrency bounds that a saved profile can override; Curl's bounded plan disables ambient config, proxies, and redirects. Dalfox's plan fixes one target, discovery-only execution, local built-in parameter mining, request/time bounds, and no redirects. SQLmap accepts one credential-free HTTP(S) target and reviewed detection options only. Assessment launches validate that narrow carrier first, then the trusted runtime adds the fixed detection level, risk, technique, request, thread, and time bounds and any private header configuration. Protected paths are appended only after normal command validation, while the persisted and displayed command keeps `[protected]` placeholders. Private values join the existing output-mask list. `services/assessments/http_profile_runtime.py` creates per-run `0700` directories with `0600` files, removes them from worker cleanup and preparation-error paths, and bounds startup recovery of abandoned material. Redirect broadening, proxy use without an allowlist, login/token capture, unsupported certificate use, stale profiles, and out-of-scope targets fail closed. App-owned launch adapters may also pass a typed immutable output-signal context through the shared run broker so a classifier can bind reviewed evidence metadata to the generated run id. Browser and API run request bodies never map that internal context, and the broker rejects invalid context before command preparation.
...
`services/assessments/schemathesis_evidence_persistence.py` runs after ordinary History persistence and revalidates the Project, active cycle, exact check, profile snapshot, execution policy, linked external run, confirmed URL target, and unchanged source artifact before it stores a reviewed report. Per-operation rows keep bounded status and count data plus minimized failure identities, never generated values, bodies, headers, or scanner messages. `schemathesis_finding_persistence.py` materializes one deterministic active-confirmation finding per target, check, operation, and failure type, with typed links to the source run and assessment check. The whole Schemathesis write uses a savepoint, so a storage rejection or failure doesn't roll back the completed run. Coverage matching uses those durable rows only: a failure-bearing report supplies exact target evidence and moves the check to needs review, while a clean report supplies API-operation coverage only when the stored operation set exactly matches the reviewed schema and every operation succeeded. Incomplete clean reports remain untested. The SQLite and Postgres migration gives reports and operation rows Project and assessment lifecycle cleanup without deleting their source runs or findings.

`services/assessments/dalfox_parameter_observations.py` accepts structured output only when the saved command proves discovery-only JSONL mode with local dictionary mining disabled. New plans use Dalfox v3's required `scan` subcommand, while the reader normalizes the earlier direct-target and `url` forms so saved evidence remains usable after an upgrade. It requires the stream summary before parameter rows, binds every row to the generated run id and exact canonical command URL, keeps only the documented location types, deduplicates stable observation ids, and rejects new rows after the fixed cap. The shared output classifier stores the summary and accepted rows in each line event's source detail, so live output and restored history use the same evidence without turning parameter discovery into a finding.
...
`services/runs/completion_policy.py` keeps documented findings exits from widening the shared run contract. The broker derives Dalfox's policy only from the same internal typed XSS context used by the classifier, and derives Schemathesis's policy only from the reviewed execution that owns the exact private report reader; request bodies can't supply either one. Tool exit code 1 becomes effective success only after Dalfox emits a valid reviewed summary with an accepted XSS observation, or Schemathesis returns a complete bounded report with at least one reviewed failure. The saved run and normal completion code use that effective result, while the exit event and `RUN_EXIT_CODE_ACCEPTED` INFO record retain the original tool code. Missing, malformed, incomplete, or empty evidence, ordinary tool runs, exit code 2, and output-sink failures keep their existing failure meaning.
...
`services/assessments/dalfox_xss_command.py` owns the app's active command shape. It accepts only the saved-evidence value above and supports query parameters that are still present in the canonical evidence URL. The fixed v3 `scan` command checks one location-qualified parameter with one worker and one target, caps payloads, aggregate request rate, reviewed request count, and scan time, and disables redirects, discovery, mining, retries, remote or blind payload sources, request/response capture, WAF probing, and bypass mutation. A provenance or deterministic observation-id mismatch makes the plan unavailable. The command isn't registered in the ordinary command catalog. `dalfox_parameter_options.py` exposes a bounded newest-first set of reviewed Project observations, and the shared Assessment UI asks the operator to choose one before it renders the exact active plan.

`services/assessments/dalfox_oast_command.py` keeps blind-XSS validation separate from ordinary active XSS. It accepts one unchanged saved query-parameter observation and either the redacted app callback placeholder or one credential-free lowercase HTTPS callback origin. The fixed command uses Dalfox's app-owned callback path with the same one-target, worker, rate, payload, request, and time ceilings as active validation while disabling scanner-managed OAST, redirects, discovery, mining, remote payloads, WAF probing, and bypass mutation. The saved-command mode recognizes only the redacted shape, including the optional protected HTTP-profile marker. Web profile 1.6 exposes it as a separate intrusive check whose provider-free preview stays unlaunchable until an explicit callback reservation has become ready.
...
`services/assessments/command_modes.py` derives a frozen execution mode from the saved command only when it matches a maintained Dalfox, generic Nuclei, or TLS shape. Dalfox discovery, active-XSS, and blind-XSS callback bounds stay separate, as do Nuclei's safe, standard, and intrusive profiles. TLS certificate-chain and fast high-severity configuration commands also have distinct exact modes, so a broad or changed TLS run can't satisfy either maintained check. Evidence rules may require that mode in addition to their command root, completion, target, and output contracts. Mode matching still applies to successful negative evidence, so a clean lower-risk or discovery run can't satisfy a different validation check. Unrecognized or drifted commands have no mode. Web profile 1.6 uses separate rules for Dalfox discovery, active XSS, blind XSS, standard Nuclei, and intrusive Nuclei; older frozen cycle snapshots keep the definitions they started with.

Generic Nuclei recommendations derive one immutable reviewed profile from the frozen check policy. The safe profile selects high and critical exposure, misconfiguration, technology, and TLS templates. The standard profile adds medium severity plus known-CVE, network-service, and API families. Both profiles allow only HTTP, network, and TLS protocols and explicitly exclude authentication, brute-force, denial-of-service, exploit, fuzzing, intrusive, callback, DAST, code, JavaScript, local-file, workflow, Whois, and headless templates. The maintained Web profile includes a separate intrusive check limited to reviewed headless and low-aggression DAST families. Safe, standard, and intrusive runs have exact command modes, including approved protected-material placeholders, so one profile's clean run can't satisfy another profile's evidence rule. Every profile disables redirects, Interactsh, and automatic update checks. Headless profiles also require Nuclei's `-system-chrome` mode and use the release image's Debian Chromium package; they never depend on Rod's runtime browser download in the non-executable temporary filesystem. Docker's default namespace restrictions prevent Chromium's internal Linux sandbox from starting, so the exact reviewed profile passes `--no-sandbox` to Chromium while retaining the outer container boundary, the unprivileged scanner account, and the existing command, target, and network controls. Its public launch plan includes the profile label, managed-cache source, included families, exclusions, and explicit-only update policy. It also reads the fixed managed cache's regular-file `.checksum` manifest through byte and entry limits, derives a stable SHA-256 revision from its normalized path/digest records, and accepts a recorded release only from a bounded Nuclei config that names that exact cache directory. Compose persists the cache and release marker. The web entrypoint runs a bounded, best-effort update only when that manifest is absent, migrates the former volume-root layout once through the image's fixed Python runtime, and keeps the live snapshot in a swappable child directory. A migration that can't rebase the manifest restores the former layout and stops startup instead of accepting stale paths; a first-install download failure remains non-fatal, and an installed snapshot is never refreshed automatically. Assessment-plan preflight adds installed-binary validation, refresh age, and compatibility state to that snapshot. Missing, oversized, invalid, unreadable, or incompatible caches make Nuclei work unavailable without contacting a scan target. An explicit operator refresh stages, validates, rebases its manifest to the live path, and atomically swaps the cache under the shared scan/update lock, then rebuilds the full preview with a new digest. Failure records identify only the fixed refresh phase, reason, bounded exception class, duration, and update or validation exit status; worker output, cache paths, and template content stay private. Nuclei launches open the entrypoint-created lock without `O_CREAT`, avoiding Linux protected-regular-file rejection for the scanner identity in sticky `/tmp`; local development creates a missing lock atomically with `O_EXCL`. Each launch then takes the shared lock and rechecks the approved digest, while batch start holds that lock across preview reconstruction and item materialization so one batch can't mix snapshots. The batch detail read model also collapses recognized template-loading failures from latest failed attempts into one bounded diagnosis and a refresh-plus-retry-preview action without changing source runs or evidence. The dedicated app-owned takeover template keeps its separate digest-pinned command and doesn't inherit the generic profile.

`services/assessments/nuclei_recommendation_evidence.py` reads only saved owner- and Project-scoped evidence through fixed run, event, byte, finding, service, and label limits. It matches normalized HTTPx technology metadata, version-inference findings, explicit service names, and correlated DNSx dangling-record reviews to the frozen target with the same host-or-descendant identity contract used by assessment evidence. `nuclei_recommendations.py` turns those signals into public check metadata for the standard vulnerability and safe takeover checks. The metadata reports its evidence counts, a short label sample, truncation state, and `manual_confirmation_only` launch mode. It never changes the frozen profile or action plan, contacts an Intel provider, writes state, or calls the run broker. Intrusive Nuclei returns an explicit abstention regardless of saved signals.

`assessment_intrusive_actions_enabled` is the deployment gate for maintained intrusive Assessment actions and the reviewed intrusive Nuclei profile used by Project probes, and it defaults to `false`. The flag is only one launch prerequisite: an enabled action must still match its exact Assessment or probe plan, confirmed Project target, applicable saved source evidence, request and time limits, and fresh per-launch confirmation. It doesn't change the ordinary command registry, doesn't enable intrusive Dalfox probes, and leaves destructive actions without a launch path.

The browser cycle-list route adds the bounded catalog projection from `services/assessments/profile_summaries.py`. It includes only the profile key, version, label, purpose, target types, and check count needed by the start-cycle picker; the full definitions remain in the immutable snapshot of a created cycle. The Assessment tab uses that one list response to avoid a second catalog-only route, then pages the selected cycle's checks through the shared read model. `manual_evidence_read.py` attaches at most 20 newest-first manual evidence references to each returned check, including unavailable tombstones and a truthful total, without loading source contents. Its controller keeps the selected cycle, coverage and priority filters, independent pages, expanded targets, and scroll position per Project. Filter and pager refreshes keep the cycle, rollups, and unaffected worklist rendered while the changed check or fix-first slice reports its own loading or retry state. Reopening Projects keeps the active Assessment tab instead of resetting to Details, and focused finding editors and confirmations return focus to the action that opened them. Desktop check actions use the shared button primitives; mobile check actions use one touch-sized trigger and the shared action sheet, while cycle actions stay in the existing sticky mobile footer.
...
`blueprints/projects_assessments.py` exposes the browser list, create, detail, update, deletion-preview, and deletion routes through the existing Projects blueprint. `blueprints/api_v1_assessments.py` and `api_v1_assessment_checks.py` expose the matching PAT-authenticated cycle, manual-state, and evidence operations. `projects_assessment_oast.py` and `api_v1_assessment_oast.py` add matching nested reads and explicit reservation writes for the blind-XSS check, while their launch modules own the separate ready-only start. Both OAST surfaces rebuild the current provider-free plan before accepting its digest, keep list and reservation responses redacted, disclose a callback only from one exact ready correlation, and never resolve the provider token or call the service. Launch rechecks the staged session and provider scope, keeps the callback in typed private execution material, and atomically binds the exact broker-created run before output handling begins. All Assessment surfaces reuse the same safe serializers and personal/team scope. Cycle and check mutations require `MUTATE_PROJECTS`; reservations and OAST launches require `RUN_COMMANDS`. Writes use the normal Project limit, reject archived Projects, record assessment-specific audit events, and log only ids, profile versions, state transitions, and counts. A rejected action launch records the bounded owner and Assessment context with a fixed broker reason, while quota enforcement emits one contextual warning from the shared service boundary with safe ids and counts. Assessment deletion and its fail-closed audit record share one transaction.

`services/assessments/evidence_matching.py` builds a bounded fact set for one saved run from registry-typed command targets, explicit scan-target observations, workflow provenance, materialized Atlas entities, findings, and artifacts. `evidence_target_parsing.py` reports a bounded WARNING when registry target parsing fails, then records DEBUG identity counts for the generic fallback and a low-cardinality parser outcome metric; none of those signals contains the command, parser input, target values, or rejected values. A profile rule matches only when its accepted evidence type, command or workflow family, completion condition, known version constraint, target relationship, and structured-output requirement agree. A successful rule with a defined negative-evidence contract can cover a clean result; a Project link by itself cannot. Unknown tool versions don't satisfy a version-constrained rule.
...
Optional Dalfox, Nmap, Schemathesis, and takeover work also runs in isolated finalization savepoints so a parser, read, query, or persistence failure can't discard the completed run. `services/runs/finalization_observability.py` gives those caught failures one privacy-safe ERROR contract: a bounded source-file, function, and line summary identifies the originating frames, the original source expression and exception message are replaced, and fixed stage plus bounded owner and error fields stay available for diagnosis. A deterministic Schemathesis Project-link change is a WARNING skip because no operation failed. Neither path logs commands, workspace paths, targets, evidence bodies, reports, or parser payloads.
...
**Assessment action launches.** `services/assessments/action_plans.py` owns the shared plan contract for direct Assessment recommendations and finding verification. Every preview re-reads the frozen profile action and policy, confirms that the exact frozen target is still a confirmed Project entity, and returns the redacted display command, target, policy, Project-only fan-out, request/time bounds, selected HTTP-profile summary, credential-use declaration, availability reason, and a digest over the complete preview. Launch accepts only `confirmed: true` plus that current digest and recomputes the plan immediately. Maintained safe and standard command templates follow the normal path. Direct intrusive launches are limited to the frozen XSS-validation and intrusive Nuclei checks. Both require the deployment opt-in and a fresh warning confirmation; XSS also requires one reviewed saved parameter observation, while Nuclei requires the exact intrusive command mode and unchanged managed-template snapshot. Private-OAST plans use their separate reservation route and remain unavailable to this generic launch path. The app revalidates those conditions immediately before launch, and destructive policy always fails closed. Archived Projects or cycles, changed targets, stale digests, unsupported HTTP-profile features, workflows without a frozen input map, unsupported commands, and evidence or contract drift also fail closed.

The bundled CLI calls that direct Assessment route with the exact Project, cycle, and check ids. Its optional HTTP-profile, source-run, parameter-observation, and schema-artifact flags are forwarded unchanged to preview and confirmation, so the server remains the only component that decides whether a selection belongs to the frozen check. The CLI always previews first, launches only with `--confirm`, and sends the newly returned digest. ZAP and private OAST keep their separate multi-step connector routes instead of being flattened into the generic command path.

Shared external CLI HTTP exceptions retain the public response status, code, message, and details for lifecycle-aware handlers. Existing commands continue to print the same readable error string unless a focused handler deliberately uses those structured fields.

Assessor-authored finding mutations use a separate grouped `finding` noun rather than changing the existing flat `project-findings` reader. Create and edit read one bounded JSON object from a file or stdin, while duplicate overrides and the expected edit revision remain explicit flags that can't be hidden inside the object. Both commands resolve the Project through the shared slug-or-id resolver and leave target ownership, immutable finding identity, duplicate detection, optimistic concurrency, and Team capability enforcement with the server. Focused parser, handler, evidence-link, service-evidence, and structured-input modules keep those additions out of the size-ratcheted CLI entrypoint.

HTTP-profile commands use one grouped `http-profile` noun and the same active Project slug-or-id resolver. Create and update read one bounded JSON object from a file or stdin, reject unknown fields, accept only Secret-name references and relative workspace Files paths in protected slots, reject credential-bearing proxy URLs, and keep the optimistic revision in an explicit update flag. List and show call the existing capability-aware API routes, so viewers receive the public profile boundary and composition counts while Secret managers may receive reference names and Files paths in JSON. Human-readable output deliberately stays on public fields and counts. Delete always reads and renders that summary first and stays read-only without `--confirm`. No output can include Secret values because the profile service never reads them into its response model. Focused parser, handler, formatter, mutation, and protected-input modules keep the family out of the size-ratcheted CLI entrypoint.

Assessment lifecycle commands resolve a Project slug or stable id through the shared Project resolver and send only the route's profile/title or status fields. The server still owns profile-version snapshots, forward-only transitions, archived-only deletion, and Team capability checks. Deletion always reads and renders the existing server preview before the shared destructive-confirmation helper checks `--confirm`; confirmed JSON writes that preview to stderr so stdout remains one machine-readable result. The CLI doesn't submit a delete when the preview says the cycle isn't archived. Pending-cancellation errors retain every affected batch id so the handler can tell the operator to wait for terminal state and retry, while Team write denials identify the required Project-mutation capability.

**Private OAST preparation and launch.** Blind-XSS preview reuses the bounded Dalfox parameter choices, builds only the redacted app-owned callback command, and adds a fixed reservation window without resolving a token or contacting the provider. `services/assessments/assessment_oast.py` accepts only a fresh digest and the same opaque evidence ids, then creates one durable reservation through the existing owner- and check-scoped correlation service. Its ten-row recovery list never reveals callback identity; one exact status read reveals the callback only after the worker has staged its encrypted provider session. The separate launch confirmation rebuilds the plan, rechecks the exact owner, Project, cycle, check, action, provider scope, active window, and staged session, then passes a typed callback-bearing Dalfox command through the protected execution override. The validated carrier, response, logs, and audit details keep the redacted placeholder. A run-created hook rechecks readiness and binds the reservation to that exact run before its worker starts handling output; a failed bind terminates the new process and leaves the reservation unavailable or retryable according to its current guarded state. The OAST signal marker also prevents blind-XSS output from being mistaken for parameter-discovery evidence.
...
**Endpoint.** `GET /atlas/entities/export` is owner-scoped — it only returns entities owned by the current anonymous or kept personal workspace.
...
Migrations `0078` through `0083` establish pseudonymous identities, personal-workspace ownership, request-independent background authorization, the clean legacy cutover, and restricted browser sessions on both backends. `principals`, `personal_workspaces`, `credentials`, `credential_scopes`, and `credential_verifier_roots` keep the stable actor, workspace, access method, and PAT permissions separate. The storage service preserves an attached anonymous workspace's existing validated directory name, stores only keyed credential digests and safe metadata, encrypts versioned verifier roots through the existing vault key, and keeps credential rotation independent of ownership and filesystem paths. Migration `0080` renames product owner columns in place, including `runs.personal_workspace_id`, so SQLite `rowid` and the external-content FTS index remain intact. Migration `0081` adds principal and credential-attribution fields to durable definitions without turning credential ids into foreign-key ownership. The offline cutover tool supports both database backends: SQLite verifies row ids and FTS5 integrity, while Postgres holds the migration advisory lock and compares full run-id and known-substring result digests before and after conversion. A development-only Postgres discard path inventories other old credentials, accepts only isolated personal History, snapshot, preference, recent-value, and starred-command data plus non-owner Team memberships, and removes those rows in the same transaction as selected conversion without deleting files. Team snapshots require exact reviewed IDs and no links; Team recent values require an exact reviewed count. Both require an active selected-owned Team and active test membership, and the selected owner's entries remain outside the delete query. Team-scoped runs remain blocked, as do count drift and unhandled references. In both backends, the reviewed ownership conversion and migration `0082` commit in one transaction, and `0082` still rejects any unconverted legacy identity. Migration `0083` adds the encrypted browser-session signing keyring and revocable session rows. Authenticated requests resolve to the workspace owner, while workers rebuild authority from persisted principal and scope ids before executing.
...
Every app-owned persistent table in the default SQLite topology and its relationships, without field bodies. `PERSONAL_OWNER` is the shared `personal_workspace_id` value: a persisted workspace id for authenticated principals, or a validated anonymous UUID for an unupgraded browser workspace.
...
erDiagram
  ...
  PERSONAL_OWNER ||--o{ USER_WORKFLOWS : "saves"
  PERSONAL_OWNER ||--o{ RECENT_VALUES : "remembers"
  PERSONAL_OWNER ||--o{ SECRETS : "stores encrypted"
  PERSONAL_OWNER ||--o{ RUNS : "owns"
  PERSONAL_OWNER ||--o{ SNAPSHOTS : "owns"
  PERSONAL_OWNER ||--o{ RUN_FILE_ARTIFACTS : "tracks"
  PERSONAL_OWNER ||--o{ ENTITIES : "indexes"
  PERSONAL_OWNER ||--o{ FINDINGS : "captures"
  PERSONAL_OWNER ||--o{ ENTITY_LABELS : "labels"
  PERSONAL_OWNER ||--o{ ENTITY_NOTES : "notes"
  PERSONAL_OWNER ||--o{ PROJECTS : "owns"
  PERSONAL_OWNER ||--o{ PROJECT_ASSESSMENTS : "owns"
  PERSONAL_OWNER ||--o{ EVIDENCE_PACKAGES : "packages"
  PERSONAL_OWNER ||--o{ AUDIT_EVENTS : "records"
  PERSONAL_OWNER ||--o{ NOTIFICATION_CHANNELS : "sends"
  PERSONAL_OWNER ||--o{ NOTIFICATION_EVENTS : "queues"
...
- `principals` — stable pseudonymous actor records with active or disabled state, bounded reason metadata, and lifecycle timestamps. The central authentication resolver joins these rows to credentials and personal workspaces before returning an authenticated context.
- `personal_workspaces` — exactly one durable personal owner per principal. Each row stores one unique immutable `ws_<32 hex>` directory key, or the validated existing `sess_<32 hex>` key retained when an anonymous workspace is attached, instead of deriving a path from a credential.
...
- `session_preferences` — one row per personal workspace `(personal_workspace_id TEXT PRIMARY KEY, preferences TEXT, updated TEXT)`. Stores the normalized Options snapshot that follows a durable principal across credentials while still allowing browser-local anonymous UUID workspaces to keep independent defaults.
- `starred_commands` — one row per starred command per personal workspace `(personal_workspace_id, command)`. Backs the `/session/starred` endpoints without binding saved state to the credential used to reach it.
- `session_variables` — one row per personal-workspace command variable `(personal_workspace_id, name, value, updated)`. Backs the `var` built-in, `/session/variables`, and app-managed command expansion before validation.
- `user_workflows` — one row per saved workflow `(id, personal_workspace_id, team_id, definition_version, title, description, inputs, steps, created, updated)`. Backs the Workflows panel's **My workflows** section, the `workflow` terminal command, legacy migration, and shared team workflows.
...
- `workflow_execution_steps` — one ordered row per execution step with stable step id, unique linked run id, status, exit code, capture-name summary, selected transition/reason, and bounded error metadata. Fan-out parents keep their run id empty and close from the terminal child checkpoint, preserving the same transition contract without pretending one child run represents the collection.
- `workflow_execution_children` — one private, value-free row per bounded fan-out ordinal and attempt, with parent execution and step identity, optional linked run id, status, exit code, bounded error code, and lifecycle timestamps. A unique parent/step/ordinal/attempt key makes initialization idempotent, while a partial unique run-id index prevents one run from representing multiple child attempts. Retries append a new pending attempt instead of rewriting prior evidence; terminal failure limits turn pending and unbound launching rows into explicit skipped results without changing bound runs. Execution cancellation preserves terminal rows, marks unfinished attempts canceled, and returns only bound run ids for process signaling.
- `recent_values` — one row per recently used autocomplete value per personal/team scope `(personal_workspace_id, team_id, kind, value, last_used, use_count)`. `kind` is one of `domain`, `ip`, `url`, or `port_set`; each kind is capped independently at 10 entries. URL recents keep the scheme, host, and path but drop query strings and fragments before storage.
- `secrets` — one row per encrypted secret name per vault scope `(owner_id, name, ciphertext, nonce, consumer_envs, created_at, updated_at)`, with a unique `(owner_id, name)` binding so replacing a secret updates the existing row. Personal scopes use the personal-workspace id; team scopes use the team id. Storage also rejects attempts to bind the same consumer env name to two different secrets in one scope, keeping command-time lookup unambiguous. Values are AES-GCM ciphertext and are never returned by list routes or stored in transcripts. The wrapping key comes from `SECRETS_MASTER_KEY` or `<data_dir>/.secrets_master_key`, with a fixed HKDF-SHA256 app context deriving the key used for row encryption. When the key file is used, the app creates or repairs it with `0600` permissions.
- `projects` — one row per project/case folder. Stores personal-workspace ownership, optional `team_id` ownership for shared team projects, display metadata, status, timestamps, and personal/team-scoped slugs. Project notes are stored through `entity_notes` with `entity_type='project'`.
- `project_links` — generic project membership rows `(project_id, entity_type, entity_id)`. The app owns the valid entity vocabulary and link sources so projects can link completed runs and Atlas entities without copying source data. Atlas-entity links also carry target-list metadata such as source, confidence, review state, and source detail so the Projects modal can keep its target workflow without a separate target table. Reobserving a command-input target links its entity to the new run while carrying forward the existing Project review state, so discovery can't undo a person's confirmation. Run-owned file artifacts are intentionally reached through linked runs, while findings are reached through linked runs or linked Atlas entities instead of direct project links.
...
- `oast_correlations` — short-lived private OAST callback reservations bound to one owner, Project, active assessment check, confirmed target, reviewed intrusive action, and eventual source run. Rows retain a random label, exact allowed callback suffix, hashed service origin, bounded counts, status, actor context, and fixed active and purge deadlines without retaining provider credentials, URLs, or interaction bodies.
- `oast_interactions` — at most 64 redacted DNS, HTTP, SMTP, or LDAP summaries for one active OAST correlation. Each row keeps a deterministic fingerprint, optional hashed provider event id, observed and received times, bounded redaction counts, and an optional exact-target finding association. Duplicate fingerprints are rejected by the database, and deleting the parent correlation removes its summaries.
...
- `audit_events` — operational audit rows for destructive, sensitive, export, curation, identity, import, team-management, automation-definition, and notification-channel config events.
  - Session ids are stored as hashes, principal and credential lifecycle events use safe public ids, team/member actor fields are optional, client IP and user-agent text are bounded request metadata, details are allowlisted and bounded by the recorder, and retention is controlled separately from permalink retention.
  - History deletion, snapshots and redaction use, workspace file write/move/delete actions, project links, assessment lifecycle changes and manual check/evidence decisions, package/report builds, download tickets, Atlas entity suppression/deletion, Atlas import apply, explicit shared NVD and OSV advisory refreshes, finding review/suppression/deletion, secret lifecycle changes, principal and credential lifecycle changes, browser/API/terminal team-management flows, browser/API/terminal schedule/watch flows, and browser/API/terminal notification-channel config flows all write through this recorder.
  - Credential audit rows store only safe ids, types, states, and bounded reasons. Workspace file write/move rows are best-effort and store path, destination, count, and byte-size metadata without file contents; workspace file delete stays fail-closed.
...
The Access panel shows anonymous actions only for a valid anonymous identity, the invalid-credential removal action only when the saved credential needs attention, and credential-management actions only for a kept workspace. Its scoped `[hidden]` rule wins over the panel's flex layouts so hidden controls don't reappear visually or in the focus order. The Credentials section uses the Options section gap, credential rows add their own inset within the shared `.panel-row` border, and the existing-credential form separates its input from its action row.

The supported terminal root is `credential`. Status and list render safe metadata, while create, use, expiry, rotation, revocation, and recovery open the shared Access UI without accepting a secret in terminal text.

Team scope is request-local on top of the principal or anonymous workspace. `team_scope.js` stores the active team id under a key derived from the safe public credential id or anonymous UUID, renders the desktop HUD scope menu and mobile selector, and dispatches `app:scope-changed` when the user switches between Personal and a team. `apiFetch()` adds `X-Team-ID` when a team is active; direct API callers can use the same header or `team_id` query parameter. Team API payloads include the current member's granted capability names from `services/teams/capabilities.py`, and the browser Teams panel gates management controls from that server-provided list instead of duplicating the role matrix in JavaScript. The selected team detail panel also exposes owner/admin Activity rows through the team-scoped audit helper; operators and viewers do not see the Team Activity subtab and stay on the read-only team overview. Runs, snapshots, recent values, active-run metadata, history reads, Run Details, user workflows, Project ownership, team-owned run links, Project targets, finding review, entity labels/notes, Atlas read/query routes, cached Atlas intel refreshes, non-destructive Atlas suppression/link actions, schedules, watchers, AI assists, evidence-package build jobs, encrypted secrets, provider readiness, and API history/run/Atlas/AI routes use the shared request-scope helper so personal and team-owned rows stay separate.

Owner-scoped routes reject missing or invalid identities instead of reading or writing under an empty namespace. This includes the main browser project, session, history, workspace, secrets, notification, schedule, watcher, and Atlas paths.

`maskSessionToken(value)` remains a compatibility-named display helper. It returns only a safe `crd_...` prefix for portable credentials and an eight-character prefix for anonymous UUIDs; callers never pass the reusable secret into rendered status text.
...
- Prometheus counters, histograms, label normalizers, cardinality policies, and multiprocess registry setup live in `app/services/metrics/__init__.py`; Project assessment transition, evidence, action, parser, and connector metrics live in `app/services/metrics/assessments.py`; workflow outcome, duration, capture-failure, cancellation, and recovery metrics live in `app/services/metrics/workflows.py`; assessment-batch recovery, lifecycle, launch, rejection, and deferral metrics live in the `app/services/metrics/assessment_batch_*.py` modules; CVE risk feed, NVD advisory acquisition, and escalation metrics live in `app/services/metrics/cve_risk.py`; scrape-time collectors for active assessment cycles and batches, batch queue and retained outcomes, database, Postgres pool state, Redis, AI Redis coordination key pressure, broker mode, workspace, intel cache size, Atlas, findings, snapshots, AI queue health, and provider-secret health live in `app/services/metrics/collectors.py`
...
The container starts as root only long enough for `entrypoint.sh` to fix `/data` ownership after volume mount, stage the private configuration tree, normalize the optional workspace root, and set `/tmp` to `1777`. A validated connector-worker process role drops directly to `appuser` through `gosu` at that point. The default web role continues with scanner cache preparation and egress rules before it starts its enabled sibling workers and drops to the Gunicorn process.
...
- ordinary backend tests that need an empty current SQLite schema copy a process-scoped pristine database into a unique test path; the source is built once through the production `db_init()` path, backed up into a sidecar-free file, and checked for integrity, migration head, FTS, and the principal-only schema before it can be copied
- migration, schema reconciliation, startup, cutover, failure, and rollback tests use genuinely empty or purpose-built databases and the real initialization path so the faster ordinary fixture cannot bypass the behavior those tests qualify
...
- external CLI mutation cases pin bounded file and stdin input, exact request bodies, structured conflicts, role failures, stable output, idempotent evidence links, and matching SQLite/PostgreSQL write behavior; shared fake clients keep those contracts consistent while independently named command-family cases and parameterized outcome variants make failures easy to locate
...
- GitLab also requires a deterministic real-image smoke tier when packaged tools, image inputs, container helpers, command/workflow catalogs, or smoke fixtures change. It exercises startup, permissions, test-owned services, workflow capture, and a network-free help/version command for each packaged tool without retries.
