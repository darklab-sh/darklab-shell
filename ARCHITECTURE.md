...
HTTP-profile reads use one grouped `http-profile` noun and the same active Project slug-or-id resolver. List and show call the existing capability-aware API routes, so viewers receive the public profile boundary and composition counts while Secret managers may receive reference names and Files paths in JSON. Human-readable output deliberately stays on public fields and counts. Neither format can include Secret values because the profile service never reads them into its response model, and the focused parser, handler, and formatter keep this family out of the size-ratcheted CLI entrypoint.
