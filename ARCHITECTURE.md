...
Version 3 definitions add bounded collection captures and `for_each` policy to that compiled snapshot while keeping v1 and v2 scalar behavior unchanged. Collection launches fail closed before an execution is created unless the server can give every child its own normal policy and scope checks. Private `workflow_execution_children` rows reserve stable ordinal and attempt identity, run linkage, status, exit code, bounded error code, and timestamps without storing a collection value, rendered command, or free-form error detail. Initial rows and the parent checkpoint are created idempotently from the validated `for_each` step, and public execution payloads expose only the checkpoint-derived count summary. The shared run-finalization hook resolves a child link before the scalar step path, advances one child checkpoint exactly once, and emits or launches only the final parent transition.
