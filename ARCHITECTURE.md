...
These routes establish and manage pseudonymous principal credentials. Authenticated browser and product requests resolve ownership through the principal's personal workspace; the legacy session interface remains behind an explicit temporary adapter until its coordinated removal.
...
Owner-scoped SQL has a second, narrower boundary in `services.teams.ownership_queries`. Its adapters cover personal-only tables, team-only tables, team-capable tables, legacy token-keyed tables, composite owner keys, and actor attribution. A team-capable call must state whether a personal row uses `NULL`, an empty string, either representation, or no team filter; the call can't quietly add a team condition while presenting itself as a behavior-preserving refactor. The compatibility helpers in `services.teams.scope` delegate to these adapters and keep their existing SQL and parameters.
...
- The recorder stores hashed personal-workspace identity, optional principal, credential, and team/member actor fields, event and target types, request/job/correlation ids, bounded details, bounded client IP, bounded user-agent text, and timestamps. A legacy session hash remains populated only while the temporary adapter exists.
...
- `session_tokens` — temporary compatibility rows for the legacy named-session interface. They validate `tok_`-prefixed `X-Session-ID` headers and map that interface into the personal-owner boundary without becoming principal credentials or durable owner keys.
- `teams` — one row per team with a unique slug/name, status, creator references, timestamps, and archive/delete markers.
- `team_members` — one row per principal membership in a team, storing role, display name, status, joined/removed timestamps, safe joining-credential attribution, and temporary legacy token/hash fields for compatibility. Authorization and uniqueness use `(team_id, principal_id)` for principal members while preserving owner, admin, operator, and viewer capabilities.
