...
- `package_advisories` and `package_advisory_ranges` — shared schema reserved for explicit OSV advisory acquisition. The pure OSV parser can normalize a bounded full-record array and the correlation service can evaluate normalized records supplied by a caller, but neither service populates these tables. The current EPSS/KEV path doesn't query them, and viewing a finding never performs package-advisory lookups.
