...
Workflow execution is bounded by `workflow_active_execution_limit` per personal/team owner and `workflow_execution_max_runtime_seconds` per execution. Postgres takes an owner-scoped transaction advisory lock around the active-count claim and insert. Fan-out child claims lock the private child, parent step, and active execution together, recheck the parent policy's parallel-child ceiling, then move the same ordinal from pending to running in the parent checkpoint before committing. Each claimed child can bind one run exactly once; completion records a bounded result and advances that checkpoint once, while recovery can return only an unbound launching child to pending without changing its identity. A retryable failure closes its current attempt before creating the next bounded private row and returning that ordinal to pending. Terminal failures count by ordinal, and reaching the parent policy's limit moves pending and unbound launching rows into a durable skipped state without changing completed children or bound active runs. Before each step, the engine rechecks durable personal tokens plus the current team, initiating token, membership, and run capability. Create, list, detail, and cancel responses use one fixed public serializer that omits the definition snapshot, input and variable maps, workspace context, owner scope, actor context, and browser ownership hints for every role. Web startup follows an immutable `(created, id)` cursor through bounded pages of active execution rows after stale run metadata cleanup: unbound launch claims return to pending, completed linked runs advance from saved normalized output, live broker runs remain attached, and missing runs fail with bounded recovery metadata. A workflow-hook exception marks only the execution failed; the completed run remains saved.
