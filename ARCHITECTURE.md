...
**Session Entity Atlas surface.** `static/js/features/atlas/` owns the top-level Atlas overlay used from the desktop rail, mobile menu, `Alt+A`, History actions, Run Details, and project-filtered launches from Projects. The Atlas surface lists deduped session entities by type, opens an entity detail side sheet, refreshes app-native intel snapshots, links entities to the active project, and edits labels/notes through `ui_entity_metadata.js`. `static/css/features/atlas.css` keeps the surface on the same sheet/modal primitives as History, Projects, and Status Monitor.
