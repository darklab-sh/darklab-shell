...
| Method | Endpoint | Description |
| -------- | ---------- | ------------- |
| `POST` | `/auth/upgrade` | Atomically attaches the caller's validated anonymous workspace to a new principal and returns the first portable credential once. |
| `POST` | `/auth/redeem` | Validates a portable credential submitted in the JSON body and returns its safe authenticated context without retaining or echoing the secret. |
| `GET` | `/auth/context` | Returns the current authenticated principal, workspace, credential type, and capabilities; PAT callers need `identity:read`. |
...
Migrations `0078` through `0080` establish pseudonymous identities and personal-workspace ownership on both backends. `principals`, `personal_workspaces`, `credentials`, `credential_scopes`, and `credential_verifier_roots` keep the stable actor, workspace, access method, and PAT permissions separate. The storage service preserves an attached anonymous workspace's existing validated directory name, stores only keyed credential digests and safe metadata, encrypts versioned verifier roots through the existing vault key, and keeps credential rotation independent of ownership and filesystem paths. Migration `0080` renames product owner columns in place, including `runs.personal_workspace_id`, so SQLite `rowid` and the external-content FTS index remain intact. Authenticated requests resolve to the workspace owner while principal and credential ids remain bounded attribution.
...
**Server-side compatibility validation:** the central resolver validates `tok_`-prefixed legacy headers against `session_tokens` on every request and returns a typed authentication failure for a revoked or unknown token. API v1's temporary token adapter consumes that same typed result and uses a secret-free compatibility key for per-token limits instead of parsing or retaining the bearer value again. Canonical anonymous UUIDs remain the only unpersisted personal identities accepted by the adapter. A valid principal credential resolves product ownership to its `personal_workspace_id`; the credential secret and id are never converted into owner keys.
