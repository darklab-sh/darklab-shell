...
Modular classic-script frontend with a small committed asset build step. `index.html` is the HTML shell — no inline styles or scripts.

**Asset composition.** CSS is split across ordered source files under `static/css/`, with `styles.css` kept as a compatibility entrypoint. JavaScript stays in classic scripts under `static/js/`, grouped into coarse shell bundles plus a self-contained permalink bundle. Runtime templates render CSS and JavaScript through the manifest-backed `asset_bundle()` helper. Bundle mode is the default and emits committed content-hashed files from `app/static/build/`. Source mode remains available for local edit-and-refresh work and emits ordered source links from `app/static/build/manifest.json` with version query strings. `assets.config.json` owns bundle membership and order, `npm run assets:sync` regenerates the committed output, and `npm run assets:check` fails when the build output, source coverage, or bundle order drifts. In a cache-disabled local comparison against the pre-bundling checkout, the shell's initial app load dropped from 188 requests to 37 requests, and total response time fell from about 660 ms to about 330 ms.
...
**JS composition.** Logic is split across `static/js/` into focused classic scripts that keep plain `<script>` semantics in both source mode and bundled mode. Load order matters: the shared store lives in `state.js`, DOM-facing helpers live in `ui_helpers.js`, `app.js` provides shared browser helpers, feature modules under `static/js/features/` own larger user-facing surfaces, and `controller.js` loads last to perform the initialization and event wiring. JavaScript remains untranspiled.
