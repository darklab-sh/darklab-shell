...
Modular classic-script frontend with a small committed CSS build step. `index.html` is the HTML shell — no inline styles or scripts.

**CSS composition.** CSS is split across ordered source files under `static/css/`, with `styles.css` kept as a compatibility entrypoint. Runtime templates render CSS through the manifest-backed `asset_bundle()` helper. In source mode, Flask emits the ordered source links from `app/static/build/manifest.json` with version query strings for local edit-and-refresh work. In bundle mode, it emits committed content-hashed CSS files from `app/static/build/`. `assets.config.json` owns bundle membership and order, `npm run assets:sync` regenerates the committed output, and `npm run assets:check` fails when the build output drifts.
...
**JS composition.** Logic is split across `static/js/` into focused modules loaded via plain `<script src="...">` tags. Load order matters: the shared store lives in `state.js`, DOM-facing helpers live in `ui_helpers.js`, `app.js` provides shared browser helpers, feature modules under `static/js/features/` own larger user-facing surfaces, and `controller.js` loads last to perform the initialization and event wiring. JavaScript is still unbundled and untranspiled.
...
- the frontend remains a classic-script app; CSS has a small committed bundle pipeline, while `Vitest` still uses extraction helpers instead of converting runtime JavaScript to ES modules
