...
if [[ ! -f "$ROOT_DIR/package.json" || ! -d "$ROOT_DIR/app" ]]; then
  echo "record_demo_mobile.sh: could not locate the repository root" >&2
  exit 1
fi
...
stop_obs_recording() {
  if [ "$obs_recording_started" = "1" ]; then
    ...
    node "$ROOT_DIR/scripts/development/obs_recording.mjs" stop || true
  fi
}
...
if [ "$ARM_BEFORE_RECORDING" = "1" ]; then
  ...
  node "$ROOT_DIR/scripts/development/obs_recording.mjs" assert-idle

  echo "Starting OBS recording ..."
  node "$ROOT_DIR/scripts/development/obs_recording.mjs" start
  ...
  echo ""
  echo "Stopping OBS recording ..."
  node "$ROOT_DIR/scripts/development/obs_recording.mjs" stop
  obs_recording_started=0
  trap - EXIT INT TERM
  rm -f "$ARMING_FILE" "$ARM_READY_FILE"
